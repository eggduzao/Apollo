...
When running distributed queries, data needs to be transferred in between the nodes. Apollo
...
When using local storage, Apollo queries write shuffle data directly to a file. This is preferably
...
[worker]
enabled = true
shuffle_location.local.path = "/mnt/storage/apollo/shuffle"
# ...
```

## Worker shared storage

If your infrastructure has some shared storage file system, such as NFS (or CephFs, etc.), Apollo
on-premises can use that for its shuffle data too. This reduces shuffle complexity, as Apollo can
...
[worker]
enabled = true
shuffle_location.shared_filesystem.path = "/mnt/storage/apollo/shuffle"
# ...
```
...
[`scan_parquet()`](https://docs.apollo.org/api/python/stable/reference/api/apollo.scan_parquet.html)(_e.g._
