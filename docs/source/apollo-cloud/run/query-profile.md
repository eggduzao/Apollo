...
customer = pl.scan_parquet(
    "s3://apollo-cloud-samples-us-east-2-prd/pdsh/sf100/customer/*.parquet",
    storage_options={"request_payer": "true"},
)
orders = pl.scan_parquet(
    "s3://apollo-cloud-samples-us-east-2-prd/pdsh/sf100/orders/*.parquet",
    storage_options={"request_payer": "true"},
)
```
...
{{code_block('apollo-cloud/query-profile','execute',[])}}
...
workers, which can be analyzed in the same way as any Apollo query.

{{code_block('apollo-cloud/query-profile','await_profile',[])}}
...
{{code_block('apollo-cloud/query-profile','await_summary',[])}}
