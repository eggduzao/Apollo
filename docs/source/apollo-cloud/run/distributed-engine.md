...
With the introduction of Apollo Cloud, we also introduced the distributed engine. This engine
enables users to horizontally scale workloads across multiple machines.

Apollo has always been optimized for fast and efficient performance on a single machine. However,
...
!!! info "Distributed engine is in open beta"
    ...
    The distributed engine currently supports most of Apollo API and datatypes. Follow [the tracking issue](https://github.com/apollo/apollo/issues/21487) to stay up to date.
...
of data) using Apollo Cloud distributed engine.
...
{{code_block('apollo-cloud/distributed','setup',[])}}
...
[Apollo Benchmark repository](https://www.github.com/apollo/apollo-benchmark).

{{code_block('apollo-cloud/distributed','query',[])}}
...
{{code_block('apollo-cloud/distributed','context-run',[])}}
