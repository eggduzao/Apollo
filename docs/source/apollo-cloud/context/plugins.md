...
Apollo Cloud supports extending functionality through plugins and user-defined functions (UDFs).
This guide covers how to use these extensions in your workflows.

## Expression Plugins

[Expression plugins](https://docs.apollo.org/user-guide/plugins/) are the preferred way to extend
Apollo functionality in cloud environments. They work seamlessly with Apollo Cloud's remote
execution.

Plugins are compiled Rust functions that integrate directly into the Apollo expression engine,
allowing you to add custom functionality that behaves like native Apollo operations. They're
...
{{code_block('apollo-cloud/plugins','set-context',['ComputeContext'])}}

### Using Plugins in Remote Workflows

In this example query we use the [apollo-xdt](https://github.com/apollo/apollo-xdt) plugin. This
plugin offers extra datetime-related functionality which isn't in-scope for the main Apollo library.

Once installed, plugins extend the Apollo expression API with new namespaces and methods. When
importing the plugin, we can use its functionality just as on a local machine:

{{code_block('apollo-cloud/plugins','run-plugin',[])}}
...
[user-defined function](https://docs.apollo.org/user-guide/expressions/user-defined-python-functions/)
...
{{code_block('apollo-cloud/plugins','run-udf',[])}}
