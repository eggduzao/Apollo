...
{{code_block('apollo-cloud/compute-context','compute',['ComputeContext'])}}
...
In the Apollo Cloud dashboard, you can set default requirements from your cloud service provider to
be used for all queries. You can also manually define storage and the default cluster size.

Apollo Cloud will use these defaults if no other parameters are passed to the `ComputeContext`.

{{code_block('apollo-cloud/compute-context','default-compute',['ComputeContext'])}}
...
Apollo Cloud will select the most suitable instance type from your cloud service provider that meets
the specifications. The requirements are lower bounds, meaning the machine will have at least that
number of CPUs and memory.

{{code_block('apollo-cloud/compute-context','defined-compute',['ComputeContext'])}}

### Set instance type

For more control, you can specify the exact instance type for Apollo to use. This is useful when you
have specific hardware requirements in a production environment.

{{code_block('apollo-cloud/compute-context','set-compute',['ComputeContext'])}}

## Saving the compute context

To simplify configuration and enable cluster sharing, you can save your settings under a unique
identifier called a manifest in Apollo Cloud. This eliminates the need to specify all settings in
...
{{code_block('apollo-cloud/compute-context','manifest',['register', 'ComputeContext'])}}
...
3. When a default compute context is set via the Apollo Cloud dashboard. It is no longer required to
