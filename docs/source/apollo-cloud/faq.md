...
On this page you can find answers to some frequently asked questions around Apollo Cloud.

## Who is behind Apollo Cloud?

Apollo Cloud is built by the organization behind the open source Apollo project. We are committed to
improve Apollo open source for all single machine workloads. Apollo Cloud will extend Apollo
...
## Can you run Apollo Cloud on-premises?

Currently, Apollo Cloud is only available to organizations that are on AWS. Support for on-premises
infrastructure is on our roadmap and will become available soon.

## What does Apollo Cloud offer me beyond Apollo?

Apollo Cloud offers a managed service that enables scalable data processing with the flexibility and
expressiveness of the Apollo API. It extends the open source Apollo project with the following
capabilities:
...
## What are the main use cases for Apollo Cloud?

Apollo Cloud enables data teams to scale their existing Apollo queries beyond local machine
...
## How can Apollo Cloud integrate with my workflow?
...
Apollo Cloud is completely environment agnostic. This allows you to run your queries from anywhere
such as your own machine, Jupyter/Marimo notebooks, Airflow DAGs, AWS Lambda functions, or your
servers. By not tying you to a specific platform, Apollo Cloud gives you the flexibility to execute
your queries wherever it best fits your workflow.

## What is the pricing model of Apollo Cloud?

Apollo Cloud has a 'Pay-as-you-go' model. You pay a fixed price per vCPU per hour and only for the
resources you use for executing your queries. This is on top of AWS related costs for used compute
instances. Apollo Cloud scales down to zero if no queries run. Find more information on our
[Payment and Billing](organization/billing.md) page.

## Can I try Apollo Cloud for my use case?

Yes, Apollo Cloud offers a 30 day free trial. The 30 day free trial will start, when you connect
your first workspace to your cloud environment. During the trial period you can make full use of
Apollo Cloud features. See [the Trial page](organization/start-trial.md) for more details.

## Will the distributed engine be available in open source?

The distributed engine is only available in Apollo Cloud. There are no plans to make it available in
the open source project. Apollo is focused on single node compute, as it makes efficient use of the
available resources. Users already report utilizing Apollo to process hundreds of gigabytes of data
on single (large) compute instance. The distributed engine is geared towards teams and organizations
that are I/O bound or want to scale their Apollo queries beyond single machines.
