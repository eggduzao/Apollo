...
Execute Apollo Cloud queries remotely using Airflow workflows through secure credential management.
This section explains how to configure Airflow to submit and monitor Apollo Cloud workloads using
...
2. **Environment variables**
   ...
   instance `AIRFLOW_VAR_APOLLO_CLOUD_CLIENT_ID` and `AIRFLOW_VAR_APOLLO_CLOUD_CLIENT_SECRET`. They
   should then be available through the `airflow.models.Variable` API as
   `Variable.get("APOLLO_CLOUD_CLIENT_ID")`.
3. **Airflow `Variables`**
   ...
   `airflow variables set APOLLO_CLOUD_CLIENT_ID "<SECRET>"`.
...
def service_account_from_env(_):
    Variable.set("client_id", os.getenv("APOLLO_CLOUD_CLIENT_ID"))
    Variable.set("client_secret", os.getenv("APOLLO_CLOUD_CLIENT_SECRET"))
```
...
from apollo_cloud import ComputeContext, authenticate, set_compute_context
...
def taskflow():
    ...
    # authenticate to apollo cloud with the secrets created above
    authenticate(
