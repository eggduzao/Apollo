...
Configure Apollo Cloud authentication securely within Prefect workflows using native secret
management patterns. This section details how to integrate Apollo Cloud service account credentials
...
3. **`Block` system** ([docs](https://docs.prefect.io/v3/concepts/blocks)): Prefect defined a
   ...
   `prefect block create secret apollo-cloud-client-id` and retrieved from the code as
   `Secret.load("apollo-cloud-client-id").get()`.
...
def service_account_from_env(_):
    return {
        "client_id": os.getenv("APOLLO_CLOUD_CLIENT_ID"),
        "client_secret": os.getenv("APOLLO_CLOUD_CLIENT_SECRET"),
    }
```
