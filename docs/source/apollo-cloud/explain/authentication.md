...
Apollo Cloud allows authentication through short-lived authentication tokens. Authentication also
...
[API Reference documentation](https://docs.cloud.apollo.org/reference/auth/index.html).

## Credential resolution

Both methods will attempt to authenticate with the following priority:

1. `APOLLO_CLOUD_ACCESS_TOKEN` environment variable
2. `APOLLO_CLOUD_CLIENT_ID` / `APOLLO_CLOUD_CLIENT_SECRET` service account environment variables
...
After successful authentication, Apollo Cloud stores the token in your OS config directory:
...
You can override this path by setting the environment variable `APOLLO_CLOUD_ACCESS_TOKEN_PATH`.
