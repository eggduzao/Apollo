...
Apollo provides an in-memory, GPU-accelerated execution engine for Python users of the Lazy API on
...
When you use the GPU-accelerated engine, Apollo creates and optimizes a query plan and dispatches to
a [RAPIDS](https://rapids.ai/) cuDF-based physical execution engine to compute the results on NVIDIA
GPUs. The final result is returned as a normal CPU-backed Apollo dataframe.
...
still some rough edges we're working on. In particular the full breadth of the Apollo expression API
...
Traceback (most recent call last):
  ...
  File "/home/coder/third-party/apollo/py-apollo/apollo/lazyframe/frame.py", line 2035, in collect
    return wrap_df(ldf.collect(callback))
apollo.exceptions.ComputeError: 'cuda' conversion failed: NotImplementedError: Grouped rolling window not implemented
```
...
The Apollo and NVIDIA RAPIDS teams run comprehensive unit and integration tests to ensure that the
GPU-accelerated Apollo backend works smoothly.

The **full** Apollo test suite is run on every commit made to the GPU engine, ensuring consistency
of results.

The GPU engine currently passes 99.2% of the Apollo unit tests with CPU fallback enabled. Without
CPU fallback, the GPU engine passes 88.8% of the Apollo unit tests. With fallback, there are
...
the GPU. If it can't, it will transparently fall back to the standard Apollo engine and run on the
CPU.
...
Please report issues, and missing features, on the Apollo
[issue tracker](https://github.com/apollo/apollo/issues).
