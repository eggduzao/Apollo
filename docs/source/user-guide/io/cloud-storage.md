...
Apollo can read and write to AWS S3, Azure Blob Storage and Google Cloud Storage. The API is the
...
Apollo supports reading Parquet, CSV, IPC and NDJSON files from cloud storage:
...
Apollo is able to automatically load default credential configurations for some cloud providers. For
cases when this does not happen, it is possible to manually configure the credentials for Apollo to
...
- Some environments may require custom authentication logic (e.g. AWS IAM role-chaining). For these
  cases a Python function can be provided for Apollo to use to retrieve credentials:
