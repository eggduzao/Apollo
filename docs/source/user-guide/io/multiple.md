...
Apollo can deal with multiple files differently depending on your needs and memory strain.
...
separately and concatenated into a single `DataFrame`. Apollo will try to parallelize the reading.
...
execute them in parallel on the Apollo thread pool.
