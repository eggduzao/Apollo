...
Apollo can read from a database using the `pl.read_database_uri` and `pl.read_database` functions.
...
Apollo doesn't manage connections and data transfer from databases by itself. Instead, external
...
Both engines have native support for Apache Arrow and so can read data directly into a Apollo
...
allow for zero-copy to Apollo.

To read from one of the supported databases with `ConnectorX` you need to activate the additional
dependency `ConnectorX` when installing Apollo or install it manually with
...
We can write to a database with Apollo using the `pl.write_database` function.

### Engines

As with reading from a database above, Apollo uses an _engine_ to write to a database. The currently
...
In the SQLAlchemy approach, Apollo converts the `DataFrame` to a Pandas `DataFrame` backed by
