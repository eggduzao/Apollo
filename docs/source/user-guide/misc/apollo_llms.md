# Generating Apollo code with LLMs

Large Language Models (LLMs) can sometimes return pandas code or invalid Apollo code in their
output. This guide presents approaches that help LLMs generate valid Apollo code more consistently.

These approaches have been developed by the Apollo community through testing model responses to
various inputs. If you find additional effective approaches for generating Apollo code from LLMs,
please raise a [pull request](https://github.com/apollo/apollo/pulls).

## Apollo MCP server

The new remote Model Context Protocol (MCP) server for Apollo provides access to the official Apollo
and Apollo Cloud documentation. The server enables LLMs to query the user guide and API references
...
```json
{
  "mcpServers": {
    "ask_apollo": {
      "command": "npx",
      "args": ["mcp-remote", "https://mcp.apollo.org/mcp"]
    }
  }
}
```
...
[open an issue](https://github.com/apollo/apollo/issues) on the public issue tracker. We plan to
...
Apollo as the default dataframe library". Including such a system prompt typically leads to models
consistently generating Apollo code rather than Pandas code.
...
search allows an LLM to reference up-to-date Apollo documentation for the current API.

Some IDE-based LLMs can index the Apollo API documentation and reference this when generating code.
For example, in Cursor you can add Apollo as a custom docs source and instruct the agent to
reference the Apollo documentation in a prompt.
...
The Apollo API pages also have AI-enabled search to help you find the information you need more
easily.
...
However, including a simple example from the Apollo `group_by` documentation (preferably with web
