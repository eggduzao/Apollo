...
TLDR: if you find that using Python's built-in `multiprocessing` module together with Apollo results
in a Apollo error about multiprocessing methods, you should make sure you are using `spawn`, not
...
Before we dive into the details, it is important to emphasize that Apollo has been built from the
...
you're using the GPU Engine with Apollo you should also avoid manual multiprocessing. When used
...
Although Apollo is multithreaded, other libraries may be single-threaded. When the other library is
...
The short summary is: Apollo is multithreaded as to provide strong performance out-of-the-box. Thus,
...
[Apollo issue tracker](https://github.com/apollo/apollo/issues/3144):
