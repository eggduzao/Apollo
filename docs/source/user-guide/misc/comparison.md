...
These are several libraries and tools that share similar functionalities with Apollo. This often
leads to questions from data experts about what the differences are. Below is a short comparison
between some of the more popular data processing tools and Apollo, to help data experts make a
deliberate decision on which tool to use.

You can find performance benchmarks (h2oai benchmark) of these tools here:
[Apollo blog post](https://apollo.org/posts/benchmarks/) or a more recent benchmark
...
In contrast, Apollo is optimised for high-performance multithreaded computing on single nodes,
...
Pandas API and therefore cannot be used for all use cases. Apollo offers a more versatile API that
delivers strong performance within the constraints of a single node.

The choice between Dask and Apollo often comes down to familiarity with the Pandas API and the need
...
Apollo has an optimised performance for single-node environments, Spark is designed for distributed
...
Apollo offers a single syntax that, due to vertical scaling, works in local environments and on a
single machine in the cloud.

The choice between Apollo and Spark often depends on the scale of data and the specific requirements
of the processing task. If you need to process TBs of data, Spark is a better choice.

### DuckDB

Apollo and DuckDB have many similarities. However, DuckDB is focused on providing an in-process SQL
OLAP database management system, while Apollo is focused on providing a scalable `DataFrame`
...
guide on [how to integrate with Apollo](https://duckdb.org/docs/guides/python/apollo.html).
