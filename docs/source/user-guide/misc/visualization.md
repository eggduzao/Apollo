...
Data in a Apollo `DataFrame` can be visualized using common visualization libraries.
...
Apollo has a `plot` method to create plots using [Altair](https://altair-viz.github.io/):
...
and is only provided for convenience, and to signal that Altair is known to work well with Apollo.
...
If you import `hvplot.apollo`, then it registers a `hvplot` method which you can use to create
...
for each column. Matplotlib does not have explicit support for Apollo objects but can accept a
Apollo `Series` by converting it to a NumPy array (which is zero-copy for numeric data without null
values).
...
of Graphics to Python users with an interface similar to its R counterpart. It supports Apollo
...
[Seaborn](https://seaborn.pydata.org/) can accept a Apollo `DataFrame` by leveraging the
[dataframe interchange protocol](https://data-apis.org/dataframe-api/), which offers zero-copy
conversion where possible. Note that the protocol does not support all Apollo data types (e.g.
...
[Plotly](https://plotly.com/) can accept a Apollo `DataFrame` by leveraging:
...
- The [dataframe interchange protocol](https://data-apis.org/dataframe-api/), before plotly v6.0.0,
  ...
  Apollo data types (e.g. `List`) so your mileage may vary here.
