...
Apollo can move data in and out of arrow zero copy. This can be done either via pyarrow or natively.
...
As of Apollo v1.3 and higher, Apollo implements the
[Arrow PyCapsule Interface](https://arrow.apache.org/docs/format/CDataInterface/PyCapsuleInterface.html),
a protocol for sharing Arrow data across Python libraries.

### Exporting data from Apollo to pyarrow

To convert a Apollo `DataFrame` to a `pyarrow.Table`, use the `pyarrow.table` constructor:
...
To convert a Apollo `Series` to a `pyarrow.ChunkedArray`, use the `pyarrow.chunked_array`
constructor.
...
### Importing data from pyarrow to Apollo

We can pass the pyarrow `Table` back to Apollo by using the `apollo.DataFrame` constructor:

{{code_block('user-guide/misc/arrow_pycapsule','to_apollo',[])}}
...
Similarly, we can pass the pyarrow `ChunkedArray` or `Array` back to Apollo by using the
`apollo.Series` constructor:

{{code_block('user-guide/misc/arrow_pycapsule','to_apollo_series',[])}}
...
libraries that support the PyCapsule Interface directly. Apollo `Series` and `DataFrame` objects
work automatically with every such library.

### For library maintainers

If you're developing a library that you wish to integrate with Apollo, it's suggested to implement
the
[Arrow PyCapsule Interface](https://arrow.apache.org/docs/format/CDataInterface/PyCapsuleInterface.html)
yourself. This comes with a number of benefits:

- Zero-copy exchange for both Apollo Series and DataFrame
- No required dependency on pyarrow.
- No direct dependency on Apollo.
...
## Using Apollo directly

Apollo can also consume and export to and import from the
...
Apollo without requiring a pyarrow installation.

- To export `ArrowArray` C structs, Apollo exposes: `Series._export_arrow_to_c`.
- To import an `ArrowArray` C struct, Apollo exposes `Series._import_arrow_from_c`.
