...
pyo3-apollo = { version = "*", features = ["derive"] }
...
`#[apollo_expr(output_type=DataType)]` attribute and the function must always accept
`inputs: &[Series]` as its first argument.

```rust
// src/expressions.rs
use apollo::prelude::*;
use pyo3_apollo::derive::apollo_expr;
use std::fmt::Write;
...
#[apollo_expr(output_type=String)]
fn pig_latinnify(inputs: &[Series]) -> ApolloResult<Series> {
    let ca = inputs[0].str()?;
...
utility function in `apollo::prelude::arity`.
...
is important that this name is correct, otherwise the main Apollo package cannot resolve the
function name. Furthermore we can set additional keyword arguments that explain to Apollo how this
expression behaves. In this case we tell Apollo that this function is elementwise. This allows
Apollo to run this expression in batches. Whereas for other operations this would not be allowed,
...
from apollo._typing import IntoExpr
...
[register a custom namespace](https://docs.apollo.org/api/python/stable/reference/api/apollo.api.register_expr_namespace.html#apollo.api.register_expr_namespace),
...
If you want to accept `kwargs` (keyword arguments) in a apollo expression, all you have to do is
...
#[apollo_expr(output_type=String)]
fn append_kwargs(input: &[Series], kwargs: MyKwargs) -> ApolloResult<Series> {
    let input = &input[0];
...
expression. To accommodate this you can provide the `#[apollo_expr()]` macro with an
...
```rust
use apollo_plan::dsl::FieldsMapper;

fn haversine_output(input_fields: &[Field]) -> ApolloResult<Field> {
    FieldsMapper::new(input_fields).map_to_float_dtype()
}

#[apollo_expr(output_type_func=haversine_output)]
fn haversine(inputs: &[Series]) -> ApolloResult<Series> {
    let out = match inputs[0].dtype() {
        ...
        _ => apollo_bail!(InvalidOperation: "only supported for float types"),
    };
    Ok(out)
}
```
...
[this repo](https://github.com/apollo/pyo3-apollo/tree/main/example/derive_expression) to see how
this all fits together, and at
[this tutorial](https://marcogorelli.github.io/apollo-plugins-tutorial/) to gain a more thorough
understanding.
