...
register different file formats as sources to the Apollo engines. Because sources can move data zero
...
You want IO plugins if you have a source file not supported by Apollo and you want to benefit from
...
Every `scan` function in Apollo has to be able to provide the schema of the data it reads. For this
...
is the Apollo schema of the complete source file (independent of projection pushdown).
...
- `predicate`
  ...
  Apollo expression. The reader must filter their rows accordingly.
...
at our [plugins repository](https://github.com/apollo/pyo3-apollo/tree/main/example/io_plugin).
...
- [Rust example (distribution source)](https://github.com/apollo/pyo3-apollo/tree/main/example/io_plugin)
