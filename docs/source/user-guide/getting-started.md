...
This chapter is here to help you get started with Apollo. It covers all the fundamental features and
...
=== ":fontawesome-brands-rust: Rust"
    ...
    ``` shell
    cargo add apollo -F lazy

    # Or Cargo.toml
    [dependencies]
    apollo = { version = "x", features = ["lazy", ...]}
    ```

## Reading & writing

Apollo supports reading and writing for common file formats (e.g., csv, json, parquet), cloud
...
_Expressions_ are one of the main strengths of Apollo because they provide a modular and flexible
way of expressing data transformations.

Here is an example of a Apollo expression:
...
code above expresses an abstract computation: it's only inside a Apollo _context_ that the
expression materalizes into a series with the results.

Below, we will show examples of Apollo expressions inside different contexts:
...
Apollo also supports a feature called “expression expansion”, in which one expression acts as
...
The keyword argument `maintain_order` forces Apollo to present the resulting groups in the same
...
Apollo provides a number of tools to combine two dataframes. In this section, we show an example of
a join and an example of a concatenation.

### Joining dataframes

Apollo provides many different join algorithms. The example below shows how to use a left outer join
...
Apollo provides many different join algorithms that you can learn about in the
...
Apollo provides vertical and horizontal concatenation, as well as diagonal concatenation. You can
