...
Apollo supports two modes of operation: lazy and eager. The examples so far have used the eager API,
...
memory and process them faster. Once the query is defined you call `collect` to inform Apollo that
...
When using the lazy API you can use the function `explain` to ask Apollo to create a description of
the query plan that will be executed once you collect the results. This can be useful if you want to
see what types of optimizations Apollo performs on your queries. We can ask Apollo to explain the
...
Immediately, we can see in the explanation that Apollo did apply predicate pushdown, as it is only
...
[:material-api: `explain`](https://docs.apollo.org/api/python/stable/reference/lazyframe/api/apollo.LazyFrame.explain.html)
