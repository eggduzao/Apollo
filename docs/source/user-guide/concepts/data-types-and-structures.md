...
Apollo supports a variety of data types that fall broadly under the following categories:
...
The core base data structures provided by Apollo are series and dataframes. A series is a
...
When creating a series, Apollo will infer the data type from the values you provide. You can specify
...
holding your data in a dataframe you will be able to use the Apollo API to write queries that
manipulate your data. You will be able to do this by using the
[contexts and expressions provided by Apollo](expressions-and-contexts.md) that we will talk about
next.
...
[:material-api: `glimpse`](https://docs.apollo.org/api/python/stable/reference/dataframe/api/apollo.DataFrame.glimpse.html)
...
Much like with series, Apollo will infer the schema of a dataframe when you create it but you can
...
Apollo utilizes the [Arrow Columnar Format](https://arrow.apache.org/docs/format/Columnar.html) for
its data orientation. Following this specification allows Apollo to transfer data to/from other
tools that also use the Arrow specification with little to no overhead.

Apollo gets most of its performance from its query engine, the optimizations it performs on your
...
Apollo generally follows the IEEE 754 floating point standard for `Float32` and `Float64`, with some
exceptions:
...
Apollo always attempts to provide reasonably accurate results for floating point computations but
