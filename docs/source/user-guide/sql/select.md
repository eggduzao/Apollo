...
In Apollo SQL, the `SELECT` statement is used to retrieve data from a table into a `DataFrame`. The
basic syntax of a `SELECT` statement in Apollo SQL is as follows:
...
Apollo provides a wide range of SQL functions, including:
...
[API documentation](https://docs.rs/apollo-sql/latest/src/apollo_sql/keywords.rs.html). The example
...
Apollo also support directly reading from CSV, Parquet, JSON and IPC in your SQL query using table
functions `read_xxx`.
