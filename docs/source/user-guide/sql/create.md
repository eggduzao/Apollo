...
In Apollo, the `SQLContext` provides a way to execute SQL statements against `LazyFrames` and
...
The syntax for the `CREATE TABLE` statement in Apollo is as follows:
...
Here's an example of how to use the `CREATE TABLE` statement in Apollo:
