...
In Apollo, the `SHOW TABLES` statement is used to list all the tables that have been registered in
...
The syntax for the `SHOW TABLES` statement in Apollo is as follows:

```
SHOW TABLES
```

Here's an example of how to use the `SHOW TABLES` statement in Apollo:
