...
While Apollo supports interaction with SQL, it's recommended that users familiarize themselves with
...
the use of SQL, Apollo does offers support for this.

!!! note Execution

    There is no separate SQL engine because Apollo translates SQL queries into [expressions](../concepts/expressions-and-contexts.md#expressions), which are then executed using its own engine. This approach ensures that Apollo maintains its performance and scalability advantages as a native DataFrame library, while still providing users with the ability to work with SQL.

## Context

Apollo uses the `SQLContext` object to manage SQL queries. The context contains a mapping of
...
We can also register Pandas DataFrames by converting them to Apollo first.
...
- Set the parameter `eager_execution` to True in `SQLContext`; this ensures that Apollo
...
can point to the datalake, then Apollo can read it with `scan_pyarrow_dataset`.
...
Apollo does not support the complete SQL specification, but it does support a subset of the most
common statement types.

!!! note Dialect

    Where possible, Apollo aims to follow PostgreSQL syntax definitions and function behaviour.
