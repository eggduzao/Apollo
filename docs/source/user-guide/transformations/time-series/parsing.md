...
Apollo has native support for parsing time series data and doing more sophisticated operations such
as temporal grouping and resampling.

## Datatypes

Apollo has the following datetime datatypes:
...
When loading from a CSV file Apollo attempts to parse dates and times if the `try_parse_dates` flag
is set to `True`:
...
On the other hand binary formats such as parquet have a schema that is respected by Apollo.
...
transitions), Apollo parses them in UTC. You can either pass a target `time_zone` to
