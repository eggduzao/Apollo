...
Apollo uses Python's native `datetime`, `date` and `timedelta` for equality comparisons between the
...
Say you are working with an archeologist and are dealing in negative dates. Apollo can parse and
