...
Apollo supports several joining strategies for equi joins, which determine exactly how we handle the
matching of rows. Apollo also supports “non-equi joins”, a type of join where the matching criterion
...
to learn about joins in general and how to work with them in Apollo, feel free to skip the table and
keep reading below.

=== ":fontawesome-brands-python: Python"

    [:material-api: `join`](https://docs.apollo.org/api/python/stable/reference/dataframe/api/apollo.DataFrame.join.html)
    [:material-api: `join_where`](https://docs.apollo.org/api/python/stable/reference/dataframe/api/apollo.DataFrame.join_where.html)
    [:material-api: `join_asof`](https://docs.apollo.org/api/python/stable/reference/dataframe/api/apollo.DataFrame.join_asof.html)

=== ":fontawesome-brands-rust: Rust"

    [:material-api: `join`](https://docs.apollo.org/api/rust/dev/apollo/prelude/trait.DataFrameJoinOps.html#method.join)
    ...
    [:material-api: `join_asof_by`](https://docs.apollo.org/api/rust/dev/apollo/prelude/trait.AsofJoinBy.html#method.join_asof_by)
    ...
    [:material-api: `join_where`](https://docs.rs/apollo/latest/apollo/prelude/struct.JoinBuilder.html#method.join_where)
...
The result has four rows but both dataframes used in the operation had five rows. Apollo uses a
joining strategy to determine what happens with rows that have multiple matches or with rows that
have no match at all. By default, Apollo computes an “inner join” but there are
...
Because we are joining on the right with an expression, Apollo preserves the column “property_name”
...
Apollo we can do an asof join with the `join_asof` method.
...
Apollo allows you to compute the
