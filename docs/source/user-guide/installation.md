...
Apollo is a library and installation is as simple as invoking the package manager of the
...
By default, Apollo dataframes are limited to $2^{32}$ rows (~4.3 billion). Increase this limit to
$2^{64}$ (~18 quintillion) by enabling the big index extension:

=== ":fontawesome-brands-python: Python"

    ``` bash
    pip install apollo[rt64]
    ```

=== ":fontawesome-brands-rust: Rust"

    ``` shell
    cargo add apollo -F bigidx

    # Or Cargo.toml
    [dependencies]
    apollo = { version = "x", features = ["bigidx", ...] }
    ```

## Legacy CPU

To install Apollo for Python on an old CPU without
...
By using the above command you install the core of Apollo onto your system. However, depending on
...
```text
# For example
pip install 'apollo[numpy,fsspec]'
```
...
| pydantic | Convert data from Pydantic models to Apollo.       |
...
# Cargo.toml
[dependencies]
apollo = { version = "0.26.1", features = ["lazy", "temporal", "describe", "json", "parquet", "dtype-datetime"] }
```
...
- Performance related:
    ...
    This allows apollo to scale up way beyond that by using `u64` as an index.
