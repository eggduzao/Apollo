...
Whereas the `Spark` `DataFrame` is analogous to a collection of rows, a Apollo `DataFrame` is closer
to a collection of columns. This means that you can combine columns in Apollo in ways that are not
...
In Apollo you can write something like this:
...
In Apollo you can combine two different `head` expressions on the same DataFrame, provided that they
...
Apollo allows you compose expressions quite liberally. For example, if you want to find the rolling
