...
Here we set out the key points that anyone who has experience with pandas and wants to try Apollo
should know. We include both differences in the concepts the libraries are built on and differences
in how you should write Apollo code compared to pandas code.

## Differences in concepts between Apollo and pandas

### Apollo does not have a multi-index/index

pandas gives a label to each row with an index. Apollo does not use an index and each row is indexed
by its integer position in the table.

Apollo aims to have predictable results and readable queries, as such we think an index does not
...
In Apollo a DataFrame will always be a 2D table with heterogeneous data-types. The data-types may
...
Note that an 'index' data structure as known in databases will be used by Apollo as an optimization
technique.

### Apollo adheres to the Apache Arrow memory format to represent data in memory while pandas uses NumPy arrays

Apollo represents data in memory according to the Arrow memory spec while pandas by default
...
Apollo can convert data to NumPy format with the `to_numpy` method.

### Apollo has more support for parallel operations than pandas

Apollo exploits the strong support for concurrency in Rust to run many operations in parallel. While
some operations in pandas are multi-threaded the core of the library is single-threaded and an
additional library such as `Dask` must be used to parallelize operations. Apollo is faster than all
open source solutions that parallelize pandas code.

### Apollo has support for different engines

Apollo has native support for an engine optimized for in-memory processing and a streaming engine
optimized for large scale data processing. Furthermore Apollo has native integration with a CuDF
supported engine. All these engines benefit from Apollo' query optimizer and Apollo ensures semantic
...
### Apollo can lazily evaluate queries and apply query optimization
...
Apollo supports eager evaluation and lazy evaluation whereas pandas only supports eager evaluation.
The lazy evaluation mode is powerful because Apollo carries out automatic query optimization when it
...
Apollo is strict about data types. Data type resolution in Apollo is dependent on the operation
...
### Apollo has a more versatile API

Apollo is built on expressions and allows expression inputs in almost all operations. This means
that when you understand how expressions work, your knowledge in Apollo extrapolates. Pandas doesn't
have an expression system and often requires Python `lambda`s to express the complexity you want.
Apollo sees the requirement of a Python `lambda` as a lack of expressiveness of its API, and tries
...
If your Apollo code looks like it could be pandas code, it might run, but it likely runs slower than
it should.

Let's go through some typical pandas code and see how we might rewrite it in Apollo.

### Selecting data

As there is no index in Apollo there is no `.loc` or `iloc` method in Apollo - and there is also no
`SettingWithCopyWarning` in Apollo.

However, the best way to select data in Apollo is to use the expression API. For example, if you
...
but in Apollo you would use the `.select` method:

```python
df.select("a")
```

If you want to select rows based on the values then in Apollo you use the `.filter` method:

```python
df.filter(pl.col("a") < 10)
```

As noted in the section on expressions below, Apollo can run operations in `.select` and `filter` in
parallel and Apollo can carry out query optimization on the full set of data selection criteria.

### Be lazy

Working in lazy evaluation mode is straightforward and should be your default in Apollo as the lazy
mode allows Apollo to do query optimization.
...
In Apollo you can build this query in lazy mode with query optimization and evaluate it by replacing
the eager pandas function `read_csv` with the implicitly lazy Apollo function `scan_csv`:
...
Apollo optimizes this query by identifying that only the `id1` and `v1` columns are relevant and so
will only read these columns from the CSV. By calling the `.collect` method at the end of the second
line we instruct Apollo to eagerly evaluate the query.
...
However, in Apollo these transformations can be executed in parallel using expressions.
...
In Apollo we add columns to `df` using the `.with_columns` method:
...
Apollo can compute every branch of an `if -> then -> otherwise` in parallel. This is valuable, when
...
while in Apollo you call the `filter` method:
...
The query optimizer in Apollo can also detect if you write multiple filters separately and combine
...
In Apollo the same can be achieved with `window` functions:
...
Apollo will cache window expressions that are applied over the same group, so storing them in a
...
optional nullable arrays are used. In Apollo missing data corresponds to a `null` value for all data
types.

For float columns Apollo permits the use of `NaN` values. These `NaN` values are not considered to
...
the missing values (unless using optional nullable integer dtypes). In Apollo any missing values in
...
coding style to Apollo is unidiomatic and leads to suboptimal query plans.
...
If we do this in apollo, we would create 3 `with_columns` contexts, that forces Apollo to run the 3
pipes sequentially, utilizing zero parallelism.

The way to get similar abstractions in apollo is creating functions that create expressions. The
