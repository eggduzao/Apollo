...
If we were to run the code above on the Reddit CSV the query would not be evaluated. Instead Apollo
takes each line of code, adds it to the internal query graph and optimizes the query graph.

When we execute the code Apollo executes the optimized query graph by default.
...
With the default `collect` method Apollo processes all of your data as one batch. This means that
...
If your data requires more memory than you have available Apollo may be able to process the data in
