...
The schema of a Apollo `DataFrame` or `LazyFrame` sets out the names of the columns and their
...
One advantage of the lazy API is that Apollo will check the schema before any data is processed.
...
When we execute a lazy query Apollo checks for any potential `InvalidOperationError` before the
time-consuming step of actually processing the data in the pipeline.

## The lazy API must know the schema

In the lazy API the Apollo query optimizer must be able to infer the schema at every step of a query
