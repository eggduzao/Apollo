...
With the lazy API, Apollo doesn't run each query line-by-line but instead processes the full query
end-to-end. To get the most out of Apollo it is important that you use the lazy API because:

- the lazy API allows Apollo to apply automatic query optimization with the query optimizer
...
In this query we tell Apollo that we want to:
