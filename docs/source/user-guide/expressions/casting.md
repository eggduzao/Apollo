...
The function `cast` includes a parameter `strict` that determines how Apollo behaves when it
encounters a value that cannot be converted from the source data type to the target data type. The
default behaviour is `strict=True`, which means that Apollo will thrown an error to notify the user
...
to downcast to a data type with insufficient precision results in an error thrown by Apollo:
...
In case the column contains a non-numerical value, or a poorly formatted one, Apollo will throw an
