...
To help visualise this operation, you may imagine that Apollo selects the subsets of the data that
share the same value for the column “Type 1” and then computes the ranking expression only for those
values. Then, the results for that specific group are projected back to the original rows and Apollo
...
mapping the resulting values to the original rows, Apollo creates a new dataframe where values from
...
Because Apollo does not need to keep track of the positions of the rows of each group, using
