...
strings have unpredictable lengths. Apollo mitigates these inefficiencies by
...
Apollo offers multiple methods for checking and parsing elements of a string column, namely checking
...
Apollo relies on the Rust crate `regex` to work with regular expressions, so you may need to
[refer to the syntax documentation](https://docs.rs/regex/latest/regex/#syntax) to see what features
and flags are supported. In particular, note that the flavor of regex supported by Apollo is
...
Akin to the functions `extract` and `extract_all`, Apollo provides the functions `replace` and
...
Converting the casing of a string is a common operation and Apollo supports it out of the box with
...
Apollo provides five functions in the namespace `str` that let you strip characters from the ends of
the string:
...
the slice is not specified or if it's past the end of the string, Apollo slices the string all the
way to the end.
...
In addition to the examples covered above, Apollo offers various other string manipulation
functions. To explore these additional methods, you can go to the API documentation of your chosen
programming language for Apollo.
