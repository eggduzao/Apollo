...
Apollo has first-class support for two homogeneous container data types: `List` and `Array`. Apollo
...
In general, Apollo does not infer that a column has the data type `Array` for performance reasons,
and defaults to the appropriate variant of the data type `List`. In Python, an exception to this
rule is when you provide a NumPy array to build a column. In that case, Apollo has the guarantee
...
Apollo provides many functions to work with values of the data type `List` and these are grouped
inside the namespace `list`. We will explore this namespace a bit now.

!!! warning "`arr` then, `list` now"

    In previous versions of Apollo, the namespace for list operations used to be `arr`.
...
However, in Apollo we often do not need to do this to operate on the list elements.

### Operating on lists

Apollo provides several standard operations on columns with the `List` data type.
...
individually, and then you can use any Apollo expression on the element:
...
time to
[look at how to work with strings and regular expressions in Apollo](strings.md#check-for-the-existence-of-a-pattern).
...
Apollo does not provide a function to do this directly, but because expressions are so versatile we
...
As [we have seen above](#the-data-type-array), Apollo usually does not infer the data type `Array`
