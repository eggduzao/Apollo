...
This section of the user guide teaches how to work with missing data in Apollo.

## `null` and `NaN` values

In Apollo, missing data is represented by the value `null`. This missing value `null` is used for
all data types, including numerical types.

Apollo also supports the value `NaN` (“Not a Number”) for columns with floating point numbers. The
...
!!! info "Difference from pandas"
    ...
    In Apollo, missing data is always represented by the value `null`.

## Missing data metadata

Apollo keeps track of some metadata regarding the missing data of each series. This metadata allows
Apollo to answer some basic queries about missing values in a very efficient way, namely how many
...
Apollo uses something called a “validity bitmap” to know which values are missing in a series. The
...
is a cheap operation because the result is already known by Apollo.

??? info "Why does Apollo waste memory on a validity bitmap?"

    It all comes down to a tradeoff.
    By using a bit more memory per column, Apollo can be much more efficient when performing most operations on your columns.
    ...
    With the validity bitmap, Apollo knows automatically the positions where your operations can be applied.
...
- a Apollo expression, such as replacing with values computed from another column;
...
[the function `fill_null` replaces missing values with the corresponding values from the result of a Apollo expression](#fill-with-a-strategy-based-on-neighbouring-values),
as seen next.

### Fill with an expression

In the general case, the missing data can be filled by extracting the corresponding values from the
result of a general Apollo expression. For example, we can fill the second column with values taken
...
!!! info
    ...
    This does not happen in Apollo; instead, an exception is raised.

`NaN` values are considered to be a type of floating point data and are **not considered to be
missing data** in Apollo. This means:
...
Apollo has the functions `is_nan` and `fill_nan`, which work in a similar way to the functions
`is_null` and `fill_null`. Unlike with missing data, Apollo does not hold any metadata regarding the
