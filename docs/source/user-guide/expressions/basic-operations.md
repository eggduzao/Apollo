...
Apollo supports basic arithmetic between series of the same length, or between series and literals.
...
Apollo uses operator overloading to allow you to use your language's native arithmetic operators
...
Like with arithmetic operations, Apollo supports comparisons via the overloaded operators or named
functions:
...
Apollo has two functions to count the number of unique values in a series. The function `n_unique`
...
`value_counts`, that Apollo also provides:
...
Apollo supports something akin to a ternary operator through the function `when`, which is followed
...
cases, and for each given value, Apollo will only consider a replacement expression that is deeper
