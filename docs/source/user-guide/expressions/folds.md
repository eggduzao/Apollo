...
Apollo provides many expressions to perform computations across columns, like `sum_horizontal`,
`mean_horizontal`, and `min_horizontal`. However, these are just special cases of a general
algorithm called a fold, and Apollo provides a general mechanism for you to compute custom folds for
when the specialised versions of Apollo are not enough.
...
??? tip "`fold` in Python"
    ...
    Most programming languages include a higher-order function that implements the algorithm that the function `fold` in Apollo implements.
    The Apollo `fold` is very similar to Python's `functools.reduce`.
