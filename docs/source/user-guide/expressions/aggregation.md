...
The Apollo [context](../concepts/expressions-and-contexts.md#contexts) `group_by` lets you apply
...
!!! note
    ...
    These functions don't cost us anything because they return Apollo expressions, we don't apply a custom function over a series during runtime of the query.
...
use a `lambda` or a custom Python function to apply during a parallelized phase, Apollo' speed is
capped running Python code, preventing any multiple threads from executing the function.

Apollo will try to parallelize the computation of the aggregating functions over the groups, so it
is recommended that you avoid using `lambda`s and custom Python functions as much as possible.
Instead, try to stay within the realm of the Apollo expression API. This is not always possible,
