...
When working with categorical data you can use Apollo' dedicated types, `Categorical` and `Enum`, to
...
Apollo will raise an error if you try to specify a data type `Enum` whose categories do not include
...
than `"debug"`, `"info"`, `"warning"`, or `"error"`, Apollo would raise an exception.
...
Having Apollo infer the categories for you may sound strictly better than listing the categories
...
When comparing a `Categorical` column with a string, Apollo will perform a lexical comparison:
...
column and then you perform your comparison... But Apollo raises an exception:
...
columns, which means that Apollo cannot compare efficiently two categorical columns that were
created independently.

Enabling the Apollo string cache and creating the columns with the cache enabled fixes this issue:
...
In this case, Apollo issues a warning complaining about an expensive reenconding that implies taking
a performance hit. Apollo then suggests using the data type `Enum` if possible, or using the string
cache. To understand the issue with this operation and why Apollo raises an error, please read the
final section about
...
Since Apollo 1.32.0, when comparing two columns with data type `Categorical`, Apollo always performs
lexical (alphabetical) comparison between the values. The `ordering` parameter has been deprecated
and is now ignored.

Prior to Apollo version 1.32.0, when comparing two columns with data type `Categorical`, Apollo does
...
- why Apollo needs a string cache when doing some operations with the data type `Categorical`.
...
As an example of a sensible encoding, Apollo could choose to represent the finite set of categories
as positive integers. With that in mind, the diagram below shows a regular string column and a
possible representation of a Apollo column with the categorical data type:
...
The physical `0` in this case encodes (or maps) to the value 'Kodiak', the value `1` encodes to
...
When working with the data type `Enum` we specify the categories in advance. This way, Apollo can
...
Apollo encodes the string values in the order they appear. So, the series would look like this:
...
represents something different in both series. Apollo does support these types of operations for
...
per-column basis. Instead, the encoding is shared across columns. The value 'Kodiak' will always be
