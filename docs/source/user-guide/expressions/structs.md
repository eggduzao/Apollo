...
!!! note "Why `value_counts` returns a `Struct`"
    ...
    Apollo expressions always operate on a single series and return another series.
...
When building series or dataframes, Apollo will convert dictionaries to the data type `Struct`:
...
That's a pretty complex set of requirements done very elegantly in Apollo! To learn more about the
...
dataframe. There is no way of composing Apollo expressions to compute the Ackermann function[^1], so
