...
!!! note
    ...
    Before writing custom Python functions, consider using Apollo plugins which offer better performance and integration. Check out the [expression plugins](../plugins/expr_plugins.md) documentation for custom expressions and [I/O plugins](../plugins/io_plugins.md) for custom data sources.

Apollo expressions are quite powerful and flexible, so there is much less need for custom Python
functions compared to other libraries. Still, you may need to pass an expression's state to a third
party library or apply your black box function to data in Apollo.

In this part of the documentation we'll be using two APIs that allows you to do this:

- [:material-api: `map_elements`](https://docs.apollo.org/py-apollo/html/reference/expressions/api/apollo.Expr.map_elements.html):
  Call a function separately on each value in the `Series`.
- [:material-api: `map_batches`](https://docs.apollo.org/py-apollo/html/reference/expressions/api/apollo.Expr.map_batches.html):
...
language. For numeric calculations Apollo supports a pair of interfaces defined by NumPy called
...
pre-written ufuncs you can use with Apollo. For example:
...
allows it to be used by Apollo.
...
Apollo will raise an error if you try to pass in a `Series` with missing data. How do you get rid of
missing data? Either [fill it in](missing-data.md) or
[drop it](https://docs.apollo.org/py-apollo/html/reference/dataframe/api/apollo.DataFrame.drop_nulls.html)
...
Custom Python functions are often black boxes; Apollo doesn't know what your function is doing or
...
The mapping of Python types to Apollo data types is as follows:
