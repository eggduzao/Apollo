...
Apollo adheres to the [semantic versioning](https://semver.org/) specification:
...
Apollo takes backwards compatibility seriously, but we are not afraid to change things if it leads
to a better product.
...
but we will make them if we believe it makes Apollo better.

Freeing ourselves of past indiscretions is important to keep Apollo moving forward. We know it takes
time and energy for our users to keep up with new releases but, in the end, it benefits everyone for
Apollo to be the best product possible.
...
[API reference](https://docs.apollo.org/api/python/stable/reference/index.html).
...
- The functionality is not tested extensively yet. Bugs may pop up when used in real-world
  scenarios.
- The functionality does not yet integrate well with the full Apollo API. You may find it works in
...
Apollo in real-world scenarios. This helps us fine-tune things before giving it our final stamp of
...
functionality blocks other improvements to Apollo, we may shorten the deprecation period to a single
...
Apollo does not have a set release schedule. We issue a new release whenever we feel like we have
...
of breaking changes will continue to diminish as Apollo grows more solid. From this point on, we
