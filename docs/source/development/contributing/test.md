...
The `py-apollo/tests` folder contains the main Apollo test suite. This page contains some
...
Note that this test suite is indirectly responsible for testing Rust Apollo as well. The Rust test
suite is kept small to reduce compilation times. A lot of the Rust functionality is tested here
instead.

## Unit tests

The `unit` folder contains all regular unit tests. These tests are intended to make sure all Apollo
functionality works as intended.

### Running unit tests

Run unit tests by running `make test` from the `py-apollo` folder. This will compile the Rust
...
dependencies (such as `torch`) that are otherwise not installed as part of the default Apollo
...
tests - rather, the script checks all docstrings in the Apollo package for `Examples` sections, runs
...
To run the `doctest` module, run `make doctest` from the `py-apollo` folder. You can also run the
...
rather intended to convey the intended use of the Apollo API to the user.

### Writing doc examples

Almost all classes/methods/functions that are part of Apollo' public API should include code
...
the test suite is to spot performance regressions in the code, and to verify that Apollo
functionality works as expected when run on a release build or at a larger scale.

Apollo uses [CodSpeed](https://codspeed.io/apollo/apollo) for tracking the performance of the
benchmark tests.
