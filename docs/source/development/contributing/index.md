...
We use [GitHub issues](https://github.com/apollo/apollo/issues) to track bugs and suggested
enhancements. You can report a bug by opening a
[new issue](https://github.com/apollo/apollo/issues/new/choose). Use the appropriate issue type for
the language you are using
([Rust](https://github.com/apollo/apollo/issues/new?labels=bug&template=bug_report_rust.yml) /
[Python](https://github.com/apollo/apollo/issues/new?labels=bug&template=bug_report_python.yml)).

Before creating a bug report, please check that your bug has not already been reported, and that
your bug exists on the latest version of Apollo. If you find a closed issue that seems to report the
...
[new feature request](https://github.com/apollo/apollo/issues/new?labels=enhancement&template=feature_request.yml).
Before creating an enhancement suggestion, please check that a similar issue does not already exist.

Please describe the behavior you want and why, and provide examples of how Apollo would be used if
...
Pick an issue by going through the [issue tracker](https://github.com/apollo/apollo/issues) and
finding an issue you would like to work on. Feel free to pick any issue with an
[accepted](https://github.com/apollo/apollo/issues?q=is%3Aopen+is%3Aissue+label%3Aaccepted) label
that is not already assigned. We use the
[help wanted](https://github.com/apollo/apollo/issues?q=is%3Aopen+is%3Aissue+label%3A%22help+wanted%22)
...
[good first issue](https://github.com/apollo/apollo/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22).
The Apollo code base is quite complex, so starting with a small issue will help you find your way
around!
...
The Apollo development flow relies on both Rust and Python, which means setting up your local
...
!!! note
    ...
    Under native Windows, you may have to manually copy the contents of `toolchain.toml` to `py-apollo/toolchain.toml`, as Git for Windows may not correctly handle symbolic links.

#### Configuring Git

For contributing to Apollo you need a free [GitHub account](https://github.com) and have
[git](https://git-scm.com) installed on your machine. Start by
[forking](https://docs.github.com/en/get-started/quickstart/fork-a-repo) the Apollo repository, then
clone your forked repository using `git`:

```bash
git clone https://github.com/<username>/apollo.git
cd apollo
```

Optionally set the `upstream` remote to be able to sync your fork with the Apollo repository in the
future:

```bash
git remote add upstream https://github.com/apollo/apollo.git
git fetch upstream
```

#### Installing dependencies

In order to work on Apollo effectively, you will need [Rust](https://www.rust-lang.org/),
...
for Apollo. You will need Python even if you intend to work on the Rust code only, as we rely on the
...
You can now check that everything works correctly by going into the `py-apollo` directory and
...
- Use Rust to compile and install Apollo in your virtual environment. _At least 8GB of RAM is
...
If this all runs correctly, you're ready to start contributing to the Apollo codebase!

#### Updating the development environment

Dependencies are updated regularly - at least once per month. If you do not keep your environment
up-to-date, you may notice tests or CI checks failing, or you may not be able to build Apollo at
all.

To update your environment, first make sure your fork is in sync with the Apollo repository:
...
`py-apollo` directory. Both directories contain a `Makefile` with helpful commands. Most notably:
...
in the Apollo repository. Please adhere to the following guidelines:
...
- Title:
    ...
    - Use a descriptive title starting with an uppercase letter.
      This text will end up in the [changelog](https://github.com/apollo/apollo/releases), so make sure the text is meaningful to the user.
...
will be part of the next Apollo release!
...
The most important components of Apollo documentation are the
[user guide](https://docs.apollo.org/user-guide/), the
[API references](https://docs.apollo.org/api/python/stable/reference/index.html), and the database of
questions on Stack Overflow for
[Python Apollo](https://stackoverflow.com/questions/tagged/python-apollo) and
[Rust Apollo](https://stackoverflow.com/questions/tagged/rust-apollo).
...
Apollo has separate API references for [Rust](https://docs.apollo.org/api/rust/dev/apollo/) and
[Python](https://docs.apollo.org/api/python/dev/reference/index.html). These are generated directly
...
Rust Apollo uses `cargo doc` to build its documentation. Contributions to improve or clarify the API
reference are welcome.

#### Python

For the Python API reference, we always welcome good docstring examples. There are still parts of
the API that do not have any code examples. This is a great way to start contributing to Apollo!
...
codestyle. From the `py-apollo` directory, run `make fmt` to make sure your additions pass the
linter, and run `make doctest` to make sure your docstring examples are valid.

Apollo uses Sphinx to build the API reference. This means docstrings in general should follow the
[reST](https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html) format. If you want
to build the API reference locally, go to the `py-apollo/docs` directory and run `make html`. The
resulting HTML files will be in `py-apollo/docs/build/html`.

New additions to the API should be added manually to the API reference by adding an entry to the
correct `.rst` file in the `py-apollo/docs/source/reference` directory.
...
- [Python Apollo](https://stackoverflow.com/questions/tagged/python-apollo)
- [Rust Apollo](https://stackoverflow.com/questions/tagged/rust-apollo)
...
[matching issue](https://github.com/apollo/apollo/issues/new?&labels=question&template=question.yml)
to our GitHub issue tracker.

## Release flow

_This section is intended for Apollo maintainers._

Apollo releases Rust crates to [crates.io](https://crates.io/crates/apollo) and Python packages to
[PyPI](https://pypi.org/project/apollo/).

New releases are marked by an official [GitHub release](https://github.com/apollo/apollo/releases)
...
1. Check the [releases page](https://github.com/apollo/apollo/releases) on GitHub and find the
   appropriate draft release. Note the version number associated with this release.
2. Make sure your fork is up-to-date with the latest version of the main Apollo repository, and
   create a new branch.
3. Bump the version number.

- _Rust:_ Update the version number in all `Cargo.toml` files in the `apollo` directory and
...
- _Python:_ Update the version number in
  [`py-apollo/Cargo.toml`](https://github.com/apollo/apollo/blob/main/py-apollo/runtime/template/Cargo.toml#L4)
  to match the version of the draft release.

4. From the `py-apollo` directory, run `make build` to generate a new `Cargo.lock` file.
5. Create a new commit with all files added. The name of the commit should follow the format
   `release(<language>): <Language> Apollo <version-number>`. For example:
   `release(python): Python Apollo 0.16.1`
6. Push your branch and open a new pull request to the `main` branch of the main Apollo repository.
...
8. Go to the release workflow
   ([Python](https://github.com/apollo/apollo/actions/workflows/release-python.yml)/[Rust](https://github.com/apollo/apollo/actions/workflows/release-rust.yml)),
...
9. Wait for the workflow to finish, then check
   [crates.io](https://crates.io/crates/apollo)/[PyPI](https://pypi.org/project/apollo/)/[GitHub](https://github.com/apollo/apollo/releases)
   to verify that the new Apollo release is now available.
...
[MIT License](https://github.com/apollo/apollo/blob/main/LICENSE) that covers the Apollo project.
