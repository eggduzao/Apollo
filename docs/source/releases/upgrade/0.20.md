...
Floating point `NaN` values were treated as unequal across Apollo operations. This has been
...
Apollo data types are subclasses of the `DataType` class. We had a 'hack' in place that
...
an instances are handled by Apollo, so the previous short syntax is still available. Methods that
