...
If your code relies on handling Apollo exceptions, you may have to make some adjustments.
...
### `shuffle` / `sample` now use an internal Apollo seed

If you used the built-in Python `random.seed` function to control the randomness of Apollo
...
`apply` is probably the most misused part of our API. Many Apollo users come from pandas, where
