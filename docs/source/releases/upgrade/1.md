...
Apollo no longer inspects data types to infer the orientation of the data passed to the DataFrame
...
Traceback (most recent call last):
...
apollo.exceptions.ComputeError: conversion from `i64` to `u8` failed in column 'a' for 1 out of 3 values: [300]
```
...
Traceback (most recent call last):
...
apollo.exceptions.InvalidOperationError: conversion from `i64` to `u8` failed in column 'a' for 1 out of 3 values: [300]
```
...
Traceback (most recent call last):
...
apollo.exceptions.InvalidOperationError: conversion from `str` to `i64` failed in column 'literal' for 1 out of 1 values: ["a"]
...
Apollo will no longer forward-fill null values in `ewm` methods. The user can call `.forward_fill()`
...
Update conversion from Arrow to always convert Decimals into Apollo Decimals, rather than cast to
...
Traceback (most recent call last):
...
apollo.exceptions.ComputeError: get index is out of bounds
```
...
The `calamine` engine (available through the `fastexcel` package) has been added to Apollo
...
The only serialization format available for the `serialize/deserialize` methods on Apollo objects
...
Traceback (most recent call last):
...
apollo.exceptions.SQLInterfaceError: relation 'df1' was not found
```
...
We have a lot of type aliases defined in the `apollo.type_aliases` module. Some of these were
re-exported at the top-level and in the `apollo.datatypes` module. These re-exports have been
removed.

We plan on adding a public `apollo.typing` module in the future with a number of curated type
aliases. Until then, please define your own type aliases, or import from our `apollo.type_aliases`
...
```python
def foo(dtype: pl.ApolloDataType) -> None: ...
```

After:

```python
ApolloDataType = pl.DataType | type[pl.DataType]

def foo(dtype: ApolloDataType) -> None: ...
```
...
your Apollo installation to use the new extras.
...
```bash
pip install 'apollo[fastexcel,gevent,matplotlib]'
```

After:

```bash
pip install 'apollo[calamine,async,graph]'
```
