      ...
      { ... }:
      {
        ...
        perSystem =
          ...
          in
          {
            ...
            # packages.apollo =
            #   let
            #     project = builtins.fromTOML (builtins.readFile ./py-apollo/Cargo.toml);
            #   in
            ...
            devShells.default = pkgs.mkShell (
              let
                ...
                buildPy =
                  alias: cmd:
                  let
                    targetDir = "$WORKSPACE_ROOT/py-apollo/apollo";
                  in
                  ''
                    ${cmd}
                    mv "${targetDir}/apollo.abi3.so" "${targetDir}/apollo.abi3.so.${alias}"
                    ln -sf "${targetDir}/apollo.abi3.so.${alias}" "${targetDir}/apollo.abi3.so"
                  '';
                step = title: alias: ''
                  echo '[${title}]'
                  ${aliasToScript alias}
                ...
                aliases = rec {
                  ...
                  pyselect = {
                    ...
                    cmd = ''
                      ...
                      ln -sf "$WORKSPACE_ROOT/py-apollo/apollo/apollo.abi3.so.$1.latest" apollo/apollo.abi3.so
                    '';
                    doc = "Build the python library";
                  };
                  pybuild = {
                    pwd = "py-apollo";
                    cmd = buildPy "debug" "maturin develop -m $WORKSPACE_ROOT/py-apollo/Cargo.toml \"$@\"";
                    doc = "Build the python library";
                  };
                  pybuild-mindebug = {
                    pwd = "py-apollo";
                  ...
                  pyselect-build = {
                    ...
                    cmd = ''
                      ...
                      TARGET_DIR="$WORKSPACE_ROOT/py-apollo/apollo"
                      ln -sf "$TARGET_DIR/apollo.abi3.so.$1" "$TARGET_DIR/apollo.abi3.so"
                    '';
                    doc = "Select a previous build of apollo";
                  };
                  pytest-all = {
                    pwd = "py-apollo";
                  ...
                  pyfmt = {
                    pwd = "py-apollo";
                    cmd = ''
                      ruff check py-apollo
                      ruff format py-apollo
                      dprint fmt crates
                      typos crates
                      typos py-apollo
                    '';
              ...
              in
              {
                ...
                postVenvCreation = ''
                  ...
                  MATURIN_PEP517_ARGS="--profile dev" uv pip install \
                    -r py-apollo/requirements-ci.txt                 \
                    -r py-apollo/requirements-dev.txt                \
                    -r py-apollo/requirements-lint.txt               \
                    -r py-apollo/docs/requirements-docs.txt          \
                ...
                postShellHook =
                  ...
                  ''
                    ...
                    export APOLLO_DOT_SVG_VIEWER="${openCmd} %file%"
            ...
            packages.apollo =
              ...
              pkgs.python3Packages.buildPythonPackage {
                ...
                postInstall = ''
									# Move apollo.abi3.so -> apollo.so
									local apolloSo=""
									local soName=""
									while IFS= read -r -d "" p ; do
										apolloSo=$p
										soName="$(basename "$apolloSo")"
										[[ "$soName" == apollo.so ]] && break
									done < <( find "$out" -iname "apollo*.so" -print0 )
									[[ -z "''${apolloSo:-}" ]] && echo "apollo.so not found" >&2 && exit 1
									if [[ "$soName" != apollo.so ]] ; then
										mv "$apolloSo" "$(dirname "$apolloSo")/apollo.so"
									fi
								'';
            ...
            packages.default = self'.packages.apollo;
          };
      }
    );
}
