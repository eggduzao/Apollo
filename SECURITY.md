# Apollo security policy

Please disclose security vulnerabilities to security@apollo.tech. This address forwards to a small
...
- could potentially lead to arbitrary code execution on the machine running Apollo, or
...
but does trust the data source (whoever produced the database, Parquet file, etc). Apollo is similar
...
high we will create a fix and immediately publish a new point release of Apollo after publication,
with a security notice in the release notes. We currently do not have the resources to backport
fixes to previous versions of Apollo, or to facilitate privileged embargoes with distributors.
