...
use apollo_error::{ApolloError, ApolloResult, apollo_bail, apollo_ensure, apollo_err};
use apollo_utils::aliases::{InitHashMaps, PlIndexMap};
...
impl<Field, Metadata: Default> Schema<Field, Metadata> {
    ...
    pub fn from_iter_check_duplicates<I, F>(iter: I) -> ApolloResult<Self>
    where
        I: IntoIterator<Item = F>,
        F: Into<(PlSmallStr, Field)>,
    {
        Self::try_from_iter_check_duplicates(
            iter.into_iter().map(ApolloResult::Ok),
            |name: &str| apollo_err!(Duplicate: "duplicate name when building schema '{}'", &name),
        )
    }

    pub fn try_from_iter_check_duplicates<I, F, E>(iter: I, err_func: E) -> ApolloResult<Self>
    where
        I: IntoIterator<Item = ApolloResult<F>>,
        F: Into<(PlSmallStr, Field)>,
        E: Fn(&str) -> ApolloError,
    {
...
impl<Field, Metadata> Schema<Field, Metadata> {
    ...
    ///
    /// Returns:
    /// - If index is out of bounds, `Err(ApolloError)`
    ...
    /// Get a reference to the dtype of the field named `name`, or `Err(ApolloErr)` if the field doesn't exist.
    pub fn try_get(&self, name: &str) -> ApolloResult<&Field> {
        self.get(name)
            .ok_or_else(|| apollo_err!(SchemaFieldNotFound: "{}", name))
    }

    /// Get a mutable reference to the dtype of the field named `name`, or `Err(ApolloErr)` if the field doesn't exist.
    pub fn try_get_mut(&mut self, name: &str) -> ApolloResult<&mut Field> {
        self.fields
            .get_mut(name)
            .ok_or_else(|| apollo_err!(SchemaFieldNotFound: "{}", name))
    }
    ...
    /// Return all data about the field named `name`: its index in the schema, its name, and its dtype.
    ///
    /// Returns `Ok((index, &name, &dtype))` if the field exists, `Err(ApolloErr)` if it doesn't.
    pub fn try_get_full(&self, name: &str) -> ApolloResult<(usize, &PlSmallStr, &Field)> {
        self.fields
            .get_full(name)
            .ok_or_else(|| apollo_err!(SchemaFieldNotFound: "{}", name))
    }
    ...
    pub fn try_get_at_index(&self, index: usize) -> ApolloResult<(&PlSmallStr, &Field)> {
        self.fields.get_index(index).ok_or_else(|| apollo_err!(ComputeError: "index {index} out of bounds with 'schema' of len: {}", self.len()))
    }
    ...
    pub fn try_insert(&mut self, name: PlSmallStr, value: Field) -> ApolloResult<()> {
        if self.fields.contains_key(&name) {
            apollo_bail!(Duplicate: "column '{}' is duplicate", name)
        }
    ...
    pub fn try_index_of(&self, name: &str) -> ApolloResult<usize> {
    ...
    Metadata: Clone,
{
    ...
    ///
    /// Returns: `Ok(new_schema)` if `index <= self.len()`, else `Err(ApolloError)`
    ...
    pub fn try_project<I>(&self, columns: I) -> ApolloResult<Self>
    where
        I: IntoIterator,
        I::Item: AsRef<str>,
    {
        let fields = columns
            ...
            .map(|c| {
                ...
                ApolloResult::Ok((name.clone(), dtype.clone()))
            })
            .collect::<ApolloResult<PlIndexMap<PlSmallStr, _>>>()?;
        Ok(Self {
            fields,
            metadata: self.metadata().clone(),
        })
    ...
    pub fn try_project_indices(&self, indices: &[usize]) -> ApolloResult<Self> {
        let fields = indices
            .iter()
            .map(|&i| {
            ...
            .collect::<ApolloResult<PlIndexMap<_, _>>>()?;
