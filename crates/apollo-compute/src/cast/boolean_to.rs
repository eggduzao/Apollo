...
pub(super) fn boolean_to_primitive_dyn<T>(array: &dyn Array) -> ApolloResult<Box<dyn Array>>
where
...
pub(super) fn boolean_to_utf8view_dyn(array: &dyn Array) -> ApolloResult<Box<dyn Array>> {
    let array = array.as_any().downcast_ref().unwrap();
...
pub(super) fn boolean_to_binaryview_dyn(array: &dyn Array) -> ApolloResult<Box<dyn Array>> {
    let array = array.as_any().downcast_ref().unwrap();
