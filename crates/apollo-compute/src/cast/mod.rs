...
pub(super) fn cast_list_to_fixed_size_list<O: Offset>(
    ...
    size: usize,
    options: CastOptionsImpl,
) -> ApolloResult<FixedSizeListArray> {
    ...
    let new_values = if null_cnt == 0 {
        ...
        apollo_ensure!(is_valid, ComputeError: "not all elements have the specified width {size}");
        ...
        for i in 1..=list.len() {
            ...
            if list.is_null(i - 1) {
                ...
                apollo_ensure!(current_offset == expected_offset, ComputeError:
    ...
    .map_err(|_| apollo_err!(ComputeError: "not all elements have the specified width {size}"))
}

fn cast_list_uint8_to_binary<O: Offset>(list: &ListArray<O>) -> ApolloResult<BinaryViewArray> {
...
pub fn cast_default(array: &dyn Array, to_type: &ArrowDataType) -> ApolloResult<Box<dyn Array>> {
    cast(array, to_type, Default::default())
}

pub fn cast_unchecked(array: &dyn Array, to_type: &ArrowDataType) -> ApolloResult<Box<dyn Array>> {
...
pub fn cast(
    ...
    match (from_type, to_type) {
        ...
        (Struct(from_fd), Struct(to_fd)) => {
            apollo_ensure!(from_fd.len() == to_fd.len(), InvalidOperation: "Cannot cast struct with different number of fields.");
            cast_struct(array.as_any().downcast_ref().unwrap(), to_type, options).map(|x| x.boxed())
        },
        (Struct(_), _) | (_, Struct(_)) => apollo_bail!(InvalidOperation:
        ...
        (BinaryView, _) => match to_type {
            ...
            _ => apollo_bail!(InvalidOperation:
...
mod tests {
    ...
    fn cast_list_uint8_to_binary_errors_too_large_list() {
        ...
        assert!(matches!(
            err,
            ApolloError::InvalidOperation(msg)
