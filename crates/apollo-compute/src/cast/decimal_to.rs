...
pub(super) fn decimal_to_float_dyn<T>(from: &dyn Array) -> ApolloResult<Box<dyn Array>>
where
    T: NativeType + Float,
    f64: AsPrimitive<T>,
{
...
pub(super) fn decimal_to_integer_dyn<T>(from: &dyn Array) -> ApolloResult<Box<dyn Array>>
where
    T: NativeType + NumCast,
{
    let from = from.as_any().downcast_ref().unwrap();
