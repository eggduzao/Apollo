...
pub fn dec128_verify_prec_scale(p: usize, s: usize) -> ApolloResult<()> {
    apollo_ensure!((1..=DEC128_MAX_PREC).contains(&p), InvalidOperation: "precision must be between 1 and 38");
    apollo_ensure!(s <= p, InvalidOperation: "scale must be less than or equal to precision");
    Ok(())
}
