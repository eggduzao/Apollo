...
static APOLLO_KEYBOARD_INTERRUPT_STRING: &str = "__APOLLO_KEYBOARD_INTERRUPT";
...
fn is_keyboard_interrupt(p: &dyn Any) -> bool {
    if let Some(s) = p.downcast_ref::<&str>() {
        s.contains(APOLLO_KEYBOARD_INTERRUPT_STRING)
...
pub fn register_apollo_keyboard_interrupt_hook() {
...
fn try_raise_keyboard_interrupt_slow() {
    std::panic::panic_any(APOLLO_KEYBOARD_INTERRUPT_STRING);
}
