...
static ERROR_STRATEGY: LazyLock<ErrorStrategy> = LazyLock::new(|| {
    if env::var("APOLLO_PANIC_ON_ERR").as_deref() == Ok("1") {
        ErrorStrategy::Panic
    } else if env::var("APOLLO_BACKTRACE_IN_ERR").as_deref() == Ok("1") {
...
impl From<io::Error> for ApolloError {
...
#[cfg(feature = "regex")]
impl From<regex::Error> for ApolloError {
    fn from(err: regex::Error) -> Self {
        ApolloError::ComputeError(format!("regex error: {err}").into())
    }
}

#[cfg(feature = "object_store")]
impl From<object_store::Error> for ApolloError {
    fn from(err: object_store::Error) -> Self {
        if let object_store::Error::Generic { store, source } = &err {
            if let Some(apollo_err) = source.as_ref().downcast_ref::<ApolloError>() {
                return apollo_err.wrap_msg(|s| format!("{s} (store: {store})"));
            }
        }
...
impl From<avro_schema::error::Error> for ApolloError {
    fn from(value: avro_schema::error::Error) -> Self {
        apollo_err!(ComputeError: "avro-error: {}", value)
    }
}

impl From<simdutf8::basic::Utf8Error> for ApolloError {
    fn from(value: simdutf8::basic::Utf8Error) -> Self {
        apollo_err!(ComputeError: "invalid utf8: {}", value)
    }
}
#[cfg(feature = "arrow-format")]
impl From<arrow_format::ipc::planus::Error> for ApolloError {
    fn from(err: arrow_format::ipc::planus::Error) -> Self {
        apollo_err!(ComputeError: "parquet error: {err:?}")
    }
}

impl From<TryReserveError> for ApolloError {
    fn from(value: TryReserveError) -> Self {
        apollo_err!(ComputeError: "OOM: {}", value)
    }
}

impl From<Infallible> for ApolloError {
    ...
    pub fn context_trace(self) -> Self {
        ...
        match self {
            Context { error, msg } => {
                ...
                if !matches!(&*error, ApolloError::Context { .. }) {
                    return *error;
                }
                ...
                while let ApolloError::Context { msg, error } = current_error {
                    current_error = error;
                    messages.push(msg)
                }
...
pub fn map_err<E: Error>(error: E) -> ApolloError {
    ApolloError::ComputeError(format!("{error}").into())
}

#[macro_export]
macro_rules! apollo_err {
    ($variant:ident: $fmt:literal $(, $arg:expr)* $(,)?) => {
        $crate::__private::must_use(
            $crate::ApolloError::$variant(format!($fmt, $($arg),*).into())
        )
    };
    ($variant:ident: $fmt:literal $(, $arg:expr)*, hint = $hint:literal) => {
        $crate::__private::must_use(
            $crate::ApolloError::$variant(format!(concat_str!($fmt, "\n\nHint: ", $hint), $($arg),*).into())
        )
    };
    ($variant:ident: $err:expr $(,)?) => {
        $crate::__private::must_use(
            $crate::ApolloError::$variant($err.into())
        )
    };
    (expr = $expr:expr, $variant:ident: $err:expr $(,)?) => {
        $crate::__private::must_use(
            $crate::ApolloError::$variant(
                format!("{}\n\nError originated in expression: '{:?}'", $err, $expr).into()
            )
        )
    };
    (expr = $expr:expr, $variant:ident: $fmt:literal, $($arg:tt)+) => {
        apollo_err!(expr = $expr, $variant: format!($fmt, $($arg)+))
    };
    ...
    (oos = $($tt:tt)+) => {
        $crate::apollo_err!(ComputeError: "out-of-spec: {}", $($tt)+)
    };
    (nyi = $($tt:tt)+) => {
        $crate::apollo_err!(ComputeError: "not yet implemented: {}", format!($($tt)+) )
    };
    (opq = $op:ident, $arg:expr) => {
        $crate::apollo_err!(op = concat!("`", stringify!($op), "`"), $arg)
    };
    (opq = $op:ident, $lhs:expr, $rhs:expr) => {
        $crate::apollo_err!(op = stringify!($op), $lhs, $rhs)
    };
    (bigidx, ctx = $ctx:expr, size = $size:expr) => {
        $crate::apollo_err!(ComputeError: "\
{} produces {} rows which is more than maximum allowed pow(2, 32)-1 rows; \
consider compiling with bigidx feature (pip install apollo[rt64])",
            $ctx,
            $size,
        )
    };
    (append) => {
        apollo_err!(SchemaMismatch: "cannot append series, data types don't match")
    };
    (extend) => {
        apollo_err!(SchemaMismatch: "cannot extend series, data types don't match")
    };
    (unpack) => {
        apollo_err!(SchemaMismatch: "cannot unpack series, data types don't match")
    };
    (not_in_enum,value=$value:expr,categories=$categories:expr) =>{
        apollo_err!(ComputeError: "value '{}' is not present in Enum: {:?}",$value,$categories)
    };
    (string_cache_mismatch) => {
        apollo_err!(StringCacheMismatch: r#"
...
on startup."#.trim_start())
    ...
    (duplicate = $name:expr) => {
        $crate::apollo_err!(Duplicate: "column with name '{}' has more than one occurrence", $name)
    };
    (duplicate_field = $name:expr) => {
        $crate::apollo_err!(Duplicate: "multiple fields with name '{}' found", $name)
    };
    (col_not_found = $name:expr) => {
        $crate::apollo_err!(ColumnNotFound: "{:?} not found", $name)
    };
    ...
    (oob = $idx:expr, $len:expr) => {
        apollo_err!(OutOfBounds: "index {} is out of bounds for sequence of length {}", $idx, $len)
    };
    ...
    (invalid_element_use) => {
        $crate::apollo_err!(InvalidOperation: "`element` is not allowed in this context")
    };
    (invalid_field_use) => {
        $crate::apollo_err!(InvalidOperation: "`field` is not allowed in this context")
    };
    (non_utf8_path) => {
        $crate::apollo_err!(ComputeError: "encountered non UTF-8 path characters")
    };
    ...
    (item_agg_count_not_one = $n:expr, allow_empty = $allow_empty:expr) => {
        if $n == 0 && !$allow_empty {
            apollo_err!(ComputeError:
                "aggregation 'item' expected a single value, got none"
            )
        } else if $n > 100 {
            if $allow_empty {
                apollo_err!(ComputeError: "aggregation 'item' expected no or a single value, got 100+ values")
            } else {
                apollo_err!(ComputeError: "aggregation 'item' expected a single value, got 100+ values")
            }
        } else if $n > 1 {
            if $allow_empty {
                apollo_err!(ComputeError:
...
#[macro_export]
macro_rules! apollo_bail {
    ($($tt:tt)+) => {
        return Err($crate::apollo_err!($($tt)+))
    };
}

#[macro_export]
macro_rules! apollo_ensure {
    ($cond:expr, $($tt:tt)+) => {
        if !$cond {
            $crate::apollo_bail!($($tt)+);
        }
    };
}
...
#[cold]
#[must_use]
pub fn to_compute_err(err: impl Display) -> ApolloError {
    ApolloError::ComputeError(err.to_string().into())
}
...
pub mod __private {
    ...
    #[cold]
    #[must_use]
    pub fn must_use(error: crate::ApolloError) -> crate::ApolloError {
        error
    }
}
