...
pub fn _limit_path_len_io_err(path: &Path, err: io::Error) -> ApolloError {
    let path = path.to_string_lossy();
    let msg = if path.len() > 88 && !verbose() {
        let truncated_path: String = path.chars().skip(path.len() - 88).collect();
        format!("{err}: ...{truncated_path} (set APOLLO_VERBOSE=1 to see full path)")
    } else {
        format!("{err}: {path}")
    };
...
pub fn open_file(path: &Path) -> ApolloResult<File> {
    File::open(path).map_err(|err| _limit_path_len_io_err(path, err))
}

pub fn open_file_write(path: &Path) -> ApolloResult<File> {
...
pub fn create_file(path: &Path) -> ApolloResult<File> {
