...
use apollo_error::apollo_bail;
use rayon::{ThreadPool, ThreadPoolBuilder};

use crate::mem::PAGE_SIZE;

pub static UNMAP_POOL: LazyLock<ThreadPool> = LazyLock::new(|| {
    let thread_name = std::env::var("APOLLO_THREAD_NAME").unwrap_or_else(|_| "apollo".to_string());
...
impl MMapSemaphore {
    ...
    pub fn new_from_file(file: &File) -> ApolloResult<MMapSemaphore> {
...
pub fn ensure_not_mapped(
    ...
    #[cfg(target_family = "unix")]
    {
        ...
        if guard.contains_key(&(file_md.dev(), file_md.ino())) {
            apollo_bail!(ComputeError: "cannot write to file: already memory mapped");
        }
    }
    Ok(())
}
