//! Centralized Apollo serialization entry.
//!
...
use apollo_error::{ApolloResult, to_compute_err};
...
fn serialize_impl<W, T, const FC: bool>(mut writer: W, value: &T) -> ApolloResult<()>
where
    W: std::io::Write,
...
pub fn deserialize_impl<T, R, const FC: bool>(mut reader: R) -> ApolloResult<T>
where
...
impl SerializeOptions {
    ...
    pub fn deserialize_from_reader<T, R, const FC: bool>(&self, reader: R) -> ApolloResult<T>
    where
        T: serde::de::DeserializeOwned,
        R: std::io::Read,
    {
    ...
    pub fn serialize_to_bytes<T, const FC: bool>(&self, value: &T) -> ApolloResult<Vec<u8>>
    where
        T: serde::ser::Serialize,
    {
        let mut v = vec![];
...
pub fn serialize_into_writer<W, T, const FC: bool>(writer: W, value: &T) -> ApolloResult<()>
where
    W: std::io::Write,
    T: serde::ser::Serialize,
{
...
pub fn deserialize_from_reader<T, R, const FC: bool>(reader: R) -> ApolloResult<T>
where
    T: serde::de::DeserializeOwned,
    R: std::io::Read,
{
...
pub fn serialize_to_bytes<T, const FC: bool>(value: &T) -> ApolloResult<Vec<u8>>
where
    T: serde::ser::Serialize,
{
    let mut v = vec![];
...
pub fn serialize_dsl<W, T>(writer: W, value: &T) -> ApolloResult<()>
where
    W: std::io::Write,
    T: serde::ser::Serialize,
{
...
pub fn deserialize_dsl<T, R>(reader: R) -> ApolloResult<T>
where
    T: serde::de::DeserializeOwned,
    R: std::io::Read,
{
...
#[cfg(feature = "python")]
pub fn python_object_deserialize(buf: &[u8]) -> ApolloResult<pyo3::Py<pyo3::PyAny>> {
