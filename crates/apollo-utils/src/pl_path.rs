...
impl PlPath {
    ...
    fn _try_from_path(path: &Path) -> ApolloResult<&PlPath> {
        path.to_str()
            .ok_or_else(|| apollo_err!(non_utf8_path))
            .map(Self::_new)
    }
    ...
    pub fn to_absolute_path(&self) -> ApolloResult<PlRefPath> {
...
impl PlRefPath {
    ...
    pub fn try_from_path(path: &Path) -> ApolloResult<PlRefPath> {
        Ok(Self::new(PlPath::_try_from_path(path)?.as_str()))
    }

    pub fn try_from_pathbuf(path: PathBuf) -> ApolloResult<PlRefPath> {
    ...
    pub fn to_absolute_path(&self) -> ApolloResult<Cow<'_, PlRefPath>> {
