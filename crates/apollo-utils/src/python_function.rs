...
mod _serde_impls {
    ...
    impl TrySerializeToBytes for PythonObject {
        fn try_serialize_to_bytes(&self) -> apollo_error::ApolloResult<Vec<u8>> {
            let mut buf = Vec::new();
            crate::pl_serialize::python_object_serialize(&self.0, &mut buf)?;
            Ok(buf)
        }

        fn try_deserialize_bytes(bytes: &[u8]) -> apollo_error::ApolloResult<Self> {
...
mod serde_wrap {
    ...
    pub trait TrySerializeToBytes: Sized {
        fn try_serialize_to_bytes(&self) -> ApolloResult<Vec<u8>>;
        fn try_deserialize_bytes(bytes: &[u8]) -> ApolloResult<Self>;
    }
