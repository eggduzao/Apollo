...
pub mod prefetch {
    ...
    pub fn get_memory_prefetch_func(verbose: bool) -> fn(&[u8]) -> () {
        let memory_prefetch_func = match std::env::var("APOLLO_MEMORY_PREFETCH").ok().as_deref() {
            None => {
            ...
            Some("madvise_sequential") => {
                ...
                {
                    panic!(
                        "APOLLO_MEMORY_PREFETCH=madvise_sequential is not supported by this system"
                    );
                }
            },
            Some("madvise_willneed") => {
                ...
                {
                    panic!(
                        "APOLLO_MEMORY_PREFETCH=madvise_willneed is not supported by this system"
                    );
                }
            },
            Some("madvise_populate_read") => {
                ...
                {
                    panic!(
                        "APOLLO_MEMORY_PREFETCH=madvise_populate_read is not supported by this system"
                    );
                }
            },
            Some("force_populate_read") => force_populate_read,
            Some(v) => panic!("invalid value for APOLLO_MEMORY_PREFETCH: {v}"),
        };
