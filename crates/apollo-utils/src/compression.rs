...
pub trait CompressionLevel<T: std::fmt::Display + std::cmp::PartialOrd> {
    ...
    fn is_valid_level(level: T) -> ApolloResult<()> {
        let compression_range = Self::MINIMUM_LEVEL..=Self::MAXIMUM_LEVEL;
        if compression_range.contains(&level) {
            Ok(())
        } else {
            apollo_bail!(InvalidOperation: "valid compression range {}..={} exceeded.",
...
impl BrotliLevel {
    ...
    pub fn try_new(level: u32) -> ApolloResult<Self> {
...
impl GzipLevel {
    ...
    pub fn try_new(level: u8) -> ApolloResult<Self> {
        Self::is_valid_level(level).map(|_| Self(level))
    }
...
impl ZstdLevel {
    ...
    pub fn try_new(level: i32) -> ApolloResult<Self> {
        Self::is_valid_level(level).map(|_| Self(level))
    }
