...
use apollo_error::{ApolloResult, apollo_bail, apollo_ensure};
...
impl CategoricalPhysical {
    ...
    pub fn smallest_physical(num_cats: usize) -> ApolloResult<Self> {
        if num_cats <= u8::MAX as usize {
            ...
            apollo_bail!(ComputeError: "attempted to insert more categories than the maximum allowed")
        }
    }
...
impl FrozenCategories {
    ...
    pub fn new<'a, I: IntoIterator<Item = &'a str>>(strings: I) -> ApolloResult<Arc<Self>> {
        ...
        for s in strings {
            ...
            apollo_ensure!(mapping.len() == builder.len(), ComputeError: "FrozenCategories must contain unique strings; found duplicate '{s}'");
        }
...
pub fn ensure_same_categories(left: &Arc<Categories>, right: &Arc<Categories>) -> ApolloResult<()> {
    if Arc::ptr_eq(left, right) {
        return Ok(());
    }

    if left.name() != right.name() {
        apollo_bail!(SchemaMismatch: "Categories name mismatch, left: '{}', right: '{}'.
...
Operations mixing different Categories are often not supported, you may have to cast.", left.name(), right.name())
        ...
        apollo_bail!(SchemaMismatch: "Categories have same name ('{}'), but have a mismatch in namespace, left: {}, right: {}.
...
Operations mixing different Categories are often not supported, you may have to cast.", left.name(), left.namespace(), right.namespace())
        ...
        apollo_bail!(SchemaMismatch: "Categories have same name and namespace ('{}', {}), but have a mismatch in dtype, left: {}, right: {}.
...
Operations mixing different Categories are often not supported, you may have to cast.", left.name(), left.namespace(), left.physical().as_str(), right.physical().as_str())
        ...
        apollo_bail!(SchemaMismatch: "Categories which should be equal have different backing objects.

This is a known problem when combining Apollo with multiprocessing using fork().")
    }
}
...
pub fn ensure_same_frozen_categories(
    ...
    apollo_bail!(SchemaMismatch: r#"Enum mismatch.
