...
use apollo_core::prelude::{IDX_DTYPE, IdxCa, InitHashMaps, PlHashMap, PlIndexMap, PlIndexSet};
use apollo_core::schema::Schema;
use apollo_error::apollo_warn;
use apollo_expr::{ExpressionConversionState, create_physical_expr};
use apollo_io::predicates::ScanIOPredicate;
use apollo_plan::dsl::default_values::{
...
use apollo_plan::plans::hive::HivePartitionsDf;
use apollo_plan::plans::predicates::{aexpr_to_column_predicates, aexpr_to_skip_batch_predicate};
use apollo_plan::plans::{AExpr, ExprIRDisplay, FileInfo, IR, MintermIter};
use apollo_plan::utils::aexpr_to_leaf_names_iter;
...
pub fn create_scan_predicate(
    ...
    if create_skip_batch_predicate {
        if let Some(node) = aexpr_to_skip_batch_predicate(predicate.node(), expr_arena, schema) {
            ...
            if std::env::var("APOLLO_OUTPUT_SKIP_BATCH_PRED").as_deref() == Ok("1") {
    ...
    let column_predicates = if create_column_predicates {
        ...
        if std::env::var("APOLLO_OUTPUT_COLUMN_PREDS").as_deref() == Ok("1") {
        ...
        PhysicalColumnPredicates {
            predicates: column_predicates
                ...
                .collect::<ApolloResult<PlHashMap<_, _>>>()?,
...
pub fn initialize_scan_predicate<'a>(
    ...
    verbose: bool,
) -> ApolloResult<(Option<SkipFilesMask>, Option<&'a ScanIOPredicate>)> {
    #[expect(clippy::never_loop)]
    loop {
