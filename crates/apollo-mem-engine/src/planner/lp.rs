...
pub type StreamingExecutorBuilder =
    fn(Node, &mut Arena<IR>, &mut Arena<AExpr>) -> ApolloResult<Box<dyn Executor>>;
...
pub fn create_multiple_physical_plans(
    ...
    expr_arena: &mut Arena<AExpr>,
    build_streaming_executor: Option<StreamingExecutorBuilder>,
) -> ApolloResult<MultiplePhysicalPlans> {
    let mut state = ConversionState::new()?;
    let mut cache_nodes = Default::default();
    ...
    let cache_prefiller = (!cache_nodes.is_empty()).then(|| {
        ...
        impl Executor for Empty {
            fn execute(&mut self, _cache: &mut ExecutionState) -> ApolloResult<DataFrame> {
                Ok(DataFrame::empty())
            }
        }
...
pub fn python_scan_predicate(
    ...
    Option<Arc<dyn apollo_expr::prelude::PhysicalExpr>>,
    Option<Vec<u8>>,
)> {
    let mut predicate_serialized = None;
    let predicate = if let PythonPredicate::Apollo(e) = &options.predicate {
        // Convert to a pyarrow eval string.
        if matches!(options.python_source, PythonScanSource::Pyarrow) {
            use apollo_core::config::verbose_print_sensitive;

            let predicate_pa = apollo_plan::plans::python::pyarrow::predicate_to_pa(
                e.node(),
                expr_arena,
                Default::default(),
            );
        ...
        // Convert to physical expression for the case the reader cannot consume the predicate.
            ...
            predicate_serialized = apollo_plan::plans::python::predicate::serialize(&dsl_expr)?;
