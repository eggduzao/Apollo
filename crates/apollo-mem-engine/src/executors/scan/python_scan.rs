...
impl PythonScanExec {
    ...
    fn check_schema(&self, df: &DataFrame) -> ApolloResult<()> {
        if self.options.validate_schema {
            let output_schema = self.get_schema();
            apollo_ensure!(df.schema() == output_schema, SchemaMismatch: "user provided schema: {:?} doesn't match the DataFrame schema: {:?}", output_schema, df.schema());
        }
        Ok(())
    }
                ...
                PythonScanSource::IOPlugin => {
                    ...
                    loop {
                        match generator.call_method0(intern!(py, "__next__")) {
                            ...
                            Err(err) => {
                                apollo_bail!(ComputeError: "caught exception during execution of a Python source, exception: {}", err)
                            },
                        }
                    }
                    if chunks.is_empty() {
