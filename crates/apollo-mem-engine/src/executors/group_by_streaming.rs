...
impl GroupByStreamingExec {
    ...
    pub fn new(
        ...
        *gb_input = scan;
        ...
        let [new_root, new_scan] = apollo_plan::plans::prune::prune(
            &[root, scan],
            lp_arena,
            expr_arena,
            &mut new_ir_arena,
    ...
    fn keys(&self, df: &DataFrame, state: &ExecutionState) -> ApolloResult<Vec<Column>> {
...
fn estimate_unique_count(keys: &[Column], mut sample_size: usize) -> ApolloResult<usize> {
...
fn can_run_partitioned(
        ...
        .all(|s| matches!(s.is_sorted_flag(), IsSorted::Not))
    {
        ...
        Ok(false)
    } else if std::env::var("APOLLO_NO_PARTITION").is_ok() {
        if state.verbose() {
            eprintln!("APOLLO_NO_PARTITION set: running default HASH AGGREGATION")
        }
        Ok(false)
    } else if std::env::var("APOLLO_FORCE_PARTITION").is_ok() {
        if state.verbose() {
            eprintln!("APOLLO_FORCE_PARTITION set: running partitioned HASH AGGREGATION")
        }
        Ok(true)
        ...
        let unique_count_boundary = std::env::var("APOLLO_PARTITION_UNIQUE_COUNT")
