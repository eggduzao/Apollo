...
impl StackExec {
    fn execute_impl(
            ...
            && self.options.run_parallel
        {
            ...
            let df = POOL.install(|| iter.collect::<ApolloResult<Vec<_>>>())?;
        ...
        // Only horizontal parallelism
            ...
            if !self.options.should_broadcast {
                debug_assert!(
                    res.iter()
                        .all(|column| column.name().starts_with("__APOLLO_CSER_0x")),
                ...
                for (i, c) in res.iter().enumerate() {
                    ...
                    if verify_scalar && len != df_height && len == 1 && df_width > 0 {
                        ...
                        if !self.exprs[i].is_scalar()
                            && std::env::var("APOLLO_ALLOW_NON_SCALAR_EXP").as_deref() != Ok("1")
                        {
                            let identifier = match self.exprs[i].as_expression() {
                                Some(e) => format!("expression: {e}"),
                                None => "this Series".to_string(),
                            };
                            apollo_bail!(InvalidOperation: "Series {}, length {} doesn't match the DataFrame height of {}\n\n\
