use apollo_plan::constants::CSE_REPLACED;
...
) -> ApolloResult<Vec<Vec<(u32, Column)>>> {
    POOL.install(|| {
        rolling
            .par_iter()
                ...
                match e {
                    ...
                    Expr::Over {
                        ...
                        if let Some((e, k)) = order_by {
                            apollo_expr::prelude::window_function_format_order_by(
                                &mut key,
                                e.as_ref(),
                                k,
                            )
    ...
    let mut all_equal_len = true;
    {
        ...
        for s in &selected_columns {
            ...
            if duplicate_check && !names.insert(name) {
                ...
                return Err(ApolloError::Duplicate(msg.into()));
            }
        }
    }
                ...
                Ok(match series.len() {
                    ...
                    1 => {
                         if !has_empty && df_height == 1 {
                            ...
                            if verify_scalar && !phys.is_scalar() && std::env::var("APOLLO_ALLOW_NON_SCALAR_EXP").as_deref() != Ok("1") {
                                    let identifier = match phys.as_expression() {
                                        Some(e) => format!("expression: {e}"),
                                        None => "this Series".to_string(),
                                    };
                                    apollo_bail!(ShapeMismatch: "Series {}, length {} doesn't match the DataFrame height of {}\n\n\
