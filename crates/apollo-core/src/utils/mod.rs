...
impl<T: ApolloDataType> Container for ChunkedArray<T> {
    fn slice(&self, offset: i64, len: usize) -> Self {
...
#[macro_export]
macro_rules! with_match_physical_float_apollo_type {(
...
#[macro_export]
macro_rules! with_match_physical_numeric_apollo_type {(
    $key_type:expr, | $_:tt $T:ident | $($body:tt)*
) => ({
    macro_rules! __with_ty__ {( $_ $T:ident ) => ( $($body)* )}
    use $crate::datatypes::DataType::*;
...
#[macro_export]
macro_rules! with_match_physical_integer_apollo_type {(
    $key_type:expr, | $_:tt $T:ident | $($body:tt)*
) => ({
    macro_rules! __with_ty__ {( $_ $T:ident ) => ( $($body)* )}
    use $crate::datatypes::DataType::*;
...
#[cold]
#[inline(never)]
fn width_mismatch(df1: &DataFrame, df2: &DataFrame) -> ApolloError {
...
pub fn accumulate_dataframes_vertical<I>(dfs: I) -> ApolloResult<DataFrame>
where
    I: IntoIterator<Item = DataFrame>,
{
    let mut iter = dfs.into_iter();
...
pub fn concat_df<'a, I>(dfs: I) -> ApolloResult<DataFrame>
where
...
pub fn accumulate_dataframes_horizontal(dfs: Vec<DataFrame>) -> ApolloResult<DataFrame> {
    let mut iter = dfs.into_iter();
    let mut acc_df = iter.next().unwrap();
    for df in iter {
...
pub fn coalesce_nulls<'a, T: ApolloDataType>(
    a: &'a ChunkedArray<T>,
    b: &'a ChunkedArray<T>,
...
mod test {
    ...
    #[test]
    fn test_align_chunks() -> ApolloResult<()> {
