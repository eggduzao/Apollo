...
use apollo_compute::decimal::{DEC128_MAX_PREC, i128_to_dec128};

use super::*;

/// Given two data types, determine the data type that both types can safely be cast to.
///
/// Returns a [`ApolloError::ComputeError`] if no such data type exists.
pub fn try_get_supertype(l: &DataType, r: &DataType) -> ApolloResult<DataType> {
    get_supertype(l, r).ok_or_else(
        || apollo_err!(SchemaMismatch: "failed to determine supertype of {} and {}", l, r),
    )
}
...
pub fn dtypes_to_supertype<'a, I>(dtypes: I) -> ApolloResult<DataType>
where
...
pub fn merge_dtypes_many<I: IntoIterator<Item = D> + Clone, D: AsRef<DataType>>(
    ...
    let mut st = iter
        .next()
        .ok_or_else(|| apollo_err!(ComputeError: "expect at least 1 dtype"))
