...
impl DataFrame {
    ...
    /// }
    /// ```
    pub fn hstack_mut(&mut self, columns: &[Column]) -> ApolloResult<&mut Self> {
        if self.shape() == (0, 0)
            && let Some(c) = columns.first()
        {
            unsafe { self.set_height(c.len()) };
...
pub fn concat_df_horizontal(
    ...
    let output_height = dfs
        ...
        .map(|df| df.height())
        .max()
        .ok_or_else(|| apollo_err!(ComputeError: "cannot concat empty dataframes"))?;
    ...
    let dfs = if !all_equal_height {
        if strict {
            return Err(
                apollo_err!(ShapeMismatch: "cannot concat dataframes with different heights in 'strict' mode"),
            );
        }
        out_width = 0;
...
mod tests {
    ...
    fn test_hstack_mut_empty_frame_height_validation() {
        ...
        assert!(
            matches!(result, Err(ApolloError::ShapeMismatch(_))),
