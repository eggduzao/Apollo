...
use apollo_row::ArrayRef;
use apollo_utils::UnitVec;
use apollo_utils::ideal_morsel_size::get_ideal_morsel_size;
use apollo_utils::itertools::Itertools;
use rayon::prelude::*;
...
impl DataFrame {
    ...
    pub fn try_apply_columns(
        &self,
        func: impl Fn(&Column) -> ApolloResult<Column> + Send + Sync,
    ) -> ApolloResult<Vec<Column>> {
        return inner(self, &func);

        fn inner(
            slf: &DataFrame,
            func: &(dyn Fn(&Column) -> ApolloResult<Column> + Send + Sync),
        ) -> ApolloResult<Vec<Column>> {
    ...
    pub fn ensure_matches_schema(&mut self, schema: &Schema) -> ApolloResult<()> {
    ...
    ///  +-----+----------+
    /// ```
    pub fn with_row_index(&self, name: PlSmallStr, offset: Option<IdxSize>) -> ApolloResult<Self> {
        let mut new_columns = Vec::with_capacity(self.width() + 1);
        let offset = offset.unwrap_or(0);

        if self.get_column_index(&name).is_some() {
            apollo_bail!(duplicate = name)
        }
    ...
    /// # Ok::<(), ApolloError>(())
    /// ```
    pub fn set_column_names<T>(&mut self, new_names: &[T]) -> ApolloResult<()>
    where
        T: AsRef<str>,
    {
        apollo_ensure!(
    ...
    /// +---------+--------+----------+
    /// ```
    pub fn hstack(&self, columns: &[Column]) -> ApolloResult<Self> {
    ...
    /// +-----------+-------------------+
    /// ```
    pub fn vstack(&self, other: &DataFrame) -> ApolloResult<Self> {
    ...
    /// +-----------+-------------------+
    /// ```
    pub fn vstack_mut(&mut self, other: &DataFrame) -> ApolloResult<&mut Self> {
        ...
        unsafe { self.columns_mut_retain_schema() }
            .iter_mut()
            .zip(other.columns())
            .try_for_each::<_, ApolloResult<_>>(|(left, right)| {
    ...
    pub fn vstack_mut_owned(&mut self, other: DataFrame) -> ApolloResult<&mut Self> {
        if self.width() != other.width() {
            apollo_ensure!(
                self.shape() == (0, 0),
                ShapeMismatch:
    ...
    pub fn extend(&mut self, other: &DataFrame) -> ApolloResult<()> {
        apollo_ensure!(
            self.width() == other.width(),
            ShapeMismatch:
    ...
    ///
    /// let s1: ApolloResult<Column> = df.drop_in_place("Average weight");
    /// assert!(s1.is_err());
    ///
    ...
    /// # Ok::<(), ApolloError>(())
    /// ```
    pub fn drop_in_place(&mut self, name: &str) -> ApolloResult<Column> {
    ...
    /// +---------+---------------------+
    /// ```
    pub fn drop_nulls<S>(&self, subset: Option<&[S]>) -> ApolloResult<Self>
    where
        for<'a> &'a S: AsRef<str>,
    {
    ...
    fn _drop_nulls_impl(&self, subset: &[Column]) -> ApolloResult<Self> {
        ...
        let mask = iter
            .next()
            .ok_or_else(|| apollo_err!(NoData: "no data to drop nulls from"))?;
    ...
    /// # Ok::<(), ApolloError>(())
    /// ```
    pub fn drop(&self, name: &str) -> ApolloResult<Self> {
        let idx = self.try_get_column_index(name)?;
    ...
    pub fn insert_column(&mut self, index: usize, column: Column) -> ApolloResult<&mut Self> {
    ...
    /// columns.
    pub fn with_column(&mut self, mut column: Column) -> ApolloResult<&mut Self> {
        if self.shape() == (0, 0) {
            unsafe { self.set_height(column.len()) };
        }
    ...
    /// Get column index of a [`Series`] by name.
    pub fn try_get_column_index(&self, name: &str) -> ApolloResult<usize> {
        self.get_column_index(name)
            .ok_or_else(|| apollo_err!(col_not_found = name))
    }
    ...
    /// # Ok::<(), ApolloError>(())
    /// ```
    pub fn column(&self, name: &str) -> ApolloResult<&Column> {
        let idx = self.try_get_column_index(name)?;
    ...
    /// ```
    /// # use apollo_core::prelude::*;
    /// fn example(df: &DataFrame) -> ApolloResult<DataFrame> {
    ///     df.select(["foo", "bar"])
    /// }
    /// ```
    pub fn select<I, S>(&self, names: I) -> ApolloResult<Self>
    where
        I: IntoIterator<Item = S>,
        S: AsRef<str>,
    {
    ...
    pub unsafe fn select_unchecked<I, S>(&self, names: I) -> ApolloResult<Self>
    where
        I: IntoIterator<Item = S>,
        S: AsRef<str>,
    {
    ...
    /// }
    /// ```
    pub fn filter(&self, mask: &BooleanChunked) -> ApolloResult<Self> {
        if self.width() == 0 {
    ...
    pub fn filter_seq(&self, mask: &BooleanChunked) -> ApolloResult<Self> {
        if self.width() == 0 {
            filter_zero_width(self.height(), mask)
        } else {
    ...
    /// }
    /// ```
    pub fn take(&self, indices: &IdxCa) -> ApolloResult<Self> {
    ...
    /// ```
    /// # use apollo_core::prelude::*;
    /// fn example(df: &mut DataFrame) -> ApolloResult<&mut DataFrame> {
    ///     let original_name = "foo";
    ///     let new_name = "bar";
    ///     df.rename(original_name, new_name.into())
    /// }
    /// ```
    pub fn rename(&mut self, column: &str, name: PlSmallStr) -> ApolloResult<&mut Self> {
        ...
        self.get_column_index(column)
            ...
            .ok_or_else(|| apollo_err!(col_not_found = column))
            .map(|c| c.rename(name))?;
    ...
    pub fn rename_many<'a>(
        ...
        for (from, to) in renames {
            ...
            match schema.get_full(from) {
                None => apollo_bail!(col_not_found = from),
                Some((idx, _, _)) => {
    ...
    pub fn sort_impl(
        ...
        // apollo-lazy that may not even be present in this dataframe. therefore
        // when we try to set the first columns as sorted, we ignore the error as
        // expressions are not present (they are renamed to _APOLLO_SORT_COLUMN_i.
    ...
    /// ```
    /// # use apollo_core::prelude::*;
    /// fn sort_by_sepal_width(df: &DataFrame) -> ApolloResult<DataFrame> {
    ...
    /// ```
    /// # use apollo_core::prelude::*;
    /// fn sort_with_specific_order(df: &DataFrame, descending: bool) -> ApolloResult<DataFrame> {
    ///     df.sort(
    ///         ["sepal_width"],
    ...
    /// ```
    /// # use apollo_core::prelude::*;
    /// fn sort_by_multiple_columns_with_specific_order(df: &DataFrame) -> ApolloResult<DataFrame> {
    ///     df.sort(
    ...
    /// # Ok::<(), ApolloError>(())
    /// ```
    pub fn replace(&mut self, column: &str, new_col: Column) -> ApolloResult<&mut Self> {
    ...
    /// # Ok::<(), ApolloError>(())
    /// ```
    pub fn replace_column(&mut self, index: usize, new_column: Column) -> ApolloResult<&mut Self> {
        apollo_ensure!(
            index < self.width(),
            ShapeMismatch:
    ...
    /// +--------+-------+
    /// ```
    pub fn apply<F, C>(&mut self, name: &str, f: F) -> ApolloResult<&mut Self>
    where
        F: FnOnce(&Column) -> C,
        C: IntoColumn,
    {
    ...
    /// +--------+-------+
    /// ```
    pub fn apply_at_idx<F, C>(&mut self, idx: usize, f: F) -> ApolloResult<&mut Self>
    where
        F: FnOnce(&Column) -> C,
        C: IntoColumn,
    {
    ...
    /// +---------------------+--------+
    /// ```
    pub fn try_apply_at_idx<F, C>(&mut self, idx: usize, f: F) -> ApolloResult<&mut Self>
    where
        F: FnOnce(&Column) -> ApolloResult<C>,
        C: IntoColumn,
    {
        let df_height = self.height();
        let width = self.width();
    ...
    /// +---------------------+--------+
    /// ```
    pub fn try_apply<F, C>(&mut self, column: &str, f: F) -> ApolloResult<&mut Self>
    where
        F: FnOnce(&Series) -> ApolloResult<C>,
        C: IntoColumn,
    {
    ...
    pub fn pipe<F, B>(self, f: F) -> ApolloResult<B>
    where
        F: Fn(DataFrame) -> ApolloResult<B>,
    {
        f(self)
    }

    /// Pipe different functions/ closure operations that work on a DataFrame together.
    pub fn pipe_mut<F, B>(&mut self, f: F) -> ApolloResult<B>
    where
        F: Fn(&mut DataFrame) -> ApolloResult<B>,
    {
        f(self)
    }

    /// Pipe different functions/ closure operations that work on a DataFrame together.
    pub fn pipe_with_args<F, B, Args>(self, f: F, args: Args) -> ApolloResult<B>
    where
        F: Fn(DataFrame, Args) -> ApolloResult<B>,
    {
        f(self, args)
    }
    ...
    /// ```
    #[cfg(feature = "algorithm_group_by")]
    pub fn is_unique(&self) -> ApolloResult<BooleanChunked> {
    ...
    /// ```
    #[cfg(feature = "algorithm_group_by")]
    pub fn is_duplicated(&self) -> ApolloResult<BooleanChunked> {
        let gb = self.group_by(self.get_column_names_owned())?;
        let groups = gb.get_groups();
        Ok(is_unique_helper(
            groups,
    ...
    pub fn get_supertype(&self) -> Option<ApolloResult<DataType>> {
        self.columns()
            .iter()
    ...
    pub fn partition_by<I, S>(&self, cols: I, include_key: bool) -> ApolloResult<Vec<DataFrame>>
    where
        I: IntoIterator<Item = S>,
        S: Into<PlSmallStr>,
    {
    ...
    fn unnest_impl(
            ...
            if cols.contains(s.name()) {
                ...
                new_cols.extend(ca.fields_as_series().into_iter().map(|mut f| {
                    if let Some(separator) = &separator {
                        f.rename(apollo_utils::format_pl_smallstr!(
                            "{}{}{}",
                            s.name(),
                            separator,
                            f.name()
        ...
        if count != cols.len() {
            ...
            for col in cols {
                let _ = schema
                    .get(col.as_str())
                    .ok_or_else(|| apollo_err!(col_not_found = col))?;
            }
        }
    ...
    pub fn append_record_batch(&mut self, rb: RecordBatchT<ArrayRef>) -> ApolloResult<()> {
...
fn ensure_can_extend(left: &Column, right: &Column) -> ApolloResult<()> {
    apollo_ensure!(
