...
use apollo_utils::hashing::DirtyHash;
use rayon::prelude::*;
...
impl DataFrame {
    ...
    /// fn group_by_sum(df: &DataFrame) -> ApolloResult<DataFrame> {
    ///     df.group_by(["column_name"])?
    ///     .select(["agg_column_name"])
    ///     .sum()
    /// }
    /// ```
    pub fn group_by<I, S>(&self, by: I) -> ApolloResult<GroupBy<'_>>
    where
    ...
    pub fn group_by_stable<I, S>(&self, by: I) -> ApolloResult<GroupBy<'_>>
    where
        I: IntoIterator<Item = S>,
        S: AsRef<str>,
    {
...
impl<'a> GroupBy<'a> {
    ...
    fn prepare_agg(&self) -> ApolloResult<(Vec<Column>, Vec<Column>)> {
        let keys = self.keys();
    ...
    /// +------------+-----------+-----------+
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn mean(&self) -> ApolloResult<DataFrame> {
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn sum(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn min(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn max(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn first(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn last(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn n_unique(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn quantile(&self, quantile: f64, method: QuantileMethod) -> ApolloResult<DataFrame> {
        apollo_ensure!(
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn median(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn var(&self, ddof: u8) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn std(&self, ddof: u8) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    /// +------------+------------+
    /// ```
    pub fn count(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
    ...
    /// +--------------+------------+
    /// ```
    pub fn groups(&self) -> ApolloResult<DataFrame> {
    ...
    /// ```
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn agg_list(&self) -> ApolloResult<DataFrame> {
        let (mut cols, agg_cols) = self.prepare_agg()?;
        for agg_col in agg_cols {
    ...
    fn prepare_apply(&self) -> ApolloResult<DataFrame> {
    ...
    #[deprecated(since = "0.24.1", note = "use apollo.lazy aggregations")]
    pub fn par_apply<F>(&self, f: F) -> ApolloResult<DataFrame>
    where
        F: Fn(DataFrame) -> ApolloResult<DataFrame> + Send + Sync,
    {
        apollo_ensure!(self.df.height() > 0, ComputeError: "cannot group_by + apply on empty 'DataFrame'");
    ...
    pub fn apply<F>(&self, f: F) -> ApolloResult<DataFrame>
    where
        ...
        F: FnMut(DataFrame) -> ApolloResult<DataFrame> + Send + Sync,
    {
        if self.df.height() == 0 {
            ...
            apollo_bail!(ComputeError: "cannot group_by + apply on empty 'DataFrame'");
        }
...
mod test {
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn test_group_by_null_handling() -> ApolloResult<()> {
        let df = df!(
            "a" => ["a", "a", "a", "b", "b"],
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn test_group_by_var() -> ApolloResult<()> {
    ...
    #[cfg_attr(miri, ignore)]
    #[cfg(feature = "dtype-categorical")]
    fn test_group_by_null_group() -> ApolloResult<()> {
