...
use apollo_compute::rolling::no_nulls::{
    MaxWindow, MinWindow, MomentWindow, QuantileWindow, RollingAggWindowNoNulls,
};
use apollo_compute::rolling::nulls::{RollingAggWindowNulls, VarianceMoment};
use apollo_compute::rolling::quantile_filter::SealedRolling;
use apollo_compute::rolling::{
...
use apollo_utils::arg_min_max::ArgMinMax;
use apollo_utils::float::IsFloat;
#[cfg(feature = "dtype-f16")]
use apollo_utils::float16::pf16;
use apollo_utils::idx_vec::IdxVec;
use apollo_utils::kahan_sum::KahanSum;
...
trait QuantileDispatcher<K> {
    fn _quantile(self, quantile: f64, method: QuantileMethod) -> ApolloResult<Option<K>>;
...
where
    ...
    T::Native: Ord,
{
    fn _quantile(self, quantile: f64, method: QuantileMethod) -> ApolloResult<Option<f64>> {
...
impl QuantileDispatcher<pf16> for Float16Chunked {
    fn _quantile(self, quantile: f64, method: QuantileMethod) -> ApolloResult<Option<pf16>> {
        self.quantile_faster(quantile, method)
    }
...
impl QuantileDispatcher<f32> for Float32Chunked {
    fn _quantile(self, quantile: f64, method: QuantileMethod) -> ApolloResult<Option<f32>> {
        self.quantile_faster(quantile, method)
    }
...
where
    ...
    K: ApolloNumericType,
    <K as datatypes::ApolloNumericType>::Native: num_traits::Float + quantile_filter::SealedRolling,
{
...
where
    ...
    ChunkedArray<T>: QuantileDispatcher<K::Native>,
    K: ApolloNumericType,
    <K as datatypes::ApolloNumericType>::Native: num_traits::Float + SealedRolling,
{
    match groups {
        GroupsType::Idx(groups) => {
            let ca = ca.rechunk();
...
#[cfg(feature = "bitwise")]
unsafe fn bitwise_agg<T: ApolloNumericType>(
    ca: &ChunkedArray<T>,
    groups: &GroupsType,
    ...
    T::Native: Pow<T::Native, Output = T::Native>,
{
    ...
    where
        <T as datatypes::ApolloNumericType>::Native: num_traits::Float,
    {
