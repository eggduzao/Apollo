...
impl<T: ApolloNumericType> AggList for ChunkedArray<T> {
...
#[cfg(feature = "object")]
impl<T: ApolloObject> AggList for ObjectChunked<T> {
    unsafe fn agg_list(&self, groups: &GroupsType) -> Series {
        let mut can_fast_explode = true;
        let mut offsets = Vec::<i64>::with_capacity(groups.len() + 1);
        let mut length_so_far = 0i64;
...
unsafe fn agg_list_by_gather_and_offsets<T: ApolloDataType>(
    ca: &ChunkedArray<T>,
    groups: &GroupsType,
) -> Series
where
