...
fn get_supertype_all(df: &DataFrame, rhs: &Series) -> ApolloResult<DataType> {
...
impl Add<&Series> for &DataFrame {
    type Output = ApolloResult<DataFrame>;
...
impl DataFrame {
    fn binary_aligned(
        &self,
        other: &DataFrame,
        f: &(dyn Fn(&Series, &Series) -> ApolloResult<Series> + Sync + Send),
        ...
        let mut cols = POOL.install(|| cols.collect::<ApolloResult<Vec<_>>>())?;
        ...
        if col_len < max_width {
            ...
            for i in col_len..max_len {
                let s = &df.columns().get(i).ok_or_else(|| apollo_err!(InvalidOperation: "cannot do arithmetic on DataFrames with shapes: {:?} and {:?}", self.shape(), other.shape()))?;
                let name = s.name();
                let dtype = s.dtype();
