...
/// ## Use declarations
///
/// All the common tools can be found in [`crate::prelude`] (or in `apollo::prelude`).
///
/// ```rust
/// use apollo_core::prelude::*; // if the crate apollo-core is used directly
/// // use apollo::prelude::*;      if the crate apollo is used
/// ```
///
/// # Initialization
/// ## Default
...
///
/// let df: ApolloResult<DataFrame> = DataFrame::new_infer_height(vec![s1, s2]);
/// ```
///
/// ## Using a macro
///
...
/// ```rust
/// # use apollo_core::prelude::*;
/// let df: ApolloResult<DataFrame> = df!("Fruit" => ["Apple", "Apple", "Pear"],
///                                       "Color" => ["Red", "Yellow", "Green"]);
/// ```
///
/// ## Using a CSV file
///
/// See the `apollo_io::csv::CsvReader`.
///
/// # Indexing
/// ## By a number
///
...
impl DataFrame {
    ...
    ///
    /// ```rust
    /// use apollo_core::prelude::DataFrame;
    ...
    pub fn new(height: usize, columns: Vec<Column>) -> ApolloResult<Self> {
    ...
    pub fn new_infer_height(columns: Vec<Column>) -> ApolloResult<Self> {
    ...
    pub fn new_with_broadcast(height: usize, mut columns: Vec<Column>) -> ApolloResult<Self> {
        broadcast_columns(height, &mut columns)?;
        DataFrame::new(height, columns)
    }

    /// Infers height as the first non-unit length column or 1 if not found.
    pub fn new_infer_broadcast(columns: Vec<Column>) -> ApolloResult<Self> {
    ...
    /// # Safety
    /// [`Column`]s must have unique names.
    pub unsafe fn new_unchecked_infer_broadcast(columns: Vec<Column>) -> ApolloResult<Self> {
