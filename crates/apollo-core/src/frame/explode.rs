...
fn get_exploded(
    ...
    match series.dtype() {
        ...
        _ => apollo_bail!(opq = explode, series.dtype()),
    }
}
...
impl DataFrame {
    pub fn explode_impl(
        ...
        apollo_ensure!(!columns.is_empty(), InvalidOperation: "no columns provided in explode");
    ...
    ///  +-----+-----+-----+
    /// ```
    pub fn explode<I, S>(&self, columns: I, options: ExplodeOptions) -> ApolloResult<DataFrame>
    where
...
mod test {
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn test_explode_df_empty_list() -> ApolloResult<()> {
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn test_explode_single_col() -> ApolloResult<()> {
