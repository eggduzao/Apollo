...
impl DataFrame {
    pub(crate) fn transpose_from_dtype(
        ...
        match dtype {
            ...
            DataType::Object(_) => {
                ...
                apollo_bail!(InvalidOperation: "Object dtype not supported in 'transpose'")
            },
            _ => {
    ...
    pub fn transpose_impl(
        ...
        let names_out = match new_col_names {
            ...
            Some(cn) => match cn {
                Either::Left(name) => {
                    ...
                    apollo_ensure!(new_names.null_count() == 0, ComputeError: "Column with new names can't have null values");
                ...
                Either::Right(names) => {
                    apollo_ensure!(names.len() == self.height(), ShapeMismatch: "Length of new column names must be the same as the row count");
                    names
                },
            },
        };
        if let Some(cn) = keep_names_as {
            ...
            apollo_ensure!(names_out.iter().all(|a| a.as_str() != cn), Duplicate: "{} is already in output column names", cn)
        }
        apollo_ensure!(
...
pub(super) fn numeric_transpose<T: ApolloNumericType>(
    cols: &[Column],
    names_out: &[PlSmallStr],
...
mod test {
    ...
    #[test]
    fn test_transpose() -> ApolloResult<()> {
        let mut df = df![
            "a" => [1, 2, 3],
            "b" => [10, 20, 30],
        ]?;
