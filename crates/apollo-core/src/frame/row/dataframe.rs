...
impl DataFrame {
    ...
    pub fn get_row(&self, idx: usize) -> ApolloResult<Row<'_>> {
        let values = self
    ...
    pub fn get_row_amortized<'a>(&'a self, idx: usize, row: &mut Row<'a>) -> ApolloResult<()> {
    ...
    pub fn from_rows_and_schema(rows: &[Row], schema: &Schema) -> ApolloResult<Self> {
    ...
    /// This should only be used when you have row wise data, as this is a lot slower
    /// than creating the [`Series`] in a columnar fashion.
    pub fn from_rows_iter_and_schema<'a, I>(mut rows: I, schema: &Schema) -> ApolloResult<Self>
    where
        I: Iterator<Item = &'a Row<'a>>,
    {
        ...
        rows.try_for_each::<_, ApolloResult<()>>(|row| {
            expected_len += 1;
    ...
    pub fn try_from_rows_iter_and_schema<'a, I>(mut rows: I, schema: &Schema) -> ApolloResult<Self>
    where
        I: Iterator<Item = ApolloResult<&'a Row<'a>>>,
    {
        let capacity = rows.size_hint().0;
    ...
    /// as this is a lot slower than creating the [`Series`] in a columnar fashion
    pub fn from_rows(rows: &[Row]) -> ApolloResult<Self> {
