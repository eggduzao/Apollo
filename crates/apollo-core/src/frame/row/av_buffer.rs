...
use apollo_compute::decimal::DecimalFmtBuffer;
...
impl<'a> AnyValueBuffer<'a> {
    ...
    pub(crate) fn add_fallible(&mut self, val: &AnyValue<'a>) -> ApolloResult<()> {
    ...
    pub fn reset(&mut self, capacity: usize, strict: bool) -> ApolloResult<Series> {
        use AnyValueBuffer::*;
        let out = match self {
            Boolean(b) => {
