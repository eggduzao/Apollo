...
pub type Schema = apollo_schema::Schema<DataType, ()>;
...
impl SchemaExt for Schema {
    ...
    /// Look up the name in the schema and return an owned [`Field`] by cloning the data.
    ///
    /// Returns `Err(ApolloErr)` if the field does not exist.
    ///
    /// This method constructs the `Field` by cloning the name and dtype. For a version that returns references, see
    /// [`get`][Self::get] or [`get_full`][Self::get_full].
    fn try_get_field(&self, name: &str) -> ApolloResult<Field> {
    ...
    fn to_supertype(&mut self, other: &Schema) -> ApolloResult<bool> {
        apollo_ensure!(self.len() == other.len(), ComputeError: "schema lengths differ");

        let mut changed = false;
        for ((k, dt), (other_k, other_dt)) in self.iter_mut().zip(other.iter()) {
            apollo_ensure!(k == other_k, ComputeError: "schema names differ: got {}, expected {}", k, other_k);
...
pub fn ensure_matching_schema<F, M>(
    lhs: &apollo_schema::Schema<F, M>,
    rhs: &apollo_schema::Schema<F, M>,
) -> ApolloResult<()>
where
    apollo_schema::Schema<F, M>: SchemaNamesAndDtypes,
{
