//! TODO
//!
//! This should ideally be moved to `apollo-schema`, currently it cannot due to dependency on
//! `apollo_core::DataType`.
use std::borrow::Cow;
use std::sync::Arc;

use arrow::datatypes::{ArrowDataType, ArrowSchema, Field as ArrowField};
use apollo_error::{ApolloResult, feature_gated, apollo_bail, apollo_err};
use apollo_utils::aliases::InitHashMaps;
...
impl IcebergSchema {
    ...
    pub fn from_arrow_schema(schema: &ArrowSchema) -> ApolloResult<Self> {
        Self::try_from_arrow_fields_iter(schema.iter_values())
    }

    pub fn try_from_arrow_fields_iter<'a, I>(iter: I) -> ApolloResult<Self>
    where
...
impl IcebergColumnType {
    pub fn to_apollo_dtype(&self) -> DataType {
        use IcebergColumnType::*;

        match self {
            Primitive { dtype } => dtype.clone(),
            List(inner) => DataType::List(Box::new(inner.type_.to_apollo_dtype())),
            FixedSizeList(inner, width) => {
                feature_gated!("dtype-array", {
                    DataType::Array(Box::new(inner.type_.to_apollo_dtype()), *width)
                })
            },
            Struct(fields) => feature_gated!("dtype-struct", {
                DataType::Struct(
                    fields
                        .values()
                        .map(|col| Field::new(col.name.clone(), col.type_.to_apollo_dtype()))
                        .collect(),
                )
            }),
        }
