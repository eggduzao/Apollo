...
use apollo_error::{ApolloResult, apollo_err, to_compute_err};
...
impl DataFrame {
    pub fn serialize_into_writer(&mut self, writer: &mut dyn std::io::Write) -> ApolloResult<()> {
    ...
    pub fn serialize_to_bytes(&mut self) -> ApolloResult<Vec<u8>> {
        let mut buf = vec![];
    ...
    pub fn deserialize_from_reader<T: Read + Seek>(reader: &mut T) -> ApolloResult<Self> {
        ...
        let dfs = reader
            ...
            .collect::<ApolloResult<Vec<DataFrame>>>()?;
        ...
        (|| {
            ...
            let flags: ApolloResult<Vec<u32>> = serde_json::from_str(flags).map_err(to_compute_err);
