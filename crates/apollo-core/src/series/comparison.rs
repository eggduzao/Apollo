...
macro_rules! impl_eq_compare {
    ($self:expr, $rhs:expr, $method:ident) => {{
        ...
        let mut out = match lhs.dtype() {
            ...
            dt => apollo_bail!(InvalidOperation: "could not apply comparison on series of dtype '{}; operand names: '{}', '{}'", dt, lhs.name(), rhs.name()),
        };
...
fn validate_types(left: &DataType, right: &DataType) -> ApolloResult<()> {
    use DataType::*;

    match (left, right) {
        (String, dt) | (dt, String) if dt.is_primitive_numeric() => {
            apollo_bail!(ComputeError: "cannot compare string with numeric type ({})", dt)
        },
        #[cfg(feature = "dtype-categorical")]
        (Categorical(_, _) | Enum(_, _), dt) | (dt, Categorical(_, _) | Enum(_, _))
            if !(dt.is_categorical() | dt.is_string() | dt.is_enum()) =>
        {
            apollo_bail!(ComputeError: "cannot compare categorical with {}", dt)
        },
        #[cfg(feature = "dtype-duration")]
        (Date, Duration(_)) | (Duration(_), Date) => {
            apollo_bail!(ComputeError: "cannot compare date with duration")
        },
        _ => (),
    };
    Ok(())
...
impl ChunkCompareEq<&str> for Series {
    ...
    fn equal(&self, rhs: &str) -> ApolloResult<BooleanChunked> {
    ...
    fn not_equal(&self, rhs: &str) -> ApolloResult<BooleanChunked> {
        validate_types(self.dtype(), &DataType::String)?;
        match self.dtype() {
