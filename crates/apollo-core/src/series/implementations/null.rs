...
use apollo_error::constants::LENGTH_LIMIT_MSG;
...
impl PrivateSeries for NullChunked {
    ...
    fn zip_with_same_type(&self, mask: &BooleanChunked, other: &Series) -> ApolloResult<Series> {
        let len = match (self.len(), mask.len(), other.len()) {
            ...
            (_, 0, _) => 0,
            _ => {
                apollo_bail!(ShapeMismatch: "shapes of `self`, `mask` and `other` are not suitable for `zip_with` operation")
            },
        };
...
fn null_arithmetic(lhs: &NullChunked, rhs: &Series, op: &str) -> ApolloResult<Series> {
    let output_len = match (lhs.len(), rhs.len()) {
        (1, len_r) => len_r,
        (len_l, 1) => len_l,
        (len_l, len_r) if len_l == len_r => len_l,
        _ => apollo_bail!(ComputeError: "Cannot {:?} two series of different lengths.", op),
    };
...
impl SeriesTrait for NullChunked {
    ...
    fn cast(&self, dtype: &DataType, _cast_options: CastOptions) -> ApolloResult<Series> {
    ...
    fn filter(&self, filter: &BooleanChunked) -> ApolloResult<Series> {
        let len = if self.is_empty() {
            ...
            apollo_ensure!(filter.len() <= 1, ShapeMismatch: "filter's length: {} differs from that of the series: 0", filter.len());
            0
            ...
            apollo_ensure!(filter.len() == self.len(), ShapeMismatch: "filter's length: {} differs from that of the series: {}", filter.len(), self.len());
    ...
    fn append(&mut self, other: &Series) -> ApolloResult<()> {
        apollo_ensure!(other.dtype() == &DataType::Null, ComputeError: "expected null dtype");
