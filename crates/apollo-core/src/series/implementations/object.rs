...
impl<T: ApolloObject> PrivateSeriesNumeric for SeriesWrap<ObjectChunked<T>> {
    ...
    T: ApolloObject,
{
    ...
    fn append(&mut self, other: &Series) -> ApolloResult<()> {
        apollo_ensure!(self.dtype() == other.dtype(), append);
    ...
    fn extend(&mut self, _other: &Series) -> ApolloResult<()> {
        apollo_bail!(opq = extend, self.dtype());
    }
    ...
    fn unique_id(&self) -> ApolloResult<(IdxSize, Vec<IdxSize>)> {
        apollo_bail!(opq = unique_id, self.dtype());
    }
    ...
    fn get_object(&self, index: usize) -> Option<&dyn ApolloObjectSafe> {
...
mod test {
    ...
    #[test]
    fn test_downcast_object() -> ApolloResult<()> {
        #[allow(non_local_definitions)]
        impl ApolloObject for i32 {
