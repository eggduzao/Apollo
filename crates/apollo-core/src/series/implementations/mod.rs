...
impl<T: ApolloDataType> From<ChunkedArray<T>> for SeriesWrap<ChunkedArray<T>> {
    fn from(ca: ChunkedArray<T>) -> Self {
        SeriesWrap(ca)
    }
}

impl<T: ApolloDataType> Deref for SeriesWrap<ChunkedArray<T>> {
...
unsafe impl<T: ApolloPhysicalType> IntoSeries for ChunkedArray<T> {
...
macro_rules! impl_dyn_series {
    ($ca: ident, $pdt:ty) => {
        ...
        impl SeriesTrait for SeriesWrap<$ca> {
            ...
            fn cast(&self, dtype: &DataType, options: CastOptions) -> ApolloResult<Series> {
            ...
            fn var_reduce(&self, ddof: u8) -> ApolloResult<Scalar> {
            ...
            fn and_reduce(&self) -> ApolloResult<Scalar> {
                let dt = <$pdt as ApolloDataType>::get_static_dtype();
...
impl<T: ApolloNumericType> private::PrivateSeriesNumeric for SeriesWrap<ChunkedArray<T>> {
