...
unsafe impl<T: ApolloCategoricalType> IntoSeries for CategoricalChunked<T> {
...
impl<T: ApolloCategoricalType> SeriesWrap<CategoricalChunked<T>> {
    unsafe fn apply_on_phys<F>(&self, apply: F) -> CategoricalChunked<T>
    where
        F: Fn(&ChunkedArray<T::ApolloPhysical>) -> ChunkedArray<T::ApolloPhysical>,
    {
        let cats = apply(self.0.physical());
        unsafe { CategoricalChunked::from_cats_and_dtype_unchecked(cats, self.0.dtype().clone()) }
    }

    unsafe fn try_apply_on_phys<F>(&self, apply: F) -> ApolloResult<CategoricalChunked<T>>
    where
        F: Fn(&ChunkedArray<T::ApolloPhysical>) -> ApolloResult<ChunkedArray<T::ApolloPhysical>>,
    {
        let cats = apply(self.0.physical())?;
        unsafe {
    ...
    ($ca: ident, $pdt:ty, $ca_fn:ident) => {
        impl private::PrivateSeries for SeriesWrap<$ca> {
            ...
            fn zip_with_same_type(&self, mask: &BooleanChunked, other: &Series) -> ApolloResult<Series> {
                apollo_ensure!(self.dtype() == other.dtype(), SchemaMismatch: "expected '{}' found '{}'", self.dtype(), other.dtype());
