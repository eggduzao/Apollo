...
impl private::PrivateSeries for SeriesWrap<TimeChunked> {
    ...
    fn subtract(&self, rhs: &Series) -> ApolloResult<Series> {
        let rhs = rhs.time().map_err(|_| apollo_err!(InvalidOperation: "cannot subtract a {} dtype with a series of type: {}", self.dtype(), rhs.dtype()))?;
    ...
    fn add_to(&self, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = add, DataType::Time, rhs.dtype());
    }

    fn multiply(&self, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = mul, self.0.dtype(), rhs.dtype());
    }

    fn divide(&self, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = div, self.0.dtype(), rhs.dtype());
    }

    fn remainder(&self, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = rem, self.0.dtype(), rhs.dtype());
    }
