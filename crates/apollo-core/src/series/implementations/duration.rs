...
impl private::PrivateSeries for SeriesWrap<DurationChunked> {
    ...
    fn zip_with_same_type(&self, mask: &BooleanChunked, other: &Series) -> ApolloResult<Series> {
    ...
    fn subtract(&self, rhs: &Series) -> ApolloResult<Series> {
        match (self.dtype(), rhs.dtype()) {
            (DataType::Duration(tu), DataType::Duration(tur)) => {
                apollo_ensure!(tu == tur, InvalidOperation: "units are different");
            ...
            (dtl, dtr) => apollo_bail!(opq = sub, dtl, dtr),
        }
    }
    fn add_to(&self, rhs: &Series) -> ApolloResult<Series> {
        match (self.dtype(), rhs.dtype()) {
            ...
            (dtl, dtr) => apollo_bail!(opq = add, dtl, dtr),
        }
    }
    ...
    fn divide(&self, rhs: &Series) -> ApolloResult<Series> {
        ...
        match rhs.dtype() {
            ...
            _ => {
                apollo_bail!(opq = div, self.dtype(), rhs.dtype());
            },
        }
    }
    fn remainder(&self, rhs: &Series) -> ApolloResult<Series> {
        apollo_ensure!(self.dtype() == rhs.dtype(), InvalidOperation: "dtypes and units must be equal in duration arithmetic");
        let lhs = self.cast(&DataType::Int64, CastOptions::NonStrict).unwrap();
        let rhs = rhs.cast(&DataType::Int64).unwrap();
        Ok(lhs
            .remainder(&rhs)?
    ...
    #[cfg(feature = "algorithm_group_by")]
    fn group_tuples(&self, multithreaded: bool, sorted: bool) -> ApolloResult<GroupsType> {
...
impl SeriesTrait for SeriesWrap<DurationChunked> {
    ...
    fn extend(&mut self, other: &Series) -> ApolloResult<()> {
        apollo_ensure!(self.0.dtype() == other.dtype(), extend);
    ...
    fn filter(&self, filter: &BooleanChunked) -> ApolloResult<Series> {
        self.0
            .physical()
            .filter(filter)
            .map(|ca| ca.into_duration(self.0.time_unit()).into_series())
    ...
    fn take(&self, indices: &IdxCa) -> ApolloResult<Series> {
        Ok(self
            .0
            .physical()
            .take(indices)?
    ...
    fn take_slice(&self, indices: &[IdxSize]) -> ApolloResult<Series> {
        Ok(self
            .0
            .physical()
            .take(indices)?
    ...
    fn cast(&self, dtype: &DataType, cast_options: CastOptions) -> ApolloResult<Series> {
    ...
    fn sort_with(&self, options: SortOptions) -> ApolloResult<Series> {
        Ok(self
            .0
            .physical()
            .sort_with(options)
    ...
    fn std_reduce(&self, ddof: u8) -> ApolloResult<Scalar> {
    ...
    fn quantile_reduce(&self, quantile: f64, method: QuantileMethod) -> ApolloResult<Scalar> {
    ...
    fn quantiles_reduce(&self, quantiles: &[f64], method: QuantileMethod) -> ApolloResult<Scalar> {
        ...
        if let AnyValue::List(float_s) = result.value() {
            ...
            apollo_bail!(ComputeError: "expected list scalar from quantiles_reduce")
        }
    }
