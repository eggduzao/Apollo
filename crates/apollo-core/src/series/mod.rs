...
use apollo_error::feature_gated;
...
impl Series {
    ...
    pub fn take_inner<T: ApolloPhysicalType>(self) -> ChunkedArray<T> {
    ...
    pub fn from_arrow_chunks(name: PlSmallStr, arrays: Vec<ArrayRef>) -> ApolloResult<Series> {
        Self::try_from((name, arrays))
    }

    pub fn from_arrow(name: PlSmallStr, array: ArrayRef) -> ApolloResult<Series> {
    ...
    pub fn append(&mut self, other: &Series) -> ApolloResult<&mut Self> {
    ...
    pub fn append_owned(&mut self, other: Series) -> ApolloResult<&mut Self> {
        let must_cast = other.dtype().matches_schema_type(self.dtype())?;
        if must_cast {
            let other = other.cast(self.dtype())?;
    ...
    pub fn extend(&mut self, other: &Series) -> ApolloResult<&mut Self> {
        let must_cast = other.dtype().matches_schema_type(self.dtype())?;
        if must_cast {
            let other = other.cast(self.dtype())?;
    ...
    pub fn as_single_ptr(&mut self) -> ApolloResult<usize> {
        self._get_inner_mut().as_single_ptr()
    }

    pub fn cast(&self, dtype: &DataType) -> ApolloResult<Self> {
    ...
    ///
    /// This can lead to invalid memory access in downstream code.
    pub unsafe fn from_physical_unchecked(&self, dtype: &DataType) -> ApolloResult<Self> {
        ...
        match (self.dtype(), dtype) {
            ...
            (phys, D::Categorical(cats, _)) if &cats.physical().dtype() == phys => {
                with_match_categorical_physical_type!(cats.physical(), |$C| {
                    type CA = ChunkedArray<<$C as ApolloCategoricalType>::ApolloPhysical>;
    ...
    pub fn to_float(&self) -> ApolloResult<Series> {
        match self.dtype() {
    ...
    pub fn sum<T>(&self) -> ApolloResult<T>
    where
        T: NumCast + IsFloat,
    {
    ...
    pub fn min<T>(&self) -> ApolloResult<Option<T>>
    where
        T: NumCast + IsFloat,
    {
    ...
    /// Returns an option because the array is nullable.
    pub fn max<T>(&self) -> ApolloResult<Option<T>>
    where
        T: NumCast + IsFloat,
    {
    ...
    pub fn explode(&self, options: ExplodeOptions) -> ApolloResult<Series> {
        match self.dtype() {
    ...
    pub fn is_nan(&self) -> ApolloResult<BooleanChunked> {
        match self.dtype() {
            ...
            _ => apollo_bail!(opq = is_nan, self.dtype()),
        }
    }
    ...
    pub fn is_not_nan(&self) -> ApolloResult<BooleanChunked> {
        match self.dtype() {
            ...
            _ => apollo_bail!(opq = is_not_nan, self.dtype()),
        }
    }
    ...
    pub fn is_finite(&self) -> ApolloResult<BooleanChunked> {
        match self.dtype() {
            ...
            _ => apollo_bail!(opq = is_finite, self.dtype()),
        }
    }
    ...
    pub fn is_infinite(&self) -> ApolloResult<BooleanChunked> {
        match self.dtype() {
            ...
            _ => apollo_bail!(opq = is_infinite, self.dtype()),
        }
    }
    ...
    /// length inputs.
    #[cfg(feature = "zip_with")]
    pub fn zip_with(&self, mask: &BooleanChunked, other: &Series) -> ApolloResult<Series> {
    ...
    pub fn gather_every(&self, n: usize, offset: usize) -> ApolloResult<Series> {
        apollo_ensure!(n > 0, ComputeError: "cannot perform gather every for `n=0`");
    ...
    pub fn dot(&self, other: &Series) -> ApolloResult<f64> {
    ...
    pub fn product(&self) -> ApolloResult<Scalar> {
        #[cfg(feature = "product")]
        {
            ...
            match self.dtype() {
                ...
                dt => {
                    apollo_bail!(InvalidOperation: "`product` operation not supported for dtype `{dt}`")
                },
            }
        }
    ...
    pub fn strict_cast(&self, dtype: &DataType) -> ApolloResult<Series> {
        self.cast_with_options(dtype, CastOptions::Strict)
    }

    #[cfg(feature = "dtype-decimal")]
    pub fn into_decimal(self, precision: usize, scale: usize) -> ApolloResult<Series> {
        match self.dtype() {
    ...
    // used for formatting
    pub fn str_value(&self, index: usize) -> ApolloResult<Cow<'_, str>> {
    ...
    pub fn unique_stable(&self) -> ApolloResult<Series> {
    ...
    pub fn row_encode_unordered(&self) -> ApolloResult<BinaryOffsetChunked> {
    ...
    pub fn row_encode_ordered(
        ...
        descending: bool,
        nulls_last: bool,
    ) -> ApolloResult<BinaryOffsetChunked> {
...
impl<T: ApolloPhysicalType> AsRef<ChunkedArray<T>> for dyn SeriesTrait + '_ {
...
impl<T: ApolloPhysicalType> AsMut<ChunkedArray<T>> for dyn SeriesTrait + '_ {
