...
use apollo_compute::cast::cast_unchecked as cast;
#[cfg(feature = "dtype-decimal")]
use apollo_compute::decimal::dec128_fits;
use apollo_error::feature_gated;
use apollo_utils::check_allow_importing_interval_as_struct;
use apollo_utils::itertools::Itertools;

use crate::chunked_array::cast::{CastOptions, cast_chunks};
#[cfg(feature = "object")]
use crate::chunked_array::object::extension::apollo_extension::ApolloExtension;
#[cfg(feature = "object")]
...
impl Series {
    ...
    /// Takes chunks and a apollo datatype and constructs the Series.
        ...
        match dtype {
            ...
            Object(_) => {
                if let Some(arr) = chunks[0].as_any().downcast_ref::<FixedSizeBinaryArray>() {
                    ...
                    {
                        let pe = ApolloExtension::new(arr.clone());
        ...
        match dtype {
            ...
            ArrowDataType::Decimal32(precision, scale) => {
                feature_gated!("dtype-decimal", {
                    apollo_compute::decimal::dec128_verify_prec_scale(*precision, *scale)?;
                    ...
                    for chunk in chunks.iter_mut() {
                        ...
                        let arr_128: PrimitiveArray<i128> = arr.iter().map(|opt_v| {
                            if let Some(v) = opt_v {
                                ...
                                smaller.ok_or_else(|| {
                                    apollo_err!(ComputeError: "Decimal256 to Decimal128 conversion overflowed, Decimal256 is not (yet) supported in Apollo")
                                }).map(Some)
                            } else {
                                Ok(None)
                            }
            ...
            ArrowDataType::Dictionary(key_type, _, _) => {
                let apollo_dtype = DataType::from_arrow(chunks[0].dtype(), md);

                let mut series_iter = chunks.into_iter().map(|arr| {
                    import_arrow_dictionary_array(name.clone(), arr, key_type, &apollo_dtype)
                });
            ...
            ArrowDataType::Extension(ext)
                if ext.name == APOLLO_OBJECT_EXTENSION_NAME && ext.metadata.is_some() =>
            {
                assert_eq!(chunks.len(), 1);
                let arr = chunks[0]
                    .as_any()
            ...
            dt => apollo_bail!(ComputeError: "cannot create series from {:?}", dt),
        }
    }
}
    ...
    if matches!(
        ...
        macro_rules! unpack_categorical_chunked {
            ($dt:ty) => {{
                ...
                with_match_categorical_physical_type!(apollo_dtype.cat_physical().unwrap(), |$C| {
        ...
        macro_rules! unpack_keys_values {
            ($dt:ty) => {{
                let arr = arr.as_any().downcast_ref::<DictionaryArray<$dt>>().unwrap();
                let keys = arr.keys();
                let keys = apollo_compute::cast::primitive_to_primitive::<
                    $dt,
                    <IdxType as ApolloNumericType>::Native,
...
#[cfg(feature = "dtype-struct")]
fn convert_month_day_nano_to_struct(chunk: Box<dyn Array>) -> ApolloResult<Box<dyn Array>> {
...
fn check_types(chunks: &[ArrayRef]) -> ApolloResult<ArrowDataType> {
    let mut chunks_iter = chunks.iter();
    let dtype: ArrowDataType = chunks_iter
        .next()
        .ok_or_else(|| apollo_err!(NoData: "expected at least one array-ref"))?
        .dtype()
        .clone();
...
impl TryFrom<(PlSmallStr, Vec<ArrayRef>)> for Series {
    ...
    fn try_from(name_arr: (PlSmallStr, Vec<ArrayRef>)) -> ApolloResult<Self> {
...
impl TryFrom<(PlSmallStr, ArrayRef)> for Series {
    ...
    fn try_from(name_arr: (PlSmallStr, ArrayRef)) -> ApolloResult<Self> {
...
impl TryFrom<(&ArrowField, Vec<ArrayRef>)> for Series {
    ...
    fn try_from(field_arr: (&ArrowField, Vec<ArrayRef>)) -> ApolloResult<Self> {
        ...
        apollo_ensure!(compatible, ComputeError: "Arrow Field dtype does not match the ArrayRef dtypes");
...
impl TryFrom<(&ArrowField, ArrayRef)> for Series {
    ...
    fn try_from(field_arr: (&ArrowField, ArrayRef)) -> ApolloResult<Self> {
