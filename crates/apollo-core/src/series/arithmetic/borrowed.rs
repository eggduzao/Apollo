...
pub trait NumOpsDispatchInner: ApolloDataType + Sized {
    fn subtract(lhs: &ChunkedArray<Self>, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = sub, lhs.dtype(), rhs.dtype());
    }
    fn add_to(lhs: &ChunkedArray<Self>, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = add, lhs.dtype(), rhs.dtype());
    }
    fn multiply(lhs: &ChunkedArray<Self>, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = mul, lhs.dtype(), rhs.dtype());
    }
    fn divide(lhs: &ChunkedArray<Self>, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = div, lhs.dtype(), rhs.dtype());
    }
    fn remainder(lhs: &ChunkedArray<Self>, rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = rem, lhs.dtype(), rhs.dtype());
    }
}

pub trait NumOpsDispatch {
    fn subtract(&self, rhs: &Series) -> ApolloResult<Series>;
    fn add_to(&self, rhs: &Series) -> ApolloResult<Series>;
    fn multiply(&self, rhs: &Series) -> ApolloResult<Series>;
    fn divide(&self, rhs: &Series) -> ApolloResult<Series>;
    fn remainder(&self, rhs: &Series) -> ApolloResult<Series>;
}
...
impl<T: ApolloNumericType> NumOpsDispatchInner for T {
    fn subtract(lhs: &ChunkedArray<T>, rhs: &Series) -> ApolloResult<Series> {
        apollo_ensure!(
            lhs.dtype() == rhs.dtype(),
            opq = add,
            rhs.dtype(),
    ...
    fn add_to(lhs: &ChunkedArray<T>, rhs: &Series) -> ApolloResult<Series> {
        apollo_ensure!(
            lhs.dtype() == rhs.dtype(),
            opq = add,
            rhs.dtype(),
    ...
    fn multiply(lhs: &ChunkedArray<T>, rhs: &Series) -> ApolloResult<Series> {
        apollo_ensure!(
            lhs.dtype() == rhs.dtype(),
            opq = add,
            rhs.dtype(),
    ...
    fn divide(lhs: &ChunkedArray<T>, rhs: &Series) -> ApolloResult<Series> {
        apollo_ensure!(
            lhs.dtype() == rhs.dtype(),
            opq = add,
            rhs.dtype(),
    ...
    fn remainder(lhs: &ChunkedArray<T>, rhs: &Series) -> ApolloResult<Series> {
        apollo_ensure!(
            lhs.dtype() == rhs.dtype(),
            opq = add,
            rhs.dtype(),
...
impl NumOpsDispatchInner for StringType {
    fn add_to(lhs: &StringChunked, rhs: &Series) -> ApolloResult<Series> {
...
impl NumOpsDispatchInner for BinaryType {
    fn add_to(lhs: &BinaryChunked, rhs: &Series) -> ApolloResult<Series> {
        let rhs = lhs.unpack_series_matching_type(rhs)?;
        let out = lhs + rhs;
        Ok(out.into_series())
    }
...
impl NumOpsDispatchInner for BooleanType {
    fn add_to(lhs: &BooleanChunked, rhs: &Series) -> ApolloResult<Series> {
        let rhs = lhs.unpack_series_matching_type(rhs)?;
        let out = lhs + rhs;
        Ok(out.into_series())
    }
...
pub mod checked {
    ...
    pub trait NumOpsDispatchCheckedInner: ApolloDataType + Sized {
        /// Checked integer division. Computes self / rhs, returning None if rhs == 0 or the division results in overflow.
        fn checked_div(lhs: &ChunkedArray<Self>, rhs: &Series) -> ApolloResult<Series> {
            apollo_bail!(opq = checked_div, lhs.dtype(), rhs.dtype());
        }
        fn checked_div_num<T: ToPrimitive>(
            lhs: &ChunkedArray<Self>,
            _rhs: T,
        ) -> ApolloResult<Series> {
            apollo_bail!(opq = checked_div_num, lhs.dtype(), Self::get_static_dtype());
        }
    }

    pub trait NumOpsDispatchChecked {
        /// Checked integer division. Computes self / rhs, returning None if rhs == 0 or the division results in overflow.
        fn checked_div(&self, rhs: &Series) -> ApolloResult<Series>;
        fn checked_div_num<T: ToPrimitive>(&self, _rhs: T) -> ApolloResult<Series>;
    }
    ...
    impl<S: NumOpsDispatchCheckedInner> NumOpsDispatchChecked for ChunkedArray<S> {
        ...
        fn checked_div_num<T: ToPrimitive>(&self, rhs: T) -> ApolloResult<Series> {
    ...
    {
        fn checked_div(lhs: &ChunkedArray<T>, rhs: &Series) -> ApolloResult<Series> {
            // SAFETY:
            // There will be UB if a ChunkedArray is alive with the wrong datatype.
            // we now only create the potentially wrong dtype for a short time.
            // Note that the physical type correctness is checked!
    ...
    impl NumOpsDispatchCheckedInner for Float16Type {
        fn checked_div(lhs: &Float16Chunked, rhs: &Series) -> ApolloResult<Series> {
            // SAFETY:
    ...
    impl NumOpsDispatchCheckedInner for Float32Type {
        fn checked_div(lhs: &Float32Chunked, rhs: &Series) -> ApolloResult<Series> {
            // SAFETY:
            // see check_div for chunkedarray<T>
            let rhs = unsafe { lhs.unpack_series_matching_physical_type(rhs) };
    ...
    impl NumOpsDispatchCheckedInner for Float64Type {
        fn checked_div(lhs: &Float64Chunked, rhs: &Series) -> ApolloResult<Series> {
            // SAFETY:
            // see check_div
            let rhs = unsafe { lhs.unpack_series_matching_physical_type(rhs) };
...
pub fn coerce_lhs_rhs<'a>(
    lhs: &'a Series,
    rhs: &'a Series,
) -> ApolloResult<(Cow<'a, Series>, Cow<'a, Series>)> {
...
pub fn _struct_arithmetic<F: FnMut(&Series, &Series) -> ApolloResult<Series>>(
    s: &Series,
    rhs: &Series,
    mut func: F,
) -> ApolloResult<Series> {
    ...
    match (s_fields.len(), rhs_fields.len()) {
        ...
        _ => {
            ...
            match (s.len(), rhs.len()) {
                ...
                (l, r) => {
                    apollo_bail!(ComputeError: "Struct arithmetic between different lengths {l} != {r}")
                },
            };
...
fn check_lengths(a: &Series, b: &Series) -> ApolloResult<()> {
    match (a.len(), b.len()) {
        // broadcasting
        (1, _) | (_, 1) => Ok(()),
        // equal
        ...
        // unequal
        (a, b) => {
            apollo_bail!(InvalidOperation: "cannot do arithmetic operation on series of different lengths: got {} and {}", a, b)
        },
    }
}

impl Add for &Series {
    type Output = ApolloResult<Series>;
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        ...
        match (self.dtype(), rhs.dtype()) {
            ...
            (_, Date) | (_, Datetime(_, _)) | (_, Time) => {
                apollo_bail!(opq = mul, self.dtype(), rhs.dtype())
            },
            (_, Duration(_)) => {
                // swap order
...
impl Div for &Series {
    ...
    fn div(self, rhs: Self) -> Self::Output {
        ...
        match (self.dtype(), rhs.dtype()) {
            ...
            | (_, Time)
            | (_, Date)
            | (_, Datetime(_, _)) => apollo_bail!(opq = div, self.dtype(), rhs.dtype()),
            (DataType::List(_), _) | (_, DataType::List(_)) => {
...
///
impl<T: ApolloNumericType> ChunkedArray<T> {
    /// Apply lhs - self
    #[must_use]
...
mod test {
    ...
    #[test]
    #[allow(clippy::eq_op)]
    fn test_arithmetic_series() -> ApolloResult<()> {
        // Series +-/* Series
