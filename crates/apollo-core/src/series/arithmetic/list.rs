...
//! use apollo_error::{feature_gated, ApolloResult};

use apollo_error::{ApolloResult, feature_gated};
...
impl NumOpsDispatchInner for ListType {
    fn add_to(lhs: &ListChunked, rhs: &Series) -> ApolloResult<Series> {
        NumericListOp::add().execute(&lhs.clone().into_series(), rhs)
    }

    fn subtract(lhs: &ListChunked, rhs: &Series) -> ApolloResult<Series> {
        NumericListOp::sub().execute(&lhs.clone().into_series(), rhs)
    }

    fn multiply(lhs: &ListChunked, rhs: &Series) -> ApolloResult<Series> {
        NumericListOp::mul().execute(&lhs.clone().into_series(), rhs)
    }

    fn divide(lhs: &ListChunked, rhs: &Series) -> ApolloResult<Series> {
        NumericListOp::div().execute(&lhs.clone().into_series(), rhs)
    }

    fn remainder(lhs: &ListChunked, rhs: &Series) -> ApolloResult<Series> {
...
impl NumericListOp {
    #[cfg_attr(not(feature = "list_arithmetic"), allow(unused))]
    pub fn execute(&self, lhs: &Series, rhs: &Series) -> ApolloResult<Series> {
...
mod inner {
    ...
    use num_traits::Zero;
    use apollo_compute::arithmetic::pl_num::PlNumArithmetic;
    ...
    impl ListNumericOpHelper {
        ...
        ) -> ApolloResult<Either<Self, ListChunked>> {
        ...
        pub(super) fn finish(mut self) -> ApolloResult<ListChunked> {
        ...
        fn _finish_impl_dispatch(&mut self) -> ApolloResult<ListChunked> {
            ...
            let out = with_match_physical_numeric_apollo_type!(&prim_dtype, |$T| {
        ...
        fn _finish_impl<T: ApolloNumericType>(
            &mut self,
            prim_s_lhs: Series,
            prim_s_rhs: Series,
        ...
        where
            ...
            PrimitiveArray<T::Native>:
                apollo_compute::comparisons::TotalEqKernel<Scalar = T::Native>,
