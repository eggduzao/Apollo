...
pub(crate) mod private {
    ...
    pub trait PrivateSeries {
        ...
        fn subtract(&self, _rhs: &Series) -> ApolloResult<Series> {
            apollo_bail!(opq = subtract, self._dtype());
        }
        fn add_to(&self, _rhs: &Series) -> ApolloResult<Series> {
            apollo_bail!(opq = add, self._dtype());
        }
        fn multiply(&self, _rhs: &Series) -> ApolloResult<Series> {
            apollo_bail!(opq = multiply, self._dtype());
        }
        fn divide(&self, _rhs: &Series) -> ApolloResult<Series> {
            apollo_bail!(opq = divide, self._dtype());
        }
        fn remainder(&self, _rhs: &Series) -> ApolloResult<Series> {
            apollo_bail!(opq = remainder, self._dtype());
        }
        #[cfg(feature = "algorithm_group_by")]
        fn group_tuples(&self, _multithreaded: bool, _sorted: bool) -> ApolloResult<GroupsType> {
            apollo_bail!(opq = group_tuples, self._dtype());
        }
        ...
        fn zip_with_same_type(
            ...
            apollo_bail!(opq = zip_with_same_type, self._dtype());
        }
        ...
        fn arg_sort_multiple(
            ...
            apollo_bail!(opq = arg_sort_multiple, self._dtype());
        }
    }
}
    ...
    Send + Sync + private::PrivateSeries + private::PrivateSeriesNumeric
{
    ...
    fn append(&mut self, other: &Series) -> ApolloResult<()>;
    fn append_owned(&mut self, other: Series) -> ApolloResult<()>;

    #[doc(hidden)]
    fn extend(&mut self, _other: &Series) -> ApolloResult<()>;

    /// Filter by boolean mask. This operation clones data.
    fn filter(&self, _filter: &BooleanChunked) -> ApolloResult<Series>;
    ...
    fn take(&self, _indices: &IdxCa) -> ApolloResult<Series>;
    ...
    ///
    /// This operation is clone.
    fn take_slice(&self, _indices: &[IdxSize]) -> ApolloResult<Series>;
    ...
    fn cast(&self, _dtype: &DataType, options: CastOptions) -> ApolloResult<Series>;

    /// Get a single value by index. Don't use this operation for loops as a runtime cast is
    /// needed for every iteration.
    fn get(&self, index: usize) -> ApolloResult<AnyValue<'_>> {
    ...
    fn sort_with(&self, _options: SortOptions) -> ApolloResult<Series> {
        apollo_bail!(opq = sort_with, self._dtype());
    }
    ...
    fn unique(&self) -> ApolloResult<Series> {
        apollo_bail!(opq = unique, self._dtype());
    }

    /// Get unique values in the Series.
    ///
    /// A `null` value also counts as a unique value.
    fn n_unique(&self) -> ApolloResult<usize> {
        apollo_bail!(opq = n_unique, self._dtype());
    }

    /// Get first indexes of unique values.
    fn arg_unique(&self) -> ApolloResult<IdxCa> {
        apollo_bail!(opq = arg_unique, self._dtype());
    }

    /// Get dense ids for each unique value.
    ///
    /// Returns: (n_unique, unique_ids)
    fn unique_id(&self) -> ApolloResult<(IdxSize, Vec<IdxSize>)>;
    ...
    fn as_single_ptr(&mut self) -> ApolloResult<usize> {
        apollo_bail!(opq = as_single_ptr, self._dtype());
    }
    ...
    /// If the [`DataType`] is one of `{Int8, UInt8, Int16, UInt16}` the `Series` is
    /// first cast to `Int64` to prevent overflow issues.
    fn sum_reduce(&self) -> ApolloResult<Scalar> {
        apollo_bail!(opq = sum, self._dtype());
    }
    /// Get the max of the Series as a new Series of length 1.
    fn max_reduce(&self) -> ApolloResult<Scalar> {
        apollo_bail!(opq = max, self._dtype());
    }
    /// Get the min of the Series as a new Series of length 1.
    fn min_reduce(&self) -> ApolloResult<Scalar> {
        apollo_bail!(opq = min, self._dtype());
    }
    /// Get the median of the Series as a new Series of length 1.
    fn median_reduce(&self) -> ApolloResult<Scalar> {
        apollo_bail!(opq = median, self._dtype());
    }
    /// Get the mean of the Series as a new Scalar
    fn mean_reduce(&self) -> ApolloResult<Scalar> {
        apollo_bail!(opq = mean, self._dtype());
    }
    /// Get the variance of the Series as a new Series of length 1.
    fn var_reduce(&self, _ddof: u8) -> ApolloResult<Scalar> {
        apollo_bail!(opq = var, self._dtype());
    }
    /// Get the standard deviation of the Series as a new Series of length 1.
    fn std_reduce(&self, _ddof: u8) -> ApolloResult<Scalar> {
        apollo_bail!(opq = std, self._dtype());
    }
    /// Get the quantile of the Series as a new Series of length 1.
    fn quantile_reduce(&self, _quantile: f64, _method: QuantileMethod) -> ApolloResult<Scalar> {
        apollo_bail!(opq = quantile, self._dtype());
    }
    ...
    fn quantiles_reduce(
        ...
        apollo_bail!(opq = quantiles, self._dtype());
    }
    /// Get the bitwise AND of the Series as a new Series of length 1,
    fn and_reduce(&self) -> ApolloResult<Scalar> {
        apollo_bail!(opq = and_reduce, self._dtype());
    }
    /// Get the bitwise OR of the Series as a new Series of length 1,
    fn or_reduce(&self) -> ApolloResult<Scalar> {
        apollo_bail!(opq = or_reduce, self._dtype());
    }
    /// Get the bitwise XOR of the Series as a new Series of length 1,
    fn xor_reduce(&self) -> ApolloResult<Scalar> {
        apollo_bail!(opq = xor_reduce, self._dtype());
    }
    ...
    fn approx_n_unique(&self) -> ApolloResult<IdxSize> {
        apollo_bail!(opq = approx_n_unique, self._dtype());
    }
    ...
    fn get_object(&self, _index: usize) -> Option<&dyn ApolloObjectSafe> {
    ...
    fn checked_div(&self, _rhs: &Series) -> ApolloResult<Series> {
        apollo_bail!(opq = checked_div, self._dtype());
    }
    ...
    fn rolling_map(
        &self,
        _f: &dyn Fn(&Series) -> ApolloResult<Series>,
        _options: RollingOptionsFixedWindow,
    ) -> ApolloResult<Series> {
        apollo_bail!(opq = rolling_map, self._dtype());
    }
}

impl dyn SeriesTrait + '_ {
    pub fn unpack<T: ApolloPhysicalType>(&self) -> ApolloResult<&ChunkedArray<T>> {
        apollo_ensure!(&T::get_static_dtype() == self.dtype(), unpack);
        Ok(self.as_ref())
    }
}
