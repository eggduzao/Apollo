...
use apollo_compute::gather::sublist::list::array_to_unit_list;
...
impl Series {
    ...
    #[cfg(feature = "dtype-array")]
    pub fn reshape_array(&self, dimensions: &[ReshapeDimension]) -> ApolloResult<Series> {
        apollo_ensure!(
            !dimensions.is_empty(),
    ...
    pub fn reshape_list(&self, dimensions: &[ReshapeDimension]) -> ApolloResult<Series> {
        apollo_ensure!(
            !dimensions.is_empty(),
            InvalidOperation: "at least one dimension must be specified"
        );
        ...
        match dimensions.len() {
            ...
            2 => {
                ...
                if s_ref.is_empty() {
                    if rows.get_or_infer(0) == 0 && cols.get_or_infer(0) <= 1 {
                        ...
                        apollo_bail!(InvalidOperation: "cannot reshape len 0 into shape {}", format_tuple!(dimensions))
                    }
                }
                ...
                let (rows, cols) = match (rows, cols) {
                    ...
                    _ => apollo_bail!(InvalidOperation: "reshape of non-zero list into zero list"),
                };
            ...
            _ => {
                apollo_bail!(InvalidOperation: "more than two dimensions not supported in reshaping to List.\n\nConsider reshaping to Array type.");
            },
        }
    }
}
