...
macro_rules! unpack_chunked_err {
    ($series:expr => $name:expr) => {
        apollo_err!(SchemaMismatch: "invalid series dtype: expected `{}`, got `{}` for series with name `{}`", $name, $series.dtype(), $series.name())
    };
}
...
impl Series {
    ...
    /// Unpack to [`ChunkedArray`] of dtype [`DataType::Time`]
    #[cfg(feature = "dtype-time")]
    pub fn time(&self) -> ApolloResult<&TimeChunked> {
        self.try_time()
    ...
    /// Unpack to [`ChunkedArray`] of dtype [`DataType::Categorical`] or [`DataType::Enum`] with a physical type of UInt8.
    #[cfg(feature = "dtype-categorical")]
    pub fn cat8(&self) -> ApolloResult<&CategoricalChunked<Categorical8Type>> {
        self.try_cat8()
    ...
    #[cfg(feature = "dtype-categorical")]
    pub fn cat16(&self) -> ApolloResult<&CategoricalChunked<Categorical16Type>> {
        self.try_cat16()
    ...
    #[cfg(feature = "dtype-categorical")]
    pub fn cat32(&self) -> ApolloResult<&CategoricalChunked<Categorical32Type>> {
        self.try_cat32()
    ...
    /// Unpack to [`ExtensionChunked`] of dtype [`DataType::Extension`].
    #[cfg(feature = "dtype-extension")]
    pub fn ext(&self) -> ApolloResult<&ExtensionChunked> {
        self.try_ext()
            .ok_or_else(|| unpack_chunked_err!(self => "Extension"))
    }

    /// Unpack to [`ChunkedArray`] of dtype [`DataType::Null`]
    pub fn null(&self) -> ApolloResult<&NullChunked> {
        self.try_null()
