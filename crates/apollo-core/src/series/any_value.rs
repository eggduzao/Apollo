...
use apollo_compute::cast::SerPrimitive;
...
fn any_values_to_primitive_nonstrict<T: ApolloNumericType>(values: &[AnyValue]) -> ChunkedArray<T> {
    values
        .iter()
...
fn any_values_to_integer<T: ApolloIntegerType>(
    values: &[AnyValue],
    strict: bool,
) -> ApolloResult<ChunkedArray<T>> {
    fn any_values_to_integer_strict<T: ApolloIntegerType>(
        values: &[AnyValue],
    ) -> ApolloResult<ChunkedArray<T>> {
...
#[cfg(feature = "dtype-f16")]
fn any_values_to_f16(values: &[AnyValue], strict: bool) -> ApolloResult<Float16Chunked> {
    fn any_values_to_f16_strict(values: &[AnyValue]) -> ApolloResult<Float16Chunked> {
        let mut builder =
...
fn any_values_to_f32(values: &[AnyValue], strict: bool) -> ApolloResult<Float32Chunked> {
    fn any_values_to_f32_strict(values: &[AnyValue]) -> ApolloResult<Float32Chunked> {
        let mut builder =
...
fn any_values_to_f64(values: &[AnyValue], strict: bool) -> ApolloResult<Float64Chunked> {
    fn any_values_to_f64_strict(values: &[AnyValue]) -> ApolloResult<Float64Chunked> {
        let mut builder =
...
fn any_values_to_bool(values: &[AnyValue], strict: bool) -> ApolloResult<BooleanChunked> {
...
fn any_values_to_string(values: &[AnyValue], strict: bool) -> ApolloResult<StringChunked> {
    fn any_values_to_string_strict(values: &[AnyValue]) -> ApolloResult<StringChunked> {
...
fn any_values_to_binary(values: &[AnyValue], strict: bool) -> ApolloResult<BinaryChunked> {
    fn any_values_to_binary_strict(values: &[AnyValue]) -> ApolloResult<BinaryChunked> {
...
#[cfg(feature = "dtype-date")]
fn any_values_to_date(values: &[AnyValue], strict: bool) -> ApolloResult<DateChunked> {
...
#[cfg(feature = "dtype-time")]
fn any_values_to_time(values: &[AnyValue], strict: bool) -> ApolloResult<TimeChunked> {
...
fn any_values_to_list(
    ...
    let ca = match inner_type {
        ...
        #[cfg(feature = "object")]
        DataType::Object(_) => apollo_bail!(nyi = "Nested object types"),
    ...
    if strict && !valid {
        apollo_bail!(SchemaMismatch: "unexpected value while building Series of type {:?}", DataType::List(Box::new(inner_type.clone())));
    }
...
fn any_values_to_array(
    ...
    if strict && !valid {
        apollo_bail!(SchemaMismatch: "unexpected value while building Series of type {:?}", target_dtype);
    }
    apollo_ensure!(
        out.width() == width,
...
#[cfg(feature = "object")]
fn any_values_to_object(values: &[AnyValue]) -> ApolloResult<Series> {
...
fn invalid_value_error(dtype: &DataType, value: &AnyValue) -> ApolloError {
    apollo_err!(
        SchemaMismatch:
