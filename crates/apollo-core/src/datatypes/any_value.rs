...
use apollo_utils::total_ord::ToTotalOrd;
...
#[cfg(feature = "object")]
#[derive(Debug)]
pub struct OwnedObject(pub Box<dyn ApolloObjectSafe>);
...
impl<'a> AnyValue<'a> {
    ...
    #[inline]
    pub fn try_extract<T: NumCast + IsFloat>(&self) -> ApolloResult<T> {
    ...
    /// Cast `AnyValue` to the provided data type and return a new `AnyValue` with type `dtype`,
    /// if possible.
    pub fn try_strict_cast(&self, dtype: &'a DataType) -> ApolloResult<AnyValue<'a>> {
        self.strict_cast(dtype).ok_or_else(
            || apollo_err!(ComputeError: "cannot cast any-value {:?} to dtype '{}'", self, dtype),
        )
    }
...
mod test {
    ...
    #[test]
    #[cfg(feature = "dtype-categorical")]
    fn test_arrow_dtypes_to_apollo() {
        let dtypes = [
            (
