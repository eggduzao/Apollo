...
impl DataType {
    ...
    /// are left unchanged.
    pub fn materialize_unknown(self, allow_unknown: bool) -> ApolloResult<DataType> {
        match self {
            DataType::Unknown(u) => match u.materialize() {
                ...
                None => {
                    if allow_unknown {
                        ...
                        apollo_bail!(SchemaMismatch: "failed to materialize unknown type")
                    }
                },
            },
    ...
    /// Get the inner data type of a nested type.
    pub fn try_into_inner_dtype(self) -> ApolloResult<DataType> {
        match self {
            DataType::List(inner) => Ok(*inner),
            #[cfg(feature = "dtype-array")]
            DataType::Array(inner, _) => Ok(*inner),
            dt => apollo_bail!(InvalidOperation: "cannot get inner datatype of `{dt}`"),
        }
    }
    ...
    pub fn max(&self) -> ApolloResult<Scalar> {
        use DataType::*;
        let v = match self {
            ...
            dt => apollo_bail!(ComputeError: "cannot determine upper bound for dtype `{}`", dt),
        };
        Ok(v)
    }

    /// Try to get the minimum value for this datatype.
    pub fn min(&self) -> ApolloResult<Scalar> {
        use DataType::*;
        let v = match self {
            ...
            dt => apollo_bail!(ComputeError: "cannot determine lower bound for dtype `{}`", dt),
        };
        Ok(v)
    }
    ...
    #[inline]
    pub fn try_to_arrow(&self, compat_level: CompatLevel) -> ApolloResult<ArrowDataType> {
        use DataType::*;
        match self {
    ...
    pub fn matches_schema_type(&self, schema_type: &DataType) -> ApolloResult<bool> {
        match (self, schema_type) {
            ...
            (DataType::Struct(l), DataType::Struct(r)) => {
                if l.len() != r.len() {
                    apollo_bail!(SchemaMismatch: "structs have different number of fields: {} vs {}", l.len(), r.len());
                }
            ...
            (l, r) if l == r => Ok(false),
            (l, r) => {
                apollo_bail!(SchemaMismatch: "type {:?} is incompatible with expected type {:?}", l, r)
            },
        }
    }
    ...
    #[cfg(feature = "dtype-categorical")]
    pub fn cat_physical(&self) -> ApolloResult<CategoricalPhysical> {
        match self {
            DataType::Categorical(cats, _) => Ok(cats.physical()),
            DataType::Enum(fcats, _) => Ok(fcats.physical()),
            _ => {
                apollo_bail!(SchemaMismatch: "invalid dtype: expected an Enum or Categorical type, received '{:?}'", self)
            },
        }
    }
    ...
    #[cfg(feature = "dtype-categorical")]
    pub fn cat_mapping(&self) -> ApolloResult<&Arc<CategoricalMapping>> {
        match self {
...
pub fn merge_dtypes(left: &DataType, right: &DataType) -> ApolloResult<DataType> {
    use DataType::*;
    Ok(match (left, right) {
        #[cfg(feature = "dtype-categorical")]
        ...
        (Struct(inner_l), Struct(inner_r)) => {
            apollo_ensure!(inner_l.len() == inner_r.len(), ComputeError: "cannot combine structs with differing amounts of fields ({} != {})", inner_l.len(), inner_r.len());
            let fields = inner_l.iter().zip(inner_r.iter()).map(|(l, r)| {
                apollo_ensure!(l.name() == r.name(), ComputeError: "cannot combine structs with different fields ({} != {})", l.name(), r.name());
        ...
        (Array(inner_l, width_l), Array(inner_r, width_r)) => {
            apollo_ensure!(width_l == width_r, ComputeError: "widths of FixedSizeWidth Series are not equal");
            let merged = merge_dtypes(inner_l, inner_r)?;
            Array(Box::new(merged), *width_l)
        },
        (left, right) if left == right => left.clone(),
        _ => apollo_bail!(ComputeError: "unable to merge datatypes"),
    })
}
...
impl CompatLevel {
    ...
    #[doc(hidden)]
    pub fn with_level(level: u16) -> ApolloResult<CompatLevel> {
        if level > CompatLevel::newest().0 {
            apollo_bail!(InvalidOperation: "invalid compat level");
        }
