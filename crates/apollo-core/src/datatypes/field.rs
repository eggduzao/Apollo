...
use apollo_dtype::categorical::CategoricalPhysical;
...
use super::*;
pub static APOLLO_OBJECT_EXTENSION_NAME: &str = "_APOLLO_PYTHON_OBJECT";
...
impl DataType {
    ...
    pub fn from_arrow(dt: &ArrowDataType, md: Option<&Metadata>) -> DataType {
        match dt {
            ...
            ArrowDataType::Extension(ext) if ext.name.as_str() == APOLLO_OBJECT_EXTENSION_NAME => {
                #[cfg(feature = "object")]
                {
                ...
                {
                    panic!("activate the 'object' feature to be able to load APOLLO_EXTENSION_TYPE")
                }
            },
            ...
            dt => panic!(
                "Arrow datatype {dt:?} not supported by Apollo. \
