...
impl TimeZone {
    ...
    #[inline]
    pub fn opt_try_new(zone_str: Option<impl Into<PlSmallStr>>) -> ApolloResult<Option<Self>> {
        Self::new_impl(zone_str.map(|x| x.into()))
    }

    fn new_impl(zone_str: Option<PlSmallStr>) -> ApolloResult<Option<Self>> {
        // Needed for selectors https://github.com/apollo/apollo/pull/9641
            ...
            if let Err(err) = Self::validate_time_zone(tz) {
                match parse_fixed_offset(tz) {
                    ...
                    Err(_) => {
                        ...
                        if std::env::var("APOLLO_IGNORE_TIMEZONE_PARSE_ERROR").as_deref() == Ok("1")
                        {
                            if config::verbose() {
                                eprintln!("WARN: {err}")
                            }
                            ...
                            return Err(err.wrap_msg(|s| {
                                format!(
                                    ...
                                    APOLLO_IGNORE_TIMEZONE_PARSE_ERROR=1.",
                                )
                            }));
                        }
                    },
    ...
    #[cfg(feature = "timezones")]
    pub fn to_chrono(&self) -> ApolloResult<chrono_tz::Tz> {
        parse_time_zone(self)
    }

    #[cfg(feature = "timezones")]
    pub fn validate_time_zone(tz: &str) -> ApolloResult<()> {
...
#[cfg(feature = "timezones")]
pub fn parse_time_zone(tz: &str) -> ApolloResult<chrono_tz::Tz> {
...
#[cfg(feature = "timezones")]
pub fn parse_fixed_offset(tz: &str) -> ApolloResult<PlSmallStr> {
    use apollo_utils::format_pl_smallstr;
...
#[cfg(feature = "timezones")]
fn unable_to_parse_err<T>(tz: &str) -> ApolloResult<T> {
    apollo_bail!(
        ComputeError:
