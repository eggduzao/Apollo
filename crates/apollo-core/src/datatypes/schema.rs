...
pub trait SchemaExtPl {
    ...
    fn matches_schema(&self, other: &Schema) -> ApolloResult<bool>;

    fn ensure_is_exact_match(&self, other: &Schema) -> ApolloResult<()>;
}

impl SchemaExtPl for Schema {
    fn matches_schema(&self, other: &Schema) -> ApolloResult<bool> {
        apollo_ensure!(self.len() == other.len(), SchemaMismatch: "found different number of fields in schema's\n\nLeft schema: {} fields, right schema: {} fields.", self.len(), other.len());
        let mut cast = false;
    ...
    fn ensure_is_exact_match(&self, other: &Schema) -> ApolloResult<()> {
        ...
        if !different_columns.is_empty() {
            ...
            return Err(
                apollo_err!(SchemaMismatch: "mismatching fields in schema's.\n\n{context}"),
            );
        }
