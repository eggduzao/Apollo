...
use apollo_dtype::categorical::{CategoricalPhysical, Categories, FrozenCategories};
use apollo_utils::pl_str::PlSmallStr;
use proptest::prelude::*;
use proptest::strategy::BoxedStrategy;

use crate::datatypes::{ApolloObjectSafe, pf16};
use crate::prelude::{
    AnyValue, ArrowDataType, ArrowField, DataType, Field, OwnedObject, ApolloObject, StructArray,
    TimeUnit,
};
...
impl ApolloObject for u64 {
    fn type_name() -> &'static str {
        "test_u64"
    }
}

impl ApolloObject for String {
    fn type_name() -> &'static str {
        "test_string"
    }
}

impl ApolloObject for bool {
    fn type_name() -> &'static str {
        "test_bool"
    }
}
...
fn av_object_strategy() -> impl Strategy<Value = AnyValue<'static>> {
    proptest::prop_oneof![
        any::<u64>().prop_map(|u| {
            let boxed: Box<dyn ApolloObjectSafe> = Box::new(u);
            let leaked: &'static dyn ApolloObjectSafe = Box::leak(boxed);
            AnyValue::Object(leaked)
        }),
        any::<String>().prop_map(|s| {
            let boxed: Box<dyn ApolloObjectSafe> = Box::new(s);
            let leaked: &'static dyn ApolloObjectSafe = Box::leak(boxed);
            AnyValue::Object(leaked)
        }),
        any::<bool>().prop_map(|b| {
            let boxed: Box<dyn ApolloObjectSafe> = Box::new(b);
            let leaked: &'static dyn ApolloObjectSafe = Box::leak(boxed);
            AnyValue::Object(leaked)
        })
    ]
