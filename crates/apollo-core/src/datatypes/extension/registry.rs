...
use crate::prelude::{DataType, APOLLO_OBJECT_EXTENSION_NAME};
    ...
    match REGISTRY.read().unwrap().get(name) {
        ...
        None => match get_unknown_extension_type_behavior() {
            ...
            UnknownExtensionTypeBehavior::WarnAndLoadAsStorage => {
                ...
                {
                    apollo_warn!("Extension type '{name}' is not registered; loading as its storage type.

To avoid this warning, register the extension type or set environment variable 'APOLLO_UNKNOWN_EXTENSION_TYPE_BEHAVIOR' to 'load_as_storage' or 'load_as_extension'.

In Apollo 2.0, the default behavior will change to 'load_as_extension'.");
                }
                None
            },
        },
...
static REGISTRY: LazyLock<RwLock<PlHashMap<PlSmallStr, Option<Arc<dyn ExtensionTypeFactory>>>>> =
    LazyLock::new(|| {
        ...
        m.insert(PlSmallStr::from_static(APOLLO_OBJECT_EXTENSION_NAME), None);
        RwLock::new(m)
    });
...
pub fn register_extension_type(
    ...
    match REGISTRY.write().unwrap().entry(name.into()) {
        Entry::Occupied(_) => {
            apollo_bail!(ComputeError: "attempted to register duplicate extension type with name '{name}'")
        },
        Entry::Vacant(v) => {
            v.insert(t);
            Ok(())
...
pub fn unregister_extension_type(
    name: &str,
) -> ApolloResult<Option<Arc<dyn ExtensionTypeFactory>>> {
    REGISTRY.write().unwrap().remove(name).ok_or_else(||
        apollo_err!(ComputeError: "attempted to unregister unknown extension type with name '{name}'")
    )
}
