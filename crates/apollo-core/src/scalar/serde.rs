...
use apollo_error::{ApolloError, ApolloResult, apollo_bail};
...
pub enum SerializableScalar {
    ...
    Categorical {
        ...
        physical: apollo_dtype::categorical::CategoricalPhysical,
    },
    #[cfg(feature = "dtype-categorical")]
    Enum {
        value: apollo_dtype::categorical::CatSize,
        categories: Series,
    },
    ...
    fn try_from(value: Scalar) -> Result<Self, Self::Error> {
        let out = match value.value {
            ...
            AnyValue::Object(..) | AnyValue::ObjectOwned(..) => {
                apollo_bail!(nyi = "Cannot serialize object value.")
            },
        ...
        Ok(match value {
            ...
            S::Struct(scs) => {
                let (avs, fields) = scs
                    ...
                    .collect::<ApolloResult<(Vec<AnyValue<'static>>, Vec<Field>)>>()?;
