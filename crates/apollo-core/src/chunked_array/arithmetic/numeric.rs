...
macro_rules! impl_op_overload {
    ($op: ident, $trait_method: ident, $ca_method: ident, $ca_method_scalar: ident) => {
        impl<T: ApolloNumericType> $op for ChunkedArray<T> {
        ...
        impl<T: ApolloNumericType, N: Num + ToPrimitive> $op<N> for ChunkedArray<T> {
            type Output = ChunkedArray<T>;
...
impl<T: ApolloNumericType> ArithmeticChunked for ChunkedArray<T> {
    type Scalar = T::Native;
    type Out = ChunkedArray<T>;
    type TrueDivOut = ChunkedArray<<T::Native as NumericNative>::TrueDivApolloType>;
