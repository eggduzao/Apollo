...
impl Add for &DecimalChunked {
    type Output = ApolloResult<DecimalChunked>;
        ...
        let phys = broadcast_try_binary_elementwise(
            ...
            |opt_l, opt_r| {
                let (Some(l), Some(r)) = (opt_l, opt_r) else {
                    return ApolloResult::Ok(None);
                };
                let ls = dec128_rescale(l, left_s, prec, scale).ok_or_else(|| {
                    apollo_err!(ComputeError: "overflow in Decimal cast for {l} from scale {left_s} to {scale}")
                })?;
                let rs = dec128_rescale(r, right_s, prec, scale).ok_or_else(|| {
                    apollo_err!(ComputeError: "overflow in Decimal cast for {r} from scale {right_s} to {scale}")
                })?;
                let ret = dec128_add(ls, rs, prec).ok_or_else(
                    || apollo_err!(ComputeError: "overflow in decimal addition for {ls} + {rs}"),
                )?;
                Ok(Some(ret))
            },
        );
        ...
        let phys = broadcast_try_binary_elementwise(
            ...
            |opt_l, opt_r| {
                ...
                let ret = dec128_sub(ls, rs, prec).ok_or_else(
                    || apollo_err!(ComputeError: "overflow in decimal subtraction for {ls} - {rs}"),
                )?;
                Ok(Some(ret))
            },
        );
        ...
        let phys = broadcast_try_binary_elementwise(
            ...
            |opt_l, opt_r| {
                ...
                let ret = dec128_mul(ls, rs, prec, scale).ok_or_else(|| {
                    apollo_err!(ComputeError: "overflow in decimal multiplication for {ls} * {rs}")
                })?;
                Ok(Some(ret))
            },
        );
    ...
    fn div(self, rhs: Self) -> Self::Output {
        ...
        let phys = broadcast_try_binary_elementwise(
            ...
            |opt_l, opt_r| {
                ...
                if r == 0 {
                    apollo_bail!(ComputeError: "division by zero Decimal");
                }
                let ls = dec128_rescale(l, left_s, prec, scale).ok_or_else(|| {
                    apollo_err!(ComputeError: "overflow in Decimal cast for {l} from scale {left_s} to {scale}")
                })?;
                ...
                let ret = dec128_div(ls, rs, prec, scale).ok_or_else(
                    || apollo_err!(ComputeError: "overflow in decimal division for {ls} / {rs}"),
                )?;
                Ok(Some(ret))
            },
        );
