...
pub use extension::set_apollo_allow_extension;
...
/// Trimmed down object safe apollo object
pub trait ApolloObjectSafe: Any + Debug + Send + Sync + Display {
    fn type_name(&self) -> &'static str;

    fn as_any(&self) -> &dyn Any;

    fn to_boxed(&self) -> Box<dyn ApolloObjectSafe>;

    fn equal(&self, other: &dyn ApolloObjectSafe) -> bool;
}

impl PartialEq for &dyn ApolloObjectSafe {
...
impl<T: ApolloObject> ApolloObjectSafe for T {
    fn type_name(&self) -> &'static str {
        T::type_name()
    }
...
impl<T: ApolloObject> Splitable for ObjectArray<T> {
...
impl<T: ApolloObject> StaticArray for ObjectArray<T> {
    type ValueT<'a> = &'a T;
...
impl<T: ApolloObject> ParameterFreeDtypeStaticArray for ObjectArray<T> {
    ...
    T: ApolloObject,
{
    ...
    ///
    /// No bounds checks
    pub unsafe fn get_object_unchecked(&self, index: usize) -> Option<&dyn ApolloObjectSafe> {
    ...
    /// Get a hold to an object that can be formatted or downcasted via the Any trait.
    pub fn get_object(&self, index: usize) -> Option<&dyn ApolloObjectSafe> {
        if index < self.len() {
...
impl<T: ApolloObject> From<Vec<T>> for ObjectArray<T> {
