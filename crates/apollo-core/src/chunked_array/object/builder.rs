    ...
    T: ApolloObject,
{
    ...
    #[inline]
    pub fn append_value_from_any(&mut self, v: &dyn Any) -> ApolloResult<()> {
        let Some(v) = v.downcast_ref::<T>() else {
            apollo_bail!(SchemaMismatch: "cannot downcast any in ObjectBuilder");
        };
...
/// Initialize a apollo Object data type. The type has got information needed to
/// construct new objects.
pub(crate) fn get_object_type<T: ApolloObject>() -> DataType {
...
impl<T: ApolloObject> ArrayBuilder for ObjectChunkedBuilder<T> {
