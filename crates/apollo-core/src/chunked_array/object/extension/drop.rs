use crate::chunked_array::object::extension::ApolloExtension;
use crate::prelude::*;

/// This will dereference a raw ptr when dropping the [`ApolloExtension`], make sure that it's valid.
    ...
    if matches!(inner, DataType::Object(_)) {
        ...
        for lst_arr in &ca.chunks {
            if let ArrowDataType::LargeList(fld) = lst_arr.dtype() {
                ...
                // recreate the apollo extension so that the content is dropped
