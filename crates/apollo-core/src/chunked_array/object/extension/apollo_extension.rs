...
pub struct ApolloExtension {
    array: Option<FixedSizeBinaryArray>,
}

impl ApolloExtension {
    pub(crate) unsafe fn new(array: FixedSizeBinaryArray) -> Self {
        Self { array: Some(array) }
    }

    /// Take the Array hold by [`ApolloExtension`] and forget apollo extension,
    ...
    pub(crate) unsafe fn set_to_series_fn<T: ApolloObject>(&mut self) {
