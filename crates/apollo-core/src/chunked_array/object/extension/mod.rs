pub(crate) mod drop;
pub(super) mod list;
pub(crate) mod apollo_extension;
...
use apollo_extension::ApolloExtension;
...
static APOLLO_ALLOW_EXTENSION: RelaxedCell<bool> = RelaxedCell::new_bool(false);

/// Control whether extension types may be created.
///
/// If the environment variable APOLLO_ALLOW_EXTENSION is set, this function has no effect.
pub fn set_apollo_allow_extension(toggle: bool) {
    APOLLO_ALLOW_EXTENSION.store(toggle)
}
...
/// Create an extension Array that can be sent to arrow and (once wrapped in [`ApolloExtension`] will
...
pub(crate) fn create_extension<I: Iterator<Item = Option<T>> + TrustedLen, T: Sized + Default>(
    ...
    let env = "APOLLO_ALLOW_EXTENSION";
    if !(APOLLO_ALLOW_EXTENSION.load() || std::env::var(env).is_ok()) {
    ...
    let extension_type = ArrowDataType::Extension(Box::new(ExtensionType {
        name: PlSmallStr::from_static(APOLLO_OBJECT_EXTENSION_NAME),
        inner: physical_type,
    ...
    unsafe { ApolloExtension::new(array) }
}
