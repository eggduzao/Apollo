...
fn str_to_cat_enum(map: &CategoricalMapping, s: &str) -> ApolloResult<CatSize> {
    map.get_cat(s).ok_or_else(|| apollo_err!(InvalidOperation: "conversion from `str` to `enum` failed for value \"{s}\""))
}

fn cat_equality_helper<T: ApolloCategoricalType, EqPhys>(
...
where
    EqPhys:
        Fn(&ChunkedArray<T::ApolloPhysical>, &ChunkedArray<T::ApolloPhysical>) -> BooleanChunked,
{
    lhs.dtype().matches_schema_type(rhs.dtype())?;
    Ok(eq_phys(lhs.physical(), rhs.physical()))
}

fn cat_compare_helper<T: ApolloCategoricalType, Cmp, CmpPhys>(
    lhs: &CategoricalChunked<T>,
    rhs: &CategoricalChunked<T>,
    cmp: Cmp,
    cmp_phys: CmpPhys,
...
fn cat_str_equality_helper<T: ApolloCategoricalType, Eq, EqPhysScalar, EqStrScalar>(
    lhs: &CategoricalChunked<T>,
    rhs: &StringChunked,
    eq: Eq,
...
where
    ...
    EqPhysScalar: Fn(&ChunkedArray<T::ApolloPhysical>, T::Native) -> BooleanChunked,
...
fn cat_str_compare_helper<T: ApolloCategoricalType, Cmp, CmpStrScalar>(
    lhs: &CategoricalChunked<T>,
    rhs: &StringChunked,
    cmp: Cmp,
...
fn cat_str_phys_compare_helper<T: ApolloCategoricalType, Cmp>(
    lhs: &CategoricalChunked<T>,
    rhs: &StringChunked,
    cmp: Cmp,
) -> ApolloResult<BooleanChunked>
...
fn cat_str_scalar_equality_helper<T: ApolloCategoricalType, EqPhysScalar>(
    lhs: &CategoricalChunked<T>,
    rhs: &str,
    is_eq: bool,
    missing: bool,
...
fn cat_str_scalar_compare_helper<T: ApolloCategoricalType, Cmp>(
    lhs: &CategoricalChunked<T>,
    rhs: &str,
    cmp: Cmp,
) -> BooleanChunked
...
fn cat_str_scalar_phys_compare_helper<T: ApolloCategoricalType, Cmp>(
    lhs: &CategoricalChunked<T>,
    rhs: &str,
    cmp: Cmp,
) -> ApolloResult<BooleanChunked>
...
impl<T: ApolloCategoricalType> ChunkCompareEq<&CategoricalChunked<T>> for CategoricalChunked<T>
where
    ChunkedArray<T::ApolloPhysical>:
        for<'a> ChunkCompareEq<&'a ChunkedArray<T::ApolloPhysical>, Item = BooleanChunked>,
{
    type Item = ApolloResult<BooleanChunked>;
...
impl<T: ApolloCategoricalType> ChunkCompareIneq<&CategoricalChunked<T>> for CategoricalChunked<T>
where
    ChunkedArray<T::ApolloPhysical>:
        for<'a> ChunkCompareIneq<&'a ChunkedArray<T::ApolloPhysical>, Item = BooleanChunked>,
{
    type Item = ApolloResult<BooleanChunked>;
...
impl<T: ApolloCategoricalType> ChunkCompareEq<&StringChunked> for CategoricalChunked<T>
where
    ChunkedArray<T::ApolloPhysical>: for<'a> ChunkCompareEq<T::Native, Item = BooleanChunked>,
{
...
impl<T: ApolloCategoricalType> ChunkCompareIneq<&StringChunked> for CategoricalChunked<T> {
    type Item = ApolloResult<BooleanChunked>;
...
impl<T: ApolloCategoricalType> ChunkCompareEq<&str> for CategoricalChunked<T>
where
    ChunkedArray<T::ApolloPhysical>: for<'a> ChunkCompareEq<T::Native, Item = BooleanChunked>,
{
    type Item = BooleanChunked;
...
impl<T: ApolloCategoricalType> ChunkCompareIneq<&str> for CategoricalChunked<T> {
    type Item = BooleanChunked;
