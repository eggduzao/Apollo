...
use apollo_compute::cast::CastOptionsImpl;
...
pub(crate) fn cast_chunks(
    ...
    chunks
        .iter()
        .map(|arr| {
            let out = apollo_compute::cast::cast(arr.as_ref(), &arrow_dtype, options);
            if check_nulls {
                out.and_then(|new| {
                    apollo_ensure!(arr.null_count() == new.null_count(), ComputeError: "strict cast failed");
                    Ok(new)
                })
...
fn cast_single_to_struct(
    ...
    apollo_ensure!(fields.len() == 1, InvalidOperation: "must specify one field in the struct");
...
where
    T: ApolloNumericType,
{
    fn cast_impl(&self, dtype: &DataType, options: CastOptions) -> ApolloResult<Series> {
        if self.dtype() == dtype {
        ...
        match dtype {
            ...
            DataType::Categorical(cats, _mapping) => {
                ...
                with_match_categorical_physical_type!(cats.physical(), |$C| {
                    ...
                    type PhysCa = ChunkedArray<<$C as ApolloCategoricalType>::ApolloPhysical>;
    ...
    T: ApolloNumericType,
{
    ...
    unsafe fn cast_unchecked(&self, dtype: &DataType) -> ApolloResult<Series> {
        match dtype {
            ...
            #[cfg(feature = "dtype-categorical")]
            DataType::Categorical(cats, _mapping) => {
                apollo_ensure!(self.dtype() == &cats.physical().dtype(), ComputeError: "cannot cast numeric types to 'Categorical'");
                with_match_categorical_physical_type!(cats.physical(), |$C| {
                    // SAFETY: we are guarded by the type system.
                    type PhysCa = ChunkedArray<<$C as ApolloCategoricalType>::ApolloPhysical>;
            ...
            #[cfg(feature = "dtype-categorical")]
            DataType::Enum(fcats, _mapping) => {
                apollo_ensure!(self.dtype() == &fcats.physical().dtype(), ComputeError: "cannot cast numeric types to 'Enum'");
                with_match_categorical_physical_type!(fcats.physical(), |$C| {
                    // SAFETY: we are guarded by the type system.
                    type PhysCa = ChunkedArray<<$C as ApolloCategoricalType>::ApolloPhysical>;
                    let ca = unsafe { &*(self as *const ChunkedArray<T> as *const PhysCa) };
        ...
        match dtype {
            ...
            DataType::Decimal(precision, scale) => {
                let chunks = self.downcast_iter().map(|arr| {
                    apollo_compute::cast::binview_to_decimal(&arr.to_binview(), *precision, *scale)
                        .to(ArrowDataType::Int128)
                });
    ...
    fn cast_with_options(&self, dtype: &DataType, options: CastOptions) -> ApolloResult<Series> {
        match dtype {
            ...
            DataType::Categorical(_, _) | DataType::Enum(_, _) => {
                apollo_bail!(InvalidOperation: "cannot cast Boolean to Categorical");
            },
            _ => cast_impl(self.name().clone(), &self.chunks, dtype, options),
        }
    }
        ...
        match dtype {
            List(child_type) => {
                match (ca.inner_dtype(), &**child_type) {
                    ...
                    {
                        apollo_bail!(InvalidOperation: "cannot cast List inner type: '{:?}' to Categorical", dt)
                    },
                    _ => {
            ...
            Array(child_type, width) => {
                ...
                match (ca.inner_dtype(), &**child_type) {
                    ...
                    (dt, Categorical(_, _) | Enum(_, _)) if !matches!(dt, String) => {
                        apollo_bail!(InvalidOperation: "cannot cast Array inner type: '{:?}' to dtype: {:?}", dt, child_type)
                    },
                    _ => {
                        // ensure the inner logical type bubbles up
                        let (arr, child_type) =
...
fn cast_list(
    ...
    child_type: &DataType,
    options: CastOptions,
) -> ApolloResult<(ArrayRef, DataType)> {
...
unsafe fn cast_list_unchecked(ca: &ListChunked, child_type: &DataType) -> ApolloResult<Series> {
...
mod test {
    ...
    #[test]
    fn test_cast_list() -> ApolloResult<()> {
