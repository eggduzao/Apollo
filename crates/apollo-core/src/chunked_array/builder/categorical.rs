...
pub struct CategoricalChunkedBuilder<T: ApolloCategoricalType> {
    name: PlSmallStr,
    dtype: DataType,
...
impl<T: ApolloCategoricalType> CategoricalChunkedBuilder<T> {
    ...
    pub fn append_str(&mut self, val: &str) -> ApolloResult<()> {
        let cat = if self.is_enum {
            self.mapping.get_cat(val).ok_or_else(|| {
                apollo_err!(ComputeError: "attempted to insert '{val}' into Enum which does not contain this string")
            })?
        } else {
