...
impl<'a> AnonymousListBuilder<'a> {
    ...
    pub fn append_opt_series(&mut self, opt_s: Option<&'a Series>) -> ApolloResult<()> {
        match opt_s {
    ...
    pub fn append_series(&mut self, s: &'a Series) -> ApolloResult<()> {
        match (s.dtype(), &self.inner_dtype) {
            (DataType::Null, _) => {},
            (dt, None) => self.inner_dtype = Some(dt.clone()),
            (dt, Some(set_dt)) => {
                apollo_bail!(ComputeError: "dtypes don't match, got {}, expected: {}", dt.pretty_format(), set_dt.pretty_format());
            },
        }
        if s.is_empty() {
            self.append_empty();
...
impl ListBuilderTrait for AnonymousOwnedListBuilder {
    fn append_series(&mut self, s: &Series) -> ApolloResult<()> {
        match (s.dtype(), &self.inner_dtype) {
            (DataType::Null, _) => {},
            (dt, None) => self.inner_dtype = Some(dt.clone()),
            (dt, Some(set_dt)) => {
                apollo_ensure!(dt == set_dt, ComputeError: "dtypes don't match, got {}, expected: {}", dt.pretty_format(), set_dt.pretty_format());
            },
        }
        if s.is_empty() {
            self.append_empty();
