...
use apollo_row::{RowEncodingContext, RowEncodingOptions, RowsEncoded, convert_columns};
...
pub fn encode_rows_vertical_par_unordered(by: &[Column]) -> ApolloResult<BinaryOffsetChunked> {
    ...
    let chunks = POOL.install(|| chunks.collect::<ApolloResult<Vec<_>>>());
...
///
/// This should be given the logical type in order to communicate Apollo datatype information down
...
pub fn get_row_encoding_context(dtype: &DataType) -> Option<RowEncodingContext> {
    match dtype {
        ...
        DataType::Categorical(_, mapping) | DataType::Enum(_, mapping) => {
            use apollo_row::RowEncodingCategoricalContext;
...
pub fn encode_rows_unordered(by: &[Column]) -> ApolloResult<BinaryOffsetChunked> {
...
pub fn _get_rows_encoded_unordered(by: &[Column]) -> ApolloResult<RowsEncoded> {
    ...
    let chunks = ca
        ...
        .map(|array| {
            let decoded_arrays = unsafe {
                apollo_row::decode::decode_rows_from_binary(array, opts, &ctxts, &dtypes, &mut rows)
            };
