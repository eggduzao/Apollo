...
impl<'a, T: ApolloDataType> GetInner for &'a ChunkedArray<T> {
...
impl<'a, T: ApolloDataType> GetInner for NonNull<&'a ChunkedArray<T>> {
...
struct LexicalCategorical<'a, T: ApolloCategoricalType> {
    mapping: &'a CategoricalMapping,
    cats: &'a ChunkedArray<T::ApolloPhysical>,
}

#[cfg(feature = "dtype-categorical")]
impl<'a, T: ApolloCategoricalType> GetInner for LexicalCategorical<'a, T> {
...
#[cfg(feature = "dtype-categorical")]
impl<'a, T: ApolloCategoricalType> IntoTotalOrdInner<'a> for &'a CategoricalChunked<T> {
    fn into_total_ord_inner(self) -> Box<dyn TotalOrdInner + 'a> {
