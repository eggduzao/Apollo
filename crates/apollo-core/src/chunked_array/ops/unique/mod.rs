...
use apollo_compute::unique::BooleanUniqueKernelState;
use apollo_utils::total_ord::{ToTotalOrd, TotalHash, TotalOrdWrap};
...
#[cfg(feature = "object")]
impl<T: ApolloObject> ChunkUnique for ObjectChunked<T> {
    fn unique(&self) -> ApolloResult<ChunkedArray<ObjectType<T>>> {
        apollo_bail!(opq = unique, self.dtype());
    }

    fn arg_unique(&self) -> ApolloResult<IdxCa> {
        apollo_bail!(opq = arg_unique, self.dtype());
    }
...
impl ChunkUnique for BooleanChunked {
    fn unique(&self) -> ApolloResult<Self> {
        use apollo_compute::unique::RangedUniqueKernel;
