...
macro_rules! impl_scatter_with {
    ($self:ident, $builder:ident, $idx:ident, $f:ident) => {{
        ...
        for current_idx in $idx.into_iter().map(|i| i as usize) {
            apollo_ensure!(current_idx < $self.len(), oob = current_idx, $self.len());
    ...
    T: ApolloNumericType,
{
        ...
        if !self.has_nulls() {
            if let Some(value) = value {
                ...
                // Other fast path. Slightly slower as it does not do a memcpy.
                    ...
                    idx.into_iter().try_for_each::<_, ApolloResult<_>>(|idx| {
                        let val = data
                            .get_mut(idx as usize)
                            .ok_or_else(|| apollo_err!(oob = idx as usize, self.len()))?;
                        *val = value;
                        Ok(())
                    })?;
    ...
    fn set(&'a self, mask: &BooleanChunked, value: Option<T::Native>) -> ApolloResult<Self> {
        check_bounds!(self, mask);

        // Fast path uses the kernel in apollo-arrow.
...
impl<'a> ChunkSet<'a, bool, bool> for BooleanChunked {
    ...
    fn set(&'a self, mask: &BooleanChunked, value: Option<bool>) -> ApolloResult<Self> {
        let mask = mask.rechunk();
        let mask = mask.downcast_as_array();
        let mask = mask.true_and_valid();
        let iter = mask.true_idx_iter();
...
impl<'a> ChunkSet<'a, &'a str, String> for StringChunked {
    ...
    fn set(&'a self, mask: &BooleanChunked, value: Option<&'a str>) -> ApolloResult<Self>
    where
        Self: Sized,
    {
        check_bounds!(self, mask);
...
impl<'a> ChunkSet<'a, &'a [u8], Vec<u8>> for BinaryChunked {
    ...
    fn set(&'a self, mask: &BooleanChunked, value: Option<&'a [u8]>) -> ApolloResult<Self>
    where
        Self: Sized,
    {
        check_bounds!(self, mask);
