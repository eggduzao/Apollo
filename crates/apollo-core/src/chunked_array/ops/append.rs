...
where
    T: ApolloDataType<IsNested = FalseT, IsObject = FalseT>,
    for<'a> T::Physical<'a>: TotalOrd,
{
    ...
    pub fn append(&mut self, other: &Self) -> ApolloResult<()> {
        update_sorted_flag_before_append::<T>(self, other);
        let len = self.len();
        self.length = self
            .length
            .checked_add(other.length)
            .ok_or_else(|| apollo_err!(ComputeError: LENGTH_LIMIT_MSG))?;
    ...
    ///
    /// See also [`extend`](Self::extend) for appends to the underlying memory
    pub fn append_owned(&mut self, mut other: Self) -> ApolloResult<()> {
        update_sorted_flag_before_append::<T>(self, &other);
        let len = self.len();
        self.length = self
            .length
...
impl<T: ApolloCategoricalType> CategoricalChunked<T> {
    ...
    pub fn append_owned(&mut self, other: Self) -> ApolloResult<()> {
        assert!(self.dtype() == other.dtype());
