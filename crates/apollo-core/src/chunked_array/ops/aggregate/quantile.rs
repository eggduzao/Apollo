...
fn quantile_slice<T: ToPrimitive + TotalOrd + Copy>(
    ...
    quantile: f64,
    method: QuantileMethod,
) -> ApolloResult<Option<f64>> {
    apollo_ensure!((0.0..=1.0).contains(&quantile),
...
where
    ...
    T::Native: TotalOrd,
{
    fn quantile(&self, quantile: f64, method: QuantileMethod) -> ApolloResult<Option<f64>> {
...
macro_rules! impl_chunk_quantile_for_float_chunked {
    ($T:ty, $CA:ty) => {
        impl ChunkQuantile<$T> for $CA {
            fn quantile(&self, quantile: f64, method: QuantileMethod) -> ApolloResult<Option<$T>> {
                // in case of sorted data, the sort is free, so don't take quickselect route
...
#[cfg(feature = "object")]
impl<T: ApolloObject> ChunkQuantile<Series> for ObjectChunked<T> {}
