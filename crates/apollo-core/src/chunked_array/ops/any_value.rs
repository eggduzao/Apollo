    ...
    match dtype {
        ...
        DataType::Categorical(cats, mapping) => {
            with_match_categorical_physical_type!(cats.physical(), |$C| {
                type A = <$C as ApolloDataType>::Array;
...
macro_rules! get_any_value {
    ($self:ident, $index:expr) => {{
        if $index >= $self.len() {
            apollo_bail!(oob = $index, $self.len());
        }
        // SAFETY:
        // bounds are checked
...
#[cfg(feature = "object")]
impl<T: ApolloObject> ChunkAnyValue for ObjectChunked<T> {
    #[inline]
    unsafe fn get_any_value_unchecked(&self, index: usize) -> AnyValue<'_> {
...
impl ChunkAnyValue for NullChunked {
    ...
    fn get_any_value(&self, _index: usize) -> ApolloResult<AnyValue<'_>> {
        Ok(AnyValue::Null)
    }
}
