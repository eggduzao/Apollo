...
fn err_fill_null() -> ApolloError {
    apollo_err!(ComputeError: "could not determine the fill value")
}
...
impl Series {
    ...
    /// example();
    /// ```
    pub fn fill_null(&self, strategy: FillNullStrategy) -> ApolloResult<Series> {
        // Nothing to fill.
        ...
        match strategy {
            ...
            FillNullStrategy::One if self.dtype().is_decimal() => {
                use apollo_compute::decimal::i128_to_dec128;

                let ca = self.decimal().unwrap();
                let precision = ca.precision();
                let scale = ca.scale();
                let fill_value = i128_to_dec128(1, precision, scale).ok_or_else(|| {
                    apollo_err!(ComputeError: "value '1' is out of range for Decimal({precision}, {scale})")
                })?;
            ...
            _ => {
                ...
                let out = match s.dtype() {
                    ...
                    dt => {
                        apollo_bail!(InvalidOperation: "fill null strategy not yet supported for dtype: {}", dt)
                    },
                }?;
...
fn fill_forward_gather(s: &Series) -> ApolloResult<Series> {
...
fn fill_forward_gather_limit(s: &Series, limit: IdxSize) -> ApolloResult<Series> {
    fill_with_gather(s, |validity| {
        let mut last_valid = 0;
...
fn fill_backward_gather(s: &Series) -> ApolloResult<Series> {
    fill_with_gather(s, |validity| {
...
fn fill_backward_gather_limit(s: &Series, limit: IdxSize) -> ApolloResult<Series> {
    fill_with_gather(s, |validity| {
        let last = validity.len() as IdxSize - 1;
        let mut last_valid = last;
        let mut conseq_invalid_count = 0;
...
fn fill_null_bool(ca: &BooleanChunked, strategy: FillNullStrategy) -> ApolloResult<Series> {
    match strategy {
        ...
        FillNullStrategy::Max => ca
            ...
            .map(|ca| ca.into_series()),
        FillNullStrategy::Mean => apollo_bail!(opq = mean, "Boolean"),
...
fn fill_null_binary(ca: &BinaryChunked, strategy: FillNullStrategy) -> ApolloResult<BinaryChunked> {
    match strategy {
        FillNullStrategy::Min => {
        ...
        FillNullStrategy::Forward(_) => unreachable!(),
        FillNullStrategy::Backward(_) => unreachable!(),
        strat => apollo_bail!(InvalidOperation: "fill-null strategy {:?} is not supported", strat),
    }
}
