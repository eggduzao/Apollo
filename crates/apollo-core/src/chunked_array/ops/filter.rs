use apollo_compute::filter::filter as filter_fn;
...
where
    T: ApolloDataType<IsObject = FalseT>,
{
...
where
    T: ApolloObject,
{
    fn filter(&self, filter: &BooleanChunked) -> ApolloResult<ChunkedArray<ObjectType<T>>>
    where
        Self: Sized,
    {
        // Broadcast.
