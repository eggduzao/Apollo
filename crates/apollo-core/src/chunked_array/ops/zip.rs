...
use apollo_compute::if_then_else::{IfThenElseKernel, if_then_else_validity};
...
fn if_then_else_broadcast_mask<T: ApolloDataType>(
    mask: bool,
    ...
    ChunkedArray<T>: ChunkExpandAtIndex<T>,
{
    ...
    let ret = match (src.len(), other.len()) {
        ...
        _ => apollo_bail!(ShapeMismatch: SHAPE_MISMATCH_STR),
    };
...
where
    T: ApolloDataType<IsStruct = FalseT>,
    ...
    ChunkedArray<T>: ChunkExpandAtIndex<T>,
{
    fn zip_with(
        ...
        // Broadcast neither.
            ...
            apollo_ensure!(mask.len() == if_true.len(), ShapeMismatch: SHAPE_MISMATCH_STR);
        ...
        // Broadcast true value.
            ...
            apollo_ensure!(mask.len() == if_false.len(), ShapeMismatch: SHAPE_MISMATCH_STR);
        ...
        // Broadcast false value.
            ...
            apollo_bail!(ShapeMismatch: SHAPE_MISMATCH_STR)
        };
...
#[cfg(feature = "object")]
impl<T: ApolloObject> IfThenElseKernel for ObjectArray<T> {
    type Scalar<'a> = &'a T;
...
impl ChunkZip<StructType> for StructChunked {
    ...
    ) -> ApolloResult<ChunkedArray<StructType>> {
