...
pub trait ChunkAnyValue {
    ...
    /// Get a single value. Beware this is slow.
    fn get_any_value(&self, index: usize) -> ApolloResult<AnyValue<'_>>;
}
...
pub trait ChunkExplode {
    fn explode(&self, options: ExplodeOptions) -> ApolloResult<Series> {
        self.explode_and_offsets(options).map(|t| t.0)
    }
    fn offsets(&self) -> ApolloResult<OffsetsBuffer<i64>>;
    fn explode_and_offsets(
        &self,
        options: ExplodeOptions,
    ) -> ApolloResult<(Series, OffsetsBuffer<i64>)>;
}
...
pub trait ChunkRollApply: AsRefDataType {
    fn rolling_map(
        &self,
        f: &dyn Fn(&Series) -> ApolloResult<Series>,
...
pub trait ChunkTake<Idx: ?Sized>: ChunkTakeUnchecked<Idx> {
    ...
    fn take(&self, indices: &Idx) -> ApolloResult<Self>
    where
        Self: Sized;
}
...
pub trait ChunkSet<'a, A, B> {
    ...
    /// ```
    fn set(&'a self, mask: &BooleanChunked, opt_value: Option<A>) -> ApolloResult<Self>
    where
        Self: Sized;
}
...
pub trait ChunkCast {
    ...
    fn cast(&self, dtype: &DataType) -> ApolloResult<Series> {
        self.cast_with_options(dtype, CastOptions::NonStrict)
    }

    /// Cast a [`ChunkedArray`] to [`DataType`]
    fn cast_with_options(&self, dtype: &DataType, options: CastOptions) -> ApolloResult<Series>;
    ...
    unsafe fn cast_unchecked(&self, dtype: &DataType) -> ApolloResult<Series>;
}
...
pub trait ChunkQuantile<T> {
    ...
    /// Returns `None` if the array is empty or only contains null values.
    fn quantile(&self, _quantile: f64, _method: QuantileMethod) -> ApolloResult<Option<T>> {
        Ok(None)
    }
    /// Aggregate a given set of quantiles of the ChunkedArray.
    /// Returns `None` if the array is empty or only contains null values.
    fn quantiles(&self, quantiles: &[f64], _method: QuantileMethod) -> ApolloResult<Vec<Option<T>>>
    where
        T: Clone,
    {
...
/// ```
/// use apollo_core::prelude::*;
/// fn filter_all_ones(df: &DataFrame) -> ApolloResult<DataFrame> {
///     let mask = df
///     .column("column_a")?
...
pub trait ChunkUnique {
    ...
    fn unique(&self) -> ApolloResult<Self>
    where
        Self: Sized;
...
pub trait ChunkSort<T: ApolloDataType> {
    ...
    fn arg_sort_multiple(
        ...
        apollo_bail!(opq = arg_sort_multiple, T::get_static_dtype());
    }
}
...
pub trait ChunkFillNullValue<T> {
    ...
    fn fill_null_with_values(&self, value: T) -> ApolloResult<Self>
    where
        Self: Sized;
}
...
pub trait ChunkFilter<T: ApolloDataType> {
    ...
    /// ```
    fn filter(&self, filter: &BooleanChunked) -> ApolloResult<ChunkedArray<T>>
    where
        Self: Sized;
}

/// Create a new ChunkedArray filled with values at that index.
pub trait ChunkExpandAtIndex<T: ApolloDataType> {
    /// Create a new ChunkedArray filled with values at that index.
...
impl<T: ApolloNumericType> ChunkExpandAtIndex<T> for ChunkedArray<T>
where
...
#[cfg(feature = "object")]
impl<T: ApolloObject> ChunkExpandAtIndex<ObjectType<T>> for ObjectChunked<T> {
...
pub trait IsFirstDistinct<T: ApolloDataType> {
    fn is_first_distinct(&self) -> ApolloResult<BooleanChunked> {
        apollo_bail!(opq = is_first_distinct, T::get_static_dtype());
    }
}

#[cfg(feature = "is_last_distinct")]
/// Mask the last unique values as `true`
pub trait IsLastDistinct<T: ApolloDataType> {
    fn is_last_distinct(&self) -> ApolloResult<BooleanChunked> {
        apollo_bail!(opq = is_last_distinct, T::get_static_dtype());
    }
}
