...
use crate::prelude::{ChunkedArray, CompatLevel, ApolloDataType, Series, StringChunked};
...
///
/// Currently unused within Apollo itself, but it's a useful utility for plugin authors.
#[inline]
...
///
/// # Safety
/// Caller must ensure that the returned `ArrayRef` belongs to `T: ApolloDataType`.
#[inline]
...
pub fn try_unary_to_series<T, F>(ca: &ChunkedArray<T>, op: F) -> ApolloResult<Series>
where
    T: ApolloDataType,
    F: FnMut(&T::Array) -> ApolloResult<Box<dyn Array>>,
{
    let chunks = ca
        .downcast_iter()
        .map(op)
...
where
    T: ApolloDataType,
    U: ApolloDataType,
    F: FnMut(&T::Array, &U::Array) -> ApolloResult<Box<dyn Array>>,
{
    let (lhs, rhs) = align_chunks_binary(lhs, rhs);
    let chunks = lhs
        .downcast_iter()
