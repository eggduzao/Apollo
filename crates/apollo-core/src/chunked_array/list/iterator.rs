...
impl ListChunked {
    ...
    pub fn try_apply_amortized_generic<F, K, V>(&self, f: F) -> ApolloResult<ChunkedArray<V>>
    where
        V: ApolloDataType,
        F: FnMut(Option<AmortSeries>) -> ApolloResult<Option<K>> + Copy,
        V::Array: ArrayFromIter<Option<K>>,
    {
        // TODO! make an amortized iter that does not flatten
        self.amortized_iter()
    ...
    where
        ...
        &'a ChunkedArray<T>: IntoIterator<IntoIter = I>,
        I: TrustedLen<Item = Option<T::Physical<'a>>>,
        F: FnMut(Option<AmortSeries>, Option<T::Physical<'a>>) -> ApolloResult<Option<Series>>,
    {
        if self.is_empty() {
            return Ok(self.clone());
        }
    ...
    pub fn try_apply_amortized<F>(&self, mut f: F) -> ApolloResult<Self>
    where
        F: FnMut(AmortSeries) -> ApolloResult<Series>,
    {
        if self.is_empty() {
            return Ok(self.clone());
        }
    ...
    /// # Safety
    /// The closure `F` must return the same dtype as the input.
    pub unsafe fn try_apply_amortized_same_type<F>(&self, mut f: F) -> ApolloResult<Self>
    where
        F: FnMut(AmortSeries) -> ApolloResult<Series>,
    {
        if self.is_empty() {
