...
pub struct Logical<Logical: ApolloDataType, Physical: ApolloDataType> {
    pub phys: ChunkedArray<Physical>,
    pub dtype: DataType,
    _phantom: PhantomData<Logical>,
}

impl<K: ApolloDataType, T: ApolloDataType> Clone for Logical<K, T> {
    fn clone(&self) -> Self {
        Self {
...
impl<K: ApolloDataType, T: ApolloDataType> Logical<K, T> {
    /// # Safety
...
pub trait LogicalType {
    ...
    fn get_any_value(&self, _i: usize) -> ApolloResult<AnyValue<'_>> {
        unimplemented!()
    }
