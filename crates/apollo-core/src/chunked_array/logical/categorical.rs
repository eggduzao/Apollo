...
use num_traits::Zero;
use apollo_utils::hashing::{_boost_hash_combine, folded_multiply};
...
pub type CategoricalChunked<T> = Logical<T, <T as ApolloCategoricalType>::ApolloPhysical>;
...
impl<T: ApolloCategoricalType> CategoricalChunked<T> {
    ...
    pub fn full_null_with_dtype(name: PlSmallStr, length: usize, dtype: DataType) -> Self {
        let phys =
            ChunkedArray::<<T as ApolloCategoricalType>::ApolloPhysical>::full_null(name, length);
    ...
    pub fn from_cats_and_dtype(
        mut cat_ids: ChunkedArray<T::ApolloPhysical>,
        dtype: DataType,
    ) -> Self {
    ...
    pub unsafe fn from_cats_and_dtype_unchecked(
        cat_ids: ChunkedArray<T::ApolloPhysical>,
        dtype: DataType,
    ) -> Self {
    ...
    pub fn iter_str(&self) -> impl ApolloIterator<Item = Option<&str>> {
    ...
    pub fn from_str_iter<'a, I: IntoIterator<Item = Option<&'a str>>>(
        ...
        let arr = <T::ApolloPhysical as ApolloDataType>::Array::from_vec(cat_ids)
            .with_validity(validity.into_opt_validity());
        let phys = ChunkedArray::<T::ApolloPhysical>::with_chunk(name, arr);
...
impl<T: ApolloCategoricalType> LogicalType for CategoricalChunked<T> {
    fn dtype(&self) -> &DataType {
        &self.dtype
    }

    fn get_any_value(&self, i: usize) -> ApolloResult<AnyValue<'_>> {
        apollo_ensure!(i < self.len(), oob = i, self.len());
    ...
    fn cast_with_options(&self, dtype: &DataType, options: CastOptions) -> ApolloResult<Series> {
        ...
        match dtype {
            ...
            _ => apollo_bail!(ComputeError: "cannot cast categorical types to {dtype:?}"),
        }
    }
}

impl<T: ApolloCategoricalType> VecHash for CategoricalChunked<T>
where
    ChunkedArray<<T as ApolloCategoricalType>::ApolloPhysical>: VecHash,
{
    fn vec_hash(
        &self,
