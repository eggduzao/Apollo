...
impl StructChunked {
    pub fn from_columns(name: PlSmallStr, length: usize, fields: &[Column]) -> ApolloResult<Self> {
        Self::from_series(
            name,
            length,
    ...
    pub fn from_series<'a, I: ExactSizeIterator<Item = &'a Series> + Clone>(
        ...
        for s in fields.clone() {
            ...
            apollo_ensure!(names.insert(s.name()), duplicate_field = s.name());

            match s.dtype() {
                #[cfg(feature = "object")]
                DataType::Object(_) => {
                    apollo_bail!(InvalidOperation: "nested objects are not allowed")
                },
                _ => {},
            }
        }
    ...
    pub(crate) unsafe fn cast_unchecked(&self, dtype: &DataType) -> ApolloResult<Series> {
        if dtype == self.dtype() {
    ...
    pub fn cast(&self, dtype: &DataType) -> ApolloResult<Series> {
        self.cast_with_options(dtype, CastOptions::NonStrict)
    }

    pub fn _apply_fields<F>(&self, mut func: F) -> ApolloResult<Self>
    where
    ...
    pub fn try_apply_fields<F>(&self, func: F) -> ApolloResult<Self>
    where
        F: FnMut(&Series) -> ApolloResult<Series>,
    {
        let fields = self
            .fields_as_series()
            .iter()
    ...
    pub fn get_row_encoded_array(&self, options: SortOptions) -> ApolloResult<BinaryArray<i64>> {
        let c = self.clone().into_column();
        _get_rows_encoded_arr(&[c], &[options.descending], &[options.nulls_last], false)
    }

    pub fn get_row_encoded(&self, options: SortOptions) -> ApolloResult<BinaryOffsetChunked> {
        let c = self.clone().into_column();
    ...
    pub fn field_by_name(&self, name: &str) -> ApolloResult<Series> {
        self.fields_as_series()
            .into_iter()
            .find(|s| s.name().as_str() == name)
            .ok_or_else(|| apollo_err!(StructFieldNotFound: "{}", name))
    }
