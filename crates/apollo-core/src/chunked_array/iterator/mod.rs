...
{
    #[inline]
    pub fn iter(&self) -> impl ApolloIterator<Item = Option<T::Physical<'_>>> {
...
/// A [`ApolloIterator`] is an iterator over a [`ChunkedArray`] which contains apollo types. A [`ApolloIterator`]
/// must implement [`ExactSizeIterator`] and [`DoubleEndedIterator`].
pub trait ApolloIterator:
    ExactSizeIterator + DoubleEndedIterator + Send + Sync + TrustedLen
{
}
unsafe impl<I> TrustedLen for Box<dyn ApolloIterator<Item = I> + '_> {}

/// Implement [`ApolloIterator`] for every iterator that implements the needed traits.
impl<T: ?Sized> ApolloIterator for T where
    ...
    T: ApolloNumericType,
{
    ...
    type IntoIter = Box<dyn ApolloIterator<Item = Self::Item> + 'a>;
...
#[cfg(feature = "object")]
impl<T: ApolloObject> ObjectChunked<T> {
    #[allow(clippy::wrong_self_convention)]
    #[doc(hidden)]
    pub fn into_no_null_iter(
        &self,
