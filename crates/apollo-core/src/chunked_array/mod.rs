...
use apollo_compute::filter::filter_with_bitmap;
...
/// ```rust
/// # use apollo_core::prelude::*;
/// fn to_chunked_array(series: &Series) -> ApolloResult<&Int32Chunked>{
///     series.i32()
/// }
///
...
///
/// To be able to append data, Apollo uses chunks to append new memory locations, hence the [`ChunkedArray<T>`] data structure.
...
/// - The [`ApolloDataType`] `T` should always map to the correct [`ArrowDataType`] in the [`ArrayRef`]
///   chunks.
...
pub struct ChunkedArray<T: ApolloDataType> {
...
impl<T: ApolloDataType> ChunkedArray<T> {
    ...
    pub fn retain_flags_from<U: ApolloDataType>(
        &mut self,
        from: &ChunkedArray<U>,
...
impl<T: ApolloDataType> AsRefDataType for ChunkedArray<T> {
...
pub(crate) trait AsSinglePtr: AsRefDataType {
    ...
    fn as_single_ptr(&mut self) -> ApolloResult<usize> {
        apollo_bail!(opq = as_single_ptr, self.as_ref_dtype());
    }
}
...
#[cfg(feature = "object")]
impl<T: ApolloObject> AsSinglePtr for ObjectChunked<T> {}

pub enum ChunkedArrayLayout<'a, T: ApolloDataType> {
    ...
    T: ApolloNumericType,
{
    ...
    pub fn cont_slice(&self) -> ApolloResult<&[T::Native]> {
        apollo_ensure!(
...
impl<T: ApolloDataType> Clone for ChunkedArray<T> {
    fn clone(&self) -> Self {
        ChunkedArray {
...
impl<T: ApolloDataType> AsRef<ChunkedArray<T>> for ChunkedArray<T> {
...
pub(crate) fn to_primitive<T: ApolloNumericType>(
    values: Vec<T::Native>,
...
pub(crate) fn to_array<T: ApolloNumericType>(
    values: Vec<T::Native>,
    validity: Option<Bitmap>,
) -> ArrayRef {
...
impl<T: ApolloDataType> Default for ChunkedArray<T> {
    fn default() -> Self {
