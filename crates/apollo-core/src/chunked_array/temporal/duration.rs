...
impl DurationChunked {
    ...
    /// strings are not supported, only the specifiers 'iso' and 'apollo'.
    pub fn to_string(&self, format: &str) -> ApolloResult<StringChunked> {
        ...
        match format {
                    ...
                    self.phys
                        .apply_nonnull_values_generic(DataType::String, |v: i64| {
                            ...
                            fmt_duration_string(&mut s, v, self.time_unit())
                                .map_err(|e| apollo_err!(ComputeError: "{:?}", e))
            ...
            _ => {
                apollo_bail!(
                    InvalidOperation: "format {:?} not supported for Duration type (expected one of 'iso' or 'apollo')",
                    format
                )
            },
        }
