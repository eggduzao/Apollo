...
pub fn get_strftime_format(fmt: &str, dtype: &DataType) -> ApolloResult<String> {
    if fmt == "apollo" && !matches!(dtype, DataType::Duration(_)) {
        apollo_bail!(InvalidOperation: "'apollo' is not a valid `to_string` format for {} dtype expressions", dtype);
    } else {
