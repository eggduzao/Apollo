...
impl DatetimeChunked {
    ...
    pub fn to_string(&self, format: &str) -> ApolloResult<StringChunked> {
        ...
        let mut ca: StringChunked = match self.time_zone() {
            ...
            Some(time_zone) => {
                ...
                |_| apollo_err!(ComputeError: "cannot format timezone-aware Datetime with format '{}'", format),
                )?
            },
            _ => {
                ...
                |_| apollo_err!(ComputeError: "cannot format timezone-naive Datetime with format '{}'", format),
                )?
            },
        };
    ...
    ///
    /// Alias for `to_string`.
    pub fn strftime(&self, format: &str) -> ApolloResult<StringChunked> {
    ...
    #[cfg(feature = "timezones")]
    pub fn set_time_zone(&mut self, time_zone: TimeZone) -> ApolloResult<()> {
