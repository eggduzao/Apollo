    ...
    T: ApolloNumericType,
{
    ...
    /// as an [ndarray]
    pub fn to_ndarray(&self) -> ApolloResult<ArrayView1<'_, T::Native>> {
...
impl ListChunked {
    ...
    pub fn to_ndarray<N>(&self) -> ApolloResult<Array2<N::Native>>
    where
        N: ApolloNumericType,
    {
        apollo_ensure!(
        ...
        let series = iter
            .next()
            .ok_or_else(|| apollo_err!(NoData: "unable to create ndarray of empty ListChunked"))?;
...
impl DataFrame {
    ...
    /// ```
    pub fn to_ndarray<N>(&self, ordering: IndexOrder) -> ApolloResult<Array2<N::Native>>
    where
        N: ApolloNumericType,
    {
...
mod test {
    ...
    #[test]
    fn test_ndarray_from_ca() -> ApolloResult<()> {
    ...
    #[test]
    fn test_ndarray_from_df_order_fortran() -> ApolloResult<()> {
    ...
    #[test]
    fn test_ndarray_from_df_order_c() -> ApolloResult<()> {
        let df = df!["a"=> [1.0, 2.0, 3.0],
            "b" => [2.0, 3.0, 4.0]
        ]?;
