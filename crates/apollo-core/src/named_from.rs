...
macro_rules! impl_named_from_owned {
    ($type:ty, $apollo_type:ident) => {
        impl NamedFromOwned<$type> for Series {
            fn from_vec(name: PlSmallStr, v: $type) -> Self {
                ChunkedArray::<$apollo_type>::from_vec(name, v).into_series()
            }
        }
    };
}
...
macro_rules! impl_named_from {
    ($type:ty, $apollo_type:ident, $method:ident) => {
        impl<T: AsRef<$type>> NamedFrom<T, $type> for Series {
            fn new(name: PlSmallStr, v: T) -> Self {
                ChunkedArray::<$apollo_type>::$method(name, v.as_ref()).into_series()
            }
        }
        impl<T: AsRef<$type>> NamedFrom<T, $type> for ChunkedArray<$apollo_type> {
            fn new(name: PlSmallStr, v: T) -> Self {
                ChunkedArray::<$apollo_type>::$method(name, v.as_ref())
            }
        }
    };
}
...
macro_rules! impl_named_from_range {
    ($range:ty, $apollo_type:ident) => {
        impl NamedFrom<$range, $apollo_type> for ChunkedArray<$apollo_type> {
            fn new(name: PlSmallStr, range: $range) -> Self {
                let values = range.collect::<Vec<_>>();
                ChunkedArray::<$apollo_type>::from_vec(name, values)
            }
        }

        impl NamedFrom<$range, $apollo_type> for Series {
            fn new(name: PlSmallStr, range: $range) -> Self {
...
#[cfg(feature = "object")]
impl<T: ApolloObject> NamedFrom<&[T], &[T]> for ObjectChunked<T> {
    fn new(name: PlSmallStr, v: &[T]) -> Self {
...
#[cfg(feature = "object")]
impl<T: ApolloObject, S: AsRef<[Option<T>]>> NamedFrom<S, [Option<T>]> for ObjectChunked<T> {
