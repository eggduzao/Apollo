...
use apollo_compute::decimal::str_to_dec128;
#[cfg(feature = "dtype-categorical")]
use apollo_core::chunked_array::builder::CategoricalChunkedBuilder;
use apollo_core::prelude::*;
use apollo_error::to_compute_err;
#[cfg(any(feature = "dtype-datetime", feature = "dtype-date"))]
use apollo_time::chunkedarray::string::Pattern;
#[cfg(any(feature = "dtype-datetime", feature = "dtype-date"))]
use apollo_time::prelude::string::infer::{
    DatetimeInfer, StrpTimeParser, TryFromWithUnit, infer_pattern_single,
};
#[cfg(feature = "dtype-f16")]
use apollo_utils::float16::pf16;
use apollo_utils::vec::PushUnchecked;

use super::options::CsvEncoding;
use super::parser::{could_be_whitespace_fast, skip_whitespace};
use super::utils::escape_field;

pub(crate) trait PrimitiveParser: ApolloNumericType {
...
where
    T: ApolloNumericType + PrimitiveParser,
{
    #[inline]
    fn parse_bytes(
        &mut self,
        ...
        match T::parse(bytes) {
            ...
            None => {
                if ignore_errors {
                    ...
                    apollo_bail!(ComputeError: "invalid primitive value found during CSV parsing")
                }
            },
        }
        Ok(())
...
impl ParsedBuilder for Utf8Field {
    ...
    fn parse_bytes(
        ...
        let escaped_bytes = if needs_escaping {
            ...
            apollo_ensure!(bytes.len() > 1 && bytes.last() == Some(&self.quote_char), ComputeError: "invalid csv file\n\nField `{}` is not properly escaped.", std::str::from_utf8(bytes).map_err(to_compute_err)?);
                ...
                false => {
                    if matches!(self.encoding, CsvEncoding::LossyUtf8) {
                        ...
                        if needs_escaping && validate_utf8(bytes) {
                            apollo_bail!(ComputeError: "string field is not properly escaped");
                        } else {
                            apollo_bail!(ComputeError: "invalid utf-8 sequence");
                        }
                    }
                },
            }
...
#[cfg(feature = "dtype-categorical")]
pub struct CategoricalField<T: ApolloCategoricalType> {
...
#[cfg(feature = "dtype-categorical")]
impl<T: ApolloCategoricalType> CategoricalField<T> {
    ...
    fn parse_bytes(
        ...
        if validate_utf8(bytes) {
            if needs_escaping {
                apollo_ensure!(bytes.len() > 1, ComputeError: "invalid csv file\n\nField `{}` is not properly escaped.", std::str::from_utf8(bytes).map_err(to_compute_err)?);
        ...
        match str_to_dec128(bytes, self.precision, self.scale, self.decimal_comma) {
            ...
            None => {
                if ignore_errors {
                    ...
                    apollo_bail!(ComputeError: "invalid decimal value found during CSV parsing")
                }
            },
        }
...
#[cfg(any(feature = "dtype-datetime", feature = "dtype-date"))]
pub struct DatetimeField<T: ApolloNumericType> {
    compiled: Option<DatetimeInfer<T>>,
    builder: PrimitiveChunkedBuilder<T>,
}

#[cfg(any(feature = "dtype-datetime", feature = "dtype-date"))]
impl<T: ApolloNumericType> DatetimeField<T> {
        ...
        None => match infer_pattern_single(val) {
            ...
            None => {
                if ignore_errors {
                    ...
                    apollo_bail!(ComputeError: "could not find a 'date/datetime' pattern for '{}'", val)
                }
            },
        },
    };
        ...
        Ok(mut infer) => {
            ...
            let Some(parsed) = parsed else {
                if ignore_errors {
                    ...
                    apollo_bail!(ComputeError: "could not parse '{}' with pattern '{:?}'", val, pattern)
                }
            };
...
impl Builder {
    pub fn into_series(self) -> ApolloResult<Series> {
        let s = match self {
