...
use apollo_utils::select::select_unpredictable;
use rayon::prelude::*;
...
#[cfg(feature = "simd")]
use apollo_utils::clmul::prefix_xorsum_inclusive;
                            ...
                            None => {
                                if bytes.get(read_sol - 1) == Some(&parse_options.eol_char) {
                                    ...
                                    if !truncate_ragged_lines && read_sol < bytes.len() {
                                        apollo_bail!(ComputeError: r#"found more fields than defined in 'Schema'

Consider setting 'truncate_ragged_lines={}'."#, apollo_error::constants::TRUE)
                                    }
