...
use apollo_core::prelude::Schema;
...
fn for_each_line_from_reader(
    ...
    reader: &mut CompressedReader,
    mut line_fn: impl FnMut(Buffer<u8>) -> ApolloResult<LineUse>,
...
fn skip_lines_naive(
    ...
    loop {
        ...
        if bytes_read == 0 {
            if raise_if_empty {
                apollo_bail!(NoData: "specified skip_lines is larger than total number of lines.");
            } else {
                return Ok(Buffer::new());
            }
        }
