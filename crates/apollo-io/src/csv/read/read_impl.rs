...
use apollo_core::utils::{accumulate_dataframes_vertical, handle_casting_failures};
#[cfg(feature = "apollo-time")]
...
impl<'a> CoreReader<'a> {
    ...
    fn get_projection(&mut self) -> ApolloResult<Vec<usize>> {
        ...
        self.projection
            ...
            .map(|mut v| {
                ...
                if let Some(idx) = v.last() {
                    apollo_ensure!(*idx < self.schema.len(), OutOfBounds: "projection index: {} is out of bounds for csv schema with length: {}", idx, self.schema.len())
                }
                Ok(v)
            })
    ...
    fn parse_csv(&mut self, bytes: &[u8]) -> ApolloResult<DataFrame> {
                ...
                if !b.is_empty() {
                    ...
                    s.spawn(move |_| {
                        if check_utf8 && !super::builder::validate_utf8(b) {
                            ...
                            results.push((
                                b.as_ptr() as usize,
                                Err(apollo_err!(ComputeError: "invalid utf-8 sequence")),
                            ));
                            return;
                        }
    ...
    pub fn finish(mut self) -> ApolloResult<DataFrame> {
