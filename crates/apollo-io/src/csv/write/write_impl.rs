...
impl CsvSerializer {
    pub fn new(schema: SchemaRef, options: Arc<SerializeOptions>) -> ApolloResult<Self> {
    ...
    ) -> ApolloResult<&'b mut [Box<ColumnSerializer<'a>>]> {
        serializers.clear();
...
pub fn csv_header(names: &[&str], options: &SerializeOptions) -> ApolloResult<Vec<u8>> {
