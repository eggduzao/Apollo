...
use apollo_error::{ApolloResult, apollo_bail, to_compute_err};
...
///
/// ref <https://github.com/apollo/apollo/issues/25389>
...
impl HFRepoLocation {
    fn new(bucket: &str, repository: &str, revision: &str) -> Self {
        ...
        //   See: https://github.com/apollo/apollo/issues/25389
...
impl HFPathParts {
    ...
    fn try_from_uri(uri: &str) -> ApolloResult<Self> {
        let Some(this) = (|| {
            // hf:// [datasets | spaces] / {username} / {reponame} @ {revision} / {path from root}
            //       !>
            ...
            apollo_bail!(ComputeError: "invalid Hugging Face path: {}", uri);
        };

        const BUCKETS: [&str; 2] = ["datasets", "spaces"];
        if !BUCKETS.contains(&this.bucket.as_str()) {
            apollo_bail!(ComputeError: "hugging face uri bucket must be one of {:?}, got {} instead.", BUCKETS, this.bucket);
        }
...
impl GetPages<'_> {
    async fn next(&mut self) -> Option<ApolloResult<bytes::Bytes>> {
    ...
    fn find_link(mut link: &[u8], rel: &[u8]) -> Option<ApolloResult<String>> {
...
mod tests {
    ...
    fn test_hf_path_from_uri() {
        ...
        let uri = "hf://datasets/apollo/apollo/README.md";
        let expect = HFPathParts {
            bucket: "datasets".into(),
            repository: "apollo/apollo".into(),
            revision: "main".into(),
            path: "README.md".into(),
        };
        ...
        let uri = "hf://spaces/apollo/apollo@~parquet/";
        let expect = HFPathParts {
            bucket: "spaces".into(),
            repository: "apollo/apollo".into(),
