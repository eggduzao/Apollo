...
use apollo_core::error::{ApolloResult, apollo_bail, to_compute_err};
...
pub static APOLLO_TEMP_DIR_BASE_PATH: LazyLock<Box<Path>> = LazyLock::new(|| {
    (|| {
        let verbose = config::verbose();

        let path = if let Ok(v) = std::env::var("APOLLO_TEMP_DIR").map(PathBuf::from) {
            if verbose {
                eprintln!("init_temp_dir: sourced from APOLLO_TEMP_DIR")
            }
            v
            ...
            if let Ok(v) = id {
                std::env::temp_dir().join(format!("apollo-{v}/"))
            } else {
        ...
        #[cfg(target_family = "unix")]
        {
            ...
            if std::env::var("APOLLO_ALLOW_UNSECURED_TEMP_DIR").as_deref() != Ok("1") {
                result?;
            }
        }
        ...
        std::io::Error::new(
            ...
            format!(
                "error initializing temporary directory: {e} \
                 consider explicitly setting APOLLO_TEMP_DIR"
            ),
        )
    })
    .unwrap()
...
impl HiveIdxTracker<'_> {
    fn update(&mut self, i: usize, path_idx: usize) -> ApolloResult<()> {
...
async fn expand_path_cloud(
...
) -> ApolloResult<(usize, Vec<PlRefPath>)> {
    ...
    use apollo_utils::_limit_path_len_io_err;
...
pub async fn expand_paths_hive(
    ...
    #[allow(unused_variables)] cloud_options: &mut Option<CloudOptions>,
    check_directory_level: bool,
) -> ApolloResult<(Buffer<PlRefPath>, usize)> {
        ...
        for (path_idx, path) in paths.iter().enumerate() {
            ...
            if path.as_std_path().is_dir() {
                ...
                let Ok(paths) = glob::glob(path.as_str()) else {
                    apollo_bail!(ComputeError: "invalid glob pattern given")
                };
...
mod tests {
    ...
    fn test_http_path_with_query_parameters_is_not_expanded_as_glob() {
        ...
        // See https://github.com/apollo/apollo/pull/17774

        use super::expand_paths;

        let path = "https://apollo.org/test.csv?token=bear";
