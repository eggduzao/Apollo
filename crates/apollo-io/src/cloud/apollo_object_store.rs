...
use apollo_core::prelude::{InitHashMaps, PlHashMap};
use apollo_error::{ApolloError, ApolloResult};
...
mod inner {
    ...
    use crate::cloud::ApolloObjectStoreBuilder;

    #[derive(Debug)]
    struct Inner {
        store: tokio::sync::Mutex<Arc<dyn ObjectStore>>,
        builder: ApolloObjectStoreBuilder,
    }

    /// Apollo wrapper around [`ObjectStore`] functionality. This struct is cheaply cloneable.
    #[derive(Clone, Debug)]
    pub struct ApolloObjectStore {
        inner: Arc<Inner>,
    ...
    impl ApolloObjectStore {
        ...
        ) -> ApolloResult<Arc<dyn ObjectStore>> {
        ...
        pub async fn try_exec_rebuild_on_err<Fn, Fut, O>(&self, mut func: Fn) -> ApolloResult<O>
        where
            Fn: FnMut(&Arc<dyn ObjectStore>) -> Fut,
            Fut: Future<Output = ApolloResult<O>>,
        {
            ...
            if config::verbose() {
                eprintln!(
                    "[ApolloObjectStore]: got error: {}, will attempt re-build",
                    &orig_err
                );
            }
            ...
            func(&store).await.map_err(|e| {
                if self.inner.builder.is_azure()
                    && std::env::var("APOLLO_AUTO_USE_AZURE_STORAGE_ACCOUNT_KEY").as_deref()
                        != Ok("1")
                {
...
APOLLO_AUTO_USE_AZURE_STORAGE_ACCOUNT_KEY=1 if you would like apollo to try to retrieve \
...
pub use inner::ApolloObjectStore;
...
impl ApolloObjectStore {
    ...
    fn get_buffered_ranges_stream<'a, T: Iterator<Item = Range<usize>>>(
        ...
        path: &'a Path,
        ranges: T,
    ) -> impl StreamExt<Item = ApolloResult<Buffer<u8>>>
    + TryStreamExt<Ok = Buffer<u8>, Error = ApolloError, Item = ApolloResult<Buffer<u8>>>
    + use<'a, T> {
    ...
    pub async fn get_range(&self, path: &Path, range: Range<usize>) -> ApolloResult<Buffer<u8>> {
        if range.is_empty() {
            return Ok(Buffer::new());
        }
                ...
                if parts.len() == 1 {
                    let out = tune_with_concurrency_budget(1, move || async move {
                        let bytes = store
                            .get_range(path, range.start as u64..range.end as u64)
                            .await?;
                        ApolloResult::Ok(Buffer::from_owner(bytes))
                    })
                    .await?;
                    ...
                    ApolloResult::Ok(Buffer::from_vec(combined))
                }
            }
        })
        .await
    ...
    ) -> ApolloResult<PlHashMap<usize, Buffer<u8>>> {
        if ranges.is_empty() {
            ...
            async {
                ...
                tune_with_concurrency_budget(
                    ...
                    || async {
                        ...
                        ApolloResult::Ok(pl_async::Size::from(len as u64))
                    },
                )
                .await?;
    ...
    pub async fn download(&self, path: &Path, file: &mut tokio::fs::File) -> ApolloResult<()> {
        ...
        self.try_exec_rebuild_on_err(|store| {
            ...
            async {
                ...
                file.sync_all().await.map_err(ApolloError::from)?;
    ...
    pub async fn head(&self, path: &Path) -> ApolloResult<ObjectMeta> {
        self.try_exec_rebuild_on_err(|store| {
            let st = store.clone();
