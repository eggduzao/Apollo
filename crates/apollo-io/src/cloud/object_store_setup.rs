...
use apollo_core::config::{self, verbose, verbose_print_sensitive};
use apollo_error::{ApolloError, ApolloResult, apollo_bail, to_compute_err};
use apollo_utils::aliases::PlHashMap;
use apollo_utils::pl_path::{PlPath, PlRefPath};
use apollo_utils::pl_str::PlSmallStr;
use apollo_utils::{format_pl_smallstr, pl_serialize};
use tokio::sync::RwLock;

use super::{CloudLocation, CloudOptions, CloudType, ApolloObjectStore};
...
static OBJECT_STORE_CACHE: LazyLock<RwLock<PlHashMap<Vec<u8>, ApolloObjectStore>>> =
...
pub fn object_path_from_str(path: &str) -> ApolloResult<object_store::path::Path> {
    object_store::path::Path::parse(path).map_err(to_compute_err)
}

#[derive(Debug, Clone)]
pub(crate) struct ApolloObjectStoreBuilder {
    path: PlRefPath,
    cloud_type: CloudType,
    options: Option<CloudOptions>,
}

impl ApolloObjectStoreBuilder {
    pub(super) async fn build_impl(
        ...
        let store = match self.cloud_type {
            CloudType::Aws => {
                #[cfg(feature = "aws")]
                {
                    let store = options
                        ...
                        .await?;
                    Ok::<_, ApolloError>(Arc::new(store) as Arc<dyn ObjectStore>)
                }
                #[cfg(not(feature = "aws"))]
            ...
            CloudType::File => {
                ...
                Ok::<_, ApolloError>(Arc::new(local) as Arc<dyn ObjectStore>)
            },
            CloudType::Http => {
                {
                    #[cfg(feature = "http")]
                    {
                        ...
                        ApolloResult::Ok(Arc::new(store) as Arc<dyn ObjectStore>)
                    }
                }
    ...
    pub(super) async fn build(self) -> ApolloResult<ApolloObjectStore> {
        ...
        let store = ApolloObjectStore::new_from_inner(store, self);
...
pub async fn build_object_store(
    ...
    options: Option<&CloudOptions>,
    glob: bool,
) -> ApolloResult<(CloudLocation, ApolloObjectStore)> {
    ...
    let store = ApolloObjectStoreBuilder {
        path,
        cloud_type,
