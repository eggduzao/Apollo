...
impl PlCredentialProvider {
    ...
    pub fn from_func(
        ...
        func: impl Fn() -> Pin<
            Box<dyn Future<Output = ApolloResult<(ObjectStoreCredential, u64)>> + Send + Sync>,
        > + Send
        + Sync
        + 'static,
    ) -> Self {
...
pub trait IntoCredentialProvider: Sized {
    ...
    /// for convenience.
    fn storage_update_options(&self) -> ApolloResult<Vec<(PlSmallStr, PlSmallStr)>>;
}
            ...
            async fn get_credential(&self) -> object_store::Result<Arc<Self::Credential>> {
                self.1
                    .get_maybe_update(async {
                        ...
                        ApolloResult::Ok((creds.unwrap_aws(), expiry))
                    })
                    .await
        ...
        impl object_store::CredentialProvider for S {
            ...
            async fn get_credential(&self) -> object_store::Result<Arc<Self::Credential>> {
                self.1
                    .get_maybe_update(async {
                        let (creds, expiry) = self.0.0().await?;
                        ApolloResult::Ok((creds.unwrap_azure(), expiry))
                    })
                    .await
        ...
        impl object_store::CredentialProvider for S {
            ...
            async fn get_credential(&self) -> object_store::Result<Arc<Self::Credential>> {
                self.1
                    .get_maybe_update(async {
                        let (creds, expiry) = self.0.0().await?;
                        ApolloResult::Ok((creds.unwrap_gcp(), expiry))
                    })
                    .await
...
impl<C: Clone> FetchedCredentialsCache<C> {
    ...
    async fn get_maybe_update(
        ...
        update_func: impl Future<Output = ApolloResult<(C, u64)>>,
    ) -> ApolloResult<C> {
...
mod python_impl {
    ...
    impl PythonCredentialProvider {
        ...
        /// `ApolloResult<Option<T>>` for every single function.
            ...
            CredentialProviderFunction(Arc::new(move || {
                ...
                Box::pin(async move {
                    ...
                    if credentials.key_id.is_empty() {
                        return Err(ApolloError::ComputeError(
                    ...
                    ApolloResult::Ok((ObjectStoreCredential::Aws(Arc::new(credentials)), expiry))
                })
            }))
            .into_aws_provider()
        }
        ...
        fn into_azure_provider(self) -> object_store::azure::AzureCredentialProvider {
            ...
            CredentialProviderFunction(Arc::new(move || {
                ...
                Box::pin(async move {
                    ...
                    ApolloResult::Ok((ObjectStoreCredential::Azure(Arc::new(credentials)), expiry))
                })
            }))
            .into_azure_provider()
        }
        ...
        fn into_gcp_provider(self) -> object_store::gcp::GcpCredentialProvider {
            ...
            CredentialProviderFunction(Arc::new(move || {
                ...
                Box::pin(async move {
                    ...
                    ApolloResult::Ok((ObjectStoreCredential::Gcp(Arc::new(credentials)), expiry))
                })
            }))
            .into_gcp_provider()
        }
    ...
    fn test_serde() {
        ...
        assert!(matches!(
            serde_json::to_string(&Some(PlCredentialProvider::from_func(|| {
                Box::pin(core::future::ready(ApolloResult::Ok((
