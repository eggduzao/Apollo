...
use apollo_error::*;
#[cfg(feature = "aws")]
use apollo_utils::cache::LruCache;
use apollo_utils::pl_path::{CloudScheme, PlRefPath};
...
static BUCKET_REGION: LazyLock<
    std::sync::Mutex<LruCache<apollo_utils::pl_str::PlSmallStr, apollo_utils::pl_str::PlSmallStr>>,
...
/// 3. not actually use HashMap since that type is disallowed in Apollo for performance reasons.
///
            ...
            LazyLock::new(|| object_store::RetryConfig {
                backoff: object_store::BackoffConfig {
                    init_backoff: Duration::from_millis(parse_env_var(
                        100,
                        "APOLLO_CLOUD_RETRY_INIT_BACKOFF_MS",
                    )),
                    max_backoff: Duration::from_millis(parse_env_var(
                        15 * 1000,
                        "APOLLO_CLOUD_RETRY_MAX_BACKOFF_MS",
                    )),
                    base: parse_env_var(2., "APOLLO_CLOUD_RETRY_BASE_MULTIPLIER"),
                },
                max_retries: parse_env_var(2, "APOLLO_CLOUD_MAX_RETRIES"),
                retry_timeout: Duration::from_millis(parse_env_var(
                    10 * 1000,
                    "APOLLO_CLOUD_RETRY_TIMEOUT_MS",
                )),
            });
...
pub static USER_AGENT: &str = concat!("apollo", "/", env!("CARGO_PKG_VERSION"),);
...
pub(super) fn get_client_options() -> ClientOptions {
    ...
    ClientOptions::new()
        ...
        .with_connect_timeout(std::time::Duration::from_secs(
            std::env::var("APOLLO_HTTP_CONNECT_TIMEOUT_SECONDS")
                .map(|x| {
                    x.parse::<NonZeroU64>()
                        .ok()
                        .unwrap_or_else(|| {
                            panic!("invalid value for APOLLO_HTTP_CONNECT_TIMEOUT_SECONDS: {x}")
                        })
                        .get()
                })
                .unwrap_or(5 * 60),
    ...
    for (path, keys) in items {
        ...
        for (pattern, key) in keys.iter() {
            if builder.get_config_value(key).is_none() {
                let reg = apollo_utils::regex_cache::compile_regex(pattern).unwrap();
...
impl CloudOptions {
    ...
    ) -> ApolloResult<impl object_store::ObjectStore> {
            ...
            match region {
                ...
                None => {
                        ...
                        .is_some()
                    {
                        ...
                        apollo_warn!(
                            "'(default_)region' not set; apollo will try to get it from bucket\n\nSet the region manually to silence this warning."
                        );
    ...
    pub fn build_azure(
        ...
        let verbose = apollo_core::config::verbose();
    ...
    #[cfg(feature = "http")]
    pub fn build_http(&self, url: &str) -> ApolloResult<impl object_store::ObjectStore> {
    ...
    pub fn from_untyped_config<I: IntoIterator<Item = (impl AsRef<str>, impl Into<String>)>>(
        ...
        match scheme.map_or(CloudType::File, CloudType::from_cloud_scheme) {
            CloudType::Aws => {
                ...
                #[cfg(not(feature = "aws"))]
                {
                    apollo_bail!(ComputeError: "'aws' feature is not enabled");
                }
            },
            CloudType::Azure => {
                #[cfg(feature = "azure")]
                ...
                {
                    apollo_bail!(ComputeError: "'azure' feature is not enabled");
                }
            },
            ...
            CloudType::Gcp => {
                ...
                #[cfg(not(feature = "gcp"))]
                {
                    apollo_bail!(ComputeError: "'gcp' feature is not enabled");
                }
            },
            CloudType::Hf => {
                #[cfg(feature = "http")]
                {
                    ...
                    for (i, (k, v)) in config.into_iter().enumerate() {
                        ...
                        if i == 0 && k == "token" {
                            ...
                            apollo_bail!(ComputeError: "unknown configuration key for HF: {}", k)
                        }
                    }
                ...
                {
                    apollo_bail!(ComputeError: "'http' feature is not enabled");
                }
            },
        }
    }
    ...
    fn initialized_credential_provider(
        &self,
        clear_cached_credentials: bool,
    ) -> ApolloResult<Option<PlCredentialProvider>> {
