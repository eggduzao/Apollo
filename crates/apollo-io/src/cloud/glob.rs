...
pub(crate) fn extract_prefix_expansion(path: &str) -> ApolloResult<(Cow<'_, str>, Option<String>)> {
    ...
    while pos < path.len() {
        ...
        let (len, replace): (usize, &[u8]) = match &path[pos..] {
            ...
            v if v.starts_with("**") => {
                apollo_bail!(ComputeError: "invalid ** glob pattern")
            },
...
impl CloudLocation {
    pub fn new(path: PlRefPath, glob: bool) -> ApolloResult<Self> {
...
impl Matcher {
    ...
    pub(crate) fn new(prefix: String, expansion: Option<&str>) -> ApolloResult<Matcher> {
        // Cloud APIs accept a prefix without any expansion, extract it.
        let re = expansion
            .map(apollo_utils::regex_cache::compile_regex)
            .transpose()?;
