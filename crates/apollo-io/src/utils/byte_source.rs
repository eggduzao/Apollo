...
#[cfg(feature = "cloud")]
use crate::cloud::{
    CloudLocation, ObjectStorePath, ApolloObjectStore, build_object_store, object_path_from_str,
};

#[allow(async_fn_in_trait)]
pub trait ByteSource: Send + Sync {
    async fn get_size(&self) -> ApolloResult<usize>;
    /// # Panics
    /// Panics if `range` is not in bounds.
    async fn get_range(&self, range: Range<usize>) -> ApolloResult<Buffer<u8>>;
