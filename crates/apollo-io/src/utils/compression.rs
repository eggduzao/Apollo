...
use apollo_error::{feature_gated, to_compute_err};
...
pub fn maybe_decompress_bytes<'a>(bytes: &'a [u8], out: &'a mut Vec<u8>) -> ApolloResult<&'a [u8]> {
    assert!(out.is_empty());
...
impl CompressedReader {
    pub fn try_new(slice: Buffer<u8>) -> ApolloResult<Self> {
