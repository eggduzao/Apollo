...
use apollo_error::{ApolloError, ApolloResult, feature_gated, apollo_err};
use apollo_utils::create_file;
use apollo_utils::file::close_file;
use apollo_utils::mmap::ensure_not_mapped;
use apollo_utils::pl_path::{PlRefPath, format_file_uri};
...
impl Writeable {
    pub fn try_new(
        ...
        Ok(if path.has_scheme() {
            ...
            feature_gated!("cloud", {
                ...
                let path = path.to_str().ok_or_else(|| apollo_err!(non_utf8_path))?;
            ...
            Self::Local(apollo_utils::open_file_write(&path)?)
        })
    }
    ...
    #[cfg(feature = "cloud")]
    pub fn try_into_async_writeable(self) -> ApolloResult<AsyncWriteable> {
...
mod async_writeable {
    ...
    impl AsyncWriteable {
        ...
        pub async fn close(mut self, sync: SyncOnCloseType) -> ApolloResult<()> {
            match sync {
            ...
            match self {
                Self::Dyn(mut v) => {
                    v.shutdown().await.map_err(ApolloError::from)?;
                ...
                .await
                .map_err(ApolloError::from),
                Self::Cloud(mut v) => v.shutdown().await.map_err(ApolloError::from),
            }
        }
    }
