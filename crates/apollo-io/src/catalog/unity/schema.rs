...
) -> ApolloResult<(Option<SchemaRef>, Option<SchemaRef>)> {
...
pub fn column_info_to_field(column_info: &ColumnInfo) -> ApolloResult<Field> {
    Ok(Field::new(
...
/// ```
pub fn parse_type_json_str(type_json: &str) -> ApolloResult<DataType> {
...
pub fn parse_type_json(type_json: &ColumnTypeJson) -> ApolloResult<DataType> {
...
fn parse_type_json_type(type_json_type: &ColumnTypeJsonType) -> ApolloResult<DataType> {
    use ColumnTypeJsonType::*;
...
fn parse_type_text(type_text: &str) -> ApolloResult<DataType> {
    use DataType::*;
    use apollo_core::prelude::TimeUnit;
...
pub fn schema_to_column_info_list(schema: &Schema) -> ApolloResult<Vec<ColumnInfo>> {
    schema
        .iter()
        .enumerate()
        .map(|(i, (name, dtype))| {
...
fn dtype_to_type_text(dtype: &DataType) -> ApolloResult<PlSmallStr> {
    use DataType::*;
    use apollo_core::prelude::TimeUnit;
...
fn dtype_to_type_name(dtype: &DataType) -> ApolloResult<PlSmallStr> {
    use DataType::*;
    use apollo_core::prelude::TimeUnit;
...
fn field_to_type_json(name: PlSmallStr, dtype: &DataType) -> ApolloResult<ColumnTypeJson> {
    Ok(ColumnTypeJson {
        name: Some(name),
...
fn dtype_to_type_json(dtype: &DataType) -> ApolloResult<ColumnTypeJsonType> {
    use DataType::*;
    use apollo_core::prelude::TimeUnit;
