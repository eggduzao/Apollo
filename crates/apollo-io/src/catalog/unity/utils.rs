...
pub(super) async fn do_request(request: reqwest::RequestBuilder) -> ApolloResult<bytes::Bytes> {
...
macro_rules! impl_page_walk {
    ($S:ty, $T:ty, key_name = $key_name:tt) => {
        impl $S {
            pub async fn next(&mut self) -> ApolloResult<Option<Vec<$T>>> {
                return self
                    .0
                    .next(|bytes| {
                        let Response {
            ...
            pub async fn read_all_pages(mut self) -> ApolloResult<Vec<$T>> {
...
impl PageWalker {
    ...
    pub(crate) async fn next<F, T>(&mut self, deserializer: F) -> ApolloResult<Option<T>>
    where
        F: Fn(&[u8]) -> ApolloResult<(T, Option<String>)>,
    {
    ...
    pub(crate) async fn next_bytes(&mut self) -> ApolloResult<Option<Bytes>> {
