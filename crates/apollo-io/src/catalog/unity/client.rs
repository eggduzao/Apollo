...
impl CatalogClient {
    pub async fn list_catalogs(&self) -> ApolloResult<Vec<CatalogInfo>> {
    ...
    pub async fn list_namespaces(&self, catalog_name: &str) -> ApolloResult<Vec<NamespaceInfo>> {
    ...
    pub async fn delete_catalog(&self, catalog_name: &str, force: bool) -> ApolloResult<()> {
...
impl CatalogClientBuilder {
    ...
    pub fn build(self) -> ApolloResult<CatalogClient> {
        let Some(workspace_url) = self.workspace_url else {
            apollo_bail!(ComputeError: "expected Some(_) for workspace_url")
        };
