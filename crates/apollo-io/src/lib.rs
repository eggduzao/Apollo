...
pub fn get_upload_chunk_size() -> usize {
    ...
    static UPLOAD_CHUNK_SIZE: LazyLock<usize> = LazyLock::new(|| {
        let v = std::env::var("APOLLO_UPLOAD_CHUNK_SIZE")
            .map(|x| {
                x.parse::<usize>()
                    .ok()
                    .filter(|x| *x > 0)
                    .unwrap_or_else(|| panic!("invalid value for APOLLO_UPLOAD_CHUNK_SIZE: {x}"))
            })
...
pub fn get_upload_concurrency() -> usize {
    ...
    static UPLOAD_CONCURRENCY: LazyLock<usize> = LazyLock::new(|| {
        ...
        let v = std::env::var("APOLLO_UPLOAD_CONCURRENCY")
            .map(|x| {
                x.parse::<usize>()
                    .ok()
                    .filter(|x| *x > 0)
                    .unwrap_or_else(|| panic!("invalid value for APOLLO_UPLOAD_CONCURRENCY: {x}"))
            })
            .unwrap_or(8);
