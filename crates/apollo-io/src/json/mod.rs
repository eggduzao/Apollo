...
use apollo_core::error::to_compute_err;
use apollo_core::prelude::*;
use apollo_error::{ApolloResult, apollo_bail};
use apollo_json::json::write::FallibleStreamingIterator;
    ...
    W: Write,
{
    ...
    fn finish(&mut self, df: &mut DataFrame) -> ApolloResult<()> {
        df.align_chunks_par();
        let fields = df.columns()
            .iter()
            .map(|s| {
                #[cfg(feature = "object")]
                apollo_ensure!(!matches!(s.dtype(), DataType::Object(_)), ComputeError: "cannot write 'Object' datatype to json");
        ...
        match self.json_format {
            JsonFormat::JsonLines => {
                let serializer = apollo_json::ndjson::write::Serializer::new(batches, vec![]);
                let writer =
                    apollo_json::ndjson::write::FileWriter::new(&mut self.buffer, serializer);
                writer.collect::<ApolloResult<()>>()?;
            },
            JsonFormat::Json => {
                let serializer = apollo_json::json::write::Serializer::new(batches, vec![]);
                apollo_json::json::write::write(&mut self.buffer, serializer)?;
            },
        }
    ...
    W: Write,
{
    ...
    pub fn write_batch(&mut self, df: &DataFrame) -> ApolloResult<()> {
        let fields = df.columns()
            .iter()
            .map(|s| {
                #[cfg(feature = "object")]
        ...
        let mut serializer = apollo_json::ndjson::write::Serializer::new(batches, vec![]);
...
pub fn remove_bom(bytes: &[u8]) -> ApolloResult<&[u8]> {
    if bytes.starts_with(&[0xEF, 0xBB, 0xBF]) {
        // UTF-8 BOM
        Ok(&bytes[3..])
    } else if bytes.starts_with(&[0xFE, 0xFF]) || bytes.starts_with(&[0xFF, 0xFE]) {
        // UTF-16 BOM
        apollo_bail!(ComputeError: "utf-16 not supported")
    } else {
        Ok(bytes)
    }
}
    ...
    R: MmapBytesReader,
{
    ...
    fn finish(mut self) -> ApolloResult<DataFrame> {
        ...
        let out = match self.json_format {
            JsonFormat::Json => {
                apollo_ensure!(!self.ignore_errors, InvalidOperation: "'ignore_errors' only supported in ndjson");
                ...
                let mut schema = if let Some(schema) = self.schema {
                    ...
                    let inner_dtype = if let BorrowedValue::Array(values) = &json_value {
                        ...
                        DataType::from_arrow_dtype(&apollo_json::json::infer(&json_value)?)
                    };

                    let DataType::Struct(fields) = inner_dtype else {
                        apollo_bail!(ComputeError: "can only deserialize json objects")
                    };
                ...
                let arr = apollo_json::json::deserialize(
                    &json_value,
                    arrow_dtype,
                ...
                let arr = arr.as_any().downcast_ref::<StructArray>().ok_or_else(
                    || apollo_err!(ComputeError: "can only deserialize json objects"),
                )?;
