...
pub fn split_lines_to_rows(bytes: &[u8]) -> ApolloResult<Series> {
    split_lines_to_rows_impl(bytes, BinviewArrayBuilder::MAX_ROW_BYTE_LEN)
}

fn split_lines_to_rows_impl(bytes: &[u8], max_buffer_size: usize) -> ApolloResult<Series> {
    if bytes.is_empty() {
...
";
        ...
        let ApolloError::ComputeError(err_str) = split_lines_to_rows_impl(data, 19).unwrap_err()
        else {
            unreachable!()
        };
