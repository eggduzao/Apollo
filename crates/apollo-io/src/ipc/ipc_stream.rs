...
use apollo_core::frame::chunk_df_for_writing;
use apollo_core::prelude::*;
...
/// use apollo_core::prelude::*;
/// use std::fs::File;
/// use apollo_io::ipc::IpcStreamReader;
...
impl<R: Read> IpcStreamReader<R> {
    ...
    pub fn schema(&mut self) -> ApolloResult<Schema> {
        Ok(Schema::from_arrow_schema(&self.metadata()?.schema))
    }

    /// Get arrow schema of the Ipc Stream File, this is faster than creating a apollo schema.
    pub fn arrow_schema(&mut self) -> ApolloResult<ArrowSchema> {
    ...
    fn metadata(&mut self) -> ApolloResult<StreamMetadata> {
        match &self.metadata {
            None => {
...
/// ```
/// use apollo_core::prelude::*;
/// use apollo_io::ipc::IpcStreamWriter;
/// use std::fs::File;
/// use apollo_io::SerWriter;
///
