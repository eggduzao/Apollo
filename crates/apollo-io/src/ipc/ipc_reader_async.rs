...
use apollo_core::datatypes::IDX_DTYPE;
...
use crate::RowIndex;
use crate::cloud::{
    CloudLocation, CloudOptions, ApolloObjectStore, build_object_store, object_path_from_str,
};
...
/// An Arrow IPC reader implemented on top of ApolloObjectStore.
pub struct IpcReaderAsync {
    store: ApolloObjectStore,
...
impl IpcReaderAsync {
    ...
    async fn object_metadata(&self) -> ApolloResult<ObjectMeta> {
        self.store.head(&self.path).await
    }

    async fn file_size(&self) -> ApolloResult<usize> {
        Ok(self.object_metadata().await?.size as usize)
    }

    pub async fn metadata(&self) -> ApolloResult<FileMetadata> {
    ...
    pub async fn count_rows(&self, _metadata: Option<&FileMetadata>) -> ApolloResult<i64> {
        // TODO: Only download what is needed rather than the entire file by
        // making use of the projection, row limit, predicate and such.
        let file = tokio::task::block_in_place(|| self.cache_entry.try_open_check_latest())?;
        let bytes = MMapSemaphore::new_from_file(&file).unwrap();
...
// TODO: Move to apollo-arrow and deduplicate parsing of footer metadata in
// sync and async readers.
fn deserialize_footer_metadata(bytes: [u8; FOOTER_METADATA_SIZE]) -> ApolloResult<usize> {
    ...
    if &bytes[4..] != b"ARROW1" {
        apollo_bail!(oos = OutOfSpecKind::InvalidFooter);
    }
