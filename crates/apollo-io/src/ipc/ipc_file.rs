...
//! It is the recommended way to serialize and deserialize Apollo DataFrames as this is most true
//! to the data schema.
//!
//! ## Example
//!
...
use apollo_utils::pl_str::PlRefStr;
#[cfg(feature = "serde")]
...
/// use apollo_core::prelude::*;
/// use std::fs::File;
/// use apollo_io::ipc::IpcReader;
/// use apollo_io::SerReader;
///
/// fn example() -> ApolloResult<DataFrame> {
...
fn check_mmap_err(err: ApolloError) -> ApolloResult<()> {
    if let ApolloError::ComputeError(s) = &err {
...
impl<R: MmapBytesReader> IpcReader<R> {
    fn get_metadata(&mut self) -> ApolloResult<&read::FileMetadata> {
    ...
    pub fn schema(&mut self) -> ApolloResult<ArrowSchemaRef> {
        self.get_metadata()?;
        Ok(self.schema.as_ref().unwrap().clone())
    }

    /// Get schema-level custom metadata of the Ipc file
    pub fn custom_metadata(&mut self) -> ApolloResult<Option<Arc<Metadata>>> {
        self.get_metadata()?;
        Ok(self
            .metadata
            .as_ref()
    ...
    pub fn finish_with_scan_ops(
        ...
        // https://github.com/apollo/apollo/pull/14984#discussion_r1520125000
...
where
    R: Read + Seek,
{
    fn next_record_batch(&mut self) -> ApolloResult<Option<RecordBatch>> {
