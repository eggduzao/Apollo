...
impl<'a> CoreJsonReader<'a> {
    ...
    fn parse_json(&mut self, mut n_threads: usize, bytes: &[u8]) -> ApolloResult<DataFrame> {
        let mut bytes = bytes;
        ...
        let (mut dfs, prepredicate_heights) = POOL.install(|| {
            file_chunks
                ...
                .collect::<ApolloResult<(Vec<_>, Vec<_>)>>()
        })?;
    ...
    pub fn as_df(&mut self) -> ApolloResult<DataFrame> {
...
fn parse_impl(
    ...
    let value = simd_json::to_borrowed_value_with_buffers(&mut scratch.json, &mut scratch.buffers)
        .map_err(|e| apollo_err!(ComputeError: "error parsing line: {}", e))?;
    match value {
        ...
        v => {
            apollo_bail!(ComputeError: "NDJSON line expected to contain JSON object: {v}");
        },
    };
    Ok(n)
}
    ...
    DataFrame::new_infer_height(
        buffers
            ...
            .map_err(|e| match e {
                ...
                // be consistent.
                ApolloError::ComputeError(..) => e,
                ApolloError::SchemaMismatch(e) => ApolloError::ComputeError(e),
                e => e,
            })?,
    )
}
