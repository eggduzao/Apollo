...
use apollo_core::frame::row::AnyValueBuffer;
use apollo_core::prelude::*;
#[cfg(any(feature = "dtype-datetime", feature = "dtype-date"))]
use apollo_time::prelude::string::Pattern;
#[cfg(any(feature = "dtype-datetime", feature = "dtype-date"))]
use apollo_time::prelude::string::infer::{DatetimeInfer, TryFromWithUnit, infer_pattern_single};
...
impl Buffer<'_> {
    ...
    #[inline]
    pub(crate) fn add(&mut self, value: &Value) -> ApolloResult<()> {
        use AnyValueBuffer::*;
        match &mut self.buf {
            _ if value.is_null() => {
            ...
            Boolean(buf) => {
                match value {
                    ...
                    v => {
                        apollo_bail!(ComputeError: "cannot parse '{}' ({}) as Boolean", v, v.value_type())
                    },
                }
                Ok(())
            },
            ...
            Null(builder) => {
                if !(matches!(value, Value::Static(StaticNode::Null)) || self.ignore_errors) {
                    apollo_bail!(ComputeError: "got non-null value for NULL-typed column: {}", value)
                };
...
pub(crate) fn init_buffers(
    ...
    capacity: usize,
    ignore_errors: bool,
) -> ApolloResult<PlIndexMap<BufferKey<'_>, Buffer<'_>>> {
    schema
        .iter()
        .map(|(name, dtype)| {
...
fn deserialize_numeric<T: ApolloNumericType>(
    value: &Value,
    n: Option<T::Native>,
    ignore_errors: bool,
) -> ApolloResult<Option<T::Native>> {
    match n {
        Some(v) => Ok(Some(v)),
        None if ignore_errors => Ok(None),
        None => Err(
            apollo_err!(ComputeError: "cannot parse '{}' ({}) as {:?}", value, value.value_type(), T::get_static_dtype()),
        ),
    }
}
    ...
    DatetimeInfer<T>: TryFromWithUnit<Pattern>,
{
    ...
    apollo_bail!(ComputeError: "cannot parse '{}' ({}) as {}", value, value.value_type(), type_name)
}
    ...
    match dtype {
        ...
        dt @ DataType::Float16 => {
            ...
            return match json.cast_f64() {
                ...
                None => {
                    apollo_bail!(ComputeError: "cannot parse '{}' ({}) as {}", json, json.value_type(), dt)
                },
            };
        },
        dt @ DataType::Float32 => {
    ...
    let out = match json {
        ...
        Value::Array(arr) => {
            let Some(inner_dtype) = dtype.inner_dtype() else {
                ...
                apollo_bail!(ComputeError: "expected dtype '{}' in JSON value, got dtype: Array\n\nEncountered value: {}", dtype, json);
            };
