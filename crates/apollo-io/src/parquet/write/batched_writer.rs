...
use apollo_parquet::read::{ParquetError, fallible_streaming_iterator};
...
impl<W: Write> BatchedWriter<W> {
    ...
    pub fn encode_and_compress<'a>(
        &'a self,
        df: &'a DataFrame,
    ) -> impl Iterator<Item = ApolloResult<RowGroupIterColumns<'static, ApolloError>>> + 'a {
    ...
    pub fn write_row_group(
        ...
        let rg = DynIter::new(rg.iter().map(|col_pages| {
            Ok(DynStreamingIterator::new(
                fallible_streaming_iterator::convert(col_pages.iter().map(ApolloResult::Ok)),
            ))
        }));
    ...
    pub fn write_row_groups(
        &self,
        rgs: Vec<RowGroupIterColumns<'static, ApolloError>>,
    ) -> ApolloResult<()> {
    ...
    pub fn finish(&self) -> ApolloResult<u64> {
        let mut writer = self.writer.lock().unwrap();
...
fn prepare_rg_iter<'a>(
    ...
    Item = (
        usize,
        ApolloResult<RowGroupIterColumns<'static, ApolloError>>,
    ),
> + 'a {
    let rb_iter = df.iter_chunks(CompatLevel::newest(), false);
    rb_iter.filter_map(move |batch| match batch.len() {
...
fn pages_iter_to_compressor(
    encoded_columns: Vec<DynIter<'static, ApolloResult<Page>>>,
    options: WriteOptions,
) -> Vec<ApolloResult<DynStreamingIterator<'static, CompressedPage, ApolloError>>> {
    encoded_columns
        .into_iter()
        .map(|encoded_pages| {
            // iterator over pages
            let pages = DynStreamingIterator::new(
                Compressor::new_from_vec(
                    encoded_pages.map(|result| {
                        result.map_err(|e| {
                            ParquetError::FeatureNotSupported(format!("reraised in apollo: {e}",))
                        })
                    }),
                    options.compression,
                    vec![],
