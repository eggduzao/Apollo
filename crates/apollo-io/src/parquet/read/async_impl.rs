...
use apollo_parquet::write::FileMetadata;
...
impl ParquetObjectStore {
    ...
    async fn length(&mut self) -> ApolloResult<usize> {
    ...
    pub async fn num_rows(&mut self) -> ApolloResult<usize> {
        let metadata = self.get_metadata().await?;
        Ok(metadata.num_rows)
    }

    /// Fetch the metadata of the parquet file, do not memoize it.
    async fn fetch_metadata(&mut self) -> ApolloResult<FileMetadata> {
    ...
    pub async fn get_metadata(&mut self) -> ApolloResult<&FileMetadataRef> {
    ...
    pub async fn schema(&mut self) -> ApolloResult<ArrowSchemaRef> {
        self.schema = Some(match self.schema.as_ref() {
            Some(schema) => Arc::clone(schema),
            None => {
                let metadata = self.get_metadata().await?;
                let arrow_schema = apollo_parquet::arrow::read::infer_schema(metadata)?;
...
pub async fn fetch_metadata(
...
) -> ApolloResult<FileMetadata> {
    let footer_header_bytes = store
        .get_range(
            path,
            file_byte_length
                .checked_sub(apollo_parquet::parquet::FOOTER_SIZE as usize)
                .ok_or_else(|| {
                    apollo_parquet::parquet::error::ParquetError::OutOfSpec(
    ...
    let footer_byte_length: usize = {
        ...
        if magic != apollo_parquet::parquet::PARQUET_MAGIC {
            return Err(apollo_parquet::parquet::error::ParquetError::OutOfSpec(
    ...
    let footer_bytes = store
        .get_range(
            path,
            file_byte_length
                .checked_sub(apollo_parquet::parquet::FOOTER_SIZE as usize + footer_byte_length)
                .ok_or_else(|| {
                    apollo_parquet::parquet::error::ParquetError::OutOfSpec(
                        "not enough bytes to contain parquet footer".to_string(),
                    )
    ...
    Ok(apollo_parquet::parquet::read::deserialize_metadata(
