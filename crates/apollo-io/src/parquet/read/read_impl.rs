...
use apollo_core::chunked_array::builder::NullChunkedBuilder;
use apollo_core::prelude::*;
use apollo_core::series::IsSorted;
use apollo_core::utils::accumulate_dataframes_vertical;
use apollo_core::{POOL, config};
use apollo_parquet::read::{self, ColumnChunkMetadata, FileMetadata, Filter, RowGroupMetadata};
use rayon::prelude::*;
...
// Ensure we get the proper apollo types from schema inference
...
fn rg_to_dfs_optionally_par_over_columns(
    ...
    for rg_idx in row_group_start..row_group_end {
        ...
        let columns = if let ParallelStrategy::Columns = parallel {
            ...
            projection.iter().map(f).collect::<ApolloResult<Vec<_>>>()?
        };
        ...
        *previous_row_count = previous_row_count
            ...
            .ok_or_else(|| {
                apollo_err!(
                    ...
                    consider compiling with apollo-bigidx feature (pip install apollo[rt64]), \
                    or set 'streaming'"
                )
            })?;
        dfs.push(df);
...
fn rg_to_dfs_par_over_rg(
    ...
    if row_group_start > 0 {
        ...
        // For details, see: https://github.com/apollo/apollo/pull/20508#discussion_r1900165649
    ...
    let dfs = POOL.install(|| {
        ...
        row_groups
            ...
            .map(|(md, slice, row_count_start)| {
                ...
                #[cfg(debug_assertions)]
                {
                    assert!(std::env::var("APOLLO_PANIC_IF_PARQUET_PARSED").is_err())
                }
...
impl PrefilterMaskSetting {
    pub fn init_from_env() -> Self {
        std::env::var("APOLLO_PQ_PREFILTERED_MASK").map_or(Self::Auto, |v| match &v[..] {
            "auto" => Self::Auto,
            "pre" => Self::Pre,
            "post" => Self::Post,
            _ => panic!("Invalid `APOLLO_PQ_PREFILTERED_MASK` value '{v}'."),
        })
    }
