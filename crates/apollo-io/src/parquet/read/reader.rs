...
impl<R: MmapBytesReader> ParquetReader<R> {
    ...
    pub fn with_arrow_schema_projection(
        ...
        .map_err(|e| {
            if !allow_missing_columns && matches!(e, ApolloError::ColumnNotFound(_)) {
                e.wrap_msg(|s| {
                    format!(
    ...
    pub fn num_rows(&mut self) -> ApolloResult<usize> {
        let metadata = self.get_metadata()?;
    ...
    pub fn get_metadata(&mut self) -> ApolloResult<&FileMetadataRef> {
