...
use apollo_core::prelude::{ArrowSchema, Column, DataFrame, DataType, IDX_DTYPE, Series};
use apollo_core::schema::{SchemaExt, SchemaNamesAndDtypes};
use apollo_error::{ApolloResult, apollo_bail};
use apollo_schema::Schema;
    ...
    for (i, field) in projected_arrow_schema.iter_values().enumerate() {
        let dtype = {
            let Some((idx, _, field)) = schema.get_full(&field.name) else {
                apollo_bail!(ColumnNotFound: "did not find column in file: {}", field.name)
            };
