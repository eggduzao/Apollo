...
use apollo_core::config::{self, verbose};
...
static DOWNLOAD_CHUNK_SIZE: LazyLock<usize> = LazyLock::new(|| {
    let v: usize = std::env::var("APOLLO_DOWNLOAD_CHUNK_SIZE")
        .as_deref()
...
fn get_semaphore() -> &'static (Semaphore, u32) {
    CONCURRENCY_BUDGET.get_or_init(|| {
        let permits = std::env::var("APOLLO_CONCURRENCY_BUDGET")
            .map(|s| {
...
impl RuntimeManager {
    fn new() -> Self {
        let n_threads = std::env::var("APOLLO_ASYNC_THREAD_COUNT")
            .map(|x| x.parse::<usize>().expect("integer"))
            .unwrap_or(POOL.current_num_threads().clamp(1, 4));

        let max_blocking = std::env::var("APOLLO_MAX_BLOCKING_THREAD_COUNT")
            .map(|x| x.parse::<usize>().expect("integer"))
            .unwrap_or(512);
