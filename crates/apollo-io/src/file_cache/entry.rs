...
impl Inner {
    fn try_open_assume_latest(&mut self) -> ApolloResult<std::fs::File> {
    ...
    fn try_open_check_latest(&mut self) -> ApolloResult<std::fs::File> {
        let verbose = config::verbose();
        ...
        if !self.file_fetcher.fetches_as_symlink() {
            ...
            static RAISE_ALLOC_ERROR: LazyLock<Option<bool>> = LazyLock::new(|| {
                let v = match std::env::var("APOLLO_IGNORE_FILE_CACHE_ALLOCATE_ERROR").as_deref() {
                    Ok("1") => Some(false),
                    Ok("0") => Some(true),
                    Err(_) => None,
                    Ok(v) => {
                        panic!("invalid value {v} for APOLLO_IGNORE_FILE_CACHE_ALLOCATE_ERROR")
                    },
                };
                if config::verbose() {
        ...
        if local_size != remote_metadata.size {
            apollo_bail!(ComputeError: "downloaded file size ({}) does not match expected size ({})", local_size, remote_metadata.size);
        }
    ...
    ) -> ApolloResult<Arc<EntryMetadata>> {
            ...
            if let Some(ref cached) = self.cached_data {
                if cached.last_modified == last_modified {
                    if cached.metadata.ttl != ttl {
                        apollo_bail!(ComputeError: "TTL mismatch");
                    }
...
impl FileCacheEntry {
    ...
    pub fn try_open_assume_latest(&self) -> ApolloResult<std::fs::File> {
    ...
    pub fn try_open_check_latest(&self) -> ApolloResult<std::fs::File> {
