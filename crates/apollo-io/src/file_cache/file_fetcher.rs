...
use crate::cloud::ApolloObjectStore;
use crate::pl_async;

pub trait FileFetcher: Send + Sync {
    fn get_uri(&self) -> &PlRefPath;
    fn fetch_metadata(&self) -> ApolloResult<RemoteMetadata>;
    /// Fetches the object to a `local_path`.
    fn fetch(&self, local_path: &std::path::Path) -> ApolloResult<()>;
...
/// Mostly used for debugging, it only ever gets called if `APOLLO_FORCE_ASYNC` is set.
...
impl FileFetcher for LocalFileFetcher {
    ...
    fn fetch_metadata(&self) -> ApolloResult<RemoteMetadata> {
        let metadata = std::fs::metadata(&self.path).map_err(ApolloError::from)?;
    ...
    fn fetch(&self, local_path: &std::path::Path) -> ApolloResult<()> {
        #[cfg(target_family = "unix")]
        {
            std::os::unix::fs::symlink(&self.path, local_path).map_err(ApolloError::from)
        }
        #[cfg(not(target_family = "unix"))]
        {
            std::fs::copy(&self.path, local_path).map_err(ApolloError::from)?;
            Ok(())
        }
    }
}
...
pub(super) struct CloudFileFetcher {
    ...
    pub(super) object_store: ApolloObjectStore,
}
