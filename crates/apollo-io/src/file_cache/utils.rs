...
use crate::path_utils::{APOLLO_TEMP_DIR_BASE_PATH, ensure_directory_init};

pub static FILE_CACHE_PREFIX: LazyLock<PlRefPath> = LazyLock::new(|| {
    let path = PlRefPath::try_from_path(&APOLLO_TEMP_DIR_BASE_PATH.join("file-cache/")).unwrap();
...
) -> ApolloResult<Vec<Arc<FileCacheEntry>>> {
