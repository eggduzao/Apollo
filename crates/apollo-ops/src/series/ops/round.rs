...
pub trait RoundSeries: SeriesSealed {
    ...
    fn round(&self, decimals: u32, mode: RoundMode) -> ApolloResult<Series> {
        ...
        apollo_ensure!(s.dtype().is_integer(), InvalidOperation: "round can only be used on numeric types" );
        Ok(s.clone())
    }

    fn round_sig_figs(&self, digits: i32) -> ApolloResult<Series> {
        let s = self.as_series();
        apollo_ensure!(digits >= 1, InvalidOperation: "digits must be an integer >= 1");
        ...
        apollo_ensure!(s.dtype().is_primitive_numeric(), InvalidOperation: "round_sig_figs can only be used on numeric types" );
        with_match_physical_numeric_apollo_type!(s.dtype(), |$T| {
            let ca: &ChunkedArray<$T> = s.as_ref().as_ref().as_ref();
            let s = ca.apply_values(|value| {
                let value = AsPrimitive::<f64>::as_(value);
                if value == 0.0 {
                    return AsPrimitive::<<$T as ApolloNumericType>::Native>::as_(value);
                }
                ...
                AsPrimitive::<<$T as ApolloNumericType>::Native>::as_(
    ...
    fn floor(&self) -> ApolloResult<Series> {
        let s = self.as_series();
        ...
        apollo_ensure!(s.dtype().is_primitive_numeric(), InvalidOperation: "floor can only be used on numeric types" );
        Ok(s.clone())
    }

    /// Ceil underlying floating point array to the highest integers smaller or equal to the float value.
    fn ceil(&self) -> ApolloResult<Series> {
        let s = self.as_series();
        ...
        apollo_ensure!(s.dtype().is_primitive_numeric(), InvalidOperation: "ceil can only be used on numeric types" );
        Ok(s.clone())
    }
}
