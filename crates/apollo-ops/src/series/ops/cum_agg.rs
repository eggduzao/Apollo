...
use apollo_core::utils::{CustomIterTools, NoNull};
...
fn cum_sum_decimal(
    ...
    reverse: bool,
    init: Option<i128>,
) -> ApolloResult<Int128Chunked> {
    use apollo_compute::decimal::{DEC128_MAX_PREC, dec128_add};

    let mut value = init.unwrap_or(0);
    let update = |opt_v| {
        if let Some(v) = opt_v {
            value = dec128_add(value, v, DEC128_MAX_PREC).ok_or_else(
                || apollo_err!(ComputeError: "overflow in decimal addition in cum_sum"),
            )?;
            Ok(Some(value))
        } else {
            Ok(None)
...
pub fn cum_prod_with_init(
    ...
    let out = match s.dtype() {
        ...
        dt => apollo_bail!(opq = cum_prod, dt),
    };
    Ok(out)
}
...
pub fn cum_prod(s: &Series, reverse: bool) -> ApolloResult<Series> {
...
/// first cast to `Int64` to prevent overflow issues.
pub fn cum_sum(s: &Series, reverse: bool) -> ApolloResult<Series> {
...
pub fn cum_min_with_init(
    ...
    match s.dtype() {
        ...
        dt => apollo_bail!(opq = cum_min, dt),
    }
}

/// Get an array with the cumulative min computed at every element.
pub fn cum_min(s: &Series, reverse: bool) -> ApolloResult<Series> {
...
pub fn cum_max_with_init(
    ...
    match s.dtype() {
        ...
        dt => apollo_bail!(opq = cum_max, dt),
    }
}

/// Get an array with the cumulative max computed at every element.
pub fn cum_max(s: &Series, reverse: bool) -> ApolloResult<Series> {
    cum_max_with_init(s, reverse, &AnyValue::Null)
}

pub fn cum_count(s: &Series, reverse: bool) -> ApolloResult<Series> {
    cum_count_with_init(s, reverse, 0)
}

pub fn cum_count_with_init(s: &Series, reverse: bool, init: IdxSize) -> ApolloResult<Series> {
