...
use apollo_utils::total_ord::TotalEq;
...
/// within the series.
pub fn index_of(series: &Series, needle: Scalar) -> ApolloResult<Option<usize>> {
    apollo_ensure!(
    ...
    match series.dtype().to_physical() {
        ...
        #[cfg(feature = "object")]
        DT::Object(_) => apollo_bail!(op = "index_of", series.dtype()),

        DT::Unknown(_) => apollo_bail!(op = "index_of", series.dtype()),
    }
}
