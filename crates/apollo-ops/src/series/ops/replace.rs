...
pub fn replace(s: &Series, old: &ListChunked, new: &ListChunked) -> ApolloResult<Series> {
    find_output_length(
        "replace",
...
fn validate_old(old: &Series) -> ApolloResult<()> {
    apollo_ensure!(
...
fn replace_by_single_strict(s: &Series, old: &Series, new: &Series) -> ApolloResult<Series> {
...
fn get_replacement_mask(s: &Series, old: &Series) -> ApolloResult<BooleanChunked> {
...
fn replace_by_multiple(
    ...
    match joined.column("__APOLLO_REPLACE_MASK") {
        Ok(col) => {
...
fn replace_by_multiple_strict(s: &Series, old: Series, new: Series) -> ApolloResult<Series> {
    validate_new(&new, &old)?;
    ...
    let replaced = joined.column("__APOLLO_REPLACE_NEW").unwrap();

    let mask = joined
        .column("__APOLLO_REPLACE_MASK")
        .unwrap()
        .bool()
        .unwrap();
...
fn create_replacer(mut old: Series, mut new: Series, add_mask: bool) -> ApolloResult<DataFrame> {
    old.rename(PlSmallStr::from_static("__APOLLO_REPLACE_OLD"));
    new.rename(PlSmallStr::from_static("__APOLLO_REPLACE_NEW"));

    let len = old.len();
    let cols = if add_mask {
        let mask = Column::new_scalar(
            PlSmallStr::from_static("__APOLLO_REPLACE_MASK"),
            true.into(),
            new.len(),
        );
...
fn validate_new(new: &Series, old: &Series) -> ApolloResult<()> {
    apollo_ensure!(
        new.len() == old.len(),
