...
use apollo_utils::slice::SliceAble;
    ...
    I: Fn(T::Native, T::Native, &[F::Native], &mut Vec<T::Native>),
{
    ...
    apollo_ensure!(by.null_count() == 0, InvalidOperation: "null values in `by` column are not yet supported in 'interpolate_by' expression");
    let by = by.rechunk();
...
pub fn interpolate_by(s: &Column, by: &Column, by_is_sorted: bool) -> ApolloResult<Column> {
    apollo_ensure!(s.len() == by.len(), InvalidOperation: "`by` column must be the same length as Series ({}), got {}", s.len(), by.len());
    ...
    match (s.dtype(), by.dtype()) {
        ...
        _ => {
            apollo_bail!(InvalidOperation: "expected series to be Float64, Float32, \
