...
pub fn clip(s: &Series, min: &Series, max: &Series) -> ApolloResult<Series> {
    apollo_ensure!(
...
pub fn clip_max(s: &Series, max: &Series) -> ApolloResult<Series> {
    apollo_ensure!(
        s.dtype().to_physical().is_primitive_numeric(),
        InvalidOperation: "`clip` only supports physical numeric types"
    );
...
pub fn clip_min(s: &Series, min: &Series) -> ApolloResult<Series> {
    apollo_ensure!(
        s.dtype().to_physical().is_primitive_numeric(),
        InvalidOperation: "`clip` only supports physical numeric types"
    );
