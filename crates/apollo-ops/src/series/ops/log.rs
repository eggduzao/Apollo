...
use apollo_core::prelude::arity::broadcast_binary_elementwise_values;
...
fn log1p<T: ApolloNumericType>(ca: &ChunkedArray<T>) -> Float64Chunked {
    ca.cast_and_apply_in_place(|v: f64| v.ln_1p())
}

fn exp<T: ApolloNumericType>(ca: &ChunkedArray<T>) -> Float64Chunked {
...
pub trait LogSeries: SeriesSealed {
    ...
    fn log1p(&self) -> Series {
        ...
        match s.dtype() {
            dt if dt.is_integer() => {
                with_match_physical_integer_apollo_type!(s.dtype(), |$T| {
                    let ca: &ChunkedArray<$T> = s.as_ref().as_ref().as_ref();
    ...
    fn entropy(&self, base: f64, normalize: bool) -> ApolloResult<f64> {
        let s = self.as_series().to_physical_repr();
        apollo_ensure!(s.dtype().is_primitive_numeric(), InvalidOperation: "expected numerical input for 'entropy'");
