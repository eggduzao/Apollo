...
use apollo_core::prelude::arity::apply_binary_kernel_broadcast;
...
pub fn is_close(
    ...
    if abs_tol < 0.0 {
        apollo_bail!(ComputeError: "`abs_tol` must be non-negative but got {}", abs_tol);
    }
    if rel_tol < 0.0 {
        apollo_bail!(ComputeError: "`rel_tol` must be non-negative but got {}", rel_tol);
    }
...
fn validate_numeric(dtype: &DataType) -> ApolloResult<()> {
