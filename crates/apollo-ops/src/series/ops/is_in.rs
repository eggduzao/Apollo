...
use apollo_core::prelude::arity::{unary_elementwise, unary_elementwise_values};
use apollo_core::prelude::*;
use apollo_core::{with_match_categorical_physical_type, with_match_physical_numeric_apollo_type};
    ...
    <T::Native as ToTotalOrd>::TotalOrdItem: Hash + Eq + Copy,
{
    match other.dtype() {
        ...
        _ => apollo_bail!(opq = is_in, ca_in.dtype(), other.dtype()),
    }
}
...
fn is_in_cat_and_enum<T: ApolloCategoricalType>(
    ...
    T::Native: ToTotalOrd<TotalOrdItem = T::Native>,
{
    let to_categories = match (ca_in.dtype(), other.dtype().inner_dtype().unwrap()) {
        (DataType::Enum(_, mapping) | DataType::Categorical(_, mapping), DataType::String) => {
            (&|s: Series| {
                let ca = s.str()?;
                let ca: ChunkedArray<T::ApolloPhysical> = ca
                    .iter()
                    .flat_map(|opt_s| {
                        if let Some(s) = opt_s {
...
fn is_in_null(s: &Series, other: &Series, nulls_equal: bool) -> ApolloResult<BooleanChunked> {
    if nulls_equal {
        let ca_in = s.null()?;
...
fn is_in_decimal(
    ...
    let Some(DataType::Decimal(other_precision, other_scale)) = other.dtype().inner_dtype() else {
        apollo_bail!(opq = is_in, ca_in.dtype(), other.dtype());
    };
...
pub fn is_in(s: &Series, other: &Series, nulls_equal: bool) -> ApolloResult<BooleanChunked> {
    apollo_ensure!(
    ...
    apollo_ensure!(other_is_valid_type, opq = is_in, s.dtype(), other.dtype());
