use apollo_compute::moment::{KurtosisState, SkewState, kurtosis, skew};
...
pub trait MomentSeries: SeriesSealed {
    ...
    fn skew(&self, bias: bool) -> ApolloResult<Option<f64>> {
    ...
    fn kurtosis(&self, fisher: bool, bias: bool) -> ApolloResult<Option<f64>> {
        let s = self.as_series();
        let s = s.cast(&DataType::Float64)?;
        let ca = s.f64().unwrap();
