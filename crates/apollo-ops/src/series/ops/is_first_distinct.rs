...
fn is_first_distinct_struct(s: &Series) -> ApolloResult<BooleanChunked> {
...
fn is_first_distinct_list(ca: &ListChunked) -> ApolloResult<BooleanChunked> {
...
pub fn is_first_distinct(s: &Series) -> ApolloResult<BooleanChunked> {
    // fast path.
    if s.is_empty() {
    ...
    let out = match s.dtype() {
        ...
        dt => apollo_bail!(opq = is_first_distinct, dt),
    };
    Ok(out)
}
