...
fn dispatcher(s: &Series, invert: bool) -> ApolloResult<BooleanChunked> {
    ...
    let out = match s.dtype() {
        ...
        dt => apollo_bail!(opq = is_unique, dt),
    };
    Ok(out)
}

pub fn is_unique(s: &Series) -> ApolloResult<BooleanChunked> {
    dispatcher(s, false)
}

pub fn is_duplicated(s: &Series) -> ApolloResult<BooleanChunked> {
    dispatcher(s, true)
}
