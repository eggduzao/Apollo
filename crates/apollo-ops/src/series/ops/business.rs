...
use chrono::DateTime;
use apollo_core::prelude::arity::{binary_elementwise_values, try_binary_elementwise};
use apollo_core::prelude::*;
#[cfg(feature = "dtype-date")]
use apollo_core::utils::arrow::temporal_conversions::SECONDS_IN_DAY;
use apollo_utils::binary_search::{find_first_ge_index, find_first_gt_index};
#[cfg(feature = "serde")]
...
pub fn business_day_count(
    ...
    if !week_mask.iter().any(|&x| x) {
        apollo_bail!(ComputeError:"`week_mask` must have at least one business day");
    }
...
pub fn add_business_days(
    ...
    match start.dtype() {
        ...
        _ => apollo_bail!(InvalidOperation: "expected date or datetime, got {}", start.dtype()),
    }
    ...
    let n = match &n.dtype() {
        ...
        _ => {
            apollo_bail!(InvalidOperation: "expected Int64, Int32, UInt64, or UInt32, got {}", n.dtype())
        },
    };
    let n = n.i32()?;
    let n_business_days_in_week_mask = week_mask.iter().filter(|&x| *x).count() as i32;
            ...
            if let Some(n) = n.get(0) {
                start_dates
                    ...
                    .try_apply_nonnull_values_generic(|start_date| {
                        ...
                        Ok::<i32, ApolloError>(add_business_days_impl(
                            start_date,
                            day_of_week,
                            n,
                            &week_mask,
            ...
            try_binary_elementwise(start_dates.physical(), n, |opt_start_date, opt_n| {
                match (opt_start_date, opt_n) {
                    (Some(start_date), Some(n)) => {
                        let (start_date, day_of_week) =
                            roll_start_date(start_date, roll, &week_mask, &holidays)?;
                        Ok::<Option<i32>, ApolloError>(Some(add_business_days_impl(
                            start_date,
                            day_of_week,
                            n,
                            &week_mask,
...
pub fn is_business_day(
    ...
    match dates.dtype() {
        ...
        _ => apollo_bail!(InvalidOperation: "expected date or datetime, got {}", dates.dtype()),
    }
