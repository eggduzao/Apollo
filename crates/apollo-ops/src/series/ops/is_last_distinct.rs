...
pub fn is_last_distinct(s: &Series) -> ApolloResult<BooleanChunked> {
    // fast path.
    if s.is_empty() {
    ...
    let out = match s.dtype() {
        ...
        dt => apollo_bail!(opq = is_last_distinct, dt),
    };
    Ok(out)
}
...
#[cfg(feature = "dtype-struct")]
fn is_last_distinct_struct(s: &Series) -> ApolloResult<BooleanChunked> {
...
fn is_last_distinct_list(ca: &ListChunked) -> ApolloResult<BooleanChunked> {
