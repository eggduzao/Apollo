...
use apollo_core::series::arithmetic::coerce_lhs_rhs;
use apollo_core::utils::dtypes_to_supertype;
use apollo_core::{POOL, with_match_physical_numeric_apollo_type};
use rayon::iter::{IntoParallelIterator, IntoParallelRefIterator, ParallelIterator};

fn validate_column_lengths(cs: &[Column]) -> ApolloResult<()> {
    let mut length = 1;
    for c in cs {
        let len = c.len();
        if len != 1 && len != length {
            if length == 1 {
                ...
                apollo_bail!(ShapeMismatch: "cannot evaluate two Series of different lengths ({len} and {length})");
            }
        }
    }
    Ok(())
...
pub trait MinMaxHorizontal {
    ...
    fn min_horizontal(&self) -> ApolloResult<Option<Column>>;
    /// Aggregate the column horizontally to their max values.
    fn max_horizontal(&self) -> ApolloResult<Option<Column>>;
}
...
pub trait SumMeanHorizontal {
    ...
    fn sum_horizontal(&self, null_strategy: NullStrategy) -> ApolloResult<Option<Column>>;

    /// Compute the mean of all numeric values horizontally across columns.
    fn mean_horizontal(&self, null_strategy: NullStrategy) -> ApolloResult<Option<Column>>;
}
...
fn min_max_binary_columns(left: &Column, right: &Column, min: bool) -> ApolloResult<Column> {
        ...
        && left.len() == right.len()
    {
        match (left, right) {
            (Column::Series(left), Column::Series(right)) => {
                ...
                with_match_physical_numeric_apollo_type!(lhs.dtype(), |$T| {
...
pub fn max_horizontal(columns: &[Column]) -> ApolloResult<Option<Column>> {
...
pub fn min_horizontal(columns: &[Column]) -> ApolloResult<Option<Column>> {
    validate_column_lengths(columns)?;
...
pub fn sum_horizontal(
...
) -> ApolloResult<Option<Column>> {
    validate_column_lengths(columns)?;
    let ignore_nulls = null_strategy == NullStrategy::Ignore;

    let apply_null_strategy = |s: Series| -> ApolloResult<Series> {
    ...
    let sum_fn = |acc: Series, s: Series| -> ApolloResult<Series> {
...
pub fn coalesce_columns(s: &[Column]) -> ApolloResult<Column> {
    // TODO! this can be faster if we have more than two inputs.
    apollo_ensure!(!s.is_empty(), NoData: "cannot coalesce empty list");
