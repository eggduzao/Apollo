    ...
    T::Native: Float + SubAssign + Pow<T::Native, Output = T::Native>,
{
    ...
    let arr = if arr.has_nulls() {
        apollo_compute::rolling::nulls::rolling_skew(arr, window_size, min_periods, center, params)
    } else {
        let values = arr.values();
        apollo_compute::rolling::no_nulls::rolling_skew(
            values,
            window_size,
            min_periods,
            center,
...
#[cfg(feature = "moment")]
pub fn rolling_skew(s: &Series, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
    ...
    match s.dtype() {
        ...
        dt => apollo_bail!(opq = rolling_skew, dt),
    }
}
    ...
    T::Native: Float + SubAssign + Pow<T::Native, Output = T::Native>,
{
    ...
    let arr = ca.downcast_get(0).unwrap();
    let arr = if arr.has_nulls() {
        apollo_compute::rolling::nulls::rolling_kurtosis(
            arr,
            window_size,
            min_periods,
            center,
        ...
        let values = arr.values();
        apollo_compute::rolling::no_nulls::rolling_kurtosis(
            values,
            window_size,
            min_periods,
            center,
...
#[cfg(feature = "moment")]
pub fn rolling_kurtosis(s: &Series, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
    let window_size = options.window_size;
    let min_periods = options.min_periods;
    let center = options.center;
    let params = options.fn_params;
    ...
    match s.dtype() {
        ...
        dt => apollo_bail!(opq = rolling_kurtosis, dt),
    }
}
