...
pub fn unique_counts(s: &Series) -> ApolloResult<Series> {
    if s.is_empty() {
    ...
    match s.dtype().to_physical() {
        dt if dt.is_primitive_numeric() => {
            ...
            with_match_physical_numeric_apollo_type!(s_physical.dtype(), |$T| {
        ...
        #[cfg(feature = "object")]
        dt @ DataType::Object(..) => apollo_bail!(opq = unique_counts, dt),
        dt @ DataType::Unknown(..) => apollo_bail!(opq = unique_counts, dt),
    }
}
