...
use apollo_core::chunked_array::ops::row_encode::_get_rows_encoded_ca;
...
use apollo_utils::total_ord::TotalOrd;
...
pub trait SeriesMethods: SeriesSealed {
    ...
    fn ensure_sorted_arg(&self, operation: &str) -> ApolloResult<()> {
        apollo_ensure!(self.is_sorted(Default::default())?, InvalidOperation: "argument in operation '{}' is not sorted, please sort the 'expr/series/column' first", operation);
        Ok(())
    }

    /// Checks if a [`Series`] is sorted. Tries to fail fast.
    fn is_sorted(&self, options: SortOptions) -> ApolloResult<bool> {
        let s = self.as_series();
...
fn is_sorted_ca_num<T: ApolloNumericType>(ca: &ChunkedArray<T>, options: SortOptions) -> bool {
