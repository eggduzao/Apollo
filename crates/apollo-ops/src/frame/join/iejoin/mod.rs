...
use apollo_core::datatypes::{IdxCa, NumericNative, ApolloNumericType};
use apollo_core::frame::DataFrame;
use apollo_core::prelude::*;
use apollo_core::series::IsSorted;
use apollo_core::utils::{_set_partition_size, split};
use apollo_core::{POOL, with_match_physical_numeric_apollo_type};
use apollo_error::{ApolloResult, apollo_err};
use apollo_utils::IdxSize;
use apollo_utils::binary_search::ExponentialSearch;
use apollo_utils::itertools::Itertools;
use apollo_utils::total_ord::{TotalEq, TotalOrd};
use rayon::prelude::*;
#[cfg(feature = "serde")]
...
fn ie_join_impl_t<T: ApolloNumericType>(
    ...
    left_height: usize,
) -> ApolloResult<(Vec<IdxSize>, Vec<IdxSize>)> {
...
pub(super) fn iejoin_par(
    ...
    let row_indices = POOL.install(|| iter.collect::<ApolloResult<Vec<_>>>())?;
...
fn iejoin_tuples(
    ...
    options: &IEJoinOptions,
    slice: Option<(i64, usize)>,
) -> ApolloResult<(IdxCa, IdxCa)> {
    if selected_left.len() != 2 {
        return Err(
            apollo_err!(ComputeError: "IEJoin requires exactly two expressions from the left DataFrame"),
        );
    };
    if selected_right.len() != 2 {
        return Err(
            apollo_err!(ComputeError: "IEJoin requires exactly two expressions from the right DataFrame"),
        );
    };
    ...
    let op2 = match options.operator2 {
        None => {
            return Err(apollo_err!(ComputeError: "IEJoin requires two inequality operators"));
        },
        Some(op2) => op2,
    };
    ...
    let (left_row_idx, right_row_idx) = with_match_physical_numeric_apollo_type!(x.dtype(), |$T| {
         ie_join_impl_t::<$T>(
            slice,
            l1_order,
            l2_order,
...
fn piecewise_merge_join_tuples(
    ...
    if selected_left.len() != 1 {
        return Err(
            apollo_err!(ComputeError: "Piecewise merge join requires exactly one expression from the left DataFrame"),
        );
    };
    if selected_right.len() != 1 {
        return Err(
            apollo_err!(ComputeError: "Piecewise merge join requires exactly one expression from the right DataFrame"),
        );
    };
    if options.operator2.is_some() {
        return Err(
            apollo_err!(ComputeError: "Piecewise merge join expects only one inequality operator"),
        );
    }
    ...
    let (left_row_idx, right_row_idx) = with_match_physical_numeric_apollo_type!(left_ordered.dtype(), |$T| {
        match op {
