...
use apollo_core::datatypes::{IdxCa, ApolloNumericType};
use apollo_core::prelude::Series;
...
) -> ApolloResult<Vec<L1Item<T::Native>>>
where
    T: ApolloNumericType,
{
...
pub(super) fn build_l2_array(s: &Series, order: &[IdxSize]) -> ApolloResult<Vec<L2Item>> {
...
fn build_l2_array_impl<T>(ca: &ChunkedArray<T>, order: &[IdxSize]) -> ApolloResult<Vec<L2Item>>
where
    T: ApolloNumericType,
    T::Native: TotalOrd,
{
