...
fn merge_series(lhs: &Series, rhs: &Series, merge_indicator: &[bool]) -> ApolloResult<Series> {
    use DataType::*;
    let out = match lhs.dtype() {
        ...
        dt => apollo_bail!(op = "merge_sorted", dt),
    };
    Ok(out)
}
...
fn series_to_merge_indicator(lhs: &Series, rhs: &Series) -> ApolloResult<Vec<bool>> {
    ...
    let out = match lhs_s.dtype() {
        ...
        dt if dt.is_primitive_numeric() => {
            with_match_physical_numeric_apollo_type!(lhs_s.dtype(), |$T| {
