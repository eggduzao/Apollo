...
use apollo_core::chunked_array::ops::row_encode::encode_rows_unordered;
use apollo_core::series::BitRepr;
use apollo_core::utils::split;
use apollo_core::with_match_physical_float_apollo_type;
use apollo_utils::aliases::PlRandomState;
...
pub trait SeriesJoin: SeriesSealed + Sized {
    ...
    fn hash_join_left(
        ...
        match lhs_dtype {
            ...
            x if x.is_float() => {
                with_match_physical_float_apollo_type!(lhs.dtype(), |$T| {
            ...
            _ => {
                ...
                let (Some(lhs), Some(rhs)) = (lhs, rhs) else {
                    apollo_bail!(nyi = "Hash Left Join between {lhs_dtype} and {rhs_dtype}");
                };
    ...
    fn hash_join_semi_anti(
        ...
        Ok(match lhs_dtype {
            ...
            _ => {
                ...
                let (Some(lhs), Some(rhs)) = (lhs, rhs) else {
                    apollo_bail!(nyi = "Hash Semi-Anti Join between {lhs_dtype} and {rhs_dtype}");
                };
    ...
    fn hash_join_inner(
        ...
        validate: JoinValidation,
        nulls_equal: bool,
    ) -> ApolloResult<(InnerJoinIds, bool)> {
        let s_self = self.as_series();
        let (lhs, rhs) = (s_self.to_physical_repr(), other.to_physical_repr());
        ...
        match lhs_dtype {
            ...
            _ => {
                ...
                let (Some(lhs), Some(rhs)) = (lhs, rhs) else {
                    apollo_bail!(nyi = "Hash Inner Join between {lhs_dtype} and {rhs_dtype}");
                };
    ...
    fn hash_join_outer(
        ...
        validate: JoinValidation,
        nulls_equal: bool,
    ) -> ApolloResult<(PrimitiveArray<IdxSize>, PrimitiveArray<IdxSize>)> {
        let s_self = self.as_series();
        let (lhs, rhs) = (s_self.to_physical_repr(), other.to_physical_repr());
        validate.validate_probe(&lhs, &rhs, true, nulls_equal)?;
        ...
        match lhs_dtype {
            ...
            _ => {
                let (Some(lhs), Some(rhs)) = (s_self.bit_repr(), other.bit_repr()) else {
                    apollo_bail!(nyi = "Hash Join Outer between {lhs_dtype} and {rhs_dtype}");
                };
...
fn get_arrays<T: ApolloDataType>(cas: &[ChunkedArray<T>]) -> Vec<&T::Array> {
