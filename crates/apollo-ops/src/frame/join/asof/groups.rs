...
use num_traits::Zero;
use apollo_core::hashing::_HASHMAP_INIT_SIZE;
use apollo_core::prelude::*;
use apollo_core::series::BitRepr;
use apollo_core::utils::flatten::flatten_nullable;
use apollo_core::utils::split_and_flatten;
use apollo_core::{POOL, with_match_physical_float_apollo_type};
use apollo_utils::abs_diff::AbsDiff;
use apollo_utils::hashing::{DirtyHash, hash_to_partition};
    ...
    F: Sync + for<'a> Fn(T::Physical<'a>, T::Physical<'a>) -> bool,
{
    let out = if left_by.width() == 1 {
        ...
        apollo_ensure!(left_dtype == right_dtype,
        ...
        match left_dtype {
            ...
            x if x.is_float() => {
                with_match_physical_float_apollo_type!(left_by_s.dtype(), |$T| {
            ...
            _ => {
                ...
                let (Some(left_by), Some(right_by)) = (left_by, right_by) else {
                    apollo_bail!(nyi = "Dispatch join for {left_dtype} and {right_dtype}");
                };
        ...
        for (lhs, rhs) in left_by.columns().iter().zip(right_by.columns()) {
            apollo_ensure!(lhs.dtype() == rhs.dtype(),
...
#[allow(clippy::too_many_arguments)]
fn dispatch_join_strategy<T: ApolloDataType>(
    left_asof: &ChunkedArray<T>,
    right_asof: &Series,
    left_by: &mut DataFrame,
    right_by: &mut DataFrame,
...
#[allow(clippy::too_many_arguments)]
fn dispatch_join_strategy_numeric<T: ApolloNumericType>(
    left_asof: &ChunkedArray<T>,
    right_asof: &Series,
    left_by: &mut DataFrame,
    right_by: &mut DataFrame,
...
fn dispatch_join_type(
    ...
    match left_asof.dtype() {
        ...
        dt => apollo_bail!(opq = asof_join, dt),
    }
}
