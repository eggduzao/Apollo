...
use apollo_core::chunked_array::ops::row_encode::{
...
pub(super) use apollo_core::series::IsSorted;
use apollo_core::utils::slice_offsets;
#[allow(unused_imports)]
use apollo_core::utils::slice_slice;
use apollo_utils::hashing::BytesHash;
use rayon::prelude::*;
...
pub trait DataFrameJoinOps: IntoDf {
        ...
        #[cfg(feature = "chunked_ids")]
        {
            ...
            if _check_rechunk
                && !(matches!(args.how, JoinType::Left)
                    || std::env::var("APOLLO_NO_CHUNKED_JOIN").is_ok())
            {
    ...
    /// # use apollo_core::prelude::*;
    /// # use apollo_ops::prelude::*;
    /// fn join_dfs(left: &DataFrame, right: &DataFrame) -> ApolloResult<DataFrame> {
...
fn prepare_keys_multiple(s: &[Series], nulls_equal: bool) -> ApolloResult<BinaryOffsetChunked> {
    let keys = s
        .iter()
        .map(|s| {
