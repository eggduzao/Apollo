...
pub trait CrossJoinFilter: Send + Sync {
    fn apply(&self, df: DataFrame) -> ApolloResult<DataFrame>;
}

impl<T> CrossJoinFilter for T
where
    T: Fn(DataFrame) -> ApolloResult<DataFrame> + Send + Sync,
{
    fn apply(&self, df: DataFrame) -> ApolloResult<DataFrame> {
        self(df)
    }
...
impl JoinValidation {
    ...
    pub fn is_valid_join(&self, join_type: &JoinType) -> ApolloResult<()> {
        if !self.needs_checks() {
            return Ok(());
        }
        apollo_ensure!(matches!(join_type, JoinType::Inner | JoinType::Full | JoinType::Left),
    ...
    pub(super) fn validate_probe(
        ...
        apollo_ensure!(valid, ComputeError: "join keys did not fulfill {} validation", self);
        Ok(())
    }
