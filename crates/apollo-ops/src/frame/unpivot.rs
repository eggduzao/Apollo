...
use apollo_core::datatypes::{DataType, PlSmallStr};
use apollo_core::frame::DataFrame;
use apollo_core::frame::column::Column;
use apollo_core::prelude::{IntoVec, Series, UnpivotArgsIR};
use apollo_core::utils::merge_dtypes_many;
...
pub trait UnpivotDF: IntoDf {
    ...
    /// ```
    fn unpivot<I, J>(&self, on: Option<I>, index: J) -> ApolloResult<DataFrame>
    where
    ...
    fn unpivot2(&self, args: UnpivotArgsIR) -> ApolloResult<DataFrame> {
        let UnpivotArgsIR {
            on,
            index,
            variable_name,
        ...
        let dtypes = on
            .iter()
            .map(|v| schema.get(v).ok_or_else(|| apollo_err!(col_not_found = v)))
        ...
        for value_column_name in &on {
            ...
            let value_col = col.cast(&st).map_err(
                |_| apollo_err!(InvalidOperation: "'unpivot' not supported for dtype: {}\n\nConsider casting to String.", col.dtype()),
            )?;
