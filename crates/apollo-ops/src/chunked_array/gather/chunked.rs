...
use apollo_core::prelude::gather::_update_gather_sorted_flag;
    ...
    unsafe fn take_chunked_unchecked<const B: u64>(
        ...
        match self.dtype() {
            dt if dt.is_primitive_numeric() => {
                with_match_physical_numeric_apollo_type!(self.dtype(), |$T| {
...
unsafe fn take_unchecked_object<const B: u64>(
    ...
    use apollo_core::chunked_array::object::registry::get_object_builder;
...
where
    T: ApolloDataType<Array = BinaryViewArrayGeneric<V>>,
    T::Array: Debug,
    V: ViewType + ?Sized,
{
    if avoid_sharing {
...
mod test {
    ...
    fn test_list_categorical_dtype_preserved_after_take() {
        ...
        unsafe {
            ...
            // Verify the Apollo dtype is preserved
