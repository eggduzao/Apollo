...
use apollo_compute::rolling::no_nulls::{MaxWindow, MinWindow};
use apollo_core::frame::group_by::aggregations::{
...
unsafe fn group_nan_max<T: ApolloFloatType>(ca: &ChunkedArray<T>, groups: &GroupsType) -> Series {
    match groups {
...
unsafe fn group_nan_min<T: ApolloFloatType>(ca: &ChunkedArray<T>, groups: &GroupsType) -> Series {
    match groups {
        GroupsType::Idx(groups) => _agg_helper_idx::<T, _>(groups, |(first, idx)| {
            debug_assert!(idx.len() <= ca.len());
            if idx.is_empty() {
