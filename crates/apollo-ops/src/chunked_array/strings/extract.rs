...
use apollo_core::prelude::arity::{try_binary_mut_with_options, try_unary_mut_with_options};
use regex::Regex;
...
pub(super) fn extract_group(
    ...
    match (ca.len(), pat.len()) {
        ...
        _ => {
            apollo_bail!(ComputeError: "ca(len: {}) and pat(len: {}) should either broadcast or have the same length", ca.len(), pat.len())
        },
    }
}
