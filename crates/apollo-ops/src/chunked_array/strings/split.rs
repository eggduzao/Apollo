...
use apollo_core::chunked_array::ops::arity::binary_elementwise_for_each;
use apollo_core::prelude::*;
use apollo_utils::regex_cache::compile_regex;
use regex::Regex;
    ...
    I: Iterator<Item = &'a str>,
{
    Ok(match (ca.len(), by.len()) {
        ...
        _ => apollo_bail!(length_mismatch = "str.split", ca.len(), by.len()),
    })
}
...
#[inline]
fn invalid_regex_err(pat: &str) -> ApolloError {
    apollo_err!(ComputeError: "invalid regex pattern in str.split_regex: {}", pat)
}
...
pub fn split_regex_helper(
    ...
    Ok(match (ca.len(), by.len()) {
        ...
        _ => apollo_bail!(length_mismatch = "str.split_regex", ca.len(), by.len()),
    })
}
