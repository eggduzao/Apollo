...
use num_traits::Num;
use apollo_core::prelude::arity::*;
use apollo_utils::regex_cache::{compile_regex, with_regex_cache};
...
where
    ...
    <<T as apollo_core::datatypes::ApolloNumericType>::Native as num_traits::Num>::FromStrRadixErr:
        std::fmt::Display,
{
    let f = |opt_s: Option<&str>, opt_base: Option<u32>| -> ApolloResult<Option<T::Native>> {
        let (Some(s), Some(base)) = (opt_s, opt_base) else {
            return Ok(None);
        };

        if !(2..=36).contains(&base) {
            apollo_bail!(ComputeError: "`to_integer` called with invalid base '{base}'");
        }
...
pub trait StringNameSpaceImpl: AsString {
    ...
    fn hex_decode(&self) -> ApolloResult<StringChunked> {
        panic!("activate 'binary_encoding' feature")
    }

    #[cfg(feature = "binary_encoding")]
    fn hex_decode(&self, strict: bool) -> ApolloResult<BinaryChunked> {
    ...
    #[cfg(not(feature = "binary_encoding"))]
    fn base64_decode(&self) -> ApolloResult<StringChunked> {
        panic!("activate 'binary_encoding' feature")
    }

    #[cfg(feature = "binary_encoding")]
    fn base64_decode(&self, strict: bool) -> ApolloResult<BinaryChunked> {
        let ca = self.as_string();
    ...
    fn to_integer(
        ...
        match dtype.unwrap_or(DataType::Int64) {
            ...
            dtype => apollo_bail!(InvalidOperation: "Invalid dtype {:?}", dtype),
        }
    }
    ...
    fn find_chunked(
        ...
        if literal {
            ...
            with_regex_cache(|reg_cache| {
                let matcher = |src: Option<&str>, pat: Option<&str>| -> ApolloResult<Option<u32>> {
    ...
    fn contains(&self, pat: &str, strict: bool) -> ApolloResult<BooleanChunked> {
        let ca = self.as_string();
        let res_reg = apollo_utils::regex_cache::compile_regex(pat);
    ...
    fn find(&self, pat: &str, strict: bool) -> ApolloResult<UInt32Chunked> {
        ...
        match apollo_utils::regex_cache::compile_regex(pat) {
            ...
            Err(e) => Err(ApolloError::ComputeError(
    ...
    fn replace<'a>(&'a self, pat: &str, val: &str) -> ApolloResult<StringChunked> {
    ...
    fn replace_all(&self, pat: &str, val: &str) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
        let reg = apollo_utils::regex_cache::compile_regex(pat)?;
    ...
    fn replace_literal_all<'a>(&'a self, pat: &str, val: &str) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
        if ca.is_empty() {
            return Ok(ca.clone());
        }
    ...
    fn extract(&self, pat: &StringChunked, group_index: usize) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
        super::extract::extract_group(ca, pat, group_index)
    }

    /// Extract each successive non-overlapping regex match in an individual string as an array.
    fn extract_all(&self, pat: &str) -> ApolloResult<ListChunked> {
        let ca = self.as_string();
        let reg = apollo_utils::regex_cache::compile_regex(pat)?;
    ...
    fn strip_chars(&self, pat: &Column) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
    ...
    fn strip_chars_start(&self, pat: &Column) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
        if pat.dtype() == &DataType::Null {
    ...
    fn strip_chars_end(&self, pat: &Column) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
        if pat.dtype() == &DataType::Null {
    ...
    fn split_exact(&self, by: &StringChunked, n: usize) -> ApolloResult<StructChunked> {
        let ca = self.as_string();
    ...
    #[cfg(feature = "dtype-struct")]
    fn split_exact_inclusive(&self, by: &StringChunked, n: usize) -> ApolloResult<StructChunked> {
        let ca = self.as_string();
    ...
    #[cfg(feature = "dtype-struct")]
    fn splitn(&self, by: &StringChunked, n: usize) -> ApolloResult<StructChunked> {
        let ca = self.as_string();

        split_to_struct(ca, by, n, |s, by| s.splitn(n, by), true)
    }

    fn split(&self, by: &StringChunked) -> ApolloResult<ListChunked> {
        let ca = self.as_string();
        split_helper(ca, by, str::split)
    }

    fn split_inclusive(&self, by: &StringChunked) -> ApolloResult<ListChunked> {
        let ca = self.as_string();
        split_helper(ca, by, str::split_inclusive)
    }

    /// Extract each successive non-overlapping regex match in an individual string as an array.
    fn extract_all_many(&self, pat: &StringChunked) -> ApolloResult<ListChunked> {
        let ca = self.as_string();
        apollo_ensure!(
            ca.len() == pat.len(),
    ...
    fn extract_groups(&self, pat: &str, dtype: &DataType) -> ApolloResult<Series> {
        let ca = self.as_string();
        super::extract::extract_groups(ca, pat, dtype)
    }

    /// Count all successive non-overlapping regex matches.
    fn count_matches(&self, pat: &str, literal: bool) -> ApolloResult<UInt32Chunked> {
        let ca = self.as_string();
        if literal {
    ...
    fn str_slice(&self, offset: &Column, length: &Column) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
    ...
    fn str_head(&self, n: &Column) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
    ...
    fn str_tail(&self, n: &Column) -> ApolloResult<StringChunked> {
        let ca = self.as_string();
        let n = n.strict_cast(&DataType::Int64)?;
