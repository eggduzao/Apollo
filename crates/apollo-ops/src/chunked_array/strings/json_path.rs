...
use apollo_core::prelude::arity::{broadcast_try_binary_elementwise, unary_elementwise};
...
pub trait Utf8JsonPathImpl: AsString {
    ...
    fn json_path_match(&self, json_path: &StringChunked) -> ApolloResult<StringChunked> {
        ...
        match (ca.len(), json_path.len()) {
            (_, 1) => {
                ...
                let out = if let Some(path) = opt_path {
                    let pat = PathCompiled::compile(path).map_err(
                        |e| apollo_err!(ComputeError: "error compiling JSON path expression {}", e),
                    )?;
                ...
                broadcast_try_binary_elementwise(ca, json_path, |opt_str, opt_path| {
                    ...
                    (Some(str_val), Some(path)) => {
                        PathCompiled::compile(path)
                            .map_err(|e| apollo_err!(ComputeError: "error compiling JSON path expression {}", e))
            ...
            (len_ca, len_path) => {
                apollo_bail!(ComputeError: "The length of `ca` and `json_path` should either 1 or the same, but `{}`, `{}` founded", len_ca, len_path)
            },
        }
    }
    ...
    fn json_infer(&self, number_of_rows: Option<usize>) -> ApolloResult<DataType> {
        let ca = self.as_string();
        let values_iter = ca
            .iter()
        ...
        apollo_json::ndjson::infer_iter(values_iter)
            .map(|d| DataType::from_arrow_dtype(&d))
            .map_err(|e| apollo_err!(ComputeError: "error inferring JSON: {}", e))
    }
    ...
    fn json_decode(
        ...
        let array = apollo_json::ndjson::deserialize::deserialize_iter(
            iter,
        ...
        .map_err(|e| apollo_err!(ComputeError: "error deserializing JSON: {}", e))?;
    ...
    fn json_path_select(&self, json_path: &str) -> ApolloResult<StringChunked> {
        let pat = PathCompiled::compile(json_path)
            .map_err(|e| apollo_err!(ComputeError: "error compiling JSONpath expression: {}", e))?;
        Ok(self
            .as_string()
