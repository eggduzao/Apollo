...
use apollo_core::prelude::{ChunkFullNull, Int64Chunked, StringChunked, UInt64Chunked};
...
pub(super) fn head(ca: &StringChunked, n: &Int64Chunked) -> ApolloResult<StringChunked> {
    match (ca.len(), n.len()) {
        (len, 1) => {
            let n = n.get(0);
            let Some(n) = n else {
        ...
        (a, b) => {
            apollo_ensure!(a == b, ShapeMismatch: "lengths of arguments do not align in 'str.head' got length: {} for column: {}, got length: {} for argument 'n'", a, ca.name(), b);
            Ok(binary_elementwise(ca, n, head_binary))
        },
    }
}

pub(super) fn tail(ca: &StringChunked, n: &Int64Chunked) -> ApolloResult<StringChunked> {
    Ok(match (ca.len(), n.len()) {
        ...
        (a, b) => {
            apollo_ensure!(a == b, ShapeMismatch: "lengths of arguments do not align in 'str.tail' got length: {} for column: {}, got length: {} for argument 'n'", a, ca.name(), b);
