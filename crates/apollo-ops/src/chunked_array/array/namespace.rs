...
use apollo_core::prelude::arity::unary_kernel;
use apollo_core::utils::slice_offsets;
...
pub trait ArrayNameSpace: AsArray {
    ...
    fn array_sum(&self) -> ApolloResult<Series> {
        let ca = self.as_array();
    ...
    fn array_mean(&self) -> ApolloResult<Series> {
        let ca = self.as_array();
        dispersion::mean_with_nulls(ca)
    }

    fn array_median(&self) -> ApolloResult<Series> {
        let ca = self.as_array();
        dispersion::median_with_nulls(ca)
    }

    fn array_std(&self, ddof: u8) -> ApolloResult<Series> {
        let ca = self.as_array();
        dispersion::std_with_nulls(ca, ddof)
    }

    fn array_var(&self, ddof: u8) -> ApolloResult<Series> {
        let ca = self.as_array();
        dispersion::var_with_nulls(ca, ddof)
    }

    fn array_unique(&self) -> ApolloResult<ListChunked> {
        let ca = self.as_array();
        ca.try_apply_amortized_to_list(|s| s.as_ref().unique())
    }

    fn array_unique_stable(&self) -> ApolloResult<ListChunked> {
        let ca = self.as_array();
        ca.try_apply_amortized_to_list(|s| s.as_ref().unique_stable())
    }

    fn array_n_unique(&self) -> ApolloResult<IdxCa> {
        let ca = self.as_array();
    ...
    #[cfg(feature = "array_any_all")]
    fn array_any(&self) -> ApolloResult<Series> {
        let ca = self.as_array();
        array_any(ca)
    }

    #[cfg(feature = "array_any_all")]
    fn array_all(&self) -> ApolloResult<Series> {
        let ca = self.as_array();
        array_all(ca)
    }

    fn array_sort(&self, options: SortOptions) -> ApolloResult<ArrayChunked> {
        let ca = self.as_array();
    ...
    fn array_get(&self, index: &Int64Chunked, null_on_oob: bool) -> ApolloResult<Series> {
        let ca = self.as_array();
        array_get(ca, index, null_on_oob)
    }

    fn array_join(&self, separator: &StringChunked, ignore_nulls: bool) -> ApolloResult<Series> {
        let ca = self.as_array();
        array_join(ca, separator, ignore_nulls).map(|ok| ok.into_series())
    }

    #[cfg(feature = "array_count")]
    fn array_count_matches(&self, element: AnyValue) -> ApolloResult<Series> {
        let ca = self.as_array();
        array_count_matches(ca, element)
    }

    fn array_shift(&self, n: &Series) -> ApolloResult<Series> {
        let ca = self.as_array();
        ...
        let out = match (ca.len(), n.len()) {
            ...
            _ => apollo_bail!(length_mismatch = "arr.shift", ca.len(), n.len()),
        };
        Ok(out.into_series())
    }

    fn array_slice(&self, offset: i64, length: i64) -> ApolloResult<Series> {
