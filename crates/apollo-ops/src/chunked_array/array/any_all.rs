...
fn array_all_any<F>(arr: &FixedSizeListArray, op: F, is_all: bool) -> ApolloResult<BooleanArray>
where
    F: Fn(&BooleanArray) -> bool,
{
    let values = arr.values();

    apollo_ensure!(values.dtype() == &ArrowDataType::Boolean, ComputeError: "expected boolean elements in array");
...
pub(super) fn array_all(ca: &ArrayChunked) -> ApolloResult<Series> {
    let chunks = ca
        .downcast_iter()
...
pub(super) fn array_any(ca: &ArrayChunked) -> ApolloResult<Series> {
    let chunks = ca
        .downcast_iter()
