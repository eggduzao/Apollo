...
pub(super) fn mean_with_nulls(ca: &ArrayChunked) -> ApolloResult<Series> {
...
pub(super) fn median_with_nulls(ca: &ArrayChunked) -> ApolloResult<Series> {
    let mut out = match ca.inner_dtype() {
        #[cfg(feature = "dtype-f16")]
        DataType::Float16 => {
            let out: Float16Chunked = ca
...
pub(super) fn std_with_nulls(ca: &ArrayChunked, ddof: u8) -> ApolloResult<Series> {
    let mut out = match ca.inner_dtype() {
        #[cfg(feature = "dtype-f16")]
        DataType::Float16 => {
            let out: Float16Chunked = ca
...
pub(super) fn var_with_nulls(ca: &ArrayChunked, ddof: u8) -> ApolloResult<Series> {
    let mut out = match ca.inner_dtype() {
        #[cfg(feature = "dtype-f16")]
        DataType::Float16 => {
            let out: Float16Chunked = ca
