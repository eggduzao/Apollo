...
pub(super) fn sum_with_nulls(ca: &ArrayChunked, inner_dtype: &DataType) -> ApolloResult<Series> {
    use DataType::*;
    let mut out = {
        match inner_dtype {
            Boolean => {
            ...
            _ => {
                apollo_bail!(ComputeError: "summing array with dtype: {} not yet supported", ca.dtype())
            },
        }
    };
