...
impl ListToStructArgs {
    ...
    fn set_output_names(&self, columns: &mut [Series]) -> ApolloResult<()> {
        match self {
            Self::FixedWidth(v) => {
...
#[derive(Clone)]
pub struct NameGenerator(pub Arc<dyn Fn(usize) -> ApolloResult<PlSmallStr> + Send + Sync>);

impl NameGenerator {
    pub fn from_func(
        func: impl Fn(usize) -> ApolloResult<PlSmallStr> + Send + Sync + 'static,
    ) -> Self {
        Self(Arc::new(func))
    }
}
...
pub trait ToStruct: AsList {
    fn to_struct(&self, args: &ListToStructArgs) -> ApolloResult<StructChunked> {
        let ca = self.as_list();
