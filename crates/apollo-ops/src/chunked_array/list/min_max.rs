...
pub(super) fn list_min_function(ca: &ListChunked) -> ApolloResult<Series> {
    fn inner(ca: &ListChunked) -> ApolloResult<Series> {
        match ca.inner_dtype() {
            ...
            dt if dt.to_physical().is_primitive_numeric() => {
                with_match_physical_numeric_apollo_type!(dt.to_physical(), |$T| {
...
pub(super) fn list_max_function(ca: &ListChunked) -> ApolloResult<Series> {
    fn inner(ca: &ListChunked) -> ApolloResult<Series> {
        match ca.inner_dtype() {
            DataType::Boolean => {
                let out: BooleanChunked = ca
