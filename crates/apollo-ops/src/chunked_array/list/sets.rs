...
use apollo_core::with_match_physical_numeric_type;
use apollo_utils::total_ord::{ToTotalOrd, TotalEq, TotalHash, TotalOrdWrap};
#[cfg(feature = "serde")]
...
fn array_set_operation(
    ...
    match dtype {
        ...
        ArrowDataType::Boolean => {
            apollo_bail!(InvalidOperation: "boolean type not yet supported in list 'set' operations")
        },
        _ => {
...
pub fn list_set_operation(
    ...
    apollo_ensure!(a.len() == b.len() || b.len() == 1 || a.len() == 1, ShapeMismatch: "column lengths don't match");
    apollo_ensure!(a.dtype() == b.dtype(), InvalidOperation: "cannot do 'set' operation on dtypes: {} and {}", a.dtype(), b.dtype());
    let mut a = a.clone();
    let mut b = b.clone();
    if a.len() != b.len() {
        a.rechunk_mut();
