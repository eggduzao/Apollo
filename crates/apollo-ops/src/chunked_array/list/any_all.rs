...
fn list_all_any<F>(arr: &ListArray<i64>, op: F, is_all: bool) -> ApolloResult<BooleanArray>
where
    F: Fn(&BooleanArray) -> bool,
{
    ...
    apollo_ensure!(values.dtype() == &ArrowDataType::Boolean, ComputeError: "expected boolean elements in list");
...
pub(super) fn list_all(ca: &ListChunked) -> ApolloResult<Series> {
    let chunks = ca
        .downcast_iter()
...
pub(super) fn list_any(ca: &ListChunked) -> ApolloResult<Series> {
    let chunks = ca
        .downcast_iter()
