...
use apollo_compute::gather::sublist::list::{index_is_oob, sublist_get};
use apollo_core::chunked_array::builder::get_list_builder;
#[cfg(feature = "diff")]
...
pub trait ListNameSpaceImpl: AsList {
    ...
    fn lst_join(
        ...
        match ca.inner_dtype() {
            ...
            dt => apollo_bail!(op = "`lst.join`", got = dt, expected = "String"),
        }
    }

    fn join_literal(&self, separator: &str, ignore_nulls: bool) -> ApolloResult<StringChunked> {
        let ca = self.as_list();
    ...
    fn lst_max(&self) -> ApolloResult<Series> {
        list_max_function(self.as_list())
    }

    #[cfg(feature = "list_any_all")]
    fn lst_all(&self) -> ApolloResult<Series> {
        let ca = self.as_list();
        list_all(ca)
    }

    #[cfg(feature = "list_any_all")]
    fn lst_any(&self) -> ApolloResult<Series> {
        let ca = self.as_list();
        list_any(ca)
    }

    fn lst_min(&self) -> ApolloResult<Series> {
        list_min_function(self.as_list())
    }

    fn lst_sum(&self) -> ApolloResult<Series> {
        let ca = self.as_list();
    ...
    fn lst_var(&self, ddof: u8) -> ApolloResult<Series> {
        let ca = self.as_list();
    ...
    fn lst_sort(&self, options: SortOptions) -> ApolloResult<ListChunked> {
        let ca = self.as_list();
    ...
    fn lst_n_unique(&self) -> ApolloResult<IdxCa> {
        let ca = self.as_list();
    ...
    fn lst_unique(&self) -> ApolloResult<ListChunked> {
        let ca = self.as_list();
    ...
    fn lst_unique_stable(&self) -> ApolloResult<ListChunked> {
        let ca = self.as_list();
    ...
    #[cfg(feature = "diff")]
    fn lst_diff(&self, n: i64, null_behavior: NullBehavior) -> ApolloResult<ListChunked> {
        let ca = self.as_list();
        ca.try_apply_amortized(|s| diff(s.as_ref(), n, null_behavior))
    }

    fn lst_shift(&self, periods: &Column) -> ApolloResult<ListChunked> {
        let ca = self.as_list();
    ...
    fn lst_get(&self, idx: i64, null_on_oob: bool) -> ApolloResult<Series> {
        let ca = self.as_list();
    ...
    #[cfg(feature = "list_gather")]
    fn lst_gather_every(&self, n: &IdxCa, offset: &IdxCa) -> ApolloResult<Series> {
        ...
        let out = match (n.len(), offset.len()) {
            ...
            _ => {
                apollo_bail!(ComputeError: "The lengths of `n` and `offset` should be 1 or equal to the length of list.")
            },
        };
        Ok(out.into_series())
    }
    ...
    #[cfg(feature = "list_gather")]
    fn lst_gather(&self, idx: &Series, null_on_oob: bool) -> ApolloResult<Series> {
        let list_ca = self.as_list();
        let idx_ca = idx.list()?;
        ...
        let index_typed_index = |idx: &Series| {
            let idx = idx.cast(&IDX_DTYPE).unwrap();
            {
                list_ca
                    ...
                    .collect::<ApolloResult<ListChunked>>()
                    .map(|mut ca| {
        ...
        match (list_ca.len(), idx_ca.len()) {
                ...
                match idx_ca.dtype() {
                    ...
                    dt if dt.is_signed_integer() => {
                        if let Some(min) = idx_ca.min::<i64>().unwrap() {
                            ...
                            apollo_bail!(ComputeError: "all indices are null");
                        }
                    },
                    dt => apollo_bail!(ComputeError: "cannot use dtype `{dt}` as an index"),
                }
            },
            (a, b) if a == b => {
                let mut out = {
            ...
            (a, b) => apollo_bail!(length_mismatch = "list.gather", a, b),
        }
    }
    ...
    fn lst_concat(&self, other: &[Column]) -> ApolloResult<ListChunked> {
        let ca = self.as_list();
        ...
        let out = if other.iter().all(|s| s.len() == 1) && ca.len() != 1 {
            ...
            let mut first_iter: Box<dyn ApolloIterator<Item = Option<Series>>> = ca.into_iter();
            let mut builder = get_list_builder(
                &inner_super_type,
                ca.get_values_size() + vals_size_other + 1,
                length,
...
#[cfg(feature = "list_gather")]
fn take_series(s: &Series, idx: Series, null_on_oob: bool) -> ApolloResult<Series> {
    let len = s.len();
...
#[cfg(feature = "list_gather")]
fn cast_signed_index_ca<T: ApolloNumericType>(idx: &ChunkedArray<T>, len: usize) -> Series
where
...
#[cfg(feature = "list_gather")]
fn cast_unsigned_index_ca<T: ApolloNumericType>(idx: &ChunkedArray<T>, len: usize) -> Series
where
...
#[cfg(feature = "list_gather")]
fn cast_index(idx: Series, len: usize, null_on_oob: bool) -> ApolloResult<Series> {
