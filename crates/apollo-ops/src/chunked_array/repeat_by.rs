...
fn check_lengths(length_srs: usize, length_by: usize) -> ApolloResult<()> {
    apollo_ensure!(
...
fn repeat_by_primitive<T>(ca: &ChunkedArray<T>, by: &IdxCa) -> ApolloResult<ListChunked>
where
    T: ApolloNumericType,
{
...
fn repeat_by_bool(ca: &BooleanChunked, by: &IdxCa) -> ApolloResult<ListChunked> {
    check_lengths(ca.len(), by.len())?;
...
fn repeat_by_binary(ca: &BinaryChunked, by: &IdxCa) -> ApolloResult<ListChunked> {
    check_lengths(ca.len(), by.len())?;
...
fn repeat_by_list(ca: &ListChunked, by: &IdxCa) -> ApolloResult<ListChunked> {
    check_lengths(ca.len(), by.len())?;
...
fn repeat_by_null(ca: &NullChunked, by: &IdxCa) -> ApolloResult<ListChunked> {
    check_lengths(ca.len(), by.len())?;
...
fn repeat_by_array(ca: &ArrayChunked, by: &IdxCa) -> ApolloResult<ListChunked> {
    check_lengths(ca.len(), by.len())?;
...
fn repeat_by_struct(ca: &StructChunked, by: &IdxCa) -> ApolloResult<ListChunked> {
    check_lengths(ca.len(), by.len())?;
...
fn repeat_by_generic_inner<T: ApolloDataType>(ca: &ChunkedArray<T>, by: &IdxCa) -> ListChunked {
...
pub fn repeat_by(s: &Series, by: &IdxCa) -> ApolloResult<ListChunked> {
    ...
    let out = match s_phys.dtype() {
        ...
        _ => apollo_bail!(opq = repeat_by, s.dtype()),
    };
    out.and_then(|ca| {
