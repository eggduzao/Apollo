...
pub fn hist_series(
    ...
    if let Some(bins) = bins {
        apollo_ensure!(bins.null_count() == 0, InvalidOperation: "nulls not supported in 'bins' argument");
    ...
    apollo_ensure!(s.dtype().is_primitive_numeric(), InvalidOperation: "'hist' is only supported for numeric data");

    let out = with_match_physical_numeric_apollo_type!(s.dtype(), |$T| {
