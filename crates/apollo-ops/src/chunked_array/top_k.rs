...
use apollo_core::chunked_array::ops::sort::arg_bottom_k::_arg_bottom_k;
...
pub fn top_k(s: &[Column], descending: bool) -> ApolloResult<Column> {
    fn extract_target_and_k(s: &[Column]) -> ApolloResult<(usize, &Column)> {
        let k_s = &s[1];
        apollo_ensure!(
            k_s.len() == 1,
        ...
        let Some(k) = k_s.strict_cast(&IDX_DTYPE)?.idx()?.get(0) else {
            apollo_bail!(ComputeError: "`k` must be set for `top_k`")
        };
...
pub fn top_k_by(s: &[Column], descending: Vec<bool>) -> ApolloResult<Column> {
    /// Return (k, src, by)
    fn extract_parameters(s: &[Column]) -> ApolloResult<(usize, &Column, &[Column])> {
        let k_s = &s[1];
    ...
    for s in by {
        if s.len() != src.len() {
            apollo_bail!(ComputeError: "`by` column's ({}) length ({}) should have the same length as the source column length ({}) in `top_k`", s.name(), s.len(), src.len())
        }
    }
