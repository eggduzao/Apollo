...
use apollo_core::prelude::arity::{binary_elementwise, ternary_elementwise, unary_elementwise};
use apollo_core::prelude::{BinaryChunked, ChunkFullNull, DataType, Int64Chunked, UInt64Chunked};
...
pub(super) fn head(ca: &BinaryChunked, n: &Int64Chunked) -> ApolloResult<BinaryChunked> {
    match (ca.len(), n.len()) {
        (len, 1) => {
            let n = n.get(0);
            let Some(n) = n else {
        ...
        (a, b) => {
            apollo_ensure!(a == b, ShapeMismatch: "lengths of arguments do not align in 'bin.head' got length: {} for column: {}, got length: {} for argument 'n'", a, ca.name(), b);
            Ok(binary_elementwise(ca, n, head_binary))
        },
    }
}

pub(super) fn tail(ca: &BinaryChunked, n: &Int64Chunked) -> ApolloResult<BinaryChunked> {
    Ok(match (ca.len(), n.len()) {
        ...
        (a, b) => {
            apollo_ensure!(a == b, ShapeMismatch: "lengths of arguments do not align in 'bin.tail' got length: {} for column: {}, got length: {} for argument 'n'", a, ca.name(), b);
