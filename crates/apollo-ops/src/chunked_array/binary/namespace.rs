...
use apollo_compute::cast::{binview_to_fixed_size_list_dyn, binview_to_primitive_dyn};
use apollo_compute::size::binary_size_bytes;
use apollo_core::prelude::arity::{broadcast_binary_elementwise_values, unary_elementwise_values};
...
pub trait BinaryNameSpaceImpl: AsBinary {
    ...
    fn bin_slice(&self, offset: &Column, length: &Column) -> ApolloResult<BinaryChunked> {
    ...
    fn bin_head(&self, n: &Column) -> ApolloResult<BinaryChunked> {
        let ca = self.as_binary();
    ...
    fn bin_tail(&self, n: &Column) -> ApolloResult<BinaryChunked> {
        let ca = self.as_binary();
        let n = n.strict_cast(&DataType::Int64)?;
    ...
    fn contains_chunked(&self, lit: &BinaryChunked) -> ApolloResult<BooleanChunked> {
        let ca = self.as_binary();
        Ok(match lit.len() {
            1 => match lit.get(0) {
    ...
    fn starts_with_chunked(&self, prefix: &BinaryChunked) -> ApolloResult<BooleanChunked> {
        let ca = self.as_binary();
        Ok(match prefix.len() {
    ...
    fn ends_with_chunked(&self, suffix: &BinaryChunked) -> ApolloResult<BooleanChunked> {
        let ca = self.as_binary();
        Ok(match suffix.len() {
    ...
    #[cfg(feature = "binary_encoding")]
    fn reinterpret(&self, dtype: &DataType, is_little_endian: bool) -> ApolloResult<Series> {
        unsafe {
    ...
    fn _reinterpret_inner(
    ...
    ) -> ApolloResult<Vec<Box<dyn Array>>> {
        ...
        match dtype {
            dtype if dtype.is_primitive_numeric() || dtype.is_temporal() => {
                ...
                with_match_physical_numeric_apollo_type!(dtype, |$T| {
                    unsafe {
                        ca.chunks().iter().map(|chunk| {
                            binview_to_primitive_dyn::<<$T as ApolloNumericType>::Native>(
                                &**chunk,
                                &arrow_data_type,
                                is_little_endian,
                            )
                ...
                if inner_dtype.is_primitive_numeric() || inner_dtype.is_temporal() =>
            {
                ...
                let result: Vec<ArrayRef> = with_match_physical_numeric_apollo_type!(inner_dtype, |$T| {
                    unsafe {
                        ca.chunks().iter().map(|chunk| {
                            binview_to_fixed_size_list_dyn::<<$T as ApolloNumericType>::Native>(
                                &**chunk,
                                *array_width,
                                is_little_endian
                            )
            ...
            _ => Err(
                apollo_err!(InvalidOperation: "unsupported data type {:?} in reinterpret. Only numeric or temporal types, or Arrays of those, are allowed.", dtype),
            ),
        }
    }
}
