...
use apollo_core::prelude::{Column, CompatLevel};
...
pub unsafe fn import_series(e: SeriesExport) -> ApolloResult<Series> {
...
/// # Safety
/// `SeriesExport` must be valid
pub unsafe fn import_series_buffer(e: *mut SeriesExport, len: usize) -> ApolloResult<Vec<Series>> {
...
/// # Safety
/// `SeriesExport` must be valid
pub unsafe fn import_df(e: *mut SeriesExport, len: usize) -> ApolloResult<DataFrame> {
    let mut out = Vec::with_capacity(len);
    for i in 0..len {
        let e = std::ptr::read(e.add(i));
...
impl CallerContext {
    ...
    /// Parallelism is done by apollo' main engine, the plugin should not run its own parallelism.
    /// If this is `false`, the plugin could use parallelism without (much) contention with apollo
