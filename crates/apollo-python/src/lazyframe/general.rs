...
#[cfg(feature = "parquet")]
use apollo_parquet::arrow::write::StatisticsOptions;
use apollo_plan::dsl::ScanSources;
use apollo_plan::plans::{AExpr, HintIR, IR, Sorted};
...
impl PyLazyFrame {
    ...
    fn new_from_csv(
        ...
        let separator = separator
            .as_bytes()
            .first()
            .ok_or_else(|| apollo_err!(InvalidOperation: "`separator` cannot be empty"))
            .copied()
            .map_err(PyApolloErr::from)?;
        let eol_char = eol_char
            .as_bytes()
            .first()
            .ok_or_else(|| apollo_err!(InvalidOperation: "`eol_char` cannot be empty"))
            .copied()
            .map_err(PyApolloErr::from)?;
        let row_index = row_index.map(|(name, offset)| RowIndex {
            name: name.into(),
        ...
        if let Some(lambda) = with_schema_modify {
            let f = |schema: Schema| {
                ...
                Python::attach(|py| {
                    ...
                    apollo_ensure!(new_names.len() == schema.len(),
            ...
            r = r.with_schema_modify(f).map_err(PyApolloErr::from)?
        }

        Ok(r.finish().map_err(PyApolloErr::from)?.into())
    }
    ...
    fn describe_plan(&self, py: Python) -> PyResult<String> {
        py.enter_apollo(|| self.ldf.read().describe_plan())
    }

    fn describe_optimized_plan(&self, py: Python) -> PyResult<String> {
        py.enter_apollo(|| self.ldf.read().describe_optimized_plan())
    }

    fn describe_plan_tree(&self, py: Python) -> PyResult<String> {
        py.enter_apollo(|| self.ldf.read().describe_plan_tree())
    }

    fn describe_optimized_plan_tree(&self, py: Python) -> PyResult<String> {
        py.enter_apollo(|| self.ldf.read().describe_optimized_plan_tree())
    }

    fn to_dot(&self, py: Python<'_>, optimized: bool) -> PyResult<String> {
        py.enter_apollo(|| self.ldf.read().to_dot(optimized))
    }

    #[cfg(feature = "new_streaming")]
    fn to_dot_streaming_phys(&self, py: Python, optimized: bool) -> PyResult<String> {
        py.enter_apollo(|| self.ldf.read().to_dot_streaming_phys(optimized))
    }
    ...
    fn profile(
        ...
        let (df, time_df) = py.enter_apollo(|| {
            let ldf = self.ldf.read().clone();
    ...
    fn rolling(
        ...
        let lazy_gb = ldf.rolling(
            ...
            RollingGroupOptions {
                index_column: "".into(),
                period: Duration::try_parse(period).map_err(PyApolloErr::from)?,
                offset: Duration::try_parse(offset).map_err(PyApolloErr::from)?,
                closed_window,
            },
        );
    ...
    fn group_by_dynamic(
        ...
        let lazy_gb = ldf.group_by_dynamic(
            ...
            group_by,
            DynamicGroupOptions {
                every: Duration::try_parse(every).map_err(PyApolloErr::from)?,
                period: Duration::try_parse(period).map_err(PyApolloErr::from)?,
                offset: Duration::try_parse(offset).map_err(PyApolloErr::from)?,
                label: label.0,
                include_boundaries,
    ...
    fn collect_schema<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyDict>> {
        let schema = py.enter_apollo(|| self.ldf.write().collect_schema())?;
...
impl PyCollectBatches {
    ...
    fn __next__(slf: PyRef<'_, Self>, py: Python) -> PyResult<Option<PyDataFrame>> {
        ...
        py.enter_apollo(|| ApolloResult::Ok(inner.lock().next().transpose()?.map(PyDataFrame::new)))
    }
...
impl Iterator for ArrowStreamIterator {
    type Item = ApolloResult<ArrayRef>;
