...
use apollo::prelude::sync_on_close::SyncOnCloseType;
use apollo::prelude::{PlRefPath, SpecialEq};
...
impl<'a, 'py> FromPyObject<'a, 'py> for Wrap<apollo_plan::dsl::SinkTarget> {
    type Error = PyErr;

    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        if let Ok(v) = ob.extract::<PyBackedStr>() {
            Ok(Wrap(apollo::prelude::SinkTarget::Path(PlRefPath::new(&*v))))
        } else {
            ...
            Ok(Wrap(apollo_plan::prelude::SinkTarget::Dyn(SpecialEq::new(
    ...
    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        if let Ok(v) = ob.extract::<PyBackedStr>() {
            ...
            Ok(Wrap(FileProviderReturn::Path(
                v.to_str()
                    .ok_or_else(|| to_py_err(apollo_err!(non_utf8_path)))?
                    .to_string(),
            )))
        } else {
            let py = ob.py();
