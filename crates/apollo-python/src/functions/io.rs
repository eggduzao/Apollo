...
use apollo::prelude::ArrowSchema;
use apollo::prelude::CloudScheme;
use pyo3::prelude::*;
...
pub fn read_ipc_schema(py: Python<'_>, py_f: Py<PyAny>) -> PyResult<Bound<'_, PyDict>> {
    ...
    let metadata = match get_either_file(py_f, false)? {
        EitherRustPythonFile::Rust(r) => {
            read_file_metadata(&mut BufReader::new(r)).map_err(PyApolloErr::from)?
        },
        EitherRustPythonFile::Py(mut r) => read_file_metadata(&mut r).map_err(PyApolloErr::from)?,
    };
...
pub fn read_parquet_metadata(
    ...
    use apollo_parquet::read::read_metadata;
    use apollo_parquet::read::schema::read_custom_key_value_metadata;
    ...
    let metadata = match get_python_scan_source_input(py_f, false)? {
        PythonScanSourceInput::Buffer(buf) => {
            read_metadata(&mut Cursor::new(buf)).map_err(PyApolloErr::from)?
        },
        PythonScanSourceInput::Path(p) => {
            ...
            if p.has_scheme() {
                feature_gated!("cloud", {
                    use apollo::prelude::ParquetObjectStore;
                    ...
                    py.detach(|| {
                        get_runtime().block_on(async {
                            ...
                            ApolloResult::Ok((**result).clone())
                        })
                    })
                })
                .map_err(PyApolloErr::from)?
            } else {
                let file = apollo_utils::open_file(p.as_std_path()).map_err(PyApolloErr::from)?;
                read_metadata(&mut BufReader::new(file)).map_err(PyApolloErr::from)?
            }
        },
        PythonScanSourceInput::File(f) => {
            read_metadata(&mut BufReader::new(f)).map_err(PyApolloErr::from)?
        },
    };
...
fn fields_to_pydict(schema: &ArrowSchema, dict: &Bound<'_, PyDict>) -> PyResult<()> {
    for field in schema.iter_values() {
        let dt = Wrap(apollo::prelude::DataType::from_arrow_field(field));
...
pub fn read_clipboard_string() -> PyResult<String> {
    use arboard;
    let mut clipboard =
        arboard::Clipboard::new().map_err(|e| PyApolloErr::Other(format!("{e}")))?;
    let result = clipboard
        .get_text()
        .map_err(|e| PyApolloErr::Other(format!("{e}")))?;
    Ok(result)
}
