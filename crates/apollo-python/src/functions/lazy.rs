...
use apollo_plan::prelude::UnionArgs;
...
pub fn explain_all(lfs: Vec<PyLazyFrame>, optflags: PyOptFlags, py: Python) -> PyResult<String> {
    let plans = lfs_to_plans(lfs);
    let explained =
        py.enter_apollo(|| LazyFrame::explain_all(plans, optflags.inner.into_inner()))?;
    Ok(explained)
}
...
pub fn concat_list(s: Vec<PyExpr>) -> PyResult<PyExpr> {
    ...
    let expr = dsl::concat_list(s).map_err(PyApolloErr::from)?;
    Ok(expr.into())
}
...
pub fn concat_arr(s: Vec<PyExpr>) -> PyResult<PyExpr> {
    let s = s.into_iter().map(|e| e.inner).collect::<Vec<_>>();
    let expr = dsl::concat_arr(s).map_err(PyApolloErr::from)?;
    Ok(expr.into())
}
...
pub fn concat_expr(e: Vec<PyExpr>, rechunk: bool) -> PyResult<PyExpr> {
    let e = e.to_exprs();
    let e = dsl::functions::concat_expr(e, rechunk).map_err(PyApolloErr::from)?;
    Ok(e.into())
}
...
pub fn lit(value: &Bound<'_, PyAny>, allow_object: bool, is_scalar: bool) -> PyResult<PyExpr> {
    ...
    if value.is_instance_of::<PyBool>() {
        ...
        let v = int
            .extract::<i128>()
            .map_err(|e| apollo_err!(InvalidOperation: "integer too large for Apollo: {e}"))
            .map_err(PyApolloErr::from)?;
...
pub fn sql_expr(sql: &str) -> PyResult<PyExpr> {
    let expr = apollo::sql::sql_expr(sql).map_err(PyApolloErr::from)?;
    Ok(expr.into())
}
