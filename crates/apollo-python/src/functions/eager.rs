...
pub fn concat_df(dfs: &Bound<'_, PyAny>, py: Python) -> PyResult<PyDataFrame> {
    ...
    use apollo_core::utils::rayon::prelude::*;
    ...
    let mut rdfs: Vec<ApolloResult<DataFrame>> = vec![Ok(first_rdf)];
    ...
    py.enter_apollo_df(|| {
        apollo_core::POOL.install(|| {
            rdfs.into_par_iter()
                .fold(identity, |acc: ApolloResult<DataFrame>, df| {
...
pub fn concat_series(series: &Bound<'_, PyAny>) -> PyResult<PySeries> {
    ...
    for res in iter {
        ...
        s.append(&item).map_err(PyApolloErr::from)?;
    }
    Ok(s.into())
}
...
pub fn concat_df_diagonal(dfs: &Bound<'_, PyAny>) -> PyResult<PyDataFrame> {
    ...
    let df = functions::concat_df_diagonal(&dfs).map_err(PyApolloErr::from)?;
    Ok(df.into())
}
...
pub fn concat_df_horizontal(dfs: &Bound<'_, PyAny>, strict: bool) -> PyResult<PyDataFrame> {
    ...
    let df =
        functions::concat_df_horizontal(&dfs, true, strict, false).map_err(PyApolloErr::from)?;
    Ok(df.into())
}
