...
use apollo_ops::series::ClosedInterval;
...
pub fn eager_int_range(
    ...
    let Some(dtype) = dtype.into_literal() else {
        return Err(PyApolloErr::from(
            apollo_err!(ComputeError: "eager `int_range` cannot be given lazy datatype expression"),
        )
        .into());
    };
    ...
    if !dtype.is_integer() {
        return Err(PyApolloErr::from(
            apollo_err!(SchemaMismatch: "non-integer `dtype` passed to `int_range`: '{}'", dtype),
        )
        .into());
    }

    with_match_physical_integer_apollo_type!(dtype, |$T| {
        let start_v: <$T as ApolloNumericType>::Native = lower.extract()?;
        let end_v: <$T as ApolloNumericType>::Native = upper.extract()?;
        let step: i64 = step.extract()?;
        py.enter_apollo_series(|| new_int_range::<$T>(start_v, end_v, step, get_literal_name()))
    })
}
...
pub fn date_range(
    ...
    let start = start.inner;
    let end = end.inner;
    let interval = Duration::try_parse(interval).map_err(PyApolloErr::from)?;
    let closed = closed.0;
...
pub fn linear_spaces(
    ...
    let closed = closed.0;
    let out =
        dsl::linear_spaces(start, end, num_samples, closed, as_array).map_err(PyApolloErr::from)?;
    Ok(out.into())
}
