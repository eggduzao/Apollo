...
use apollo::prelude::{InitHashMaps, PlHashSet};
...
pub fn get_timeout() -> Option<Duration> {
    ...
    // sure that if you want to use APOLLO_TIMEOUT_MS it is set before the first
    // apollo call.
    if TIMEOUT_DISABLED.load() {
        return None;
    }

    let Ok(timeout) = std::env::var("APOLLO_TIMEOUT_MS") else {
    ...
    match timeout.parse() {
        ...
        Err(e) => {
            eprintln!("failed to parse APOLLO_TIMEOUT_MS: {e:?}");
            None
        },
    }
}
        ...
        let request = if let Some(Priority(timeout, _)) = shortest_timeout.peek() {
            match recv.recv_timeout(timeout.0) {
                Err(RecvTimeoutError::Timeout) => {
                    eprintln!("exiting the process, APOLLO_TIMEOUT_MS exceeded");
...
pub fn schedule_apollo_timeout() -> Option<u64> {
...
pub fn cancel_apollo_timeout(opt_id: Option<u64>) {
