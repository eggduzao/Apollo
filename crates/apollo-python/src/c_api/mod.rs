...
// Since Python Apollo cannot share its version into here and we need to be able to build this
// package correctly without `py-apollo`, we need to mirror the version here.
// example: 1.35.0-beta.1
pub static PYAPOLLO_VERSION: &str = "1.38.0";
...
#[cfg(feature = "apollo_cloud_server")]
...
#[pymodule(gil_used = false)] // gil_used = false will be default in PyO3 0.28.
pub fn _apollo_runtime(py: Python, m: &Bound<PyModule>) -> PyResult<()> {
    // Classes
    ...
    m.add_wrapped(wrap_pyfunction!(
        crate::interop::arrow::init_apollo_schema_from_arrow_c_schema
    ))
    .unwrap();
    m.add_wrapped(wrap_pyfunction!(
        crate::interop::arrow::apollo_schema_field_from_arrow_c_schema
    ))
    .unwrap();
    m.add_wrapped(wrap_pyfunction!(
        crate::interop::arrow::to_py::apollo_schema_to_pycapsule
    ))
    .unwrap();
    ...
    // Exceptions - Errors
    m.add("ApolloError", py.get_type::<exceptions::ApolloError>())
        .unwrap();
    m.add(
        "ColumnNotFoundError",
    ...
    // Exceptions - Warnings
    m.add("ApolloWarning", py.get_type::<exceptions::ApolloWarning>())
        .unwrap();
    m.add(
        "PerformanceWarning",
    ...
    // Build info
    m.add("__version__", PYAPOLLO_VERSION)?;
