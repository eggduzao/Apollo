use apollo_core::chunked_array::cast::CastOptions;
use apollo_core::series::IsSorted;
use apollo_core::utils::flatten::flatten_series;
use apollo_utils::python_function::PythonObject;
...
impl PySeries {
    ...
    fn reshape(&self, py: Python<'_>, dims: Vec<i64>) -> PyResult<Self> {
        ...
        py.enter_apollo_series(|| self.series.read().reshape_array(&dims))
    }
    ...
    pub fn rechunk(&self, py: Python<'_>, in_place: bool) -> PyResult<Option<Self>> {
        let series = py.enter_apollo_ok(|| self.series.read().rechunk())?;
        if in_place {
    ...
    fn get_index(&self, py: Python<'_>, index: usize) -> PyResult<Py<PyAny>> {
        ...
        let av = match s.get(index) {
            Ok(v) => v,
            Err(ApolloError::OutOfBounds(err)) => {
                return Err(PyIndexError::new_err(err.to_string()));
            },
            Err(e) => return Err(PyApolloErr::from(e).into()),
        };

        match av {
            AnyValue::List(s) | AnyValue::Array(s, _) => {
                let pyseries = PySeries::new(s);
                apollo(py).getattr(py, "wrap_s")?.call1(py, (pyseries,))
            },
            ...
            match self.len().checked_sub(index.unsigned_abs()) {
                ...
                None => {
                    return Err(PyIndexError::new_err(
                        apollo_err!(oob = index, self.len()).to_string(),
                    ));
                },
            }
        } else {
    ...
    fn bitand(&self, py: Python<'_>, other: &PySeries) -> PyResult<Self> {
        py.enter_apollo_series(|| &*self.series.read() & &*other.series.read())
    }
    ...
    fn new_from_index(&self, py: Python<'_>, index: usize, length: usize) -> PyResult<Self> {
        ...
        if index >= s.len() {
            ...
            py.enter_apollo_series(|| Ok(s.new_from_index(index, length)))
        }
    }
    ...
    fn filter(&self, py: Python<'_>, filter: &PySeries) -> PyResult<Self> {
        ...
        if let Ok(ca) = filter_series.bool() {
            py.enter_apollo_series(|| self.series.read().filter(ca))
        } else {
    ...
    fn gather_with_series(&self, py: Python<'_>, indices: &PySeries) -> PyResult<Self> {
        py.enter_apollo_series(|| self.series.read().take(indices.series.read().idx()?))
    }
    ...
    fn equals(
        ...
        if null_equal {
            py.enter_apollo_ok(|| s.equals_missing(&o))
        } else {
            py.enter_apollo_ok(|| s.equals(&o))
        }
    }
    ...
    fn as_single_ptr(&self, py: Python) -> PyResult<usize> {
        py.enter_apollo(|| self.series.write().as_single_ptr())
    }
    ...
    fn zip_with(&self, py: Python<'_>, mask: &PySeries, other: &PySeries) -> PyResult<Self> {
        ...
        let mask = ms.bool().map_err(PyApolloErr::from)?;
        py.enter_apollo_series(|| self.series.read().zip_with(mask, &other.series.read()))
    }
    ...
    fn n_unique(&self, py: Python) -> PyResult<usize> {
        py.enter_apollo(|| self.series.read().n_unique())
    }

    fn floor(&self, py: Python) -> PyResult<Self> {
        py.enter_apollo_series(|| self.series.read().floor())
    }

    fn shrink_to_fit(&self, py: Python) -> PyResult<()> {
        py.enter_apollo_ok(|| self.series.write().shrink_to_fit())
    }
    ...
    fn dot<'py>(&self, other: &PySeries, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        ...
        if !lhs_dtype.is_primitive_numeric() {
            return Err(PyApolloErr::from(apollo_err!(opq = dot, lhs_dtype)).into());
        };
        if !rhs_dtype.is_primitive_numeric() {
            return Err(PyApolloErr::from(apollo_err!(opq = dot, rhs_dtype)).into());
        }

        let result: AnyValue = if lhs_dtype.is_float() || rhs_dtype.is_float() {
            py.enter_apollo(|| (s * o)?.sum::<f64>())?.into()
        } else {
            py.enter_apollo(|| (s * o)?.sum::<i64>())?.into()
        };
    ...
    fn __getstate__<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyBytes>> {
        ...
        Ok(PyBytes::new(
            py,
            &py.enter_apollo(|| self.series.read().serialize_to_bytes())?,
        ))
    }
    ...
    fn skew(&self, py: Python<'_>, bias: bool) -> PyResult<Option<f64>> {
        py.enter_apollo(|| self.series.read().skew(bias))
    }

    fn kurtosis(&self, py: Python<'_>, fisher: bool, bias: bool) -> PyResult<Option<f64>> {
        py.enter_apollo(|| self.series.read().kurtosis(fisher, bias))
    }
    ...
    fn cast(
        ...
        py.enter_apollo_series(|| self.series.read().cast_with_options(&dtype.0, options))
    }

    fn get_chunks(&self) -> PyResult<Vec<Py<PyAny>>> {
        Python::attach(|py| {
            let wrap_s = py_modules::apollo(py).getattr(py, "wrap_s").unwrap();
    ...
    fn is_sorted(&self, py: Python<'_>, descending: bool, nulls_last: bool) -> PyResult<bool> {
        ...
        py.enter_apollo(|| self.series.read().is_sorted(options))
    }
    ...
    fn head(&self, py: Python<'_>, n: usize) -> PyResult<Self> {
        py.enter_apollo_series(|| Ok(self.series.read().head(Some(n))))
    }

    fn tail(&self, py: Python<'_>, n: usize) -> PyResult<Self> {
        py.enter_apollo_series(|| Ok(self.series.read().tail(Some(n))))
    }
    ...
    pub fn not_(&self, py: Python) -> PyResult<Self> {
        py.enter_apollo_series(|| apollo_ops::series::negate_bitwise(&self.series.read()))
    }
    ...
    fn str_to_datetime_infer(
        ...
        Ok(py
            .enter_apollo(|| {
                ...
                apollo_time::prelude::string::infer::to_datetime_with_inferred_tz(
                    datetime_strings,
    ...
    fn ext_to(&self, dtype: Wrap<DataType>) -> PyResult<Self> {
        ...
        if storage.as_ref() != s.dtype() {
            return Err(PyErr::from(PyApolloErr::from(apollo_err!(SchemaMismatch:
