...
// Import arrow data directly without requiring pyarrow (used in pyo3-apollo)
#[pymethods]
impl PySeries {
    #[staticmethod]
    unsafe fn _import_arrow_from_c(
        ...
        let s = Series::try_new(name.into(), chunks).map_err(PyApolloErr::from)?;
        Ok(s.into())
    }
    ...
    unsafe fn _export_arrow_to_c(
        ...
        export_chunk(&self.series.read(), out_ptr, out_schema_ptr).map_err(PyApolloErr::from)?;
        Ok(())
    }
}
...
unsafe fn export_chunk(
    ...
    apollo_ensure!(s.chunks().len() == 1, InvalidOperation: "expect a single chunk");
