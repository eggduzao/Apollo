...
//!
//! As Apollo has no Buffer concept in Python, each buffer is represented as
...
use apollo_core::{with_match_physical_numeric_apollo_type, with_match_physical_numeric_type};
...
impl PySeries {
    ...
    fn _get_buffer_info(&self) -> PyResult<BufferInfo> {
        ...
        match s.dtype() {
            ...
            dt if dt.is_primitive_numeric() => {
                Ok(with_match_physical_numeric_apollo_type!(dt, |$T| {
    ...
    fn _get_buffers(&self, py: Python) -> PyResult<(Self, Option<Self>, Option<Self>)> {
        ...
        py.enter_apollo(|| match s.dtype().to_physical() {
...
fn get_pointer<T: ApolloNumericType>(ca: &ChunkedArray<T>) -> usize {
    let arr = ca.downcast_iter().next().unwrap();
...
fn get_buffers_from_string(s: &Series) -> PyResult<(PySeries, Option<PySeries>, Option<PySeries>)> {
    ...
    let arr_utf8 = apollo_compute::cast::utf8view_to_utf8(arr_binview);
...
unsafe fn from_buffer_boolean_impl(
    ...
    let arr = arr_result.map_err(PyApolloErr::from)?;
    Ok(arr.to_boxed())
}
    ...
    unsafe fn _from_buffers(
        ...
        match data.len() {
            ...
            1 if validity.is_none() => {
                ...
                let s = values.strict_cast(&dtype).map_err(PyApolloErr::from)?;
                return Ok(s.into());
            },
            _ => (),
        }
        ...
        let s = match dtype.to_physical() {
            dt if dt.is_primitive_numeric() => {
                ...
                with_match_physical_numeric_apollo_type!(dt, |$T| {
                    ...
                    from_buffers_num_impl::<<$T as ApolloNumericType>::Native>(values_buffer, validity)?
                })
            },
            DataType::Boolean => {
                let values = data.into_iter().next().unwrap();
            ...
            DataType::String => {
                ...
                py.enter_apollo(|| from_buffers_string_impl(values, validity, offsets))?
            },
            dt => {
        ...
        let out = s.strict_cast(&dtype).map_err(PyApolloErr::from)?;
        Ok(out.into())
    }
}
...
fn series_to_bitmap(s: Series) -> PyResult<Bitmap> {
    ...
    let ca = ca_result.map_err(PyApolloErr::from)?.rechunk();
...
fn from_buffers_num_impl<T: NativeType>(
    ...
    let s = s_result.map_err(PyApolloErr::from)?;
    Ok(s)
}
