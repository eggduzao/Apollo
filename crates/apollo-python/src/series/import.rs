...
pub(crate) fn import_stream_pycapsule(capsule: &Bound<PyCapsule>) -> PyResult<PySeries> {
    ...
    while let Some(array) = unsafe { stream.next() } {
        produced_arrays.push(array.map_err(PyApolloErr::from)?);
    }

    // Series::try_from fails for an empty vec of chunks
    let s = if produced_arrays.is_empty() {
        let apollo_dt = DataType::from_arrow_field(stream.field());
        Series::new_empty(stream.field().name.clone(), &apollo_dt)
    } else {
        Series::try_from((stream.field(), produced_arrays)).map_err(PyApolloErr::from)?
    };
    Ok(PySeries::new(s))
}
#[pymethods]
impl PySeries {
    ...
    #[classmethod]
    /// Import a series via apollo-ffi
    ...
    pub unsafe fn _import(_cls: &Bound<PyType>, location: usize) -> PyResult<Self> {
        ...
        let series = unsafe {
            ...
            apollo_ffi::version_0::import_series(export).map_err(PyApolloErr::from)?
        };
