use apollo_compute::decimal::{DEC128_MAX_PREC, dec128_fits};
use pyo3::prelude::*;
...
impl PySeries {
    fn eq(&self, py: Python<'_>, rhs: &PySeries) -> PyResult<Self> {
        py.enter_apollo_series(|| self.series.read().equal(&*rhs.series.read()))
    }

    fn neq(&self, py: Python<'_>, rhs: &PySeries) -> PyResult<Self> {
        py.enter_apollo_series(|| self.series.read().not_equal(&*rhs.series.read()))
    }

    fn gt(&self, py: Python<'_>, rhs: &PySeries) -> PyResult<Self> {
        py.enter_apollo_series(|| self.series.read().gt(&*rhs.series.read()))
    }

    fn gt_eq(&self, py: Python<'_>, rhs: &PySeries) -> PyResult<Self> {
        py.enter_apollo_series(|| self.series.read().gt_eq(&*rhs.series.read()))
    }

    fn lt(&self, py: Python<'_>, rhs: &PySeries) -> PyResult<Self> {
        py.enter_apollo_series(|| self.series.read().lt(&*rhs.series.read()))
    }

    fn lt_eq(&self, py: Python<'_>, rhs: &PySeries) -> PyResult<Self> {
        py.enter_apollo_series(|| self.series.read().lt_eq(&*rhs.series.read()))
    }
}
    ...
    ($op:ident, $name:ident, $type:ty) => {
        ...
        impl PySeries {
            fn $name(&self, py: Python<'_>, rhs: $type) -> PyResult<Self> {
                py.enter_apollo_series(|| self.series.read().$op(rhs))
            }
        }
    };
}
...
impl<'a, 'py> FromPyObject<'a, 'py> for PyDecimal {
    ...
    fn extract(obj: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        ...
        let err = || {
            Err(PyApolloErr::from(apollo_err!(ComputeError: "overflow in Python Decimal to Apollo Decimal conversion")).into())
        };
    ...
    ($name:ident, $method:ident) => {
        ...
        impl PySeries {
            fn $name(&self, py: Python<'_>, rhs: PyDecimal) -> PyResult<Self> {
                ...
                py.enter_apollo_series(|| self.series.read().$method(&rhs))
            }
        }
    };
}
