...
impl PySeries {
    ...
    fn new_bool(
        ...
        py.enter_apollo_series(|| Series::new(name.into(), vals).cast(&DataType::Boolean))
    }
    ...
    fn new_f16(
        ...
        if nan_is_null {
            py.enter_apollo_series(|| {
                let validity = apollo_compute::nan::is_not_nan(arr.values());
...
impl PySeries {
    ...
    fn from_arrow(name: &str, array: &Bound<PyAny>) -> PyResult<Self> {
        ...
        match arr.dtype() {
            ...
            _ => {
                let series: Series =
                    Series::try_new(name.into(), arr).map_err(PyApolloErr::from)?;
                Ok(series.into())
            },
        }
    }
