...
impl PySeries {
    fn any(&self, py: Python<'_>, ignore_nulls: bool) -> PyResult<Option<bool>> {
        py.enter_apollo(|| {
            ...
            let s = s.bool()?;
            ApolloResult::Ok(if ignore_nulls {
                Some(s.any())
            } else {
                s.any_kleene()
            })
    ...
    fn arg_max(&self, py: Python) -> PyResult<Option<usize>> {
        py.enter_apollo_ok(|| self.series.read().arg_max())
    }

    fn arg_min(&self, py: Python) -> PyResult<Option<usize>> {
        py.enter_apollo_ok(|| self.series.read().arg_min())
    }

    fn min<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().min_reduce()), py)
    }

    fn max<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().max_reduce()), py)
    }
    ...
    fn mean<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        ...
        match s.dtype() {
            Boolean => scalar_to_py(
                py.enter_apollo(|| s.cast(&DataType::UInt8).unwrap().mean_reduce()),
                py,
            ),
            // For non-numeric output types we require mean_reduce.
            dt if dt.is_temporal() => scalar_to_py(py.enter_apollo(|| s.mean_reduce()), py),
    ...
    fn median<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        ...
        match s.dtype() {
            Boolean => scalar_to_py(
                py.enter_apollo(|| s.cast(&DataType::UInt8).unwrap().median_reduce()),
                py,
            ),
            // For non-numeric output types we require median_reduce.
            dt if dt.is_temporal() => scalar_to_py(py.enter_apollo(|| s.median_reduce()), py),
            _ => Ok(s.median().into_pyobject(py)?),
        }
    }

    fn product<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().product()), py)
    }
    ...
    fn quantile<'py>(
        ...
        if let Ok(q_float) = quantile.extract::<f64>() {
            ...
            scalar_to_py(
                py.enter_apollo(|| self.series.read().quantile_reduce(q_float, interpolation.0)),
                py,
            )
    ...
    fn std<'py>(&self, py: Python<'py>, ddof: u8) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().std_reduce(ddof)), py)
    }

    fn var<'py>(&self, py: Python<'py>, ddof: u8) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().var_reduce(ddof)), py)
    }

    fn sum<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().sum_reduce()), py)
    }

    fn first<'py>(&self, py: Python<'py>, ignore_nulls: bool) -> PyResult<Bound<'py, PyAny>> {
        let result = if ignore_nulls {
            py.enter_apollo_ok(|| self.series.read().first_non_null())
        } else {
            py.enter_apollo_ok(|| self.series.read().first())
        };
    ...
    fn last<'py>(&self, py: Python<'py>, ignore_nulls: bool) -> PyResult<Bound<'py, PyAny>> {
        let result = if ignore_nulls {
            py.enter_apollo_ok(|| self.series.read().last_non_null())
        } else {
            py.enter_apollo_ok(|| self.series.read().last())
        };
        scalar_to_py(result, py)
    }
    ...
    fn approx_n_unique(&self, py: Python) -> PyResult<IdxSize> {
        py.enter_apollo(|| self.series.read().approx_n_unique())
    }

    #[cfg(feature = "bitwise")]
    fn bitwise_and<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().and_reduce()), py)
    }

    #[cfg(feature = "bitwise")]
    fn bitwise_or<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().or_reduce()), py)
    }

    #[cfg(feature = "bitwise")]
    fn bitwise_xor<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        scalar_to_py(py.enter_apollo(|| self.series.read().xor_reduce()), py)
    }
}
