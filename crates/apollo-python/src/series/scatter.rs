...
fn scatter(s: Series, idx: &Series, values: &Series) -> Result<Series, (Series, ApolloError)> {
    ...
    let values = if logical_dtype.is_categorical() || logical_dtype.is_enum() {
        if matches!(
            ...
            return Err((
                s,
                apollo_err!(InvalidOperation: "invalid values dtype '{}' for scattering into dtype '{}'", values.dtype(), logical_dtype),
            ));
        }
    ...
    let idx = match apollo_ops::prelude::convert_and_bound_index(idx, s.len(), null_on_oob) {
        Ok(idx) => idx,
        Err(err) => return Err((s, err)),
    };
    let idx = idx.rechunk();
    ...
    if idx.has_nulls() {
        return Err((
            s,
            ApolloError::ComputeError("index values should not be null".into()),
        ));
    }
...
fn scatter_impl(
    ...
    match mutable_s.dtype() {
        ...
        _ => Err(ApolloError::ComputeError(
