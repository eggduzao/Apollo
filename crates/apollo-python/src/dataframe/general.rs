...
use crate::utils::{EnterApolloExt, to_py_err};
...
impl PyDataFrame {
    ...
    pub fn __init__(columns: Vec<PySeries>) -> PyResult<Self> {
        ...
        let df = DataFrame::new_infer_height(columns).map_err(PyApolloErr::from)?;
    ...
    pub fn empty_with_height(height: u64) -> PyResult<Self> {
        Ok(PyDataFrame::new(DataFrame::empty_with_height(
            IdxSize::try_from(height)
                .map_err(|_| apollo_err!(bigidx, ctx = "DataFrame(height = _)", size = height))
    ...
    pub fn add(&self, py: Python<'_>, s: &PySeries) -> PyResult<Self> {
        py.enter_apollo_df(|| &*self.df.read() + &*s.series.read())
    }
    ...
    pub fn add_df(&self, py: Python<'_>, s: &Self) -> PyResult<Self> {
        py.enter_apollo_df(|| &*self.df.read() + &*s.df.read())
    }
    ...
    pub fn hstack(&self, py: Python<'_>, columns: Vec<PySeries>) -> PyResult<Self> {
        ...
        py.enter_apollo_df(|| self.df.read().hstack(&columns))
    }
    ...
    pub fn hstack_mut(&self, py: Python<'_>, columns: Vec<PySeries>) -> PyResult<()> {
        ...
        // @scalar-opt
        let columns = columns.into_iter().map(Into::into).collect::<Vec<_>>();
        py.enter_apollo(|| self.df.write().hstack_mut(&columns).map(drop))?;
        Ok(())
    }

    pub fn vstack(&self, py: Python<'_>, other: &PyDataFrame) -> PyResult<Self> {
        py.enter_apollo_df(|| self.df.read().vstack(&other.df.read()))
    }
    ...
    pub fn to_series(&self, index: isize) -> PyResult<PySeries> {
        ...
        match s {
            ...
            None => Err(PyIndexError::new_err(
                apollo_err!(oob = index, df.width()).to_string(),
            )),
        }
    }
    ...
    pub fn select(&self, py: Python<'_>, columns: Vec<PyBackedStr>) -> PyResult<Self> {
        py.enter_apollo_df(|| self.df.read().select(columns.iter().map(|x| &**x)))
    }
    ...
    pub fn head(&self, py: Python<'_>, n: usize) -> PyResult<Self> {
        py.enter_apollo_df(|| Ok(self.df.read().head(Some(n))))
    }

    pub fn tail(&self, py: Python<'_>, n: usize) -> PyResult<Self> {
        py.enter_apollo_df(|| Ok(self.df.read().tail(Some(n))))
    }

    pub fn is_unique(&self, py: Python) -> PyResult<PySeries> {
        py.enter_apollo_series(|| self.df.read().is_unique())
    }

    pub fn is_duplicated(&self, py: Python) -> PyResult<PySeries> {
        py.enter_apollo_series(|| self.df.read().is_duplicated())
    }

    pub fn equals(&self, py: Python<'_>, other: &PyDataFrame, null_equal: bool) -> PyResult<bool> {
        if null_equal {
            py.enter_apollo_ok(|| self.df.read().equals_missing(&other.df.read()))
        } else {
            py.enter_apollo_ok(|| self.df.read().equals(&other.df.read()))
        }
    }
    ...
    pub fn with_row_index(
        ...
        py.enter_apollo_df(|| self.df.read().with_row_index(name.into(), offset))
    }
            ...
            let function = move |df: DataFrame| {
                Python::attach(|py| {
                    ...
                    let python_df_wrapper =
                        pyapollo.getattr("wrap_df").unwrap().call1((pydf,)).unwrap();
    ...
    pub fn unpivot(
        ...
        use apollo_ops::unpivot::UnpivotDF;
        ...
        py.enter_apollo_df(|| self.df.read().unpivot2(args))
    }
    ...
    pub fn to_dummies(
        ...
        py.enter_apollo_df(|| match columns {
    ...
    pub fn null_count(&self, py: Python) -> PyResult<Self> {
        py.enter_apollo_df(|| Ok(self.df.read().null_count()))
    }

    pub fn shrink_to_fit(&self, py: Python) -> PyResult<()> {
        py.enter_apollo_ok(|| self.df.write().shrink_to_fit())
    }
    ...
    pub fn hash_rows(
        ...
        py.enter_apollo_series(|| self.df.write().hash_rows(Some(hb)))
    }
    ...
    pub fn transpose(
        ...
        py.enter_apollo_df(|| self.df.write().transpose(keep_names_as, new_col_names))
    }
    ...
    pub fn upsample(
        ...
        let every = Duration::try_parse(every).map_err(PyApolloErr::from)?;
        py.enter_apollo_df(|| {
            if stable {
    ...
    pub fn clear(&self, py: Python) -> PyResult<Self> {
        py.enter_apollo_df(|| Ok(self.df.read().clear()))
    }

    /// Export the columns via apollo-ffi
    /// # Safety
    ...
    pub unsafe fn _export_columns(&self, location: usize) {
        use apollo_ffi::version_0::export_column;
    ...
    pub unsafe fn _import_columns(
        ...
        use apollo_ffi::version_0::import_df;

        let location = location as *mut SeriesExport;

        let df = unsafe { import_df(location, width) }.map_err(PyApolloErr::from)?;
