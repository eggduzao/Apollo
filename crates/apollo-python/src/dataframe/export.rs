...
impl PyDataFrame {
    ...
    pub fn row_tuple<'py>(&self, idx: i64, py: Python<'py>) -> PyResult<Bound<'py, PyTuple>> {
        ...
        if idx >= df.height() {
            return Err(PyApolloErr::from(apollo_err!(oob = idx, df.height())).into());
        }
        PyTuple::new(
            py,
    ...
    pub fn row_tuples<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyList>> {
        ...
        let df = if df.max_n_chunks() > 16 {
            rechunked = df.clone();
            py.enter_apollo_ok(|| rechunked.rechunk_mut_par())?;
            &rechunked
        } else {
            &df
        };
    ...
    pub fn to_arrow(
        ...
        py.enter_apollo_ok(|| dfr.align_chunks_par())?;
        ...
        // so we make sure to run chunks.next() within enter_apollo().
        while let Some(rb) = py.enter_apollo_ok(|| chunks.next())? {
    ...
    pub fn to_pandas(&self, py: Python) -> PyResult<Vec<Py<PyAny>>> {
        let mut df = self.df.write();
        let dfr = &mut *df; // Lock guard isn't Send, but mut ref is.
        py.enter_apollo_ok(|| dfr.rechunk_mut_par())?;
        let df = RwLockWriteGuard::downgrade(df);
        Python::attach(|py| {
            let pyarrow = py.import("pyarrow")?;
            let cat_columns = df
            ...
            let rbs = df
                ...
                .map(|rb| {
                    ...
                    for i in &cat_columns {
                        ...
                        let out = apollo_compute::cast::cast(
                            &**arr,
