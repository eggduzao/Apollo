use apollo::frame::row::{Row, rows_to_schema_first_non_null};
    ...
    pub fn map_rows(
        ...
        if let Ok(first_row) = first_val.cast::<PyTuple>() {
            ...
            let out = if null_count > 0 {
                ...
                tmp.append_owned(s).map_err(PyApolloErr::from)?;
                tmp
            } else {
                s
            };
...
fn collect_lambda_ret_with_rows_output<'py>(
    ...
    let mut row_iter = ret_iter.map(|retval| {
        ...
        if retval.is_none() {
            ...
            let tuple = retval.cast::<PyTuple>().map_err(|_| apollo_err!(ComputeError: format!("expected tuple, got {}", retval.get_type().qualname().unwrap())))?;
            row_buf.0.clear();
            for v in tuple {
            ...
            // SAFETY:
            // we know that row constructor of apollo dataframe does not keep a reference
