use apollo::frame::row::{Row, rows_to_schema_supertypes, rows_to_supertypes};
use apollo::prelude::*;
use pyo3::prelude::*;
...
impl PyDataFrame {
    ...
    pub fn from_rows(
        ...
        py.enter_apollo(move || finish_from_rows(data, schema, None, infer_schema_length))
    }
...
fn finish_from_rows(
    ...
    let schema = if let Some(mut schema) = schema {
        ...
        rows_to_schema_supertypes(&rows, infer_schema_length).map_err(PyApolloErr::from)?
    };

    let df = DataFrame::from_rows_and_schema(&rows, &schema).map_err(PyApolloErr::from)?;
    Ok(df.into())
}
...
fn update_schema_from_rows(
    ...
    let inferred_dtypes =
        rows_to_supertypes(rows, infer_schema_length).map_err(PyApolloErr::from)?;
    ...
    for (i, dtype) in schema.iter_values_mut().enumerate() {
        if !dtype.is_known() {
            *dtype = inferred_dtypes_slice.get(i).ok_or_else(|| {
                apollo_err!(SchemaMismatch: "the number of columns in the schema does not match the data")
            })
