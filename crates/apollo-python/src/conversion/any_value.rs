...
#[cfg(feature = "object")]
use apollo::datatypes::OwnedObject;
use apollo::datatypes::{DataType, Field, TimeUnit};
use apollo::prelude::{AnyValue, PlSmallStr, Series, TimeZone};
use apollo_compute::decimal::{DEC128_MAX_PREC, DecimalFmtBuffer, dec128_fits};
use apollo_core::utils::any_values_to_supertype_and_n_dtypes;
use apollo_core::utils::arrow::temporal_conversions::date32_to_date;
use apollo_utils::aliases::PlFixedStateQuality;
    ...
    fn get_int(ob: &Bound<'_, PyAny>, strict: bool) -> PyResult<AnyValue<'static>> {
        if let Ok(v) = ob.extract::<i64>() {
            ...
            Err(PyOverflowError::new_err(format!(
                "int value too large for Apollo integer types: {ob}"
            )))
        }
    }
