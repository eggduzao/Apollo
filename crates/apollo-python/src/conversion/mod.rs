...
#[cfg(feature = "object")]
use apollo::chunked_array::object::ApolloObjectSafe;
use apollo::frame::row::Row;
#[cfg(feature = "avro")]
use apollo::io::avro::AvroCompression;
use apollo::prelude::ColumnMapping;
use apollo::prelude::default_values::{
    DefaultFieldValues, IcebergIdentityTransformedPartitionFields,
};
use apollo::prelude::deletion::DeletionFilesList;
use apollo::series::ops::NullBehavior;
use apollo_buffer::Buffer;
use apollo_compute::decimal::dec128_verify_prec_scale;
use apollo_core::datatypes::extension::get_extension_type_or_generic;
use apollo_core::schema::iceberg::IcebergSchema;
use apollo_core::utils::arrow::array::Array;
use apollo_core::utils::materialize_dyn_int;
use apollo_lazy::prelude::*;
#[cfg(feature = "parquet")]
use apollo_parquet::write::StatisticsOptions;
use apollo_plan::dsl::ScanSources;
use apollo_utils::compression::{BrotliLevel, GzipLevel, ZstdLevel};
use apollo_utils::pl_str::PlSmallStr;
use apollo_utils::total_ord::{TotalEq, TotalHash};
...
use crate::prelude::*;
use crate::py_modules::{pl_series, apollo};
...
impl<'a, 'py> FromPyObject<'a, 'py> for Wrap<NullValues> {
    ...
    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        if let Ok(s) = ob.extract::<PyBackedStr>() {
            ...
            Err(
                PyApolloErr::Other("could not extract value from null_values argument".into())
                    .into(),
            )
        }
    }
    ...
    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        ...
        let dtype = match &*type_name {
                ...
                match &*name {
                    ...
                    "Decimal" => {
                        return Err(PyTypeError::new_err(
                            "Decimal without precision/scale set is not a valid Apollo datatype",
                        ));
                    },
                    dt => {
                        return Err(PyTypeError::new_err(format!(
                            "'{dt}' is not a Apollo data type",
                        )));
                    },
                }
            },
            ...
            "Enum" => {
                ...
                let ca = s.str().map_err(PyApolloErr::from)?;
...
#[cfg(feature = "object")]
impl ApolloObject for ObjectValue {
...
#[cfg(feature = "object")]
impl From<&dyn ApolloObjectSafe> for &ObjectValue {
    fn from(val: &dyn ApolloObjectSafe) -> Self {
        unsafe { &*(val as *const dyn ApolloObjectSafe as *const ObjectValue) }
    }
}
    ...
    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        if ob.is_none() {
            ...
            let out = DEFAULT.get_or_try_init(ob.py(), || {
                let ob = PyModule::import(ob.py(), "apollo.io.scan_options.cast_options")
                    .unwrap()
