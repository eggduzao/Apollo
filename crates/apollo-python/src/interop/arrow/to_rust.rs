...
use apollo_core::utils::arrow::ffi;
...
pub fn field_to_rust_arrow(obj: Bound<'_, PyAny>) -> PyResult<ArrowField> {
    ...
    let field = unsafe { ffi::import_field_from_c(schema.as_ref()).map_err(PyApolloErr::from)? };
    Ok(field)
}
...
pub fn array_to_rust(obj: &Bound<PyAny>) -> PyResult<ArrayRef> {
    ...
    unsafe {
        let field = ffi::import_field_from_c(schema.as_ref()).map_err(PyApolloErr::from)?;
        let array = ffi::import_array_from_c(*array, field.dtype).map_err(PyApolloErr::from)?;
        Ok(array)
    }
}
...
pub fn to_rust_df(
    ...
    let ArrowDataType::Struct(fields) = field_to_rust_arrow(schema)?.dtype else {
        return Err(PyApolloErr::Other("invalid top-level schema".into()).into());
    };
    ...
    if schema.len() != fields.len() {
        ...
        return Err(PyApolloErr::Apollo(ApolloError::Duplicate(
