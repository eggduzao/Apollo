...
use apollo_plan::plans::predicates::aexpr_to_skip_batch_predicate;
use apollo_plan::plans::{ExprToIRContext, RowEncodingVariant, node_to_expr, to_expr_ir};
...
impl PyExpr {
    ...
    fn rolling(
        ...
        let period = Duration::try_parse(period).map_err(PyApolloErr::from)?;
        let offset = Duration::try_parse(offset).map_err(PyApolloErr::from)?;
        let closed = closed.0;
    ...
    fn ewm_mean_by(&self, times: PyExpr, half_life: &str) -> PyResult<Self> {
        let half_life = Duration::try_parse(half_life).map_err(PyApolloErr::from)?;
        Ok(self
            .inner
            .clone()
    ...
    fn into_selector(&self) -> PyResult<PySelector> {
        Ok(self
            ...
            .into_selector()
            .ok_or_else(
                || apollo_err!(InvalidOperation: "expr `{}` is not a selector", &self.inner),
            )
            .map_err(PyApolloErr::from)?
            .into())
    }
