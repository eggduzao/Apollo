...
use apollo::prelude::{CloudScheme, LazyFrame, PlHashMap, PlSmallStr, Schema};
use apollo_io::catalog::unity::client::{CatalogClient, CatalogClientBuilder};
use apollo_io::catalog::unity::models::{
    CatalogInfo, ColumnInfo, DataSourceFormat, NamespaceInfo, TableInfo, TableType,
};
use apollo_io::catalog::unity::schema::parse_type_json_str;
...
impl PyCatalogClient {
    ...
    pub fn scan_table(
        ...
        let table_info = py.enter_apollo(|| {
            pl_async::get_runtime().block_in_place_on(self.client().get_table_info(
                catalog_name,
                namespace,
                table_name,
    ...
    #[staticmethod]
    pub fn type_json_to_apollo_type(py: Python<'_>, type_json: &str) -> PyResult<Py<PyAny>> {
