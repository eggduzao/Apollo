...
use apollo::frame::DataFrame;
use apollo::series::IntoSeries;
use apollo_error::ApolloResult;
use apollo_error::signals::{KeyboardInterrupt, catch_keyboard_interrupt};
...
use crate::timeout::{cancel_apollo_timeout, schedule_apollo_timeout};

/// Calls method on downcasted ChunkedArray for all possible publicly exposed Apollo dtypes.
#[macro_export]
macro_rules! apply_all_apollo_dtypes {
...
/// Boilerplate for `|e| PyApolloErr::from(e).into()`
#[allow(unused)]
pub(crate) fn to_py_err<E: Into<PyApolloErr>>(e: E) -> PyErr {
    e.into().into()
}

pub trait EnterApolloExt {
    /// Whenever you have a block of code in the public Python API that
    /// (potentially) takes a long time, wrap it in enter_apollo. This will
    ...
    fn enter_apollo<T, E, F>(self, f: F) -> PyResult<T>
    where
        F: Ungil + Send + FnOnce() -> Result<T, E>,
        T: Ungil + Send,
        E: Ungil + Send + Into<PyApolloErr>;

    /// Same as enter_apollo, but wraps the result in PyResult::Ok, useful
    /// shorthand for infallible functions.
    #[inline(always)]
    fn enter_apollo_ok<T, F>(self, f: F) -> PyResult<T>
    where
        Self: Sized,
        F: Ungil + Send + FnOnce() -> T,
        T: Ungil + Send,
    {
        self.enter_apollo(move || PyResult::Ok(f()))
    }

    /// Same as enter_apollo, but expects a ApolloResult<DataFrame> as return
    /// which is converted to a PyDataFrame.
    #[inline(always)]
    fn enter_apollo_df<F>(self, f: F) -> PyResult<PyDataFrame>
    where
        Self: Sized,
        F: Ungil + Send + FnOnce() -> ApolloResult<DataFrame>,
    {
        self.enter_apollo(f).map(PyDataFrame::new)
    }

    /// Same as enter_apollo, but expects a ApolloResult<S> as return which
    /// is converted to a PySeries through S: IntoSeries.
    #[inline(always)]
    fn enter_apollo_series<T, F>(self, f: F) -> PyResult<PySeries>
    where
        Self: Sized,
        T: Ungil + Send + IntoSeries,
        F: Ungil + Send + FnOnce() -> ApolloResult<T>,
    {
        self.enter_apollo(f).map(|s| PySeries::new(s.into_series()))
    }
}

impl EnterApolloExt for Python<'_> {
    fn enter_apollo<T, E, F>(self, f: F) -> PyResult<T>
    where
        F: Ungil + Send + FnOnce() -> Result<T, E>,
        T: Ungil + Send,
        E: Ungil + Send + Into<PyApolloErr>,
    {
        let timeout = schedule_apollo_timeout();
        let ret = self.detach(|| catch_keyboard_interrupt(AssertUnwindSafe(f)));
        cancel_apollo_timeout(timeout);
        match ret {
            Ok(Ok(ret)) => Ok(ret),
