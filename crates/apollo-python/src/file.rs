...
use apollo::io::mmap::MmapBytesReader;
use apollo::prelude::PlRefPath;
use apollo::prelude::file::{Writeable, WriteableTrait};
        ...
        Python::attach(|py| {
            ...
            if let Ok(bytes) = opt_bytes {
                ...
                Err(io::Error::new(
                    ...
                    apollo_err!(InvalidOperation: "could not read from input"),
                ))
            }
        })
    }
        ...
        .unwrap_or_default()
    {
        if !is_utf8_encoding(&py_f)? {
            return Err(PyApolloErr::from(
                apollo_err!(InvalidOperation: "file encoding is not UTF-8"),
            )
            .into());
        }
...
fn get_either_buffer_or_path(
    ...
    Python::attach(|py| {
        ...
        if let Ok(s) = py_f.extract::<Cow<str>>() {
            ...
            let f = if write {
                create_file(&file_path).map_err(PyApolloErr::from)?
            } else {
                apollo_utils::open_file(&file_path).map_err(PyApolloErr::from)?
            };
            Ok((
