//! This module supports mapping SQL datatypes to Apollo datatypes.
//!
//! It also provides utility functions for working with SQL datatypes.
use apollo_core::datatypes::{DataType, TimeUnit};
use apollo_error::{ApolloResult, apollo_bail};
use apollo_plan::dsl::Expr;
use apollo_plan::dsl::functions::lit;
use sqlparser::ast::{
    ArrayElemTypeDef, DataType as SQLDataType, ExactNumberInfo, Ident, ObjectName, ObjectNamePart,
    TimezoneInfo,
};

apollo_utils::regex_cache::cached_regex! {
...
pub fn bitstring_to_bytes_literal(b: &String) -> ApolloResult<Expr> {
    let n_bits = b.len();
...
fn timeunit_from_precision(prec: &Option<u64>) -> ApolloResult<TimeUnit> {
    Ok(match prec {
        ...
        Some(n) => {
            apollo_bail!(SQLSyntax: "invalid temporal type precision (expected 1-9, found {})", n)
        },
    })
}

pub(crate) fn map_sql_dtype_to_apollo(dtype: &SQLDataType) -> ApolloResult<DataType> {
    Ok(match dtype {
        // ---------------------------------
        // array/list
        // ---------------------------------
        ...
        | SQLDataType::Array(ArrayElemTypeDef::SquareBracket(inner_type, _)) => {
            DataType::List(Box::new(map_sql_dtype_to_apollo(inner_type)?))
        },
        ...
        SQLDataType::Float(n_bytes) => match n_bytes {
            ...
            ExactNumberInfo::Precision(n) => {
                apollo_bail!(SQLSyntax: "unsupported `float` size (expected a value between 1 and 53, found {})", n)
            },
            ExactNumberInfo::None => DataType::Float64,
            ExactNumberInfo::PrecisionAndScale(_, _) => {
                apollo_bail!(SQLSyntax: "FLOAT does not support scale parameter")
            },
        },
        ...
        SQLDataType::Interval { fields, precision } => {
            if !fields.is_none() {
                // eg: "YEARS TO MONTH"
                apollo_bail!(SQLInterface: "`interval` definition with fields={:?} is not supported", fields)
            }
            let time_unit = match precision {
                ...
                Some(p) => {
                    apollo_bail!(SQLSyntax: "invalid `interval` precision (expected 1-9, found {})", p)
                },
                None => TimeUnit::Microseconds,
            };
        ...
        SQLDataType::Time(_, tz) => match tz {
            ...
            _ => {
                apollo_bail!(SQLInterface: "`time` with timezone is not supported; found tz={}", tz)
            },
        },
        ...
        SQLDataType::Timestamp(prec, tz) => match tz {
            ...
            _ => {
                apollo_bail!(SQLInterface: "`timestamp` with timezone is not (yet) supported")
            },
        },
        ...
        SQLDataType::Custom(ObjectName(idents), _) => match idents.as_slice() {
            [ObjectNamePart::Identifier(Ident { value, .. })] => {
                match value.to_lowercase().as_str() {
                    ...
                    _ => {
                        apollo_bail!(SQLInterface: "datatype {:?} is not currently supported", value)
                    },
                }
            },
            _ => {
                apollo_bail!(SQLInterface: "datatype {:?} is not currently supported", idents)
            },
        },
        _ => {
            apollo_bail!(SQLInterface: "datatype {:?} is not currently supported", dtype)
        },
    })
}
