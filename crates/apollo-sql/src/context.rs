...
use apollo_core::frame::row::Row;
use apollo_core::prelude::*;
use apollo_lazy::prelude::*;
use apollo_ops::frame::JoinCoalesce;
use apollo_plan::dsl::function_expr::StructFunction;
use apollo_plan::prelude::*;
use apollo_utils::aliases::{PlHashSet, PlIndexSet};
use apollo_utils::format_pl_smallstr;
use sqlparser::ast::{
...
use crate::table_functions::ApolloTableFunctions;
use crate::types::map_sql_dtype_to_apollo;
...
impl SQLContext {
    ...
    /// # }
    ///```
    pub fn execute(&mut self, query: &str) -> ApolloResult<LazyFrame> {
        ...
        apollo_ensure!(ast.len() == 1, SQLInterface: "one (and only one) statement can be parsed at a time");
...
impl SQLContext {
    pub(crate) fn execute_statement(&mut self, stmt: &Statement) -> ApolloResult<LazyFrame> {
        let ast = stmt;
        Ok(match ast {
    ...
    pub(crate) fn execute_query(&mut self, query: &Query) -> ApolloResult<LazyFrame> {
        self.register_ctes(query)?;
        self.execute_query_no_ctes(query)
    }

    pub(crate) fn execute_query_no_ctes(&mut self, query: &Query) -> ApolloResult<LazyFrame> {
        ...
        let (limit, offset) = match &query.limit_clause {
            Some(LimitClause::LimitOffset {
                ...
                if !limit_by.is_empty() {
                    ...
                    apollo_bail!(SQLSyntax: "LIMIT BY clause is not supported");
                }
    ...
    pub(crate) fn get_frame_schema(&mut self, frame: &mut LazyFrame) -> ApolloResult<SchemaRef> {
    ...
    pub(crate) fn execute_isolated<F>(&mut self, query: F) -> ApolloResult<(LazyFrame, SchemaRef)>
    where
        F: FnOnce(&mut Self) -> ApolloResult<LazyFrame>,
    {
    ...
    fn expr_or_ordinal(
        ...
        match e {
            ...
            SQLExpr::Value(v) => Err(apollo_err!(
                SQLSyntax:
                ...
                if matches!(e, SQLExpr::CompoundIdentifier(_)) {
                    if let Some(schema) = schema {
                        expr = expr.map_expr(|ex| match &ex {
                            Expr::Column(name) => {
                                let prefixed = format!("__APOLLO_ORIG_{}", name.as_str());
    ...
    fn process_query(&mut self, expr: &SetExpr, query: &Query) -> ApolloResult<LazyFrame> {
        match expr {
            ...
            SetExpr::Table(tbl) => {
                if let Some(table_name) = tbl.table_name.as_ref() {
                    ...
                    apollo_bail!(SQLInterface: "'TABLE' requires valid table name")
                }
            },
            op => {
                apollo_bail!(SQLInterface: "'{}' operation is currently unsupported", op)
            },
        }
    }
        ...
        let rf_cols = match quantifier {
            ...
            SetQuantifier::Distinct | SetQuantifier::None => {
                ...
                if lf_cols.len() != rf_cols.len() {
                    apollo_bail!(SQLInterface: "{} requires equal number of columns in each table (use '{} BY NAME' to combine mismatched tables)", op_name, op_name)
                }
                Some(rf_cols)
            },
            _ => {
                apollo_bail!(SQLInterface: "'{} {}' is not supported", op_name, quantifier.to_string())
            },
        };
                ...
                let out = match quantifier {
                    ...
                    SetQuantifier::All | SetQuantifier::Distinct | SetQuantifier::None => {
                        ...
                        if lf_schema.len() != rf_schema.len() {
                            apollo_bail!(SQLInterface: "UNION requires equal number of columns in each table (use 'UNION BY NAME' to combine mismatched tables)")
                        }
                    ...
                    _ => {
                        apollo_bail!(SQLInterface: "'UNION {}' is not currently supported", quantifier)
                    },
                };
    ...
    fn process_unnest_lateral(
        ...
        let alias = alias
            .as_ref()
            .ok_or_else(|| apollo_err!(SQLSyntax: "UNNEST table must have an alias"))?;
        apollo_ensure!(!with_offset, SQLInterface: "UNNEST tables do not (yet) support WITH ORDINALITY|OFFSET");
    ...
    fn process_values(&mut self, values: &[Vec<SQLExpr>]) -> ApolloResult<LazyFrame> {
        let frame_rows: Vec<Row> = values.iter().map(|row| {
            let row_data: Result<Vec<_>, _> = row.iter().map(|expr| {
                ...
                match expr {
                    Expr::Literal(value) => {
                        value.to_any_value()
                            .ok_or_else(|| apollo_err!(SQLInterface: "invalid literal value: {:?}", value))
                            .map(|av| av.into_static())
                    },
                    _ => apollo_bail!(SQLInterface: "VALUES clause expects literals; found {}", expr),
                }
            }).collect();
    ...
    fn execute_explain(&mut self, stmt: &Statement) -> ApolloResult<LazyFrame> {
        match stmt {
            ...
            _ => apollo_bail!(SQLInterface: "unexpected statement type; expected EXPLAIN"),
        }
    }

    // SHOW TABLES
    fn execute_show_tables(&mut self, _: &Statement) -> ApolloResult<LazyFrame> {
    ...
    // DROP TABLE <tbl>
    fn execute_drop_table(&mut self, stmt: &Statement) -> ApolloResult<LazyFrame> {
        match stmt {
            ...
            _ => apollo_bail!(SQLInterface: "unexpected statement type; expected DROP"),
        }
    }

    // DELETE FROM <tbl> [WHERE ...]
    fn execute_delete_from_table(&mut self, stmt: &Statement) -> ApolloResult<LazyFrame> {
        ...
        }) = stmt
        {
                ...
                || !order_by.is_empty()
            {
                ...
                apollo_bail!(SQLInterface: error_message);
            }
            ...
            if from_tables.len() > 1 {
                apollo_bail!(SQLInterface: "cannot have multiple tables in DELETE FROM (found {})", from_tables.len())
            }
            let tbl_expr = from_tables.first().unwrap();
            if !tbl_expr.joins.is_empty() {
                apollo_bail!(SQLInterface: "DELETE does not support table JOINs")
            }
            ...
            apollo_bail!(SQLInterface: "unexpected statement type; expected DELETE")
        }
    }

    // TRUNCATE <tbl>
    fn execute_truncate_table(&mut self, stmt: &Statement) -> ApolloResult<LazyFrame> {
            ...
            match partitions {
                None => {
                    if table_names.len() != 1 {
                        apollo_bail!(SQLInterface: "TRUNCATE expects exactly one table name; found {}", table_names.len())
                    }
                    ...
                    if let Some(lf) = self.table_map.get_mut(&tbl) {
                        *lf = DataFrame::empty_with_schema(
                        ...
                        apollo_bail!(SQLInterface: "table '{}' does not exist", tbl);
                    }
                },
                _ => {
                    apollo_bail!(SQLInterface: "TRUNCATE does not support use of 'partitions'")
                },
            }
        } else {
            apollo_bail!(SQLInterface: "unexpected statement type; expected TRUNCATE")
        }
    }
    ...
    fn register_ctes(&mut self, query: &Query) -> ApolloResult<()> {
        if let Some(with) = &query.with {
            if with.recursive {
                apollo_bail!(SQLInterface: "recursive CTEs are not supported")
            }
    ...
    fn execute_from_statement(&mut self, tbl_expr: &TableWithJoins) -> ApolloResult<LazyFrame> {
        ...
        if !tbl_expr.joins.is_empty() {
            for join in &tbl_expr.joins {
                ...
                lf = match &join.join_operator {
                    ...
                    join_type => {
                        apollo_bail!(SQLInterface: "join type '{:?}' not currently supported", join_type)
                    },
                };
    ...
    fn validate_select(&self, select_stmt: &Select) -> ApolloResult<()> {
        ...
        apollo_ensure!(cluster_by.is_empty(), SQLInterface: "`CLUSTER BY` clause is not supported");
        apollo_ensure!(connect_by.is_none(), SQLInterface: "`CONNECT BY` clause is not supported");
        apollo_ensure!(distribute_by.is_empty(), SQLInterface: "`DISTRIBUTE BY` clause is not supported");
        apollo_ensure!(exclude.is_none(), SQLInterface: "`EXCLUDE` clause is not supported");
        apollo_ensure!(into.is_none(), SQLInterface: "`SELECT INTO` clause is not supported");
        apollo_ensure!(lateral_views.is_empty(), SQLInterface: "`LATERAL VIEW` clause is not supported");
        apollo_ensure!(prewhere.is_none(), SQLInterface: "`PREWHERE` clause is not supported");
        apollo_ensure!(sort_by.is_empty(), SQLInterface: "`SORT BY` clause is not supported; use `ORDER BY` instead");
        apollo_ensure!(top.is_none(), SQLInterface: "`TOP` clause is not supported; use `LIMIT` instead");
        apollo_ensure!(value_table_mode.is_none(), SQLInterface: "`SELECT AS VALUE/STRUCT` is not supported");

        Ok(())
    }

    /// Execute the 'SELECT' part of the query.
    fn execute_select(&mut self, select_stmt: &Select, query: &Query) -> ApolloResult<LazyFrame> {
        ...
        let (mut lf, base_table_name) = if select_stmt.from.is_empty() {
            ...
            if from.len() > 1 {
                apollo_bail!(SQLInterface: "multiple tables in FROM clause are not currently supported (found {}); use explicit JOIN syntax instead", from.len())
            }
                    ...
                    match input.as_ref() {
                        ...
                        other_expr => {
                            ...
                            if !has_expr(other_expr, |e| matches!(e, Expr::Agg(_) | Expr::Len)) {
                                let temp_name =
                                    format_pl_smallstr!("__APOLLO_UNNEST_{}", explode_exprs.len());
        ...
        match &select_stmt.group_by {
            ...
            GroupByExpr::Expressions(group_by_exprs, modifiers) => {
                if !modifiers.is_empty() {
                    apollo_bail!(SQLInterface: "GROUP BY does not support CUBE, ROLLUP, or TOTALS modifiers")
                }
        ...
        lf = if group_by_keys.is_empty() {
            ...
            if select_stmt.having.is_some() {
                apollo_bail!(SQLSyntax: "HAVING clause not valid outside of GROUP BY; found:\n{:?}", select_stmt.having);
            };
                    ...
                    lf = lf
                        ...
                        .join(
                            ...
                            JoinArgs {
                                ...
                                maintain_order: apollo_ops::frame::MaintainOrderJoin::Left,
                                build_side: None,
                            },
                        );
                }
        ...
        lf = match &select_stmt.distinct {
            ...
            Some(Distinct::On(exprs)) => {
                // TODO: support exprs in `unique` see https://github.com/apollo/apollo/issues/5760
                ...
                let cols = exprs
                    ...
                    .map(|e| {
                        ...
                        if let Expr::Column(name) = expr {
                            ...
                            Err(apollo_err!(SQLSyntax:"DISTINCT ON only supports column names"))
                        }
                    })
                    .collect::<ApolloResult<Vec<_>>>()?;
            ...
            match select_item {
                ...
                SelectItem::QualifiedWildcard(kind, wildcard_options) => match kind {
                    ...
                    SelectItemQualifiedWildcardKind::Expr(_) => {
                        apollo_bail!(SQLSyntax: "qualified wildcard on expressions not yet supported: {:?}", select_item)
                    },
                },
    ...
    fn execute_create_table(&mut self, stmt: &Statement) -> ApolloResult<LazyFrame> {
        ...
        }) = stmt
        {
            ...
            if *if_not_exists && self.table_map.contains_key(tbl_name) {
                apollo_bail!(SQLInterface: "relation '{}' already exists", tbl_name);
            }
            let lf = match (query, columns.is_empty(), like) {
                ...
                (None, false, None) => {
                    ...
                    for col in columns {
                        ...
                        let dtype = map_sql_dtype_to_apollo(&col.data_type)?;
                ...
                (None, true, Some(like_kind)) => {
                    ...
                    if let Some(mut table) = self.table_map.get(like_table).cloned() {
                        ...
                        apollo_bail!(SQLInterface: "table given in LIKE does not exist: {}", like_table)
                    }
                },
                // No valid options provided
                (None, true, None) => {
                    apollo_bail!(SQLInterface: "CREATE TABLE expected a query, column definitions, or LIKE clause")
                },
    ...
    fn get_table(&mut self, relation: &TableFactor) -> ApolloResult<(String, LazyFrame)> {
        match relation {
            TableFactor::Table {
                name, alias, args, ..
            } => {
                ...
                if let Some(lf) = self.get_table_from_current_scope(tbl_name) {
                    ...
                    apollo_bail!(SQLInterface: "relation '{}' was not found", tbl_name);
                }
            },
            TableFactor::Derived {
                lateral,
                ...
                apollo_ensure!(!(*lateral), SQLInterface: "LATERAL not supported");
                if let Some(alias) = alias {
                    ...
                    apollo_bail!(SQLSyntax: "derived tables must have aliases");
                }
            },
            TableFactor::UNNEST {
                alias,
                ...
                if let Some(alias) = alias {
                    ...
                    apollo_ensure!(!column_names.is_empty(),
                        SQLSyntax:
                    ...
                    if *with_offset {
                        ...
                        apollo_bail!(SQLInterface: "UNNEST tables do not (yet) support WITH ORDINALITY|OFFSET");
                    }
                    let table_name = alias.name.value.clone();
                    self.table_map.insert(table_name.clone(), lf.clone());
                    Ok((table_name, lf))
                } else {
                    apollo_bail!(SQLSyntax: "UNNEST table must have an alias");
                }
            },
            ...
            _ => apollo_bail!(SQLInterface: "not yet implemented: {}", relation),
        }
    }
    ...
    fn execute_table_function(
        ...
        alias: &Option<TableAlias>,
        args: &[FunctionArg],
    ) -> ApolloResult<(String, LazyFrame)> {
        let tbl_fn = name.0.first().unwrap().as_ident().unwrap().value.as_str();
        let read_fn = tbl_fn.parse::<ApolloTableFunctions>()?;
    ...
    fn process_group_by(
        ...
        // Note: remove the `group_by` keys as Apollo adds those implicitly.
        ...
        for (e, &matches_group_key) in projections.iter().zip(&projection_matches_group_key) {
            ...
            if is_non_group_key_expr {
                ...
                if group_by_keys_schema.get(&field.name).is_some() {
                    let alias_name = format!("__APOLLO_AGG_{}", field.name);
                ...
                } = e_inner
                {
                    if !group_by_keys_schema.contains(&field.name) {
                        apollo_bail!(SQLSyntax: "'{}' should participate in the GROUP BY clause or an aggregate function", &field.name);
                    }
                }
            }
        }
        ...
        let having_filter = if let Some(having_expr) = having {
            ...
            let updated_having = having_expr.map_expr(|e| {
                ...
                let name = agg_to_name
                    ...
                    .unwrap_or_else(|| {
                        let n = format_pl_smallstr!("__APOLLO_HAVING_{n_having_aggs}");
        ...
        for key_name in group_by_keys_schema.iter_names() {
            if !projection_schema.contains(key_name) {
                ...
                if is_cross_aliased {
                    ...
                    let internal_name = format_pl_smallstr!("__APOLLO_ORIG_{}", key_name);
    ...
    fn process_limit_offset(
        ...
        match (offset, limit) {
            (
                ...
                offset
                    .parse()
                    .map_err(|e| apollo_err!(SQLInterface: "OFFSET conversion error: {}", e))?,
                limit
                    .parse()
                    .map_err(|e| apollo_err!(SQLInterface: "LIMIT conversion error: {}", e))?,
            )),
            (
                Some(SQLExpr::Value(ValueWithSpan {
                    value: SQLValue::Number(offset, _),
    ...
    fn process_wildcard_additional_options(
        ...
        if options.opt_except.is_some() && options.opt_exclude.is_some() {
            apollo_bail!(SQLInterface: "EXCLUDE and EXCEPT wildcard options cannot be used together (prefer EXCLUDE)")
        } else if options.opt_exclude.is_some() && options.opt_ilike.is_some() {
            apollo_bail!(SQLInterface: "EXCLUDE and ILIKE wildcard options cannot be used together")
        }
        ...
        if let Some(item) = &options.opt_ilike {
            ...
            modifiers.ilike = Some(
                apollo_utils::regex_cache::compile_regex(format!("^(?is){rx}$").as_str()).unwrap(),
            );
        }
...
fn expand_exprs(expr: Expr, schema: &SchemaRef) -> Vec<Expr> {
    match expr {
        Expr::Column(nm) if is_regex_colname(nm.as_str()) => {
            let re = apollo_utils::regex_cache::compile_regex(&nm).unwrap();
            schema
                .iter_names()
...
/// Check if all columns referred to in a Apollo expression exist in the given Schema.
...
fn determine_left_right_join_on(
    ...
    tbl_right: &TableInfo,
    join_schema: &Schema,
) -> ApolloResult<(Vec<Expr>, Vec<Expr>)> {
    ...
    match constraint {
        ...
        JoinConstraint::Using(idents) if !idents.is_empty() => {
            let using: Vec<Expr> = idents
                ...
                .map(|ObjectName(parts)| {
                    if parts.len() != 1 {
                        apollo_bail!(SQLSyntax: "JOIN \"USING\" clause expects simple column names, not qualified names");
                    }
                    match parts[0].as_ident() {
                        Some(ident) => Ok(col(ident.value.as_str())),
                        None => apollo_bail!(SQLSyntax: "JOIN \"USING\" clause expects identifiers, not functions"),
                    }
                })
                .collect::<ApolloResult<Vec<_>>>()?;
        ...
        JoinConstraint::Natural => {
            ...
            let on: Vec<Expr> = left_names
                ...
                .collect();
            if on.is_empty() {
                apollo_bail!(SQLInterface: "no common columns found for NATURAL JOIN")
            }
            Ok((on.clone(), on))
        },
        _ => apollo_bail!(SQLInterface: "unsupported SQL join constraint:\n{:?}", constraint),
    }
}
