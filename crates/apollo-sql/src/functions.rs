...
use apollo_core::chunked_array::ops::{SortMultipleOptions, SortOptions};
use apollo_core::prelude::{
    DataType, ExplodeOptions, ApolloResult, QuantileMethod, Schema, TimeUnit, apollo_bail,
    apollo_err,
};
use apollo_lazy::dsl::Expr;
#[cfg(feature = "rank")]
use apollo_lazy::prelude::{RankMethod, RankOptions};
use apollo_ops::chunked_array::UnicodeForm;
use apollo_ops::series::RoundMode;
use apollo_plan::dsl::functions::{
    as_struct, coalesce, col, cols, concat_str, element, int_range, len, lit, max_horizontal,
    min_horizontal, when,
};
use apollo_plan::plans::{DynLiteralValue, LiteralValue, typed_lit};
use apollo_plan::prelude::StrptimeOptions;
...
/// SQL functions that are supported by Apollo
pub(crate) enum ApolloSQLFunctions {
    // ----
    // Bitwise functions
    // ----
...
impl ApolloSQLFunctions {
    fn try_from_sql(function: &'_ SQLFunction, ctx: &'_ SQLContext) -> ApolloResult<Self> {
        ...
        Ok(match function_name.as_str() {
            ...
            other => {
                if ctx.function_registry.contains(other) {
                    ...
                    apollo_bail!(SQLInterface: "unsupported function '{}'", other);
                }
            },
        })
    }
...
impl SQLFunctionVisitor<'_> {
    pub(crate) fn visit_function(&mut self) -> ApolloResult<Expr> {
        use ApolloSQLFunctions::*;
        use apollo_lazy::prelude::Literal;

        let function_name = ApolloSQLFunctions::try_from_sql(self.func, self.ctx)?;
        let function = self.func;

        // TODO: implement the following functions where possible
        if !function.within_group.is_empty() {
            apollo_bail!(SQLInterface: "'WITHIN GROUP' is not currently supported")
        }
        if function.filter.is_some() {
            apollo_bail!(SQLInterface: "'FILTER' is not currently supported")
        }
        if function.null_treatment.is_some() {
            apollo_bail!(SQLInterface: "'IGNORE|RESPECT NULLS' is not currently supported")
        }
        ...
        match function_name {
                        ...
                        Ok(e.round(match decimals {
                            Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Int(n))) => {
                                if n >= 0 { n as u32 } else {
                                    apollo_bail!(SQLInterface: "ROUND does not currently support negative decimals value ({})", args[1])
                                }
                            },
                            _ => apollo_bail!(SQLSyntax: "invalid value for ROUND decimals ({})", args[1]),
                        }, RoundMode::default()))
                    }),
                    _ => apollo_bail!(SQLSyntax: "ROUND expects 1-2 arguments (found {})", args.len()),
                }
            },
            ...
            If => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "IF expects 3 arguments (found {})", args.len()
                        )
                    },
                }
            },
            IfNull => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "IFNULL expects 2 arguments (found {})", args.len())
                    },
                }
            },
            ...
            NullIf => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "NULLIF expects 2 arguments (found {})", args.len())
                    },
                }
            },
            ...
            DatePart => self.try_visit_binary(|part, e| {
                match part {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "invalid 'part' for EXTRACT/DATE_PART ({})", part);
                    },
                }
            }),
            Strftime => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "STRFTIME expects 2 arguments (found {})", args.len())
                    },
                }
            },
            ...
            Concat => {
                let args = extract_args(function)?;
                if args.is_empty() {
                    apollo_bail!(SQLSyntax: "CONCAT expects at least 1 argument (found 0)");
                } else {
            ...
            ConcatWS => {
                let args = extract_args(function)?;
                if args.len() < 2 {
                    apollo_bail!(SQLSyntax: "CONCAT_WS expects at least 2 arguments (found {})", args.len());
                } else {
                    self.try_visit_variadic(|exprs: &[Expr]| {
                        match &exprs[0] {
                            ...
                            _ => apollo_bail!(SQLSyntax: "CONCAT_WS 'separator' must be a literal string (found {:?})", exprs[0]),
                        }
                    })
                }
            },
            Date => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "DATE expects 1-2 arguments (found {})", args.len())
                    },
                }
            },
            ...
            Left => self.try_visit_binary(|e, length| {
                Ok(match length {
                    ...
                    Expr::Literal(v) => {
                        apollo_bail!(SQLSyntax: "invalid 'n_chars' for LEFT ({:?})", v)
                    },
            ...
            LTrim => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "LTRIM expects 1-2 arguments (found {})", args.len())
                    },
                }
            },
            Normalize => {
                    ...
                    2 => {
                        ...
                        })) = args[1]
                        {
                            match s.to_uppercase().as_str() {
                                ...
                                _ => {
                                    apollo_bail!(SQLSyntax: "invalid 'form' for NORMALIZE (found {})", s)
                                },
                            }
                        } else {
                            apollo_bail!(SQLSyntax: "invalid 'form' for NORMALIZE (found {})", args[1])
                        };
                        self.try_visit_binary(|e, _form: Expr| Ok(e.str().normalize(form.clone())))
                    },
                    _ => {
                        apollo_bail!(SQLSyntax: "NORMALIZE expects 1-2 arguments (found {})", args.len())
                    },
                }
            },
                        ...
                        Ok(e.str().contains(
                            match (pat, flags) {
                                (Expr::Literal(s_lv), Expr::Literal(f_lv)) if s_lv.extract_str().is_some() && f_lv.extract_str().is_some() => {
                                    ...
                                    if f.is_empty() {
                                        apollo_bail!(SQLSyntax: "invalid/empty 'flags' for REGEXP_LIKE ({})", args[2]);
                                    };
                                    lit(format!("(?{f}){s}"))
                                },
                                _ => {
                                    apollo_bail!(SQLSyntax: "invalid arguments for REGEXP_LIKE ({}, {})", args[1], args[2]);
                                },
                            },
                            true))
                    }),
                    _ => apollo_bail!(SQLSyntax: "REGEXP_LIKE expects 2-3 arguments (found {})",args.len()),
                }
            },
            Replace => {
                let args = extract_args(function)?;
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "REPLACE expects 3 arguments (found {})", args.len())
                    },
                }
            },
            ...
            Right => self.try_visit_binary(|e, length| {
                Ok(match length {
                    ...
                    Expr::Literal(v) => {
                        apollo_bail!(SQLSyntax: "invalid 'n_chars' for RIGHT ({:?})", v)
                    },
            ...
            RTrim => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "RTRIM expects 1-2 arguments (found {})", args.len())
                    },
                }
            },
            SplitPart => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "SPLIT_PART expects 3 arguments (found {})", args.len())
                    },
                }
            },
            ...
            StringToArray => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "STRING_TO_ARRAY expects 2 arguments (found {})", args.len())
                    },
                }
            },
            Strptime => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "STRPTIME expects 2 arguments (found {})", args.len())
                    },
                }
            },
            Time => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "TIME expects 1-2 arguments (found {})", args.len())
                    },
                }
            },
            Timestamp => {
                ...
                match args.len() {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "DATETIME expects 1-2 arguments (found {})", args.len())
                    },
                }
            },
            Substring => {
                ...
                match args.len() {
                    ...
                    2 => self.try_visit_binary(|e, start| {
                        Ok(match start {
                            ...
                            Expr::Literal(_) => apollo_bail!(SQLSyntax: "invalid 'start' for SUBSTR ({})", args[1]),
                            _ => start.clone() + lit(1),
                        })
                    }),
                    3 => self.try_visit_ternary(|e: Expr, start: Expr, length: Expr| {
                        Ok(match (start.clone(), length.clone()) {
                            ...
                            (_, Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Int(n)))) if n < 0 => {
                                apollo_bail!(SQLSyntax: "SUBSTR does not support negative length ({})", args[2])
                            },
                            ...
                            (Expr::Literal(_), _) => apollo_bail!(SQLSyntax: "invalid 'start' for SUBSTR ({})", args[1]),
                            (_, Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Float(_)))) => {
                                apollo_bail!(SQLSyntax: "invalid 'length' for SUBSTR ({})", args[1])
                            },
                            _ => {
                    ...
                    _ => apollo_bail!(SQLSyntax: "SUBSTR expects 2-3 arguments (found {})", args.len()),
                }
            },
            ...
            Corr => self.visit_binary(apollo_lazy::dsl::pearson_corr),
            Count => self.visit_count(),
            CovarPop => self.visit_binary(|a, b| apollo_lazy::dsl::cov(a, b, 0)),
            CovarSamp => self.visit_binary(|a, b| apollo_lazy::dsl::cov(a, b, 1)),
                        ...
                        let value = match q {
                            Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Float(f))) => {
                                if (0.0..=1.0).contains(&f) {
                                    ...
                                    apollo_bail!(SQLSyntax: "QUANTILE_CONT value must be between 0 and 1 ({})", args[1])
                                }
                            },
                            Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Int(n))) => {
                                if (0..=1).contains(&n) {
                            ...
                            _ => apollo_bail!(SQLSyntax: "invalid value for QUANTILE_CONT ({})", args[1])
                        };
                        Ok(e.quantile(value, QuantileMethod::Linear))
                    }),
                    _ => apollo_bail!(SQLSyntax: "QUANTILE_CONT expects 2 arguments (found {})", args.len()),
                }
            },
            QuantileDisc => {
                let args = extract_args(function)?;
                        ...
                        let value = match q {
                            Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Float(f))) => {
                                if (0.0..=1.0).contains(&f) {
                                    ...
                                    apollo_bail!(SQLSyntax: "QUANTILE_DISC value must be between 0 and 1 ({})", args[1])
                                }
                            },
                            Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Int(n))) => {
                                if (0..=1).contains(&n) {
                            ...
                            _ => apollo_bail!(SQLSyntax: "invalid value for QUANTILE_DISC ({})", args[1])
                        };
                        Ok(e.quantile(value, QuantileMethod::Equiprobable))
                    }),
                    _ => apollo_bail!(SQLSyntax: "QUANTILE_DISC expects 2 arguments (found {})", args.len()),
                }
            },
                ...
                self.try_visit_unary(|e: Expr| match e {
                    Expr::Literal(lv) if lv.extract_str().is_some() => {
                        ...
                        if let Some(active_schema) = &active_schema {
                            let rx = apollo_utils::regex_cache::compile_regex(&pat).unwrap();
                    ...
                    _ => apollo_bail!(SQLSyntax: "COLUMNS expects a regex; found {:?}", e),
                })
            },
            ...
            Rank | DenseRank => {
                ...
                let args = extract_args(function)?;
                if !args.is_empty() {
                    apollo_bail!(SQLSyntax: "{} expects 0 arguments (found {})", func_name, args.len());
                }
                let window_spec = match &self.func.over {
                    Some(WindowType::WindowSpec(spec)) if !spec.order_by.is_empty() => spec,
                    _ => {
                        apollo_bail!(SQLSyntax: "{} requires an OVER clause with ORDER BY", func_name)
                    },
                };
            ...
            RowNumber => {
                let args = extract_args(function)?;
                if !args.is_empty() {
                    apollo_bail!(SQLSyntax: "ROW_NUMBER expects 0 arguments (found {})", args.len());
                }
    ...
    fn visit_window_offset_function(&mut self, offset_multiplier: i64) -> ApolloResult<Expr> {
        // LAG/LEAD require an OVER clause
        if self.func.over.is_none() {
            apollo_bail!(SQLSyntax: "{} requires an OVER clause", self.func.name);
        }
        ...
        if window_spec.order_by.is_empty() {
            apollo_bail!(SQLSyntax: "{} requires an ORDER BY in the OVER clause", self.func.name);
        }
        ...
        match args.as_slice() {
            ...
            [FunctionArgExpr::Expr(sql_expr), FunctionArgExpr::Expr(offset_expr)] => {
                ...
                if let Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Int(n))) = offset {
                    if n <= 0 {
                        apollo_bail!(SQLSyntax: "offset must be positive (found {})", n)
                    }
                    Ok(expr.shift((offset_multiplier * n as i64).into()))
                } else {
                    apollo_bail!(SQLSyntax: "offset must be an integer (found {:?})", offset)
                }
            },
            _ => apollo_bail!(SQLSyntax: "{} expects 1 or 2 arguments (found {})", self.func.name, args.len()),
        }.and_then(|e| self.apply_window_spec(e, &self.func.over))
    }

    fn visit_udf(&mut self, func_name: &str) -> ApolloResult<Expr> {
        let args = extract_args(self.func)?
            .into_iter()
            .map(|arg| {
                if let FunctionArgExpr::Expr(e) = arg {
                    ...
                    apollo_bail!(SQLInterface: "only expressions are supported in UDFs")
                }
            })
        ...
        Ok(self
            ...
            .function_registry
            .get_udf(func_name)?
            .ok_or_else(|| apollo_err!(SQLInterface: "UDF {} not found", func_name))?
            .call(args))
    }

    /// Validate window frame specifications.
    ///
    /// Apollo only supports ROWS frame semantics, and does
    ...
    fn validate_window_frame(&self, window_frame: &Option<WindowFrame>) -> ApolloResult<()> {
    ...
    fn visit_unary(&mut self, f: impl Fn(Expr) -> Expr) -> ApolloResult<Expr> {
        self.try_visit_unary(|e| Ok(f(e)))
    }

    fn try_visit_unary(&mut self, f: impl Fn(Expr) -> ApolloResult<Expr>) -> ApolloResult<Expr> {
        let args = extract_args(self.func)?;
        match args.as_slice() {
            [FunctionArgExpr::Expr(sql_expr)] => {
    ...
    fn resolve_window_spec(&self, window_type: &WindowType) -> ApolloResult<WindowSpec> {
        match window_type {
    ...
    fn try_visit_binary<Arg: FromSQLExpr>(
        &mut self,
        f: impl Fn(Expr, Arg) -> ApolloResult<Expr>,
    ) -> ApolloResult<Expr> {
        let args = extract_args(self.func)?;
        match args.as_slice() {
            [
    ...
    fn visit_variadic(&mut self, f: impl Fn(&[Expr]) -> Expr) -> ApolloResult<Expr> {
        self.try_visit_variadic(|e| Ok(f(e)))
    }

    fn try_visit_variadic(
        &mut self,
        f: impl Fn(&[Expr]) -> ApolloResult<Expr>,
    ) -> ApolloResult<Expr> {
        let args = extract_args(self.func)?;
    ...
    fn try_visit_ternary<Arg: FromSQLExpr>(
        &mut self,
        f: impl Fn(Expr, Arg, Arg) -> ApolloResult<Expr>,
    ) -> ApolloResult<Expr> {
        let args = extract_args(self.func)?;
        match args.as_slice() {
            [
    ...
    fn visit_nullary(&self, f: impl Fn() -> Expr) -> ApolloResult<Expr> {
        let args = extract_args(self.func)?;
        if !args.is_empty() {
            return self.not_supported_error();
        }
    ...
    fn visit_arr_agg(&mut self) -> ApolloResult<Expr> {
        ...
        match args.as_slice() {
            [FunctionArgExpr::Expr(sql_expr)] => {
                ...
                if let Some(limit_expr) = limit_clause {
                    ...
                    match limit {
                        ...
                        _ => {
                            apollo_bail!(SQLSyntax: "LIMIT in ARRAY_AGG must be a positive integer")
                        },
                    };
                }
                Ok(base.implode())
            ...
            _ => {
                apollo_bail!(SQLSyntax: "ARRAY_AGG must have exactly one argument; found {}", args.len())
            },
        }
    }

    fn visit_arr_to_string(&mut self) -> ApolloResult<Expr> {
        let args = extract_args(self.func)?;
        match args.len() {
            ...
            3 => self.try_visit_ternary(|e, sep, null_value| match null_value {
                ...
                _ => {
                    apollo_bail!(SQLSyntax: "invalid null value for ARRAY_TO_STRING ({})", args[2])
                },
            }),
            _ => {
                apollo_bail!(SQLSyntax: "ARRAY_TO_STRING expects 2-3 arguments (found {})", args.len())
            },
        }
    }

    fn visit_count(&mut self) -> ApolloResult<Expr> {
    ...
    fn apply_order_by(&mut self, expr: Expr, order_by: &[OrderByExpr]) -> ApolloResult<Expr> {
    ...
    fn not_supported_error(&self) -> ApolloResult<Expr> {
        apollo_bail!(
            SQLInterface:
...
fn extract_args(func: &SQLFunction) -> ApolloResult<Vec<&FunctionArgExpr>> {
    let (args, _, _) = _extract_func_args(func, false, false)?;
    Ok(args)
}

fn extract_args_distinct(func: &SQLFunction) -> ApolloResult<(Vec<&FunctionArgExpr>, bool)> {
...
fn extract_args_and_clauses(
    func: &SQLFunction,
) -> ApolloResult<(Vec<&FunctionArgExpr>, bool, Vec<FunctionArgumentClause>)> {
    ...
    match &func.args {
        FunctionArguments::List(FunctionArgumentList {
            ...
            if !(get_clauses || get_distinct) && is_distinct {
                apollo_bail!(SQLSyntax: "unexpected use of DISTINCT found in '{}'", func.name)
            } else if !get_clauses && !clauses.is_empty() {
                apollo_bail!(SQLSyntax: "unexpected clause found in '{}' ({})", func.name, clauses[0])
            } else {
        ...
        FunctionArguments::Subquery { .. } => {
            Err(apollo_err!(SQLInterface: "subquery not expected in {}", func.name))
        },
...
pub(crate) trait FromSQLExpr {
    fn from_sql_expr(expr: &SQLExpr, ctx: &mut SQLContext) -> ApolloResult<Self>
    where
        Self: Sized;
}

impl FromSQLExpr for f64 {
    fn from_sql_expr(expr: &SQLExpr, _ctx: &mut SQLContext) -> ApolloResult<Self>
    where
        Self: Sized,
    {
        match expr {
            SQLExpr::Value(ValueWithSpan { value: v, .. }) => match v {
                SQLValue::Number(s, _) => s
                    .parse()
                    .map_err(|_| apollo_err!(SQLInterface: "cannot parse literal {:?}", s)),
                _ => apollo_bail!(SQLInterface: "cannot parse literal {:?}", v),
            },
            _ => apollo_bail!(SQLInterface: "cannot parse literal {:?}", expr),
        }
    }
}
        ...
        Self: Sized,
    {
        match expr {
            SQLExpr::Value(ValueWithSpan { value: v, .. }) => match v {
                ...
                _ => apollo_bail!(SQLInterface: "cannot parse boolean {:?}", v),
            },
            _ => apollo_bail!(SQLInterface: "cannot parse boolean {:?}", expr),
        }
    }
}

impl FromSQLExpr for String {
    fn from_sql_expr(expr: &SQLExpr, _: &mut SQLContext) -> ApolloResult<Self>
    where
        Self: Sized,
    {
        match expr {
