//! Expressions that are supported by the Apollo SQL interface.
//!
//! This is useful for syntax highlighting
//!
//! This module defines:
//! - all Apollo SQL keywords [`all_keywords`]
//! - all Apollo SQL functions [`all_functions`]
...
use apollo_plan::plans::DynLiteralValue;
use apollo_plan::prelude::typed_lit;
use apollo_time::Duration;
use apollo_utils::unique_column_name;
#[cfg(feature = "serde")]
...
use crate::types::{
    bitstring_to_bytes_literal, is_iso_date, is_iso_datetime, is_iso_time, map_sql_dtype_to_apollo,
};

#[inline]
#[cold]
#[must_use]
/// Convert a Display-able error to ApolloError::SQLInterface
pub fn to_sql_interface_err(err: impl Display) -> ApolloError {
    ApolloError::SQLInterface(err.to_string().into())
}
...
/// Recursively walks a SQL Expr to create a apollo Expr
...
impl SQLExprVisitor<'_> {
    fn array_expr_to_series(&mut self, elements: &[SQLExpr]) -> ApolloResult<Series> {
        ...
        for e in elements {
            let val = match e {
                ...
                SQLExpr::UnaryOp { op, expr } => match expr.as_ref() {
                    ...
                    _ => Err(apollo_err!(SQLInterface: "array element {:?} is not supported", e)),
                },
    ...
    fn visit_expr(&mut self, expr: &SQLExpr) -> ApolloResult<Expr> {
        match expr {
            SQLExpr::AllOp {
                left,
                compare_op,
            ...
            SQLExpr::CompoundFieldAccess { root, access_chain } => {
                ...
                if access_chain.len() == 1 {
                    match &access_chain[0] {
                        ...
                        AccessExpr::Dot(_) => {
                            apollo_bail!(SQLSyntax: "dot-notation field access is currently unsupported: {:?}", access_chain[0])
                        },
                    }
                }
                // chained dot/bracket notation (eg: "struct_col.field[2].foo[0].bar")
                apollo_bail!(SQLSyntax: "complex field access chains are currently unsupported: {:?}", access_chain[0])
            },
            ...
            SQLExpr::Like {
                ...
                if *any {
                    apollo_bail!(SQLSyntax: "LIKE ANY is not a supported syntax")
                }
            ...
            SQLExpr::ILike {
                ...
                if *any {
                    apollo_bail!(SQLSyntax: "ILIKE ANY is not a supported syntax")
                }
                let escape_str = escape_char.as_ref().and_then(|v| match v {
                    SQLValue::SingleQuotedString(s) => Some(s.clone()),
                    _ => None,
            ...
            SQLExpr::Subquery(_) => apollo_bail!(SQLInterface: "unexpected subquery"),
            SQLExpr::Substring {
                expr,
                substring_from,
                substring_for,
            ...
            SQLExpr::TypedString(TypedString {
                ...
                SQLDataType::Date => {
                    if is_iso_date(v) {
                        ...
                        apollo_bail!(SQLSyntax: "invalid DATE literal '{}'", v)
                    }
                },
                SQLDataType::Time(None, TimezoneInfo::None) => {
                    if is_iso_time(v) {
                        ...
                        apollo_bail!(SQLSyntax: "invalid TIME literal '{}'", v)
                    }
                },
                SQLDataType::Timestamp(None, TimezoneInfo::None) | SQLDataType::Datetime(None) => {
                    if is_iso_datetime(v) {
                        ...
                        apollo_bail!(SQLSyntax: "invalid {} literal '{}'", fn_name, v)
                    }
                },
                _ => {
                    apollo_bail!(SQLInterface: "typed literal should be one of DATE, DATETIME, TIME, or TIMESTAMP (found {})", data_type)
                },
            },
            ...
            other => {
                apollo_bail!(SQLInterface: "expression {:?} is not currently supported", other)
            },
        }
    }
    ...
    fn visit_subquery(
        ...
        if subquery.with.is_some() {
            apollo_bail!(SQLSyntax: "SQL subquery cannot be a CTE 'WITH' clause");
        }
        ...
        if restriction == SubqueryRestriction::SingleColumn {
            if schema.len() != 1 {
                apollo_bail!(SQLSyntax: "SQL subquery returns more than one column");
            }
        ...
        apollo_bail!(SQLInterface: "subquery type not supported");
    }

    /// Visit a single SQL identifier.
    ///
    /// e.g. column
    fn visit_identifier(&self, ident: &Ident) -> ApolloResult<Expr> {
    ...
    fn visit_compound_identifier(&mut self, idents: &[Ident]) -> ApolloResult<Expr> {
    ...
    fn visit_like(
        ...
        if escape_char.is_some() {
            apollo_bail!(SQLInterface: "ESCAPE char for LIKE/ILIKE is not currently supported; found '{}'", escape_char.clone().unwrap());
        }
        let pat = match self.visit_expr(pattern) {
            ...
            _ => {
                apollo_bail!(SQLSyntax: "LIKE/ILIKE pattern must be a string literal; found {}", pattern)
            },
        };
    ...
    fn visit_subscript(&mut self, expr: &SQLExpr, subscript: &Subscript) -> ApolloResult<Expr> {
        let expr = self.visit_expr(expr)?;
        Ok(match subscript {
            ...
            Subscript::Slice { .. } => {
                apollo_bail!(SQLSyntax: "array slice syntax is not currently supported")
            },
        })
    }
    ...
    fn visit_binary_op(
        ...
        let (lhs, mut rhs) = match (left, op, right) {
            ...
            (SQLExpr::Interval(v1), _, SQLExpr::Interval(v2)) => {
                ...
                let res = match op {
                    ...
                    _ => apollo_bail!(SQLInterface: "invalid interval comparison operator"),
                };
                if res.is_ok() {
                    return res;
                }
        ...
        Ok(match op {
            ...
            SQLBinaryOperator::PGRegexMatch => match rhs {  // "x ~ y"
                ...
                _ => apollo_bail!(SQLSyntax: "invalid pattern for '~' operator: {:?}", rhs),
            },
            ...
            SQLBinaryOperator::PGRegexNotIMatch => match rhs {  // "x !~* y"
                ...
                _ => {
                    apollo_bail!(SQLSyntax: "invalid pattern for '!~*' operator: {:?}", rhs)
                },
            },
            // ----
            ...
            SQLBinaryOperator::Arrow | SQLBinaryOperator::LongArrow => match rhs {  // "x -> y", "x ->> y"
                ...
                _ => {
                    apollo_bail!(SQLSyntax: "invalid json/struct path-extract definition: {:?}", right)
                },
            },
            SQLBinaryOperator::HashArrow | SQLBinaryOperator::HashLongArrow => {  // "x #> y", "x #>> y"
                match rhs {
                    ...
                    _ => {
                        apollo_bail!(SQLSyntax: "invalid json/struct path-extract definition: {:?}", rhs)
                    }
                }
            },
            other => {
                apollo_bail!(SQLInterface: "operator {:?} is not currently supported", other)
            },
        })
    }
    ...
    fn visit_unary_op(&mut self, op: &UnaryOperator, expr: &SQLExpr) -> ApolloResult<Expr> {
        let expr = self.visit_expr(expr)?;
        Ok(match (op, expr.clone()) {
            ...
            other => apollo_bail!(SQLInterface: "unary operator {:?} is not supported", other),
        })
    }
    ...
    fn visit_function(&mut self, function: &SQLFunction) -> ApolloResult<Expr> {
    ...
    fn visit_all(
        ...
        match compare_op {
            ...
            SQLBinaryOperator::Eq => apollo_bail!(SQLSyntax: "ALL cannot be used with ="),
            SQLBinaryOperator::NotEq => apollo_bail!(SQLSyntax: "ALL cannot be used with !="),
            _ => apollo_bail!(SQLInterface: "invalid comparison operator"),
        }
    }
    ...
    fn visit_cast(
        ...
        if format.is_some() {
            return Err(
                apollo_err!(SQLInterface: "use of FORMAT is not currently supported in CAST"),
            );
        }
        let expr = self.visit_expr(expr)?;
        ...
        let apollo_type = map_sql_dtype_to_apollo(dtype)?;
        Ok(match cast_kind {
            CastKind::Cast | CastKind::DoubleColon => expr.strict_cast(apollo_type),
            CastKind::TryCast | CastKind::SafeCast => expr.cast(apollo_type),
        })
    }
    ...
    fn visit_literal(&self, value: &SQLValue) -> ApolloResult<Expr> {
        ...
        Ok(match value {
            ...
            SQLValue::HexStringLiteral(x) => {
                if x.len() % 2 != 0 {
                    apollo_bail!(SQLSyntax: "hex string literal must have an even number of digits; found '{}'", x)
                };
            ...
            other => {
                apollo_bail!(SQLInterface: "value {:?} is not a supported literal type", other)
            },
        })
    }
        ...
        Ok(match value {
            ...
            SQLValue::Number(s, _) => {
                let negate = match op {
                    ...
                    Some(op) => {
                        apollo_bail!(SQLInterface: "unary op {:?} not supported for numeric SQL value", op)
                    },
                };
                // Check for existence of decimal separator dot
                if s.contains('.') {
            ...
            SQLValue::SingleQuotedByteStringLiteral(b) => {
                ...
                match bytes_literal {
                    ...
                    _ => {
                        apollo_bail!(SQLInterface: "failed to parse bitstring literal: {:?}", b)
                    },
                }
            },
            SQLValue::SingleQuotedString(s) => AnyValue::StringOwned(s.as_str().into()),
            other => apollo_bail!(SQLInterface: "value {:?} is not currently supported", other),
        })
    }
    ...
    fn visit_trim(
        ...
        if trim_characters.is_some() {
            ...
            return Err(apollo_err!(SQLSyntax: "unsupported TRIM syntax (custom chars)"));
        };
        let expr = self.visit_expr(expr)?;
    ...
    fn visit_substring(
        ...
        match (substring_from, substring_for) {
            ...
            (Some(from_expr), Some(for_expr)) => {
                ...
                Ok(match (start.clone(), length.clone()) {
                    ...
                    (_, Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Int(n)))) if n < 0 => {
                        apollo_bail!(SQLSyntax: "SUBSTR does not support negative length ({})", n)
                    },
                    ...
                    (Expr::Literal(_), _) => {
                        apollo_bail!(SQLSyntax: "invalid 'start' for SUBSTRING")
                    },
                    (_, Expr::Literal(LiteralValue::Dyn(DynLiteralValue::Float(_)))) => {
                        apollo_bail!(SQLSyntax: "invalid 'length' for SUBSTRING")
                    },
                    _ => {
            ...
            (None, _) => {
                apollo_bail!(SQLSyntax: "SUBSTR expects 2-3 arguments (found 1)")
            },
        }
    }
    ...
    fn visit_case_when_then(&mut self, expr: &SQLExpr) -> ApolloResult<Expr> {
        if let SQLExpr::Case {
            case_token: _,
            end_token: _,
            operand,
        ...
        } = expr
        {
            ...
            if first.is_none() {
                apollo_bail!(SQLSyntax: "WHEN and THEN expressions must have at least one element");
            }
    ...
    fn err(&self, expr: &Expr) -> ApolloResult<Expr> {
        apollo_bail!(SQLInterface: "expression {:?} is not currently supported", expr);
    }
}

/// parse a SQL expression to a apollo expression
/// # Example
/// ```rust
/// # use apollo_sql::{SQLContext, sql_expr};
/// # use apollo_core::prelude::*;
/// # use apollo_lazy::prelude::*;
/// # fn main() {
///
...
/// # }
/// ```
pub fn sql_expr<S: AsRef<str>>(s: S) -> ApolloResult<Expr> {
    let mut ctx = SQLContext::new();
    ...
    Ok(match &expr {
        ...
        _ => apollo_bail!(SQLInterface: "unable to parse '{}' as Expr", s.as_ref()),
    })
}

pub(crate) fn interval_to_duration(interval: &Interval, fixed: bool) -> ApolloResult<Duration> {
    ...
    {
        apollo_bail!(SQLSyntax: "unsupported interval syntax ('{}')", interval)
    }
    let s = match &*interval.value {
        SQLExpr::UnaryOp { .. } => {
            apollo_bail!(SQLSyntax: "unary ops are not valid on interval strings; found {}", interval.value)
        },
    ...
    match s {
        Some(s) if s.contains('-') => {
            apollo_bail!(SQLInterface: "minus signs are not yet supported in interval strings; found '{}'", s)
        },
        Some(s) => {
            ...
            if fixed && duration.months() != 0 {
                apollo_bail!(SQLSyntax: "fixed-duration interval cannot contain years, quarters, or months; found {}", s)
            };
            Ok(duration)
        },
        None => apollo_bail!(SQLSyntax: "invalid interval {:?}", interval),
    }
}
...
pub(crate) fn parse_sql_array(expr: &SQLExpr, ctx: &mut SQLContext) -> ApolloResult<Series> {
    match expr {
        SQLExpr::Array(arr) => {
        ...
        _ => apollo_bail!(SQLSyntax: "Expected array expression, found {:?}", expr),
    }
}

pub(crate) fn parse_extract_date_part(expr: Expr, field: &DateTimeField) -> ApolloResult<Expr> {
    let field = match field {
        ...
        DateTimeField::Custom(Ident { value, .. }) => {
            ...
            match value.as_str() {
                ...
                _ => {
                    apollo_bail!(SQLSyntax: "EXTRACT/DATE_PART does not support '{}' part", value)
                },
            }
        },
        _ => field,
    ...
    Ok(match field {
        ...
        DateTimeField::Week(weekday) => {
            if weekday.is_some() {
                apollo_bail!(SQLSyntax: "EXTRACT/DATE_PART does not support '{}' part", field)
            }
            expr.dt().week()
        },
...
/// be adjusted from 1-indexed (SQL) to 0-indexed (Rust/Apollo)
...
fn resolve_column<'a>(
    ...
    name: &'a str,
    dtype: &'a DataType,
) -> ApolloResult<(Expr, Option<&'a DataType>)> {
...
pub(crate) fn resolve_compound_identifier(
    ...
    if lf.is_none() && schema.is_empty() {
        ...
        for ident in remaining_idents {
            ...
            match dtype {
                ...
                Some(dtype) if name == "*" => {
                    apollo_bail!(SQLSyntax: "cannot expand '*' on non-Struct dtype; found {:?}", dtype)
                },
                _ => dtype = None,
            }
    ...
    let col_dtype: ApolloResult<(Expr, Option<&DataType>)> =
