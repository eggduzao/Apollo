...
fn assert_sql_to_apollo(df: &DataFrame, sql: &str, f: impl FnOnce(LazyFrame) -> LazyFrame) {
...
#[test]
fn test_simple_select() -> ApolloResult<()> {
...
#[test]
fn test_nested_expr() -> ApolloResult<()> {
    let df = create_sample_df();
    let mut context = SQLContext::new();
    context.register("df", df.clone().lazy());
    let df_sql = context
...
#[test]
fn test_group_by_simple() -> ApolloResult<()> {
    let df = create_sample_df();
    let mut context = SQLContext::new();
    context.register("df", df.clone().lazy());
    let df_sql = context
...
#[test]
fn test_group_by_expression_key() -> ApolloResult<()> {
    let df = df! {
        "a" => &["xx", "yy", "xx", "yy", "xx", "zz"],
        "b" => &[1, 2, 3, 4, 5, 6],
        "c" => &[99, 99, 66, 66, 66, 66],
...
fn test_arr_agg() {
    ...
    for (sql, expr) in exprs {
        assert_sql_to_apollo(&df, sql, |df| df.select(&expr));
    }
}
...
fn test_explode_with_multiple_columns() {
    ...
    let df_pl_api = df.clone().lazy().explode(
        apollo_lazy::dsl::Selector::ByName {
...
fn test_multiple_explodes_different_columns() {
    ...
    // Test using both the Apollo API and SQL
...
#[test]
fn test_cte_values() -> ApolloResult<()> {
    let sql = r#"
        WITH
...
#[test]
#[cfg(feature = "ipc")]
fn test_group_by_2() -> ApolloResult<()> {
