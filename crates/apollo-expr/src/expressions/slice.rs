...
use apollo_core::utils::{CustomIterTools, slice_offsets};
...
fn extract_offset(offset: &Column, expr: &Expr) -> ApolloResult<i64> {
    apollo_ensure!(
    ...
    offset.get(0).unwrap().extract().ok_or_else(
        || apollo_err!(expr = expr, ComputeError: "unable to extract offset from {:?}", offset),
    )
}

fn extract_length(length: &Column, expr: &Expr) -> ApolloResult<usize> {
    apollo_ensure!(
    ...
    match length.get(0).unwrap() {
        ...
        v => v.extract().ok_or_else(
            || apollo_err!(expr = expr, ComputeError: "unable to extract length from {:?}", length),
        ),
    }
}

fn extract_args(offset: &Column, length: &Column, expr: &Expr) -> ApolloResult<(i64, usize)> {
    Ok((extract_offset(offset, expr)?, extract_length(length, expr)?))
}

fn check_argument(arg: &Column, groups: &GroupsType, name: &str, expr: &Expr) -> ApolloResult<()> {
    apollo_ensure!(
