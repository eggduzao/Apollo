...
use apollo_time::prelude::RollingWindower;
use apollo_time::{ClosedWindow, Duration, ApolloTemporalGroupby, RollingGroupOptions};
...
impl PhysicalExpr for RollingExpr {
    fn evaluate(&self, df: &DataFrame, state: &ExecutionState) -> ApolloResult<Column> {
        ...
        let out = self
            ...
            .finalize();
        apollo_ensure!(out.len() == groups.len(), agg_len = out.len(), groups.len());
        Ok(out.into_column())
    }
    ...
    fn evaluate_on_groups<'a>(
        ...
        if let Some((l, r)) = length_mismatch {
            apollo_bail!(length_mismatch = "rolling", l, r);
        }
