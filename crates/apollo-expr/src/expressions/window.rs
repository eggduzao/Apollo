...
use apollo_core::prelude::row_encode::encode_rows_unordered;
use apollo_core::prelude::sort::perfect_sort;
use apollo_core::prelude::*;
use apollo_core::series::IsSorted;
use apollo_core::utils::_split_offsets;
use apollo_core::{POOL, downcast_as_macro_arg_physical};
use apollo_ops::frame::SeriesJoin;
use apollo_ops::frame::join::{ChunkJoinOptIds, private_left_join_multiple_keys};
use apollo_ops::prelude::*;
use apollo_plan::prelude::*;
use apollo_utils::UnitVec;
use apollo_utils::sync::SyncPtr;
    ...
    fn evaluate(&self, df: &DataFrame, state: &ExecutionState) -> ApolloResult<Column> {
        ...
        let create_groups = || {
            ...
            if let Some((order_by, options)) = &self.order_by {
                ...
                apollo_ensure!(order_by.len() == df.height(), ShapeMismatch: "the order by expression evaluated to a length: {} that doesn't match the input DataFrame: {}", order_by.len(), df.height());
                groups = update_groups_sort_by(&groups, order_by.as_materialized_series(), options)?
                    .into_sliceable()
            }

            let out: ApolloResult<GroupPositions> = Ok(groups);
            out
        };
        ...
        let (mut groups, cache_key) = if state.cache_window() {
            ...
            if let Some((e, options)) = &self.order_by {
                let e = match e.as_expression() {
                    Some(e) => e,
                    None => {
                        apollo_bail!(InvalidOperation: "cannot order by this expression in window function")
                    },
                };
...
fn materialize_column(join_opt_ids: &ChunkJoinOptIds, out_column: &Column) -> Column {
    {
        use arrow::Either;
        use apollo_ops::chunked_array::TakeChunked;
...
fn set_numeric<T: ApolloNumericType>(
    ca: &ChunkedArray<T>,
    groups: &GroupsType,
    len: usize,
) -> Series {
