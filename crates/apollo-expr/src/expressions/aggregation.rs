...
use apollo_core::utils::{_split_offsets, NoNull};
use apollo_ops::prelude::ArgAgg;
#[cfg(feature = "propagate_nans")]
use apollo_ops::prelude::nan_propagating_aggregate;
use rayon::prelude::*;
...
impl PhysicalExpr for AggregationExpr {
    ...
    fn evaluate(&self, df: &DataFrame, state: &ExecutionState) -> ApolloResult<Column> {
        ...
        match group_by {
            ...
            GroupByMethod::NanMin => parallel_op_columns(
                |s| {
                    Ok(apollo_ops::prelude::nan_propagating_aggregate::nan_min_s(
            ...
            GroupByMethod::NanMax => parallel_op_columns(
                |s| {
                    Ok(apollo_ops::prelude::nan_propagating_aggregate::nan_max_s(
                        s.as_materialized_series(),
                        s.name().clone(),
                    )
                    .into_column())
            ...
            GroupByMethod::Item { allow_empty } => Ok(match s.len() {
                ...
                1 => s,
                n => apollo_bail!(item_agg_count_not_one = n, allow_empty = allow_empty),
            }),
    ...
    ) -> ApolloResult<AggregationContext<'a>> {
        let mut ac = self.input.evaluate_on_groups(df, groups, state)?;

        // don't change names by aggregations as is done in apollo-core
                ...
                GroupByMethod::Item { allow_empty } => {
                    ...
                    for gc in groups.group_count().iter() {
                        match gc {
                            ...
                            Some(n) => {
                                apollo_bail!(item_agg_count_not_one = n, allow_empty = allow_empty);
                            },
                        }
                    }
                    let agg_s = s.agg_first(&groups);
    ...
    fn to_field(&self, _input_schema: &Schema) -> ApolloResult<Field> {
...
impl PhysicalExpr for AggQuantileExpr {
    ...
    fn evaluate(&self, df: &DataFrame, state: &ExecutionState) -> ApolloResult<Column> {
        ...
        apollo_ensure!(quantile.len() <= 1, ComputeError:
            "apollo does not support varying quantiles yet, \
        ...
        match s.dtype() {
            DataType::List(_) => {
                ...
                if inner_s.has_nulls() {
                    apollo_bail!(ComputeError: "quantile expression contains null values");
                }
    ...
    fn evaluate_on_groups<'a>(
        ...
        apollo_ensure!(quantile_column.len() <= 1, ComputeError:
            "apollo only supports computing a single quantile in a groupby aggregation context"
        );
    ...
    fn to_field(&self, input_schema: &Schema) -> ApolloResult<Field> {
...
fn parallel_op_columns<F>(f: F, s: Column, allow_threading: bool) -> ApolloResult<Column>
where
    F: Fn(Column) -> ApolloResult<Column> + Send + Sync,
{
