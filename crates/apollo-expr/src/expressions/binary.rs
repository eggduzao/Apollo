...
use apollo_ops::prelude::floor_div_series;
...
fn apply_operator_owned(left: Column, right: Column, op: Operator) -> ApolloResult<Column> {
    match op {
...
pub fn apply_operator(left: &Column, right: &Column, op: Operator) -> ApolloResult<Column> {
    use DataType::*;
    match op {
        ...
        Operator::TrueDivide => match left.dtype() {
            ...
            _ if left.dtype().is_string() || right.dtype().is_string() => {
                apollo_bail!(InvalidOperation: "cannot divide using strings")
            },
            _ => {
...
impl BinaryExpr {
    ...
    fn apply_all_literal<'a>(
        ...
        apollo_ensure!(ac_l.groups.len() == ac_r.groups.len(),
