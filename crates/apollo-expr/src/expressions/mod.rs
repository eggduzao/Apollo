...
use apollo_io::predicates::PhysicalIoExpr;
...
impl AggState {
    fn try_map<F>(&self, func: F) -> ApolloResult<Self>
    where
        F: FnOnce(&Column) -> ApolloResult<Column>,
    {
        Ok(match self {
...
pub trait PhysicalExpr: Send + Sync {
    ...
    fn evaluate(&self, df: &DataFrame, _state: &ExecutionState) -> ApolloResult<Column>;
...
/// Wrapper struct that allow us to use a PhysicalExpr in apollo-io.
///
...
impl PhysicalIoExpr for PhysicalIoHelper {
    fn evaluate_io(&self, df: &DataFrame) -> ApolloResult<Series> {
