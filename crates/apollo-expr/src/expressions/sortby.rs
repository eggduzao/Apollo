...
use apollo_core::chunked_array::from_iterator_par::ChunkedCollectParIterExt;
...
fn check_groups(a: &GroupsType, b: &GroupsType) -> ApolloResult<()> {
    apollo_ensure!(a.iter().zip(b.iter()).all(|(a, b)| {
        a.len() == b.len()
...
fn sort_by_groups_no_match_single<'a>(
    ...
    let ca: ApolloResult<ListChunked> = POOL.install(|| {
        s_in.par_iter_indexed()
            .zip(s_by.par_iter_indexed())
            .map(|(opt_s, s_sort_by)| match (opt_s, s_sort_by) {
                (Some(s), Some(s_sort_by)) => {
                    apollo_ensure!(s.len() == s_sort_by.len(), ComputeError: "series lengths don't match in 'sort_by' expression");
...
fn sort_by_groups_multiple_by(
    ...
    let first = new_idx
        .first()
        .ok_or_else(|| apollo_err!(ComputeError: "{}", ERR_MSG))?;
