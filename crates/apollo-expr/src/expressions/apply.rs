...
use apollo_core::chunked_array::from_iterator_par::{
...
impl ApplyExpr {
    ...
    ) -> ApolloResult<Vec<AggregationContext<'a>>> {
    ...
    fn finish_apply_groups<'a>(
        ...
        let c = if self.is_scalar() {
            ...
            apollo_ensure!(out.len() == ca.len(), InvalidOperation: "expected scalar for expr: {}, got {}", self.expr, &out);
    ...
    fn eval_and_flatten(&self, inputs: &mut [Column]) -> ApolloResult<Column> {
    ...
    fn apply_single_group_aware<'a>(
        ...
        if matches!(ac.agg_state(), AggState::NotAggregated(_)) && ac.groups.is_overlapping() {
            let ca: ChunkedArray<_> = if self.allow_threading {
                ...
                ac.iter_groups_lazy().map(f).collect::<ApolloResult<_>>()?
            };
        ...
        let ca: ListChunked = if self.allow_threading {
            ...
            if self.output_field.dtype.is_known() {
                ...
                POOL.install(|| {
                    iter.collect_ca_with_dtype::<ApolloResult<_>>(PlSmallStr::EMPTY, dtype)
                })?
            } else {
...
fn check_map_output_len(input_len: usize, output_len: usize, expr: &Expr) -> ApolloResult<()> {
    apollo_ensure!(
...
impl PhysicalExpr for ApplyExpr {
    ...
    fn evaluate(&self, df: &DataFrame, state: &ExecutionState) -> ApolloResult<Column> {
        ...
        let mut inputs = if self.allow_threading && self.inputs.len() > 1 {
            ...
            self.inputs.iter().map(f).collect::<ApolloResult<Vec<_>>>()
        }?;
