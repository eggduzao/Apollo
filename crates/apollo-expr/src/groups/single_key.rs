...
#[derive(Default)]
pub struct SingleKeyHashGrouper<T: ApolloDataType> {
    idx_map: TotalIndexMap<T::Physical<'static>, ()>,
    null_idx: IdxSize,
}

impl<K, T: ApolloDataType> SingleKeyHashGrouper<T>
where
...
impl<K, T: ApolloDataType> Grouper for SingleKeyHashGrouper<T>
where
    for<'a> T: ApolloDataType<Physical<'a> = K>,
