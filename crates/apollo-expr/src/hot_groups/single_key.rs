...
use apollo_utils::total_ord::{BuildHasherTotalExt, TotalEq, TotalHash};
...
pub struct SingleKeyHashHotGrouper<T: ApolloDataType> {
    dtype: DataType,
...
impl<K, T: ApolloDataType> SingleKeyHashHotGrouper<T>
where
...
where
    for<'a> T: ApolloPhysicalType<Physical<'a> = K>,
