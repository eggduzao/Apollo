...
use apollo_plan::constants::{get_literal_name, get_pl_element_name, get_pl_structfields_name};
...
pub fn get_expr_depth_limit() -> ApolloResult<u16> {
    let depth = if let Ok(d) = std::env::var("APOLLO_MAX_EXPR_DEPTH") {
        let v = d
            .parse::<u64>()
            .map_err(|_| apollo_err!(ComputeError: "could not parse 'max_expr_depth': {}", d))?;
...
fn ok_checker(_i: usize, _state: &ExpressionConversionState) -> ApolloResult<()> {
    Ok(())
}
...
) -> ApolloResult<Vec<Arc<dyn PhysicalExpr>>> {
...
where
    F: Fn(usize, &ExpressionConversionState) -> ApolloResult<()>,
{
    exprs
        .iter()
        .enumerate()
...
pub fn create_physical_expr(
    ...
    state: &mut ExpressionConversionState,
) -> ApolloResult<Arc<dyn PhysicalExpr>> {
    ...
    match aexpr.clone() {
        ...
        Explode { expr, options } => {
            ...
            let function = SpecialEq::new(Arc::new(
                move |c: &mut [apollo_core::frame::column::Column]| c[0].explode(options),
