...
use apollo_core::frame::row::AnyValueBufferTrusted;
...
trait Policy: Copy + Send + Sync + 'static {
    ...
    fn check_count(_count: Self::Count, _allow_empty: bool) -> ApolloResult<()> {
        Ok(())
    }
...
impl Policy for Item {
    ...
    fn check_count(count: Self::Count, allow_empty: bool) -> ApolloResult<()> {
        apollo_ensure!(
