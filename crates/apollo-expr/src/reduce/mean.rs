...
pub fn new_mean_reduction(dtype: DataType) -> ApolloResult<Box<dyn GroupedReduction>> {
    ...
    Ok(match dtype {
        ...
        _ => apollo_bail!(InvalidOperation: "`mean` operation not supported for dtype `{dtype}`"),
    })
}
