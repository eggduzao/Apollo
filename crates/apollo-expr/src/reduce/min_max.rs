...
use apollo_ops::prelude::nan_propagating_aggregate::ca_nan_agg;
...
pub fn new_min_reduction(
    ...
    Ok(match &dtype {
        ...
        _ if dtype.is_integer() || dtype.is_temporal() || dtype.is_enum() => {
            with_match_physical_integer_apollo_type!(dtype.to_physical(), |$T| {
        ...
        _ => apollo_bail!(InvalidOperation: "`min` operation not supported for dtype `{dtype}`"),
    })
}
...
pub fn new_max_reduction(
    ...
    Ok(match &dtype {
        ...
        _ => apollo_bail!(InvalidOperation: "`max` operation not supported for dtype `{dtype}`"),
    })
}
...
#[cfg(feature = "propagate_nans")]
impl<T: ApolloFloatType> NumericReduction for NanMin<T> {
    type Dtype = T;
...
#[cfg(feature = "propagate_nans")]
impl<T: ApolloFloatType> NumericReduction for NanMax<T> {
    type Dtype = T;
...
#[cfg(feature = "dtype-categorical")]
impl<T: ApolloCategoricalType> Reducer for CatMinReducer<T> {
    ...
    fn reduce_ca(&self, v: &mut Self::Value, ca: &ChunkedArray<T::ApolloPhysical>, _seq_id: u64) {
...
#[cfg(feature = "dtype-categorical")]
impl<T: ApolloCategoricalType> Reducer for CatMaxReducer<T> {
    type Dtype = T::ApolloPhysical;
    type Value = T::Native;
