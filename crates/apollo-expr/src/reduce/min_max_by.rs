...
use apollo_core::chunked_array::arg_min_max::{
...
pub fn new_min_by_reduction(
    dtype: DataType,
    by_dtype: DataType,
) -> ApolloResult<Box<dyn GroupedReduction>> {
    ...
    Ok(match &by_dtype {
        ...
        _ if by_dtype.is_integer() || by_dtype.is_temporal() || by_dtype.is_enum() => {
            with_match_physical_integer_apollo_type!(by_dtype.to_physical(), |$T| {
        ...
        _ => {
            apollo_bail!(InvalidOperation: "`min_by` operation not supported for by dtype `{by_dtype}`")
        },
    })
}
...
pub fn new_max_by_reduction(
    ...
    Ok(match &by_dtype {
        ...
        _ => {
            apollo_bail!(InvalidOperation: "`max_by` operation not supported for by dtype `{by_dtype}`")
        },
    })
}
...
trait SelectReducer: Clone + Send + Sync + 'static {
    ...
    fn select_one(
        &self,
        a: &mut Self::Value,
        b: <Self::Dtype as ApolloDataType>::Physical<'_>,
    ) -> bool;
...
#[cfg(feature = "dtype-categorical")]
impl<T: ApolloCategoricalType> SelectReducer for CatMinSelector<T> {
    type Dtype = T::ApolloPhysical;
    type Value = T::Native;
    ...
    fn select_ca(&self, v: &mut Self::Value, ca: &ChunkedArray<Self::Dtype>) -> Option<usize> {
        use apollo_core::chunked_array::arg_min_max::arg_min_opt_iter;
...
#[cfg(feature = "dtype-categorical")]
impl<T: ApolloCategoricalType> SelectReducer for CatMaxSelector<T> {
    type Dtype = T::ApolloPhysical;
    type Value = T::Native;
    ...
    fn select_ca(&self, v: &mut Self::Value, ca: &ChunkedArray<Self::Dtype>) -> Option<usize> {
        use apollo_core::chunked_array::arg_min_max::arg_max_opt_iter;
    ...
    R: SelectReducer,
{
    ...
    fn finalize(&mut self) -> ApolloResult<Series> {
