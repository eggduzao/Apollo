...
pub fn new_approx_n_unique_reduction(dtype: DataType) -> ApolloResult<Box<dyn GroupedReduction>> {
    ...
    Ok(match dtype {
        ...
        _ if dtype.is_primitive_numeric() || dtype.is_temporal() => {
            with_match_physical_numeric_apollo_type!(dtype.to_physical(), |$T| {
        ...
        _ => {
            apollo_bail!(InvalidOperation: "`approx_n_unique` operation not supported for dtype `{dtype}`")
        },
    })
}
