...
pub trait Reducer: Send + Sync + Clone + 'static {
    ...
    fn reduce_one(
        &self,
        a: &mut Self::Value,
        b: Option<<Self::Dtype as ApolloDataType>::Physical<'_>>,
        seq_id: u64,
    );
...
pub trait NumericReduction: Send + Sync + 'static {
    type Dtype: ApolloNumericType;
    fn init() -> <Self::Dtype as ApolloNumericType>::Native;
    fn combine(
        a: <Self::Dtype as ApolloNumericType>::Native,
        b: <Self::Dtype as ApolloNumericType>::Native,
    ) -> <Self::Dtype as ApolloNumericType>::Native;
    fn reduce_ca(
        ca: &ChunkedArray<Self::Dtype>,
    ) -> Option<<Self::Dtype as ApolloNumericType>::Native>;
}
...
impl<R: NumericReduction> Reducer for NumReducer<R> {
    ...
    type Value = <<R as NumericReduction>::Dtype as ApolloNumericType>::Native;
