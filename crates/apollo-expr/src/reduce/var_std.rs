...
use apollo_compute::moment::VarState;
...
pub fn new_var_std_reduction(
    ...
    Ok(match dtype {
        ...
        _ => {
            apollo_bail!(InvalidOperation: "`{op_name}` operation not supported for dtype `{dtype}`")
        },
    })
}
...
impl<T: ApolloNumericType> Reducer for VarStdReducer<T> {
    type Dtype = T;
    type Value = VarState;
    ...
    fn reduce_ca(&self, v: &mut Self::Value, ca: &ChunkedArray<Self::Dtype>, _seq_id: u64) {
        for arr in ca.downcast_iter() {
            v.combine(&apollo_compute::moment::var(arr))
        }
    }
