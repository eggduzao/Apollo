...
use apollo_core::prelude::{Column, DataType, ExplodeOptions, IntoColumn, SortOptions};
use apollo_ops::prelude::array::ArrayNameSpace;
#[cfg(feature = "array_to_struct")]
use apollo_plan::dsl::DslNameGenerator;
use apollo_plan::dsl::{ColumnsUdf, SpecialEq};
use apollo_plan::plans::IRArrayFunction;
...
pub(super) fn length(s: &Column) -> ApolloResult<Column> {
    ...
    let width = IdxSize::try_from(width)
        .map_err(|_| apollo_err!(bigidx, ctx = "array length", size = width))?;
...
pub(super) fn to_list(s: &Column) -> ApolloResult<Column> {
    if let DataType::Array(inner, _) = s.dtype() {
        s.cast(&DataType::List(inner.clone()))
    } else {
        apollo_bail!(ComputeError: "expected array dtype")
    }
}

#[cfg(feature = "array_any_all")]
pub(super) fn any(s: &Column) -> ApolloResult<Column> {
    s.array()?.array_any().map(Column::from)
}

#[cfg(feature = "array_any_all")]
pub(super) fn all(s: &Column) -> ApolloResult<Column> {
...
pub(super) fn get(s: &[Column], null_on_oob: bool) -> ApolloResult<Column> {
    let ca = s[0].array()?;
...
#[cfg(feature = "is_in")]
pub(super) fn contains(s: &[Column], nulls_equal: bool) -> ApolloResult<Column> {
    let array = &s[0];
    let item = &s[1];
    apollo_ensure!(matches!(array.dtype(), DataType::Array(_, _)),
        SchemaMismatch: "invalid series dtype: expected `Array`, got `{}`", array.dtype(),
    );
    let mut ca = apollo_ops::series::is_in(
...
pub(super) fn slice(s: &Column, offset: i64, length: i64) -> ApolloResult<Column> {
    let ca = s.array()?;
    ca.array_slice(offset, length).map(Column::from)
}

fn explode(c: &[Column], options: ExplodeOptions) -> ApolloResult<Column> {
    c[0].explode(options)
}

fn concat_arr(args: &[Column]) -> ApolloResult<Column> {
    let dtype = concat_arr_output_dtype(&mut args.iter().map(|c| (c.name().as_str(), c.dtype())))?;

    apollo_ops::series::concat_arr::concat_arr(args, &dtype)
}
...
#[cfg(feature = "array_to_struct")]
fn arr_to_struct(s: &Column, name_generator: Option<DslNameGenerator>) -> ApolloResult<Column> {
    use apollo_ops::prelude::array::ToStruct;
