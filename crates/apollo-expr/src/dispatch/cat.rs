...
use apollo_core::prelude::{
    ChunkTakeUnchecked, ChunkedArray, Column, FalseT, IDX_DTYPE, IntoColumn, ApolloPhysicalType,
    StringChunked,
};
use apollo_core::series::Series;
use apollo_ops::prelude::BinaryNameSpaceImpl;
#[cfg(feature = "strings")]
use apollo_ops::prelude::StringNameSpaceImpl;
use apollo_plan::dsl::{ColumnsUdf, SpecialEq};
use apollo_plan::plans::IRCategoricalFunction;
...
fn get_categories(s: &Column) -> ApolloResult<Column> {
...
fn apply_to_cats<F, T>(c: &Column, mut op: F) -> ApolloResult<Column>
where
    F: FnMut(StringChunked) -> ChunkedArray<T>,
    T: ApolloPhysicalType<HasViews = FalseT, IsStruct = FalseT, IsNested = FalseT>,
{
...
#[cfg(feature = "strings")]
fn len_bytes(c: &Column) -> ApolloResult<Column> {
    apply_to_cats(c, |s| s.str_len_bytes())
}

#[cfg(feature = "strings")]
fn len_chars(c: &Column) -> ApolloResult<Column> {
    apply_to_cats(c, |s| s.str_len_chars())
}

#[cfg(feature = "strings")]
fn starts_with(c: &Column, prefix: &str) -> ApolloResult<Column> {
    apply_to_cats(c, |s| s.as_binary().starts_with(prefix.as_bytes()))
}

#[cfg(feature = "strings")]
fn ends_with(c: &Column, suffix: &str) -> ApolloResult<Column> {
    apply_to_cats(c, |s| s.as_binary().ends_with(suffix.as_bytes()))
}

#[cfg(feature = "strings")]
fn slice(c: &Column, offset: i64, length: Option<usize>) -> ApolloResult<Column> {
    ...
    let result = unsafe {
        categories.apply_views(|view, val| {
            let (start, end) =
                apollo_ops::prelude::substring_ternary_offsets_value(val, offset, length);
            apollo_ops::prelude::update_view(view, start, end, val)
        })
    };
    // SAFETY: physical idx array is valid.
    let out = unsafe { result.take_unchecked(phys.idx().unwrap()) };
