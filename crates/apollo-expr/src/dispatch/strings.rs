...
use apollo_core::utils::{CustomIterTools, handle_casting_failures};
#[cfg(feature = "regex")]
use apollo_ops::chunked_array::strings::split_regex_helper;
use apollo_ops::prelude::{BinaryNameSpaceImpl, StringNameSpaceImpl};
#[cfg(feature = "temporal")]
use apollo_plan::dsl::StrptimeOptions;
use apollo_plan::dsl::{ColumnsUdf, SpecialEq};
use apollo_plan::plans::IRStringFunction;
use apollo_time::prelude::StringMethods;
#[cfg(feature = "regex")]
...
#[cfg(feature = "find_many")]
fn contains_any(s: &[Column], ascii_case_insensitive: bool) -> ApolloResult<Column> {
    let ca = s[0].str()?;
    let patterns = s[1].list()?;
    apollo_ops::chunked_array::strings::contains_any(ca, patterns, ascii_case_insensitive)
...
fn replace_many(
    ...
    apollo_ops::chunked_array::strings::replace_all(
        ca,
        patterns,
        replace_with,
        ascii_case_insensitive,
...
fn extract_many(
    ...
    apollo_ops::chunked_array::strings::extract_many(
        ca,
        patterns,
        ascii_case_insensitive,
        overlapping,
...
fn find_many(
    ...
    apollo_ops::chunked_array::strings::find_many(
        ca,
        patterns,
        ascii_case_insensitive,
        overlapping,
...
fn uppercase(s: &Column) -> ApolloResult<Column> {
    let ca = s.str()?;
    Ok(ca.to_uppercase().into_column())
}

fn lowercase(s: &Column) -> ApolloResult<Column> {
    let ca = s.str()?;
    Ok(ca.to_lowercase().into_column())
}

#[cfg(feature = "nightly")]
pub(super) fn titlecase(s: &Column) -> ApolloResult<Column> {
    let ca = s.str()?;
    Ok(ca.to_titlecase().into_column())
}

pub(super) fn len_chars(s: &Column) -> ApolloResult<Column> {
    let ca = s.str()?;
    Ok(ca.str_len_chars().into_column())
}

pub(super) fn len_bytes(s: &Column) -> ApolloResult<Column> {
    let ca = s.str()?;
    Ok(ca.str_len_bytes().into_column())
}

#[cfg(feature = "regex")]
pub(super) fn contains(s: &[Column], literal: bool, strict: bool) -> ApolloResult<Column> {
...
#[cfg(feature = "regex")]
pub(super) fn find(s: &[Column], literal: bool, strict: bool) -> ApolloResult<Column> {
...
pub(super) fn ends_with(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn starts_with(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn extract(s: &[Column], group_index: usize) -> ApolloResult<Column> {
    let ca = s[0].str()?;
    let pat = s[1].str()?;
...
pub(super) fn extract_groups(s: &Column, pat: &str, dtype: &DataType) -> ApolloResult<Column> {
    let ca = s.str()?;
    ca.extract_groups(pat, dtype).map(Column::from)
}

#[cfg(feature = "string_pad")]
pub(super) fn pad_start(s: &[Column], fill_char: char) -> ApolloResult<Column> {
...
#[cfg(feature = "string_pad")]
pub(super) fn pad_end(s: &[Column], fill_char: char) -> ApolloResult<Column> {
    let s1 = s[0].as_materialized_series();
    let length = &s[1];
    apollo_ensure!(
        s1.len() == 1 || length.len() == 1 || s1.len() == length.len(),
...
#[cfg(feature = "string_pad")]
pub(super) fn zfill(s: &[Column]) -> ApolloResult<Column> {
    let s1 = s[0].as_materialized_series();
    let length = &s[1];
    apollo_ensure!(
        s1.len() == 1 || length.len() == 1 || s1.len() == length.len(),
...
pub(super) fn strip_chars(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn strip_chars_start(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn strip_chars_end(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn strip_prefix(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn strip_suffix(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn extract_all(args: &[Column]) -> ApolloResult<Column> {
    let s = &args[0];
    let pat = &args[1];
...
pub(super) fn count_matches(args: &[Column], literal: bool) -> ApolloResult<Column> {
    let s = &args[0];
    let pat = &args[1];
...
pub(super) fn strptime(
    ...
    match dtype {
        ...
        dt => apollo_bail!(ComputeError: "not implemented for dtype {}", dt),
    }
}

#[cfg(feature = "dtype-struct")]
pub(super) fn split_exact(s: &[Column], n: usize, inclusive: bool) -> ApolloResult<Column> {
    let ca = s[0].str()?;
    let by = s[1].str()?;
...
#[cfg(feature = "dtype-struct")]
pub(super) fn splitn(s: &[Column], n: usize) -> ApolloResult<Column> {
    let ca = s[0].str()?;
    let by = s[1].str()?;
...
pub(super) fn split(s: &[Column], inclusive: bool) -> ApolloResult<Column> {
    let ca = s[0].str()?;
    let by = s[1].str()?;
...
#[cfg(feature = "regex")]
pub(super) fn split_regex(s: &[Column], inclusive: bool, strict: bool) -> ApolloResult<Column> {
    let ca = s[0].str()?;
    let by = s[1].str()?;
...
#[cfg(feature = "dtype-date")]
fn to_date(s: &Column, options: &StrptimeOptions) -> ApolloResult<Column> {
    let ca = s.str()?;
    let out = {
        if options.exact {
...
fn to_datetime(
    ...
    let tz_aware = match &options.format {
        ...
        Some(format) => apollo_plan::plans::TZ_AWARE_RE.is_match(format),
        _ => false,
    };
...
#[cfg(feature = "dtype-time")]
fn to_time(s: &Column, options: &StrptimeOptions) -> ApolloResult<Column> {
    apollo_ensure!(
...
#[cfg(feature = "concat_str")]
pub(super) fn join(s: &Column, delimiter: &str, ignore_nulls: bool) -> ApolloResult<Column> {
    let str_s = s.cast(&DataType::String)?;
    let joined = apollo_ops::chunked_array::str_join(str_s.str()?, delimiter, ignore_nulls);
...
pub(super) fn concat_hor(
    ...
    Ok(apollo_ops::chunked_array::hor_str_concat(&cas, delimiter, ignore_nulls)?.into_column())
}

#[cfg(feature = "regex")]
fn get_pat(pat: &StringChunked) -> ApolloResult<&str> {
    pat.get(0).ok_or_else(
        || apollo_err!(ComputeError: "pattern cannot be 'null' in 'replace' expression"),
    )
}
    ...
    match (pat.len(), val.len()) {
        (1, 1) => {
            ...
            match literal {
                ...
                false => {
                    if n > 1 {
                        apollo_bail!(ComputeError: "regex replacement with 'n > 1' not yet supported")
                    }
                    ca.replace(pat, val)
                },
            }
        ...
        (1, len_val) => {
            if n > 1 {
                apollo_bail!(ComputeError: "multivalue replacement with 'n > 1' not yet supported")
            }
            ...
            let reg = apollo_utils::regex_cache::compile_regex(&pat)?;
    ...
    match (pat.len(), val.len()) {
        (1, 1) => {
            ...
            let val = val.get(0).ok_or_else(
                || apollo_err!(ComputeError: "value cannot be 'null' in 'replace' expression"),
            )?;
            let literal = literal || is_literal_pat(pat);
...
pub(super) fn format(s: &mut [Column], format: &str, insertions: &[usize]) -> ApolloResult<Column> {
    apollo_ops::series::str_format(s, format, insertions)
}

#[cfg(feature = "regex")]
pub(super) fn replace(s: &[Column], literal: bool, n: i64) -> ApolloResult<Column> {
    let column = &s[0];
    let pat = &s[1];
    let val = &s[2];
    let all = n < 0;
...
pub(super) fn normalize(
    s: &Column,
    form: apollo_ops::prelude::UnicodeForm,
) -> ApolloResult<Column> {
    let ca = s.str()?;
...
fn _check_same_length(s: &[Column], fn_name: &str) -> Result<(), ApolloError> {
    apollo_ensure!(
        _ensure_lengths(s),
...
pub(super) fn str_slice(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn str_head(s: &[Column]) -> ApolloResult<Column> {
...
pub(super) fn str_tail(s: &[Column]) -> ApolloResult<Column> {
...
#[cfg(feature = "string_encoding")]
pub(super) fn hex_encode(s: &Column) -> ApolloResult<Column> {
    Ok(s.str()?.hex_encode().into_column())
}

#[cfg(feature = "binary_encoding")]
pub(super) fn hex_decode(s: &Column, strict: bool) -> ApolloResult<Column> {
    s.str()?.hex_decode(strict).map(|ca| ca.into_column())
}

#[cfg(feature = "string_encoding")]
pub(super) fn base64_encode(s: &Column) -> ApolloResult<Column> {
    Ok(s.str()?.base64_encode().into_column())
}

#[cfg(feature = "binary_encoding")]
pub(super) fn base64_decode(s: &Column, strict: bool) -> ApolloResult<Column> {
    s.str()?.base64_decode(strict).map(|ca| ca.into_column())
}

#[cfg(feature = "dtype-decimal")]
pub(super) fn to_decimal(s: &Column, scale: usize) -> ApolloResult<Column> {
    let ca = s.str()?;
    ca.to_decimal(apollo_compute::decimal::DEC128_MAX_PREC, scale)
        .map(Column::from)
}

#[cfg(feature = "extract_jsonpath")]
pub(super) fn json_decode(s: &Column, dtype: DataType) -> ApolloResult<Column> {
    use apollo_ops::prelude::Utf8JsonPathImpl;
...
#[cfg(feature = "extract_jsonpath")]
pub(super) fn json_path_match(s: &[Column]) -> ApolloResult<Column> {
    use apollo_ops::prelude::Utf8JsonPathImpl;
...
#[cfg(feature = "regex")]
pub(super) fn escape_regex(s: &Column) -> ApolloResult<Column> {
    let ca = s.str()?;
