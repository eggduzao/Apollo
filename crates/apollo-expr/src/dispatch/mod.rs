...
use apollo_core::prelude::{Column, GroupPositions};
use apollo_plan::dsl::{ColumnsUdf, SpecialEq};
use apollo_plan::plans::{IRBooleanFunction, IRFunctionExpr, IRPowFunction};
...
macro_rules! wrap {
    ...
    ($e:expr, $($args:expr),*) => {{
        let f = move |s: &mut [::apollo_core::prelude::Column]| {
            $e(s, $($args),*)
        };
...
macro_rules! map_owned {
    ($func:path) => {{
        let f = move |c: &mut [::apollo_core::prelude::Column]| {
...
pub fn function_expr_to_udf(func: IRFunctionExpr) -> SpecialEq<Arc<dyn ColumnsUdf>> {
    ...
    match func {
        ...
        F::RollingExpr { function, options } => {
            ...
            use apollo_plan::plans::IRRollingFunction;
            match function {
        ...
        F::RollingExprBy {
            ...
            use apollo_plan::plans::IRRollingFunctionBy;
            match function_by {
        ...
        #[cfg(feature = "top_k")]
        F::TopK { descending } => {
            map_as_slice!(apollo_ops::prelude::top_k, descending)
        },
        #[cfg(feature = "top_k")]
        F::TopKBy { descending } => {
            map_as_slice!(apollo_ops::prelude::top_k_by, descending.clone())
        },
        ...
        #[cfg(feature = "rle")]
        F::RLE => map!(apollo_ops::series::rle),
        #[cfg(feature = "rle")]
        F::RLEID => map!(apollo_ops::series::rle_id),
        F::ToPhysical => map!(misc::to_physical),
        #[cfg(feature = "random")]
        F::Random { method, seed } => {
            use IRRandomMethod::*;
            use apollo_plan::plans::IRRandomMethod;
            match method {
        ...
        F::FfiPlugin {
            ...
            map_as_slice!(
                apollo_plan::plans::plugin::call_plugin,
                lib.as_ref(),
                symbol.as_ref(),
                kwargs.as_ref()
            )
...
pub fn function_expr_to_groups_udf(func: &IRFunctionExpr) -> Option<SpecialEq<Arc<dyn GroupsUdf>>> {
    ...
    Some(match func {
        ...
        #[cfg(feature = "bitwise")]
        F::Bitwise(f) => {
            use apollo_plan::plans::IRBitwiseFunction as B;
            match f {
        ...
        F::FillNullWithStrategy(apollo_core::prelude::FillNullStrategy::Forward(limit)) => {
            wrap_groups!(groups_dispatch::forward_fill_null, (*limit, v: Option<IdxSize>))
        },
        F::FillNullWithStrategy(apollo_core::prelude::FillNullStrategy::Backward(limit)) => {
