use apollo_core::error::{ApolloResult, apollo_bail, apollo_ensure, apollo_err};
use apollo_core::prelude::row_encode::{_get_rows_encoded_ca, _get_rows_encoded_ca_unordered};
use apollo_core::prelude::*;
use apollo_core::scalar::Scalar;
use apollo_core::series::ops::NullBehavior;
use apollo_core::series::{IsSorted, Series};
use apollo_core::utils::try_get_supertype;
#[cfg(feature = "interpolate")]
use apollo_ops::series::InterpolationMethod;
#[cfg(feature = "rank")]
use apollo_ops::series::RankOptions;
use apollo_ops::series::{ArgAgg, NullStrategy, SeriesMethods};
#[cfg(feature = "dtype-array")]
use apollo_plan::dsl::ReshapeDimension;
#[cfg(feature = "fused")]
use apollo_plan::plans::FusedOperator;
#[cfg(feature = "cov")]
use apollo_plan::plans::IRCorrelationMethod;
use apollo_plan::plans::RowEncodingVariant;
use apollo_row::RowEncodingOptions;
use apollo_utils::IdxSize;
use apollo_utils::pl_str::PlSmallStr;

#[cfg(feature = "abs")]
pub(super) fn abs(s: &Column) -> ApolloResult<Column> {
    apollo_ops::prelude::abs(s.as_materialized_series()).map(Column::from)
}

pub(super) fn reverse(s: &Column) -> ApolloResult<Column> {
    Ok(s.reverse())
}

#[cfg(feature = "approx_unique")]
pub(super) fn approx_n_unique(s: &Column) -> ApolloResult<Column> {
    s.approx_n_unique()
        .map(|v| Column::new_scalar(s.name().clone(), Scalar::new(IDX_DTYPE, v.into()), 1))
}

#[cfg(feature = "diff")]
pub(super) fn diff(s: &[Column], null_behavior: NullBehavior) -> ApolloResult<Column> {
    ...
    match n.i64()?.get(0) {
        Some(n) => apollo_ops::prelude::diff(s1, n, null_behavior).map(Column::from),
        None => apollo_bail!(ComputeError: "'n' can not be None for diff"),
    }
}

#[cfg(feature = "pct_change")]
pub(super) fn pct_change(s: &[Column]) -> ApolloResult<Column> {
    apollo_ops::prelude::pct_change(s[0].as_materialized_series(), s[1].as_materialized_series())
        .map(Column::from)
}

#[cfg(feature = "interpolate")]
pub(super) fn interpolate(s: &Column, method: InterpolationMethod) -> ApolloResult<Column> {
    Ok(apollo_ops::prelude::interpolate(s.as_materialized_series(), method).into())
}

#[cfg(feature = "interpolate_by")]
pub(super) fn interpolate_by(s: &[Column]) -> ApolloResult<Column> {
    use apollo_ops::series::SeriesMethods;

    let by = &s[1];
    let by_is_sorted = by.as_materialized_series().is_sorted(Default::default())?;
    apollo_ops::prelude::interpolate_by(&s[0], by, by_is_sorted)
}

pub(super) fn to_physical(s: &Column) -> ApolloResult<Column> {
    Ok(s.to_physical_repr())
}

pub(super) fn set_sorted_flag(s: &Column, sorted: IsSorted) -> ApolloResult<Column> {
    let mut s = s.clone();
...
pub(super) fn replace_time_zone(
    ...
    let s2 = &s[1].str()?;
    Ok(apollo_ops::prelude::replace_time_zone(ca, time_zone, s2, non_existent)?.into_column())
}
...
pub(super) fn unique_counts(s: &Column) -> ApolloResult<Column> {
    apollo_ops::prelude::unique_counts(s.as_materialized_series()).map(Column::from)
}

#[cfg(feature = "dtype-array")]
pub(super) fn reshape(c: &Column, dimensions: &[ReshapeDimension]) -> ApolloResult<Column> {
    c.reshape_array(dimensions)
}

#[cfg(feature = "repeat_by")]
pub(super) fn repeat_by(s: &[Column]) -> ApolloResult<Column> {
    let by = &s[1];
    let s = &s[0];
    let by = by.strict_cast(&IDX_DTYPE)?;
    apollo_ops::chunked_array::repeat_by(s.as_materialized_series(), by.idx()?)
        .map(|ok| ok.into_column())
}

pub(super) fn max_horizontal(s: &mut [Column]) -> ApolloResult<Column> {
    apollo_ops::prelude::max_horizontal(s).map(Option::unwrap)
}

pub(super) fn min_horizontal(s: &mut [Column]) -> ApolloResult<Column> {
    apollo_ops::prelude::min_horizontal(s).map(Option::unwrap)
}

pub(super) fn sum_horizontal(s: &mut [Column], ignore_nulls: bool) -> ApolloResult<Column> {
    let null_strategy = if ignore_nulls {
        NullStrategy::Ignore
    } else {
        NullStrategy::Propagate
    };
    apollo_ops::prelude::sum_horizontal(s, null_strategy).map(Option::unwrap)
}

pub(super) fn mean_horizontal(s: &mut [Column], ignore_nulls: bool) -> ApolloResult<Column> {
    let null_strategy = if ignore_nulls {
        NullStrategy::Ignore
    } else {
        NullStrategy::Propagate
    };
    apollo_ops::prelude::mean_horizontal(s, null_strategy).map(Option::unwrap)
}

pub(super) fn drop_nulls(s: &Column) -> ApolloResult<Column> {
    Ok(s.drop_nulls())
}

pub fn rechunk(s: &Column) -> ApolloResult<Column> {
    Ok(s.rechunk())
}

pub fn append(s: &[Column], upcast: bool) -> ApolloResult<Column> {
    assert_eq!(s.len(), 2);
...
#[cfg(feature = "mode")]
pub(super) fn mode(s: &Column, maintain_order: bool) -> ApolloResult<Column> {
    apollo_ops::prelude::mode::mode(s.as_materialized_series(), maintain_order).map(Column::from)
}

#[cfg(feature = "moment")]
pub(super) fn skew(s: &Column, bias: bool) -> ApolloResult<Column> {
    // @scalar-opt

    use apollo_ops::series::MomentSeries;
    s.as_materialized_series()
        .skew(bias)
...
#[cfg(feature = "moment")]
pub(super) fn kurtosis(s: &Column, fisher: bool, bias: bool) -> ApolloResult<Column> {
    // @scalar-opt
...
pub(super) fn arg_unique(s: &Column) -> ApolloResult<Column> {
    // @scalar-opt
    s.as_materialized_series()
        .arg_unique()
        .map(|ok| ok.into_column())
...
pub(super) fn arg_min(s: &Column) -> ApolloResult<Column> {
    // @scalar-opt
...
pub(super) fn arg_max(s: &Column) -> ApolloResult<Column> {
    // @scalar-opt
    Ok(s.as_materialized_series()
        .arg_max()
        .map_or(Scalar::null(IDX_DTYPE), |v| {
...
pub(super) fn arg_sort(s: &Column, descending: bool, nulls_last: bool) -> ApolloResult<Column> {
    // @scalar-opt
    Ok(s.as_materialized_series()
...
pub(super) fn min_by(s: &[Column]) -> ApolloResult<Column> {
    assert!(s.len() == 2);
    let input = &s[0];
    let by = &s[1];
    if input.len() != by.len() {
        apollo_bail!(ShapeMismatch: "'by' column in `min_by` operation has incorrect length (got {}, expected {})", by.len(), input.len());
    }
...
pub(super) fn max_by(s: &[Column]) -> ApolloResult<Column> {
    assert!(s.len() == 2);
    let input = &s[0];
    let by = &s[1];
    if input.len() != by.len() {
        apollo_bail!(ShapeMismatch: "'by' column in `max_by` operation has incorrect length (got {}, expected {})", by.len(), input.len());
    }
...
pub(super) fn product(s: &Column) -> ApolloResult<Column> {
    // @scalar-opt
    s.as_materialized_series()
        .product()
...
#[cfg(feature = "rank")]
pub(super) fn rank(s: &Column, options: RankOptions, seed: Option<u64>) -> ApolloResult<Column> {
    use apollo_ops::series::SeriesRank;
...
pub(super) fn hist(
    ...
    apollo_ops::prelude::hist_series(
        s,
        bin_count,
...
#[cfg(feature = "replace")]
pub(super) fn replace(s: &[Column]) -> ApolloResult<Column> {
    apollo_ops::series::replace(s[0].as_materialized_series(), s[1].list()?, s[2].list()?)
        .map(Column::from)
}

#[cfg(feature = "replace")]
pub(super) fn replace_strict(s: &[Column], return_dtype: Option<DataType>) -> ApolloResult<Column> {
    match s.get(3) {
        Some(default) => apollo_ops::series::replace_or_default(
        ...
        None => apollo_ops::series::replace_strict(
            s[0].as_materialized_series(),
            s[1].list()?,
            s[2].list()?,
            return_dtype,
...
pub(super) fn gather_every(s: &Column, n: usize, offset: usize) -> ApolloResult<Column> {
    s.gather_every(n, offset)
}

#[cfg(feature = "reinterpret")]
pub(super) fn reinterpret(s: &Column, signed: bool) -> ApolloResult<Column> {
    apollo_ops::series::reinterpret(s.as_materialized_series(), signed).map(Column::from)
}

pub(super) fn negate(s: &Column) -> ApolloResult<Column> {
    apollo_ops::series::negate(s.as_materialized_series()).map(Column::from)
}

pub(super) fn extend_constant(s: &[Column]) -> ApolloResult<Column> {
    let value = &s[1];
    let n = &s[2];
    apollo_ensure!(value.len() == 1 && n.len() == 1, ComputeError: "value and n should have unit length.");
    ...
    match n.u64()?.get(0) {
        ...
        None => {
            apollo_bail!(ComputeError: "n can not be None for extend_constant.")
        },
    }
}

#[cfg(feature = "row_hash")]
pub(super) fn row_hash(c: &Column, k0: u64, k1: u64, k2: u64, k3: u64) -> ApolloResult<Column> {
...
pub(super) fn arg_where(s: &mut [Column]) -> ApolloResult<Column> {
    use apollo_core::utils::arrow::bitmap::utils::SlicesIterator;
...
pub(super) fn index_of(s: &mut [Column]) -> ApolloResult<Column> {
    use apollo_core::series::IsSorted;
    use apollo_ops::series::index_of as index_of_op;
    ...
    let result = match is_sorted_flag {
        ...
        IsSorted::Ascending | IsSorted::Descending if !needle.is_null() => {
            use apollo_ops::series::SearchSortedSide;

            apollo_ops::series::search_sorted(
                series,
...
pub(super) fn search_sorted_impl(
    s: &mut [Column],
    side: apollo_ops::series::SearchSortedSide,
    descending: bool,
) -> ApolloResult<Column> {
...
#[cfg(feature = "sign")]
pub(super) fn sign(s: &Column) -> ApolloResult<Column> {
    use num_traits::{One, Zero};
    use apollo_core::prelude::{ChunkedArray, ApolloNumericType};
    ...
    let dtype = s.dtype();
    use apollo_core::datatypes::*;
    match dtype {
        _ if dtype.is_primitive_numeric() => with_match_physical_numeric_apollo_type!(dtype, |$T| {
            let ca: &ChunkedArray<$T> = s.as_ref().as_ref();
            Ok(sign_impl(ca))
        }),
        DataType::Decimal(_, scale) => {
            use apollo_core::prelude::ChunkApply;

            let ca = s.decimal()?;
            let out = ca
                .physical()
                .apply_values(|x| apollo_compute::decimal::dec128_sign(x, *scale))
                .into_column();
            unsafe { out.from_physical_unchecked(dtype) }
        },
        _ => apollo_bail!(opq = sign, dtype),
    }
}

pub(super) fn fill_null(s: &[Column]) -> ApolloResult<Column> {
    match (s[0].len(), s[1].len()) {
        (a, b) if a == b || b == 1 => {
            ...
            // default branch
            fn default(series: Column, fill_value: Column) -> ApolloResult<Column> {
        ...
        (self_len, other_len) => apollo_bail!(length_mismatch = "fill_null", self_len, other_len),
    }
}

pub(super) fn coalesce(s: &mut [Column]) -> ApolloResult<Column> {
    apollo_ops::series::coalesce_columns(s)
}

pub(super) fn drop_nans(s: Column) -> ApolloResult<Column> {
    match s.dtype() {
...
pub(super) fn clip(s: &[Column], has_min: bool, has_max: bool) -> ApolloResult<Column> {
    match (has_min, has_max) {
        (true, true) => apollo_ops::series::clip(
            s[0].as_materialized_series(),
            s[1].as_materialized_series(),
            s[2].as_materialized_series(),
        ),
        (true, false) => apollo_ops::series::clip_min(
            s[0].as_materialized_series(),
            s[1].as_materialized_series(),
        ),
        (false, true) => apollo_ops::series::clip_max(
            s[0].as_materialized_series(),
            s[1].as_materialized_series(),
        ),
        _ => unreachable!(),
...
#[cfg(feature = "dtype-struct")]
pub fn as_struct(cols: &[Column]) -> ApolloResult<Column> {
    use apollo_core::prelude::StructChunked;

    let Some(fst) = cols.first() else {
        apollo_bail!(nyi = "turning no columns as_struct");
    };
...
#[cfg(feature = "log")]
pub(super) fn entropy(s: &Column, base: f64, normalize: bool) -> ApolloResult<Column> {
    use apollo_ops::series::LogSeries;
...
#[cfg(feature = "log")]
pub(super) fn log(columns: &[Column]) -> ApolloResult<Column> {
    use apollo_ops::series::LogSeries;
...
#[cfg(feature = "log")]
pub(super) fn log1p(s: &Column) -> ApolloResult<Column> {
    use apollo_ops::series::LogSeries;
...
#[cfg(feature = "log")]
pub(super) fn exp(s: &Column) -> ApolloResult<Column> {
    use apollo_ops::series::LogSeries;

    Ok(s.as_materialized_series().exp().into())
}

pub(super) fn unique(s: &Column, stable: bool) -> ApolloResult<Column> {
    if stable {
        s.unique_stable()
    } else {
        s.unique()
...
#[cfg(feature = "fused")]
pub(super) fn fused(input: &[Column], op: FusedOperator) -> ApolloResult<Column> {
    use apollo_plan::plans::FusedOperator;
    ...
    let s2 = &input[2];
    match op {
        FusedOperator::MultiplyAdd => Ok(apollo_ops::series::fma_columns(s0, s1, s2)),
        FusedOperator::SubMultiply => Ok(apollo_ops::series::fsm_columns(s0, s1, s2)),
        FusedOperator::MultiplySub => Ok(apollo_ops::series::fms_columns(s0, s1, s2)),
    }
}

pub(super) fn concat_expr(s: &[Column], rechunk: bool) -> ApolloResult<Column> {
...
#[cfg(feature = "cov")]
pub(super) fn corr(s: &[Column], method: IRCorrelationMethod) -> ApolloResult<Column> {
    use apollo_plan::plans::IRCorrelationMethod;

    fn covariance(s: &[Column], ddof: u8) -> ApolloResult<Column> {
        let a = &s[0];
        let b = &s[1];
        let name = PlSmallStr::from_static("cov");

        use apollo_ops::chunked_array::cov::cov;
    ...
    fn pearson_corr(s: &[Column]) -> ApolloResult<Column> {
        let a = &s[0];
        let b = &s[1];
        let name = PlSmallStr::from_static("pearson_corr");

        use apollo_ops::chunked_array::cov::pearson_corr;
        let ret = match a.dtype() {
            #[cfg(feature = "dtype-f16")]
            DataType::Float16 => {
                use num_traits::AsPrimitive;
    ...
    fn spearman_rank_corr(s: &[Column], propagate_nans: bool) -> ApolloResult<Column> {
        use apollo_core::utils::coalesce_nulls_columns;
        use apollo_ops::chunked_array::nan_propagating_aggregate::nan_max_s;
        use apollo_ops::series::{RankMethod, SeriesRank};
        let a = &s[0];
        let b = &s[1];
...
#[cfg(feature = "peaks")]
pub(super) fn peak_min(s: &Column) -> ApolloResult<Column> {
    apollo_ops::prelude::peaks::peak_min_max(s, &AnyValue::Int8(0), &AnyValue::Int8(0), false)
        .map(IntoColumn::into_column)
}

#[cfg(feature = "peaks")]
pub(super) fn peak_max(s: &Column) -> ApolloResult<Column> {
    apollo_ops::prelude::peaks::peak_min_max(s, &AnyValue::Int8(0), &AnyValue::Int8(0), true)
        .map(IntoColumn::into_column)
}
...
pub(super) fn ewm_mean(
    s: &Column,
    options: apollo_ops::series::EWMOptions,
) -> ApolloResult<Column> {
    apollo_ops::prelude::ewm_mean(s.as_materialized_series(), options).map(Column::from)
}

#[cfg(feature = "ewma")]
pub(super) fn ewm_std(s: &Column, options: apollo_ops::series::EWMOptions) -> ApolloResult<Column> {
    apollo_ops::prelude::ewm_std(s.as_materialized_series(), options).map(Column::from)
}

#[cfg(feature = "ewma")]
pub(super) fn ewm_var(s: &Column, options: apollo_ops::series::EWMOptions) -> ApolloResult<Column> {
    apollo_ops::prelude::ewm_var(s.as_materialized_series(), options).map(Column::from)
}

#[cfg(feature = "ewma_by")]
pub(super) fn ewm_mean_by(s: &[Column], half_life: apollo_time::Duration) -> ApolloResult<Column> {
    use apollo_ops::series::SeriesMethods;
    ...
    apollo_ensure!(!half_life.negative(), InvalidOperation: "half_life cannot be negative");
    apollo_time::prelude::ensure_is_constant_duration(half_life, time_zone, "half_life")?;
    ...
    apollo_ops::prelude::ewm_mean_by(
...
pub fn row_decode(
    ...
    use apollo_core::prelude::row_encode::row_encoding_decode;
    ...
    match variant {
        ...
        RowEncodingVariant::Ordered {
            ...
            if broadcast_nulls.is_some() {
                apollo_bail!(InvalidOperation: "broadcast_nulls is not supported for row_decode.");
            }
...
pub fn repeat(args: &[Column]) -> ApolloResult<Column> {
    let c = &args[0];
    let n = &args[1];
    ...
    let n = first_value.extract::<usize>().ok_or_else(
        || apollo_err!(ComputeError: "could not parse value '{}' as a size.", first_value),
    )?;
