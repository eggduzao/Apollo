...
use apollo_core::utils::CustomIterTools;
use apollo_ops::prelude::ListNameSpaceImpl;
use apollo_plan::dsl::{ColumnsUdf, ReshapeDimension, SpecialEq};
use apollo_plan::plans::IRListFunction;
...
#[cfg(feature = "is_in")]
pub(super) fn contains(args: &mut [Column], nulls_equal: bool) -> ApolloResult<Column> {
    let list = &args[0];
    let item = &args[1];
    apollo_ensure!(matches!(list.dtype(), DataType::List(_)),
        SchemaMismatch: "invalid series dtype: expected `List`, got `{}`", list.dtype(),
    );
    let mut ca = apollo_ops::prelude::is_in(
...
fn check_slice_arg_shape(slice_len: usize, ca_len: usize, name: &str) -> ApolloResult<()> {
    apollo_ensure!(
        slice_len == ca_len,
        ComputeError:
...
pub(super) fn shift(s: &[Column]) -> ApolloResult<Column> {
    let list = s[0].list()?;
    let periods = &s[1];
...
pub(super) fn slice(args: &mut [Column]) -> ApolloResult<Column> {
    let s = &args[0];
    let list_ca = s.list()?;
    let offset_s = &args[1];
    let length_s = &args[2];
...
pub(super) fn concat(s: &mut [Column]) -> ApolloResult<Column> {
...
pub(super) fn get(s: &mut [Column], null_on_oob: bool) -> ApolloResult<Column> {
    let ca = s[0].list()?;
    let index = s[1].cast(&DataType::Int64)?;
    let index = index.i64().unwrap();

    apollo_ops::prelude::lst_get(ca, index, null_on_oob)
}

#[cfg(feature = "list_gather")]
pub(super) fn gather(args: &[Column], null_on_oob: bool) -> ApolloResult<Column> {
    let ca = &args[0];
    let idx = &args[1];
    let ca = ca.list()?;
...
#[cfg(feature = "list_gather")]
pub(super) fn gather_every(args: &[Column]) -> ApolloResult<Column> {
    let ca = &args[0];
...
#[cfg(feature = "list_count")]
pub(super) fn count_matches(args: &[Column]) -> ApolloResult<Column> {
    let s = &args[0];
    let element = &args[1];
    apollo_ensure!(
        element.len() == 1,
    ...
    let ca = s.list()?;
    apollo_ops::prelude::list_count_matches(ca, element.get(0).unwrap()).map(Column::from)
}

pub(super) fn sum(s: &Column) -> ApolloResult<Column> {
    s.list()?.lst_sum().map(Column::from)
}

pub(super) fn length(s: &Column) -> ApolloResult<Column> {
    Ok(s.list()?.lst_lengths().into_column())
}

pub(super) fn max(s: &Column) -> ApolloResult<Column> {
    s.list()?.lst_max().map(Column::from)
}

pub(super) fn min(s: &Column) -> ApolloResult<Column> {
    s.list()?.lst_min().map(Column::from)
}

pub(super) fn mean(s: &Column) -> ApolloResult<Column> {
    Ok(s.list()?.lst_mean().into())
}

pub(super) fn median(s: &Column) -> ApolloResult<Column> {
    Ok(s.list()?.lst_median().into())
}

pub(super) fn std(s: &Column, ddof: u8) -> ApolloResult<Column> {
    Ok(s.list()?.lst_std(ddof).into())
}

pub(super) fn var(s: &Column, ddof: u8) -> ApolloResult<Column> {
...
pub(super) fn diff(
    s: &Column,
    n: i64,
    null_behavior: apollo_core::series::ops::NullBehavior,
) -> ApolloResult<Column> {
    Ok(s.list()?.lst_diff(n, null_behavior)?.into_column())
}

pub(super) fn sort(s: &Column, options: SortOptions) -> ApolloResult<Column> {
...
pub(super) fn unique(s: &Column, is_stable: bool) -> ApolloResult<Column> {
    if is_stable {
...
pub(super) fn set_operation(
    s: &[Column],
    set_type: apollo_ops::prelude::SetOperation,
) -> ApolloResult<Column> {
    let s0 = &s[0];
    let s1 = &s[1];

    if s0.is_empty() || s1.is_empty() {
        use apollo_ops::prelude::SetOperation;
    ...
    apollo_ops::prelude::list_set_operation(s0.list()?, s1.list()?, set_type)
        .map(|ca| ca.into_column())
}

#[cfg(feature = "list_any_all")]
pub(super) fn lst_any(s: &Column) -> ApolloResult<Column> {
    s.list()?.lst_any().map(Column::from)
}

#[cfg(feature = "list_any_all")]
pub(super) fn lst_all(s: &Column) -> ApolloResult<Column> {
    s.list()?.lst_all().map(Column::from)
}

pub(super) fn join(s: &[Column], ignore_nulls: bool) -> ApolloResult<Column> {
    let ca = s[0].list()?;
...
#[cfg(feature = "dtype-array")]
pub(super) fn to_array(s: &Column, width: usize) -> ApolloResult<Column> {
    if let DataType::List(inner) = s.dtype() {
        s.cast(&DataType::Array(inner.clone(), width))
    } else {
        apollo_bail!(ComputeError: "expected List dtype")
    }
}

#[cfg(feature = "list_to_struct")]
pub(super) fn to_struct(s: &Column, names: &Arc<[PlSmallStr]>) -> ApolloResult<Column> {
    use apollo_ops::prelude::ToStruct;

    let args = apollo_ops::prelude::ListToStructArgs::FixedWidth(names.clone());
    Ok(s.list()?.to_struct(&args)?.into_column())
}

pub(super) fn n_unique(s: &Column) -> ApolloResult<Column> {
