use apollo_core::downcast_as_macro_arg_physical;
use apollo_core::error::{ApolloResult, apollo_bail, apollo_ensure, apollo_warn};
...
fn shift_and_fill_with_mask(s: &Column, n: i64, fill_value: &Column) -> ApolloResult<Column> {
    use arrow::array::BooleanArray;
    use arrow::bitmap::BitmapBuilder;
    use apollo_core::prelude::BooleanChunked;
...
pub(super) fn shift_and_fill(args: &[Column]) -> ApolloResult<Column> {
    let s = &args[0];
        ...
        match logical {
            Boolean => {
                ...
                let fill_value = match fill_value {
                    ...
                    AnyValue::Null => None,
                    v => apollo_bail!(ComputeError: "fill value '{}' is not supported", v),
                };
            ...
            dt => apollo_bail!(opq = shift_and_fill, dt),
        }
    } else {
        apollo_warn!(
            Deprecation, // @2.0
...
pub(super) fn shift(args: &[Column]) -> ApolloResult<Column> {
    let s = &args[0];
    let n_s = &args[1];
    apollo_ensure!(
    n_s.len() == 1,
