...
use apollo_core::prelude::arity::{broadcast_binary_elementwise, unary_elementwise_values};
use apollo_core::prelude::{
    ChunkApply, ChunkedArray, Column, DataType, IntoColumn, ApolloFloatType, ApolloIntegerType,
    ApolloNumericType,
};
use apollo_core::with_match_physical_integer_type;
...
fn pow_on_floats<T>(base: &ChunkedArray<T>, exponent: &ChunkedArray<T>) -> ApolloResult<Column>
where
    T: ApolloFloatType,
...
fn pow_on_series(base: &Column, exponent: &Column) -> ApolloResult<Column> {
    ...
    if base_dtype.is_integer() {
        with_match_physical_integer_type!(base_dtype, |$native_type| {
            if exponent_dtype.is_float() {
                ...
                let exponent = exponent.strict_cast(&DataType::UInt32).map_err(|err| apollo_err!(
                    InvalidOperation:
...
pub(super) fn pow(s: &mut [Column]) -> ApolloResult<Column> {
    let base = &s[0];
    let exponent = &s[1];
...
pub(super) fn sqrt(base: &Column) -> ApolloResult<Column> {
    match base.dtype() {
        #[cfg(feature = "dtype-f16")]
        DataType::Float16 => {
            let ca = base.f16().unwrap();
...
fn sqrt_on_floats<T>(base: &ChunkedArray<T>) -> ApolloResult<Column>
where
    T: ApolloFloatType,
    T::Native: Pow<T::Native, Output = T::Native> + ToPrimitive + Float,
    ChunkedArray<T>: IntoColumn,
...
pub(super) fn cbrt(base: &Column) -> ApolloResult<Column> {
    match base.dtype() {
        #[cfg(feature = "dtype-f16")]
        DataType::Float16 => {
            let ca = base.f16().unwrap();
...
fn cbrt_on_floats<T>(base: &ChunkedArray<T>) -> ApolloResult<Column>
where
    T: ApolloFloatType,
    T::Native: Pow<T::Native, Output = T::Native> + ToPrimitive + Float,
    ChunkedArray<T>: IntoColumn,
