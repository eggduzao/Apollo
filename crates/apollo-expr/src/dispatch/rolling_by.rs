...
use apollo_core::prelude::{Column, DataType, IntoColumn, TimeUnit};
use apollo_core::series::Series;
use apollo_time::prelude::{RollingOptionsDynamicWindow, SeriesOpsTime};

fn roll_by_with_temporal_conversion<F: FnOnce(&Series, &Series) -> ApolloResult<Series>>(
    s: &[Column],
    op: F,
) -> ApolloResult<Column> {
    let by = &s[1];
