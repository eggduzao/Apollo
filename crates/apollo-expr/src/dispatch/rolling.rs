...
#[cfg(feature = "cov")]
use apollo_plan::dsl::RollingCovOptions;
use apollo_plan::prelude::PlanCallback;
use apollo_time::prelude::SeriesOpsTime;
use apollo_utils::pl_str::PlSmallStr;

fn roll_with_temporal_conversion<F: FnOnce(&Series) -> ApolloResult<Series>>(
    s: &Column,
    op: F,
) -> ApolloResult<Column> {
    let dt = s.dtype();
...
pub(super) fn rolling_min(s: &Column, options: RollingOptionsFixedWindow) -> ApolloResult<Column> {
    // @scalar-opt
...
pub(super) fn rolling_max(s: &Column, options: RollingOptionsFixedWindow) -> ApolloResult<Column> {
    // @scalar-opt
    s.as_materialized_series()
        .rolling_max(options)
        .map(Column::from)
...
pub(super) fn rolling_mean(s: &Column, options: RollingOptionsFixedWindow) -> ApolloResult<Column> {
    roll_with_temporal_conversion(s, |s| s.rolling_mean(options))
}

pub(super) fn rolling_sum(s: &Column, options: RollingOptionsFixedWindow) -> ApolloResult<Column> {
    // @scalar-opt
    s.as_materialized_series()
        .rolling_sum(options)
        .map(Column::from)
...
pub(super) fn rolling_var(s: &Column, options: RollingOptionsFixedWindow) -> ApolloResult<Column> {
    // @scalar-opt
    s.as_materialized_series()
        .rolling_var(options)
        .map(Column::from)
...
pub(super) fn rolling_std(s: &Column, options: RollingOptionsFixedWindow) -> ApolloResult<Column> {
    // @scalar-opt
    s.as_materialized_series()
        .rolling_std(options)
        .map(Column::from)
...
pub(super) fn rolling_rank(s: &Column, options: RollingOptionsFixedWindow) -> ApolloResult<Column> {
    // @scalar-opt
    s.as_materialized_series()
...
#[cfg(feature = "moment")]
pub(super) fn rolling_skew(s: &Column, options: RollingOptionsFixedWindow) -> ApolloResult<Column> {
    // @scalar-opt
    let s = s.as_materialized_series();
    apollo_ops::series::rolling_skew(s, options).map(Column::from)
}
...
pub(super) fn rolling_kurtosis(
    ...
    // @scalar-opt
    let s = s.as_materialized_series();
    apollo_ops::series::rolling_kurtosis(s, options).map(Column::from)
}
