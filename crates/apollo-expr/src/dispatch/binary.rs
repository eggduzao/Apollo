...
use apollo_plan::plans::IRBinaryFunction;
...
pub(super) fn contains(s: &[Column]) -> ApolloResult<Column> {
    let ca = s[0].binary()?;
    let lit = s[1].binary()?;
    Ok(ca
...
pub(super) fn size_bytes(s: &Column) -> ApolloResult<Column> {
    let ca = s.binary()?;
...
pub(super) fn bin_slice(s: &mut [Column]) -> ApolloResult<Column> {
    let ca = s[0].binary()?;
    Ok(ca
        .bin_slice(&s[1], &s[2])?
        .with_name(ca.name().clone())
...
pub(super) fn bin_head(s: &mut [Column]) -> ApolloResult<Column> {
    let ca = s[0].binary()?;
    Ok(ca
        .bin_head(&s[1])?
        .with_name(ca.name().clone())
...
pub(super) fn bin_tail(s: &mut [Column]) -> ApolloResult<Column> {
    let ca = s[0].binary()?;
    Ok(ca
        .bin_tail(&s[1])?
        .with_name(ca.name().clone())
