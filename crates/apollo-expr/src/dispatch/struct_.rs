...
use apollo_plan::plans::IRStructFunction;
...
pub(super) fn get_by_name(s: &Column, name: &str) -> ApolloResult<Column> {
    let ca = s.struct_()?;
    ca.field_by_name(name).map(Column::from)
}

pub(super) fn rename_fields(s: &Column, names: Arc<[PlSmallStr]>) -> ApolloResult<Column> {
    let ca = s.struct_()?;
    let fields = ca
        .fields_as_series()
        .iter()
...
pub(super) fn prefix_fields(s: &Column, prefix: &str) -> ApolloResult<Column> {
    let ca = s.struct_()?;
    let fields = ca
        .fields_as_series()
        .iter()
...
pub(super) fn suffix_fields(s: &Column, suffix: &str) -> ApolloResult<Column> {
    let ca = s.struct_()?;
    let fields = ca
        .fields_as_series()
        .iter()
...
#[cfg(feature = "json")]
pub(super) fn to_json(col: &Column) -> ApolloResult<Column> {
    use apollo_core::prelude::CompatLevel;

    let s = col.as_materialized_series();
    let iter = (0..s.n_chunks()).map(|i| {
        apollo_json::json::write::serialize_to_utf8(&*s.to_arrow(i, CompatLevel::newest()))
    });

    Ok(StringChunked::from_chunk_iter(s.name().clone(), iter).into_column())
}

pub(crate) fn with_fields(args: &[Column]) -> ApolloResult<Column> {
    let s = &args[0];
    ...
    let fields = ca
        ...
        .map(|s| {
            ...
            let new_name = function.call(name.clone()).map_err(
                |e| apollo_err!(ComputeError: "'name.map_fields' produced an error: {e}."),
            )?;
            s.rename(new_name);
            Ok(s)
        })
