...
use apollo_plan::plans::IRTemporalFunction;
...
pub fn temporal_func_to_udf(func: IRTemporalFunction) -> SpecialEq<Arc<dyn ColumnsUdf>> {
    ...
    match func {
        ...
        Duration(tu) => map_as_slice!(apollo_ops::series::impl_duration, tu),
...
pub(super) fn datetime(
    ...
    use apollo_core::prelude::{DataType, DatetimeChunked};
    use apollo_time::prelude::DatetimeMethods;
...
pub(super) fn combine(s: &[Column], tu: TimeUnit) -> ApolloResult<Column> {
    let date = &s[0];
    let time = &s[1];
    ...
    let tz = match date.dtype() {
        ...
        _dtype => {
            apollo_bail!(ComputeError: format!("expected Date or Datetime, got {}", _dtype))
        },
    };
    ...
    match tz {
        #[cfg(feature = "timezones")]
        Some(tz) => Ok(apollo_ops::prelude::replace_time_zone(
