...
use apollo_ops::series::Roll;
use apollo_plan::dsl::{ColumnsUdf, SpecialEq};
use apollo_plan::plans::IRBusinessFunction;
...
pub(super) fn business_day_count(
    ...
    let start = &s[0];
    let end = &s[1];
    apollo_ops::prelude::business_day_count(
...
pub(super) fn add_business_days(
    ...
    let start = &s[0];
    let n = &s[1];
    apollo_ops::prelude::add_business_days(
        start.as_materialized_series(),
...
pub(super) fn is_business_day(
    ...
    let dates = &s[0];
    apollo_ops::prelude::is_business_day(dates.as_materialized_series(), week_mask, holidays)
        .map(Column::from)
}
