...
use apollo_ops::series::new_int_range;

use super::utils::{ensure_items_contain_exactly_one_value, numeric_ranges_impl_broadcast};

const CAPACITY_FACTOR: usize = 5;

pub(super) fn int_range(s: &[Column], step: i64, dtype: DataType) -> ApolloResult<Column> {
    let start = &s[0];
    let end = &s[1];
    let name = start.name();
...
fn get_first_series_value<T>(s: &Column) -> ApolloResult<T::Native>
where
    T: ApolloIntegerType,
{
    ...
    let value =
        value_opt.ok_or_else(|| apollo_err!(ComputeError: "invalid null input for `int_range`"))?;
    Ok(value)
}

pub(super) fn int_ranges(s: &[Column], dtype: DataType) -> ApolloResult<Column> {
    let start = &s[0];
    let end = &s[1];
    let step = &s[2];
