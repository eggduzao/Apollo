...
use apollo_core::prelude::time_zone::parse_time_zone;
use apollo_core::prelude::*;
#[cfg(feature = "dtype-date")]
use apollo_plan::dsl::DateRangeArgs;
use apollo_time::{ClosedWindow, Duration, datetime_range_impl};
...
fn dt_range_start_end_interval(
    ...
    if let DataType::Datetime(tu, time_zone) = dtype {
        ...
        let start = temporal_series_to_i64_scalar(start)
            .ok_or_else(|| apollo_err!(ComputeError: "start is an out-of-range time."))?;
        let end = temporal_series_to_i64_scalar(end)
            .ok_or_else(|| apollo_err!(ComputeError: "end is an out-of-range time."))?;
        let result =
            datetime_range_impl(name.clone(), start, end, interval, closed, *tu, tz.as_ref())?;
        Ok(result.into_column())
    } else {
        apollo_bail!(ComputeError: "expected Datetime input, got {:?}", dtype);
    }
}
