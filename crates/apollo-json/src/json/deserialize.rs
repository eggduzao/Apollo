...
use apollo_compute::decimal::{f64_to_dec128, i128_to_dec128, str_to_dec128};
...
fn fill_array_from<B, T, A>(
    f: fn(&mut MutablePrimitiveArray<T>, &[B]) -> ApolloResult<()>,
    dtype: ArrowDataType,
    rows: &[B],
        ...
        ArrowDataType::Timestamp(tu, tz) => {
            ...
            let iter = rows.iter().enumerate().map(|(i, row)| match row.borrow() {
                ...
                BorrowedValue::String(v) => match (tu, tz) {
                    (_, None) => {
                        apollo_compute::cast::temporal::utf8_to_naive_timestamp_scalar(v, "%+", tu)
                    },
                    (_, Some(tz)) => {
