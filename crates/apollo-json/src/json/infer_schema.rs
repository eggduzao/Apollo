...
pub fn infer(json: &BorrowedValue) -> ApolloResult<ArrowDataType> {
    Ok(match json {
...
fn infer_object(inner: &Object) -> ApolloResult<ArrowDataType> {
    let fields = inner
        .iter()
...
fn infer_array(values: &[BorrowedValue]) -> ApolloResult<ArrowDataType> {
    let types = values
        .iter()
        .map(infer)
        // deduplicate entries
        .collect::<ApolloResult<PlIndexSet<_>>>()?;
