...
use apollo_error::*;
use apollo_utils::aliases::{PlIndexSet, PlRandomState};
use simd_json::BorrowedValue;

/// Reads up to a number of lines from `reader` into `rows` bounded by `limit`.
fn read_rows<R: BufRead>(reader: &mut R, rows: &mut [String], limit: usize) -> ApolloResult<usize> {
    if limit == 0 {
        return Ok(0);
    }
    ...
    for row in rows.iter_mut() {
        loop {
            ...
            let _ = reader.read_line(row).map_err(|e| {
                ApolloError::ComputeError(format!("{e} at line {row_number}").into())
            })?;
            if row.is_empty() {
                break;
            }
...
impl<R: BufRead> FallibleStreamingIterator for FileReader<R> {
    ...
    fn advance(&mut self) -> ApolloResult<()> {
...
fn parse_value<'a>(scratch: &'a mut Vec<u8>, val: &[u8]) -> ApolloResult<BorrowedValue<'a>> {
    scratch.clear();
    scratch.extend_from_slice(val);
    // 0 because it is row by row

    simd_json::to_borrowed_value(scratch)
        .map_err(|e| ApolloError::ComputeError(format!("{e}").into()))
}
...
) -> ApolloResult<impl Iterator<Item = ArrowDataType>> {
...
pub fn infer_iter<A: AsRef<str>>(rows: impl Iterator<Item = A>) -> ApolloResult<ArrowDataType> {
