//! # Apollo: *<small>DataFrames in Rust</small>*
//!
//! Apollo is a DataFrame library for Rust. It is based on [Apache Arrow](https://arrow.apache.org/)'s memory model.
...
//!
//! ## Quickstart
//! We recommend building queries directly with [apollo-lazy]. This allows you to combine
...
//!
//! [apollo-lazy]: apollo_lazy
//!
//! ```no_run
//! use apollo::prelude::*;
//! # fn example() -> ApolloResult<()> {
//!
...
//! ```
//!
//! This means that Apollo data structures can be shared zero copy with processes in many different
//! languages.
//!
//! ## Tree Of Contents
//!
...
//!
//! ## Data Structures
//! The base data structures provided by apollo are [`DataFrame`], [`Series`], and [`ChunkedArray<T>`].
...
//!
//! ### Series
//! [`Series`] are the type-agnostic columnar data representation of Apollo. The [`Series`] struct and
...
//! [`Vec<dyn ArrowArray>`]. These are the root data structures of Apollo, and implement many operations.
...
//!
//! ## SIMD
//! Apollo / Arrow uses packed_simd to speed up kernels with SIMD operations. SIMD is an optional
//! `feature = "nightly"`, and requires a nightly compiler. If you don't need SIMD, **Apollo runs on stable!**
//!
//! ## API
//! Apollo supports an eager and a lazy API. The eager API directly yields results, but is overall
...
//!
//! ## Expressions
//! Apollo has a powerful concept called expressions.
//! Apollo expressions can be used in various contexts and are a functional mapping of
...
//! be piped together.
//! You can run an expression by passing them on one of apollo execution contexts.
...
//! # }
//! ```
//! All expressions are run in parallel, meaning that separate apollo expressions are embarrassingly parallel.
//! (Note that within an expression there may be more parallelization going on).
//!
//! Understanding Apollo expressions is most important when starting with the Apollo library. Read more
//! about them in the [user guide](https://docs.apollo.org/user-guide/expressions).
//!
//! ### Eager
...
//!
//! ### Lazy
//! Unlock full potential with lazy computation. This allows query optimizations and provides Apollo
//! the full query context so that the fastest algorithm can be chosen.
//!
//! **[Read more in the lazy module.](apollo_lazy)**
//!
//! ## Compile times
...
//! * A lot of datatypes
//!
//! Both of these really put strain on compile times. To keep Apollo lean, we make both **opt-in**,
...
//! * `sql` - Pass SQL queries to Apollo.
//! * `random` - Generate arrays with randomly sampled values
//! * `ndarray`- Convert from [`DataFrame`] to [ndarray](https://docs.rs/ndarray/)
//! * `temporal` - Conversions between [Chrono](https://docs.rs/chrono/) and Apollo for temporal data types
...
//!       This allows Apollo to scale up beyond 2^32 rows by using an index with a `u64` data type.
//!       Apollo will be a bit slower with this feature activated as many data structures
...
//! [column selection]: apollo_lazy::dsl::col
//! [`ObjectChunked<T>`]: apollo_core::datatypes::ObjectChunked
//!
//!
...
//!     - `mode` - [Return the most occurring value(s)](apollo_ops::chunked_array::mode)
//!     - `cum_agg` - [`cum_sum`], [`cum_min`], [`cum_max`] aggregation.
//!     - `rolling_window` - rolling window functions, like [`rolling_mean`]
//!     - `interpolate` - [interpolate None values](apollo_ops::series::interpolate())
...
//! [`cum_sum`]: apollo_ops::prelude::cum_sum
//! [`cum_min`]: apollo_ops::prelude::cum_min
//! [`cum_max`]: apollo_ops::prelude::cum_max
//! [`rolling_mean`]: crate::series::Series#method.rolling_mean
//! [`diff`]: apollo_ops::prelude::diff
//! [`List`]: crate::datatypes::DataType::List
//! [`Struct`]: crate::datatypes::DataType::Struct
//!
//! ## Compile times and opt-in data types
//! As mentioned above, Apollo [`Series`] are wrappers around
...
//!
//! ## Performance
//! To get the best performance out of Apollo we recommend compiling on a nightly compiler
...
//!
//! #### Notes
//! [Benchmarks](https://github.com/apollo/apollo/pull/3108) have shown that on Linux and macOS JeMalloc
...
//!
//! * `APOLLO_FMT_TABLE_FORMATTING` -> define styling of tables using any of the following options (default = UTF8_FULL_CONDENSED). These options are defined by comfy-table which provides examples for each at <https://github.com/Nukesor/comfy-table/blob/main/src/style/presets.rs>
//!   * `ASCII_FULL`
...
//!   * `NOTHING`
//! * `APOLLO_FMT_TABLE_CELL_ALIGNMENT` -> define cell alignment using any of the following options (default = LEFT):
//!   * `LEFT`
//!   * `CENTER`
//!   * `RIGHT`
//! * `APOLLO_FMT_TABLE_DATAFRAME_SHAPE_BELOW` -> print shape information below the table.
//! * `APOLLO_FMT_TABLE_HIDE_COLUMN_NAMES` -> hide table column names.
//! * `APOLLO_FMT_TABLE_HIDE_COLUMN_DATA_TYPES` -> hide data types for columns.
//! * `APOLLO_FMT_TABLE_HIDE_COLUMN_SEPARATOR` -> hide separator that separates column names from rows.
//! * `APOLLO_FMT_TABLE_HIDE_DATAFRAME_SHAPE_INFORMATION"` -> omit table shape information.
//! * `APOLLO_FMT_TABLE_INLINE_COLUMN_DATA_TYPE` -> put column data type on the same line as the column name.
//! * `APOLLO_FMT_TABLE_ROUNDED_CORNERS` -> apply rounded corners to UTF8-styled tables.
//! * `APOLLO_FMT_MAX_COLS` -> maximum number of columns shown when formatting DataFrames.
//! * `APOLLO_FMT_MAX_ROWS` -> maximum number of rows shown when formatting DataFrames, `-1` to show all.
//! * `APOLLO_FMT_STR_LEN` -> maximum number of characters printed per string value.
//! * `APOLLO_TABLE_WIDTH` -> width of the tables used during DataFrame formatting.
//! * `APOLLO_MAX_THREADS` -> maximum number of threads used to initialize thread pool (on startup).
//! * `APOLLO_VERBOSE` -> print logging info to stderr.
//! * `APOLLO_NO_PARTITION` -> apollo may choose to partition the group_by operation, based on data
//!   cardinality. Setting this env var will turn partitioned group_by's off.
//! * `APOLLO_PARTITION_UNIQUE_COUNT` -> at which (estimated) key count a partitioned group_by should run.
//!   defaults to `1000`, any higher cardinality will run default group_by.
//! * `APOLLO_FORCE_PARTITION` -> force partitioned group_by if the keys and aggregations allow it.
//! * `APOLLO_ALLOW_EXTENSION` -> allows for [`ObjectChunked<T>`] to be used in arrow, opening up possibilities like using
//!   `T` in complex lazy expressions. However this does require `unsafe` code allow this.
//! * `APOLLO_NO_PARQUET_STATISTICS` -> if set, statistics in parquet files are ignored.
//! * `APOLLO_PANIC_ON_ERR` -> panic instead of returning an Error.
//! * `APOLLO_BACKTRACE_IN_ERR` -> include a Rust backtrace in Error messages.
//! * `APOLLO_NO_CHUNKED_JOIN` -> force rechunk before joins.
//!
//! ## User guide
//!
//! If you want to read more, check the [user guide](https://docs.apollo.org/).
...
#[doc(hidden)]
pub use apollo_compute;
pub use apollo_core::{
    apply_method_all_arrow_series, chunked_array, datatypes, df, error, frame, functions, series,
    testing,
};
#[cfg(feature = "apollo-io")]
pub use apollo_io as io;
#[cfg(feature = "lazy")]
pub use apollo_lazy as lazy;
#[cfg(feature = "temporal")]
pub use apollo_time as time;
#[doc(hidden)]
pub use apollo_utils;

/// Apollo crate version
