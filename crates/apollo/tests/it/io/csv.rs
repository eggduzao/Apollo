...
fn write_dates() {
    ...
    let with_timezone = apollo_ops::chunked_array::replace_time_zone(
...
#[test]
fn test_read_csv_filter() -> ApolloResult<()> {
    let df = CsvReadOptions::default()
...
#[test]
fn test_projection() -> ApolloResult<()> {
    let df = CsvReadOptions::default()
...
fn test_with_dtype() -> ApolloResult<()> {
...
#[test]
fn test_skip_rows() -> ApolloResult<()> {
...
#[test]
fn test_projection_idx() -> ApolloResult<()> {
...
#[test]
fn test_missing_fields() -> ApolloResult<()> {
    let csv = r"1,2,3,4,5
1,2,3
1,2,3,4,5
1,3,5
...
#[test]
fn test_comment_lines() -> ApolloResult<()> {
    let csv = r"1,2,3,4,5
# this is a comment
1,2,3,4,5
...
#[test]
fn test_null_values_argument() -> ApolloResult<()> {
    let csv = r"1,a,foo
null-value,b,bar
3,null-value,ham
";
...
#[test]
fn test_no_newline_at_end() -> ApolloResult<()> {
    let csv = r"a,b
foo,foo
bar,bar";
    let file = Cursor::new(csv);
...
#[test]
#[cfg(feature = "temporal")]
fn test_automatic_datetime_parsing() -> ApolloResult<()> {
...
#[test]
#[cfg(feature = "temporal")]
fn test_automatic_datetime_parsing_default_formats() -> ApolloResult<()> {
...
#[test]
fn test_no_quotes() -> ApolloResult<()> {
...
#[test]
fn test_header_inference() -> ApolloResult<()> {
...
#[test]
fn test_header_with_comments() -> ApolloResult<()> {
    let csv = "# ignore me\na,b,c\nd,e,f";
...
#[test]
#[cfg(feature = "temporal")]
fn test_ignore_parse_dates() -> ApolloResult<()> {
...
#[test]
fn test_projection_and_quoting() -> ApolloResult<()> {
    let csv = "a,b,c,d
A1,'B1',C1,1
A2,\"B2\",C2,2
A3,\"B3\",C3,3
...
#[test]
fn test_infer_schema_0_rows() -> ApolloResult<()> {
    let csv = r#"a,b,c,d
1,a,1.0,true
1,a,1.0,false
"#;
...
#[test]
fn test_infer_schema_eol() -> ApolloResult<()> {
    // no eol after header
...
#[test]
fn test_whitespace_separators() -> ApolloResult<()> {
    let tsv = "\ta\tb\tc\n1\ta1\tb1\tc1\n2\ta2\tb2\tc2\n".to_string();
...
#[test]
fn test_scientific_floats() -> ApolloResult<()> {
    let csv = r#"foo,bar
10000001,1e-5
10000002,.04
"#;
...
#[test]
fn test_tsv_header_offset() -> ApolloResult<()> {
    let csv = "foo\tbar\n\t1000011\t1\n\t1000026\t2\n\t1000949\t2";
    let file = Cursor::new(csv);
    let df = CsvReadOptions::default()
        .map_parse_options(|parse_options| {
...
#[test]
fn test_null_values_infer_schema() -> ApolloResult<()> {
    let csv = r#"a,b
1,2
3,NA
5,6"#;
...
#[test]
fn test_comma_separated_field_in_tsv() -> ApolloResult<()> {
...
#[test]
fn test_quoted_projection() -> ApolloResult<()> {
...
#[test]
fn test_last_line_incomplete() -> ApolloResult<()> {
...
#[test]
fn test_quoted_bool_ints() -> ApolloResult<()> {
...
#[test]
fn test_skip_inference() -> ApolloResult<()> {
    let csv = r#"metadata
line
foo,bar
1,2
...
#[test]
fn test_with_row_index() -> ApolloResult<()> {
    let df = CsvReadOptions::default()
...
#[test]
fn test_empty_string_cols() -> ApolloResult<()> {
    let csv = "\nabc\n\nxyz\n";
    let file = Cursor::new(csv);
    let df = CsvReadOptions::default()
        .with_has_header(false)
...
#[test]
fn test_empty_col_names() -> ApolloResult<()> {
    let csv = "a,b,c\n1,2,3";
    let file = Cursor::new(csv);
    let df = CsvReader::new(file).finish()?;
    let expected = df![
...
#[test]
fn test_trailing_empty_string_cols() -> ApolloResult<()> {
    let csv = "colx\nabc\nxyz\n\"\"";
    let file = Cursor::new(csv);
    let df = CsvReader::new(file).finish()?;
...
#[test]
fn test_escaping_quotes() -> ApolloResult<()> {
    let csv = "a\n\"\"\"\"";
    let file = Cursor::new(csv);
    let df = CsvReader::new(file).finish()?;
    let col = df.column("a")?;
...
#[test]
fn test_header_only() -> ApolloResult<()> {
    let csv = "a,b,c";
    let file = Cursor::new(csv);
    ...
    for h in [true, false] {
        assert!(matches!(
            CsvReadOptions::default()
                ...
                .into_reader_with_file_handle(file.clone())
                .finish(),
            Err(ApolloError::NoData(_))
        ))
    }
}

#[test]
fn test_try_parse_dates_empty() -> ApolloResult<()> {
    let csv = "date
1745-04-02
1742-03-21
1743-06-16
...
#[test]
fn test_try_parse_dates_3380() -> ApolloResult<()> {
...
#[test]
fn test_leading_whitespace_with_quote() -> ApolloResult<()> {
    let csv = r#"
"ABC","DEF",
"24.5","  4.1"
"#;
...
#[test]
fn test_single_column_leading_empty_line_25166() -> ApolloResult<()> {
