...
use ethnum::AsI256;
use apollo::prelude::{PlSmallStr, get_encodings};
use apollo_error::ApolloResult;
use apollo_parquet::read::{self as p_read};
use apollo_parquet::write::*;
...
pub fn read_column<R: Read + Seek>(mut reader: R, column: &str) -> ApolloResult<Box<dyn Array>> {
...
fn integration_read(data: &[u8], limit: Option<usize>) -> ApolloResult<IntegrationRead> {
    ...
    let batches = reader.collect::<ApolloResult<Vec<_>>>()?;
...
fn test_list_array_required_required(limit: Option<usize>) -> ApolloResult<()> {
...
#[test]
fn list_array_required_required() -> ApolloResult<()> {
    test_list_array_required_required(None)
}

#[test]
fn list_array_optional_optional() -> ApolloResult<()> {
    assert_array_roundtrip(true, data(0..12, true), None)
}

#[test]
fn list_array_required_optional() -> ApolloResult<()> {
    assert_array_roundtrip(true, data(0..12, false), None)
}

#[test]
fn list_array_optional_required() -> ApolloResult<()> {
    assert_array_roundtrip(false, data(0..12, true), None)
}

#[test]
fn list_slice() -> ApolloResult<()> {
    let data = vec![
...
#[test]
fn list_struct_slice() -> ApolloResult<()> {
...
#[test]
fn list_int_nullable() -> ApolloResult<()> {
    let data = vec![
        Some(vec![Some(1)]),
        None,
        Some(vec![None, Some(2)]),
...
#[test]
fn limit_list() -> ApolloResult<()> {
    test_list_array_required_required(Some(2))
}

#[test]
fn filter_chunk() -> ApolloResult<()> {
    ...
    let new_chunks = reader.collect::<ApolloResult<Vec<_>>>()?;
