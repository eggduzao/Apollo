...
use apollo_parquet::arrow::read::{Filter, column_iter_to_arrays};
use apollo_parquet::parquet::metadata::ColumnChunkMetadata;
use apollo_parquet::parquet::read::{BasicDecompressor, PageReader};
...
impl Iterator for RowGroupDeserializer {
    type Item = ApolloResult<RecordBatchT<Box<dyn Array>>>;
...
pub fn read_columns<'a, R: Read + Seek>(
    ...
    field_name: &'a str,
) -> ApolloResult<Vec<(&'a ColumnChunkMetadata, Vec<u8>)>> {
    row_group_metadata
...
) -> ApolloResult<(&'a ColumnChunkMetadata, Vec<u8>)>
where
    R: Read + Seek,
{
...
pub fn read_columns_many<R: Read + Seek>(
    ...
    fields: &ArrowSchema,
    filter: Option<Filter>,
) -> ApolloResult<Vec<Vec<Box<dyn Array>>>> {
