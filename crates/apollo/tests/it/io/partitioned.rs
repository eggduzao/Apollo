...
use apollo::io::ipc::{IpcReader, IpcWriterOption};
use apollo::io::prelude::SerReader;
use apollo::io::PartitionedWriter;
use apollo_error::ApolloResult;

#[test]
#[cfg(feature = "ipc")]
fn test_ipc_partition() -> ApolloResult<()> {
    ...
    for (expected_dir, expected_df) in expected.iter() {
        ...
        let reader = BufReader::new(apollo_utils::open_file(&ipc_paths[0])?);
