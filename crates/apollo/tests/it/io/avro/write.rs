...
use apollo::io::avro::{AvroReader, AvroWriter};
use apollo::io::{SerReader, SerWriter};
use apollo::prelude::df;
...
fn roundtrip(compression: Option<Compression>) -> ApolloResult<()> {
    let expected = data();
...
#[test]
fn no_compression() -> ApolloResult<()> {
    roundtrip(None)
}

#[test]
fn snappy() -> ApolloResult<()> {
    roundtrip(Some(Compression::Snappy))
}

#[test]
fn deflate() -> ApolloResult<()> {
...
#[test]
fn check_large_format() -> ApolloResult<()> {
...
#[test]
fn avro_record_schema() -> ApolloResult<()> {
...
#[test]
fn test_write_and_read_with_compression() -> ApolloResult<()> {
    let mut write_df = df!(
        "i64" => &[1, 2],
        "f64" => &[0.1, 0.2],
        "string" => &["a", "b"]
...
#[test]
fn test_with_projection() -> ApolloResult<()> {
    let mut df = df!(
        "i64" => &[1, 2],
        "f64" => &[0.1, 0.2],
        "string" => &["a", "b"]
...
#[test]
fn test_with_columns() -> ApolloResult<()> {
    let mut df = df!(
        "i64" => &[1, 2],
        "f64" => &[0.1, 0.2],
        "string" => &["a", "b"]
