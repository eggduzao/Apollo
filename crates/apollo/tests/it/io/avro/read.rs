...
) -> ApolloResult<(RecordBatchT<Box<dyn Array>>, ArrowSchema)> {
    let file = &mut avro;
...
fn test(codec: Codec) -> ApolloResult<()> {
...
#[test]
fn read_without_codec() -> ApolloResult<()> {
    test(Codec::Null)
}

#[test]
fn read_deflate() -> ApolloResult<()> {
    test(Codec::Deflate)
}

#[test]
fn read_snappy() -> ApolloResult<()> {
    test(Codec::Snappy)
}

#[test]
fn test_projected() -> ApolloResult<()> {
    let expected = data();
