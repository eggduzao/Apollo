...
#[test]
fn ternary_expand_sizes() -> ApolloResult<()> {
    let df = df! {
        "a" => [Some("a1"), None, None],
...
#[test]
#[cfg(feature = "strings")]
fn includes_null_predicate_3038() -> ApolloResult<()> {
    let df = df! {
        "a" => [Some("a1"), None, None],
    }?;
    let res = df
...
fn test_when_then_otherwise_cats() -> ApolloResult<()> {
...
#[test]
fn test_when_then_otherwise_single_bool() -> ApolloResult<()> {
    let df = df![
        "key" => ["a", "b", "b"],
...
#[test]
#[cfg(feature = "unique_counts")]
fn test_update_groups_in_cast() -> ApolloResult<()> {
    let df = df![
        "group" =>  ["A" ,"A", "A", "B", "B", "B", "B"],
        "id"=> [1, 2, 1, 4, 5, 4, 6],
    ]?;
...
#[test]
fn test_when_then_otherwise_sum_in_agg() -> ApolloResult<()> {
    let df = df![
        "groups" => [1, 1, 2, 2],
...
#[test]
fn test_binary_over_3930() -> ApolloResult<()> {
    let df = df![
        "class" => ["a", "a", "a", "b", "b", "b"],
...
#[test]
#[cfg(feature = "rank")]
fn test_ternary_aggregation_set_literals() -> ApolloResult<()> {
    let df = df![
        "name" => ["a", "b", "a", "b"],
        "value" => [1, 3, 2, 4]
    ]?;
...
#[test]
fn test_binary_group_consistency() -> ApolloResult<()> {
    let lf = df![
        "name" => ["a", "b", "c", "d"],
        "category" => [1, 2, 3, 4],
        "score" => [3, 5, 1, 2],
