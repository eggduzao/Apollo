...
#[test]
#[cfg(feature = "range")]
fn test_int_range_agg() -> ApolloResult<()> {
    let df = df![
        "x" => [5, 5, 4, 4, 2, 2]
    ]?;
...
fn test_groups_update() -> ApolloResult<()> {
...
#[test]
#[cfg(feature = "log")]
fn test_groups_update_binary_shift_log() -> ApolloResult<()> {
    let out = df![
        "a" => [1, 2, 3, 5],
        "b" => [1, 2, 1, 2],
    ]?
...
#[test]
#[cfg(feature = "cum_agg")]
fn test_expand_list() -> ApolloResult<()> {
    let out = df![
        "a" => [1, 2],
        "b" => [2, 3],
    ]?
...
#[test]
fn test_apply_groups_empty() -> ApolloResult<()> {
    let df = df![
        "id" => [1, 1],
        "hi" => ["here", "here"]
    ]?;
