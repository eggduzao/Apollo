...
#[test]
fn test_predicate_after_renaming() -> ApolloResult<()> {
    let df = df![
        "foo" => [1, 2, 3],
        "bar" => [3, 2, 1]
    ]?
...
#[test]
fn filter_true_lit() -> ApolloResult<()> {
    let df = df! {
...
fn and_filters(expr: Vec<Expr>) -> Expr {
    expr.into_iter().reduce(apollo::prelude::Expr::and).unwrap()
}

#[test]
fn test_many_filters() -> ApolloResult<()> {
...
#[test]
fn test_filter_no_combine() -> ApolloResult<()> {
    let df = df![
        "vals" => [1, 2, 3, 4, 5]
    ]?;
...
#[test]
fn test_filter_block_join() -> ApolloResult<()> {
    let df_a = df![
        "a" => ["a", "b", "c"],
        "c" => [1, 4, 6]
    ]?;
...
fn test_is_in_categorical_3420() -> ApolloResult<()> {
    let df = df![
        "a" => ["a", "b", "c", "d", "e"],
        "b" => [1, 2, 3, 4, 5]
    ]?;
...
#[test]
fn test_predicate_pushdown_blocked_by_outer_join() -> ApolloResult<()> {
    let df1 = df! {
        "a" => ["a1", "a2"],
        "b" => ["b1", "b2"]
    }?;
...
#[test]
fn test_binaryexpr_pushdown_left_join_9506() -> ApolloResult<()> {
    let df1 = df! {
        "a" => ["a1"],
        "b" => ["b1"]
    }?;
...
#[test]
fn test_count_blocked_at_union_3963() -> ApolloResult<()> {
    let lf1 = df![
        "k" => ["x", "x", "y"],
        "v" => [3, 2, 6]
    ]?
...
#[test]
fn test_predicate_on_join_select_4884() -> ApolloResult<()> {
    let lf = df![
      "x" => [0, 1],
      "y" => [1, 2],
    ]?
...
#[test]
fn test_predicate_pushdown_block_8847() -> ApolloResult<()> {
    let ldf = df![
        "A" => [1, 2, 3]
    ]?
    .lazy();
