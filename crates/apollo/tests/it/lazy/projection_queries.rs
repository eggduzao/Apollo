...
#[test]
fn test_sum_after_filter() -> ApolloResult<()> {
    let df = df![
        "ids" => 0..10,
        "values" => 10..20,
    ]?
...
#[test]
fn test_swap_rename() -> ApolloResult<()> {
    let df = df![
        "a" => [1],
        "b" => [2],
    ]?
...
#[test]
fn test_full_outer_join_with_column_2988() -> ApolloResult<()> {
    let ldf1 = df![
        "key1" => ["foo", "bar"],
        "key2" => ["foo", "bar"],
        "val1" => [3, 1]
...
#[test]
fn test_many_aliasing_projections_5070() -> ApolloResult<()> {
    let df = df! {
        "date" => [1, 2, 3],
        "val" => [1, 2, 3],
    }?;
...
#[test]
#[cfg(feature = "cum_agg")]
fn test_projection_5086() -> ApolloResult<()> {
    let df = df![
        "a" => ["a", "a", "a", "b"],
        "b" => [1, 0, 1, 0],
        "c" => [0, 1, 2, 0],
...
fn test_unnest_pushdown() -> ApolloResult<()> {
    let df = df![
...
#[test]
fn test_join_duplicate_7314() -> ApolloResult<()> {
