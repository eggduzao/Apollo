...
fn test_special_group_by_schemas() -> ApolloResult<()> {
    let df = df![
        "a" => [1, 2, 3, 4, 5],
        "b" => [1, 2, 3, 4, 5],
    ]?;
...
#[test]
fn max_on_empty_df_3027() -> ApolloResult<()> {
    let df = df! {
        "id" => ["1"],
        "name" => ["one"],
        "numb" => [1]
...
#[test]
fn test_alias_before_cast() -> ApolloResult<()> {
    let out = df![
        "a" => [1, 2, 3],
    ]?
    .lazy()
...
#[test]
fn test_sorted_path() -> ApolloResult<()> {
...
#[test]
fn test_sorted_path_joins() -> ApolloResult<()> {
    let dfa = df![
        "a"=> [1, 2, 3]
    ]?;
...
#[test]
fn test_unknown_supertype_ignore() -> ApolloResult<()> {
    let df = df![
        "col1" => [0., 3., 2., 1.],
        "col2" => [0., 0., 1., 1.],
    ]?;
...
#[test]
fn test_apply_multiple_columns() -> ApolloResult<()> {
    let df = fruits_cars();
...
#[test]
fn test_group_by_on_lists() -> ApolloResult<()> {
