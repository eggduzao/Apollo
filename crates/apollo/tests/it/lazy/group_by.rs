...
#[test]
#[cfg(feature = "rank")]
fn test_filter_sort_diff_2984() -> ApolloResult<()> {
...
#[test]
fn test_filter_after_tail() -> ApolloResult<()> {
    let df = df![
        "a" => ["foo", "foo", "bar"],
        "b" => [1, 2, 3]
    ]?;
...
#[test]
#[cfg(feature = "diff")]
fn test_filter_diff_arithmetic() -> ApolloResult<()> {
    let df = df![
        "user" => [1, 1, 1, 1, 2],
        "group" => [1, 2, 1, 1, 2],
        "value" => [1, 5, 14, 17, 20]
...
#[test]
fn test_group_by_lit_agg() -> ApolloResult<()> {
    let df = df![
        "group" => [1, 2, 1, 1, 2],
    ]?;
...
#[test]
#[cfg(feature = "diff")]
fn test_group_by_agg_list_with_not_aggregated() -> ApolloResult<()> {
    let df = df![
...
fn test_logical_mean_partitioned_group_by_block() -> ApolloResult<()> {
...
#[test]
fn test_filter_aggregated_expression() -> ApolloResult<()> {
    let df: DataFrame = df![
    "day" => [2, 2, 2, 2, 2, 2, 1, 1],
