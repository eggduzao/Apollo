use apollo_core::utils::{accumulate_dataframes_vertical, split_df};

use super::*;

#[test]
fn test_chunked_left_join() -> ApolloResult<()> {
...
fn test_inner_join() {
    ...
    for i in 1..8 {
        unsafe { std::env::set_var("APOLLO_MAX_THREADS", format!("{i}")) };
...
#[test]
#[cfg_attr(miri, ignore)]
fn test_full_outer_join() -> ApolloResult<()> {
    let (temp, rain) = create_frames();
...
#[test]
#[cfg_attr(miri, ignore)]
fn test_empty_df_join() -> ApolloResult<()> {
    ...
    // https://github.com/apollo/apollo/issues/1824
...
#[test]
#[cfg_attr(miri, ignore)]
fn test_unit_df_join() -> ApolloResult<()> {
    let df1 = df![
        "a" => [1],
        "b" => [2]
    ]?;
...
#[test]
#[cfg_attr(miri, ignore)]
fn test_joins_with_duplicates() -> ApolloResult<()> {
...
#[test]
#[cfg_attr(miri, ignore)]
fn test_multi_joins_with_duplicates() -> ApolloResult<()> {
...
#[test]
#[cfg_attr(miri, ignore)]
fn test_join_floats() -> ApolloResult<()> {
    let df_a = df! {
        "a" => &[1.0, 2.0, 1.0, 1.0],
        "b" => &["a", "b", "c", "c"],
        "c" => &[0.0, 1.0, 2.0, 3.0]
...
#[cfg_attr(miri, ignore)]
#[cfg(feature = "lazy")]
fn test_4_threads_bit_offset() -> ApolloResult<()> {
