...
use apollo_core::prelude::PlRandomState;
use apollo_core::schema::{Schema, SchemaRef};
use apollo_core::{POOL, config};
use apollo_error::{ApolloResult, apollo_bail, apollo_ensure, apollo_err};
use apollo_expr::groups::new_hash_grouper;
use apollo_expr::planner::{ExpressionConversionState, create_physical_expr};
use apollo_expr::reduce::into_reduction;
use apollo_expr::state::ExecutionState;
use apollo_mem_engine::create_physical_plan;
use apollo_mem_engine::scan_predicate::create_scan_predicate;
use apollo_plan::dsl::{
    FileSinkOptions, JoinOptionsIR, PartitionStrategyIR, PartitionedSinkOptionsIR, ScanSources,
};
use apollo_plan::plans::expr_ir::ExprIR;
use apollo_plan::plans::{AExpr, ArenaExprIter, IR, IRAggExpr};
use apollo_plan::prelude::FunctionFlags;
...
use apollo_utils::relaxed_cell::RelaxedCell;
...
) -> ApolloResult<(Graph, SecondaryMap<PhysNodeKey, GraphNodeKey>)> {
    ...
    let graph_key = match &node.kind {
        ...
        OrderedUnion { inputs } => {
            let input_keys = inputs
                .iter()
                .map(|i| ApolloResult::Ok((to_graph_rec(i.node, ctx)?, i.port)))
                .try_collect_vec()?;
            ctx.graph.add_node(
        ...
        | SemiAntiJoin {
            ...
            let unique_left_on = left_on
                .iter()
                .enumerate()
                .map(|(i, expr)| expr.with_alias(format_pl_smallstr!("__APOLLO_KEYCOL_{i}")))
                .collect_vec();
        ...
        PythonScan { options } => {
            ...
            use apollo_plan::dsl::python_dsl::PythonScanSource as S;
            use apollo_plan::plans::PythonPredicate;
            use apollo_utils::relaxed_cell::RelaxedCell;
            ...
            let (pl_predicate, predicate_serialized) = apollo_mem_engine::python_scan_predicate(
                &mut options,
                ctx.expr_arena,
                &mut ctx.expr_conversion_state,
            )?;
            ...
            let (name, get_batch_fn) = match options.python_source {
                ...
                S::IOPlugin => {
                    ...
                    let (generator, can_parse_predicate) = {
                        Python::attach(|py| {
                            let pl = PyModule::import(py, intern!(py, "apollo")).unwrap();
                            ...
                            let generator = generator_init.get_item(0).map_err(
                                |_| apollo_err!(ComputeError: "expected tuple got {generator_init}"),
                            )?;
                            let can_parse_predicate = generator_init.get_item(1).map_err(
                                |_| apollo_err!(ComputeError: "expected tuple got {generator}"),
                            )?;
                            let can_parse_predicate = can_parse_predicate.extract::<bool>().map_err(
                                |_| apollo_err!(ComputeError: "expected bool got {can_parse_predicate}"),
                            )? && could_serialize_predicate;

                            let generator = generator.into_py_any(py).map_err(
                                |_| apollo_err!(ComputeError: "unable to grab reference to IO plugin generator"),
                            )?;

                            ApolloResult::Ok((generator, can_parse_predicate))
                        })
                    }?;
                    ...
                    let get_batch_fn = Box::new(move |state: &StreamingExecutionState| {
                        let df = Python::attach(|py| {
                            match generator.bind(py).call_method0(intern!(py, "__next__")) {
                                Ok(out) => apollo_plan::plans::python_df_to_rust(py, out).map(Some),
                                Err(err)
            ...
            use apollo_plan::dsl::{CastColumnsPolicy, MissingColumnsPolicy};
        ...
        | ewm_variant @ EwmStd { input, options } => {
            ...
            use apollo_compute::ewm::mean::EwmMeanState;
            use apollo_compute::ewm::{EwmCovState, EwmStateUpdate, EwmStdState, EwmVarState};
            use apollo_core::with_match_physical_float_type;
