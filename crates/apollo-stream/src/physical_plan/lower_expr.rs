...
use apollo_expr::{ExpressionConversionState, create_physical_expr};
use apollo_ops::frame::{JoinArgs, JoinType};
use apollo_ops::series::{RLE_LENGTH_COLUMN_NAME, RLE_VALUE_COLUMN_NAME};
use apollo_plan::plans::AExpr;
use apollo_plan::plans::expr_ir::{ExprIR, OutputName};
...
use apollo_utils::{unique_column_name, unitvec};
use slotmap::SlotMap;
        ...
        cache.is_elementwise.insert(
            ...
            (|| {
                ...
                loop {
                    ...
                    if !apollo_plan::plans::is_elementwise(&mut stack, ae, arena) {
                        return false;
                    }
...
pub fn is_input_independent_rec(
    ...
    let ret = match arena.get(expr_key) {
        ...
        AExpr::Agg(agg_expr) => match agg_expr.get_input() {
            apollo_plan::plans::NodeInputs::Leaf => true,
            apollo_plan::plans::NodeInputs::Single(expr) => {
                is_input_independent_rec(expr, arena, cache)
            },
            apollo_plan::plans::NodeInputs::Many(exprs) => exprs
                .iter()
                .all(|expr| is_input_independent_rec(*expr, arena, cache)),
        },
        AExpr::Ternary {
...
fn build_fallback_node_with_ctx(
    ...
    let mut select_names: PlHashSet<_> = exprs
        .iter()
        .flat_map(|expr| {
            apollo_plan::utils::aexpr_to_leaf_names_iter(expr.node(), ctx.expr_arena).cloned()
        })
        .collect();
...
fn simplify_input_streams(
    ...
    ctx: &mut LowerExprContext,
) -> ApolloResult<PlIndexSet<PhysStream>> {
...
fn lower_exprs_with_ctx(
    ...
    exprs: &[Node],
    ctx: &mut LowerExprContext,
) -> ApolloResult<(PhysStream, Vec<Node>)> {
    ...
    for expr in exprs.iter().copied() {
        ...
        match ctx.expr_arena.get(expr).clone() {
            ...
            AExpr::Function {
                ...
                use apollo_core::prelude::ExplodeOptions;
            ...
            AExpr::StructEval {
                ...
                let eval_col_names: PlHashSet<_> = evaluation
                    .iter()
                    .flat_map(|expr| {
                        apollo_plan::utils::aexpr_to_leaf_names_iter(expr.node(), ctx.expr_arena)
                    })
                    .cloned()
                    .collect();
                ...
                let validity_name = apollo_utils::format_pl_smallstr!(
                    "{}{}",
                    out_name,
                ...
                let field_prefix = apollo_utils::format_pl_smallstr!("{}{}", out_name, separator);
                ...
                evaluation.iter_mut().for_each(|e| {
                    *e = e.with_alias(apollo_utils::format_pl_smallstr!(
                        "{}{}",
                        &field_prefix,
                        e.output_name()
                    ));
    ...
    let output_schema: Schema = exprs
        ...
        .map(|e| {
            ...
            let dtype = e
                ...
                .unwrap();
            ApolloResult::Ok(Field::new(name, dtype))
        })
        .try_collect()?;
...
) -> ApolloResult<(PhysStream, Vec<ExprIR>)> {
