...
use apollo_core::prelude::{Field, InitHashMaps, PlIndexMap, PlIndexSet, SortMultipleOptions};
...
use apollo_plan::plans::{AExpr, IR, IRAggExpr, IRFunctionExpr, NaiveExprMerger, write_group_by};
use apollo_plan::prelude::{GroupbyOptions, *};
use apollo_utils::arena::{Arena, Node};
use apollo_utils::pl_str::PlSmallStr;
use apollo_utils::{IdxSize, unique_column_name};
...
fn try_build_streaming_group_by(
...
) -> ApolloResult<Option<PhysStream>> {
    if apply.is_some() {
        return Ok(None); // TODO
    }
    ...
    if keys.is_empty() {
        return Err(
            apollo_err!(ComputeError: "at least one key is required in a group_by operation"),
        );
    }
...
pub fn build_group_by_stream(
    ...
    if (are_keys_sorted || std::env::var("APOLLO_FORCE_SORTED_GROUP_BY").is_ok_and(|v| v == "1"))
