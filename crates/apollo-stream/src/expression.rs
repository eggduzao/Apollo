...
use apollo_expr::prelude::{AggregationContext, ExecutionState, PhysicalExpr};
...
impl StreamExpr {
    ...
    pub async fn evaluate(&self, df: &DataFrame, state: &ExecutionState) -> ApolloResult<Column> {
        if self.reentrant {
            let state = state.clone();
            let phys_expr = self.inner.clone();
            let df = df.clone();
            apollo_io::pl_async::get_runtime()
