...
use apollo_expr::planner::{ExpressionConversionState, create_physical_expr, get_expr_depth_limit};
use apollo_plan::plans::{IR, IRPlan};
use apollo_plan::prelude::AExpr;
use apollo_plan::prelude::expr_ir::ExprIR;
...
fn cfg_prepare_visualization_data() -> bool {
    if !PREPARE_VISUALIZATION_DATA.load() {
        PREPARE_VISUALIZATION_DATA.fetch_or(
            std::env::var("APOLLO_STREAM_ALWAYS_PREPARE_VISUALIZATION_DATA").as_deref() == Ok("1"),
        );
    }
...
impl StreamingQuery {
    pub fn build(
        ...
        if let Ok(visual_path) = std::env::var("APOLLO_VISUALIZE_IR") {
            let plan = IRPlan {
                lp_top: node,
        ...
        if let Ok(visual_path) = std::env::var("APOLLO_VISUALIZE_PHYSICAL_PLAN") {
            let visualization =
        ...
        let metrics = if std::env::var("APOLLO_TRACK_METRICS").as_deref() == Ok("1")
            || std::env::var("APOLLO_LOG_METRICS").as_deref() == Ok("1")
        {
    ...
    pub fn execute(self) -> ApolloResult<QueryResult> {
        let StreamingQuery {
            top_ir,
            mut graph,
            root_phys_node,
