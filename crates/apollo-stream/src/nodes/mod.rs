...
mod compute_node_prelude {
    pub use apollo_core::frame::DataFrame;
    pub use apollo_error::ApolloResult;
    pub use apollo_expr::state::ExecutionState;
...
pub trait ComputeNode: Send {
    ...
    /// in-memory nodes.
    fn get_output(&mut self) -> ApolloResult<Option<DataFrame>> {
        Ok(None)
    }
}
