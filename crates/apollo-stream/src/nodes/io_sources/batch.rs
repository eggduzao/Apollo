...
pub mod builder {
    ...
    impl FileReaderBuilder for BatchFnReaderBuilder {
        ...
        fn build_file_reader(
            &self,
            _source: apollo_plan::prelude::ScanSource,
            _cloud_options: Option<Arc<apollo_io::cloud::CloudOptions>>,
...
pub type GetBatchFn =
    Box<dyn Fn(&StreamingExecutionState) -> ApolloResult<Option<DataFrame>> + Send + Sync>;
...
mod get_batch_state {
    ...
    use super::{DataFrame, GetBatchFn, ApolloResult, StreamingExecutionState};
    ...
    impl GetBatchState {
        ...
        ) -> ApolloResult<(Self, Option<DataFrame>)> {
            get_runtime()
                .spawn_blocking({
        ...
        let handle = spawn(TaskPriority::Low, async move {
            ...
            if let Some(row_position_on_end_tx) = row_position_on_end_tx {
                let n_rows_seen = IdxSize::try_from(n_rows_seen)
                    .map_err(|_| apollo_err!(bigidx, ctx = "batch reader", size = n_rows_seen))?;
