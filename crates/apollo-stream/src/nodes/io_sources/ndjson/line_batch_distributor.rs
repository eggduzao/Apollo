...
impl LineBatchDistributor {
    ...
    pub(super) async fn run(self) -> ApolloResult<usize> {
        ...
        let fixed_read_size = std::env::var("APOLLO_FORCE_NDJSON_CHUNK_SIZE")
            .map(|x| {
                x.parse::<NonZeroUsize>().ok().unwrap_or_else(|| {
                    panic!("invalid value for APOLLO_FORCE_NDJSON_CHUNK_SIZE: {x}")
                })
            })
            .ok();
