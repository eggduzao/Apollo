...
use apollo_io::ndjson;
use apollo_io::prelude::{is_json_line, parse_ndjson};
...
impl ChunkReader {
    pub(super) fn read_chunk(&self, chunk: &[u8]) -> ApolloResult<DataFrame> {
        match self {
            Self::NDJson {
                projected_schema,
                ignore_errors,
            ...
            Self::Lines { projection } => {
                use apollo_core::prelude::IntoColumn;
                use apollo_core::series::Series;
                use apollo_io::scan_lines;
