...
use apollo_mem_engine::scan_predicate::initialize_scan_predicate;
use apollo_plan::dsl::PredicateFileSkip;
                ...
                AbortOnDropHandle::new(async_executor::spawn(TaskPriority::Low, async move {
                    let (scan_source, reader, n_rows_in_file) = async {
                        ...
                        if let Some((reader, n_rows_in_file)) = maybe_initialized {
                            return ApolloResult::Ok((scan_source, reader, Some(n_rows_in_file)));
                        }
                        ...
                        ApolloResult::Ok((scan_source, reader, opt_n_rows))
                    }
                    .await?;
