...
use apollo_io::utils::byte_source::{ByteSource, DynByteSourceBuilder};
use apollo_io::utils::compression::SupportedCompression;
...
pub fn calc_n_readers_pre_init(
    ...
    if let Ok(v) = std::env::var("APOLLO_NUM_READERS_PRE_INIT").map(|x| {
        x.parse::<NonZeroUsize>()
            .ok()
            .unwrap_or_else(|| panic!("invalid value for APOLLO_NUM_READERS_PRE_INIT: {x}"))
            .get()
    }) {
        return v;
    }
...
pub fn calc_max_concurrent_scans(num_pipelines: usize, num_sources: usize) -> usize {
    if let Ok(v) = std::env::var("APOLLO_MAX_CONCURRENT_SCANS").map(|x| {
        x.parse::<NonZeroUsize>()
            .ok()
            .unwrap_or_else(|| panic!("invalid value for APOLLO_MAX_CONCURRENT_SCANS: {x}"))
            .get()
    }) {
        return v;
    }
