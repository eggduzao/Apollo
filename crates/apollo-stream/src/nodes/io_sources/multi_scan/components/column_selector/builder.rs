...
use apollo_core::prelude::{Column, DataType, InitHashMaps, IntoColumn, PlHashMap};
use apollo_core::scalar::Scalar;
use apollo_core::schema::Schema;
use apollo_core::schema::iceberg::{IcebergColumn, IcebergColumnType, LIST_ELEMENT_DEFAULT_ID};
use apollo_core::series::{IntoSeries, Series};
use apollo_core::utils::get_numeric_upcast_supertype_lossless;
use apollo_error::{ApolloResult, feature_gated, apollo_bail};
use apollo_plan::dsl::{CastColumnsPolicy, ExtraColumnsPolicy, MissingColumnsPolicy};
...
impl ColumnSelectorBuilder {
    ...
    pub fn attach_transforms(
        ...
        let attach_cast = |options: CastOptions| -> ApolloResult<ColumnSelector> {
            Ok(ColumnTransform::Cast {
        ...
        {
            use apollo_core::prelude::{TimeUnit, TimeZone};
                                ...
                                MissingColumnsPolicy::Insert => {
                                    ColumnSelector::Constant(Box::new((
                                        ...
                                        iceberg_default_value_provider
                                            ...
                                            .flatten()
                                            .unwrap_or_else(|| {
                                                Scalar::null(output_column.type_.to_apollo_dtype())
                                            }),
                                    )))
                                },
                                MissingColumnsPolicy::Raise => {
        ...
        ICT::Struct(fields) => {
            ...
            for field in fields.values() {
                ...
                field_columns.push(if let Some(default) = opt_default {
                    ...
                    Column::full_null(field.name.clone(), 1, &field.type_.to_apollo_dtype())
                });
            }
