...
use apollo_core::schema::iceberg::{IcebergSchema, IcebergSchemaRef};
use apollo_core::schema::{Schema, SchemaRef};
use apollo_error::{ApolloResult, apollo_err};
use apollo_plan::dsl::default_values::IcebergIdentityTransformedPartitionFields;
use apollo_plan::dsl::{CastColumnsPolicy, ColumnMapping, MissingColumnsPolicy};
                ...
                let projected_iceberg_schema: IcebergSchemaRef = Arc::new(
                    projected_schema
                        ...
                        .map(|(output_name, output_dtype)| {
                            ...
                            assert_eq!(&column.type_.to_apollo_dtype(), output_dtype);
    ...
    pub fn build_projection(
        ...
        Ok(match self {
            ...
            Self::Iceberg {
                ...
                ApolloResult::Ok(Projection::Mapped {
                    projected_schema: projected_schema.clone(),
                    mapping: mapping.map(Arc::new),
                    missing_columns_mask: missing_columns_mask.map(|x| x.freeze()),
