...
use apollo_io::utils::byte_source::{BufferByteSource, ByteSource, DynByteSource};
...
pub(super) struct RecordBatchDataFetcher {
    ...
    pub(super) prefetch_send: Sender<(
        tokio_handle_ext::AbortOnDropHandle<ApolloResult<RecordBatchData>>,
...
impl RecordBatchDataFetcher {
    pub(super) async fn run(&mut self) -> ApolloResult<()> {
        ...
        if !self.fetch_metadata_only {
            ...
            while self.record_batch_idx < n_record_batches {
                ...
                let handle = io_runtime.spawn(async move {
                    ...
                    let fetched_bytes =
                        if let DynByteSource::Buffer(mem_slice) = current_byte_source.as_ref() {
                            ...
                            if !std::ptr::eq(
                                ...
                                apollo_utils::mem::prefetch::no_prefetch as *const (),
                            ) {
                    ...
                    ApolloResult::Ok(RecordBatchData {
                        fetched_bytes,
                        block_index,
                        num_rows,
                        row_offset: None,
        ...
        let n_rows = match &*byte_source {
            DynByteSource::Buffer(BufferByteSource(memslice)) => {
                ...
                IdxSize::try_from(n_rows)
                    .map_err(|_| apollo_err!(bigidx, ctx = "ipc file", size = n_rows))?
            },
