...
use apollo_core::chunked_array::flags::StatisticsFlags;
use apollo_core::frame::DataFrame;
use apollo_core::prelude::PlHashMap;
use apollo_core::schema::Schema;
use apollo_core::utils::arrow::io::ipc::read::common::apply_projection;
use apollo_core::utils::arrow::io::ipc::read::{BlockReader, FileMetadata, read_batch};
    ...
    pub(super) async fn record_batch_data_to_df(
        ...
        let mut df = if pl_schema.is_empty() {
            ...
            if let Some(Some(flags)) = flags {
                apollo_ensure!(flags.len() == df.columns().len(),
...
) -> ApolloResult<Option<Vec<Option<StatisticsFlags>>>> {
                ...
                match kv.value()? {
                    Some(s) => {
                        let flags: Vec<u32> = serde_json::from_str(s)
                            .map_err(|e| apollo_err!(ComputeError: "Unable to parse IPC statistics flags: {}", e))?;
                        let flags = flags
                            .iter()
                    ...
                    None => {
                        apollo_bail!(ComputeError: "Expected IPC statistics flags value, found None")
                    },
                }
            }
        }
