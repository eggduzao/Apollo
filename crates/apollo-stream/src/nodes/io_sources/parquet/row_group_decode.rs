...
use apollo_core::prelude::{ArrowField, BooleanChunked, ChunkFilter, Column, DataType, IntoColumn};
use apollo_core::series::Series;
use apollo_core::utils::arrow::bitmap::{Bitmap, MutableBitmap};
use apollo_error::ApolloResult;
use apollo_io::RowIndex;
use apollo_io::predicates::{
    ColumnPredicateExpr, ColumnPredicates, ScanIOPredicate, SpecializedColumnPredicate,
};
pub use apollo_io::prelude::_internal::PrefilterMaskSetting;
use apollo_io::prelude::try_set_sorted_flag;
use apollo_parquet::read::{Filter, ParquetType, PredicateFilter, PrimitiveLogicalType};
use apollo_utils::pl_str::PlSmallStr;
use apollo_utils::{IdxSize, UnitVec};
...
impl RowGroupDecoder {
    ...
    async fn row_group_data_to_df_impl(
        ...
        self.decode_projected_columns(
            &mut decoded_cols,
            &row_group_data,
            Some(apollo_parquet::read::Filter::Range(slice_range.clone())),
        )
        .await?;
    ...
    async fn decode_projected_columns(
        ...
        filter: Option<apollo_parquet::read::Filter>,
    ) -> ApolloResult<()> {
                    ...
                    .map(move |offset| {
                        ...
                        async move {
                            ...
                            (offset
                                ...
                                .collect::<ApolloResult<UnitVec<_>>>()
                        }
                    }),
            )
        };
...
fn decode_column(
    ...
    let (arrays, pred_true_mask) = apollo_io::prelude::_internal::to_deserializer(
...
fn decode_column_prefiltered(
    ...
    let deserialize_filter =
        prefilter.then(|| apollo_parquet::read::Filter::Mask(mask_bitmap.clone()));

    let (array, _) = apollo_io::prelude::_internal::to_deserializer(
        columns_to_deserialize,
        arrow_field.clone(),
        deserialize_filter,
    )?;
