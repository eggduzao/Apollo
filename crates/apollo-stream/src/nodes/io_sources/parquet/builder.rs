...
use apollo_io::prelude::{FileMetadata, ParallelStrategy, ParquetOptions};
use apollo_io::utils::byte_source::DynByteSourceBuilder;
...
impl FileReaderBuilder for ParquetReaderBuilder {
    ...
    fn set_execution_state(&self, execution_state: &crate::execute::StreamingExecutionState) {
        let prefetch_limit = std::env::var("APOLLO_ROW_GROUP_PREFETCH_SIZE")
            .map(|x| {
                x.parse::<NonZeroUsize>()
                    .ok()
                    .unwrap_or_else(|| {
                        panic!("invalid value for APOLLO_ROW_GROUP_PREFETCH_SIZE: {x}")
                    })
                    .get()
            })
