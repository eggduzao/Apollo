...
use apollo_core::prelude::ArrowSchema;
use apollo_core::schema::{Schema, SchemaExt, SchemaRef};
use apollo_error::{ApolloResult, apollo_err};
use apollo_io::cloud::CloudOptions;
use apollo_io::predicates::ScanIOPredicate;
use apollo_io::prelude::{FileMetadata, ParquetOptions};
use apollo_io::utils::byte_source::{BufferByteSource, DynByteSource, DynByteSourceBuilder};
use apollo_io::{RowIndex, pl_async};
use apollo_parquet::read::schema::infer_schema_with_options;
use apollo_plan::dsl::ScanSource;
use apollo_utils::IdxSize;
use apollo_utils::mem::prefetch::get_memory_prefetch_func;
use apollo_utils::slice_enum::Slice;
...
impl FileReader for ParquetFileReader {
    async fn initialize(&mut self) -> ApolloResult<()> {
        ...
        let file_metadata = if let Some(v) = self.metadata.clone() {
            ...
            Arc::new(apollo_parquet::parquet::read::deserialize_metadata(
    ...
    fn prepare_read(&mut self) -> ApolloResult<()> {
    ...
    fn begin_read(
        &mut self,
        args: BeginReadArgs,
    ) -> ApolloResult<(FileReaderOutputRecv, JoinHandle<ApolloResult<()>>)> {
        let verbose = self.verbose;
        ...
        let mut projected_arrow_fields = || {
            ...
            ApolloResult::Ok(_projected_arrow_fields.as_ref().unwrap().clone())
        };
        ...
        let target_values_per_thread =
            std::env::var("APOLLO_PARQUET_DECODE_TARGET_VALUES_PER_THREAD")
    ...
    async fn file_schema(&mut self) -> ApolloResult<SchemaRef> {
        Ok(self._file_schema().clone())
    }

    async fn file_arrow_schema(&mut self) -> ApolloResult<Option<ArrowSchemaRef>> {
        Ok(Some(self._file_arrow_schema().clone()))
    }

    async fn n_rows_in_file(&mut self) -> ApolloResult<IdxSize> {
        self._n_rows_in_file()
    }

    async fn fast_n_rows_in_file(&mut self) -> ApolloResult<Option<IdxSize>> {
...
impl ParquetFileReader {
    ...
    fn _n_rows_in_file(&self) -> ApolloResult<IdxSize> {
        let n = self.init_data.as_ref().unwrap().file_metadata.num_rows;
        IdxSize::try_from(n).map_err(|_| apollo_err!(bigidx, ctx = "parquet file", size = n))
    }

    fn _row_position_after_slice(&self, pre_slice: Option<Slice>) -> ApolloResult<IdxSize> {
...
type AsyncTaskData = (
    ...
    tokio_handle_ext::AbortOnDropHandle<ApolloResult<()>>,
);
