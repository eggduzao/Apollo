...
use apollo_core::utils::arrow::bitmap::Bitmap;
use apollo_error::ApolloResult;
use apollo_io::predicates::ScanIOPredicate;
use apollo_io::prelude::{FileMetadata, create_sorting_map};
use apollo_io::utils::byte_source::{ByteSource, DynByteSource};
use apollo_parquet::read::RowGroupMetadata;
...
impl RowGroupDataFetcher {
    pub(super) async fn next(
        &mut self,
    ) -> Option<ApolloResult<tokio_handle_ext::AbortOnDropHandle<ApolloResult<RowGroupData>>>> {
            ...
            let handle = io_runtime.spawn(async move {
                    ...
                    current_byte_source.as_ref()
                {
                    ...
                    if memory_prefetch_func as usize
                        != apollo_utils::mem::prefetch::no_prefetch as *const () as usize
                    {
