...
use apollo_core::utils::Container;
use apollo_ops::frame::merge_join::*;
use apollo_ops::frame::{JoinArgs, JoinType, MaintainOrderJoin};
...
pub const KEY_COL_NAME: &str = "__APOLLO_JOIN_KEY_TMP";
...
) -> ApolloResult<Result<UnitVec<(DataFrameBuffer, DataFrameBuffer)>, NeedMore>> {
...
fn find_mergeable_limiting(
    ...
    probe: &mut DataFrameBuffer,
    fmp: FindMergeableParams,
) -> ApolloResult<Result<(DataFrameBuffer, DataFrameBuffer), NeedMore>> {
...
fn find_mergeable_partition(
    ...
    probe: DataFrameBuffer,
    fmp: FindMergeableParams,
) -> ApolloResult<UnitVec<(DataFrameBuffer, DataFrameBuffer)>> {
    let morsel_size = get_ideal_morsel_size();
