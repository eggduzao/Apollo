...
use apollo_core::frame::builder::DataFrameBuilder;
use apollo_core::prelude::*;
use apollo_core::schema::{Schema, SchemaExt};
use apollo_core::{POOL, config};
use apollo_expr::hash_keys::HashKeys;
use apollo_expr::idx_table::{IdxTable, new_idx_table};
use apollo_io::pl_async::get_runtime;
use apollo_ops::frame::{JoinArgs, JoinBuildSide, JoinType, MaintainOrderJoin};
use apollo_ops::series::coalesce_columns;
use apollo_utils::cardinality_sketch::CardinalitySketch;
use apollo_utils::hashing::HashPartitioner;
use apollo_utils::itertools::Itertools;
use apollo_utils::pl_str::PlSmallStr;
use apollo_utils::priority::Priority;
use apollo_utils::relaxed_cell::RelaxedCell;
use apollo_utils::sparse_init_vec::SparseInitVec;
use apollo_utils::{IdxSize, format_pl_smallstr};
use rayon::prelude::*;
...
fn compute_payload_selector(
    ...
    is_left: bool,
    args: &JoinArgs,
) -> ApolloResult<Vec<Option<PlSmallStr>>> {
    ...
    this.iter_names()
        .map(|c| {
            ...
            loop {
                let selector = if args.how == JoinType::Right {
                    ...
                    if is_left {
                        ...
                        let name = format_pl_smallstr!("__APOLLO_COALESCE_KEYCOL_{key_idx}");
                        Some(name)
                    } else {
                        None
                    }
            ...
            if other.contains(&suffixed) {
                apollo_bail!(Duplicate: "column with name '{suffixed}' already exists\n\n\
                You may want to try:\n\
        ...
        let new_cols = df
            ...
            .filter_map(|c| {
                if let Some(key_idx) = params.left_key_schema.index_of(c.name()) {
                    let other = df
                        .column(&format_pl_smallstr!("__APOLLO_COALESCE_KEYCOL_{key_idx}"))
                        .unwrap();
                    return Some(coalesce_columns(&[c.clone(), other.clone()]).unwrap());
                }

                if c.name().starts_with("__APOLLO_COALESCE_KEYCOL") {
                    return None;
                }
        ...
        let sample_cardinality = morsels[..to_process_end]
            ...
            .try_fold(
                ...
                |mut sketch, (morsel_idx, morsel)| {
                    ...
                    ApolloResult::Ok(sketch)
                },
            )
            .map(|sketch| ApolloResult::Ok(sketch?.estimate()))
...
impl SampleState {
    ...
    fn try_transition_to_build(
    ...
    ) -> ApolloResult<Option<BuildState>> {
        ...
        let estimate_cardinalities = || {
            ...
            ApolloResult::Ok((left_cardinality, right_cardinality))
        };
        ...
        if !sampled_build_morsels.is_empty() {
            crate::async_executor::task_scope(|scope| {
                ...
                apollo_io::pl_async::get_runtime().block_on(async move {
        ...
        let left_is_build = match args.maintain_order {
            ...
            MaintainOrderJoin::Left | MaintainOrderJoin::LeftRight => {
                if args.build_side == Some(JoinBuildSide::ForceLeft) {
                    apollo_warn!("can't force left build-side with left-maintaining cross-join");
                }
                Some(false)
            },
            MaintainOrderJoin::Right | MaintainOrderJoin::RightLeft => {
                if args.build_side == Some(JoinBuildSide::ForceRight) {
                    apollo_warn!("can't force right build-side with right-maintaining cross-join");
                }
                Some(true)
            },
        };
...
impl ComputeNode for EquiJoinNode {
    ...
    fn spawn<'env, 's>(
        ...
        join_handles: &mut Vec<JoinHandle<ApolloResult<()>>>,
    ) {
