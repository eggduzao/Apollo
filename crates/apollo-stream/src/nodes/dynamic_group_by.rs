...
use apollo_core::prelude::{Column, DataType, GroupsType, Int64Chunked, IntoColumn, TimeUnit};
use apollo_core::schema::Schema;
use apollo_core::series::IsSorted;
use apollo_error::{ApolloError, ApolloResult, apollo_bail, apollo_ensure};
use apollo_expr::state::ExecutionState;
use apollo_time::prelude::{GroupByDynamicWindower, Label, ensure_duration_matches_dtype};
use apollo_time::{DynamicGroupOptions, LB_NAME, UB_NAME};
...
impl DynamicGroupBy {
    pub fn new(
        ...
        apollo_ensure!(!every.negative(), ComputeError: "'every' argument must be positive");
    ...
    fn next_windows(&mut self, finalize: bool) -> ApolloResult<Option<NextWindows>> {
        ...
        join_handles.extend(rxs.into_iter().zip(send).map(|(mut rx, mut tx)| {
            ...
            scope.spawn_task(TaskPriority::High, async move {
                while let Ok((mut morsel, windows, lower_bound, upper_bound)) = rx.recv().await {
                    morsel = morsel
                        .async_try_map::<ApolloError, _, _>(async |df| {
                            Self::evaluate_one(
                                windows,
                                lower_bound,
                                upper_bound,
