...
use apollo_plan::dsl::file_provider::FileProviderType;
use apollo_plan::dsl::{FileWriteFormat, SinkTarget, UnifiedSinkArgs};
...
impl IOSinkNodeConfig {
    ...
    pub fn inflight_morsel_limit(&self, num_pipelines: NonZeroUsize) -> NonZeroUsize {
        if let Ok(v) = std::env::var("APOLLO_INFLIGHT_SINK_MORSEL_LIMIT").map(|x| {
            x.parse::<NonZeroUsize>().ok().unwrap_or_else(|| {
                panic!("invalid value for APOLLO_INFLIGHT_SINK_MORSEL_LIMIT: {x}")
            })
        }) {
            return v;
        };
    ...
    pub fn max_open_sinks(&self) -> NonZeroUsize {
        if let Ok(v) = std::env::var("APOLLO_MAX_OPEN_SINKS").map(|x| {
            x.parse::<NonZeroUsize>()
                .ok()
                .unwrap_or_else(|| panic!("invalid value for APOLLO_MAX_OPEN_SINKS: {x}"))
        }) {
            return v;
        }
    ...
    pub fn cloud_upload_chunk_size(&self) -> usize {
        apollo_io::get_upload_chunk_size()
    }

    pub fn partitioned_cloud_upload_chunk_size(&self) -> usize {
        if let Ok(v) = std::env::var("APOLLO_PARTITIONED_UPLOAD_CHUNK_SIZE").map(|x| {
            x.parse::<NonZeroUsize>()
                .ok()
                .unwrap_or_else(|| {
                    panic!("invalid value for APOLLO_PARTITIONED_UPLOAD_CHUNK_SIZE: {x}")
                })
                .get()
        }) {
            return v;
    ...
    pub fn upload_concurrency(&self) -> usize {
        apollo_io::get_upload_concurrency()
    }

    pub fn partitioned_upload_concurrency(&self) -> usize {
        if let Ok(v) = std::env::var("APOLLO_PARTITIONED_UPLOAD_CONCURRENCY").map(|x| {
            x.parse::<NonZeroUsize>()
                .ok()
                .unwrap_or_else(|| {
                    panic!("invalid value for APOLLO_PARTITIONED_UPLOAD_CONCURRENCY: {x}")
                })
                .get()
        }) {
            return v;
