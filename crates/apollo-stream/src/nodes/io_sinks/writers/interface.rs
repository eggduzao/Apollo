...
use apollo_io::utils::file::Writeable;
use apollo_io::utils::sync_on_close::SyncOnCloseType;
use apollo_utils::IdxSize;
use apollo_utils::index::NonZeroIdxSize;
...
pub const IPC_RW_RECORD_BATCH_FLAGS_KEY: PlSmallStr =
    PlSmallStr::from_static("apollo:statistics:v1");
...
pub trait FileWriterStarter: Send + Sync + 'static {
    ...
    ) -> ApolloResult<async_executor::JoinHandle<ApolloResult<()>>>;
}

pub struct FileOpenTaskHandle {
    handle: tokio_handle_ext::AbortOnDropHandle<ApolloResult<Writeable>>,
...
impl std::future::Future for FileOpenTaskHandle {
    type Output = ApolloResult<(Writeable, SyncOnCloseType)>;
    ...
    fn poll(
        ...
        let file: ApolloResult<Writeable> = file.unwrap();
...
pub(super) fn ideal_sink_morsel_size_env() -> (Option<IdxSize>, Option<u64>) {
    let num_rows = std::env::var("APOLLO_IDEAL_SINK_MORSEL_SIZE_ROWS")
        .map(|x| {
            x.parse::<NonZeroIdxSize>()
                .ok()
                .unwrap_or_else(|| {
                    panic!("invalid value for APOLLO_IDEAL_SINK_MORSEL_SIZE_ROWS: {x}")
                })
                .get()
        })
        .ok();

    let num_bytes = std::env::var("APOLLO_IDEAL_SINK_MORSEL_SIZE_BYTES")
        .map(|x| {
            x.parse::<NonZeroU64>()
                .ok()
                .unwrap_or_else(|| {
                    panic!("invalid value for APOLLO_IDEAL_SINK_MORSEL_SIZE_BYTES: {x}")
                })
                .get()
        })
        .ok();
