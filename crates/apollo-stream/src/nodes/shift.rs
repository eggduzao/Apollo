    ...
    fn update_state(
        ...
        if recv[1..].iter().all(|p| *p == PortState::Done) {
            ...
            } = self
            {
                ...
                apollo_ensure!(offset_frame.height() == 1, ComputeError: "got more than one value for 'n' in shift");
                ...
                let offset = if offset_item.is_null() {
                    ...
                    *fill = None;
                    ...
                    offset_item.extract::<i64>().ok_or_else(
                        || apollo_err!(ComputeError: "invalid value of 'n' in shift: {:?}", offset_item),
                    )?
                };
