...
use apollo_core::prelude::row_encode::_get_rows_encoded;
...
use apollo_utils::sort::ReorderWithNulls;
...
struct PrimitiveBottomK<T: ApolloNumericType, const REVERSE: bool, const NULLS_LAST: bool> {
    inner: BottomKWithPayload<
        ReorderWithNulls<TotalOrdWrap<T::Physical<'static>>, REVERSE, NULLS_LAST>,
    >,
}

impl<T: ApolloNumericType, const REVERSE: bool, const NULLS_LAST: bool>
...
impl<T: ApolloNumericType, const REVERSE: bool, const NULLS_LAST: bool> DfByKeyReducer
    ...
    if key_schema.len() == 1 {
        ...
        match dt {
            dt if dt.is_primitive_numeric() | dt.is_temporal() | dt.is_decimal() | dt.is_enum() => {
                return with_match_physical_numeric_apollo_type!(dt.to_physical(), |$T| {
    ...
    fn update_state(
        ...
        match &mut self.state {
            ...
            TopKState::WaitingForK(inner) if recv[1] == PortState::Done => {
                ...
                apollo_ensure!(k_frame.height() == 1, ComputeError: "got more than one value for 'k' in top_k");
                let k_item = k_frame.columns()[0].get(0)?;
                let k = k_item.extract::<usize>().ok_or_else(
                    || apollo_err!(ComputeError: "invalid value of 'k' in top_k: {:?}", k_item),
                )?;
