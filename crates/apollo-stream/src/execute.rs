...
use apollo_utils::aliases::PlHashSet;
use apollo_utils::relaxed_cell::RelaxedCell;
use apollo_utils::reuse_vec::reuse_vec;
...
pub struct StreamingExecutionState {
    ...
    query_tasks_send: Sender<JoinHandle<ApolloResult<()>>>,
    subphase_tasks_send: Sender<JoinHandle<ApolloResult<()>>>,
}

impl StreamingExecutionState {
    /// Spawns a task which is awaited at the end of the query.
    #[allow(unused)]
    pub fn spawn_query_task<F: Future<Output = ApolloResult<()>> + Send + 'static>(&self, fut: F) {
        self.query_tasks_send
            .send(apollo_io::pl_async::get_runtime().spawn(fut))
            .unwrap();
    }
    ...
    /// complete.
    pub fn spawn_subphase_task<F: Future<Output = ApolloResult<()>> + Send + 'static>(
        &self,
        fut: F,
    ) {
...
pub fn execute_graph(
    graph: &mut Graph,
    metrics: Option<Arc<Mutex<GraphMetrics>>>,
) -> ApolloResult<SparseSecondaryMap<GraphNodeKey, DataFrame>> {
    ...
    loop {
        ...
        if apollo_core::config::verbose() {
            eprintln!("apollo-stream: updating graph state");
        }
        graph.update_all_states(&state, metrics.as_deref())?;
        apollo_io::pl_async::get_runtime().block_on(async {
        ...
        if apollo_core::config::verbose() {
            for node in &nodes {
                eprintln!(
                    "apollo-stream: running {} in subgraph",
        ...
        if apollo_core::config::verbose() {
            eprintln!("apollo-stream: done running graph phase");
        }
