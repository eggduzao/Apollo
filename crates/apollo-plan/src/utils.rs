...
/// output name of expr
pub fn expr_output_name(expr: &Expr) -> ApolloResult<PlSmallStr> {
    for e in expr {
        match e {
            ...
            Expr::KeepName(_) => apollo_bail!(nyi = "`name.keep` is not allowed here"),
...
pub fn expr_to_leaf_column_name(expr: &Expr) -> ApolloResult<PlSmallStr> {
    let mut leaves = expr_to_leaf_column_exprs_iter(expr).collect::<Vec<_>>();
    apollo_ensure!(leaves.len() <= 1, ComputeError: "found more than one root column name");
    match leaves.pop() {
        Some(Expr::Column(name)) => Ok(name.clone()),
        Some(Expr::Selector(_)) => apollo_bail!(
...
pub fn merge_schemas(schemas: &[SchemaRef]) -> ApolloResult<Schema> {
    ...
    for schema in schemas {
        schema.iter().try_for_each(|(name, dtype)| {
            if merged_schema.with_column(name.clone(), dtype.clone()).is_none() {
                ...
                Err(apollo_err!(Duplicate: "Column with name '{}' has more than one occurrence", name))
            }
        })?;
    }
