...
use apollo_core::datatypes::{DataType, Field};
use apollo_core::error::*;
use apollo_core::frame::DataFrame;
use apollo_core::frame::column::Column;
...
// Will be overwritten on Python Apollo start up.
#[allow(clippy::type_complexity)]
pub static mut CALL_COLUMNS_UDF_PYTHON: Option<
    fn(s: &[Column], output_dtype: Option<DataType>, lambda: &Py<PyAny>) -> ApolloResult<Column>,
> = None;

#[allow(clippy::type_complexity)]
pub static mut CALL_DF_UDF_PYTHON: Option<
    fn(s: DataFrame, lambda: &Py<PyAny>) -> ApolloResult<DataFrame>,
> = None;

pub use apollo_utils::python_function::PythonFunction;
#[cfg(feature = "serde")]
pub use apollo_utils::python_function::{PYTHON_SERDE_MAGIC_BYTE_MARK, PYTHON3_VERSION};
...
impl PythonUdfExpression {
    ...
    #[cfg(feature = "serde")]
    pub(crate) fn try_deserialize(buf: &[u8]) -> ApolloResult<Arc<dyn AnonymousColumnsUdf>> {
        use apollo_utils::pl_serialize;

        if !buf.starts_with(PYTHON_SERDE_MAGIC_BYTE_MARK) {
            apollo_bail!(InvalidOperation: "serialization expected python magic byte mark");
        }
...
impl DataFrameUdf for apollo_utils::python_function::PythonFunction {
...
impl ColumnsUdf for PythonUdfExpression {
    fn call_udf(&self, s: &mut [Column]) -> ApolloResult<Column> {
...
impl AnonymousColumnsUdf for PythonUdfExpression {
    ...
    #[cfg(feature = "serde")]
    fn try_serialize(&self, buf: &mut Vec<u8>) -> ApolloResult<()> {
        use apollo_utils::pl_serialize;
    ...
    fn get_field(&self, input_schema: &Schema, fields: &[Field]) -> ApolloResult<Field> {
