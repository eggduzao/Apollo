...
/// When using the apollo lazy API, don't construct an `Expr` directly; instead, create one using
/// the functions in the `apollo_lazy::dsl` module. See that module's docs for more info.
...
pub enum Expr {
    ...
    /// Apollo flavored window functions.
    Over {
...
impl Expr {
    ...
    pub fn to_field(&self, schema: &Schema) -> ApolloResult<Field> {
    ...
    pub fn try_into_selector(self) -> ApolloResult<Selector> {
        match self {
            ...
            expr => Err(apollo_err!(InvalidOperation: "cannot turn `{expr}` into selector")),
        }
    }

    /// Extract a constant usize from an expression.
    pub fn extract_usize(&self) -> ApolloResult<usize> {
        match self {
            ...
            Expr::Cast { expr, dtype, .. } => {
                ...
                if dtype.as_literal().is_some_and(|dt| dt.is_integer()) {
                    ...
                    apollo_bail!(InvalidOperation: "expression must be constant literal to extract integer")
                }
            },
            _ => {
                apollo_bail!(InvalidOperation: "expression must be constant literal to extract integer")
    ...
    pub fn extract_i64(&self) -> ApolloResult<i64> {
        match self {
    ...
    pub fn try_map_n_ary(
        ...
        exprs: impl IntoIterator<Item = ApolloResult<Expr>>,
    ) -> ApolloResult<Expr> {
...
impl EvalVariant {
    ...
    pub fn element_dtype<'a>(&self, dtype: &'a DataType) -> ApolloResult<&'a DataType> {
        match (self, dtype) {
            ...
            _ => apollo_bail!(op = self.to_name(), dtype),
        }
    }
...
impl RenameAliasFn {
    pub fn call(&self, name: &PlSmallStr) -> ApolloResult<PlSmallStr> {
        let out = match self {
            ...
            Self::Replace {
                ...
                if *literal {
                    ...
                    feature_gated!("regex", {
                        let rx = apollo_utils::regex_cache::compile_regex(pattern)?;
...
pub type RenameAliasRustFn =
    dyn Fn(&PlSmallStr) -> ApolloResult<PlSmallStr> + 'static + Send + Sync;
