...
use apollo_error::*;
use apollo_utils::pl_serialize::deserialize_map_bytes;
...
fn deserialize_named_registry(buf: &[u8]) -> ApolloResult<(Arc<dyn ExprRegistry>, &str, &[u8])> {
    let bytes = &buf[NAMED_SERDE_MAGIC_BYTE_MARK.len()..];
    let Some(pos) = bytes.iter().position(|b| *b == NAMED_SERDE_MAGIC_BYTE_END) else {
        apollo_bail!(ComputeError: "named-serde expected magic byte end")
    };

    let Ok(name) = std::str::from_utf8(&bytes[..pos]) else {
        apollo_bail!(ComputeError: "named-serde name should be valid utf8")
    };
    ...
    match &*registry {
        ...
        None => apollo_bail!(ComputeError: "named serde registry not set"),
    }
}
...
pub(super) fn deserialize_column_udf(buf: &[u8]) -> ApolloResult<Arc<dyn AnonymousColumnsUdf>> {
    #[cfg(feature = "python")]
    ...
    if buf.starts_with(NAMED_SERDE_MAGIC_BYTE_MARK) {
        ...
        apollo_bail!(ComputeError: "deserialization not supported for this 'opaque' function")
    }
}
...
pub(super) fn deserialize_anon_agg(buf: &[u8]) -> ApolloResult<Arc<dyn AnonymousAgg>> {
    if buf.starts_with(NAMED_SERDE_MAGIC_BYTE_MARK) {
        let (reg, name, payload) = deserialize_named_registry(buf)?;
