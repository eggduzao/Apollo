...
pub trait AnonymousColumnsUdf: ColumnsUdf {
    ...
    fn try_serialize(&self, _buf: &mut Vec<u8>) -> ApolloResult<()> {
        apollo_bail!(ComputeError: "serialization not supported for this 'opaque' function")
    }

    fn get_field(&self, input_schema: &Schema, fields: &[Field]) -> ApolloResult<Field>;
}

/// A wrapper trait for any closure `Fn(Vec<Series>) -> ApolloResult<Series>`
...
where
    F: Fn(&mut [Column]) -> ApolloResult<Column> + Send + Sync,
{
    fn call_udf(&self, s: &mut [Column]) -> ApolloResult<Column> {
        self(s)
    }
...
where
    F: Fn(&mut [Column]) -> ApolloResult<Column> + Send + Sync,
    DT: Fn(&Schema, &[Field]) -> ApolloResult<Field> + Send + Sync,
{
    fn call_udf(&self, s: &mut [Column]) -> ApolloResult<Column> {
        (self.f)(s)
    }
...
impl OpaqueColumnUdf {
    pub fn materialize(self) -> ApolloResult<SpecialEq<Arc<dyn AnonymousColumnsUdf>>> {
        match self {
