...
impl TryFrom<&SerializableDslPlan> for DslPlan {
    ...
    fn try_from(ser_dsl_plan: &SerializableDslPlan) -> Result<Self, Self::Error> {
        ...
        let root = ser_dsl_plan
            ...
            .ok_or(apollo_err!(ComputeError: "Could not find root DslPlan in serialized plan"))?;
...
fn get_dataframe(
    ...
    ser_dsl_plan: &SerializableDslPlan,
    arenas: &mut DeserializeArenas,
) -> Result<Arc<DataFrame>, ApolloError> {
    if let Some(df) = arenas.dataframes.get(key) {
        Ok(df.0.clone())
    } else {
        let df = ser_dsl_plan.dataframes.get(key).ok_or(apollo_err!(
...
fn get_dsl_plan(
    ...
    ser_dsl_plan: &SerializableDslPlan,
    arenas: &mut DeserializeArenas,
) -> Result<Arc<DslPlan>, ApolloError> {
    if let Some(dsl_plan) = arenas.dsl_plans.get(key) {
        Ok(dsl_plan.clone())
    } else {
        let node = ser_dsl_plan.dsl_plans.get(key).ok_or(apollo_err!(
...
mod _serde_impl {
    ...
    use apollo_utils::chunked_bytes_cursor::FixedSizeChunkedBytesCursor;
    use serde::de::Error;
    use serde::{Deserialize, Serialize};
    ...
    fn max_byte_slice_len() -> usize {
        std::env::var("APOLLO_SERIALIZE_LAZYFRAME_MAX_BYTE_SLICE_LEN")
            .as_deref()
            .map_or(
