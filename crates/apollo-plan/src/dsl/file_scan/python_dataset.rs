...
/// This is for `apollo-python` to inject so that the implementation can be done there:
...
pub struct PythonDatasetProviderVTable {
    ...
    pub schema: fn(dataset_object: &PythonObject) -> ApolloResult<SchemaRef>,
    ...
    ) -> ApolloResult<Option<(DslPlan, PlSmallStr)>>,
}
...
impl PythonDatasetProvider {
    ...
    pub fn schema(&self) -> ApolloResult<SchemaRef> {
