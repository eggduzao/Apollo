...
use apollo_core::error::{ApolloResult, feature_gated};
use apollo_error::apollo_err;
use apollo_io::cloud::CloudOptions;
#[cfg(feature = "cloud")]
use apollo_io::file_cache::FileCacheEntry;
use apollo_io::utils::byte_source::{DynByteSource, DynByteSourceBuilder};
use apollo_io::{expand_paths, expand_paths_hive, expanded_from_single_directory};
use apollo_utils::mmap::MMapSemaphore;
...
impl ScanSources {
    pub async fn expand_paths(&self, scan_args: &mut UnifiedScanArgs) -> ApolloResult<Self> {
        match self {
    ...
    pub fn first_or_empty_expand_err(
        ...
        glob: bool,
        hint: &'static str,
    ) -> ApolloResult<ScanSourceRef<'_>> {
        let hint_padding = if hint.is_empty() { "" } else { " Hint: " };

        self.first().ok_or_else(|| match self {
            Self::Paths(_) if !sources_before_expansion.is_empty() => apollo_err!(
                ComputeError:
...
impl ScanSourceRef<'_> {
    ...
    pub fn into_owned(&self) -> ApolloResult<ScanSource> {
        Ok(match self {
    ...
    pub fn to_memslice(&self) -> ApolloResult<Buffer<u8>> {
        self.to_buffer_possibly_async(false, None, 0)
    }

    #[allow(clippy::wrong_self_convention)]
    #[cfg(feature = "cloud")]
    fn to_buffer_async<F: Fn(Arc<FileCacheEntry>) -> ApolloResult<std::fs::File>>(
        &self,
        open_cache_entry: F,
        run_async: bool,
    ) -> ApolloResult<Buffer<u8>> {
        match self {
            ScanSourceRef::Path(path) => {
                let file = if run_async {
                    open_cache_entry(
                        apollo_io::file_cache::FILE_CACHE
    ...
    #[cfg(feature = "cloud")]
    pub fn to_buffer_async_assume_latest(&self, run_async: bool) -> ApolloResult<Buffer<u8>> {
        self.to_buffer_async(|entry| entry.try_open_assume_latest(), run_async)
    }

    #[cfg(feature = "cloud")]
    pub fn to_buffer_async_check_latest(&self, run_async: bool) -> ApolloResult<Buffer<u8>> {
        self.to_buffer_async(|entry| entry.try_open_check_latest(), run_async)
    }

    #[cfg(not(feature = "cloud"))]
    #[allow(clippy::wrong_self_convention)]
    fn to_buffer_async(&self, run_async: bool) -> ApolloResult<Buffer<u8>> {
        match self {
            ScanSourceRef::Path(path) => {
                let file = apollo_utils::open_file(path.as_std_path())?;
                Ok(Buffer::from_owner(MMapSemaphore::new_from_file(&file)?))
            },
            ScanSourceRef::File(file) => {
                Ok(Buffer::from_owner(MMapSemaphore::new_from_file(file)?))
    ...
    pub fn to_buffer_possibly_async(
        ...
        #[cfg(feature = "cloud")] cache_entries: Option<
            &Vec<Arc<apollo_io::file_cache::FileCacheEntry>>,
        >,
    ...
    pub fn run_async(&self) -> bool {
        matches!(self, Self::Path(p) if p.has_scheme() || apollo_core::config::force_async())
    }
}
