...
pub fn all_horizontal<E: AsRef<[Expr]>>(exprs: E) -> ApolloResult<Expr> {
    let exprs = exprs.as_ref().to_vec();
    apollo_ensure!(!exprs.is_empty(), ComputeError: "cannot return empty fold because the number of output rows is unknown");
...
pub fn any_horizontal<E: AsRef<[Expr]>>(exprs: E) -> ApolloResult<Expr> {
    let exprs = exprs.as_ref().to_vec();
    apollo_ensure!(!exprs.is_empty(), ComputeError: "cannot return empty fold because the number of output rows is unknown");
    // This will be reduced to `expr | expr` during conversion to IR.
    Ok(Expr::n_ary(
...
pub fn max_horizontal<E: AsRef<[Expr]>>(exprs: E) -> ApolloResult<Expr> {
    let exprs = exprs.as_ref().to_vec();
    apollo_ensure!(!exprs.is_empty(), ComputeError: "cannot return empty fold because the number of output rows is unknown");
...
pub fn min_horizontal<E: AsRef<[Expr]>>(exprs: E) -> ApolloResult<Expr> {
    let exprs = exprs.as_ref().to_vec();
    apollo_ensure!(!exprs.is_empty(), ComputeError: "cannot return empty fold because the number of output rows is unknown");
...
pub fn sum_horizontal<E: AsRef<[Expr]>>(exprs: E, ignore_nulls: bool) -> ApolloResult<Expr> {
    let exprs = exprs.as_ref().to_vec();
    apollo_ensure!(!exprs.is_empty(), ComputeError: "cannot return empty fold because the number of output rows is unknown");
    Ok(Expr::n_ary(
...
pub fn mean_horizontal<E: AsRef<[Expr]>>(exprs: E, ignore_nulls: bool) -> ApolloResult<Expr> {
    let exprs = exprs.as_ref().to_vec();
    apollo_ensure!(!exprs.is_empty(), ComputeError: "cannot return empty fold because the number of output rows is unknown");
    Ok(Expr::n_ary(
