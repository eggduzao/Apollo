...
pub fn format_str<E: AsRef<[Expr]>>(format: &str, args: E) -> ApolloResult<Expr> {
...
pub fn concat_list<E: AsRef<[IE]>, IE: Into<Expr> + Clone>(s: E) -> ApolloResult<Expr> {
    let s: Vec<_> = s.as_ref().iter().map(|e| e.clone().into()).collect();

    apollo_ensure!(!s.is_empty(), ComputeError: "`concat_list` needs one or more expressions");
...
pub fn concat_arr(input: Vec<Expr>) -> ApolloResult<Expr> {
    feature_gated!("dtype-array", {
        apollo_ensure!(!input.is_empty(), ComputeError: "`concat_arr` needs one or more expressions");
...
pub fn concat_expr<E: AsRef<[IE]>, IE: Into<Expr> + Clone>(
    ...
    let s: Vec<_> = s.as_ref().iter().map(|e| e.clone().into()).collect();
    apollo_ensure!(!s.is_empty(), ComputeError: "`concat_expr` needs one or more expressions");
