...
impl ArrayNameSpace {
    ...
    pub fn slice(self, offset: Expr, length: Expr, as_array: bool) -> ApolloResult<Expr> {
        if as_array {
            let Ok(offset) = offset.extract_i64() else {
                apollo_bail!(InvalidOperation: "Offset must be a constant `i64` value if `as_array=true`, got: {}", offset)
            };
            let Ok(length) = length.extract_i64() else {
                apollo_bail!(InvalidOperation: "Length must be a constant `i64` value if `as_array=true`, got: {}", length)
            };
            Ok(self
                .0
    ...
    pub fn head(self, n: Expr, as_array: bool) -> ApolloResult<Expr> {
        self.slice(lit(0), n, as_array)
    }

    /// Get the tail of every subarray
    pub fn tail(self, n: Expr, as_array: bool) -> ApolloResult<Expr> {
