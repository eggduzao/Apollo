...
use apollo_core::error::{ApolloResult, feature_gated, apollo_bail, apollo_ensure};
use apollo_core::prelude::{DataType, Field};
use apollo_core::schema::Schema;
use apollo_utils::arena::Arena;
...
fn into_datatype_impl(
    ...
    let dtype = match dt_expr {
        ...
        D::OfExpr(expr) => {
            ...
            apollo_ensure!(!dtype.contains_unknown(),InvalidOperation:"DataType expression is not allowed to instantiate to `unknown`");
            dtype
        },
        ...
        D::InnerDataType { input, validation } => {
            ...
            match (dt, validation) {
                ...
                (dt, SequenceKind::Array) => {
                    apollo_bail!(SchemaMismatch: "expected `arr` type but got `{dt}`")
                },
                (dt, SequenceKind::List) => {
                    apollo_bail!(SchemaMismatch: "expected `list` type but got `{dt}`")
                },
            }
        },
        D::Int(dt_expr, f) => {
            ...
            apollo_ensure!(dt.is_integer(), InvalidOperation: "`{dt}` is not an integer type");
            match f {
                IntDataTypeExpr::ToUnsigned => match dt {
                    ...
                    DT::Int128 => {
                        apollo_bail!(InvalidOperation: "`int128` has no unsigned equivalent")
                    },
                    _ => unreachable!(),
                },
        ...
        D::Struct(dt_expr, f) => {
            let fields: Vec<Field> = match into_datatype_impl(*dt_expr, schema, self_dtype)? {
                ...
                dt => apollo_bail!(InvalidOperation: "`{dt}` is not a `struct`"),
            };
            match f {
        ...
        D::StructWithFields(field_exprs) => feature_gated!("dtype-struct", {
            use apollo_core::prelude::{Field, InitHashMaps, PlHashSet};
...
impl DataTypeExpr {
    pub fn into_datatype(self, schema: &Schema) -> ApolloResult<DataType> {
