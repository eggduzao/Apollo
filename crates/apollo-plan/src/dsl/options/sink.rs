...
use apollo_core::prelude::PlHashSet;
...
impl SinkTarget {
    ...
    ) -> ApolloResult<Writeable> {
        match self {
            SinkTarget::Path(path) => {
                if mkdir {
                    apollo_io::utils::mkdir::mkdir_recursive(path)?;
                }

                apollo_io::utils::file::Writeable::try_new(
                    path.clone(),
                    cloud_options,
    ...
    pub async fn open_into_writeable_async(
        ...
        #[cfg(feature = "cloud")]
        {
            match self {
                SinkTarget::Path(path) => {
                    if mkdir {
                        apollo_io::utils::mkdir::tokio_mkdir_recursive(path).await?;
                    }
