...
impl Selector {
    ...
    pub fn into_columns(
        ...
        schema: &Schema,
        ignored_columns: &PlHashSet<PlSmallStr>,
    ) -> ApolloResult<PlIndexSet<PlSmallStr>> {
        let out = match self {
            Self::Union(lhs, rhs) => {
            ...
            Self::ByName { names, strict } => {
                ...
                for name in names.iter() {
                    if schema.contains(name) {
                        ...
                        apollo_bail!(col_not_found = name);
                    }
                }
                out
            },
            Self::ByIndex { indices, strict } => {
                ...
                for &idx in indices.iter() {
                    let Some(idx) = idx.negative_to_usize(schema.len()) else {
                        apollo_ensure!(!strict, ColumnNotFound: "cannot get the {idx}-th column when schema has {} columns", schema.len());
                        continue;
                    };
                    let (name, _) = schema.get_at_index(idx).unwrap();
                    if !set.insert(idx) {
                        apollo_bail!(Duplicate: "duplicate column name {name}");
                    }
                    out.insert(name.clone());
                }
                out
            ...
            Self::Matches(regex_str) => {
                let re = apollo_utils::regex_cache::compile_regex(regex_str).map_err(
                    |_| apollo_err!(InvalidOperation: "invalid regex in selector '{regex_str}'"),
                )?;
                PlIndexSet::from_iter(
                    schema
                        .iter_names()
