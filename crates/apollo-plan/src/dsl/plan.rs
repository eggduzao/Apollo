...
pub enum DslPlan {
    ...
    /// Apollo' `select` operation, this can mean projection, but also full data access.
    Select {
        expr: Vec<Expr>,
        input: Arc<DslPlan>,
...
impl DslPlan {
    pub fn describe(&self) -> ApolloResult<String> {
        Ok(self.clone().to_alp()?.describe())
    }

    pub fn describe_tree_format(&self) -> ApolloResult<String> {
        Ok(self.clone().to_alp()?.describe_tree_format())
    }

    pub fn display(&self) -> ApolloResult<impl fmt::Display> {
    ...
    pub fn to_alp(self) -> ApolloResult<IRPlan> {
    ...
    pub fn serialize_versioned<W: Write>(
        ...
        apollo_utils::pl_serialize::USE_CLOUDPICKLE.set(ctx.use_cloudpickle);
        ...
        pl_serialize::serialize_dsl(writer, &serializable_plan)
            .map_err(|e| apollo_err!(ComputeError: "serialization failed\n\nerror: {e}"))
    }

    #[cfg(feature = "serde")]
    pub fn deserialize_versioned<R: Read>(mut reader: R) -> ApolloResult<Self> {
        const MAGIC_LEN: usize = DSL_MAGIC_BYTES.len();
        let mut version_magic = [0u8; MAGIC_LEN + 4];
        reader
            .read_exact(&mut version_magic)
            .map_err(|e| apollo_err!(ComputeError: "failed to read incoming DSL_VERSION: {e}"))?;

        if &version_magic[..MAGIC_LEN] != DSL_MAGIC_BYTES {
            apollo_bail!(ComputeError: "dsl magic bytes not found")
        }
        ...
        if major != MAJOR {
            apollo_bail!(ComputeError:
                "deserialization failed\n\ngiven DSL_VERSION: {major}.{minor} is not compatible with this Apollo version which uses DSL_VERSION: {MAJOR}.{MINOR}\n{}",
        ...
        reader.read_exact(&mut schema_hash).map_err(
            |e| apollo_err!(ComputeError: "failed to read incoming DSL_SCHEMA_HASH: {e}"),
        )?;

        let incoming_hash = SchemaHash::new(&schema_hash).ok_or_else(
            || apollo_err!(ComputeError: "failed to read incoming DSL schema hash, not a valid hex string")
        )?;
        ...
        if std::env::var("APOLLO_SKIP_DSL_HASH_VERIFICATION").as_deref() != Ok("1")
            && incoming_hash != DSL_SCHEMA_HASH
        {
            apollo_bail!(ComputeError:
                "deserialization failed\n\ngiven DSL_SCHEMA_HASH: {incoming_hash} is not compatible with this Apollo version which uses DSL_SCHEMA_HASH: {DSL_SCHEMA_HASH}\n{}",
        ...
        let serializable_plan: serializable_plan::SerializableDslPlan =
            pl_serialize::deserialize_dsl(reader)
                .map_err(|e| apollo_err!(ComputeError: "deserialization failed\n\nerror: {e}"))?;
