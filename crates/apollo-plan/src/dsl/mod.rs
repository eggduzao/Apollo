...
pub(crate) use apollo_time::prelude::*;
...
impl Expr {
    ...
    where
        F: Fn(Column) -> ApolloResult<Column> + 'static + Send + Sync,
        DT: Fn(&Schema, &Field) -> ApolloResult<Field> + 'static + Send + Sync,
    {
    ...
    where
        F: Fn(&mut [Column]) -> ApolloResult<Column> + 'static + Send + Sync,
        DT: Fn(&Schema, &[Field]) -> ApolloResult<Field> + 'static + Send + Sync,
    {
    ...
    /// Shift the values in the array by some period. See [the eager implementation](apollo_core::series::SeriesTrait::shift).
    ...
    ///
    /// ``` rust
    /// #[macro_use] extern crate apollo_core;
    /// use apollo_core::prelude::*;
    /// use apollo_lazy::prelude::*;
    ///
    ...
    pub fn over_with_options<E: AsRef<[IE]>, IE: Into<Expr> + Clone>(
        ...
        apollo_ensure!(partition_by.is_some() || order_by.is_some(), InvalidOperation: "At least one of `partition_by` and `order_by` must be specified in `over`");
    ...
    pub fn rolling_quantile_by(
        ...
        use apollo_compute::rolling::{RollingFnParams, RollingQuantileParams};
