...
use apollo_utils::hashing::hash_to_partition;
#[cfg(feature = "serde")]
...
impl DynListLiteralValue {
    pub fn try_materialize_to_dtype(
        ...
        let Some(inner_dtype) = dtype.inner_dtype() else {
            apollo_bail!(InvalidOperation: "conversion from list literal to `{dtype}` failed.");
        };
...
impl RangeLiteralValue {
    pub fn try_materialize_to_series(self, dtype: &DataType) -> ApolloResult<Series> {
        fn handle_range_oob(range: &RangeLiteralValue, to_dtype: &DataType) -> ApolloResult<()> {
            apollo_bail!(
                InvalidOperation:
        ...
        let s = match dtype {
            ...
            _ => apollo_bail!(InvalidOperation: "unsupported range datatype `{dtype}`"),
        };
...
impl LiteralValue {
        ...
        macro_rules! cast_usize {
            ($v:expr) => {
                usize::try_from($v).map_err(
                    |_| apollo_err!(InvalidOperation: "cannot convert value {} to usize", $v)
                )
            }
        }
        match &self {
        ...
        macro_rules! cast_i64 {
            ($v:expr) => {
                i64::try_from($v).map_err(
                    |_| apollo_err!(InvalidOperation: "cannot convert value {} to i64", $v)
                )
            }
        }
        match &self {
    ...
    pub fn implode(self) -> ApolloResult<Self> {
