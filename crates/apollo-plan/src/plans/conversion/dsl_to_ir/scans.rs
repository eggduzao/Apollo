...
use apollo_io::csv::read::streaming::read_until_start_and_infer_schema;
use apollo_io::prelude::*;
use apollo_io::utils::compression::CompressedReader;
...
pub(super) async fn dsl_to_ir(
    ...
    if is_not_cached {
            ...
            && let Some(file_schema) = file_info.reader_schema.as_ref()
        {
            let paths = sources
                .as_paths()
                .ok_or_else(|| apollo_err!(nyi = "Hive-partitioning of in-memory buffers"))?;
...
fn prepare_schemas(
    mut schema: Schema,
    row_index: Option<&RowIndex>,
) -> ApolloResult<(SchemaRef, SchemaRef)> {
...
pub(super) async fn parquet_file_info(
    ...
    row_index: Option<&RowIndex>,
    #[allow(unused)] cloud_options: Option<&apollo_io::cloud::CloudOptions>,
    n_sources: usize,
) -> ApolloResult<(FileInfo, Option<FileMetadataRef>)> {
...
pub fn max_metadata_scan_cached() -> usize {
    static MAX_SCANS_METADATA_CACHED: LazyLock<usize> = LazyLock::new(|| {
        let value = std::env::var("APOLLO_MAX_CACHED_METADATA_SCANS").map_or(8, |v| {
            v.parse::<usize>()
                .expect("invalid `APOLLO_MAX_CACHED_METADATA_SCANS` value")
        });
        if value == 0 {
            return usize::MAX;
        }
...
pub(super) async fn ipc_file_info(
    first_scan_source: ScanSourceRef<'_>,
    row_index: Option<&RowIndex>,
    cloud_options: Option<&apollo_io::cloud::CloudOptions>,
) -> ApolloResult<(FileInfo, arrow::io::ipc::read::FileMetadata)> {
    use apollo_core::error::feature_gated;
    ...
    let metadata = match first_scan_source {
        ScanSourceRef::Path(path) => {
            if path.has_scheme() {
                feature_gated!("cloud", {
                    apollo_io::ipc::IpcReaderAsync::from_uri(path.clone(), cloud_options)
                        .await?
                        .metadata()
                        .await?
                })
                ...
                arrow::io::ipc::read::read_file_metadata(&mut std::io::BufReader::new(
                    apollo_utils::open_file(path.as_std_path())?,
                ))?
            }
        },
...
pub async fn csv_file_info(
    ...
    let cache_entries = {
        if run_async {
            ...
            feature_gated!("cloud", {
                Some(
                    apollo_io::file_cache::init_entries_from_uri_list(
                        (0..sources.len())
    ...
    let merge_func =
        |a: ApolloResult<(Schema, usize)>, b: ApolloResult<(Schema, usize)>| match (a, b) {
...
pub async fn ndjson_file_info(
    ...
    let mut schema = if let Some(schema) = ndjson_options.schema.clone() {
        ...
        Arc::new(apollo_io::ndjson::infer_schema(
            &mut reader,
...
impl SourcesToFileInfo {
    ...
    ) -> ApolloResult<(FileInfo, FileScanIR)> {
...
this scan to succeed with an empty DataFrame.",
                        ...
                        ApolloResult::Ok((file_info, FileScanIR::Parquet { options, metadata }))
                    }
            ...
            FileScanDsl::Csv { mut options } => {
                {
                    ...
                    ApolloResult::Ok((file_info, FileScanIR::Csv { options }))
                }
            ...
            FileScanDsl::NDJson { options } => {
                ...
                ApolloResult::Ok((file_info, FileScanIR::NDJson { options }))
            }
            ...
            FileScanDsl::PythonDataset { dataset_object } => (|| {
                if crate::dsl::DATASET_PROVIDER_VTABLE.get().is_none() {
                    apollo_bail!(ComputeError: "DATASET_PROVIDER_VTABLE (python) not initialized")
                }
