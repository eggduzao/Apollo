...
use super::*;
use crate::constants::{
    APOLLO_ELEMENT, APOLLO_STRUCTFIELDS, get_pl_element_name, get_pl_structfields_name,
};
...
fn expand_expression_rec(
    ...
    match &expr {
        ...
        Expr::Column(column) if schema.contains(APOLLO_ELEMENT) && column.is_empty() => {
            out.push(Expr::Element)
        },
        Expr::Selector(Selector::ByIndex { indices, strict: _ })
            if schema.contains(APOLLO_ELEMENT) && indices.len() == 1 && indices[0] == 0 =>
        {
            out.push(Expr::Element)
        },
        ...
        Expr::Selector(selector) => {
            ...
            if schema.contains(APOLLO_ELEMENT) {
                schema.to_mut().remove(APOLLO_ELEMENT);
            }
            if schema.contains(APOLLO_STRUCTFIELDS) {
                schema.to_mut().remove(APOLLO_STRUCTFIELDS);
            }
        ...
        Expr::Function { input, function } => {
            ...
            if function_expansion.expand_into_input {
                ...
                if expanded_input.is_empty() && !function_expansion.allow_empty_input {
                    ...
                    apollo_bail!(InvalidOperation: "expected at least 1 input in {expr}")
                }
                    ...
                    FunctionExpr::StructExpr(StructFunction::SelectFields(selector)) => {
                        ...
                        for e in tmp_out {
                            ...
                            let DataType::Struct(fields) = dtype else {
                                apollo_bail!(op = "struct.field", &dtype);
                            };
