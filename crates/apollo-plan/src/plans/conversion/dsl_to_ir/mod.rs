...
use apollo_core::config::verbose;
use apollo_io::ExternalCompression;
use apollo_io::pl_async::get_runtime;
use apollo_utils::format_pl_smallstr;
use apollo_utils::itertools::Itertools;
use apollo_utils::pl_path::PlRefPath;
use apollo_utils::unique_id::UniqueId;
...
pub fn to_alp(
    ...
    match to_alp_impl(lp, &mut ctxt) {
        ...
        Err(err) => {
            ...
            let node_name = if let ApolloError::Context { msg, .. } = &err {
                msg
            } else {
                "THIS_NODE"
            };
...
fn run_conversion(lp: IR, ctxt: &mut DslConversionContext, name: &str) -> ApolloResult<Node> {
...
#[recursive]
pub fn to_alp_impl(lp: DslPlan, ctxt: &mut DslConversionContext) -> ApolloResult<Node> {
    ...
    let v = match lp {
        ...
        DslPlan::Union { inputs, args } => {
            ...
            let first_n = *inputs.first().ok_or_else(
                || apollo_err!(InvalidOperation: "expected at least one input in 'union'/'concat'"),
            )?;
            let schema = ctxt.lp_arena.get(first_n).schema(ctxt.lp_arena);
            for n in &inputs[1..] {
                let schema_i = ctxt.lp_arena.get(*n).schema(ctxt.lp_arena);
                // The first argument
                schema_i.matches_schema(schema.as_ref()).map_err(|_| apollo_err!(InvalidOperation:  "'union'/'concat' inputs should all have the same schema,\
                    got\n{:?} and \n{:?}", schema, schema_i)
                )?;
            }
        ...
        DslPlan::Filter { input, predicate } => {
            ...
            if ctxt.opt_flags.predicate_pushdown() {
                ...
                if let Some(SplitPredicates { pushable, fallible }) = SplitPredicates::new(
                    ...
                    let mut update_input = |predicate: Node| -> ApolloResult<()> {
        ...
        DslPlan::GroupBy {
            ...
            if !predicates.is_empty() {
                let predicate_names = (0..predicates.len())
                    .map(|i| format_pl_smallstr!("__APOLLO_HAVING_{i}"))
                    .collect::<Arc<[_]>>();
        ...
        DslPlan::MatchToSchema {
            ...
            if let Some(lst) = found_missing_columns.first() {
                ...
                apollo_bail!(SchemaMismatch: "missing columns in `match_to_schema`: {formatted}");
            }
                ...
                && extra_columns == ExtraColumnsPolicy::Raise
            {
                ...
                apollo_bail!(SchemaMismatch: "extra columns in `match_to_schema`: {formatted}");
            }
        ...
        DslPlan::Pivot {
            ...
            apollo_ensure!(!on.is_empty(), InvalidOperation: "`pivot` called without `on` columns.");
            apollo_ensure!(on.len() == on_columns.width(), InvalidOperation: "`pivot` expected `on` and `on_columns` to have the same amount of columns.");
            if on.len() > 1 {
                apollo_ensure!(
            ...
            apollo_ensure!(!values.is_empty(), InvalidOperation: "`pivot` called without `values` columns.");
            ...
            let (input, subset, temp_cols) = if let Some(exprs) = options.subset {
                ...
                if subset_exprs.is_empty() {
                    ...
                    let (aliased_exprs, temp_names): (Vec<_>, Vec<_>) = subset_exprs
                        ...
                        .enumerate()
                        .map(|(idx, expr)| {
                            let temp_name = format_pl_smallstr!("__APOLLO_UNIQUE_SUBSET_{}", idx);
        ...
        DslPlan::MapFunction { input, function } => {
            ...
            match function {
                DslFunction::Explode {
                    ...
                    apollo_ensure!(!columns.is_empty() || allow_empty, InvalidOperation: "no columns provided in explode");
                    if columns.is_empty() {
                        return Ok(input);
                    }
...
fn resolve_with_columns(
    ...
    expr_arena: &mut Arena<AExpr>,
    opt_flags: &mut OptFlags,
) -> ApolloResult<(Vec<ExprIR>, SchemaRef)> {
...
fn resolve_group_by(
    ...
    expr_arena: &mut Arena<AExpr>,
    opt_flags: &mut OptFlags,
) -> ApolloResult<(Vec<ExprIR>, Vec<ExprIR>, SchemaRef)> {
    let input_schema = lp_arena.get(input).schema(lp_arena);
    ...
    if aggs_schema.len() < aggs.len() {
        ...
        for agg in aggs.iter() {
            ...
            apollo_ensure!(names.insert(name.clone()), duplicate = name)
        }
    }
