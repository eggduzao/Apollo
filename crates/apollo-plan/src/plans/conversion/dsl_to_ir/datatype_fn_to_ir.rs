...
use apollo_core::prelude::{AnyValue, DataType, Field, NamedFrom};
    ...
    Ok(match f {
        ...
        DTF::Array(dt_expr, f) => {
            let (inner, width): (DataType, usize) = match dt_expr.into_datatype(ctx.schema)? {
                ...
                dt => apollo_bail!(InvalidOperation: "`{dt}` is not an Array"),
            };
        ...
        DTF::Struct(dt_expr, f) => {
            let fields: Vec<Field> = match dt_expr.into_datatype(ctx.schema)? {
                ...
                dt => apollo_bail!(InvalidOperation: "`{dt}` is not an Struct"),
            };
