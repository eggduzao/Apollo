...
use apollo_core::utils::{get_numeric_upcast_supertype_lossless, try_get_supertype};
use apollo_utils::format_pl_smallstr;
use apollo_utils::itertools::Itertools;

use super::*;
use crate::constants::APOLLO_TMP_PREFIX;
use crate::dsl::Expr;
#[cfg(feature = "iejoin")]
use crate::plans::AExpr;

fn check_join_keys(keys: &[Expr]) -> ApolloResult<()> {
    for e in keys {
...
pub fn resolve_join(
    ...
    if options.args.how.is_cross() {
        apollo_ensure!(left_on.len() + right_on.len() == 0, InvalidOperation: "a 'cross' join doesn't expect any join keys");
    } else {
        apollo_ensure!(left_on.len() + right_on.len() > 0, InvalidOperation: "expected join keys/predicates");
        ...
        if let JoinType::AsOf(options) = &options.args.how {
            match (&options.left_by, &options.right_by) {
                (None, None) => {},
                (Some(l), Some(r)) => {
                    apollo_ensure!(l.len() == r.len(), InvalidOperation: "expected equal number of columns in 'by_left' and 'by_right' in 'asof_join'");
                    validate_columns_in_input(l, &schema_left, "asof_join")?;
                    validate_columns_in_input(r, &schema_right, "asof_join")?;
                },
                _ => {
                    apollo_bail!(InvalidOperation: "expected both 'by_left' and 'by_right' to be set in 'asof_join'")
                },
            }
        }
    ...
    let (schema_left, schema_right) = if has_scalars {
        ...
        let mut count = 0;
        let get_tmp_name = |i| format_pl_smallstr!("{APOLLO_TMP_PREFIX}{i}");
    ...
    if let JoinType::AsOf(options) = &mut options.args.how {
        use apollo_core::utils::arrow::temporal_conversions::MILLISECONDS_IN_DAY;

        // prepare the tolerance
        // we must ensure that we use the right units
        if let Some(tol) = &options.tolerance_str {
            let duration = apollo_time::Duration::try_parse(tol)?;
            apollo_ensure!(
                duration.months() == 0,
    ...
    if has_scalars {
        let names = join_schema
            .iter_names()
            .filter_map(|n| {
                if n.starts_with(APOLLO_TMP_PREFIX) {
                    None
                } else {
                    Some(n.clone())
                }
    ...
    let expr_origin = match expr_arena.get(*node) {
        AExpr::Column(name) => {
            if schema_left.contains(name) {
                ...
                apollo_bail!(ColumnNotFound: "{}", name);
            }
        },
                        ...
                        if dtype_left != dtype_right {
                                ...
                                || dtype_right.is_primitive_numeric()
                            {
                                get_numeric_upcast_supertype_lossless(&dtype_left, &dtype_right)
                                    .ok_or(ApolloError::SchemaMismatch(
                                        format!(
