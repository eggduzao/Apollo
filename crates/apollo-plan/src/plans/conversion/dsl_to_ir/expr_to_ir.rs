...
pub fn to_expr_ir(expr: Expr, ctx: &mut ExprToIRContext) -> ApolloResult<ExprIR> {
    let (node, output_name) = to_aexpr_impl(expr, ctx)?;
    Ok(ExprIR::new(node, OutputName::Alias(output_name)))
}

pub fn to_expr_ir_materialized_lit(expr: Expr, ctx: &mut ExprToIRContext) -> ApolloResult<ExprIR> {
    ...
    let (v, output_name) = match expr {
        ...
        Expr::StructEval { expr, evaluation } => {
            ...
            let DataType::Struct(fields) = &expr_dtype else {
                apollo_bail!(op = "struct.with_fields", expr_dtype);
            };
            ...
            for e in evaluation {
                ...
                apollo_ensure!(field_names.insert(field_name.clone()),
        ...
        Expr::Field(name) => {
            ...
            let Some(with_fields) = &ctx.with_fields else {
                apollo_bail!(InvalidOperation: "`pl.field()` called outside of struct context");
            };
        ...
        e @ Expr::SubPlan { .. } | e @ Expr::Selector(_) => {
            apollo_bail!(InvalidOperation: "'Expr: {}' not allowed in this context/location", e)
        },
    };
