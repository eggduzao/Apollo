...
pub(super) fn resolve_is_in(
    ...
    flat_idx: usize,
    nested_idx: usize,
) -> ApolloResult<Option<IsInTypeCoercionResult>> {
...
See https://github.com/apollo/apollo/issues/22149 for more information."
    ...
    if left_nl + 1 != right_nl {
        apollo_bail!(InvalidOperation: "'{op}' cannot check for {:?} values in {:?} data", &type_other, &type_left);
    }
    ...
    let casted_expr = match (&type_left, type_other_inner) {
        ...
        (DataType::Datetime(lhs_unit, _), DataType::Datetime(rhs_unit, _)) => {
            if lhs_unit <= rhs_unit {
                ...
                apollo_bail!(InvalidOperation: "'{op}' cannot check for {:?} precision values in {:?} Datetime data", &rhs_unit, &lhs_unit)
            }
        },
        (DataType::Duration(lhs_unit), DataType::Duration(rhs_unit)) => {
            if lhs_unit <= rhs_unit {
                ...
                apollo_bail!(InvalidOperation: "'{op}' cannot check for {:?} precision values in {:?} Duration data", &rhs_unit, &lhs_unit)
            }
        },
        (_, DataType::List(_)) => {
            apollo_ensure!(
        ...
        (a, b) => {
            if (a.is_primitive_numeric() && b.is_primitive_numeric()) || (a == &DataType::Null) {
                if a != b {
                    ...
                    let super_type = apollo_core::utils::get_numeric_upcast_supertype_lossless(
                        &type_left,
                        type_other_inner,
                    )
                    .map_or_else(
                        || apollo_core::utils::try_get_supertype(&type_left, type_other_inner),
                        Ok,
                    )?;
