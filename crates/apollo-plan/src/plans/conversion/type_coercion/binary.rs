...
use apollo_utils::matches_any_order;
...
fn err_date_str_compare() -> ApolloResult<()> {
    ...
    if op.is_arithmetic() {
        match (&type_left, &type_right) {
            ...
            (String, a) | (a, String) if a.is_primitive_numeric() => {
                apollo_bail!(InvalidOperation: "arithmetic on string and numeric not allowed, try an explicit cast first")
            },
            (Datetime(_, _), _)
            | (_, Datetime(_, _))
            | (Date, _)
