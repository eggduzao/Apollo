...
fn try_get_dtype(expr_arena: &Arena<AExpr>, e: Node, schema: &Schema) -> ApolloResult<DataType> {
    expr_arena
        .get(e)
            ...
            AExpr::Function {
                ...
                let input = input.clone().into_iter().enumerate().map(|(i, expr)| {
                    ...
                    Ok(match &dtype {
                        ...
                        dt => {
                            apollo_bail!(InvalidOperation: "expected integer or float dtype, (got {dt}) in input {i} of duration")
                        },
                    })
                }).try_collect()?;
...
See https://github.com/apollo/apollo/issues/22149 for more information."
                    );
...
See https://github.com/apollo/apollo/issues/22149 for more information."
            ...
            AExpr::Function {
                ...
                apollo_ensure!(dtype.is_integer(), ComputeError: "non-integer `dtype` passed to `int_range`: {:?}", dtype);
            ...
            AExpr::Slice { offset, length, .. } => {
                ...
                apollo_ensure!(offset_dtype.is_integer(), InvalidOperation: "offset must be integral for slice expression, not {}", offset_dtype);
                let (_, length_dtype) = unpack!(get_aexpr_and_type(expr_arena, length, schema));
                apollo_ensure!(length_dtype.is_integer() || length_dtype.is_null(), InvalidOperation: "length must be integral for slice expression, not {}", length_dtype);
                None
            },
            _ => None,
        };
...
fn try_inline_literal_cast(
    ...
    dtype: &DataType,
    options: CastOptions,
) -> ApolloResult<Option<LiteralValue>> {
    let lv = match lv {
...
fn check_cast(from: &DataType, to: &DataType) -> ApolloResult<()> {
    apollo_ensure!(
...
fn raise_supertype(
    ...
    // specific operation.
    if st.is_some() {
        apollo_bail!(InvalidOperation: "got invalid or ambiguous dtypes: '{}' in expression '{}'\
                        \n\nConsider explicitly casting your input types to resolve potential ambiguity.", format_list!(&dtypes), function);
    } else {
        apollo_bail!(InvalidOperation: "could not determine supertype of: {} in expression '{}'\
                        \n\nIt might also be the case that the type combination isn't allowed in this specific operation.", format_list!(&dtypes), function);
    }
}

fn inline_implode(expr: Node, expr_arena: &mut Arena<AExpr>) -> ApolloResult<Option<AExpr>> {
...
fn can_cast_to_lossless(to: &DataType, from: &DataType) -> ApolloResult<()> {
    let can_cast = match (to, from) {
        ...
        (to, DataType::Unknown(UnknownKind::Int(value))) => match to {
            ...
            _ => apollo_bail!(InvalidOperation: "cannot cast {} losslessly to {}", value, to),
        },
    ...
    if !can_cast {
        apollo_bail!(InvalidOperation: "cannot cast losslessly from {} to {}", from, to)
    }
    Ok(())
}
