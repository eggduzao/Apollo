...
use apollo_io::prelude::schema_inference::{finish_infer_field_schema, infer_field_schema};
...
pub fn hive_partitions_from_paths(
...
) -> ApolloResult<Option<HivePartitionsDf>> {
    ...
    let mut buffers = apollo_io::csv::read::builder::init_builders(
        &(0..hive_schema.len()).collect::<Vec<_>>(),
        paths.len(),
        hive_schema.as_ref(),
        None,
        apollo_io::prelude::CsvEncoding::Utf8,
        false,
    )?;
