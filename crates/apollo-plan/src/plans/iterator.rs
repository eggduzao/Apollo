...
impl<F: FnMut(Expr) -> ApolloResult<Expr>> RewritingVisitor for ExprMapper<F> {
    type Node = Expr;
    type Arena = ();

    fn mutate(&mut self, node: Self::Node, _arena: &mut Self::Arena) -> ApolloResult<Self::Node> {
        (self.f)(node)
    }
}
...
impl Expr {
    ...
    pub fn try_map_expr<F: FnMut(Self) -> ApolloResult<Self>>(self, f: F) -> ApolloResult<Self> {
