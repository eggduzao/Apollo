...
use apollo_core::prelude::{Field, Schema};
...
impl TreeWalker for Expr {
    ...
    fn apply_children<F: FnMut(&Self, &Self::Arena) -> ApolloResult<VisitRecursion>>(
        &self,
        op: &mut F,
        arena: &Self::Arena,
    ...
    fn map_children<F: FnMut(Self, &mut Self::Arena) -> ApolloResult<Self>>(
        self,
        f: &mut F,
        _arena: &mut Self::Arena,
    ) -> ApolloResult<Self> {
        use apollo_utils::functions::try_arc_map as am;
...
impl AexprNode {
    ...
    pub fn to_field(&self, schema: &Schema, arena: &Arena<AExpr>) -> ApolloResult<Field> {
