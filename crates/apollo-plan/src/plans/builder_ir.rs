...
impl<'a> IRBuilder<'a> {
    ...
    pub fn add_alp_optimize_exprs<F>(self, f: F) -> ApolloResult<Self>
    where
        F: FnOnce(Node) -> IR,
    {
        let lp = f(self.root);
    ...
    pub fn add_expr(&mut self, expr: Expr) -> ApolloResult<ExprIR> {
    ...
    pub fn project_simple_nodes<I, N>(self, nodes: I) -> ApolloResult<Self>
    where
        ...
        I::IntoIter: ExactSizeIterator,
    {
        ...
        if names.size_hint().0 == 0 {
            ...
            let schema = names
                ...
                .collect::<ApolloResult<Schema>>()?;

            apollo_ensure!(count == schema.len(), Duplicate: "found duplicate columns");
    ...
    pub fn project_simple<I, S>(self, names: I) -> ApolloResult<Self>
    where
