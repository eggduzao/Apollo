...
use apollo_compute::decimal::DEC128_MAX_PREC;
...
use super::*;
use crate::constants::{
    APOLLO_ELEMENT, APOLLO_STRUCTFIELDS, get_literal_name, get_pl_element_name,
    get_pl_structfields_name,
};

fn validate_expr(node: Node, ctx: &ToFieldContext) -> ApolloResult<()> {
...
impl AExpr {
    pub fn to_dtype(&self, ctx: &ToFieldContext<'_>) -> ApolloResult<DataType> {
    ...
    pub fn to_field(&self, ctx: &ToFieldContext<'_>) -> ApolloResult<Field> {
    ...
    /// (e.g. `alias`, `cast`).
    #[recursive]
    pub fn to_field_impl(&self, ctx: &ToFieldContext) -> ApolloResult<Field> {
        use AExpr::*;
        use DataType::*;
        match self {
            Element => ctx
                .schema
                .get_field(APOLLO_ELEMENT)
                .ok_or_else(|| apollo_err!(invalid_element_use)),
            ...
            Column(name) => ctx
                .schema
                .get_field(name)
                .ok_or_else(|| ApolloError::ColumnNotFound(name.to_string().into())),
            #[cfg(feature = "dtype-struct")]
            StructField(name) => {
                let struct_field = ctx
                    .schema
                    .get_field(APOLLO_STRUCTFIELDS)
                    .ok_or_else(|| apollo_err!(invalid_field_use))?;
                let DataType::Struct(fields) = struct_field.dtype() else {
                    return Err(apollo_err!(
                ...
                Err(ApolloError::StructFieldNotFound(name.to_string().into()))
            },
            ...
            AnonymousFunction {
                ...
                apollo_ensure!(!fields.is_empty(), ComputeError: "expression: '{}' didn't get any inputs", fmt_str);
            ...
            StructEval { expr, evaluation } => {
                ...
                if let DataType::Struct(expr_fields) = struct_field.dtype() {
                    ...
                    apollo_bail!(op = "with_fields", got = dt, expected = "Struct")
                }
            },
            Function {
                function,
                ...
                let fields = func_args_to_fields(input, ctx)?;
                apollo_ensure!(!fields.is_empty(), ComputeError: "expression: '{}' didn't get any inputs", function);
                let out = function.get_field(ctx.schema, &fields)?;
...
fn func_args_to_fields(input: &[ExprIR], ctx: &ToFieldContext) -> ApolloResult<Vec<Field>> {
    input
        .iter()
        .map(|e| {
        ...
        Operator::Minus => {
            ...
            match (&left_field.dtype, &right_type) {
                ...
                (_, Datetime(_, _)) | (Datetime(_, _), _) => {
                    apollo_bail!(InvalidOperation: "{} not allowed on {} and {}", op, left_field.dtype, right_type)
                },
...
fn get_truediv_field(left: Node, right: Node, ctx: &ToFieldContext) -> ApolloResult<Field> {
...
fn get_truediv_dtype(left_dtype: &DataType, right_dtype: &DataType) -> ApolloResult<DataType> {
    use DataType::*;
    ...
    let out_type = match (left_dtype, right_dtype) {
        #[cfg(feature = "dtype-struct")]
        (Struct(a), Struct(b)) => {
            apollo_ensure!(a.len() == b.len() || b.len() == 1,
        ...
        #[cfg(feature = "dtype-duration")]
        (Duration(_), dt) => {
            apollo_bail!(InvalidOperation: "true division of {} with {} is not allowed", left_dtype, dt)
        },
        #[cfg(feature = "dtype-datetime")]
        (Datetime(_, _), _) => {
            apollo_bail!(InvalidOperation: "division of 'Datetime' datatype is not allowed")
        },
        #[cfg(feature = "dtype-time")]
        (Time, _) => apollo_bail!(InvalidOperation: "division of 'Time' datatype is not allowed"),
        #[cfg(feature = "dtype-date")]
        (Date, _) => apollo_bail!(InvalidOperation: "division of 'Date' datatype is not allowed"),
