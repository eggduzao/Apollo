use apollo_core::utils::SuperTypeOptions;
...
impl<'a> FieldsMapper<'a> {
    ...
    pub fn ensure_is_list(self) -> ApolloResult<Self> {
    ...
    pub(super) fn get_field(&self, mapper: FieldsMapper) -> ApolloResult<Field> {
        ...
        match self {
            ...
            Join(_) => mapper.try_map_dtype(|dtype| {
                ...
                apollo_ensure!(inner_dtype.is_string(), InvalidOperation:
...
#[cfg(feature = "dtype-array")]
fn map_list_dtype_to_array_dtype(datatype: &DataType, width: usize) -> ApolloResult<DataType> {
