        ...
        match self {
            FieldByName(name) => mapper.try_map_field(|field| {
                if let DataType::Struct(ref fields) = field.dtype {
                    ...
                    apollo_bail!(StructFieldNotFound: "{}", name);
                }
            }),
            ...
            PrefixFields(prefix) => mapper.try_map_dtype(|dt| match dt {
                ...
                _ => apollo_bail!(op = "prefix_fields", got = dt, expected = "Struct"),
            }),
            SuffixFields(suffix) => mapper.try_map_dtype(|dt| match dt {
                ...
                _ => apollo_bail!(op = "suffix_fields", got = dt, expected = "Struct"),
            }),
            #[cfg(feature = "json")]
            ...
            MapFieldNames(function) => mapper.try_map_dtype(|dt| match dt {
                DataType::Struct(fields) => {
                    let fields = fields
                        ...
                        .map(|fld| {
                            let name = fld.name();
                            let new_name = function.call(name.clone()).map_err(|e| apollo_err!(ComputeError: "'name.map_fields' produced an error: {e}."))?;
