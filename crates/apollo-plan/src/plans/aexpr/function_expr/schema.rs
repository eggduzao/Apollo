use apollo_core::utils::materialize_dyn_int;
        ...
        match self {
            ...
            Diff(_) => mapper.map_dtype(|dt| match dt {
                ...
                DataType::Decimal(_, scale) => {
                    DataType::Decimal(apollo_compute::decimal::DEC128_MAX_PREC, *scale)
                },
                dt => dt.clone(),
            }),
            ...
            Reshape(dims) => mapper.try_map_dtype(|dt: &DataType| {
                ...
                for dim in dims[1..].iter().rev() {
                    let Some(array_size) = dim.get() else {
                        apollo_bail!(InvalidOperation: "can only infer the first dimension");
                    };
...
impl<'a> FieldsMapper<'a> {
    ...
    pub fn with_same_dtype(&self) -> ApolloResult<Field> {
        self.map_dtype(|dtype| dtype.clone())
    }

    /// Set a dtype.
    pub fn with_dtype(&self, dtype: DataType) -> ApolloResult<Field> {
        Ok(Field::new(self.fields[0].name().clone(), dtype))
    }

    /// Map a single dtype.
    pub fn map_dtype(&self, func: impl FnOnce(&DataType) -> DataType) -> ApolloResult<Field> {
    ...
    pub fn try_map_field(
        &self,
        func: impl FnOnce(&Field) -> ApolloResult<Field>,
    ) -> ApolloResult<Field> {
        func(&self.fields[0])
    }

    pub fn var_dtype(&self) -> ApolloResult<Field> {
        if self.fields[0].dtype().leaf_dtype().is_duration() {
            let map_inner = |dt: &DataType| match dt {
                dt if dt.is_temporal() => {
                    apollo_bail!(InvalidOperation: "operation `var` is not supported for `{dt}`")
                },
                dt => Ok(dt.clone()),
            };
    ...
    pub fn moment_dtype(&self) -> ApolloResult<Field> {
        let map_inner = |dt: &DataType| match dt {
    ...
    pub fn map_to_float_dtype(&self) -> ApolloResult<Field> {
    ...
    pub fn map_numeric_to_float_dtype(&self, coerce_decimal: bool) -> ApolloResult<Field> {
        self.map_dtype(|dt| {
    ...
    pub fn to_physical_type(&self) -> ApolloResult<Field> {
    ...
    pub fn try_map_dtype(
        &self,
        func: impl FnOnce(&DataType) -> ApolloResult<DataType>,
    ) -> ApolloResult<Field> {
        let dtype = func(self.fields[0].dtype())?;
        Ok(Field::new(self.fields[0].name().clone(), dtype))
    }
    ...
    pub fn map_to_supertype(&self) -> ApolloResult<Field> {
    ...
    pub fn map_to_list_and_array_inner_dtype(&self) -> ApolloResult<Field> {
        let mut first = self.fields[0].clone();
        let dt = first
            .dtype()
            .inner_dtype()
    ...
    pub fn try_map_to_array_inner_dtype(&self) -> ApolloResult<Field> {
        let dt = self.fields[0].dtype();
        match dt {
            DataType::Array(_, _) => self.map_to_list_and_array_inner_dtype(),
            _ => apollo_bail!(InvalidOperation: "expected Array type, got: {}", dt),
        }
    }

    /// Map the dtypes to the "supertype" of a list of lists.
    pub fn map_to_list_supertype(&self) -> ApolloResult<Field> {
    ...
    pub fn map_datetime_dtype_timezone(&self, tz: Option<&TimeZone>) -> ApolloResult<Field> {
        self.try_map_dtype(|dt| {
            if let DataType::Datetime(tu, _) = dt {
                Ok(DataType::Datetime(*tu, tz.cloned()))
            } else {
                apollo_bail!(op = "replace-time-zone", got = dt, expected = "Datetime");
            }
        })
    }

    pub fn sum_dtype(&self) -> ApolloResult<Field> {
        use DataType::*;
        self.map_dtype(|dtype| match dtype {
    ...
    pub fn nested_sum_type(&self) -> ApolloResult<Field> {
        let mut first = self.fields[0].clone();
        use DataType::*;
    ...
    pub fn nested_mean_median_type(&self) -> ApolloResult<Field> {
        let mut first = self.fields[0].clone();
        use DataType::*;
        let dt = first.dtype().inner_dtype().cloned().ok_or_else(|| {
            apollo_err!(
    ...
    pub(super) fn pow_dtype(&self) -> ApolloResult<Field> {
    ...
    pub(super) fn log_dtype(&self) -> ApolloResult<Field> {
        let dtype1 = self.fields[0].dtype();
        let dtype2 = self.fields[1].dtype();
    ...
    pub fn with_opt_dtype(&self, dtype: Option<DataType>) -> ApolloResult<Field> {
        let dtype = dtype.unwrap_or_else(|| DataType::Unknown(Default::default()));
        self.with_dtype(dtype)
    }

    #[cfg(feature = "replace")]
    pub fn replace_dtype(&self, return_dtype: Option<DataType>) -> ApolloResult<Field> {
...
pub(crate) fn args_to_supertype<D: AsRef<DataType>>(dtypes: &[D]) -> ApolloResult<DataType> {
