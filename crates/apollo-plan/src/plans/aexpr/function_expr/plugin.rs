...
fn get_lib(lib: &str) -> ApolloResult<&'static PluginAndVersion> {
    ...
    if let Some(library) = lib_map.get(lib) {
        ...
        let library = unsafe {
            Library::new(&load_path).map_err(|e| {
                ApolloError::ComputeError(format!("error loading dynamic library: {e}").into())
            })?
        };
        let version_function: libloading::Symbol<unsafe extern "C" fn() -> u32> = unsafe {
            library
                .get("_apollo_plugin_get_version".as_bytes())
                .unwrap()
        };
...
fn retrieve_error_msg(lib: &Library) -> String {
    unsafe {
        // SAFETY: _apollo_plugin_get_last_error_message returns data stored
        ...
        let symbol: libloading::Symbol<unsafe extern "C" fn() -> *mut std::os::raw::c_char> =
            lib.get(b"_apollo_plugin_get_last_error_message\0").unwrap();
        let msg_ptr = symbol();
...
pub unsafe fn call_plugin(
    ...
    if major == 0 {
        ...
        > = lib
            .get(format!("_apollo_plugin_{symbol}").as_bytes())
            .unwrap();
        ...
        if !return_value.is_null() {
            ...
            apollo_bail!(ComputeError: "the plugin failed with message: {}", msg)
        }
    } else {
        apollo_bail!(ComputeError: "this apollo engine doesn't support plugin version: {}", major)
    }
}
    ...
    if major == 0 {
        match minor {
            0 => {
                ...
                apollo_ensure!(!views, ComputeError: "cannot call plugin\n\nThis Apollo' version has a different 'binary/string' layout. Please compile with latest 'pyo3-apollo'");
                ...
                > = lib
                    .get((format!("_apollo_plugin_field_{symbol}")).as_bytes())
                    .unwrap();
            ...
            _ => {
                apollo_bail!(ComputeError: "this Apollo engine doesn't support plugin version: {}-{}", major, minor)
            },
        }
        if !return_value.is_null() {
...
fn check_panic(msg: &str) -> ApolloResult<()> {
    apollo_ensure!(msg != "PANIC", ComputeError: "the plugin panicked\n\nThe message is suppressed. Set APOLLO_VERBOSE=1 to send the panic message to stderr.");
    Ok(())
}
