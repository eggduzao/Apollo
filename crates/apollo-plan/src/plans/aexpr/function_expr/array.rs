...
use apollo_ops::chunked_array::array::*;
...
impl<'a> FieldsMapper<'a> {
    ...
    pub fn ensure_is_array(self) -> ApolloResult<Self> {
...
impl IRArrayFunction {
    pub(super) fn get_field(&self, mapper: FieldsMapper) -> ApolloResult<Field> {
        use IRArrayFunction::*;
        ...
        match self {
            ...
            ToStruct(name_generator) => mapper.ensure_is_array()?.try_map_dtype(|dtype| {
                let DataType::Array(inner, width) = dtype else {
                    apollo_bail!(InvalidOperation: "expected Array type, got: {dtype}")
                };
                ...
                (0..*width)
                    ...
                    .collect::<ApolloResult<Vec<Field>>>()
                    .map(DataType::Struct)
            }),
        }
    }
...
fn map_array_dtype_to_list_dtype(datatype: &DataType) -> ApolloResult<DataType> {
...
fn map_to_array_fixed_length(
    offset: &i64,
    length: &i64,
) -> impl FnOnce(&DataType) -> ApolloResult<DataType> {
        ...
        if let DataType::Array(inner, array_len) = datatype {
            let length: usize = if *length < 0 {
                ...
                *length
                ...
                apollo_err!(OutOfBounds: "length must be a non-negative integer, got: {}", length)
            })?;
            let (_, slice_offset) = slice_offsets(*offset, length, *array_len);
            Ok(DataType::Array(inner.clone(), slice_offset))
        } else {
            apollo_bail!(ComputeError: "expected array dtype, got {}", datatype);
        }
    }
}
