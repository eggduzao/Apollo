...
pub fn count_rows(
    ...
    feature_gated!("csv", {
        let count: ApolloResult<usize> = match scan_type {
            #[cfg(feature = "csv")]
        ...
        let count: IdxSize = count.try_into().map_err(
            |_| apollo_err!(ComputeError: "count of {} exceeded maximum row size", count),
        )?;
...
fn count_all_rows_csv(
    sources: &ScanSources,
    options: &apollo_io::prelude::CsvReadOptions,
) -> ApolloResult<usize> {
    ...
    sources
        ...
        .map(|source| match source {
            ScanSourceRef::Path(path) => apollo_io::csv::read::count_rows(
                path.clone(),
            ...
            _ => {
                ...
                apollo_io::csv::read::count_rows_from_slice_par(
                    memslice,
                    parse_options.quote_char,
                    parse_options.comment_prefix.as_ref(),
                    parse_options.eol_char,
