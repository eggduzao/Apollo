    ...
    pub(crate) fn get_column_origin(
        ...
        Ok(
                    ...
                    .is_some_and(|x| right_schema.contains(x))
            {
                ...
                apollo_bail!(ColumnNotFound: "{}", column_name)
            },
        )
    }
}
...
pub(super) fn remove_suffix<'a>(
    ...
    // https://github.com/apollo/apollo/issues/20831.
    ...
    impl RewritingVisitor for RemoveSuffix<'_> {
        ...
        ) -> apollo_core::prelude::ApolloResult<crate::prelude::visitor::RewriteRecursion> {
        ...
        fn mutate(
            ...
            node: Self::Node,
            arena: &mut Self::Arena,
        ) -> apollo_core::prelude::ApolloResult<Self::Node> {
            let AExpr::Column(colname) = arena.get(node.node()) else {
                unreachable!();
            };
