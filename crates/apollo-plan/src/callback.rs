...
pub enum PlanCallback<Args, Out> {
    #[cfg(feature = "python")]
    Python(SpecialEq<Arc<apollo_utils::python_function::PythonFunction>>),
    Rust(SpecialEq<Arc<dyn Fn(Args) -> ApolloResult<Out> + Send + Sync>>),
}
        ...
        D: serde::Deserializer<'de>,
    {
        #[cfg(feature = "python")]
        {
            Ok(Self::Python(SpecialEq::new(Arc::new(
                apollo_utils::python_function::PythonFunction::deserialize(_deserializer)?,
            ))))
        }
...
mod _python {
        ...
        ($(($type:path, $from:ident, $to:ident)),+) => {
            ...
            impl super::PlanCallbackArgs for $type {
                fn into_pyany<'py>(self, _py: Python<'py>) -> PyResult<Py<PyAny>> {
                    let registry = apollo_utils::python_convert_registry::get_python_convert_registry();
    ...
    impl_registrycb_type! {
        (apollo_core::series::Series, series, series),
        (apollo_core::frame::DataFrame, df, df),
        (crate::dsl::DslPlan, dsl_plan, dsl_plan),
        (apollo_core::schema::Schema, schema, schema)
    }
...
impl<Args: PlanCallbackArgs, Out: PlanCallbackOut> PlanCallback<Args, Out> {
    pub fn call(&self, args: Args) -> ApolloResult<Out> {
        match self {
            #[cfg(feature = "python")]
    ...
    #[cfg(feature = "python")]
    pub fn new_python(pyfn: apollo_utils::python_function::PythonFunction) -> Self {
        Self::Python(SpecialEq::new(Arc::new(pyfn)))
    }

    pub fn new(f: impl Fn(Args) -> ApolloResult<Out> + Send + Sync + 'static) -> Self {
