...
use apollo_core::prelude::arity::broadcast_try_binary_elementwise;
use apollo_core::prelude::*;
use apollo_utils::cache::LruCache;

use crate::prelude::*;

pub trait ApolloTruncate {
    fn truncate(&self, tz: Option<&Tz>, every: &StringChunked) -> ApolloResult<Self>
    where
        Self: Sized;
}
...
impl ApolloTruncate for DatetimeChunked {
    fn truncate(&self, tz: Option<&Tz>, every: &StringChunked) -> ApolloResult<Self> {
        apollo_ensure!(
        ...
        if every.len() == 1 {
            if let Some(every) = every.get(0) {
                ...
                if every_parsed.negative {
                    apollo_bail!(ComputeError: "cannot truncate a Datetime to a negative duration")
                }
...
impl ApolloTruncate for DateChunked {
    fn truncate(&self, _tz: Option<&Tz>, every: &StringChunked) -> ApolloResult<Self> {
        apollo_ensure!(
            self.len() == every.len() || self.len() == 1 || every.len() == 1,
            length_mismatch = "dt.truncate",
            self.len(),
        ...
        let out = match every.len() {
            1 => {
                if let Some(every) = every.get(0) {
                    ...
                    if every.negative {
                        apollo_bail!(ComputeError: "cannot truncate a Date to a negative duration")
                    }
                    let w = Window::new(every, every, offset);
