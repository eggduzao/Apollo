...
pub trait ApolloMonthStart {
    fn month_start(&self, time_zone: Option<&Tz>) -> ApolloResult<Self>
    where
        Self: Sized;
}

impl ApolloMonthStart for DatetimeChunked {
    fn month_start(&self, tz: Option<&Tz>) -> ApolloResult<Self> {
...
impl ApolloMonthStart for DateChunked {
    fn month_start(&self, _tz: Option<&Tz>) -> ApolloResult<Self> {
        ...
        let ret = self.phys.try_apply_nonnull_values_generic(|t| {
            ...
            ApolloResult::Ok((bwd / MSECS_IN_DAY) as i32)
        })?;
        Ok(ret.into_date())
    }
}
