...
fn check_sortedness_slice(v: &[i64]) -> ApolloResult<()> {
    apollo_ensure!(v.is_sorted_ascending(), ComputeError: "input data is not sorted");
    Ok(())
}
...
pub trait ApolloTemporalGroupby {
    fn rolling(
        &self,
        group_by: Option<GroupsSlice>,
        options: &RollingGroupOptions,
    ) -> ApolloResult<(Column, GroupPositions)>;

    fn group_by_dynamic(
        &self,
        group_by: Option<GroupsSlice>,
        options: &DynamicGroupOptions,
    ) -> ApolloResult<(Column, Vec<Column>, GroupPositions)>;
}

impl ApolloTemporalGroupby for DataFrame {
    fn rolling(
        &self,
        group_by: Option<GroupsSlice>,
        options: &RollingGroupOptions,
...
impl Wrap<&DataFrame> {
    fn rolling(
        ...
        apollo_ensure!(time.null_count() == 0, ComputeError: "null values in `rolling` not supported, fill nulls.");
    ...
    fn group_by_dynamic(
        ...
        apollo_ensure!(time.null_count() == 0, ComputeError: "null values in dynamic group_by not supported, fill nulls.");
    ...
    fn impl_group_by_dynamic(
        ...
        apollo_ensure!(!options.every.negative, ComputeError: "'every' argument must be positive");
        if dt.is_empty() {
        ...
        let groups = if let Some(groups) = group_by.as_ref() {
            ...
            let res = POOL.install(|| iter.collect::<ApolloResult<Vec<_>>>())?;
            ...
            ApolloResult::Ok(GroupsType::new_slice(groups, overlapping, true))
        } else {
            let vals = dt.physical().downcast_iter().next().unwrap();
            let ts = vals.values().as_slice();
            let (groups, lower, upper) = group_by_windows(
    ...
    fn impl_rolling(
        ...
        let groups = if let Some(groups) = group_by {
            ...
            let groups = POOL.install(|| iter.collect::<ApolloResult<Vec<_>>>())?;
            let groups = POOL.install(|| flatten_par(&groups));
            ApolloResult::Ok(GroupsType::new_slice(groups, true, true))
        } else {
...
mod test {
    ...
    use apollo_ops::prelude::*;

    use super::*;

    #[test]
    fn test_rolling_group_by_tu() -> ApolloResult<()> {
    ...
    #[test]
    fn test_rolling_group_by_aggs() -> ApolloResult<()> {
        let mut date = StringChunked::new(
            "dt".into(),
            [
                "2020-01-01 13:45:48",
