...
use apollo_compute::cast::{CastOptionsImpl, cast};
use apollo_core::prelude::*;
#[cfg(feature = "timezones")]
use apollo_ops::chunked_array::datetime::replace_time_zone;

use super::*;

fn cast_and_apply<
    F: Fn(&dyn Array) -> ApolloResult<PrimitiveArray<T::Native>>,
    T: ApolloNumericType,
>(
    ca: &DatetimeChunked,
    func: F,
...
pub trait DatetimeMethods: AsDatetime {
    ...
    fn is_leap_year(&self) -> BooleanChunked {
        ...
        let ca_local = match ca.dtype() {
            #[cfg(feature = "timezones")]
            DataType::Datetime(_, Some(_)) => &apollo_ops::chunked_array::replace_time_zone(
                ca,
                None,
            ...
            .map(|((((((y, m), d), h), mnt), s), ns)| {
                    ...
                    (y, m, d, h, mnt, s, ns)
                {
                    NaiveDate::from_ymd_opt(y, m as u32, d as u32).map_or_else(
                        ...
                        || Err(apollo_err!(ComputeError: format!("Invalid date components ({}, {}, {}) supplied", y, m, d))),
                        // We have a valid date.
                        |date| {
                            date.and_hms_nano_opt(h as u32, mnt as u32, s as u32, ns as u32)
                                .map_or_else(
                                    // We have invalid time components for the specified date.
                                    || Err(apollo_err!(ComputeError: format!("Invalid time components ({}, {}, {}, {}) supplied", h, mnt, s, ns))),
