...
use crate::chunkedarray::{ApolloResult, apollo_bail};
...
/// Tries to convert a chrono `fmt` to a `fmt` that the apollo parser consumes.
/// E.g. chrono supports single letter date identifiers like %F, whereas apollo only consumes
/// year, day, month distinctively with %Y, %d, %m.
pub(super) fn compile_fmt(fmt: &str) -> ApolloResult<String> {
    ...
    if HOUR_PATTERN.is_match(fmt) ^ MINUTE_PATTERN.is_match(fmt) {
        apollo_bail!(ComputeError: "Invalid format string: \
