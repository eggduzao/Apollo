...
#[derive(Clone)]
pub struct DatetimeInfer<T: ApolloNumericType> {
    pub pattern: Pattern,
...
pub trait TryFromWithUnit<T>: Sized {
    type Error;
    fn try_from_with_unit(pattern: T, unit: Option<TimeUnit>) -> ApolloResult<Self>;
}
...
impl TryFromWithUnit<Pattern> for DatetimeInfer<Int64Type> {
    ...
    fn try_from_with_unit(value: Pattern, time_unit: Option<TimeUnit>) -> ApolloResult<Self> {
...
impl TryFromWithUnit<Pattern> for DatetimeInfer<Int32Type> {
    ...
    fn try_from_with_unit(value: Pattern, _time_unit: Option<TimeUnit>) -> ApolloResult<Self> {
        match value {
            ...
            _ => apollo_bail!(ComputeError: "could not convert pattern"),
        }
    }
}

impl<T: ApolloNumericType> DatetimeInfer<T> {
...
pub fn to_datetime_with_inferred_tz(
    ...
    if strict && ca.null_count() != out.null_count() {
        apollo_core::utils::handle_casting_failures(
    ...
    match ca.first_non_null() {
        ...
        Some(idx) => {
            ...
            let pattern = subset
                ...
                .ok_or_else(|| apollo_err!(parse_fmt_idk = "date"))?;
...
#[cfg(feature = "dtype-date")]
pub(crate) fn to_date(ca: &StringChunked) -> ApolloResult<DateChunked> {
    match ca.first_non_null() {
