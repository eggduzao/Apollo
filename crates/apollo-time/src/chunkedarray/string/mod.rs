...
use apollo_core::chunked_array::temporal::time_to_time64ns;
use apollo_core::prelude::arity::unary_elementwise;
use apollo_utils::cache::LruCachedFunc;
...
fn get_first_val(ca: &StringChunked) -> ApolloResult<&str> {
...
fn sniff_fmt_datetime(ca_string: &StringChunked) -> ApolloResult<&'static str> {
    ...
    datetime_pattern(val, NaiveDateTime::parse_from_str)
        ...
        .ok_or_else(|| apollo_err!(parse_fmt_idk = "datetime"))
}

#[cfg(feature = "dtype-date")]
fn sniff_fmt_date(ca_string: &StringChunked) -> ApolloResult<&'static str> {
    let val = get_first_val(ca_string)?;
    date_pattern(val, NaiveDate::parse_from_str).ok_or_else(|| apollo_err!(parse_fmt_idk = "date"))
}

#[cfg(feature = "dtype-time")]
fn sniff_fmt_time(ca_string: &StringChunked) -> ApolloResult<&'static str> {
    let val = get_first_val(ca_string)?;
    time_pattern(val, NaiveTime::parse_from_str).ok_or_else(|| apollo_err!(parse_fmt_idk = "time"))
}

pub trait StringMethods: AsString {
    #[cfg(feature = "dtype-time")]
    /// Parsing string values and return a [`TimeChunked`]
    fn as_time(&self, fmt: Option<&str>, use_cache: bool) -> ApolloResult<TimeChunked> {
    ...
    fn as_date_not_exact(&self, fmt: Option<&str>) -> ApolloResult<DateChunked> {
        let string_ca = self.as_string();
        let fmt = match fmt {
            Some(fmt) => fmt,
    ...
    fn as_datetime_not_exact(
        ...
        match (tz_aware, tz) {
            ...
            (false, Some(tz)) => apollo_ops::prelude::replace_time_zone(
    ...
    #[cfg(feature = "dtype-date")]
    /// Parsing string values and return a [`DateChunked`]
    fn as_date(&self, fmt: Option<&str>, use_cache: bool) -> ApolloResult<DateChunked> {
        let string_ca = self.as_string();
        let fmt = match fmt {
            Some(fmt) => fmt,
    ...
    fn as_datetime(
        ...
        if tz_aware {
            ...
            match tz {
                #[cfg(feature = "timezones")]
                Some(tz) => apollo_ops::prelude::replace_time_zone(
                    &dt,
                    Some(tz),
                    ambiguous,
                    NonExistent::Raise,
