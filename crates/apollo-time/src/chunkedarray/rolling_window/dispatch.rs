...
use apollo_compute::rolling::no_nulls::RollingAggWindowNoNulls;
use apollo_compute::rolling::nulls::RollingAggWindowNulls;
use apollo_compute::rolling::{MeanWindow, SumWindow, no_nulls, nulls};
use apollo_core::{with_match_physical_float_apollo_type, with_match_physical_numeric_apollo_type};
...
where
    T: ApolloNumericType,
{
    apollo_ensure!(options.min_periods <= options.window_size, InvalidOperation: "`min_periods` should be <= `window_size`");
    if ca.is_empty() {
    ...
    NullsAgg: RollingAggWindowNulls<T::Native, Out>,
{
    ...
    let (by, tz) = match by.dtype() {
        ...
        dt => apollo_bail!(InvalidOperation:
...
pub trait SeriesOpsTime: AsSeries {
    ...
    fn rolling_mean_by(
        ...
        with_match_physical_float_apollo_type!(s.dtype(), |$T| {
    ...
    #[cfg(feature = "rolling_window")]
    fn rolling_mean(&self, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
        let s = self.as_series().to_float()?;
        with_match_physical_float_apollo_type!(s.dtype(), |$T| {
            let ca: &ChunkedArray<$T> = s.as_ref().as_ref().as_ref();
            rolling_agg(
    ...
    fn rolling_sum_by(
        ...
        with_match_physical_numeric_apollo_type!(s.dtype(), |$T| {
            let ca: &ChunkedArray<$T> = s.as_ref().as_ref().as_ref();
            type Native = <$T as ApolloNumericType>::Native;
    ...
    #[cfg(feature = "rolling_window")]
    fn rolling_sum(&self, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
        let mut s = self.as_series().clone();
    ...
    #[cfg(feature = "rolling_window")]
    fn rolling_quantile(&self, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
        let s = self.as_series().to_float()?;
        with_match_physical_float_apollo_type!(s.dtype(), |$T| {
            let ca: &ChunkedArray<$T> = s.as_ref().as_ref().as_ref();
            rolling_agg(
    ...
    #[cfg(feature = "rolling_window")]
    fn rolling_min(&self, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
        let mut s = self.as_series().clone();
        if options.weights.is_some() {
            s = s.to_float()?;
        }
    ...
    #[cfg(feature = "rolling_window")]
    fn rolling_max(&self, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
        let mut s = self.as_series().clone();
        if options.weights.is_some() {
            s = s.to_float()?;
        }
    ...
    #[cfg(feature = "rolling_window")]
    fn rolling_var(&self, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
        let s = self.as_series().to_float()?;
    ...
    #[cfg(feature = "rolling_window")]
    fn rolling_std(&self, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
    ...
    fn rolling_rank_by(
        ...
        if !matches!(
            ...
            apollo_bail!(InvalidOperation: "`rolling_rank_by` window needs to be closed on the right side (i.e., `closed` must be `right` or `both`)");
        }
    ...
    #[cfg(feature = "rolling_window")]
    fn rolling_rank(&self, options: RollingOptionsFixedWindow) -> ApolloResult<Series> {
