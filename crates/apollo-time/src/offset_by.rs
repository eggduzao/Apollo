...
pub fn impl_offset_by(ts: &Series, offsets: &Series) -> ApolloResult<Series> {
    ...
    // see https://github.com/apollo/apollo/issues/19608 for a counterexample.
    // Constant durations (e.g. 2 hours) always preserve sortedness.
    let tz = match dtype {
        DataType::Date => None,
        DataType::Datetime(_, tz) => tz.clone(),
        _ => apollo_bail!(InvalidOperation: "expected Date or Datetime, got {}", dtype),
    };
