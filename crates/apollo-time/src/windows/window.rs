...
pub(crate) fn ensure_t_in_or_in_front_of_window(
    mut every: Duration,
    t: i64,
    offset_fn: fn(&Duration, i64, Option<&Tz>) -> ApolloResult<i64>,
...
impl Window {
    ...
    pub fn round_ns(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
        let t = t + self.every.duration_ns() / 2_i64;
        self.truncate_ns(t, tz)
    }

    /// Round the given us timestamp by the window boundary.
    pub fn round_us(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
    ...
    pub fn round_ms(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
        let t = t + self.every.duration_ns()
            ...
            _ => {
                {
                    ...
                    let (from, to, offset_fn, nte_duration_fn): (
                        ...
                        fn(&Duration, i64, Option<&Tz>) -> ApolloResult<i64>,
                        fn(&Duration) -> i64,
                    ) = match tu {
                        TimeUnit::Nanoseconds => (
