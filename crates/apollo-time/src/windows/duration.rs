...
use apollo_core::datatypes::DataType;
use apollo_core::prelude::{
    ApolloResult, TimeZone, datetime_to_timestamp_ms, datetime_to_timestamp_ns,
    datetime_to_timestamp_us, apollo_bail,
};
use apollo_error::apollo_ensure;
#[cfg(feature = "serde")]
...
impl Duration {
    ...
    pub fn try_parse(duration: &str) -> ApolloResult<Self> {
        Self::_parse(duration, false)
    }

    pub fn try_parse_interval(interval: &str) -> ApolloResult<Self> {
        Self::_parse(&interval.to_ascii_lowercase(), true)
    }

    fn _parse(s: &str, as_interval: bool) -> ApolloResult<Self> {
        ...
        macro_rules! error_on_second_plus_minus {
            ($ch:expr) => {{
                ...
                if previously_seen {
                    apollo_bail!(InvalidOperation: "{} string can only have a single '{}' sign", parse_type, $ch as char);
                }
                let sign = if $ch == b'-' { "minus" } else { "plus" };
                if as_interval {
                    apollo_bail!(InvalidOperation: "{} signs are not currently supported in interval strings", sign);
                } else {
                    apollo_bail!(InvalidOperation: "only a single {} sign is allowed, at the front of the string", sign);
                }
            }};
        }
            ...
            match unit {
                ...
                _ if as_interval => match unit {
                    ...
                    _ => {
                        ...
                        apollo_bail!(InvalidOperation: "unit: '{}' not supported; available units include: {} (and their plurals)", unit_str, valid_units);
                    },
                },
                _ => {
                    let unit_str = std::str::from_utf8(unit).unwrap_or("<invalid>");
                    apollo_bail!(InvalidOperation: "unit: '{}' not supported; available units are: 'y', 'mo', 'q', 'w', 'd', 'h', 'm', 's', 'ms', 'us', 'ns'", unit_str);
                },
            }
        }
    ...
    fn add_month(ts: NaiveDateTime, n_months: i64, negative: bool) -> NaiveDateTime {
        ...
        new_datetime(year, month as u32, day, hour, minute, sec, nsec).expect(
            "Expected valid datetime, please open an issue at https://github.com/apollo/apollo/issues"
        )
    }
        ...
        J: Fn(NaiveDateTime) -> i64,
    {
        match (self.months, self.weeks, self.days, self.nsecs) {
            (0, 0, 0, 0) => apollo_bail!(ComputeError: "duration cannot be zero"),
            ...
            _ => {
                apollo_bail!(ComputeError: "cannot mix month, week, day, and sub-daily units for this operation")
            },
        }
    }
    ...
    #[inline]
    pub fn truncate_ns(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
        self.truncate_impl(
            t,
            tz,
            |nsecs| nsecs,
    ...
    // Truncate the given ns timestamp by the window boundary.
    #[inline]
    pub fn truncate_us(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
        self.truncate_impl(
            t,
            tz,
            |nsecs| nsecs / 1000,
    ...
    #[inline]
    pub fn truncate_ms(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
        self.truncate_impl(
            t,
            tz,
    ...
    pub fn add_ns(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
        let d = self;
    ...
    pub fn add_us(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
        let d = self;
        let new_t = self.add_impl_month_week_or_day(
            t,
            tz,
    ...
    pub fn add_ms(&self, t: i64, tz: Option<&Tz>) -> ApolloResult<i64> {
        let d = self;
        let new_t = self.add_impl_month_week_or_day(
            t,
            tz,
...
pub fn ensure_is_constant_duration(
    ...
    apollo_ensure!(duration.is_constant_duration(time_zone),
...
pub fn ensure_duration_matches_dtype(
    ...
    match dtype {
        DataType::Int64 | DataType::UInt64 | DataType::Int32 | DataType::UInt32 => {
            apollo_ensure!(duration.parsed_int || duration.is_zero(),
        ...
        DataType::Datetime(_, _) | DataType::Date | DataType::Duration(_) | DataType::Time => {
            apollo_ensure!(!duration.parsed_int,
        ...
        _ => {
            apollo_bail!(InvalidOperation: "unsupported data type: {} for temporal/index column, expected UInt64, UInt32, Int64, Int32, Datetime, Date, Duration, or Time", dtype)
        },
    }
    Ok(())
}
