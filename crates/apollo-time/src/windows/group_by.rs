...
pub fn group_by_windows(
    ...
    include_upper_bound: bool,
    start_by: StartBy,
) -> ApolloResult<(GroupsSlice, Vec<i64>, Vec<i64>)> {
    let start = time[0];
...
) -> ApolloResult<impl TrustedLen<Item = ApolloResult<(IdxSize, IdxSize)>> + '_> {
...
pub(crate) fn group_by_values_iter_window_behind_t(
    ...
    tu: TimeUnit,
    tz: Option<Tz>,
) -> impl TrustedLen<Item = ApolloResult<(IdxSize, IdxSize)>> + '_ {
    let add = match tu {
        TimeUnit::Nanoseconds => Duration::add_ns,
        TimeUnit::Microseconds => Duration::add_us,
        TimeUnit::Milliseconds => Duration::add_ms,
...
pub struct RollingWindower {
    ...
    add: fn(&Duration, i64, Option<&Tz>) -> ApolloResult<i64>,
    tz: Option<Tz>,
...
impl GroupByDynamicWindower {
    ...
    pub fn find_first_window_around(
        ...
        mut lower_bound: i64,
        target: i64,
    ) -> ApolloResult<Result<(i64, i64), i64>> {
    ...
    fn start_lower_bound(&self, first: i64) -> ApolloResult<i64> {
        match self.start_by {
