...
use apollo_core::utils::arrow::temporal_conversions::{
...
pub trait ApolloMonthEnd {
    fn month_end(&self, time_zone: Option<&Tz>) -> ApolloResult<Self>
    where
        Self: Sized;
}

impl ApolloMonthEnd for DatetimeChunked {
    fn month_end(&self, time_zone: Option<&Tz>) -> ApolloResult<Self> {
        let timestamp_to_datetime: fn(i64) -> NaiveDateTime;
        let datetime_to_timestamp: fn(NaiveDateTime) -> i64;
        let offset_fn: fn(&Duration, i64, Option<&Tz>) -> ApolloResult<i64>;
...
impl ApolloMonthEnd for DateChunked {
    fn month_end(&self, _time_zone: Option<&Tz>) -> ApolloResult<Self> {
        ...
        let ret = self.phys.try_apply_nonnull_values_generic(|t| {
            ...
            ApolloResult::Ok((fwd / MSECS_IN_DAY) as i32)
        })?;
        Ok(ret.into_date())
    }
}
