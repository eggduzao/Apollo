...
pub trait ApolloRound {
    fn round(&self, every: &StringChunked, tz: Option<&Tz>) -> ApolloResult<Self>
    where
        Self: Sized;
}

impl ApolloRound for DatetimeChunked {
    fn round(&self, every: &StringChunked, tz: Option<&Tz>) -> ApolloResult<Self> {
        ...
        if every.len() == 1 {
            if let Some(every) = every.get(0) {
                ...
                if every_parsed.negative {
                    apollo_bail!(ComputeError: "cannot round a Datetime to a negative duration")
                }
...
impl ApolloRound for DateChunked {
    fn round(&self, every: &StringChunked, _tz: Option<&Tz>) -> ApolloResult<Self> {
        let offset = Duration::new(0);
        let out = match every.len() {
            1 => {
                if let Some(every) = every.get(0) {
                    ...
                    if every.negative {
                        apollo_bail!(ComputeError: "cannot round a Date to a negative duration")
                    }
                    let w = Window::new(every, every, offset);
