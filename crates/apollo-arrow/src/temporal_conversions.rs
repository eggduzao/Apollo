...
pub fn parse_offset(offset: &str) -> ApolloResult<FixedOffset> {
    if offset == "UTC" {
    ...
    let first: &str = a
        .next()
        .ok_or_else(|| apollo_err!(InvalidOperation: error))?;
    let last = a
        .next()
        .ok_or_else(|| apollo_err!(InvalidOperation: error))?;
    let hours: i32 = first
        .parse()
        .map_err(|_| apollo_err!(InvalidOperation: error))?;
...
pub fn parse_offset_tz(timezone: &str) -> ApolloResult<chrono_tz::Tz> {
    timezone
        .parse::<chrono_tz::Tz>()
        .map_err(|_| apollo_err!(InvalidOperation: "timezone \"{timezone}\" cannot be parsed"))
}
