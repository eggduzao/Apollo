...
impl NullArray {
    ...
    pub fn try_new(dtype: ArrowDataType, length: usize) -> ApolloResult<Self> {
        if dtype.to_physical_type() != PhysicalType::Null {
            apollo_bail!(ComputeError: "NullArray can only be initialized with a DataType whose physical type is Null");
        }
...
impl<A: ffi::ArrowArrayRef> FromFfi<A> for NullArray {
    unsafe fn try_from_ffi(array: A) -> ApolloResult<Self> {
