...
{
    fn try_extend<II: IntoIterator<Item = Option<I>>>(&mut self, iter: II) -> ApolloResult<()> {
...
{
    #[inline]
    fn try_push(&mut self, item: Option<I>) -> ApolloResult<()> {
...
impl<O: Offset, M: MutableArray> MutableListArray<O, M> {
    ...
    pub fn try_push_valid(&mut self) -> ApolloResult<()> {
    ...
    pub fn try_extend_from_lengths<II>(&mut self, iterator: II) -> ApolloResult<()>
    where
