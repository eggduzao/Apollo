...
impl<O: Offset> ListArray<O> {
    ...
    pub fn try_new(
        ...
        if child_dtype != values_dtype {
            apollo_bail!(ComputeError: "ListArray's child's DataType must match. However, the expected DataType is {child_dtype:?} while it got {values_dtype:?}.");
        }
...
impl<O: Offset> ListArray<O> {
    ...
    /// # Errors
    /// Panics iff the logical type is not consistent with this struct.
    pub fn try_get_child(dtype: &ArrowDataType) -> ApolloResult<&Field> {
        if O::IS_LARGE {
            match dtype.to_storage() {
                ArrowDataType::LargeList(child) => Ok(child.as_ref()),
                _ => apollo_bail!(ComputeError: "ListArray<i64> expects DataType::LargeList"),
            }
        } else {
            match dtype.to_storage() {
                ArrowDataType::List(child) => Ok(child.as_ref()),
                _ => apollo_bail!(ComputeError: "ListArray<i32> expects DataType::List"),
            }
        }
    }
