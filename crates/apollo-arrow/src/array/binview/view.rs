    ...
    for view in views {
        if let Some(inline_slice) = view.get_inlined_slice() {
            ...
            let b = data
                .as_ref()
                .get(start..end)
                .ok_or_else(|| apollo_err!(OutOfBounds: "buffer slice out of bounds"))?;

            apollo_ensure!(b.starts_with(&view.prefix.to_le_bytes()), ComputeError: "prefix does not match string data");
            validate_bytes(b)?;
        }
    }
...
pub fn validate_binary_views<B: AsRef<[u8]>>(views: &[View], buffers: &[B]) -> ApolloResult<()> {
    validate_views(views, buffers, |_| Ok(()))
}

fn validate_utf8(b: &[u8]) -> ApolloResult<()> {
    match simdutf8::basic::from_utf8(b) {
        Ok(_) => Ok(()),
        Err(_) => Err(apollo_err!(ComputeError: "invalid utf8")),
    }
}

pub fn validate_utf8_views<B: AsRef<[u8]>>(views: &[View], buffers: &[B]) -> ApolloResult<()> {
