...
impl<T: ViewType + ?Sized> BinaryViewArrayGeneric<T> {
    ...
    pub fn try_new(
        ...
        if let Some(validity) = &validity {
            apollo_ensure!(validity.len()== views.len(), ComputeError: "validity mask length must match the number of values" )
        }
...
impl BinaryViewArray {
    ...
    pub fn validate_utf8(&self) -> ApolloResult<()> {
    ...
    pub fn to_utf8view(&self) -> ApolloResult<Utf8ViewArray> {
        self.validate_utf8()?;
