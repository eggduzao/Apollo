...
impl MapArray {
    ...
    pub fn try_new(
        ...
        if let ArrowDataType::Struct(inner) = inner_field.dtype() {
            if inner.len() != 2 {
                apollo_bail!(ComputeError: "MapArray's inner `Struct` must have 2 fields (keys and maps)")
            }
        } else {
            apollo_bail!(ComputeError: "MapArray expects `DataType::Struct` as its inner logical type")
        }
        if field.dtype() != inner_field.dtype() {
            apollo_bail!(ComputeError: "MapArray expects `field.dtype` to match its inner DataType")
        }
        ...
        {
            apollo_bail!(ComputeError: "validity mask length must match the number of values")
        }
...
impl MapArray {
    ...
    pub(crate) fn try_get_field(dtype: &ArrowDataType) -> ApolloResult<&Field> {
        if let ArrowDataType::Map(field, _) = dtype.to_storage() {
            Ok(field.as_ref())
        } else {
            apollo_bail!(ComputeError: "The dtype's logical type must be DataType::Map")
        }
    }
