...
use apollo_utils::index::{Bounded, Indexable, NullCount};
...
/// # Example
/// ```
/// use apollo_arrow::array::PrimitiveArray;
/// use apollo_arrow::bitmap::Bitmap;
/// use apollo_buffer::Buffer;
///
...
pub(super) fn check<T: NativeType>(
    ...
    if dtype.to_physical_type() != PhysicalType::Primitive(T::PRIMITIVE) {
        apollo_bail!(ComputeError: "PrimitiveArray can only be initialized with a DataType whose physical type is Primitive")
    }
    Ok(())
}
...
impl<T: NativeType> PrimitiveArray<T> {
    ...
    /// # Example
    /// ```
    /// use apollo_arrow::array::Int32Array;
    /// use apollo_arrow::datatypes::ArrowDataType;
    ///
