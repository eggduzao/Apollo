...
/// # Example
/// ```
/// use apollo_arrow::array::*;
/// use apollo_arrow::datatypes::*;
    ...
    pub fn try_new(
        ...
        values
            ...
            .try_for_each(|(index, f_length)| {
                if f_length != length {
                    apollo_bail!(ComputeError: "The children must have the given number of values.
        ...
        {
            apollo_bail!(ComputeError:"The validity length of a StructArray must match its number of elements")
        }
...
impl StructArray {
    ...
    pub(crate) fn try_get_fields(dtype: &ArrowDataType) -> ApolloResult<&[Field]> {
        match dtype.to_storage() {
            ArrowDataType::Struct(fields) => Ok(fields),
            _ => {
                apollo_bail!(ComputeError: "Struct array must be created with a DataType whose physical type is Struct")
            },
        }
    }
