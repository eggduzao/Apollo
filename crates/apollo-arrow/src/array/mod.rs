...
//! All immutable arrays are backed by [`Buffer`](apollo_buffer::Buffer) and thus cloning and slicing them is `O(1)`.
//!
...
pub trait TryExtend<A> {
    ...
    fn try_extend<I: IntoIterator<Item = A>>(&mut self, iter: I) -> ApolloResult<()>;
}

/// A trait describing the ability of a struct to receive new items.
pub trait TryPush<A> {
    /// Tries to push a new element.
    fn try_push(&mut self, item: A) -> ApolloResult<()>;
}
