...
impl<O: Offset> MutableUtf8Array<O> {
    ...
    pub fn try_new(
        ...
        {
            apollo_bail!(ComputeError: "validity's length must be equal to the number of values")
        }
...
impl<O: Offset, T: AsRef<str>> TryExtend<Option<T>> for MutableUtf8Array<O> {
    fn try_extend<I: IntoIterator<Item = Option<T>>>(&mut self, iter: I) -> ApolloResult<()> {
        let mut iter = iter.into_iter();
        self.reserve(iter.size_hint().0, 0);
...
impl<O: Offset, T: AsRef<str>> TryPush<Option<T>> for MutableUtf8Array<O> {
    #[inline]
    fn try_push(&mut self, value: Option<T>) -> ApolloResult<()> {
        match value {
            Some(value) => {
