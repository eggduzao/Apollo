...
impl FixedSizeBinaryArray {
    pub(crate) fn maybe_get_size(dtype: &ArrowDataType) -> ApolloResult<usize> {
        match dtype.to_storage() {
            ArrowDataType::Float16 => Ok(2),
            ArrowDataType::FixedSizeBinary(size) => {
                apollo_ensure!(*size != 0, ComputeError: "FixedSizeBinaryArray expects a positive size");
                Ok(*size)
            },
            other => {
                apollo_bail!(ComputeError: "FixedSizeBinaryArray expects DataType::FixedSizeBinary. found {other:?}")
            },
        }
    }
