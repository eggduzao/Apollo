...
pub(crate) fn try_check_offsets_bounds<O: Offset>(
    ...
    if offsets.last().unwrap().to_usize() > values_len {
        apollo_bail!(ComputeError: "offsets must not exceed the values length")
    } else {
        Ok(())
    }
}
...
pub fn try_check_utf8<O: Offset>(offsets: &[O], values: &[u8]) -> ApolloResult<()> {
    if offsets.len() == 1 {
        return Ok(());
    }
    ...
    if values_range.is_ascii() {
        ...
        if any_invalid {
            apollo_bail!(ComputeError: "non-valid char boundary detected")
        }
        Ok(())
    }
}
...
pub(crate) unsafe fn check_indexes_unchecked<K: DictionaryKey>(
    ...
    if invalid {
        ...
        apollo_bail!(ComputeError: "one of the dictionary keys is {key} but it must be < than the length of the dictionary values, which is {len}")
    } else {
        Ok(())
    }
}

pub fn check_indexes<K>(keys: &[K], len: usize) -> ApolloResult<()>
where
    K: std::fmt::Debug + Copy + TryInto<usize>,
{
    keys.iter().try_for_each(|key| {
        let key: usize = (*key)
            .try_into()
            .map_err(|_| apollo_err!(ComputeError: "The dictionary key must fit in a `usize`, but {key:?} does not")
            )?;
        if key >= len {
            apollo_bail!(ComputeError: "one of the dictionary keys is {key} but it must be < than the length of the dictionary values, which is {len}")
        } else {
