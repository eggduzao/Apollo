...
impl<O: Offset> MutableBinaryValuesArray<O> {
    ...
    pub fn try_new(
        ...
        if dtype.to_physical_type() != Self::default_dtype().to_physical_type() {
            apollo_bail!(ComputeError: "MutableBinaryValuesArray can only be initialized with DataType::Binary or DataType::LargeBinary",)
        }
...
impl<O: Offset> MutableBinaryValuesArray<O> {
    ...
    pub fn try_from_iter<P: AsRef<[u8]>, I: IntoIterator<Item = P>>(iter: I) -> ApolloResult<Self> {
...
impl<O: Offset, T: AsRef<[u8]>> TryExtend<T> for MutableBinaryValuesArray<O> {
    fn try_extend<I: IntoIterator<Item = T>>(&mut self, iter: I) -> ApolloResult<()> {
        let mut iter = iter.into_iter();
        self.reserve(iter.size_hint().0, 0);
...
impl<O: Offset, T: AsRef<[u8]>> TryPush<T> for MutableBinaryValuesArray<O> {
    #[inline]
    fn try_push(&mut self, value: T) -> ApolloResult<()> {
...
impl<O: Offset> TryExtendFromSelf for MutableBinaryValuesArray<O> {
    fn try_extend_from_self(&mut self, other: &Self) -> ApolloResult<()> {
