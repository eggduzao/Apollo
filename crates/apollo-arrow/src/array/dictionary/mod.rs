...
fn check_dtype(
    ...
    if let ArrowDataType::Dictionary(key, value, _) = dtype.to_storage() {
        if *key != key_type {
            apollo_bail!(ComputeError: "DictionaryArray must be initialized with a DataType::Dictionary whose integer is compatible to its keys")
        }
        if value.as_ref().to_storage() != values_dtype.to_storage() {
            apollo_bail!(ComputeError: "DictionaryArray must be initialized with a DataType::Dictionary whose value is equal to its values")
        }
    } else {
        apollo_bail!(ComputeError: "DictionaryArray must be initialized with logical DataType::Dictionary")
    }
    Ok(())
}
...
impl<K: DictionaryKey> DictionaryArray<K> {
    ...
    /// This function errors iff
    /// * any of the keys's values is not represented in `usize` or is `>= values.len()`
    pub fn try_from_keys(keys: PrimitiveArray<K>, values: Box<dyn Array>) -> ApolloResult<Self> {
    ...
    pub fn values_iter_typed<V: DictValue>(
        &self,
    ) -> ApolloResult<DictionaryValuesIterTyped<'_, K, V>> {
        let keys = &self.keys;
    ...
    pub fn iter_typed<V: DictValue>(&self) -> ApolloResult<DictionaryIterTyped<'_, K, V>> {
        let keys = &self.keys;
        let values = self.values.as_ref();
        let values = V::downcast_values(values)?;
    ...
    pub(crate) fn try_get_child(dtype: &ArrowDataType) -> ApolloResult<&ArrowDataType> {
        Ok(match dtype.to_storage() {
            ArrowDataType::Dictionary(_, values, _) => values.as_ref(),
            _ => {
                apollo_bail!(ComputeError: "Dictionaries must be initialized with DataType::Dictionary")
            },
        })
    }
