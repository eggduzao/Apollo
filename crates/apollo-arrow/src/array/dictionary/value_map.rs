...
impl<K: DictionaryKey, M: MutableArray> ValueMap<K, M> {
    pub fn try_empty(values: M) -> ApolloResult<Self> {
        if !values.is_empty() {
            apollo_bail!(ComputeError: "initializing value map with non-empty values array")
        }
        Ok(Self {
            values,
    ...
    pub fn from_values(values: M) -> ApolloResult<Self>
    where
        M: Indexable,
        M::Type: Eq + Hash,
    {
        ...
        for index in 0..values.len() {
            let key = K::try_from(index).map_err(|_| apollo_err!(ComputeError: "overflow"))?;
            ...
            match entry {
                Entry::Occupied(_) => {
                    apollo_bail!(InvalidOperation: "duplicate value in dictionary values array")
                },
                Entry::Vacant(entry) => {
    ...
    pub fn try_push_valid<V>(
        &mut self,
        value: V,
        mut push: impl FnMut(&mut M, V) -> ApolloResult<()>,
    ) -> ApolloResult<K>
    where
        M: Indexable,
        V: AsIndexed<M>,
