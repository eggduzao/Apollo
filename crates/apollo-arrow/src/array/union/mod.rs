...
impl UnionArray {
    ...
    pub fn try_new(
        ...
        if f.len() != fields.len() {
            apollo_bail!(ComputeError: "the number of `fields` must equal the number of children fields in DataType::Union")
        };
        let number_of_fields: i8 = fields.len().try_into().map_err(
            |_| apollo_err!(ComputeError: "the number of `fields` cannot be larger than i8::MAX"),
        )?;
...
impl UnionArray {
    fn try_get_all(dtype: &ArrowDataType) -> ApolloResult<UnionComponents<'_>> {
