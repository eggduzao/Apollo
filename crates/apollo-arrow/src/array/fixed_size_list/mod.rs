...
impl FixedSizeListArray {
    ...
    pub fn try_new(
        ...
        if child_dtype != values_dtype {
            apollo_bail!(ComputeError: "FixedSizeListArray's child's DataType must match. However, the expected DataType is {child_dtype:?} while it got {values_dtype:?}.")
        }

        apollo_ensure!(size == 0 || values.len().is_multiple_of(size), ComputeError:
            "values (of len {}) must be a multiple of size ({}) in FixedSizeListArray.",
            values.len(),
            size
        );

        apollo_ensure!(size == 0 || values.len() / size == length, ComputeError:
            "length of values ({}) is not equal to given length ({}) in FixedSizeListArray({size}).",
            values.len() / size,
            length,
        );
        apollo_ensure!(size != 0 || values.is_empty(), ComputeError:
        ...
        {
            apollo_bail!(ComputeError: "validity mask length must be equal to the number of values divided by size")
        }
    ...
    pub fn from_shape(
        ...
        apollo_ensure!(num_infers <= 1, InvalidOperation: "can only specify one inferred dimension");
...
impl FixedSizeListArray {
    pub(crate) fn try_child_and_size(dtype: &ArrowDataType) -> ApolloResult<(&Field, usize)> {
        match dtype.to_storage() {
            ArrowDataType::FixedSizeList(child, size) => Ok((child.as_ref(), *size)),
            _ => apollo_bail!(ComputeError: "FixedSizeListArray expects DataType::FixedSizeList"),
        }
    }
