...
use apollo_error::{ApolloError, apollo_bail};
#[cfg(feature = "serde")]
...
impl FromStr for Ambiguous {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            s => apollo_bail!(InvalidOperation:
...
pub fn convert_to_naive_local(
...
) -> ApolloResult<Option<NaiveDateTime>> {
    ...
    match to_tz.from_local_datetime(&ndt) {
        ...
        LocalResult::Ambiguous(dt_earliest, dt_latest) => match ambiguous {
            ...
            Ambiguous::Raise => {
                apollo_bail!(ComputeError: "datetime '{}' is ambiguous in time zone '{}'. Please use `ambiguous` to tell how it should be localized.", ndt, to_tz)
            },
        },
        LocalResult::None => match non_existent {
            NonExistent::Raise => apollo_bail!(ComputeError:
