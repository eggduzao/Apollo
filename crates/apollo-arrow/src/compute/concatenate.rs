...
pub fn concatenate(arrays: &[&dyn Array]) -> ApolloResult<Box<dyn Array>> {
    if arrays.is_empty() {
        apollo_bail!(InvalidOperation: "concat requires input of at least one array")
    }
    ...
    {
        apollo_bail!(InvalidOperation: "It is not possible to concatenate arrays of different data types.")
    }
...
pub fn concatenate_unchecked<A: AsRef<dyn Array>>(arrays: &[A]) -> ApolloResult<Box<dyn Array>> {
    if arrays.is_empty() {
        apollo_bail!(InvalidOperation: "concat requires input of at least one array")
    }
...
fn concatenate_utf8<O: Offset, A: AsRef<dyn Array>>(arrays: &[A]) -> ApolloResult<Utf8Array<O>> {
    let dtype = arrays[0].as_ref().dtype().clone();
    let (total_len, null_count) = len_null_count(arrays);
    let validity = concatenate_validities_with_len_null_count(arrays, total_len, null_count);
...
fn concatenate_list<O: Offset, A: AsRef<dyn Array>>(arrays: &[A]) -> ApolloResult<ListArray<O>> {
    let dtype = arrays[0].as_ref().dtype().clone();
    let (total_len, null_count) = len_null_count(arrays);
    let validity = concatenate_validities_with_len_null_count(arrays, total_len, null_count);
...
fn concatenate_fixed_size_binary<A: AsRef<dyn Array>>(
    arrays: &[A],
) -> ApolloResult<FixedSizeBinaryArray> {
    let dtype = arrays[0].as_ref().dtype().clone();
    let (total_len, null_count) = len_null_count(arrays);
    let validity = concatenate_validities_with_len_null_count(arrays, total_len, null_count);
...
fn concatenate_struct<A: AsRef<dyn Array>>(arrays: &[A]) -> ApolloResult<StructArray> {
    let dtype = arrays[0].as_ref().dtype().clone();
    let (total_len, null_count) = len_null_count(arrays);
    let validity = concatenate_validities_with_len_null_count(arrays, total_len, null_count);
