...
fn make_mutable(
    ...
    capacity: usize,
) -> ApolloResult<Box<dyn MutableArray>> {
    Ok(match dtype.to_physical_type() {
        ...
        _ => match dtype {
            ...
            other => {
                apollo_bail!(nyi = "Deserializing type {other:#?} is still not implemented")
            },
        },
    })
}
        ...
        _ => match dtype.to_physical_type() {
            ...
            PhysicalType::Primitive(primitive) => match primitive {
                ...
                PrimitiveType::Int128 => {
                    ...
                    if len > 16 {
                        apollo_bail!(oos = "Avro decimal bytes return more than 16 bytes")
                    }
            ...
            PhysicalType::Utf8 => {
                let len: usize = util::zigzag_i64(&mut block)?.try_into().map_err(|_| {
                    apollo_err!(oos = "Avro format contains a non-usize number of bytes")
                })?;
            ...
            loop {
                ...
                let bytes: Option<usize> = bytes
                    .map(|bytes| {
                        bytes
                            .try_into()
                            .map_err(|_| apollo_err!(oos = "Avro block size negative or too large"))
                    })
                    .transpose()?;
