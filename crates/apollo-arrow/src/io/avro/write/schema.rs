...
pub fn to_record(schema: &ArrowSchema, name: String) -> ApolloResult<Record> {
...
fn field_to_field(field: &Field, name_counter: &mut i32) -> ApolloResult<AvroField> {
...
fn _type_to_schema(dtype: &ArrowDataType, name_counter: &mut i32) -> ApolloResult<AvroSchema> {
    Ok(match dtype.to_storage() {
        ...
        other => apollo_bail!(nyi = "write {other:?} to avro"),
    })
}
