...
use apollo_utils::compression::ZstdLevel;
    ...
    match array.dtype().to_physical_type() {
        ...
        Dictionary(key_type) => match_integer_type!(key_type, |$T| {
            let dict_id = field.dictionary_id
                .ok_or_else(|| apollo_err!(InvalidOperation: "Dictionaries must have an associated id"))?;
        ...
        Struct => {
            ...
            if array.fields().len() != fields.len() {
                apollo_bail!(InvalidOperation: "The number of fields in a struct must equal the number of children in IpcField");
            }
            fields
                .iter()
        ...
        List => {
            let values = array
                ...
                .unwrap()
                .values();
            let field = field.fields.first().ok_or_else(|| apollo_err!(ComputeError: "Invalid IPC field structure: expected nested field but fields vector is empty"))?;
...
pub fn encode_chunk(
    ...
    dictionary_tracker: &mut DictionaryTracker,
    options: &WriteOptions,
) -> ApolloResult<(Vec<EncodedData>, EncodedData)> {
...
impl DictionaryTracker {
    ...
    ///   inserted.
    pub fn insert(&mut self, dict_id: i64, array: &dyn Array) -> ApolloResult<bool> {
