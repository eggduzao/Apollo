...
use apollo_utils::aliases::PlHashMap;
...
pub fn read_record_batch<R: Read + Seek>(
    ...
    let buffers = batch
        .buffers()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferBuffers(err)))?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingMessageBuffers))?;
    ...
    let field_nodes = batch
        .nodes()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferNodes(err)))?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingMessageNodes))?;
            ...
            .map(|maybe_field| match maybe_field {
                ProjectionResult::Selected((field, ipc_field)) => Ok(Some(read(
                    ...
                    batch.compression().map_err(|err| {
                        apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferCompression(err))
                    })?,
                    limit,
                    version,
                    scratch,
...
) -> ApolloResult<(&'a Field, &'a IpcField)> {
...
pub fn read_dictionary<R: Read + Seek>(
    ...
    if batch
        .is_delta()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferIsDelta(err)))?
    {
        apollo_bail!(ComputeError: "delta dictionary batches not supported")
    }

    let id = batch
        .id()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferId(err)))?;
    let (first_field, first_ipc_field) = first_dict_field(id, fields, &ipc_schema.fields)?;

    let batch = batch
        .data()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferData(err)))?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingData))?;

    let value_type =
        if let ArrowDataType::Dictionary(_, value_type, _) = first_field.dtype.to_storage() {
            value_type.as_ref()
        } else {
            apollo_bail!(oos = OutOfSpecKind::InvalidIdDataType { requested_id: id })
        };
