...
use apollo_error::{ApolloResult, apollo_bail, apollo_err};
...
fn try_unzip_vec<A, B, I: Iterator<Item = ApolloResult<(A, B)>>>(
    iter: I,
) -> ApolloResult<(Vec<A>, Vec<B>)> {
    let mut a = vec![];
    let mut b = vec![];
...
fn deserialize_field(ipc_field: arrow_format::ipc::FieldRef) -> ApolloResult<(Field, IpcField)> {
    ...
    let field = Field {
        name: PlSmallStr::from_str(
            ipc_field
                .name()?
                .ok_or_else(|| apollo_err!(oos = "Every field in IPC must have a name"))?,
        ),
        dtype,
...
fn read_metadata(field: &arrow_format::ipc::FieldRef) -> ApolloResult<Option<Metadata>> {
...
fn deserialize_integer(int: arrow_format::ipc::IntRef) -> ApolloResult<IntegerType> {
    Ok(match (int.bit_width()?, int.is_signed()?) {
        ...
        _ => apollo_bail!(oos = "IPC: indexType can only be 8, 16, 32, 64 or 128."),
    })
}

fn deserialize_timeunit(time_unit: arrow_format::ipc::TimeUnit) -> ApolloResult<TimeUnit> {
...
fn deserialize_time(time: TimeRef) -> ApolloResult<(ArrowDataType, IpcField)> {
...
fn deserialize_timestamp(timestamp: TimestampRef) -> ApolloResult<(ArrowDataType, IpcField)> {
...
fn deserialize_union(union_: UnionRef, field: FieldRef) -> ApolloResult<(ArrowDataType, IpcField)> {
    let mode = UnionMode::sparse(union_.mode()? == arrow_format::ipc::UnionMode::Sparse);
    let ids = union_.type_ids()?.map(|x| x.iter().collect());

    let fields = field
        .children()?
        .ok_or_else(|| apollo_err!(oos = "IPC: Union must contain children"))?;
    if fields.is_empty() {
        apollo_bail!(oos = "IPC: Union must contain at least one child");
    }
...
fn deserialize_map(map: MapRef, field: FieldRef) -> ApolloResult<(ArrowDataType, IpcField)> {
    let is_sorted = map.keys_sorted()?;

    let children = field
        .children()?
        .ok_or_else(|| apollo_err!(oos = "IPC: Map must contain children"))?;
    let inner = children
        .get(0)
        .ok_or_else(|| apollo_err!(oos = "IPC: Map must contain one child"))??;
...
fn deserialize_struct(field: FieldRef) -> ApolloResult<(ArrowDataType, IpcField)> {
    let fields = field
        .children()?
        .ok_or_else(|| apollo_err!(oos = "IPC: Struct must contain children"))?;
    let (fields, ipc_fields) = try_unzip_vec(fields.iter().map(|field| {
        let (field, fields) = deserialize_field(field?)?;
        Ok((field, fields))
    }))?;
...
fn deserialize_list(field: FieldRef) -> ApolloResult<(ArrowDataType, IpcField)> {
    let children = field
        .children()?
        .ok_or_else(|| apollo_err!(oos = "IPC: List must contain children"))?;
    let inner = children
        .get(0)
        .ok_or_else(|| apollo_err!(oos = "IPC: List must contain one child"))??;
    let (field, ipc_field) = deserialize_field(inner)?;
...
fn deserialize_large_list(field: FieldRef) -> ApolloResult<(ArrowDataType, IpcField)> {
    let children = field
        .children()?
        .ok_or_else(|| apollo_err!(oos = "IPC: List must contain children"))?;
    let inner = children
...
fn deserialize_fixed_size_list(
    ...
    field: FieldRef,
) -> ApolloResult<(ArrowDataType, IpcField)> {
    let children = field
        .children()?
        .ok_or_else(|| apollo_err!(oos = "IPC: FixedSizeList must contain children"))?;
    let inner = children
        .get(0)
        .ok_or_else(|| apollo_err!(oos = "IPC: FixedSizeList must contain one child"))??;
    let (field, ipc_field) = deserialize_field(inner)?;

    let size = list
        .list_size()?
        .try_into()
        .map_err(|_| apollo_err!(oos = OutOfSpecKind::NegativeFooterLength))?;
...
fn get_dtype(
    ...
    if let Some(dictionary) = field.dictionary()? {
        if may_be_dictionary {
            let int = dictionary
                .index_type()?
                .ok_or_else(|| apollo_err!(oos = "indexType is mandatory in Dictionary."))?;
    ...
    let type_ = field
        .type_()?
        .ok_or_else(|| apollo_err!(oos = "IPC: field type is mandatory"))?;
    ...
    Ok(match type_ {
        ...
        Decimal(decimal) => {
            ...
            let dtype = match bit_width {
                ...
                _ => return Err(apollo_err!(oos = OutOfSpecKind::NegativeFooterLength)),
            };
...
pub fn deserialize_schema(
    message: &[u8],
) -> ApolloResult<(ArrowSchema, IpcSchema, Option<Metadata>)> {
    let message = arrow_format::ipc::MessageRef::read_as_root(message)
        .map_err(|err| apollo_err!(oos = format!("Unable deserialize message: {err:?}")))?;

    let schema = match message
        .header()?
        .ok_or_else(|| apollo_err!(oos = "Unable to convert header to a schema".to_string()))?
    {
        arrow_format::ipc::MessageHeaderRef::Schema(schema) => ApolloResult::Ok(schema),
        _ => apollo_bail!(ComputeError: "The message is expected to be a Schema message"),
    }?;
...
pub(super) fn fb_to_schema(
    ...
    let fields = schema
        .fields()?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingFields))?;
...
pub(super) fn deserialize_stream_metadata(meta: &[u8]) -> ApolloResult<StreamMetadata> {
    let message = arrow_format::ipc::MessageRef::read_as_root(meta)
        .map_err(|err| apollo_err!(oos = format!("Unable to get root as message: {err:?}")))?;
    let version = message.version()?;
    // message header is a Schema, so read it
    let header = message
        .header()?
        .ok_or_else(|| apollo_err!(oos = "Unable to read the first IPC message"))?;
    let schema = if let arrow_format::ipc::MessageHeaderRef::Schema(schema) = header {
        schema
    } else {
        apollo_bail!(oos = "The first IPC message of the stream must be a schema")
    };
