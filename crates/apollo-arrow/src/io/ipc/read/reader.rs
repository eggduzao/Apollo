...
impl<R: Read + Seek> FileReader<R> {
    ...
    pub fn read_dictionaries(&mut self) -> ApolloResult<()> {
    ...
    pub fn skip_blocks_till_limit(&mut self, offset: u64) -> ApolloResult<u64> {
    ...
    pub fn next_record_batch(
        &mut self,
    ) -> Option<ApolloResult<arrow_format::ipc::RecordBatchRef<'_>>> {
...
impl<R: Read + Seek> BlockReader<R> {
    ...
    pub fn record_batch_num_rows(&mut self, message_scratch: &mut Vec<u8>) -> ApolloResult<usize> {
        let offset: u64 = 0;
    ...
    ) -> ApolloResult<Option<Vec<KeyValueRef<'a>>>> {
        let offset: u64 = 0;
        let message = get_message_from_block_offset(&mut self.reader, offset, message_scratch)?;
