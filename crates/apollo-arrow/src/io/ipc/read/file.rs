...
use apollo_utils::aliases::{InitHashMaps, PlHashMap};
...
pub fn get_row_count<R: Read + Seek>(reader: &mut R) -> ApolloResult<i64> {
...
) -> ApolloResult<arrow_format::ipc::DictionaryBatchRef<'a>> {
    let header = message
        .header()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferHeader(err)))?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingMessageHeader))?;
    match header {
        arrow_format::ipc::MessageHeaderRef::DictionaryBatch(batch) => Ok(batch),
        _ => apollo_bail!(oos = OutOfSpecKind::UnexpectedMessageType),
    }
}
...
pub fn read_dictionary_block<R: Read + Seek>(
    ...
    let offset: u64 = if force_zero_offset {
        ...
        block
            .offset
            .try_into()
            .map_err(|_| apollo_err!(oos = OutOfSpecKind::UnexpectedNegativeInteger))?
    };
...
pub(super) fn decode_footer_len(footer: [u8; 10], end: u64) -> ApolloResult<(u64, usize)> {
    let footer_len = i32::from_le_bytes(footer[..4].try_into().unwrap());

    if footer[4..] != ARROW_MAGIC_V2 {
        if footer[..4] == ARROW_MAGIC_V1 {
            apollo_bail!(ComputeError: "feather v1 not supported");
        }
        return Err(apollo_err!(oos = OutOfSpecKind::InvalidFooter));
    }
...
fn read_footer_len<R: Read + Seek>(reader: &mut R) -> ApolloResult<(u64, usize)> {
...
fn read_footer<R: Read + Seek>(reader: &mut R, footer_len: usize) -> ApolloResult<Vec<u8>> {
    // read footer
...
fn deserialize_footer_blocks(
    footer_data: &[u8],
) -> ApolloResult<(FooterRef<'_>, Vec<arrow_format::ipc::Block>)> {
    let footer = arrow_format::ipc::FooterRef::read_as_root(footer_data)
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferFooter(err)))?;

    let blocks = footer
        .record_batches()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferRecordBatches(err)))?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingRecordBatches))?;
...
pub(super) fn deserialize_footer_ref(footer_data: &[u8]) -> ApolloResult<FooterRef<'_>> {
...
) -> ApolloResult<arrow_format::ipc::SchemaRef<'_>> {
    footer
        .schema()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferSchema(err)))?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingSchema))
}

/// Get the IPC blocks from the footer containing record batches
pub(super) fn iter_recordbatch_blocks_from_footer(
    footer: arrow_format::ipc::FooterRef<'_>,
) -> ApolloResult<impl SendableIterator<Item = ApolloResult<arrow_format::ipc::Block>> + '_> {
    let blocks = footer
        .record_batches()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferRecordBatches(err)))?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingRecordBatches))?;
...
pub(super) fn iter_dictionary_blocks_from_footer(
    footer: arrow_format::ipc::FooterRef<'_>,
) -> ApolloResult<Option<impl SendableIterator<Item = ApolloResult<arrow_format::ipc::Block>> + '_>>
{
    let dictionaries = footer
        .dictionaries()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferDictionaries(err)))?;
...
pub fn deserialize_footer(footer_data: &[u8], size: u64) -> ApolloResult<FileMetadata> {
    let footer = deserialize_footer_ref(footer_data)?;
    let blocks = iter_recordbatch_blocks_from_footer(footer)?.collect::<ApolloResult<Vec<_>>>()?;
    let dictionaries = iter_dictionary_blocks_from_footer(footer)?
        .map(|dicts| dicts.collect::<ApolloResult<Vec<_>>>())
        .transpose()?;
...
pub fn read_file_metadata<R: Read + Seek>(reader: &mut R) -> ApolloResult<FileMetadata> {
...
) -> ApolloResult<arrow_format::ipc::RecordBatchRef> {
    let header = message
        .header()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferHeader(err)))?
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::MissingMessageHeader))?;
...
pub fn get_message_from_block_offset<'a, R: Read + Seek>(
...
) -> ApolloResult<arrow_format::ipc::MessageRef<'a>> {
    ...
    arrow_format::ipc::MessageRef::read_as_root(message_scratch)
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferMessage(err)))
}
...
pub fn read_batch<R: Read + Seek>(
    ...
    checked: UnsafeBool,
) -> ApolloResult<RecordBatchT<Box<dyn Array>>> {
    ...
    read_record_batch(
        ...
        message
            .version()
            .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferVersion(err)))?,
        reader,
        offset + length,
        data_scratch,
        checked,
