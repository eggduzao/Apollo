...
fn read_uncompressed_bytes<R: Read + Seek>(
    ...
    if is_native_little_endian() == is_little_endian {
        ...
        apollo_ensure!(buffer.len() == buffer_length, ComputeError: "Malformed IPC file: expected compressed buffer of len {buffer_length}, got {}", buffer.len());
...
fn read_compressed_buffer<T: NativeType, R: Read + Seek>(
    ...
    apollo_ensure!(scratch.len() == buffer_length, ComputeError: "Malformed IPC file: expected compressed buffer of len {buffer_length}, got {}", scratch.len());
    ...
    let decompressed_bytes: usize = if decompressed_len_field == -1 {
        ...
        decompressed_len_field.try_into().map_err(|_| {
            apollo_err!(ComputeError: "Malformed IPC file: got invalid decompressed length {decompressed_len_field}")
        })?
    };

    apollo_ensure!(decompressed_bytes.is_multiple_of(size_of::<T>()),
    ...
    let compression = compression
        .codec()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferCompression(err)))?;
...
pub fn read_bytes<R: Read + Seek>(
    ...
    let buf = buf
        .pop_front()
        .ok_or_else(|| apollo_err!(oos = OutOfSpecKind::ExpectedBuffer))?;
...
fn read_uncompressed_bitmap<R: Read + Seek>(
    ...
    apollo_ensure!(buffer.len() == bytes, ComputeError: "Malformed IPC file: expected compressed buffer of len {bytes}, got {}", buffer.len());
...
fn read_compressed_bitmap<R: Read + Seek>(
    ...
    if scratch.len() != bytes {
        apollo_bail!(ComputeError: "Malformed IPC file: expected compressed buffer of len {bytes}, got {}", scratch.len());
    }
    ...
    // see https://github.com/apollo/apollo/issues/26126
    // and https://github.com/apache/arrow/issues/48883
    apollo_ensure!(decompressed_bytes >= row_limit.div_ceil(8),
