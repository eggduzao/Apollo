    ...
    R: AsyncRead + AsyncSeek + Unpin + Send + 'a,
{
    ...
    let block_length: u64 = message
        .body_length()
        .map_err(|err| apollo_err!(oos = OutOfSpecKind::InvalidFlatbufferBodyLength(err)))?
        .try_into()
...
pub async fn into_flight_stream<R: AsyncRead + AsyncSeek + Unpin + Send>(
    reader: &mut R,
) -> ApolloResult<impl Stream<Item = ApolloResult<EncodedData>> + '_> {
...
pub struct FlightStreamProducer<'a, R: AsyncRead + AsyncSeek + Unpin + Send> {
    ...
    footer_data: Vec<u8>,
    dict_blocks: Option<Box<dyn SendableIterator<Item = ApolloResult<Block>>>>,
    data_blocks: Option<Box<dyn SendableIterator<Item = ApolloResult<Block>>>>,
    reader: &'a mut R,
}
...
impl<'a, R: AsyncRead + AsyncSeek + Unpin + Send> FlightStreamProducer<'a, R> {
    pub async fn new(reader: &'a mut R) -> ApolloResult<Pin<Box<Self>>> {
        let (_end, len) = read_footer_len(reader).await?;
        let footer_data = read_footer(reader, len).await?;
    ...
    pub fn init(self: &mut Pin<Box<Self>>) -> ApolloResult<()> {
    ...
    pub fn get_schema(self: &Pin<Box<Self>>) -> ApolloResult<EncodedData> {
...
impl FlightConsumer {
    pub fn new(first: EncodedData) -> ApolloResult<Self> {
    ...
    pub fn consume(&mut self, msg: EncodedData) -> ApolloResult<Option<RecordBatch>> {
        // Parse the header
        ...
        match header {
            MessageHeaderRef::Schema(_) => {
                apollo_bail!(ComputeError: "Unexpected schema message while parsing Stream");
            },
...
pub struct FlightstreamConsumer<S: Stream<Item = ApolloResult<EncodedData>> + Unpin> {
    inner: FlightConsumer,
    stream: S,
}

impl<S: Stream<Item = ApolloResult<EncodedData>> + Unpin> FlightstreamConsumer<S> {
    pub async fn new(mut stream: S) -> ApolloResult<Self> {
        let Some(first) = stream.next().await else {
            apollo_bail!(ComputeError: "expected the schema")
        };
        let first = first?;
    ...
    pub async fn next_batch(&mut self) -> ApolloResult<Option<RecordBatch>> {
...
mod test {
    ...
    fn get_file_path() -> PathBuf {
        let apollo_arrow = std::env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set");
        Path::new(&apollo_arrow).join("../../py-apollo/tests/unit/io/files/foods1.ipc")
    }
