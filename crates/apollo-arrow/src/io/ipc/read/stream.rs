...
use apollo_error::{ApolloError, ApolloResult, apollo_bail, apollo_err};
...
pub fn read_stream_metadata(reader: &mut dyn std::io::Read) -> ApolloResult<StreamMetadata> {
...
fn read_next<R: Read + Seek>(
    ...
    scratch: &mut Vec<u8>,
    checked: UnsafeBool,
) -> ApolloResult<Option<StreamState>> {
    // determine metadata length
...
///
/// For a more thorough walkthrough consult [this example](https://github.com/jorgecarleitao/apollo_arrow/tree/main/examples/ipc_pyarrow).
...
impl<R: Read + Seek> StreamReader<R> {
    ...
    fn maybe_next(&mut self) -> ApolloResult<Option<StreamState>> {
        if self.finished {
            return Ok(None);
        }
        let batch = read_next(
...
impl<R: Read + Seek> Iterator for StreamReader<R> {
    type Item = ApolloResult<StreamState>;
