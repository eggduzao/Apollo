...
pub fn read_binary<O: Offset, R: Read + Seek>(
    ...
    .or_else(|_| ApolloResult::Ok(Buffer::<O>::from(vec![O::default()])))?;
...
pub fn skip_binary(
    ...
    let _ = buffers
        .pop_front()
        .ok_or_else(|| apollo_err!(oos = "IPC: missing offsets buffer."))?;
    let _ = buffers
        .pop_front()
        .ok_or_else(|| apollo_err!(oos = "IPC: missing values buffer."))?;
    Ok(())
}
