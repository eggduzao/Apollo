...
impl<R: Read + Seek + Write> FileWriter<R> {
    ...
    pub fn try_from_file(
    ...
    ) -> ApolloResult<FileWriter<R>> {
        if metadata.ipc_schema.is_little_endian != is_native_little_endian() {
            apollo_bail!(ComputeError: "appending to a file of a non-native endianness is not supported")
        }
        ...
        let last_block = metadata.blocks.last().ok_or_else(|| {
            apollo_err!(oos = "an Arrow IPC file must have at least 1 message (the schema message)")
        })?;
        let offset: u64 = last_block
            .offset
            .try_into()
            .map_err(|_| apollo_err!(oos = "the block's offset must be a positive number"))?;
        ...
        let body_length: u64 = last_block
            .body_length
            .try_into()
            .map_err(|_| apollo_err!(oos = "the block's body length must be a positive number"))?;
