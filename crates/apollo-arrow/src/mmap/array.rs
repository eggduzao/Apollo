...
fn get_buffer_bounds(buffers: &mut VecDeque<IpcBuffer>) -> ApolloResult<(usize, usize)> {
    let buffer = buffers.pop_front().ok_or_else(
        || apollo_err!(ComputeError: "out-of-spec {:?}", OutOfSpecKind::ExpectedBuffer),
    )?;

    let offset: usize = buffer.offset().try_into().map_err(
        |_| apollo_err!(ComputeError: "out-of-spec {:?}", OutOfSpecKind::NegativeFooterLength),
    )?;
...
fn check_bytes_len_and_is_aligned<T: NativeType>(
    ...
    if bytes.len() < size_of::<T>() * expected_len {
        apollo_bail!(ComputeError: "buffer's length is too small in mmap")
    };
...
fn get_buffer<'a, T: NativeType>(
    ...
    let values = data
        ...
        .ok_or_else(|| apollo_err!(ComputeError: "buffer out of bounds"))?;

    if !check_bytes_len_and_is_aligned::<T>(values, num_rows)? {
        apollo_bail!(ComputeError: "buffer not aligned for mmap");
    }
...
fn get_num_rows_and_null_count(node: &Node) -> ApolloResult<(usize, usize)> {
...
fn mmap_binview<T: AsRef<[u8]> + Send + Sync + 'static>(
    ...
    let n_variadic = variadic_buffer_counts
        .pop_front()
        .ok_or_else(|| apollo_err!(ComputeError: "expected variadic_buffer_count"))?;
...
fn mmap_fixed_size_binary<T: AsRef<[u8]> + Send + Sync + 'static>(
    ...
    let bytes_per_row = if let ArrowDataType::FixedSizeBinary(bytes_per_row) = dtype {
        ...
        apollo_bail!(ComputeError: "out-of-spec {:?}", OutOfSpecKind::InvalidDataType);
    };
    let (num_rows, null_count) = get_num_rows_and_null_count(node)?;
...
fn mmap_dict<K: DictionaryKey, T: AsRef<[u8]> + Send + Sync + 'static>(
    ...
    let dictionary = dictionaries
        ...
        .ok_or_else(|| apollo_err!(ComputeError: "out-of-spec: missing dictionary"))?
        .clone();
