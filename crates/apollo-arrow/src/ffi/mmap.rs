...
pub unsafe fn bitmap(data: &[u8], offset: usize, length: usize) -> ApolloResult<BooleanArray> {
    if offset >= 8 {
        apollo_bail!(InvalidOperation: "offset should be < 8")
    };
    if length > data.len() * 8 - offset {
        apollo_bail!(InvalidOperation: "given length is oob")
    }
