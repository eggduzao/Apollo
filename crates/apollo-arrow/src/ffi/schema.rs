...
pub(crate) unsafe fn to_field(schema: &ArrowSchema) -> ApolloResult<Field> {
...
fn to_integer_type(format: &str) -> ApolloResult<IntegerType> {
    use IntegerType::*;
    Ok(match format {
        "c" => Int8,
        "C" => UInt8,
...
unsafe fn to_dtype(schema: &ArrowSchema) -> ApolloResult<ArrowDataType> {
        ...
        other => {
            match other.splitn(2, ':').collect::<Vec<_>>()[..] {
                ...
                ["w", size_raw] => {
                    ...
                    let size = size_raw
                        .parse::<usize>()
                        .map_err(|_| apollo_err!(ComputeError: "size is not a valid integer"))?;
                    ...
                    let (precision, scale) = match raw.split(',').collect::<Vec<_>>()[..] {
                        ...
                        [precision_raw, scale_raw, width_raw] => {
                            ...
                            let bit_width = width_raw.parse::<usize>().map_err(|_| {
                                apollo_err!(ComputeError: "Decimal bit width is not a valid integer")
                            })?;
                            match bit_width {
                                32 => return Ok(ArrowDataType::Decimal32(
                                    precision_raw.parse::<usize>().map_err(|_| {
                                        apollo_err!(ComputeError: "Decimal precision is not a valid integer")
                                    })?,
                                    scale_raw.parse::<usize>().map_err(|_| {
                                        apollo_err!(ComputeError: "Decimal scale is not a valid integer")
                                    })?,
                                )),
...
fn to_format(dtype: &ArrowDataType) -> String {
    match dtype {
        ...
        // Doesn't exist in arrow, '_pl' prefixed is Apollo specific
...
pub(super) fn get_child(dtype: &ArrowDataType, index: usize) -> ApolloResult<ArrowDataType> {
    match (index, dtype) {
        ...
        (child, dtype) => apollo_bail!(ComputeError:
