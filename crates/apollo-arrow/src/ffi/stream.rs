...
unsafe fn handle_error(iter: &mut ArrowArrayStream) -> ApolloError {
    let error = unsafe { (iter.get_last_error.unwrap())(&mut *iter) };

    if error.is_null() {
        return apollo_err!(ComputeError: "got unspecified external error");
    }

    let error = unsafe { CStr::from_ptr(error) };
    apollo_err!(ComputeError: "got external error: {}", error.to_str().unwrap())
}
...
impl<Iter: DerefMut<Target = ArrowArrayStream>> ArrowArrayStreamReader<Iter> {
    ...
    pub unsafe fn try_new(mut iter: Iter) -> ApolloResult<Self> {
        if iter.release.is_none() {
            apollo_bail!(InvalidOperation: "the C stream was already released")
        };

        if iter.get_next.is_none() {
            apollo_bail!(InvalidOperation: "the C stream must contain a non-null get_next")
        };

        if iter.get_last_error.is_none() {
            apollo_bail!(InvalidOperation: "The C stream MUST contain a non-null get_last_error")
        };
    ...
    pub unsafe fn next(&mut self) -> Option<ApolloResult<Box<dyn Array>>> {
...
struct PrivateData {
    iter: Box<dyn Iterator<Item = ApolloResult<Box<dyn Array>>>>,
    field: Field,
