...
use apollo_buffer::{Buffer, SharedStorage};
...
pub unsafe fn try_from<A: ArrowArrayRef>(array: A) -> ApolloResult<Box<dyn Array>> {
    use PhysicalType::*;
...
unsafe fn buffer_len(array: &ArrowArray, dtype: &ArrowDataType, i: usize) -> ApolloResult<usize> {
...
unsafe fn create_child(
    ...
    index: usize,
) -> ApolloResult<ArrowArrayChild<'static>> {
    let dtype = get_child(dtype, index)?;

    // catch what we can
    if array.children.is_null() {
        apollo_bail!(ComputeError: "an ArrowArray of type {dtype:?} must have non-null children");
    }
...
unsafe fn create_dictionary(
    ...
    dtype: &ArrowDataType,
    parent: InternalArrowArray,
) -> ApolloResult<Option<ArrowArrayChild<'static>>> {
...
pub trait ArrowArrayRef: std::fmt::Debug {
    ...
    unsafe fn validity(&self) -> ApolloResult<Option<Bitmap>> {
    ...
    unsafe fn buffer<T: NativeType>(&self, index: usize) -> ApolloResult<Buffer<T>> {
    ...
    /// * the buffer at position `index` is valid for the declared length
    /// * the buffers' pointer is not mutable for the lifetime of `owner`
    unsafe fn bitmap(&self, index: usize) -> ApolloResult<Bitmap> {
    ...
    /// * the pointer of `array.children` at `index` is valid
    /// * the pointer of `array.children` at `index` is not mutably shared for the lifetime of `parent`
    unsafe fn child(&self, index: usize) -> ApolloResult<ArrowArrayChild<'_>> {
        create_child(self.array(), self.dtype(), self.parent().clone(), index)
    }

    unsafe fn dictionary(&self) -> ApolloResult<Option<ArrowArrayChild<'_>>> {
