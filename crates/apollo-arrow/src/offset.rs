...
impl<O: Offset> Offsets<O> {
    ...
    #[inline]
    pub fn try_from_iter<I: IntoIterator<Item = usize>>(iter: I) -> ApolloResult<Self> {
    ...
    #[inline]
    pub fn try_push(&mut self, length: usize) -> ApolloResult<()> {
        if O::IS_LARGE {
            ...
            let length =
                O::from_usize(length).ok_or_else(|| apollo_err!(ComputeError: "overflow"))?;

            let old_length = self.last();
            let new_length = old_length
                .checked_add(&length)
                .ok_or_else(|| apollo_err!(ComputeError: "overflow"))?;
            self.0.push(new_length);
            Ok(())
        }
    }
    ...
    #[inline]
    pub fn try_from_lengths<I: Iterator<Item = usize>>(lengths: I) -> ApolloResult<Self> {
    ...
    pub fn try_extend_from_lengths<I: Iterator<Item = usize>>(
        ...
        let last_offset = original_offset
            ...
            .ok_or_else(|| apollo_err!(ComputeError: "overflow"))?;
        O::from_usize(last_offset).ok_or_else(|| apollo_err!(ComputeError: "overflow"))?;
        Ok(())
    }
    ...
    /// # Errors
    /// This function errors iff this operation overflows for the maximum value of `O`.
    pub fn try_extend_from_self(&mut self, other: &Self) -> ApolloResult<()> {
...
fn try_check_offsets<O: Offset>(offsets: &[O]) -> ApolloResult<()> {
    // this code is carefully constructed to auto-vectorize, don't change naively!
    match offsets.first() {
        None => apollo_bail!(ComputeError: "offsets must have at least one element"),
        Some(first) => {
            if *first < O::zero() {
                apollo_bail!(ComputeError: "offsets must be larger than 0")
            }
            ...
            if any_invalid {
                apollo_bail!(ComputeError: "offsets must be monotonically increasing")
            } else {
                Ok(())
            }
        },
...
impl TryFrom<&OffsetsBuffer<i64>> for OffsetsBuffer<i32> {
    ...
    fn try_from(offsets: &OffsetsBuffer<i64>) -> Result<Self, Self::Error> {
        i32::try_from(*offsets.last()).map_err(|_| apollo_err!(ComputeError: "overflow"))?;
