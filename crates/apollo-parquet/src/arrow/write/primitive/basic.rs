    ...
    P: num_traits::AsPrimitive<i64>,
{
    match encoding {
        ...
        other => apollo_bail!(nyi = "Encoding integer as {other:?}"),
    }
    .map(Page::Data)
}
    ...
    T: num_traits::AsPrimitive<P>,
{
    let (min_value, max_value) = match (options.min_value, options.max_value) {
        (true, true) => {
            match apollo_compute::min_max::dyn_array_min_max_propagate_nan(array as &dyn Array) {
                None => (None, None),
                Some((l, r)) => (Some(l), Some(r)),
            }
        },
        (true, false) => (
            apollo_compute::min_max::dyn_array_min_propagate_nan(array as &dyn Array),
            None,
        ),
        (false, true) => (
            None,
            apollo_compute::min_max::dyn_array_max_propagate_nan(array as &dyn Array),
        ),
