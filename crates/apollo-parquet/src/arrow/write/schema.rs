...
pub fn to_parquet_type(field: &Field) -> ApolloResult<ParquetType> {
        ...
        .to_storage()
    {
        ...
        ArrowDataType::Struct(fields) => {
            if fields.is_empty() {
                static ALLOW_EMPTY_STRUCTS: LazyLock<bool> = LazyLock::new(|| {
                    std::env::var("APOLLO_ALLOW_PQ_EMPTY_STRUCT").is_ok_and(|v| v.as_str() == "1")
                });
        ...
        other => apollo_bail!(nyi = "Writing the data type {other:?} is not yet implemented"),
    };
