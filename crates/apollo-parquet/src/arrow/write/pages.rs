...
pub fn to_nested(array: &dyn Array, type_: &ParquetType) -> ApolloResult<Vec<Vec<Nested>>> {
    let mut nested = vec![];
...
fn to_nested_recursive(
    ...
    if !is_optional && array.null_count() > 0 {
        apollo_bail!(InvalidOperation: "writing a missing value to required field '{}'", type_.name());
    }
...
pub fn array_to_columns<A: AsRef<dyn Array> + Send + Sync>(
    ...
    encoding: &[Encoding],
) -> ApolloResult<Vec<DynIter<'static, ApolloResult<Page>>>> {
    ...
    let x = values
        ...
        .collect::<ApolloResult<Vec<DynIter<'static, ApolloResult<Page>>>>>()?;
    Ok(x)
}
