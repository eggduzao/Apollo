...
pub fn to_parquet_schema(schema: &ArrowSchema) -> ApolloResult<SchemaDescriptor> {
...
) -> ApolloResult<DynIter<'static, ApolloResult<Page>>> {
...
pub fn array_to_page_simple(
    ...
    if type_.field_info.repetition == Repetition::Required && array.null_count() > 0 {
        apollo_bail!(InvalidOperation: "writing a missing value to required parquet column '{}'", type_.field_info.name);
    }
    ...
    match dtype {
        ...
        ArrowDataType::LargeUtf8 => {
            let array =
                apollo_compute::cast::cast(array, &ArrowDataType::LargeBinary, Default::default())
                    .unwrap();
        ...
        ArrowDataType::Utf8View => {
            let array =
                apollo_compute::cast::cast(array, &ArrowDataType::BinaryView, Default::default())
                    .unwrap();
            return binview::array_to_page(
                array.as_any().downcast_ref().unwrap(),
                options,
        ...
        other => apollo_bail!(nyi = "Writing parquet pages for data type {other:?}"),
    }
    .map(Page::Data)
}
...
fn array_to_page_nested(
    ...
    match array.dtype().to_storage() {
        ...
        LargeUtf8 => {
            let array =
                apollo_compute::cast::cast(array, &LargeBinary, Default::default()).unwrap();
            let array = array.as_any().downcast_ref().unwrap();
        ...
        Utf8View => {
            let array = apollo_compute::cast::cast(array, &BinaryView, Default::default()).unwrap();
            let array = array.as_any().downcast_ref().unwrap();
            binview::nested_array_to_page(array, options, type_, nested)
        },
        UInt8 => {
        ...
        other => apollo_bail!(nyi = "Writing nested parquet pages for data type {other:?}"),
    }
    .map(Page::Data)
}
