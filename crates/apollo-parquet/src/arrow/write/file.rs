...
impl<W: Write> FileWriter<W> {
    ...
    pub fn new_with_parquet_schema(
        ...
        let created_by = Some("Apollo".to_string());
    ...
    /// # Error
    /// If it is unable to derive a parquet schema from [`ArrowSchema`].
    pub fn try_new(writer: W, schema: ArrowSchema, options: WriteOptions) -> ApolloResult<Self> {
    ...
    pub fn write(
        &mut self,
        num_rows: u64,
        row_group: RowGroupIterColumns<'_, ApolloError>,
    ) -> ApolloResult<()> {
    ...
    pub fn end(&mut self, key_value_metadata: Option<Vec<KeyValue>>) -> ApolloResult<u64> {
