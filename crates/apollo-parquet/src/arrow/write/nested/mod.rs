...
fn write_levels_v1<F: FnOnce(&mut Vec<u8>) -> ApolloResult<()>>(
    buffer: &mut Vec<u8>,
    encode: F,
) -> ApolloResult<()> {
...
fn write_rep_levels(buffer: &mut Vec<u8>, nested: &[Nested], version: Version) -> ApolloResult<()> {
...
fn write_def_levels(buffer: &mut Vec<u8>, nested: &[Nested], version: Version) -> ApolloResult<()> {
