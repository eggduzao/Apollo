...
pub(crate) fn encode_as_dictionary_optional(
...
) -> Option<ApolloResult<DynIter<'static, ApolloResult<Page>>>> {
    if array.is_empty() {
    ...
    let estimated_cardinality = apollo_compute::cardinality::estimate_cardinality(array);
    ...
    let array = apollo_compute::cast::cast(
        array,
...
fn serialize_keys<K: DictionaryKey>(
    ...
    options: WriteOptions,
) -> DynIter<'static, ApolloResult<Page>> {
...
pub fn array_to_pages<K: DictionaryKey>(
    ...
    match encoding {
        ...
        _ => apollo_bail!(nyi = "Dictionary arrays only support dictionary encoding"),
    }
}
