...
pub fn read_schema_from_metadata(metadata: &mut Metadata) -> ApolloResult<Option<ArrowSchema>> {
    metadata
...
fn convert_field(field: &mut Field) {
    // @NOTE: We cast non-Apollo dictionaries to normal values because Apollo does not have a
...
fn get_arrow_schema_from_metadata(encoded_meta: &str) -> ApolloResult<ArrowSchema> {
