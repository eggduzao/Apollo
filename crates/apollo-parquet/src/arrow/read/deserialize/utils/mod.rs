...
use apollo_compute::filter::filter_boolean_kernel;
...
pub(super) trait Decoder: Sized {
    ...
    fn unspecialized_predicate_decode(
        ...
        let filtered =
            apollo_compute::filter::filter_with_bitmap(intermediate_array.as_ref(), &mask);
...
pub struct PageDecoder<D: Decoder> {
    ...
    /// Used to track metrics with `APOLLO_PARQUET_METRICS=1`.
...
static APOLLO_PARQUET_METRICS: OnceLock<bool> = OnceLock::new();
...
impl<D: Decoder> PageDecoder<D> {
    pub fn new(
        ...
        let do_metrics = APOLLO_PARQUET_METRICS
            .get_or_init(|| std::env::var("APOLLO_PARQUET_METRICS").as_deref() == Ok("1"));
