...
use apollo_parquet_format::RowGroup;
use apollo_parquet_format::thrift::protocol::TCompactOutputProtocol;
...
fn create_column_orders(schema_desc: &SchemaDescriptor) -> Vec<apollo_parquet_format::ColumnOrder> {
    ...
    (0..schema_desc.columns().len())
        .map(|_| {
            apollo_parquet_format::ColumnOrder::TYPEORDER(
                apollo_parquet_format::TypeDefinedOrder {},
            )
        })
        .collect()
}
