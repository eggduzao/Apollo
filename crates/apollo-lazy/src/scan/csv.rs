...
impl LazyCsvReader {
    ...
    pub fn with_schema_modify<F>(mut self, f: F) -> ApolloResult<Self>
    where
        F: Fn(Schema) -> ApolloResult<Schema>,
    {
        ...
        let schema = match self.sources.clone() {
            ScanSources::Paths(paths) => {
                ...
                let Some(path) = paths.first() else {
                    apollo_bail!(ComputeError: "no paths specified for this reader");
                };
            ...
            ScanSources::Files(files) => {
                let Some(file) = files.first() else {
                    apollo_bail!(ComputeError: "no buffers specified for this reader");
                };
...
impl LazyFileListReader for LazyCsvReader {
    ...
    fn finish(self) -> ApolloResult<LazyFrame> {
    ...
    fn finish_no_glob(self) -> ApolloResult<LazyFrame> {
        unreachable!();
    }
    ...
    fn concat_impl(&self, lfs: Vec<LazyFrame>) -> ApolloResult<LazyFrame> {
