...
use apollo_io::parquet::read::ParallelStrategy;
use apollo_io::prelude::ParquetOptions;
...
impl LazyFrame {
    ...
    pub fn scan_parquet(path: PlRefPath, args: ScanArgsParquet) -> ApolloResult<Self> {
        Self::scan_parquet_sources(ScanSources::Paths(Buffer::from_iter([path])), args)
    }

    /// Create a LazyFrame directly from a parquet scan.
    pub fn scan_parquet_sources(sources: ScanSources, args: ScanArgsParquet) -> ApolloResult<Self> {
