...
use apollo_core::POOL;
use apollo_core::error::feature_gated;
use apollo_core::prelude::*;
use apollo_io::RowIndex;
use apollo_mem_engine::scan_predicate::functions::apply_scan_predicate_to_scan_ir;
use apollo_mem_engine::{Executor, create_multiple_physical_plans, create_physical_plan};
use apollo_ops::frame::{JoinBuildSide, JoinCoalesce, MaintainOrderJoin};
#[cfg(feature = "is_between")]
use apollo_ops::prelude::ClosedInterval;
pub use apollo_plan::frame::{AllowedOptimizations, OptFlags};
...
impl LazyFrame {
    ...
    pub fn describe_optimized_plan(&self) -> ApolloResult<String> {
    ...
    ///
    /// Returns `Err` if optimizing the logical plan fails.
    pub fn describe_optimized_plan_tree(&self) -> ApolloResult<String> {
    ...
    pub fn explain(&self, optimized: bool) -> ApolloResult<String> {
        if optimized {
    ...
    ///
    /// Any given columns that are not in the schema will give a [`ApolloError::ColumnNotFound`]
    ...
    /// with `Nones`.
    ///
    /// See the method on [Series](apollo_core::series::SeriesTrait::shift) for more info on the `shift` operation.
    ...
    pub fn to_alp_optimized(mut self) -> ApolloResult<IRPlan> {
    ...
    pub fn to_alp(mut self) -> ApolloResult<IRPlan> {
        let (mut lp_arena, mut expr_arena) = self.get_arenas();
        let node = to_alp(
            self.logical_plan,
            &mut expr_arena,
    ...
    fn prepare_collect_post_opt<P>(
        ...
        post_opt: P,
    ) -> ApolloResult<(ExecutionState, Box<dyn Executor>, bool)>
    where
        P: FnOnce(
            Node,
            &mut Arena<IR>,
    ...
    pub fn _collect_post_opt<P>(self, post_opt: P) -> ApolloResult<DataFrame>
    where
        P: FnOnce(
            Node,
            &mut Arena<IR>,
    ...
    pub fn collect_with_engine(mut self, mut engine: Engine) -> ApolloResult<DataFrame> {
        let payload = match &self.logical_plan {
            DslPlan::Sink { payload, .. } => payload.clone(),
            DslPlan::SinkMultiple { .. } => {
                apollo_ensure!(matches!(engine, Engine::Auto | Engine::Streaming), InvalidOperation: "lazy multisinks only supported on streaming engine");
                feature_gated!("new_streaming", {
                    let sink_multiple = self.with_new_streaming(true);
                    let mut alp_plan = sink_multiple.to_alp_optimized()?;
                    let result = apollo_stream::run_query(
                        alp_plan.lp_top,
        ...
        match engine {
            ...
            Engine::Gpu => {
                Err(apollo_err!(InvalidOperation: "sink is not supported for the gpu engine"))
            },
            Engine::InMemory => {
    ...
    pub fn explain_all(plans: Vec<DslPlan>, opt_state: OptFlags) -> ApolloResult<String> {
    ...
    /// }
    /// ```
    pub fn collect(self) -> ApolloResult<DataFrame> {
    ...
    pub fn collect_batches(
        ...
        let runner = move || {
            ...
            // thread pool limit.
            apollo_io::pl_async::get_runtime().spawn_blocking(move || {
    ...
    pub fn _profile_post_opt<P>(self, post_opt: P) -> ApolloResult<(DataFrame, DataFrame)>
    where
        P: FnOnce(
            Node,
            &mut Arena<IR>,
    ...
    pub fn profile(self) -> ApolloResult<(DataFrame, DataFrame)> {
    ...
    pub fn sink_batches(
        ...
        use apollo_plan::prelude::sink::CallbackSinkType;
    ...
    pub fn try_new_streaming_if_requested(
        &mut self,
    ) -> Option<ApolloResult<apollo_stream::QueryResult>> {
        let auto_new_streaming = std::env::var("APOLLO_AUTO_NEW_STREAMING").as_deref() == Ok("1");
        let force_new_streaming = std::env::var("APOLLO_FORCE_NEW_STREAMING").as_deref() == Ok("1");
    ...
    /// Apply explode operation. [See eager explode](apollo_core::frame::DataFrame::explode).
    ...
    where
        F: 'static + Fn(DataFrame) -> ApolloResult<DataFrame> + Send + Sync,
    {
        let opt_state = self.get_opt_state();
        let lp = self
            .get_plan_builder()
    ...
    pub fn map_python(
        self,
        function: apollo_utils::python_function::PythonFunction,
        optimizations: AllowedOptimizations,
    ...
    pub fn merge_sorted<S>(self, other: LazyFrame, key: S) -> ApolloResult<LazyFrame>
    where
        S: Into<PlSmallStr>,
    {
        let key = key.into();
    ...
    pub fn hint(self, hint: HintIR) -> ApolloResult<LazyFrame> {
...
pub const BUILD_STREAMING_EXECUTOR: Option<apollo_mem_engine::StreamingExecutorBuilder> = {
    ...
    #[cfg(feature = "new_streaming")]
    {
        Some(apollo_stream::build_streaming_query_executor)
    }
};

pub struct CollectBatches {
    recv: Receiver<ApolloResult<DataFrame>>,
...
impl Iterator for CollectBatches {
    type Item = ApolloResult<DataFrame>;
