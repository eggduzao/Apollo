...
impl LazyFrame {
    pub fn collect_concurrently(self) -> ApolloResult<InProcessQuery> {
...
pub struct InProcessQuery {
    rx: Arc<Mutex<Receiver<ApolloResult<DataFrame>>>>,
...
impl InProcessQuery {
    ...
    pub fn fetch(&self) -> Option<ApolloResult<DataFrame>> {
        let rx = self.rx.lock().unwrap();
        rx.try_recv().ok()
    }

    /// Await the result synchronously.
    pub fn fetch_blocking(&self) -> ApolloResult<DataFrame> {
        let rx = self.rx.lock().unwrap();
        rx.recv().unwrap()
    }
}
