...
#[test]
#[cfg(feature = "parquet")]
fn test_multiple_roots() -> ApolloResult<()> {
...
fn test_issue_2472() -> ApolloResult<()> {
    let df = df![
...
#[test]
fn test_pass_unrelated_apply() -> ApolloResult<()> {
...
#[test]
fn filter_added_column_issue_2470() -> ApolloResult<()> {
    let df = fruits_cars();
...
#[test]
fn filter_blocked_by_map() -> ApolloResult<()> {
    let df = fruits_cars();
...
fn test_strptime_block_predicate() -> ApolloResult<()> {
    let df = df![
...
#[test]
fn test_strict_cast_predicate_pushdown() -> ApolloResult<()> {
    let df = df![
        "a" => ["a", "b", "c"]
    ]?;
...
#[test]
fn test_filter_nulls_created_by_join() -> ApolloResult<()> {
    // #2602
    let a = df![
        "key" => ["foo", "bar"],
        "bar" => [1, 2]
...
#[test]
fn test_filter_null_creation_by_cast() -> ApolloResult<()> {
    let df = df![
        "int" => [1, 2, 3],
        "empty" => ["", "", ""]
    ]?;
...
#[test]
#[cfg(feature = "cse")]
fn test_predicate_on_join_suffix_4788() -> ApolloResult<()> {
    let lf = df![
      "x" => [1, 2],
      "y" => [1, 1],
    ]?
...
#[test]
fn test_push_join_col_predicates_to_both_sides_7247() -> ApolloResult<()> {
    let df1 = df! {
        "a" => ["a1", "a2"],
        "b" => ["b1", "b2"],
    }?;
...
fn test_push_join_col_predicates_to_both_sides_semi_12565() -> ApolloResult<()> {
    let df1 = df! {
        "a" => ["a1", "a2"],
        "b" => ["b1", "b2"],
    }?;
