...
#[test]
fn test_shift_and_fill() -> ApolloResult<()> {
    let out = df![
        "a" => [1, 2, 3]
    ]?
    .lazy()
...
#[test]
fn test_shift_and_fill_non_numeric() -> ApolloResult<()> {
    let out = df![
...
#[test]
fn test_lazy_query_4() -> ApolloResult<()> {
    let df = df! {
        "uid" => [0, 0, 0, 1, 1, 1],
        "day" => [1, 2, 3, 1, 2, 3],
...
#[test]
#[cfg(feature = "is_in")]
fn test_lazy_query_8() -> ApolloResult<()> {
    // https://github.com/apollo/apollo/issues/842
    let df = df![
        "A" => [1, 2, 3],
        "B" => [1, 2, 3],
        "C" => [1, 2, 3],
...
#[test]
fn test_lazy_query_9() -> ApolloResult<()> {
    // https://github.com/apollo/apollo/issues/958
    let cities = df![
...
#[test]
fn test_lazy_group_by_filter() -> ApolloResult<()> {
    let df = df! {
        "a" => ["a", "a", "a", "b", "b", "c"],
        "b" => [1, 2, 3, 4, 5, 6]
    }?;
...
#[test]
fn test_group_by_projection_pd_same_column() -> ApolloResult<()> {
...
#[test]
fn test_group_by_sort_slice() -> ApolloResult<()> {
    let df = df![
        "groups" => [1, 2, 2, 3, 3, 3],
        "vals" => [1, 5, 6, 3, 9, 8]
    ]?;
...
#[test]
#[cfg(feature = "cum_agg")]
fn test_group_by_cum_sum() -> ApolloResult<()> {
    let df = df![
        "groups" => [1, 2, 2, 3, 3, 3],
        "vals" => [1, 5, 6, 3, 9, 8]
    ]?;
...
#[test]
#[cfg(feature = "range")]
fn test_arg_sort_multiple() -> ApolloResult<()> {
    let df = df![
        "int" => [1, 2, 3, 1, 2],
...
#[test]
fn test_multiple_explode() -> ApolloResult<()> {
    let df = df![
        "a" => [0, 1, 2, 0, 2],
        "b" => [5, 4, 3, 2, 1],
        "c" => [2, 3, 4, 1, 5]
...
#[test]
fn test_filter_and_alias() -> ApolloResult<()> {
    let df = df![
        "a" => [0, 1, 2, 0, 2]
    ]?;
...
#[test]
fn test_filter_lit() {
    // see https://github.com/apollo/apollo/issues/790
...
#[test]
fn test_ternary_null() -> ApolloResult<()> {
    let df = df![
        "a" => ["a", "b", "c"]
    ]?;
...
#[test]
fn test_fill_forward() -> ApolloResult<()> {
    let df = df![
        "a" => ["a", "b", "a"],
        "b" => [Some(1), None, None]
    ]?;
...
#[test]
fn test_cross_join() -> ApolloResult<()> {
    let df1 = df![
        "a" => ["a", "b", "a"],
        "b" => [Some(1), None, None]
    ]?;
...
#[test]
fn test_select_empty_df() -> ApolloResult<()> {
    // https://github.com/apollo/apollo/issues/1056
    let df1 = df![
    "a" => [1, 2, 3],
    "b" => [1, 2, 3]
    ]?;
...
#[test]
fn test_keep_name() -> ApolloResult<()> {
    let df = df![
    "a" => [1, 2, 3],
    "b" => [1, 2, 3]
    ]?;
...
#[test]
#[cfg(feature = "regex")]
fn test_regex_selection() -> ApolloResult<()> {
    let df = df![
    "anton" => [1, 2, 3],
...
#[test]
fn test_filter_after_shift_in_groups() -> ApolloResult<()> {
    let df = fruits_cars();
...
#[test]
fn test_lazy_ternary_predicate_pushdown() -> ApolloResult<()> {
    let df = df![
        "a" => &[10, 1, 2, 3]
    ]?;
...
fn test_categorical_addition() -> ApolloResult<()> {
    let df = fruits_cars();
...
#[test]
fn test_filter_count() -> ApolloResult<()> {
    let df = fruits_cars();
    let out = df
        .lazy()
        .select([col("fruits")
...
fn test_group_by_small_ints() -> ApolloResult<()> {
    let df = df![
        "id_32" => [1i32, 2],
        "id_16" => [1i16, 2]
    ]?;

    // https://github.com/apollo/apollo/issues/1255
    let out = df
        .lazy()
...
#[test]
fn test_when_then_schema() -> ApolloResult<()> {
    let df = fruits_cars();
...
#[test]
fn test_singleton_broadcast() -> ApolloResult<()> {
    let df = fruits_cars();
    let out = df
        .lazy()
...
#[test]
fn test_list_in_select_context() -> ApolloResult<()> {
...
fn test_round_after_agg() -> ApolloResult<()> {
    let df = fruits_cars();
    ...
    let out = df
        ...
        .agg([col("A")
            ...
            .mean()
            .round(2, apollo_ops::series::RoundMode::default())
            .alias("foo")])
        .collect()?;
...
#[test]
#[cfg(feature = "regex")]
fn test_exclude_regex() -> ApolloResult<()> {
    let df = fruits_cars();
    let out = df
        .lazy()
...
#[test]
#[cfg(feature = "rank")]
fn test_group_by_rank() -> ApolloResult<()> {
    let df = fruits_cars();
    let out = df
        .lazy()
...
#[test]
pub fn test_select_by_dtypes() -> ApolloResult<()> {
    let df = df![
...
#[test]
fn test_binary_expr() -> ApolloResult<()> {
    // test panic in schema names
    let df = fruits_cars();
    let _ = df.lazy().select([col("A").neq(lit(1))]).collect()?;

    // test type coercion
    // https://github.com/apollo/apollo/issues/1649
    let df = df!(
...
#[test]
fn test_single_group_result() -> ApolloResult<()> {
...
#[test]
#[cfg(feature = "rank")]
fn test_single_ranked_group() -> ApolloResult<()> {
...
#[test]
#[cfg(feature = "abs")]
fn test_apply_flatten() -> ApolloResult<()> {
    let df = df![
...
#[test]
fn test_partitioned_gb_1() -> ApolloResult<()> {
...
#[test]
fn test_partitioned_gb_count() -> ApolloResult<()> {
    // don't move these to integration tests
    let out = df![
...
#[test]
fn test_partitioned_gb_mean() -> ApolloResult<()> {
    // don't move these to integration tests
    let out = df![
...
#[test]
fn test_partitioned_gb_binary() -> ApolloResult<()> {
    // don't move these to integration tests
    let df = df![
...
#[test]
fn test_partitioned_gb_ternary() -> ApolloResult<()> {
    // don't move these to integration tests
    let df = df![
        "col" => (0..20).map(|_| Some(0)).collect::<Int32Chunked>().into_series(),
...
#[test]
fn test_sort_maintain_order_true() -> ApolloResult<()> {
    let q = df![
        "A" => [1, 1, 1, 1],
        "B" => ["A", "B", "C", "D"],
    ]?
...
#[test]
fn test_over_with_options_empty_join() -> ApolloResult<()> {
...
#[test]
#[cfg(feature = "serde")]
fn test_named_udfs() -> ApolloResult<()> {
    use apollo_plan::dsl::named_serde::{ExprRegistry, set_named_serde_registry};
