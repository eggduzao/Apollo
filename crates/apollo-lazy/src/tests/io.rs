...
#[test]
#[cfg(feature = "parquet")]
fn test_parquet_exec() -> ApolloResult<()> {
...
#[test]
#[cfg(all(feature = "parquet", feature = "is_between"))]
fn test_parquet_statistics() -> ApolloResult<()> {
    let _guard = SINGLE_LOCK.lock().unwrap();
    init_files();
    unsafe { std::env::set_var("APOLLO_PANIC_IF_PARQUET_PARSED", "1") };
    let par = true;
    ...
    unsafe { std::env::remove_var("APOLLO_PANIC_IF_PARQUET_PARSED") };
...
fn test_parquet_globbing() -> ApolloResult<()> {
    // for side effects
    init_files();
    let _guard = SINGLE_LOCK.lock().unwrap();
...
#[test]
#[cfg(not(target_os = "windows"))]
fn test_ipc_globbing() -> ApolloResult<()> {
    // for side effects
    init_files();
...
#[test]
fn test_csv_globbing() -> ApolloResult<()> {
...
#[test]
#[cfg(feature = "json")]
fn test_ndjson_globbing() -> ApolloResult<()> {
    // for side effects
    init_files();
...
#[test]
pub fn test_simple_slice() -> ApolloResult<()> {
    let _guard = SINGLE_LOCK.lock().unwrap();
...
#[test]
fn test_union_and_agg_projections() -> ApolloResult<()> {
    init_files();
    let _guard = SINGLE_LOCK.lock().unwrap();
...
fn test_slice_filter() -> ApolloResult<()> {
    init_files();
    let _guard = SINGLE_LOCK.lock().unwrap();
...
#[test]
fn skip_rows_and_slice() -> ApolloResult<()> {
...
#[test]
fn test_row_index_on_files() -> ApolloResult<()> {
    let _guard = SINGLE_LOCK.lock().unwrap();
...
#[test]
fn scan_predicate_on_set_null_values() -> ApolloResult<()> {
...
#[test]
fn scan_anonymous_fn_with_options() -> ApolloResult<()> {
    struct MyScan {}
    ...
    impl AnonymousScan for MyScan {
        ...
        fn scan(&self, scan_opts: AnonymousScanArgs) -> ApolloResult<DataFrame> {
...
#[test]
fn scan_anonymous_fn_count() -> ApolloResult<()> {
    struct MyScan {}
...
fn scan_small_dtypes() -> ApolloResult<()> {
    let small_dt = vec![
        DataType::Int8,
        DataType::UInt8,
        DataType::Int16,
