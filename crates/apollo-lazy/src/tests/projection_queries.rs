...
#[test]
fn test_join_suffix_and_drop() -> ApolloResult<()> {
    let weight = df![
        "id" => [1, 2, 3, 4, 5, 0],
...
fn test_cross_join_pd() -> ApolloResult<()> {
    let food = df![
...
#[test]
fn test_row_number_pd() -> ApolloResult<()> {
    let df = df![
        "x" => [1, 2, 3],
        "y" => [3, 2, 1],
    ]?;
...
#[test]
#[cfg(feature = "cse")]
fn scan_join_same_file() -> ApolloResult<()> {
...
fn concat_str_regex_expansion() -> ApolloResult<()> {
    let df = df![
        "a"=> [1, 1, 1],
        "b_a_1"=> ["a--", "", ""],
        "b_a_2"=> ["", "b--", ""],
...
#[test]
fn test_coalesce_toggle_projection_pushdown() -> ApolloResult<()> {
...
#[test]
fn test_select_hconcat_pushdown_non_strict_25263() -> ApolloResult<()> {
    let df_a = df![
        "a" => [1, 2, 2],
        "b" => [4, 5, 6],
    ]?
...
#[test]
fn test_select_hconcat_pushdown_strict_25263() -> ApolloResult<()> {
    let df_a = df![
        "a" => [1, 2],
        "b" => [4, 5],
    ]?
