...
#[test]
fn test_pred_pd_1() -> ApolloResult<()> {
    let df = fruits_cars();
...
#[test]
fn test_no_left_join_pass() -> ApolloResult<()> {
    let df1 = df![
        "foo" => ["abc", "def", "ghi"],
        "idx1" => [0, 0, 1],
    ]?;
...
#[cfg(feature = "parquet")]
#[cfg(feature = "cse")]
pub fn test_slice_pushdown_join() -> ApolloResult<()> {
    let _guard = SINGLE_LOCK.lock().unwrap();
...
#[test]
#[cfg(feature = "parquet")]
pub fn test_slice_pushdown_group_by() -> ApolloResult<()> {
    let _guard = SINGLE_LOCK.lock().unwrap();
...
#[test]
#[cfg(feature = "parquet")]
pub fn test_slice_pushdown_sort() -> ApolloResult<()> {
    let _guard = SINGLE_LOCK.lock().unwrap();
    let q = scan_foods_parquet(false).limit(100);
...
pub fn test_predicate_block_cast() -> ApolloResult<()> {
    let df = df![
        "value" => [10, 20, 30, 40]
    ]?;
...
#[test]
fn test_with_row_index_opts() -> ApolloResult<()> {
    let df = df![
...
#[test]
fn test_string_addition_to_concat_str() -> ApolloResult<()> {
    let df = df![
        "a"=> ["a"],
        "b"=> ["b"],
    ]?;
...
#[test]
fn test_with_column_prune() -> ApolloResult<()> {
    // don't
    let df = df![
        "c0" => [0],
        "c1" => [0],
...
#[test]
#[cfg(feature = "csv")]
fn test_slice_at_scan_group_by() -> ApolloResult<()> {
...
#[test]
fn test_flatten_unions() -> ApolloResult<()> {
    let (mut expr_arena, mut lp_arena) = get_arenas();
