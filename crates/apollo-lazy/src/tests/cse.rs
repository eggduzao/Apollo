...
#[test]
fn test_cse_self_joins() -> ApolloResult<()> {
...
#[test]
fn test_cse_unions() -> ApolloResult<()> {
    let lf = scan_foods_ipc();
...
#[test]
fn test_cse_cache_union_projection_pd() -> ApolloResult<()> {
    let q = df![
        "a" => [1],
        "b" => [2],
        "c" => [3],
...
#[test]
fn test_cse_union2_4925() -> ApolloResult<()> {
    let lf1 = df![
        "ts" => [1],
        "sym" => ["a"],
        "c" => [true],
...
#[test]
fn test_cse_joins_4954() -> ApolloResult<()> {
    let x = df![
        "a"=> [1],
        "b"=> [1],
        "c"=> [1],
...
fn test_cache_with_partial_projection() -> ApolloResult<()> {
    let lf1 = df![
        "id" => ["a"],
        "x" => [1],
        "freq" => [2]
...
fn test_cse_columns_projections() -> ApolloResult<()> {
    let right = df![
        "A" => [1, 2],
        "B" => [3, 4],
        "D" => [5, 6]
...
#[test]
fn test_cse_prune_scan_filter_difference() -> ApolloResult<()> {
    let lf = scan_foods_ipc();
    let lf = lf.with_column(col("category").str().to_uppercase());
