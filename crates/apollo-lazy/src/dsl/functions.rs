...
pub use apollo_plan::dsl::functions::*;
...
pub(crate) fn concat_impl<L: AsRef<[LazyFrame]>>(
    ...
    let lf = std::mem::take(
        inputs
            .get_mut(0)
            .ok_or_else(|| apollo_err!(NoData: "empty container given"))?,
    );
...
pub fn concat_lf_horizontal<L: AsRef<[LazyFrame]>>(
    ...
    let (opt_state, cached_arena) = lfs
        ...
        .ok_or_else(
            || apollo_err!(NoData: "Require at least one LazyFrame for horizontal concatenation"),
        )?;
...
pub fn concat<L: AsRef<[LazyFrame]>>(inputs: L, args: UnionArgs) -> ApolloResult<LazyFrame> {
    concat_impl(inputs, args)
}
...
mod test {
    ...
    #[test]
    #[cfg(feature = "diagonal_concat")]
    fn test_diag_concat_lf() -> ApolloResult<()> {
        let a = df![
            "a" => [1, 2],
            "b" => ["a", "b"]
        ]?;
