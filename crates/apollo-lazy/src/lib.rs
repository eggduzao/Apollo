//! Lazy API of Apollo
//!
//! The lazy API of Apollo supports a subset of the eager API. Apart from the distributed compute,
...
//! # Lazy DSL
//!
//! The lazy API of apollo replaces the eager [`DataFrame`] with the [`LazyFrame`], through which
...
//! This allows apollo to optimize/reorder the query which may lead to faster queries or fewer type errors.
//!
//! [`DataFrame`]: apollo_core::frame::DataFrame
//! [`LazyFrame`]: crate::frame::LazyFrame
//! [`collect`]: crate::frame::LazyFrame::collect
//!
//! In general, a [`LazyFrame`] requires a concrete data source — a [`DataFrame`], a file on disk, etc. — which apollo-lazy
...
//!
//! The other major component of the apollo lazy API is [`Expr`](crate::dsl::Expr), which represents an operation to be
