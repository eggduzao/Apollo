...
RUNTIME_CARGO_TOML=py-apollo/runtime/apollo-runtime-32/Cargo.toml
...
requirements:  ## Install/refresh Python project requirements
	...
	&& $(VENV_BIN)/uv pip install --no-deps -e py-apollo \
	&& $(VENV_BIN)/uv pip uninstall apollo-runtime-compat apollo-runtime-64  ## Uninstall runtimes which might take precedence over apollo-runtime-32

.PHONY: requirements-all
requirements-all:  ## Install/refresh all Python requirements (including those needed for CI tests)
	$(VENV_BIN)/uv pip install --upgrade --compile-bytecode -r py-apollo/requirements-ci.txt
	$(MAKE) requirements

.PHONY: build
build: .venv  ## Compile and install Python Apollo for development
	@unset CONDA_PREFIX \
...
.PHONY: build-release
build-release: .venv  ## Compile and install Python Apollo binary with optimizations, with minimal debug symbols
	@unset CONDA_PREFIX \
...
build-dist-release: .venv  ## Compile and install Python Apollo binary with super slow extra optimization turned on, for distribution
	@unset CONDA_PREFIX \
