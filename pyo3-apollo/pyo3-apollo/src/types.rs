...
use apollo_ffi::version_0::{export_series, import_series, SeriesExport};
#[cfg(feature = "lazy")]
use apollo_lazy::frame::LazyFrame;
#[cfg(feature = "lazy")]
use apollo_plan::dsl::DslPlan;
#[cfg(feature = "lazy")]
...
impl<'a, 'py> FromPyObject<'a, 'py> for PySeries {
    ...
    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        ...
        let series = unsafe { import_series(export).map_err(PyApolloErr::from)? };
        Ok(PySeries(series))
    }
}
...
impl<'a, 'py> FromPyObject<'a, 'py> for PyLazyFrame {
    ...
    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        ...
        let lp = DslPlan::deserialize_versioned(b).map_err(
            |e| PyApolloErr::Other(
                format!("Error when deserializing LazyFrame. This may be due to mismatched apollo versions. {e}")
            ))
            ?;
...
impl<'a, 'py> FromPyObject<'a, 'py> for PyExpr {
    ...
    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        ...
        let e: Expr = pl_serialize::SerializeOptions::default()
            ...
            .map_err(
            |e| PyApolloErr::Other(
                format!("Error when deserializing 'Expr'. This may be due to mismatched apollo versions. {e}")
            )
        )?;
...
impl<'py> IntoPyObject<'py> for PyDataFrame {
    ...
    fn into_pyobject(self, py: Python<'py>) -> Result<Self::Output, Self::Error> {
        ...
        let apollo = APOLLO.bind(py);
        apollo.call_method1("DataFrame", (pyseries,))
    }
}
...
impl<'py> IntoPyObject<'py> for PyLazyFrame {
    ...
    fn into_pyobject(self, py: Python<'py>) -> Result<Self::Output, Self::Error> {
        use apollo::prelude::PlanSerializationContext;

        let apollo = APOLLO.bind(py);
        let cls = apollo.getattr("LazyFrame")?;
        ...
        let dtype = match type_name.as_ref() {
            "DataTypeClass" => {
                ...
                match &*name {
                    ...
                    #[cfg(feature = "dtype-decimal")]
                    "Decimal" => {
                        return Err(PyTypeError::new_err("Decimal without specifying precision and scale is not a valid Apollo data type".to_string()));
                    },
                    ...
                    dt => {
                        return Err(PyTypeError::new_err(format!(
                            "'{dt}' is not a Apollo data type, or the plugin isn't compiled with the right features",
                        )));
                    },
                }
            },
