...
static ALLOCATOR_CAPSULE_NAME: &[u8] = b"apollo.apollo._allocator\0";

/// A memory allocator that relays allocations to the allocator used by Apollo.
///
...
/// ```
///
/// If the allocator capsule (`apollo.apollo._allocator`) is not available,
/// this allocator fallbacks to [`std::alloc::System`].
pub struct ApolloAllocator(OnceRef<'static, AllocatorCapsule>);

impl ApolloAllocator {
    ...
    /// Create a `ApolloAllocator`.
    pub const fn new() -> Self {
        ApolloAllocator(OnceRef::new())
    }
}

impl Default for ApolloAllocator {
    fn default() -> Self {
        Self::new()
    }
}

unsafe impl GlobalAlloc for ApolloAllocator {
    #[inline]
