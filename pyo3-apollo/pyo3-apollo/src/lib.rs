...
//!     };
//!
//!     // wrap the dataframe and it will be automatically converted to a python apollo dataframe
//!     Ok(PyDataFrame(df))
//! }
//!
...
pub use crate::alloc::ApolloAllocator;

pub(crate) static APOLLO: Lazy<Py<PyModule>> =
    Lazy::new(|| Python::attach(|py| PyModule::import(py, "apollo").unwrap().unbind()));

pub(crate) static SERIES: Lazy<Py<PyAny>> =
    Lazy::new(|| Python::attach(|py| APOLLO.getattr(py, "Series").unwrap()));
