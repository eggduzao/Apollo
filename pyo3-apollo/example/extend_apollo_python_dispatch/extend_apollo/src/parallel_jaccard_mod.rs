...
/// Create `n` splits so that we can slice a apollo data structure
...
fn compute_jaccard_similarity(sa: &Series, sb: &Series) -> ApolloResult<Series> {
    let sa = sa.list()?;
    let sb = sb.list()?;
    ...
    let ca = sa
        ...
        .collect::<ApolloResult<Float64Chunked>>()?;
    Ok(ca.into_series())
}

pub(super) fn parallel_jaccard(df: DataFrame, col_a: &str, col_b: &str) -> ApolloResult<DataFrame> {
