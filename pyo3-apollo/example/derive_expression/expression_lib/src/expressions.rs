...
use apollo::prelude::*;
use apollo_plan::prelude::FieldsMapper;
use pyo3_apollo::derive::{apollo_expr, CallerContext};
use pyo3_apollo::export::apollo_core::POOL;
...
#[apollo_expr(output_type=Float64)]
fn jaccard_similarity(inputs: &[Series]) -> ApolloResult<Series> {
    let a = inputs[0].list()?;
    let b = inputs[1].list()?;
...
#[apollo_expr(output_type=Float64)]
fn hamming_distance(inputs: &[Series]) -> ApolloResult<Series> {
    let a = inputs[0].str()?;
    let b = inputs[1].str()?;
...
#[apollo_expr(output_type=Boolean)]
fn is_leap_year(input: &[Series]) -> ApolloResult<Series> {
    let input = &input[0];
    let ca = input.date()?;
...
#[apollo_expr(output_type=Boolean)]
fn panic(_input: &[Series]) -> ApolloResult<Series> {
    todo!()
}
...
fn convert_timezone(input_fields: &[Field], kwargs: TimeZone) -> ApolloResult<Field> {
    FieldsMapper::new(input_fields).try_map_dtype(|dtype| match dtype {
        ...
        _ => apollo_bail!(ComputeError: "expected datetime"),
    })
}

/// This expression is for demonstration purposes as we have a dedicated
/// `convert_time_zone` in Apollo.
#[apollo_expr(output_type_func_with_kwargs=convert_timezone)]
fn change_time_zone(input: &[Series], kwargs: TimeZone) -> ApolloResult<Series> {
    let input = &input[0];
    ...
    let Some(timezone) = datatypes::TimeZone::opt_try_new(Some(kwargs.tz))? else {
        apollo_bail!(ComputeError: "expected timezone")
    };
