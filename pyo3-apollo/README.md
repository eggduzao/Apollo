## 1. Shared library plugins for Apollo

<a href="https://crates.io/crates/pyo3-apollo">
    <img src="https://img.shields.io/crates/v/pyo3-apollo.svg"/>
  </a>

Documentation for this functionality may also be found in the
[Apollo User Guide](https://docs.apollo.org/user-guide/plugins/). This is new functionality and should
be preferred over `2.` as this will circumvent the GIL and will be the way we want to support
extending apollo.

Parallelism and optimizations are managed by the default apollo runtime. That runtime will call into
the plugin function. The plugin functions are compiled separately.

We can therefore keep apollo more lean and maybe add support for a `apollo-distance`, `apollo-geo`,
`apollo-ml`, etc. Those can then have specialized expressions and don't have to worry as much for
code bloat as they can be optionally installed.

The idea is that you define an expression in another Rust crate with a proc_macro `apollo_expr`.
...
fn pig_latinnify(inputs: &[Series], kwargs: PigLatinKwargs) -> ApolloResult<Series> {
    let ca = inputs[0].str()?;
...
[register a custom namespace](https://docs.apollo.org/py-apollo/html/reference/api/apollo.api.register_expr_namespace.html#apollo.api.register_expr_namespace),
...
https://github.com/apollo/pyo3-apollo/tree/main/example/derive_expression

## 2. PyO3 extensions for Apollo

See the `example` directory for a concrete example. Here we send a apollo `DataFrame` to Rust and
...
beyond that the latest definitions should work for the latest version of Apollo.
