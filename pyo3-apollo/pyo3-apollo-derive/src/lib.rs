...
fn quote_get_kwargs() -> proc_macro2::TokenStream {
    ...
    let kwargs = match pyo3_apollo::derive::_parse_kwargs(kwargs)  {
        Ok(value) => value,
        Err(err) => {
            let err = apollo_error::apollo_err!(InvalidOperation: "could not parse kwargs: '{}'\n\nCheck: registration of kwargs in the plugin.", err);
            pyo3_apollo::derive::_update_last_error(err);
            return;
        }
    };
...
fn quote_call_kwargs(ast: &syn::ItemFn, fn_name: &syn::Ident) -> proc_macro2::TokenStream {
    ...
    quote!(
            ...
            // call the function
        let result: apollo_error::ApolloResult<apollo_core::prelude::Series> = #fn_name(&inputs, kwargs);
...
fn quote_call_context(ast: &syn::ItemFn, fn_name: &syn::Ident) -> proc_macro2::TokenStream {
    quote!(
            ...
            // call the function
        let result: apollo_error::ApolloResult<apollo_core::prelude::Series> = #fn_name(&inputs, context);
    )
}
...
fn quote_call_context_kwargs(ast: &syn::ItemFn, fn_name: &syn::Ident) -> proc_macro2::TokenStream {
    quote!(
            ...
            // call the function
        let result: apollo_error::ApolloResult<apollo_core::prelude::Series> = #fn_name(&inputs, context, kwargs);
    )
}
...
fn quote_call_no_kwargs(ast: &syn::ItemFn, fn_name: &syn::Ident) -> proc_macro2::TokenStream {
    quote!(
            ...
            #ast
            // call the function
            let result: apollo_error::ApolloResult<apollo_core::prelude::Series> = #fn_name(&inputs);
    )
}
...
fn quote_process_results() -> proc_macro2::TokenStream {
    quote!(match result {
        Ok(out) => {
            // Update return value.
            *return_value = apollo_ffi::version_0::export_series(&out);
        },
        Err(err) => {
...
fn create_expression_function(ast: syn::ItemFn) -> proc_macro2::TokenStream {
    ...
    quote!(
        use ::pyo3_apollo::export::*;

        // create the outer public function
        #[no_mangle]
        pub unsafe extern "C" fn #fn_name (
            e: *mut apollo_ffi::version_0::SeriesExport,
            input_len: usize,
            kwargs_ptr: *const u8,
            kwargs_len: usize,
            return_value: *mut apollo_ffi::version_0::SeriesExport,
            context: *mut apollo_ffi::version_0::CallerContext
        )  {
            let panic_result = std::panic::catch_unwind(move || {
                let inputs = apollo_ffi::version_0::import_series_buffer(e, input_len).unwrap();
...
fn get_field_function_name(fn_name: &syn::Ident) -> syn::Ident {
    syn::Ident::new(&format!("_apollo_plugin_field_{fn_name}"), fn_name.span())
}

fn get_expression_function_name(fn_name: &syn::Ident) -> syn::Ident {
    syn::Ident::new(&format!("_apollo_plugin_{fn_name}"), fn_name.span())
}
...
fn quote_get_inputs() -> proc_macro2::TokenStream {
    quote!(
             ...
             let inputs = inputs.iter().map(|field| {
                 let field = apollo_arrow::ffi::import_field_from_c(field).unwrap();
                 let out = apollo_core::prelude::Field::from(&field);
                 out
             }).collect::<Vec<_>>();
    )
}
...
fn create_field_function(
    ...
    quote! (
        ...
        pub unsafe extern "C" fn #map_field_name(
            field: *mut apollo_arrow::ffi::ArrowSchema,
            len: usize,
            return_value: *mut apollo_arrow::ffi::ArrowSchema,
            kwargs_ptr: *const u8,
            kwargs_len: usize,
        ) {
            let panic_result = std::panic::catch_unwind(move || {
                ...
                match result {
                    Ok(out) => {
                        let out = apollo_arrow::ffi::export_field_to_c(&out.to_arrow(apollo_core::datatypes::CompatLevel::newest()));
                        *return_value = out;
                    },
                    Err(err) => {
                        // Set latest error, but leave return value in empty state.
...
fn create_field_function_from_with_dtype(
    ...
    quote! (
        ...
        pub unsafe extern "C" fn #map_field_name(
            ...
            let mapper = apollo_plan::prelude::FieldsMapper::new(&inputs);
            let dtype = apollo_core::datatypes::DataType::#dtype;
...
pub fn apollo_expr(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded_field_fn = if let Some(fn_name) = options.output_type_fn {
        ...
        panic!("didn't understand apollo_expr attribute")
    };
