...
from apollo._cpu_check import get_runtime_repr
...
def show_versions() -> None:
    """
    Print out the version of Apollo and its optional dependencies.
    ...
    # module) as a micro-optimization for apollo' initial import
    import platform

    deps = _get_dependency_list()
    core_properties = ("Apollo", "Index type", "Platform", "Python", "Runtime")
    keylen = max(len(x) for x in [*core_properties, "Azure CLI", *deps]) + 1

    print("--------Version info---------")
    print(f"{'Apollo:':{keylen}s} {get_apollo_version()}")
    ...
    from apollo.io.cloud.credential_provider import CredentialProviderAzure
...
def _get_dependency_version(dep_name: str) -> str:
    ...
    # optimisation for initial apollo module import
    import importlib
