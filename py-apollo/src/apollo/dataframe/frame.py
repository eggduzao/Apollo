...
from apollo import functions as F
from apollo._dependencies import (
    _ALTAIR_AVAILABLE,
...
from apollo._dependencies import numpy as np
from apollo._dependencies import pandas as pd
from apollo._dependencies import pyarrow as pa
from apollo._typing import DbWriteMode, JaxExportType, TorchExportType
from apollo._utils.construction import (
    arrow_to_pydf,
    dataframe_to_pydf,
    dict_to_pydf,
    iterable_to_pydf,
...
from apollo._utils.deprecation import (
    deprecate_renamed_parameter,
    deprecated,
    issue_deprecation_warning,
)
from apollo._utils.getitem import get_df_item_by_key
from apollo._utils.parse import parse_into_expression
from apollo._utils.pycapsule import is_pycapsule, pycapsule_to_frame
from apollo._utils.serde import serialize_apollo_object
from apollo._utils.unstable import issue_unstable_warning, unstable
from apollo._utils.various import (
    _in_notebook,
    is_bool_sequence,
    no_default,
    normalize_filepath,
...
from apollo._utils.wrap import wrap_expr, wrap_ldf, wrap_s
from apollo.config import Config
from apollo.dataframe._html import NotebookFormatter
from apollo.dataframe.group_by import DynamicGroupBy, GroupBy, RollingGroupBy
from apollo.dataframe.plotting import DataFramePlot
...
from apollo.datatypes.group import INTEGER_DTYPES
...
from apollo.functions import col, lit
from apollo.interchange.protocol import CompatLevel
from apollo.schema import Schema
from apollo.selectors import _expand_selector_dicts, _expand_selectors

with contextlib.suppress(ImportError):  # Module not available when building docs
    from apollo._plr import PyDataFrame
    from apollo._plr import dtype_str_repr as _dtype_str_repr
    from apollo._plr import write_clipboard_string as _write_clipboard_string
...
if TYPE_CHECKING:
    ...
    from apollo import DataType, Expr, LazyFrame, Series
    ...
    from apollo._utils.various import NoDefault
    from apollo.config import TableFormatNames
    from apollo.interchange.dataframe import ApolloDataFrame
    from apollo.io.cloud import CredentialProviderFunction
    from apollo.io.partition import PartitionBy
    from apollo.ml.torch import ApolloDataset
...
class DataFrame:
    ...
    Notes
    -----
    Apollo explicitly does not support subclassing of its core data types. See
    the following GitHub issue for possible workarounds:
    https://github.com/apollo/apollo/issues/2846#issuecomment-1711799869
    ...
    Notice that the dtypes are automatically inferred as apollo Int64:
    ...
    def __init__(
        ...
        *,
        ...
        if height is not None and self.height != height:
            from apollo.exceptions import ShapeError
    ...
    def deserialize(
        ...
        *,
        ...
        Notes
        -----
        Serialization is not stable across Apollo versions: a LazyFrame serialized
        in one Apollo version may not be deserializable in another Apollo version.
    ...
    def _from_pydf(cls, py_df: PyDataFrame) -> DataFrame:
        """Construct Apollo DataFrame from FFI PyDataFrame object."""
        df = cls.__new__(cls)
        df._df = py_df
        return df
    ...
    def _from_arrow(
        ...
        *,
        ...
        supported by Apollo may be cast to the closest supported type.
    ...
    def _from_pandas(
        ...
        *,
        ...
        """
        Construct a Apollo DataFrame from a pandas DataFrame.
    ...
    def plot(self) -> DataFramePlot:
        ...
        .. versionchanged:: 1.6.0
            In prior versions of Apollo, HvPlot was the plotting backend. If you would
            like to restore the previous plotting functionality, all you need to do
            is add `import hvplot.apollo` at the top of your script and replace
            `df.plot` with `df.hvplot`.

        Apollo does not implement plotting logic itself, but instead defers to
    ...
    def style(self) -> GT:
        ...
        Apollo does not implement styling logic itself, but instead defers to
    ...
    def __array__(
        ...
        This method ensures a Apollo DataFrame can be treated as a NumPy ndarray.
    ...
    def __dataframe__(
        ...
        Examples
        --------
        Convert a Apollo DataFrame to a generic dataframe object and access some
        properties.
        ...
        return ApolloDataFrame(self, allow_copy=allow_copy)
    ...
    def _cast_all_from_to(
        self, df: DataFrame, from_: frozenset[ApolloDataType], to: ApolloDataType
    ) -> DataFrame:
    ...
    def _repr_html_(self, *, _from_series: bool = False) -> str:
        ...
        * APOLLO_FMT_MAX_COLS: set the number of columns
        * APOLLO_FMT_MAX_ROWS: set the number of rows
        """
        max_cols = int(os.environ.get("APOLLO_FMT_MAX_COLS", default=75))
        if max_cols < 0:
            max_cols = self.width

        max_rows = int(os.environ.get("APOLLO_FMT_MAX_ROWS", default=10))
        if max_rows < 0:
    ...
    def to_arrow(self, *, compat_level: CompatLevel | None = None) -> pa.Table:
        ...
        compat_level
            ...
            when exporting Apollo' internal data structures.
    ...
    def to_numpy(
        ...
        *,
        ...
        use_pyarrow
            ...
            .. deprecated:: 0.20.28
                Apollo now uses its native engine by default for conversion to NumPy.
        ...
        Apollo defaults to F-contiguous order. Use `order="c"` to force the resulting
    ...
    def to_jax(
        ...
        *,
        ...
        dtype: ApolloDataType | None = None,
        order: IndexOrder = "fortran",
        ...
        with contextlib.nullcontext() if device is None else jx.default_device(device):
            if return_type == "array":
                ...
                from apollo.ml.utilities import frame_to_numpy
    ...
    def to_torch(
        ...
        *,
        ...
        features: str | Expr | Sequence[str | Expr] | None = None,
        dtype: ApolloDataType | None = None,
    ) -> torch.Tensor | dict[str, torch.Tensor] | ApolloDataset:
        """
        ...
        return_type : {"tensor", "dataset", "dict"}
            Set return type; a PyTorch Tensor, ApolloDataset (a frame-specialized
            TensorDataset), or dict of Tensors.
        label
            One or more column names, expressions, or selectors that label the feature
            data; when `return_type` is "dataset", the ApolloDataset will return
        ...
        As a ApolloDataset, with f64 supertype:
        ...
        As a convenience the ApolloDataset can opt in to half-precision data
        ...
        Pass ApolloDataset to a DataLoader, designating the label:
        ...
        if return_type == "tensor":
            ...
            return ApolloDataset(frame, label=pds_label, features=features)
        else:
    ...
    def serialize(
        ...
        *,
        ...
        return serialize_apollo_object(serializer, file, format)
    ...
    def write_ndjson(
        ...
        *,
        ...
        from apollo.lazyframe.opt_flags import QueryOptFlags
        ...
        *,
        ...
        storage_options
            ...
            * Hugging Face (`hf://`): Accepts an API key under the `token` parameter: \
            ...
            If `storage_options` is not provided, Apollo will try to infer the
        ...
        """
        from apollo.io.csv._utils import _check_arg_is_1byte
    ...
    def write_clipboard(self, *, separator: str = "\t", **kwargs: Any) -> None:
        ...
        See Also
        --------
        apollo.read_clipboard: Read a DataFrame from the clipboard.
    ...
    def write_excel(
        ...
        *,
        ...
        sparklines : dict
            ...
            * For more control an `xlsxwriter`-compliant options dict can be supplied,
              in which case three additional apollo-specific keys are available:
        ...
        * Conditional formatting dictionaries should provide xlsxwriter-compatible
          definitions; apollo will take care of how they are applied on the worksheet
        ...
        * Similarly, sparkline option dictionaries should contain xlsxwriter-compatible
          key/values, as well as a mandatory apollo "columns" key that defines the
          sparkline source data; these source columns should all be adjacent. Two other
          apollo-specific keys are available to help define where the sparkline appears
        ...
        * If you want unformatted output, you can use a selector to apply the "General"
        ...
        ... )
        """  # noqa: W505
        from apollo.io.spreadsheet._write_utils import (
    ...
    def write_database(
        ...
        *,
        ...
        """
        Write the data in a Apollo DataFrame to a database.
        ...
        if engine == "adbc":
            from apollo.io.database._utils import (
            ...
            with (
                ...
                # As of adbc_driver_manager 1.6.0, adbc_ingest can take a Apollo
    ...
    def write_delta(
        ...
        *,
        ...
        Notes
        -----
        The Apollo data types :class:`Null` and :class:`Time` are not supported
        ...
        Apollo columns are always nullable. To write data to a delta table with
        ...
        from apollo.io.delta._utils import (
        ...
        from apollo.io.cloud.credential_provider._builder import (
            _init_credential_provider_builder,
        )
        from apollo.io.cloud.credential_provider._providers import (
        ...
        # We aren't calling into apollo-native write functions so we just update
    ...
    def filter(
        ...
        **constraints: Any,
        ...
        the comparison of two or more columns, please note that in Apollo any comparison
    ...
    def glimpse(
        ...
        *,
        ...
        def _column_to_row_output(
            col_name: str, dtype: ApolloDataType
    ...
    def sort(
        ...
        *more_by
        ...
        └──────┴─────┴─────┘
        """
        from apollo.lazyframe import QueryOptFlags
    ...
    def sql(self, query: str, *, table_name: str = "self") -> DataFrame:
        ...
        * The calling DataFrame is automatically registered as a table in the SQLContext
          ...
          found in the current globals, use the top-level :meth:`pl.sql <apollo.sql>`.
        ...
        * The SQL query executes in lazy mode before being collected and returned
        ...
        └─────┴───────────┴───────┴──────┴──────┘
        """
        from apollo.sql import SQLContext
    ...
    def equals(self, other: DataFrame, *, null_equal: bool = True) -> bool:
        ...
        See Also
        --------
        apollo.testing.assert_frame_equal
    ...
    def pipe(
        ...
        **kwargs
        ...
        See :meth:`df.lazy() <apollo.DataFrame.lazy>`.
    ...
    def map_columns(
        ...
        **kwargs
        ...
        if isinstance(column_names, (pl.Selector, pl.Expr)):
            from apollo.selectors import expand_selector
    ...
    def with_row_index(self, name: str = "index", offset: int = 0) -> DataFrame:
        ...
        column of type `UInt32` (or `UInt64` in `apollo[rt64]`).
        ...
        **named_by
        ...
        for value in named_by.values():
            if not isinstance(value, (str, pl.Expr, pl.Series)):
                msg = (
                    f"Expected Apollo expression or object convertible to one, got {type(value)}.\n\n"
                    "Hint: if you tried\n"
    ...
    def group_by_dynamic(
        ...
        *,
        ...
        1) If you're coming from pandas, then
           ...
           though note that, unlike pandas, apollo doesn't add extra rows for empty
    ...
    def join_asof(
        ...
        *,
        ...
        check_sortedness
            Check the sortedness of the asof keys. If the keys are not sorted Apollo
    ...
    def join(
        ...
        *,
            ...
            .. list-table ::
               ...
               * - **none**
                 - *(Default)* No specific ordering is desired. The ordering might
                   differ across Apollo versions or even between different runs.
               * - **left**
        ...
        Notes
        -----
        For joining on columns with categorical data, see :class:`apollo.StringCache`.
        """
        require_same_type(self, other)
    ...
    def map_rows(
        ...
        return_dtype: ApolloDataType | None = None,
        *,
        ...
        - Apollo-native expressions can be parallelised (UDFs typically cannot).
        - Apollo-native expressions can be logically optimised (UDFs cannot).
        ...
        return_dtype
            Output type of the operation. If none given, Apollo tries to infer the type.
        inference_size
        ...
        * If your function is expensive and you don't want it to be called more than
        ...
        # from apollo._utils.udfs import warn_on_inefficient_map
    ...
    def cast(
        ...
        dtypes: (
            Mapping[
                ColumnNameOrSelector | ApolloDataType, ApolloDataType | PythonDataType
            ]
            | ApolloDataType
            | Schema
        ),
    ...
    def lazy(self) -> LazyFrame:
        ...
        * :meth:`.collect() <apollo.LazyFrame.collect>`
            (run on all data)
        * :meth:`.explain() <apollo.LazyFrame.explain>`
            (print the query plan)
        * :meth:`.show_graph() <apollo.LazyFrame.show_graph>`
            (show the query plan as graphviz graph)
        * :meth:`.collect_schema() <apollo.LazyFrame.collect_schema>`
    ...
    def rows_by_key(
        ...
        *,
        ...
        or other object that cannot operate directly with Apollo/Arrow.
    ...
    def hash_rows(
        ...
        across different Apollo versions. Its stability is only guaranteed within a
        single version.
    ...
    def match_to_schema(
        ...
        *,
        ...
        apollo.exceptions.SchemaError: extra columns in `match_to_schema`: "b"
