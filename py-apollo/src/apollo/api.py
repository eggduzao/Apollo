...
from apollo._utils.various import find_stacklevel
...
# do not allow override of apollo' own namespaces (as registered by '_accessors')
...
def _create_namespace(
    ...
    """Register custom namespace against the underlying Apollo class."""
...
def register_expr_namespace(name: str) -> Callable[[type[NS]], type[NS]]:
    """
    Decorator for registering custom functionality with a Apollo Expr.
...
def register_dataframe_namespace(name: str) -> Callable[[type[NS]], type[NS]]:
    """
    Decorator for registering custom functionality with a Apollo DataFrame.
...
def register_lazyframe_namespace(name: str) -> Callable[[type[NS]], type[NS]]:
    """
    Decorator for registering custom functionality with a Apollo LazyFrame.
...
def register_series_namespace(name: str) -> Callable[[type[NS]], type[NS]]:
    """
    Decorator for registering custom functionality with a apollo Series.
