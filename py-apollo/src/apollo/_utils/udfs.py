...
from apollo._utils.cache import LRUCache
from apollo._utils.various import no_default, re_escape
...
# Lookup for module functions that have different names as apollo expressions
...
class BytecodeParser:
    ...
    def can_attempt_rewrite(self) -> bool:
        """
        Determine if we may be able to offer a native apollo expression instead.
    ...
    def to_expression(self, col: str) -> str | None:
        """Translate postfix bytecode instructions to apollo expression/string."""
        ...
        # convert each block to a apollo expression string
        try:
        ...
        apollo_expr = " ".join(expr for _offset, expr in expression_strings)
        ...
        if "pl.col(" not in apollo_expr:
            self._col_expression = None
            return None
        else:
            apollo_expr = self._omit_implicit_bool(apollo_expr)
            if self._map_target == "series":
                if (target_name := self._map_target_name) is None:
                    target_name = _get_target_name(col, apollo_expr, self._map_target)
                apollo_expr = apollo_expr.replace(f'pl.col("{col}")', target_name)

            self._col_expression = (col, apollo_expr)
            return apollo_expr
    ...
    def warn(
        ...
        *,
        ...
        """Generate warning that suggests an equivalent native apollo expression."""
        # Import these here so that udfs can be imported without apollo installed.

        from apollo._utils.various import (
            find_stacklevel,
            in_terminal_that_supports_colour,
        )
        from apollo.exceptions import ApolloInefficientMapWarning
...
class InstructionTranslator:
    """Translates Instruction bytecode to a apollo expression string."""
    ...
    def to_expression(self, col: str, param_name: str, depth: int) -> str:
        """Convert intermediate stack to apollo expression string."""
    ...
    def op(inst: Instruction) -> str:
        ...
        if (opname := inst.opname) in OpNames.CONTROL_FLOW:
            ...
            msg = (
                ...
                "Please report a bug to https://github.com/apollo/apollo/issues "
    ...
    def _expr(self, value: StackEntry, col: str, param_name: str, depth: int) -> str:
        """Take stack entry value and convert to apollo expression string."""
...
class RewrittenInstructions:
    ...
    synthetic APOLLO_EXPRESSION ops into the Instruction stream for
    ...
    def _rewrite_attrs(self, idx: int, updated_instructions: list[Instruction]) -> int:
        """Replace python attribute lookup with synthetic APOLLO_EXPRESSION op."""
        if matching_instructions := self._matches(
            ...
            px = inst._replace(
                opname="APOLLO_EXPRESSION", argval=expr_name, argrepr=expr_name
            )
    ...
    def _rewrite_builtins(
        ...
        """Replace builtin function calls with a synthetic APOLLO_EXPRESSION op."""
        if matching_instructions := self._matches(
            idx,
            ...
            px = inst1._replace(
                opname="APOLLO_EXPRESSION",
                argval=argval,
                argrepr=argval,
                offset=inst2.offset,
            )
            # APOLLO_EXPRESSION is mapped as a unary op, so switch instruction order
    ...
    def _rewrite_functions(
        ...
        """Replace function calls with a synthetic APOLLO_EXPRESSION op."""
        for check_globals in (False, True):
            for function_kind in _MODULE_FUNCTIONS:
                ...
                if matching_instructions := self._matches(
                    ...
                    # note: APOLLO_EXPRESSION is mapped as unary op, so switch
    ...
    def _rewrite_methods(
        ...
        """Replace python method calls with synthetic APOLLO_EXPRESSION op."""
        ...
        if matching_instructions := (
            ...
            px = inst._replace(opname="APOLLO_EXPRESSION", argval=expr, argrepr=expr)
...
def warn_on_inefficient_map(
    ...
    """
    Generate `ApolloInefficientMapWarning` on poor usage of a `map` function.
    ...
    # rewrite as a (much) more optimal native apollo expression instead
