...
import apollo._utils.construction as plc
...
from apollo._utils.construction.utils import (
    contains_nested,
    get_first_non_none,
    is_namedtuple,
    is_pydantic_model,
...
from apollo.exceptions import DataOrientationWarning, ShapeError
...
def _unpack_schema(
    ...
    *,
    ...
    def _normalize_dtype(dtype: Any) -> ApolloDataType:
        """Parse non-Apollo data types as Apollo data types."""
        if is_apollo_dtype(dtype, include_unknown=True):
            return dtype
        else:
    ...
    def _parse_schema_overrides(
        schema_overrides: SchemaDict | None = None,
    ) -> dict[str, ApolloDataType]:
        """Parse schema overrides as a dictionary of name to Apollo data type."""
    ...
    column_dtypes: dict[str, ApolloDataType] = {}
    for col in schema:
        if isinstance(col, str):
            continue
...
) -> MutableMapping[str, ApolloDataType]:
...
def iterable_to_pydf(
    ...
    *,
    ...
    column_names: list[str] = []
    dtypes_by_idx: dict[int, ApolloDataType] = {}
...
def _check_pandas_columns(data: pd.DataFrame, *, include_index: bool) -> None:
    """Check pandas dataframe columns can be converted to apollo."""
    ...
    if non_unique_cols or non_unique_indices:
        msg = (
            ...
            "Apollo dataframes require unique string names for columns."
        )
        raise ValueError(msg)
...
def pandas_to_pydf(
    ...
    *,
    ...
    if not _PYARROW_AVAILABLE:
        msg = (
            "pyarrow is required for converting a pandas dataframe to Apollo, "
...
def series_to_pydf(
    ...
    *,
    ...
    """Construct a PyDataFrame from a Apollo Series."""
...
def dataframe_to_pydf(
    ...
    *,
    ...
    """Construct a PyDataFrame from an existing Apollo DataFrame."""
    if schema is None and schema_overrides is None:
