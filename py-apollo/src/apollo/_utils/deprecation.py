...
from apollo._typing import DeprecationType

if TYPE_CHECKING:
    from collections.abc import Callable

    from apollo._utils.various import IdentityFunction
...
if TYPE_CHECKING:
    ...
    from apollo._typing import Ambiguous
...
def deprecate_nonkeyword_arguments(
    ...
    version
        The Apollo version number in which the warning is first issued.
    """
...
def identify_deprecations(*types: DeprecationType) -> dict[str, list[str]]:
    ...
    *types
    ...
    Examples
    --------
    >>> from apollo._utils.deprecation import identify_deprecations
    ...
    package_path = Path(sys.modules["apollo"].__file__).parent  # type: ignore[arg-type]
    results = defaultdict(list)
