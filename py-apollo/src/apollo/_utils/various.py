...
from apollo._dependencies import _check_for_numpy, import_optional, subprocess
...
from apollo.datatypes.group import FLOAT_DTYPES, INTEGER_DTYPES
...
if TYPE_CHECKING:
    ...
    from apollo import DataFrame, Expr
    from apollo._typing import ApolloDataType, SizeUnit
...
def range_to_series(
    name: str, rng: range, dtype: ApolloDataType | None = None
) -> pl.Series:
...
def _cast_repr_strings_with_schema(
    df: DataFrame, schema: dict[str, ApolloDataType | None]
) -> DataFrame:
    """
...
def find_stacklevel() -> int:
    """
    Find the first place in the stack that is not inside Apollo.
...
def _apollo_warn(msg: str, category: type[Warning] = UserWarning) -> None:
    warnings.warn(
        msg,
        category=category,
        stacklevel=find_stacklevel(),
...
def re_escape(s: str) -> str:
    """Escape a string for use in a Apollo (Rust) regex."""
...
# Don't rename or move. This is used by apollo cloud
def display_dot_graph(
    *,
    ...
    output_type = (
        ...
        or "APOLLO_DOT_SVG_VIEWER" in os.environ
        else "png"
    )
    ...
    if _in_notebook():
        ...
        if (cmd := os.environ.get("APOLLO_DOT_SVG_VIEWER", None)) is not None:
            import tempfile
...
def qualified_type_name(obj: Any, *, qualify_apollo: bool = False) -> str:
    """
    ...
    qualify_apollo
        If False (default), omit the module path for our own (Apollo) objects.
    """
    ...
    if (
        not module
        or module == "builtins"
        or (not qualify_apollo and module.startswith("apollo."))
    ):
        return name
