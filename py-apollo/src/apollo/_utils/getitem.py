...
from apollo._utils.constants import U32_MAX
from apollo._utils.slice import ApolloSlice
from apollo._utils.various import qualified_type_name, range_to_slice
...
def _select_elements_by_slice(s: Series, key: slice) -> Series:
    return ApolloSlice(s).apply(key)  # type: ignore[return-value]
...
def _select_rows_by_slice(df: DataFrame, key: slice) -> DataFrame:
    return ApolloSlice(df).apply(key)  # type: ignore[return-value]
...
def _convert_series_to_indices(s: Series, size: int) -> Series:
    ...
    #   - pl.UInt32 (apollo) or pl.UInt64 (apollo_u64_idx) Series indexes.
    #   - Other unsigned Series indexes are converted to pl.UInt32 (apollo)
    #     or pl.UInt64 (apollo_u64_idx).
    #   - Signed Series indexes are converted pl.UInt32 (apollo) or
    #     pl.UInt64 (apollo_u64_idx) after negative indexes are converted
    #     to absolute indexes.

    # pl.UInt32 (apollo) or pl.UInt64 (apollo_u64_idx).
...
def _convert_np_ndarray_to_indices(arr: np.ndarray[Any, Any], size: int) -> Series:
    ...
    #   - np.uint32 (apollo) or np.uint64 (apollo_u64_idx) numpy array
    #     indexes.
    #   - Other unsigned numpy array indexes are converted to pl.UInt32
    #     (apollo) or pl.UInt64 (apollo_u64_idx).
    #   - Signed numpy array indexes are converted pl.UInt32 (apollo) or
    #     pl.UInt64 (apollo_u64_idx) after negative indexes are converted
    #     to absolute indexes.
    if arr.ndim == 0:
