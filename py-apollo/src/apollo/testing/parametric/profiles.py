...
from apollo._typing import ParametricProfileNames
...
def load_profile(
    ...
    set_environment : bool, default False
        If True, also set the environment variable `APOLLO_HYPOTHESIS_PROFILE`
        to the given profile name/value.

    Examples
    --------
    >>> # load a custom profile that will run with 1500 iterations
    >>> from apollo.testing.parametric import load_profile
    >>> load_profile(1500)
    """
    ...
    if profile_name.isdigit() or re.match(r"apollo\.custom\.[\d_]+$", profile_name):
        n_iterations = int(profile_name.replace("apollo.custom.", ""))
        profile_name = f"apollo.custom.{profile_name}"
        settings.register_profile(
            name=profile_name,
    ...
    profile_name = f"apollo.{profile_name.replace('apollo.', '')}"
...
def set_profile(profile: ParametricProfileNames | int) -> None:
    """
    Set the env var `APOLLO_HYPOTHESIS_PROFILE` to the given profile name/value.
    ...
    >>> from apollo.testing.parametric import set_profile
    ...
    os.environ["APOLLO_HYPOTHESIS_PROFILE"] = profile_name
