...
from apollo import register_extension_type
...
if TYPE_CHECKING:
    ...
    from apollo._typing import ApolloDataType, TimeUnit
    from apollo.datatypes import DataTypeClass
...
_SIMPLE_DTYPES: list[ApolloDataType] = [
    Int8,
    Int16,
    Int32,
    Int64,
...
def dtypes(
    *,
    allowed_dtypes: Collection[ApolloDataType] | None = None,
    excluded_dtypes: Sequence[ApolloDataType] | None = None,
    allow_time_zones: bool = True,
    nesting_level: int = 3,
) -> SearchStrategy[DataType]:
    """
    Create a strategy for generating Apollo :class:`DataType` objects.
...
def _parse_dtype_restrictions(
    allowed_dtypes: Collection[ApolloDataType] | None = None,
    excluded_dtypes: Sequence[ApolloDataType] | None = None,
) -> tuple[list[ApolloDataType], list[ApolloDataType], list[DataType]]:
    """
    ...
    excluded_dtypes_class: list[ApolloDataType] = []
    if excluded_dtypes:
    ...
    allowed_dtypes_flat: list[ApolloDataType]
    allowed_dtypes_nested: list[ApolloDataType]
...
def _flat_dtypes(
    draw: DrawFn,
    allowed_dtypes: Sequence[ApolloDataType] | None = None,
    excluded_dtypes: Sequence[ApolloDataType] | None = None,
    *,
    allow_time_zones: bool = True,
) -> DataType:
    """Create a strategy for generating non-nested Apollo :class:`DataType` objects."""
    if allowed_dtypes is None:
...
def _instantiate_flat_dtype(
    draw: DrawFn, dtype: ApolloDataType, *, allow_time_zones: bool = True
) -> DataType:
...
def _nested_dtypes(
    ...
    *,
    ...
    """Create a strategy for generating nested Apollo :class:`DataType` objects."""
    if allowed_dtypes is None:
...
def _instantiate_nested_dtype(
    ...
    *,
    ...
    def instantiate_inner(inner_dtype: ApolloDataType | None) -> DataType:
...
def _time_zones() -> SearchStrategy[str]:
    ...
    from apollo._plr import _known_timezones
...
def _instantiate_dtype(
    ...
    *,
    ...
    def draw_inner(dtype: ApolloDataType | None) -> DataType:
        if dtype is None:
            return draw(
                dtypes(
                    allowed_dtypes=allowed_dtypes,
