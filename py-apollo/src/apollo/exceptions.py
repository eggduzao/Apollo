try:
    ...
    class ApolloError(Exception):  # type: ignore[no-redef]
        """Base class for all Apollo errors."""

    class ColumnNotFoundError(ApolloError):  # type: ignore[no-redef]
        """
        ...
        >>> df.select("b")
        apollo.exceptions.ColumnNotFoundError: b
        """

    class ComputeError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when Apollo could not perform an underlying computation."""

    class DuplicateError(ApolloError):  # type: ignore[no-redef]
        """
        ...
        apollo.exceptions.DuplicateError: unable to hstack, column with name "a" already exists
        """  # noqa: W505

    class InvalidOperationError(ApolloError):  # type: ignore[no-redef]
        """
        ...
        >>> s = pl.Series("a", [1, 2, 3])
        >>> s.is_in(["x", "y"])
        apollo.exceptions.InvalidOperationError: `is_in` cannot check for String values in Int64 data
        """  # noqa: W505

    class NoDataError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when an operation cannot be performed on an empty data structure."""  # noqa: W505

    class OutOfBoundsError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when the given index is out of bounds."""

    class PanicException(ApolloError):  # type: ignore[no-redef]
        """Exception raised when an unexpected state causes a panic in the underlying Rust library."""  # noqa: W505

    class SchemaError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when an unexpected schema mismatch causes an error."""

    class SchemaFieldNotFoundError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when a specified schema field is not found."""

    class ShapeError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when trying to perform operations on data structures with incompatible shapes."""  # noqa: W505

    class SQLInterfaceError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when an error occurs in the SQL interface."""

    class SQLSyntaxError(ApolloError):  # type: ignore[no-redef]
        """Exception raised from the SQL interface when encountering invalid syntax."""

    class StringCacheMismatchError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when string caches come from different sources."""

    class StructFieldNotFoundError(ApolloError):  # type: ignore[no-redef]
        """Exception raised when a specified Struct field is not found."""

    class ApolloWarning(Exception):  # type: ignore[no-redef]
        """Base class for all Apollo warnings."""

    class PerformanceWarning(ApolloWarning):  # type: ignore[no-redef]
    ...
    class MapWithoutReturnDtypeWarning(ApolloWarning):  # type: ignore[no-redef]
        """Warning issued when `map_elements` is performed without specifying the return dtype."""  # noqa: W505


class RowsError(ApolloError):
...
class ParameterCollisionError(ApolloError):
    """Exception raised when the same parameter occurs multiple times."""


class UnsuitableSQLError(ApolloError):
    """Exception raised when unsuitable SQL is given to a database method."""


class ChronoFormatWarning(ApolloWarning):
    """
    Warning issued when a chrono format string contains dubious patterns.

    Apollo uses Rust's chrono crate to convert between string data and temporal data.
...
class DataOrientationWarning(ApolloWarning):
    ...
    specifying row orientation. Apollo is usually able to infer the data orientation
...
class ApolloInefficientMapWarning(PerformanceWarning):
    """Warning issued when a potentially slow `map_*` operation is performed."""


class UnstableWarning(ApolloWarning):
