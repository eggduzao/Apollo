...
from apollo._utils.various import extend_bool, qualified_type_name
from apollo._utils.wrap import wrap_df, wrap_expr, wrap_s
from apollo.datatypes import DTYPE_TEMPORAL_UNITS, Date, Datetime, Int64
from apollo.datatypes._parse import parse_into_datatype_expr
from apollo.lazyframe.opt_flags import (
...
if TYPE_CHECKING:
    ...
    from apollo import DataFrame, Expr, LazyFrame, Series
...
def fold(
    ...
    *,
    returns_scalar: bool = False,
    return_dtype: pl.DataTypeExpr | ApolloDataType | None = None,
) -> Expr:
    """
...
def exclude(
    columns: str | ApolloDataType | Collection[str] | Collection[ApolloDataType],
    *more_columns: str | ApolloDataType,
) -> Expr:
    """
...
def groups(column: str) -> Expr:
    ...
    """
    warnings.warn(
        "pl.groups() is deprecated and will be removed in Apollo 2.0. "
        "Use df.with_row_index().group_by(...).agg(pl.col('index')) instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def collect_all(
    ...
    *,
    ...
    from apollo.lazyframe.frame import _select_engine
...
def collect_all_async(
    ...
    *,
    ...
    Collects into a list of DataFrame (like :func:`apollo.collect_all`),
...
def select(
    *exprs: IntoExpr | Iterable[IntoExpr], eager: bool = True, **named_exprs: IntoExpr
    ...
    """
    Run apollo expressions without a context.
...
def from_epoch(
    ...
    """
    Utility function that parses an epoch timestamp (or Unix time) to Apollo Date(time).
...
def sql_expr(sql: str | Sequence[str]) -> Expr | list[Expr]:
    """
    Parse one or more SQL expressions to Apollo expression(s).
