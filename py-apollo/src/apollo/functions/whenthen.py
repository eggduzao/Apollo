...
from apollo._utils.parse import parse_predicates_constraints_into_expression
...
if TYPE_CHECKING:
    ...
    from apollo._typing import IntoExprColumn
...
def when(
    ...
    **constraints: Any,
    ...
    Warnings
    --------
    Apollo computes all expressions passed to `when-then-otherwise` in parallel and
    ...
    * The expression output name is taken from the first `then` statement. It is
    ...
    Note that in regular Apollo usage, a single string is parsed as a column name.
