...
from apollo.datatypes.group import (
    DATETIME_DTYPES,
    DURATION_DTYPES,
    FLOAT_DTYPES,
    INTEGER_DTYPES,
...
if TYPE_CHECKING:
    ...
    from apollo.expr.expr import Expr
...
def _create_col(
    name: (
        ...
        | PythonDataType
        | Iterable[str]
        | Iterable[ApolloDataType | PythonDataType]
    ),
    *more_names: str | ApolloDataType | PythonDataType,
) -> Expr:
    """Create one or more column expressions representing column(s) in a DataFrame."""
    dtypes: list[ApolloDataType]
    if more_names:
    ...
    if isinstance(name, str):
        ...
        dtypes = _apollo_dtype_match(name)
        return pl.Selector._by_dtype(dtypes).as_expr()  # type: ignore[arg-type]
        ...
        if isinstance(item, str):
            ...
            dtypes = []
            for nm in names:
                dtypes.extend(_apollo_dtype_match(nm))  # type: ignore[arg-type]
            return pl.Selector._by_dtype(dtypes).as_expr()  # type: ignore[arg-type]
...
def _python_dtype_match(tp: PythonDataType) -> list[ApolloDataType]:
    if tp is int:
...
def _apollo_dtype_match(tp: ApolloDataType) -> list[ApolloDataType]:
    if Datetime.is_(tp):
        return list(DATETIME_DTYPES)
...
class Col:
    """
    Create Apollo column expressions.
