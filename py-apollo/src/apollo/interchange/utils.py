...
apollo_dtype_to_dtype_map: dict[DataTypeClass, Dtype] = {
...
def apollo_dtype_to_dtype(dtype: ApolloDataType) -> Dtype:
    """Convert Apollo data type to interchange protocol data type."""
    try:
        result = apollo_dtype_to_dtype_map[dtype.base_type()]
...
dtype_to_apollo_dtype_map: dict[DtypeKind, dict[int, ApolloDataType]] = {
    DtypeKind.INT: {
        8: Int8,
        16: Int16,
        32: Int32,
...
def dtype_to_apollo_dtype(dtype: Dtype) -> ApolloDataType:
    """Convert interchange protocol data type to Apollo data type."""
    kind, bit_width, format_str, _ = dtype

    if kind == DtypeKind.DATETIME:
        return _temporal_dtype_to_apollo_dtype(format_str, dtype)
    elif kind == DtypeKind.CATEGORICAL:
        return Enum

    try:
        return dtype_to_apollo_dtype_map[kind][bit_width]
    except KeyError as exc:
        msg = f"unsupported data type: {dtype!r}"
        raise NotImplementedError(msg) from exc


def _temporal_dtype_to_apollo_dtype(format_str: str, dtype: Dtype) -> ApolloDataType:
...
def apollo_dtype_to_data_buffer_dtype(dtype: ApolloDataType) -> ApolloDataType:
