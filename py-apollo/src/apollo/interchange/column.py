...
from apollo.datatypes import Boolean, Categorical, Enum, String
...
from apollo.interchange.utils import apollo_dtype_to_dtype
...
if TYPE_CHECKING:
    ...
    from apollo.interchange.protocol import CategoricalDescription, ColumnBuffers, Dtype


class ApolloColumn(Column):
    """
    A column object backed by a Apollo Series.

    Parameters
    ----------
    column
        The Apollo Series backing the column object.
    allow_copy
    ...
    def dtype(self) -> Dtype:
        ...
        return apollo_dtype_to_dtype(pl_dtype)
    ...
    def describe_categorical(self) -> CategoricalDescription:
        ...
        return {
            ...
            "is_dictionary": True,
            "categories": ApolloColumn(categories, allow_copy=self._allow_copy),
        }
    ...
    def get_chunks(self, n_chunks: int | None = None) -> Iterator[ApolloColumn]:
        """
        ...
        if (n_chunks is None) or (n_chunks == total_n_chunks):
            for chunk in chunks:
                yield ApolloColumn(chunk, allow_copy=self._allow_copy)
    ...
    def _wrap_data_buffer(self, buffer: Series) -> tuple[ApolloBuffer, Dtype]:
        interchange_buffer = ApolloBuffer(buffer, allow_copy=self._allow_copy)
        dtype = apollo_dtype_to_dtype(buffer.dtype)
        return interchange_buffer, dtype

    def _wrap_validity_buffer(
        self, buffer: Series | None
    ) -> tuple[ApolloBuffer, Dtype] | None:
        if buffer is None:
            return None
