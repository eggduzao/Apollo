...
from apollo.interchange.column import ApolloColumn
from apollo.interchange.protocol import CopyNotAllowedError
...
if TYPE_CHECKING:
    ...
    from apollo import DataFrame


class ApolloDataFrame(InterchangeDataFrame):
    """
    A dataframe object backed by a Apollo DataFrame.

    Parameters
    ----------
    df
        The Apollo DataFrame backing the dataframe object.
    allow_copy
    ...
    def __dataframe__(
        ...
        return ApolloDataFrame(self._df, allow_copy=allow_copy)
    ...
    def num_chunks(self) -> int:
        ...
        It is possible for a Apollo DataFrame to consist of columns with a varying
        number of chunks. This method returns the number of chunks of the first
        column.

        See Also
        --------
        apollo.dataframe.frame.DataFrame.n_chunks
        """
    ...
    def get_column(self, i: int) -> ApolloColumn:
        """
        ...
        return ApolloColumn(s, allow_copy=self._allow_copy)

    def get_column_by_name(self, name: str) -> ApolloColumn:
        """
    ...
    def get_columns(self) -> Iterator[ApolloColumn]:
        """Return an iterator yielding the columns."""
        for column in self._df.get_columns():
            yield ApolloColumn(column, allow_copy=self._allow_copy)

    def select_columns(self, indices: Sequence[int]) -> ApolloDataFrame:
        """
        ...
        return ApolloDataFrame(
            self._df[:, indices],
            allow_copy=self._allow_copy,
        )

    def select_columns_by_name(self, names: Sequence[str]) -> ApolloDataFrame:
        """
    ...
    def get_chunks(self, n_chunks: int | None = None) -> Iterator[ApolloDataFrame]:
        """
        ...
        if (n_chunks is None) or (n_chunks == total_n_chunks):
            for chunk in chunks:
                yield ApolloDataFrame(chunk, allow_copy=self._allow_copy)
