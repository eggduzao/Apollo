...
from apollo._dependencies import _PYARROW_AVAILABLE, import_optional
from apollo._utils.unstable import issue_unstable_warning
from apollo._utils.various import parse_version, qualified_type_name
from apollo.datatypes import N_INFER_DEFAULT
from apollo.exceptions import ModuleUpgradeRequiredError
from apollo.io.database._cursor_proxies import ODBCCursorProxy
from apollo.io.database._executor import ConnectionExecutor
...
if TYPE_CHECKING:
    ...
    from apollo._typing import ConnectionOrCursor, DbReadEngine, SchemaDict
...
def read_database(
    ...
    *,
    ...
    connection
        ...
        to Apollo. Async driver connections are also supported, though this is currently
    ...
    * If Apollo has to create a cursor from your connection in order to execute the
      query then that cursor will be automatically closed when the query completes;
      however, Apollo will *never* close any other open connection or cursor.

    * Apollo is able to support more than just relational databases and SQL queries
...
def read_database_uri(
    ...
    *,
    ...
    ... )  # doctest: +SKIP
    """
    from apollo.io.database._utils import _read_sql_adbc, _read_sql_connectorx
