...
from apollo.datatypes._parse import parse_py_type_into_dtype
...
def dtype_from_database_typename(
    ...
    *,
    ...
    """
    Attempt to infer Apollo dtype from database cursor `type_code` string value.
    ...
    if value.endswith(array_aliases) or value.startswith(array_aliases):
        ...
        nested: ApolloDataType | None = None
...
def dtype_from_cursor_description(
    ...
    """Attempt to infer Apollo dtype from database cursor description `type_code`."""
...
def integer_dtype_from_nbits(
    ...
    *,
    unsigned: bool,
    default: ApolloDataType | None = None,
) -> ApolloDataType | None:
    """
    Return matching Apollo integer dtype from num bits and signed/unsigned flag.
