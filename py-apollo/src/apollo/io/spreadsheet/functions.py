...
from apollo._utils.various import deduplicate_names, normalize_filepath, parse_version
...
from apollo.datatypes.group import FLOAT_DTYPES, INTEGER_DTYPES, NUMERIC_DTYPES
from apollo.exceptions import (
    ModuleUpgradeRequiredError,
    NoDataError,
    ParameterCollisionError,
)
from apollo.functions import concat
from apollo.io._utils import looks_like_url, process_file_url
from apollo.io.csv.functions import read_csv

if TYPE_CHECKING:
    from collections.abc import Callable
    from typing import Literal

    from apollo._typing import ExcelSpreadsheetEngine, FileSource, SchemaDict
...
def read_excel(
    ...
    *,
    ...
    engine : {'calamine', 'openpyxl', 'xlsx2csv'}
        ...
        * "xlsx2csv": converts the data to an in-memory CSV before using the native
          apollo `read_csv` method to parse the result.
    engine_options
    ...
    * When using the `xlsx2csv` engine the target Excel sheet is first converted
      to CSV using `xlsx2csv.Xlsx2csv(source).convert()` and then parsed with Apollo'
...
def _reorder_columns(
    ...
    if columns:
        from apollo.selectors import by_index, by_name
