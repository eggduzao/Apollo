...
from apollo._utils.logging import eprint, verbose
from apollo.io.cloud._utils import NoPickleOption

if TYPE_CHECKING:
    from typing import TypeAlias

    from apollo._dependencies import boto3
...
class CachingCredentialProvider(CredentialProvider, abc.ABC):
    ...
    def __call__(self) -> CredentialProviderFunctionReturn:
        if os.getenv("APOLLO_DISABLE_PYTHON_CREDENTIAL_CACHING") == "1":
    ...
    def _can_use_as_provider(self) -> bool:
        ...
        try:
            ...
            if self.profile_name:
                ...
                raise apollo.exceptions.ComputeError(msg) from e
...
class CredentialProviderAzure(CachingCredentialProvider):
    ...
    def _try_get_azure_storage_account_credential_if_permitted(
        ...
        APOLLO_AUTO_USE_AZURE_STORAGE_ACCOUNT_KEY = os.getenv(
            "APOLLO_AUTO_USE_AZURE_STORAGE_ACCOUNT_KEY"
        )
        ...
        if verbose:
            eprint(
                "[CredentialProviderAzure]: "
                f"{self.account_name = } "
                f"{APOLLO_AUTO_USE_AZURE_STORAGE_ACCOUNT_KEY = }"
            )

        if (
            self.account_name is not None
            and APOLLO_AUTO_USE_AZURE_STORAGE_ACCOUNT_KEY == "1"
        ):
            try:
                creds = {
