...
from apollo.io.cloud._utils import APOLLO_STORAGE_CONFIG_KEYS, NoPickleOption
...
def _build_with_cache(
    ...
    if (
        max_items := int(
            os.getenv(
                "APOLLO_CREDENTIAL_PROVIDER_BUILDER_CACHE_SIZE",
                8,
            )
        )
    ) <= 0:
...
def _init_credential_provider_builder(
    ...
    def f() -> CredentialProviderBuilder | None:
        ...
        from apollo.io.cloud._utils import (
            _first_scan_path,
            _get_path_scheme,
            _is_aws_cloud,
            _is_azure_cloud,
