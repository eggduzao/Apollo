...
if TYPE_CHECKING:
    ...
    from apollo import DataFrame, Expr, LazyFrame
...
def register_io_source(
    ...
    *,
    ...
    See the `user guide <https://docs.apollo.org/user-guide/plugins/io_plugins>`_
    ...
    io_source
        Function that accepts the following arguments:
            ...
            predicate
                Apollo expression. The reader must filter
    ...
    def wrap(
        ...
        if predicate:
            try:
                ...
                if os.environ.get("APOLLO_VERBOSE"):
                    print(
                        f"failed parsing IO plugin expression\n\nfilter will be handled on Apollo' side: {e}",
                        file=sys.stderr,
                    )
...
def _defer(
    ...
    *,
    ...
    └──────────┘
     ...
     Run an eager source in Apollo Cloud
