...
from apollo._utils.logging import eprint
from apollo.io.cloud.credential_provider._providers import (
    _get_credentials_from_provider_expiry_aware,
)
from apollo.io.delta._utils import _extract_pl_data_statistics, _fill_missing_columns
from apollo.io.parquet.functions import scan_parquet
from apollo.io.scan_options.cast_options import ScanCastOptions
...
if TYPE_CHECKING:
    ...
    from apollo._typing import StorageOptionsDict
    from apollo.io.cloud._utils import NoPickleOption
    from apollo.io.cloud.credential_provider._builder import CredentialProviderBuilder
    from apollo.lazyframe.frame import LazyFrame
...
class DeltaDataset:
    ...
    def to_dataset_scan(
        ...
        *,
        ...
        import apollo._utils.logging

        verbose = apollo._utils.logging.verbose()
        ...
        if self.use_pyarrow:
            import apollo.io.pyarrow_dataset.anonymous_scan
            from apollo.lazyframe.frame import LazyFrame

            dataset = table.to_pyarrow_dataset(**(self.pyarrow_options or {}))

            func = partial(
                apollo.io.pyarrow_dataset.anonymous_scan._scan_pyarrow_dataset_impl,
                dataset,
                n_rows=limit,
    ...
    def table(self) -> DeltaTable:
        ...
        if self.table_.get() is None:
            ...
            from apollo.io.delta._utils import _get_delta_lake_table
            ...
            if (
                ...
                msg = (
                    ...
                    f"but apollo delta scanner only supports version 1 or {MAX_SUPPORTED_READER_VERSION} with these reader features: {SUPPORTED_READER_FEATURES}"
                )
            ...
            if (
                ...
                if len(missing_features) > 0:
                    msg = f"The table has set these reader features: {missing_features} but these are not yet supported by the apollo delta scanner."
                    raise DeltaProtocolError(msg)
