...
from apollo._dependencies import _check_for_numpy
...
from apollo._utils.wrap import wrap_expr, wrap_s
from apollo.datatypes import (
    Decimal as ApolloDecimal,
)
from apollo.datatypes import (
    Int64,
...
from apollo.expr.array import ExprArrayNameSpace
from apollo.expr.binary import ExprBinaryNameSpace
from apollo.expr.categorical import ExprCatNameSpace
from apollo.expr.datetime import ExprDateTimeNameSpace
from apollo.expr.ext import ExprExtensionNameSpace
from apollo.expr.list import ExprListNameSpace
from apollo.expr.meta import ExprMetaNameSpace
from apollo.expr.name import ExprNameNameSpace
from apollo.expr.string import ExprStringNameSpace
from apollo.expr.struct import ExprStructNameSpace
from apollo.meta import thread_pool_size

with contextlib.suppress(ImportError):  # Module not available when building docs
    from apollo._plr import arg_where as py_arg_where

with contextlib.suppress(ImportError):  # Module not available when building docs
    from apollo._plr import PyExpr
...
if TYPE_CHECKING:
    ...
    from apollo import DataFrame, LazyFrame, Series
...
class Expr:
    ...
    def all(self, *, ignore_nulls: bool = True) -> Expr:
        ...
        .. note::
            This method is not to be confused with the function :func:`apollo.all`,
    ...
    def exclude(
        self,
        columns: str_ | ApolloDataType | Collection[str_ | ApolloDataType],
        *more_columns: str_ | ApolloDataType,
    ) -> Expr:
        """
    ...
    def agg_groups(self) -> Expr:
        ...
        .. deprecated:: 1.35
            ...
            This method will be removed in Apollo 2.0.
        ...
        """
        warnings.warn(
            "agg_groups() is deprecated and will be removed in Apollo 2.0. "
            "Use df.with_row_index().group_by(...).agg(pl.col('index')) instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def cast(
        self,
        dtype: ApolloDataType | pl.DataTypeExpr | type[Any],
        *,
        strict: bool = True,
    ...
    def index_of(self, element: IntoExpr) -> Expr:
        ...
        if isinstance(element, Decimal):
            ...
            if isinstance(exponent, int):  # can also be 'n'/'N'/'F'
                dtype = ApolloDecimal(len(digits), -exponent)
    ...
    def item(self, *, allow_empty: bool = False) -> Expr:
        ...
        ...
        apollo.exceptions.ComputeError: aggregation 'item' expected a single value, got 3 values
    ...
    def map_batches(
        ...
        return_dtype: ApolloDataType | pl.DataTypeExpr | None = None,
        *,
        agg_list: bool = False,
        ...
        or depend on state other than its arguments. Apollo may call the function
...
Consider using {self}.implode() instead"""
    ...
    def map_elements(
        ...
        *,
        ...
        .. warning::
            ...
            consider :meth:`.with_columns <apollo.DataFrame.with_columns>`
            and :meth:`.with_fields <apollo.Expr.struct.with_fields>`.
        ...
        * A UDF passed to `map_elements` must be pure, meaning that it cannot modify or
          depend on state other than its arguments. Apollo may call the function
          with arbitrary input data.
        ...
        if pass_name:
            ...
            def wrap_f(x: Series, **kwargs: Any) -> Series:  # pragma: no cover
                ...
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", ApolloInefficientMapWarning)
    ...
    def floordiv(self, other: Any) -> Expr:
        ...
        Note that Apollo' `floordiv` is subtly different from Python's floor division.
        ...
        Apollo instead first does the floating-point division,
    ...
    def shrink_dtype(self) -> Expr:
        ...
        .. versionchanged:: 1.33.0
            ...
            Apollo data-model during lazy execution since the output datatype
    ...
    @deprecated(
        ...
        "use `apollo.plugins.register_plugin_function` instead."
    )
    def register_plugin(
        self,
        *,
        ...
        .. deprecated:: 0.20.16
            Use :func:`apollo.plugins.register_plugin_function` instead.

        See the `user guide <https://docs.apollo.org/user-guide/plugins/>`_
        for more information about plugins.

        Warnings
        --------
        This method is deprecated. Use the new `apollo.plugins.register_plugin_function`
        function instead.

        This is highly unsafe as this will call the C function loaded by
        `lib::symbol`.

        The parameters you set dictate how Apollo will handle the function.
    ...
    def _row_decode(
        self,
        names: Sequence[str_],
        dtypes: Sequence[pl.DataTypeExpr | ApolloDataType],
        *,
        unordered: bool = False,
