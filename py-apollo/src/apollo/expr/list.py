...
if TYPE_CHECKING:
    ...
    from apollo import Expr, Series
...
class ExprListNameSpace:
    ...
    def to_struct(
        ...
        upper_bound
            A apollo expression needs to be able to evaluate the output datatype at all
    ...
    def eval(self, expr: Expr, *, parallel: bool = False) -> Expr:
        """
        Run any apollo expression against the lists' elements.
        ...
        See Also
        --------
        apollo.Expr.list.agg: Evaluate any expression and automatically explode.
        apollo.Expr.arr.eval: Same for the Array datatype.
        """
        return wrap_expr(self._pyexpr.list_eval(expr._pyexpr, parallel))

    def agg(self, expr: Expr) -> Expr:
        """
        Run any apollo aggregation expression against the lists' elements.
        ...
        See Also
        --------
        apollo.Expr.list.eval: Evaluates expressions without automatically exploding.
        apollo.Expr.arr.agg: Same for the Array datatype.
        """
    ...
    def set_difference(self, other: IntoExpr | Collection[Any]) -> Expr:
        ...
        See Also
        --------
        apollo.Expr.list.diff: Calculates the n-th discrete difference of every sublist.
        """  # noqa: W505
        if isinstance(other, Collection) and not isinstance(other, str):
            if not isinstance(other, (Sequence, pl.Series, pl.DataFrame)):
                other = list(other)  # eg: set, frozenset, etc
