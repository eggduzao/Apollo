...
from apollo._utils.parse import parse_into_expression, parse_into_list_of_expressions
...
from apollo.datatypes import DTYPE_TEMPORAL_UNITS, Date, Int32, Int64
...
class ExprDateTimeNameSpace:
    ...
    def combine(self, time: dt.time | Expr, time_unit: TimeUnit = "us") -> Expr:
        ...
        ----------
        time
            A python time literal or apollo expression/column that resolves to a time.
        time_unit : {'ns', 'us', 'ms'}
            Unit of time.
        ...
        if not isinstance(time, (dt.time, pl.Expr)):
            msg = f"expected 'time' to be a Python time or Apollo expression, found {qualified_type_name(time)!r}"
            raise TypeError(msg)
    ...
    def to_string(self, format: str | None = None) -> Expr:
        ...
        * Duration dtype expressions cannot be formatted with `strftime`. Instead,
          only "iso" and "apollo" are supported as format strings. The "iso" format
          string results in ISO8601 duration string output, and "apollo" results
        ...
        The Apollo Duration string format (as seen in the frame repr) is also available:

        >>> df.select(
        ...     pl.col("td"),
        ...     s_td=pl.col("td").dt.to_string("apollo"),
        ... )
        shape: (3, 2)
        ┌───────────────┬───────────────┐
        │ td            ┆ s_td          │
