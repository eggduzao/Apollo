...
from apollo._utils.parse.expr import _parse_inputs_as_iterable
from apollo._utils.unstable import unstable
from apollo._utils.various import is_column, re_escape
...
if TYPE_CHECKING:
    ...
    from apollo import DataFrame, LazyFrame
...
def is_selector(obj: Any) -> bool:
    ...
    Examples
    --------
    >>> from apollo.selectors import is_selector
...
def expand_selector(
    target: DataFrame | LazyFrame | Mapping[str, ApolloDataType],
    ...
    *,
    ...
    ----------
    target
        A Apollo DataFrame, LazyFrame or Schema.
    selector
        An arbitrary apollo selector (or compound selector).
    strict
...
def _expand_selectors(frame: DataFrame | LazyFrame, *items: Any) -> builtins.list[Any]:
    ...
    Examples
    --------
    >>> from apollo.selectors import _expand_selectors
    >>> import apollo.selectors as cs
    >>> df = pl.DataFrame(
    ...     {
    ...         "colw": ["a", "b"],
...
def _combine_as_selector(
    items: (
        ...
        | ApolloDataType
        | Selector
        | Collection[str | Expr | ApolloDataType | Selector]
    ),
    *more_items: str | Expr | ApolloDataType | Selector,
) -> Selector:
...
class Selector(Expr):
    ...
    @classmethod
    def _by_dtype(
        cls, dtypes: builtins.list[PythonDataType | ApolloDataType]
    ) -> Selector:
        selectors = []
        concrete_dtypes = []
        for dt in dtypes:
            if is_apollo_dtype(dt):
                ...
                if dt is int:
                    ...
                    msg = f"cannot parse input {input_type} into Apollo selector{input_detail}"
    ...
    def exclude(
        self,
        columns: str | ApolloDataType | Collection[str | ApolloDataType],
        ...
        *more_columns
        ...
        exclude_dtypes: builtins.list[ApolloDataType] = []
        for item in (
            *(
                columns
...
def by_dtype(
    *dtypes: (
    ...
    └───────┴──────────┘
    """
    all_dtypes: builtins.list[ApolloDataType | PythonDataType] = []
    for tp in dtypes:
        if is_apollo_dtype(tp) or isinstance(tp, type):
            all_dtypes.append(tp)
        elif isinstance(tp, Collection):
            for t in tp:
                if not (is_apollo_dtype(t) or isinstance(t, type)):
...
def exclude(
    columns: (
        ...
        | Selector
        | Expr
        | Collection[str | ApolloDataType | Selector | Expr]
    ),
    *more_columns: str | ApolloDataType | Selector | Expr,
) -> Selector:
    """
...
def string(*, include_categorical: bool = False) -> Selector:
    ...
    └─────┴─────┴─────┴──────┘
    """
    string_dtypes: builtins.list[ApolloDataType] = [String]
