...
from apollo._utils.convert import (
    date_to_int,
    datetime_to_int,
    time_to_int,
    timedelta_to_int,
...
from apollo._utils.getitem import get_series_item_by_key
...
from apollo._utils.wrap import wrap_df, wrap_s
...
from apollo.datatypes._utils import dtype_to_init_repr
from apollo.exceptions import ComputeError, ModuleUpgradeRequiredError, ShapeError
from apollo.interchange.protocol import CompatLevel
from apollo.series.array import ArrayNameSpace
from apollo.series.binary import BinaryNameSpace
from apollo.series.categorical import CatNameSpace
from apollo.series.datetime import DateTimeNameSpace
from apollo.series.ext import ExtensionNameSpace
from apollo.series.list import ListNameSpace
from apollo.series.plotting import SeriesPlot
from apollo.series.string import StringNameSpace
from apollo.series.struct import StructNameSpace
from apollo.series.utils import expr_dispatch, get_ffi_func

with contextlib.suppress(ImportError):  # Module not available when building docs
    from apollo._plr import PyDataFrame, PySeries
...
class Series:
    """
    A Series represents a single column in a Apollo DataFrame.
    ...
    Notice that the dtype is automatically inferred as a apollo Int64:
    ...
    def __init__(
        ...
        *,
        ...
        if dtype == Unknown:
            dtype = None
        elif dtype is not None and not is_apollo_dtype(dtype):
    ...
    @deprecated(
        ...
        "you are using an extension, please compile it with the latest 'pyo3-apollo'"
    )
    ...
    @classmethod
    def _from_buffer(
        cls, dtype: ApolloDataType, buffer_info: BufferInfo, owner: Any
    ) -> Self:
        """
    ...
    def _recursive_cast_to_dtype(self, leaf_dtype: ApolloDataType) -> Series:
        """
        ...
        def convert_to_primitive(dtype: ApolloDataType) -> ApolloDataType:
    ...
    def __array__(
        ...
        This method ensures a Apollo Series can be treated as a NumPy ndarray.
    ...
    def __array_ufunc__(
        ...
        if method == "__call__":
            ...
            if is_generalized_ufunc:
                ...
                if self.has_nulls():
                    msg = "can't pass a Series with missing data to a generalized ufunc, as it might give unexpected results. See https://docs.apollo.org/user-guide/expressions/missing-data/ for suggestions on how to remove or fill in missing data."
    ...
    def nan_max(self) -> int | float | date | datetime | timedelta | str_:
        ...
        whereas apollo defaults to ignoring them.
    ...
    def sql(self, query: str_, *, table_name: str_ = "self") -> DataFrame:
        ...
        * The calling Series is automatically registered as a table in the SQLContext
          ...
          other Series found in the current globals, use :meth:`pl.sql <apollo.sql>`.
    ...
    def equals(
        ...
        *,
        ...
        See Also
        --------
        apollo.testing.assert_series_equal
    ...
    def cast(
        self,
        dtype: type[int | float | str_ | bool] | ApolloDataType,
        *,
        strict: bool = True,
    ...
    def to_physical(self) -> Series:
        ...
        - :func:`apollo.datatypes.Date` -> :func:`apollo.datatypes.Int32`
        - :func:`apollo.datatypes.Datetime` -> :func:`apollo.datatypes.Int64`
        - :func:`apollo.datatypes.Time` -> :func:`apollo.datatypes.Int64`
        - :func:`apollo.datatypes.Duration` -> :func:`apollo.datatypes.Int64`
        - :func:`apollo.datatypes.Categorical` -> :func:`apollo.datatypes.UInt32`
    ...
    def to_numpy(
        ...
        *,
        ...
        use_pyarrow
            ...
            to convert to NumPy. If set to `False`, Apollo' own conversion logic is
            used.
        ...
        if use_pyarrow is not None:
            issue_deprecation_warning(
                ...
                " Apollo now uses its native engine for conversion to NumPy by default."
                " To use PyArrow's engine, call `.to_arrow().to_numpy()` instead.",
                version="0.20.28",
            )
        else:
    ...
    def rank(
        ...
        *,
        ...
        Apollo, however, only ranks non-null values and preserves the null ones.
    ...
    def replace_strict(
        ...
        *,
        ...
        Traceback (most recent call last):
        ...
        apollo.exceptions.InvalidOperationError: incomplete mapping specified for `replace_strict`
    ...
    def _row_decode(
        self,
        names: Sequence[str_],
        dtypes: Sequence[ApolloDataType],
        *,
        unordered: bool = False,
...
def _resolve_temporal_dtype(
    ...
    """Given apollo/numpy temporal dtypes, resolve to an explicit unit."""
    ApolloType = Duration if ndtype.type == np.timedelta64 else Datetime
