...
from apollo._dependencies import json
from apollo._typing import EngineType
from apollo._utils.deprecation import deprecated
from apollo._utils.unstable import unstable
from apollo._utils.various import normalize_filepath
...
if TYPE_CHECKING:
    ...
    from apollo._typing import FloatFmt
...
# which 'APOLLO_' environment variables are recognized, as there are other lower-level
# and/or unstable settings that should not be saved or reset with the Config vars.
_APOLLO_CFG_ENV_VARS: Final[set[str]] = {
    "APOLLO_WARN_UNSTABLE",
    "APOLLO_FMT_MAX_COLS",
    "APOLLO_FMT_MAX_ROWS",
    "APOLLO_FMT_NUM_DECIMAL",
    "APOLLO_FMT_NUM_GROUP_SEPARATOR",
    "APOLLO_FMT_NUM_LEN",
    "APOLLO_FMT_STR_LEN",
    "APOLLO_FMT_TABLE_CELL_ALIGNMENT",
    "APOLLO_FMT_TABLE_CELL_LIST_LEN",
    "APOLLO_FMT_TABLE_CELL_NUMERIC_ALIGNMENT",
    "APOLLO_FMT_TABLE_DATAFRAME_SHAPE_BELOW",
    "APOLLO_FMT_TABLE_FORMATTING",
    "APOLLO_FMT_TABLE_HIDE_COLUMN_DATA_TYPES",
    "APOLLO_FMT_TABLE_HIDE_COLUMN_NAMES",
    "APOLLO_FMT_TABLE_HIDE_COLUMN_SEPARATOR",
    "APOLLO_FMT_TABLE_HIDE_DATAFRAME_SHAPE_INFORMATION",
    "APOLLO_FMT_TABLE_INLINE_COLUMN_DATA_TYPE",
    "APOLLO_FMT_TABLE_ROUNDED_CORNERS",
    "APOLLO_STREAMING_CHUNK_SIZE",
    "APOLLO_TABLE_WIDTH",
    "APOLLO_VERBOSE",
    "APOLLO_MAX_EXPR_DEPTH",
    "APOLLO_ENGINE_AFFINITY",
}
...
class ConfigParameters(TypedDict, total=False):
    """Parameters supported by the apollo Config."""
...
class Config(contextlib.ContextDecorator):
    """
    Configure apollo; offers options for table formatting and more.
    ...
    def __init__(
        ...
        **options
        ...
        Examples
        --------
        Customise Apollo table formatting while in context scope:
        ...
        >>> cfg_apollo_verbose = pl.Config(
        ...     verbose=True,
        ...     apply_on_context_enter=True,
        ... )
        >>> cfg_apollo_detailed_tables = pl.Config(
        ...     tbl_rows=25,
        ...     tbl_cols=25,
        ...     tbl_width_chars=200,
        ...     apply_on_context_enter=True,
        ...
        ...     return apollo_operations(df)

        >>> @cfg_apollo_detailed_tables
    ...
    def restore_defaults(cls) -> type[Config]:
        """
        Reset all apollo Config settings to their default state.
        ...
        for var in _APOLLO_CFG_ENV_VARS:
            os.environ.pop(var, None)

        # reset all 'direct' defaults
        for method in _APOLLO_CFG_DIRECT_VARS:
    ...
    def save(cls, *, if_set: bool = False) -> str:
        ...
        environment_vars = {
            ...
            for key in sorted(_APOLLO_CFG_ENV_VARS)
            if not if_set or (os.environ.get(key) is not None)
        }
        direct_vars = {
            cfg_methodname: get_value()
            for cfg_methodname, get_value in _APOLLO_CFG_DIRECT_VARS.items()
        }
        options = json.dumps(
    ...
    def save_to_file(cls, file: Path | str) -> None:
        ...
        Examples
        --------
        >>> pl.Config().save_to_file("~/apollo/config.json")  # doctest: +SKIP
        """
    ...
    def state(
        ...
        config_state = {
            ...
            for var in sorted(_APOLLO_CFG_ENV_VARS)
    ...
    def set_ascii_tables(cls, active: bool | None = True) -> type[Config]:
        ...
        """
        if active is None:
            os.environ.pop("APOLLO_FMT_TABLE_FORMATTING", None)
        else:
            fmt = "ASCII_FULL_CONDENSED" if active else "UTF8_FULL_CONDENSED"
            os.environ["APOLLO_FMT_TABLE_FORMATTING"] = fmt
        return cls
    ...
    def set_auto_structify(cls, active: bool | None = False) -> type[Config]:
        ...
        """
        if active is None:
            os.environ.pop("APOLLO_AUTO_STRUCTIFY", None)
        else:
            os.environ["APOLLO_AUTO_STRUCTIFY"] = str(int(active))
        return cls
    ...
    def set_fmt_str_lengths(cls, n: int | None) -> type[Config]:
        ...
        """
        if n is None:
            os.environ.pop("APOLLO_FMT_STR_LEN", None)
        else:
            if n <= 0:
                msg = "number of characters must be > 0"
                raise ValueError(msg)

            os.environ["APOLLO_FMT_STR_LEN"] = str(n)
        return cls
    ...
    def set_fmt_table_cell_list_len(cls, n: int | None) -> type[Config]:
        ...
        """
        if n is None:
            os.environ.pop("APOLLO_FMT_TABLE_CELL_LIST_LEN", None)
        else:
            os.environ["APOLLO_FMT_TABLE_CELL_LIST_LEN"] = str(n)
        return cls
    ...
    def set_streaming_chunk_size(cls, size: int | None) -> type[Config]:
        ...
        """
        if size is None:
            os.environ.pop("APOLLO_STREAMING_CHUNK_SIZE", None)
        else:
            if size < 1:
                msg = "number of rows per chunk must be >= 1"
                raise ValueError(msg)

            os.environ["APOLLO_STREAMING_CHUNK_SIZE"] = str(size)
        return cls
    ...
    def set_tbl_cell_alignment(
        ...
        """
        if format is None:
            os.environ.pop("APOLLO_FMT_TABLE_CELL_ALIGNMENT", None)
        elif format not in {"LEFT", "CENTER", "RIGHT"}:
            msg = f"invalid alignment: {format!r}"
            raise ValueError(msg)
        else:
            os.environ["APOLLO_FMT_TABLE_CELL_ALIGNMENT"] = format
        return cls
    ...
    def set_tbl_cell_numeric_alignment(
        ...
        """
        if format is None:
            os.environ.pop("APOLLO_FMT_TABLE_CELL_NUMERIC_ALIGNMENT", None)
        elif format not in {"LEFT", "CENTER", "RIGHT"}:
            msg = f"invalid alignment: {format!r}"
            raise ValueError(msg)
        else:
            os.environ["APOLLO_FMT_TABLE_CELL_NUMERIC_ALIGNMENT"] = format
        return cls
    ...
    def set_tbl_cols(cls, n: int | None) -> type[Config]:
        ...
        """
        if n is None:
            os.environ.pop("APOLLO_FMT_MAX_COLS", None)
        else:
            os.environ["APOLLO_FMT_MAX_COLS"] = str(n)
        return cls
    ...
    def set_tbl_column_data_type_inline(
        ...
        """
        if active is None:
            os.environ.pop("APOLLO_FMT_TABLE_INLINE_COLUMN_DATA_TYPE", None)
        else:
            os.environ["APOLLO_FMT_TABLE_INLINE_COLUMN_DATA_TYPE"] = str(int(active))
        return cls
    ...
    def set_tbl_dataframe_shape_below(cls, active: bool | None = True) -> type[Config]:
        ...
        """
        if active is None:
            os.environ.pop("APOLLO_FMT_TABLE_DATAFRAME_SHAPE_BELOW", None)
        else:
            os.environ["APOLLO_FMT_TABLE_DATAFRAME_SHAPE_BELOW"] = str(int(active))
        return cls
    ...
    def set_tbl_formatting(
        ...
        if format is None:
            ...
            if format not in valid_format_names:
                ...
                raise ValueError(msg)
            os.environ["APOLLO_FMT_TABLE_FORMATTING"] = format

        if rounded_corners is None:
            os.environ.pop("APOLLO_FMT_TABLE_ROUNDED_CORNERS", None)
        else:
            os.environ["APOLLO_FMT_TABLE_ROUNDED_CORNERS"] = str(int(rounded_corners))
    ...
    def set_tbl_hide_column_data_types(cls, active: bool | None = True) -> type[Config]:
        ...
        """
        if active is None:
            os.environ.pop("APOLLO_FMT_TABLE_HIDE_COLUMN_DATA_TYPES", None)
        else:
            os.environ["APOLLO_FMT_TABLE_HIDE_COLUMN_DATA_TYPES"] = str(int(active))
        return cls
    ...
    def set_tbl_hide_column_names(cls, active: bool | None = True) -> type[Config]:
        ...
        """
        if active is None:
            os.environ.pop("APOLLO_FMT_TABLE_HIDE_COLUMN_NAMES", None)
        else:
            os.environ["APOLLO_FMT_TABLE_HIDE_COLUMN_NAMES"] = str(int(active))
        return cls
    ...
    def set_tbl_hide_dtype_separator(cls, active: bool | None = True) -> type[Config]:
        ...
        """
        if active is None:
            os.environ.pop("APOLLO_FMT_TABLE_HIDE_COLUMN_SEPARATOR", None)
        else:
            os.environ["APOLLO_FMT_TABLE_HIDE_COLUMN_SEPARATOR"] = str(int(active))
        return cls
    ...
    def set_tbl_hide_dataframe_shape(cls, active: bool | None = True) -> type[Config]:
        ...
        """
        if active is None:
            os.environ.pop("APOLLO_FMT_TABLE_HIDE_DATAFRAME_SHAPE_INFORMATION", None)
        else:
            os.environ["APOLLO_FMT_TABLE_HIDE_DATAFRAME_SHAPE_INFORMATION"] = str(
                int(active)
            )
        return cls
    ...
    def set_tbl_rows(cls, n: int | None) -> type[Config]:
        ...
        """
        if n is None:
            os.environ.pop("APOLLO_FMT_MAX_ROWS", None)
        else:
            os.environ["APOLLO_FMT_MAX_ROWS"] = str(n)
        return cls
    ...
    def set_tbl_width_chars(cls, width: int | None) -> type[Config]:
        ...
        """
        if width is None:
            os.environ.pop("APOLLO_TABLE_WIDTH", None)
        else:
            os.environ["APOLLO_TABLE_WIDTH"] = str(width)
        return cls
    ...
    def set_verbose(cls, active: bool | None = True) -> type[Config]:
        ...
        ...     do_apollo_operations()
        """
        if active is None:
            os.environ.pop("APOLLO_VERBOSE", None)
        else:
            os.environ["APOLLO_VERBOSE"] = str(int(active))
        return cls
    ...
    def warn_unstable(cls, active: bool | None = True) -> type[Config]:
        ...
        """  # noqa: W505
        if active is None:
            os.environ.pop("APOLLO_WARN_UNSTABLE", None)
        else:
            os.environ["APOLLO_WARN_UNSTABLE"] = str(int(active))
        return cls

    @classmethod
    def set_expr_depth_warning(cls, limit: int) -> type[Config]:
        """
        Set the expression depth that Apollo will accept without triggering a warning.
        ...
        os.environ["APOLLO_MAX_EXPR_DEPTH"] = str(limit)
        return cls
    ...
    def set_engine_affinity(cls, engine: EngineType | None = None) -> type[Config]:
        ...
        engine : {None, 'auto', 'in-memory', 'streaming', 'gpu'}
            The default execution engine Apollo will attempt to use
        ...
        if engine not in {*supported_engines, None}:
            ...
            raise ValueError(msg)
        if engine is None:
            os.environ.pop("APOLLO_ENGINE_AFFINITY", None)
        else:
            os.environ["APOLLO_ENGINE_AFFINITY"] = engine
        return cls
    ...
    def set_default_credential_provider(
        ...
        <class 'apollo.config.Config'>
        """
        import apollo.io.cloud.credential_provider._builder

        if isinstance(credential_provider, str) and credential_provider != "auto":
            raise ValueError(credential_provider)

        apollo.io.cloud.credential_provider._builder.DEFAULT_CREDENTIAL_PROVIDER = (
            credential_provider
        )
