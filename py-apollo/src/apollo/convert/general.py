...
from apollo._dependencies import _check_for_pyarrow
from apollo._dependencies import pandas as pd
from apollo._dependencies import pyarrow as pa
from apollo._utils.construction.dataframe import (
    arrow_to_pydf,
    dict_to_pydf,
    numpy_to_pydf,
    pandas_to_pydf,
...
from apollo._utils.construction.series import arrow_to_pyseries, pandas_to_pyseries
...
from apollo.datatypes import N_INFER_DEFAULT, Categorical, String
from apollo.exceptions import NoDataError
...
if TYPE_CHECKING:
    ...
    from apollo._dependencies import torch
    ...
    from apollo.interchange.protocol import SupportsInterchange
...
def from_torch(
    ...
    *,
    ...
    force : bool
        ...
        before passing the Tensor to Apollo.
...
def from_pandas(
    ...
    *,
    ...
    """
    Construct a Apollo DataFrame or Series from a pandas DataFrame, Series, or Index.
...
def from_repr(data: str) -> DataFrame | Series:
    """
    Construct a Apollo DataFrame or Series from its string representation.
    ...
    ----------
    data
        A string containing a apollo DataFrame or Series repr; does not need
...
def _from_dataframe_repr(m: re.Match[str]) -> DataFrame:
    ...
    from apollo.datatypes.convert import dtype_short_repr_to_dtype
    from apollo.io.database._inference import dtype_from_database_typename

    def _dtype_from_name(tp: str | None) -> ApolloDataType | None:
        return (
            None
            if tp is None
            else (
    ...
    if no_dtypes:
        if df.is_empty():
            ...
            if all(tp == String for tp in df.schema.values()):
                from apollo.io import read_csv
...
def from_dataframe(
    ...
    *,
    ...
    """
    Build a Apollo DataFrame from any dataframe supporting the PyCapsule Interface.
    ...
    Examples
    --------
    Convert a pandas dataframe to Apollo.
    ...
    from apollo.interchange.from_dataframe import from_dataframe
