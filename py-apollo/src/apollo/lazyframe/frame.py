...
from apollo._dependencies import apollo_cloud as pc
from apollo._dependencies import pyarrow as pa
from apollo._typing import (
    ParquetMetadata,
)
from apollo._utils.async_ import _AioDataFrameResult, _GeventDataFrameResult
from apollo._utils.convert import negate_duration_string, parse_as_duration_string
...
from apollo._utils.parquet import wrap_parquet_metadata_callback
from apollo._utils.parse import (
    parse_into_expression,
    parse_into_list_of_expressions,
)
from apollo._utils.parse.expr import parse_list_into_selector
from apollo._utils.serde import serialize_apollo_object
from apollo._utils.slice import LazyApolloSlice
...
from apollo._utils.wrap import wrap_df, wrap_expr
...
from apollo.datatypes.group import DataTypeGroup
from apollo.exceptions import InvalidOperationError, PerformanceWarning
from apollo.interchange.protocol import CompatLevel
from apollo.lazyframe.engine_config import GPUEngine
from apollo.lazyframe.group_by import LazyGroupBy
from apollo.lazyframe.in_process import InProcessQuery
from apollo.lazyframe.opt_flags import DEFAULT_QUERY_OPT_FLAGS, forward_old_opt_flags
from apollo.schema import Schema
from apollo.selectors import by_dtype, expand_selector

with contextlib.suppress(ImportError):  # Module not available when building docs
    from apollo._plr import PyLazyFrame, get_engine_affinity
...
if TYPE_CHECKING:
    ...
    with contextlib.suppress(ImportError):  # Module not available when building docs
        from apollo._plr import PyExpr, PySelector

    with contextlib.suppress(ImportError):  # Module not available when building docs
        import apollo._plr as plr

    from apollo import DataFrame, DataType, Expr
...
class LazyFrame:
    ...
    is the preferred (and highest-performance) mode of operation for apollo.
    ...
    def __getitem__(self, item: slice_) -> LazyFrame:
        ...
        if not isinstance(item, slice):
            ...
            raise TypeError(msg)
        return LazyApolloSlice(self).apply(item)
...
"""
    ...
    def describe(
        ...
        * We do not guarantee the output of `describe` to be stable. It will show
        ...
        └────────────┴──────────┴──────────┴──────────┴──────┴─────────────────────┴──────────┘
        """  # noqa: W505
        from apollo.convert import from_dict
        ...
        @lru_cache
        def skip_minmax(dt: ApolloDataType) -> bool:
    ...
    def explain(
        ...
        *,
        ...
        engine
            ...
            is run using the apollo in-memory engine. Apollo will also
            attempt to use the engine set by the `APOLLO_ENGINE_AFFINITY`
            environment variable. If it cannot run the query using the
            selected engine, the query is run using the apollo in-memory
            ...
            .. note::
               ...
               Running with `APOLLO_VERBOSE=1` will provide information if a query
               falls back (and why).
    ...
    def collect(
        ...
        **_kwargs: Any,
        ...
        apollo.collect_all : Collect multiple LazyFrames at the same time.
        apollo.Config.set_streaming_chunk_size : Set the size of streaming batches.
    ...
    def collect_async(
        ...
        *,
        ...
        --------
        apollo.collect_all : Collect multiple LazyFrames at the same time.
        apollo.collect_all_async : Collect multiple LazyFrames at the same time lazily.
    ...
    def sink_parquet(
        ...
        *,
        ...
        engine
            ...
            using the apollo streaming engine. Apollo will also
            attempt to use the engine set by the `APOLLO_ENGINE_AFFINITY`
            environment variable. If it cannot run the query using the
            selected engine, the query is run using the apollo streaming
            engine.
        optimizations
            The optimization passes done during query optimization.
        ...
        from apollo.io.partition import _SinkOptions
    ...
    def cast(
        ...
        *,
        ...
        for c, dtype in dtypes.items():
            if (is_apollo_dtype(c) or isinstance(c, DataTypeGroup)) or (
                isinstance(c, Collection) and all(is_apollo_dtype(x) for x in c)
            ):
    ...
    def select(
        ...
        **named_exprs
        ...
        └───────────┘
        """
        structify = bool(int(os.environ.get("APOLLO_AUTO_STRUCTIFY", 0)))
    ...
    def rename(
        ...
        apollo will block projection and predicate pushdowns at this node.
    ...
    def fill_null(
        ...
        *,
        ...
        └─────┴──────┘
        """
        from apollo import Decimal

        dtypes: Sequence[ApolloDataType] | None
    ...
    def map_batches(
        ...
        *,
        ...
        It is important that the function returns a Apollo DataFrame.
        ...
        validate_output_schema
            It is paramount that apollo' schema is correct. This flag will ensure that
        ...
        *,
        ...
        if on is None:
            if left_on is None and right_on is None:
                ...
                row_index_used = True
                row_index_name = "__APOLLO_ROW_INDEX"
    ...
    def remote(
        ...
        *,
        ...
        """
        Run a query remotely on Apollo Cloud.

        This allows you to run Apollo remotely on
        one or more workers via several strategies
        for distributed compute.

        Read more in the `Announcement post <https://apollo.org/posts/apollo-cloud-what-we-are-building/>`_
        ...
        engine: {'auto', 'streaming', 'in-memory'}
            This will serve as a hint that tells Apollo which engine
