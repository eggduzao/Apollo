...
import apollo.datatypes
import apollo.functions as F

with contextlib.suppress(ImportError):  # Module not available when building docs
    import apollo._plr as plr
    from apollo._plr import PyCategories
    from apollo._plr import dtype_str_repr as _dtype_str_repr

import apollo.datatypes.classes as pldt
...
class DataTypeClass(type):
    ...
    @classmethod
    def is_(cls, other: ApolloDataType) -> bool:  # noqa: D102
        ...
    ...
    @classmethod
    def from_python(cls, py_type: PythonDataType) -> ApolloDataType:  # noqa: D102
        ...
...
class DataType(metaclass=DataTypeClass):
    """Base class for all Apollo data types."""
    ...
    @overload
    def __eq__(self, other: ApolloDataType) -> bool: ...

    def __eq__(self, other: pl.DataTypeExpr | ApolloDataType) -> pl.Expr | bool:
    ...
    @classinstmethod
    def is_(self, other: ApolloDataType) -> bool:
        """
        ...
        ----------
        other
            the other Apollo dtype to compare with.
    ...
    @classmethod
    def from_python(cls, py_type: PythonDataType) -> ApolloDataType:
        """
        Return the Apollo data type corresponding to a given Python type.

        Notes
        -----
        Not every Python type has a corresponding Apollo data type; in general
        you should declare Apollo data types explicitly to exactly specify
        ...
        TypeError: cannot parse input <class 'datetime.tzinfo'> into Apollo data type
        """
        from apollo.datatypes._parse import parse_into_dtype
    ...
    def to_python(self) -> PythonDataType:
        """
        Return the Python type corresponding to this Apollo data type.
        ...
        <class 'list'>
        """
        from apollo.datatypes import dtype_to_py_type
    ...
    def to_dtype_expr(self) -> pl.DataTypeExpr:
        ...
        Int16
        """
        from apollo._plr import PyDataTypeExpr
...
class Decimal(NumericType):
    ...
    def __eq__(self, other: ApolloDataType) -> bool:  # type: ignore[override]
...
class Categories:
    ...
    mapping stored in Apollo; if no actual data exists using this mapping (like
    a `Series` or `DataFrame`), the mapping is cleaned up by Apollo:
    ...
    def __init__(
        ...
        name: str | None = None,
        namespace: str = "",
        physical: ApolloDataType = pldt.UInt32,
    ) -> None:
    ...
    @staticmethod
    def random(
        namespace: str = "", physical: ApolloDataType = pldt.UInt32
    ) -> Categories:
        """
    ...
    def physical(self) -> ApolloDataType:
    ...
    def __getstate__(self) -> tuple[str, str, ApolloDataType]:
        return self.name(), self.namespace(), self.physical()

    def __setstate__(self, state: tuple[str, str, ApolloDataType]) -> None:
...
class List(NestedType):
    ...
    inner: ApolloDataType

    def __init__(self, inner: ApolloDataType | PythonDataType) -> None:
        self.inner = apollo.datatypes.parse_into_dtype(inner)
...
class Array(NestedType):
    ...
    def __init__(
        self,
        inner: ApolloDataType | PythonDataType,
        ...
        *,
        ...
        inner_parsed = apollo.datatypes.parse_into_dtype(inner)
...
class Field:
    ...
    def __init__(self, name: str, dtype: ApolloDataType) -> None:
        self.name = name
        self.dtype = apollo.datatypes.parse_into_dtype(dtype)
...
class Struct(NestedType):
    ...
    When initializing a Series, Apollo can infer a struct data type from the data.
    ...
    def __iter__(self) -> Iterator[tuple[str, ApolloDataType]]:
        for fld in self.fields:
            yield fld.name, fld.dtype

    def __reversed__(self) -> Iterator[tuple[str, ApolloDataType]]:
    ...
    def to_schema(self) -> OrderedDict[str, ApolloDataType]:
...
class BaseExtension(DataType):
    ...
    --------
    Extension
    apollo.register_extension_type
    """

    def __init__(
        self, name: str, storage: ApolloDataType, metadata: str | None = None
    ) -> None:
        self._name = name
    ...
    @classmethod
    def ext_from_params(
        cls, name: str, storage: ApolloDataType, metadata: str | None
    ) -> Any:
    ...
    def ext_storage(self) -> ApolloDataType:
    ...
    def __getstate__(self) -> tuple[str, ApolloDataType, str | None]:
        return self.ext_name(), self.ext_storage(), self.ext_metadata()

    def __setstate__(self, state: tuple[str, ApolloDataType, str | None]) -> None:
