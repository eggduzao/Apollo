...
from apollo.datatypes.convert import is_apollo_dtype

if TYPE_CHECKING:
    from apollo._typing import ApolloDataType, PythonDataType, SchemaDict
...
def parse_into_dtype(input: Any) -> ApolloDataType:
    """
    Parse an input into a Apollo data type.

    Raises
    ------
    TypeError
        If the input cannot be parsed into a Apollo data type.
    """
    if is_apollo_dtype(input):
        return input
...
def try_parse_into_dtype(input: Any) -> ApolloDataType | None:
    """Try parsing an input into a Apollo data type, returning None on failure."""
    try:
...
def parse_py_type_into_dtype(input: PythonDataType | type[object]) -> ApolloDataType:
    """Convert Python data type to Apollo data type."""
    if input is int:
        return Int64()
    elif input is float:
        return Float64()
...
def _parse_generic_into_dtype(input: Any) -> ApolloDataType:
    """Parse a generic type (from typing annotation) into a Apollo data type."""
...
def _parse_forward_ref_into_dtype(input: ForwardRef) -> ApolloDataType:
    """Parse a ForwardRef into a Apollo data type."""
    annotation = input.__forward_arg__

    # Strip "optional" designation - Apollo data types are always nullable
...
def _parse_union_type_into_dtype(input: Any) -> ApolloDataType:
    """
    Parse a union of types into a Apollo data type.
...
def _raise_on_invalid_dtype(input: Any) -> NoReturn:
    ...
    msg = f"cannot parse input {input_type} into Apollo data type{input_detail}"
