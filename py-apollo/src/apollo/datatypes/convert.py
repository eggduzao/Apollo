...
from apollo.datatypes.classes import (
    Array,
    Binary,
    Boolean,
    Categorical,
...
if TYPE_CHECKING:
    ...
    from apollo._typing import ApolloDataType, PythonDataType, TimeUnit
...
) -> TypeGuard[ApolloDataType]:
    """Indicate whether the given input is a Apollo dtype, or dtype specialization."""
...
def unpack_dtypes(
    ...
    *dtypes
    ...
    Examples
    --------
    >>> from apollo.datatypes import unpack_dtypes
    ...
    unpacked: set[ApolloDataType] = set()
    for tp in dtypes:
        ...
        elif tp is not None and is_apollo_dtype(tp):
            unpacked.add(tp)
    return unpacked
...
class _DataTypeMappings:
    ...
    def DTYPE_TO_FFINAME(self) -> dict[ApolloDataType, str]:
        return {
            Binary: "binary",
            Boolean: "bool",
    ...
    @property
    @functools.lru_cache  # noqa: B019
    def DTYPE_TO_PY_TYPE(self) -> dict[ApolloDataType, PythonDataType]:
        return {
            Array: list,
            Binary: bytes,
            Boolean: bool,
    ...
    @property
    @functools.lru_cache  # noqa: B019
    def NUMPY_KIND_AND_ITEMSIZE_TO_DTYPE(self) -> dict[tuple[str, int], ApolloDataType]:
        return {
    ...
    @property
    @functools.lru_cache  # noqa: B019
    def REPR_TO_DTYPE(self) -> dict[str, ApolloDataType]:
        def _dtype_str_repr_safe(o: Any) -> ApolloDataType | None:
            try:
        ...
        return {
            ...
            if is_apollo_dtype(obj) and _dtype_str_repr_safe(obj) is not None
        }
...
def dtype_to_ffiname(dtype: ApolloDataType) -> str:
    """Return FFI function name associated with the given Apollo dtype."""
    try:
        dtype = dtype.base_type()
        return DataTypeMappings.DTYPE_TO_FFINAME[dtype]
    except KeyError:  # pragma: no cover
        msg = f"conversion of apollo data type {dtype!r} to FFI not implemented"
        raise NotImplementedError(msg) from None


def dtype_to_py_type(dtype: ApolloDataType) -> PythonDataType:
    """Convert a Apollo dtype to a Python dtype."""
    try:
        dtype = dtype.base_type()
        return DataTypeMappings.DTYPE_TO_PY_TYPE[dtype]
    except KeyError:  # pragma: no cover
        msg = f"conversion of apollo data type {dtype!r} to Python type not implemented"
        raise NotImplementedError(msg) from None
...
def dtype_short_repr_to_dtype(dtype_string: str | None) -> ApolloDataType | None:
    """Map a ApolloDataType short repr (eg: 'i64', 'list[str]') back into a dtype."""
...
def supported_numpy_char_code(dtype_char: str) -> bool:
    """Check if the input can be mapped to a Apollo dtype."""
...
def numpy_char_code_to_dtype(dtype_char: str) -> ApolloDataType:
    """Convert a numpy character dtype to a Apollo dtype."""
    dtype = np.dtype(dtype_char)
    if dtype.kind == "U":
        return String
    elif dtype.kind == "S":
    ...
    try:
        ...
        msg = f"cannot parse numpy data type {dtype!r} into Apollo data type"
        raise ValueError(msg) from None


def maybe_cast(el: Any, dtype: ApolloDataType) -> Any:
    """Try casting a value to a value that is valid for the given Apollo dtype."""
