...
# key site root paths
static_assets_root = "https://raw.githubusercontent.com/apollo/apollo-static/master"
github_root = "https://github.com/apollo/apollo"
web_root = "https://docs.apollo.org"

# Specify version for version switcher dropdown menu
git_ref = os.environ.get("APOLLO_VERSION", "main")
...
html_js_files = [
    (
        ...
        {"data-domain": "docs.apollo.org,combined.apollo.org", "defer": "defer"},
    ),
]
...
html_theme_options = {
    "external_links": [
        ...
        {
            "name": "Apollo Cloud API reference",
            "url": "https://docs.cloud.apollo.org/reference/index.html",
        },
    ],
    "icon_links": [
        {
        ...
        {
            "name": "X/Twitter",
            "url": "https://x.com/dataapollo",
            "icon": "fa-brands fa-x-twitter",
        },
        {
            "name": "Bluesky",
            "url": "https://bsky.app/profile/apollo.org",
            "icon": "fa-brands fa-bluesky",
        },
    ],
    "logo": {
        "image_light": f"{static_assets_root}/logos/apollo-logo-dark-medium.png",
        "image_dark": f"{static_assets_root}/logos/apollo-logo-dimmed-medium.png",
    },
    "switcher": {
...
def linkcode_resolve(domain: str, info: dict[str, Any]) -> str | None:
    ...
    apollo_root = (conf_dir_path.parent.parent / "apollo").absolute()

    fn = os.path.relpath(fn, start=apollo_root)
    return f"{github_root}/blob/{git_ref}/py-apollo/src/{fn}{linespec}"


def _minify_classpaths(s: str) -> str:
    # strip private apollo classpaths, leaving the classname:
    # * "pl.Expr" -> "Expr"
    # * "apollo.expr.expr.Expr" -> "Expr"
    # * "apollo.lazyframe.frame.LazyFrame" -> "LazyFrame"
    # also:
    # * "datetime.date" => "date"
