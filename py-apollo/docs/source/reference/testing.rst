...
.. note::
    ...
    the primary ``apollo`` module. Either import ``apollo.testing`` and *then* use that
...
Apollo provides some standard asserts for use with unit tests:
...
Apollo provides the following `hypothesis <https://hypothesis.readthedocs.io>`_
testing strategies:
...
Running apollo' own parametric unit tests on ``0.17.6`` against release
...
.. code-block:: python
    ...
    from apollo import NUMERIC_DTYPES
...
.. code-block:: python
    ...
    import apollo as pl
    from apollo.testing.parametric import column, dataframes, lists
