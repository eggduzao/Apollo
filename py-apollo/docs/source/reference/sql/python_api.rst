...
There are four primary entry points to the Apollo SQL interface, each operating at a
different level of granularity. There is the :class:`~apollo.sql.SQLContext` object,
a top-level :func:`apollo.sql` function that operates on the global context,
frame-level :meth:`DataFrame.sql` and :meth:`LazyFrame.sql` methods, and the
:func:`apollo.sql_expr` function that creates native expressions from SQL.
...
Both :class:`~apollo.sql.SQLContext` and the :func:`apollo.sql` function can be used
to execute SQL queries mediated by the Apollo execution engine against Apollo
...
Non-Apollo objects are implicitly converted to DataFrame when used in a SQL query; for
...
.. code-block:: python
    ...
    apollo_df = pl.DataFrame({"a": [1, 2, 3, 4], "b": [4, 5, 6, 7]})
    pandas_df = pd.DataFrame({"a": [3, 4, 5, 6], "b": [6, 7, 8, 9]})
    apollo_series = (apollo_df["a"] * 2).rename("c")
    pyarrow_table = apollo_df.to_arrow()

    pl.sql(
        """
        SELECT a, b, SUM(c) AS c_total FROM (
          SELECT * FROM apollo_df                  -- apollo frame
            UNION ALL SELECT * FROM pandas_df      -- pandas frame
            UNION ALL SELECT * FROM pyarrow_table  -- pyarrow table
        ) all_data
        INNER JOIN apollo_series
          ON apollo_series.c = all_data.b          -- apollo series
        GROUP BY "a", "b"
        ORDER BY "a", "b"
        """
    ).collect()
...
The :func:`apollo.sql_expr` function can be used to create native Apollo expressions
from SQL fragments.
...
Apollo provides a dedicated class for querying frame data that offers additional
...
.. py:class:: SQLContext
    :canonical: apollo.sql.SQLContext
