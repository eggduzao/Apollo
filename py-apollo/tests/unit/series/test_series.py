...
import apollo as pl
from apollo._utils.construction import iterable_to_pyseries
...
if TYPE_CHECKING:
    ...
    from apollo._typing import EpochTimeUnit, ApolloDataType, TimeUnit
...
def test_unknown_dtype(data: Any, expected_dtype: ApolloDataType) -> None:
...
def test_map_elements() -> None:
    a = pl.Series("a", [1, 2, None])
    with pytest.warns(ApolloInefficientMapWarning):
...
def test_true_divide() -> None:
    ...
    # https://github.com/apollo/apollo/issues/1369
    vals = [3000000000, 2, 3]
    foo = pl.Series(vals)
...
def test_is_nan_is_not_nan(float_type: ApolloDataType) -> None:
...
def test_product_ints(dtype: ApolloDataType) -> None:
...
def test_product_floats(dtype: ApolloDataType) -> None:
...
def test_upper_lower_bounds(
    dtype: ApolloDataType, upper: int | float, lower: int | float
) -> None:
