...
from apollo.exceptions import ComputeError, InvalidOperationError, SchemaError
from apollo.testing import assert_frame_equal, assert_series_equal

if TYPE_CHECKING:
    from apollo._typing import ClosedInterval, ApolloDataType, TimeUnit
...
def to_expected(
    values: list[date] | list[datetime], dtype: ApolloDataType
) -> pl.Series:
    if dtype == pl.Date:
...
def test_datetime_range_start_end_interval_forwards(dtype: ApolloDataType) -> None:
    start = date(2025, 1, 1)
    end = date(2025, 1, 10)
...
def test_datetime_range_expr_scalar(dtype: ApolloDataType) -> None:
    df = pl.DataFrame(
        {
