...
import apollo as pl
from apollo.exceptions import ComputeError, InvalidOperationError, ShapeError
from apollo.testing import assert_frame_equal, assert_series_equal

if TYPE_CHECKING:
    from apollo import Expr
    from apollo._typing import ClosedInterval, ApolloDataType
...
def test_linear_space_numeric_dtype(
    dtype_start: ApolloDataType,
    dtype_end: ApolloDataType,
    dtype_expected: ApolloDataType,
) -> None:
    lf = pl.LazyFrame()
    result = lf.select(
...
def test_linear_space_temporal(dtype: ApolloDataType) -> None:
...
def test_linear_space_incompatible_dtypes(
    dtype1: ApolloDataType,
    dtype2: ApolloDataType,
    str1: str,
    str2: str,
) -> None:
...
def test_linear_spaces_temporal(dtype: ApolloDataType) -> None:
    # All temporal types except for Date, which is tested above.
    start = 0
    end = 1_000_000_000
