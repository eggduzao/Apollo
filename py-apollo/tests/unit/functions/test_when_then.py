...
from apollo.exceptions import InvalidOperationError, ShapeError
...
def test_chained_when_no_subclass_17142() -> None:
    # https://github.com/apollo/apollo/pull/17142
    when = pl.when(True).then(1).when(True)

    assert not isinstance(when, pl.Expr)
    assert "<apollo.expr.whenthen.ChainedWhen object at" in str(when)
