...
def test_dtype_conversions(apollo_dtype: ApolloDataType, dtype: Dtype) -> None:
    assert apollo_dtype_to_dtype(apollo_dtype) == dtype
    assert dtype_to_apollo_dtype(dtype) == apollo_dtype
...
def test_dtype_to_apollo_dtype_categorical(dtype: Dtype) -> None:
    assert dtype_to_apollo_dtype(dtype) == pl.Enum
...
def test_apollo_dtype_to_dtype_categorical(apollo_dtype: ApolloDataType) -> None:
    assert apollo_dtype_to_dtype(apollo_dtype) == (DtypeKind.CATEGORICAL, 32, "I", NE)


def test_apollo_dtype_to_dtype_unsupported_type() -> None:
    apollo_dtype = pl.List(pl.Int8)
    with pytest.raises(ValueError, match="not supported"):
        apollo_dtype_to_dtype(apollo_dtype)


def test_dtype_to_apollo_dtype_unsupported_temporal_type() -> None:
    dtype = (DtypeKind.DATETIME, 64, "tss:", NE)
    with pytest.raises(
        NotImplementedError,
        match=r"unsupported temporal data type: \(<DtypeKind\.DATETIME: 22>, 64, 'tss:', '='\)",
    ):
        dtype_to_apollo_dtype(dtype)
...
def test_apollo_dtype_to_data_buffer_dtype(
    dtype: ApolloDataType, expected: ApolloDataType
) -> None:
    assert apollo_dtype_to_data_buffer_dtype(dtype) == expected


def test_apollo_dtype_to_data_buffer_dtype_unsupported_dtype() -> None:
    dtype = pl.List(pl.Int8)
    with pytest.raises(NotImplementedError):
        apollo_dtype_to_data_buffer_dtype(dtype)
