...
load_profile(
    profile=os.environ.get("APOLLO_HYPOTHESIS_PROFILE", "fast"),  # type: ignore[arg-type]
)
...
def partition_limit() -> int:
    """The limit at which Apollo will start partitioning in debug builds."""
    return 15
...
def memory_usage_without_pyarrow() -> Generator[MemoryUsage, Any, Any]:
    ...
    if os.getenv("APOLLO_FORCE_ASYNC", "0") == "1":
