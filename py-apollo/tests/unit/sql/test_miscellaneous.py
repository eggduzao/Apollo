...
import apollo as pl
from apollo.exceptions import ColumnNotFoundError, SQLInterfaceError, SQLSyntaxError
from apollo.testing import assert_frame_equal
from tests.unit.utils.pycapsule_utils import PyCapsuleStreamHolder

if TYPE_CHECKING:
    from apollo.datatypes import DataType
...
def test_global_misc_lookup() -> None:
    ...
    from apollo import col  # noqa: F401
...
def test_sql_on_compatible_frame_types() -> None:
    ...
    # run apollo sql query against all frame types
    for dfs in (  # noqa: B007
        (df["a"] * 2).rename("c"),  # apollo series
        (dfp["a"] * 2).rename("c"),  # pandas series
    ):
        res = pl.sql(
            """
            SELECT a, b, SUM(c) AS cc FROM (
              SELECT * FROM df               -- apollo frame
            ...
            INNER JOIN dfs ON dfs.c == tbl.b -- join on pandas/apollo series
            GROUP BY "a", "b"
            ORDER BY "a", "b"
            """
        ).collect()
    ...
    # register and operate on non-apollo frames
    for obj in (dfa, dfp):
