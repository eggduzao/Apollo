...
_APOLLO_TO_SQLITE_: dict[ApolloDataType, str] = {
...
def _execute_with_sqlite(
    ...
    with contextlib.closing(sqlite3.connect(":memory:")) as conn:
        ...
        for name, df in frames.items():
            ...
            types = (_APOLLO_TO_SQLITE_[frame_schema[col]] for col in df.columns)
            schema = ", ".join(
...
def assert_sql_matches(
    ...
    *,
    ...
    """
    Assert that a Apollo SQL query produces the same result as a reference backend.

    This function executes the provided SQL query using both Apollo and a reference
    ...
    check_row_order
        Set False to ignore the row order in the Apollo/comparison frame match.
    check_column_names
        Set False to ignore the column names in the Apollo/comparison frame match
    ...
    result values when executed with Apollo and executed with SQLite:
    ...
    with pl.SQLContext(frames=frames, eager=True) as ctx:
        apollo_result = ctx.execute(query=query, eager=True)
    ...
    for comparison_backend in compare_with:
        ...
        if not check_column_names:
            ...
            comparison_result.columns = apollo_result.columns[:n_comparison_cols]
