...
# https://github.com/apollo/apollo/issues/15139
@pytest.mark.parametrize("dtype", [pl.List(pl.Int64), None])
def test_sequence_of_series_with_dtype(dtype: ApolloDataType | None) -> None:
    values = [1, 2, 3]
...
def test_upcast_primitive_and_strings(
    values: list[Any], dtype: ApolloDataType, expected_dtype: ApolloDataType
) -> None:
...
# https://github.com/apollo/apollo/issues/15518
...
def test_temporal_dtype_string_values(
    data: list[str], dtype: ApolloDataType, expected_values: list[Any]
) -> None:
