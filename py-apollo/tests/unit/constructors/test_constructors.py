...
from apollo._dependencies import dataclasses, pydantic
from apollo._utils.construction.utils import try_get_type_hints
from apollo.datatypes import numpy_char_code_to_dtype
from apollo.exceptions import DuplicateError, ShapeError
...
def test_init_dict() -> None:
    ...
    for dates, datetimes, coldefs in (
        # test inferred and explicit (given both py/apollo dtypes)
...
def test_init_structured_objects_nested(foo: Any, bar: Any, baz: Any) -> None:
    ...
    override_struct_schema: dict[str, ApolloDataType] = {
        "x": pl.Int16,
        "y": pl.Struct(
            [
                pl.Field("a", pl.String),
...
def test_init_py_dtype(dtype: Any, expected_dtype: ApolloDataType) -> None:
    for s in (
...
def test_from_dicts_schema() -> None:
    ...
    # let apollo infer the dtypes, but inform it about a 3rd column.
...
def test_from_records_nullable_structs() -> None:
    ...
    schema: list[tuple[str, ApolloDataType]] = [
        ("id", pl.UInt16),
        (
            "items",
            pl.List(
    ...
    schema_options: list[list[tuple[str, ApolloDataType]] | None] = [schema, None]
    ...
    dtype: ApolloDataType = dict_schema["items"]
...
def test_arrow_to_pyseries_with_one_chunk_does_not_copy_data() -> None:
    from apollo._utils.construction import arrow_to_pyseries
...
def test_numpy_inference(
    input: Any,
    infer_func: Callable[[Any], ApolloDataType],
    expected_dtype: ApolloDataType,
) -> None:
