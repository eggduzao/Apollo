...
def test_fallback_with_dtype_strict(
    dtype: ApolloDataType, values: list[Any], strict: bool
) -> None:
    result = wrap_s(
...
def test_fallback_with_dtype_strict_failure(
    dtype: ApolloDataType, values: list[Any]
) -> None:
...
def test_fallback_with_dtype_nonstrict(
    dtype: ApolloDataType, values: list[Any], expected: list[Any]
) -> None:
    result = wrap_s(
...
def test_fallback_without_dtype(
    expected_dtype: ApolloDataType, values: list[Any], strict: bool
) -> None:
...
def test_fallback_without_dtype_large_int() -> None:
    ...
    with pytest.raises(
        OverflowError,
        match="int value too large for Apollo integer types",
    ):
        PySeries.new_from_any_values("", values, strict=True)
