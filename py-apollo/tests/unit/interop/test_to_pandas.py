...
@pytest.mark.parametrize("apollo_dtype", [pl.Categorical, pl.Enum(["a", "b"])])
def test_series_to_pandas_categorical(apollo_dtype: ApolloDataType) -> None:
    s = pl.Series("x", ["a", "b", "a"], dtype=apollo_dtype)
    result = s.to_pandas()
...
@pytest.mark.parametrize("apollo_dtype", [pl.Categorical, pl.Enum(["a", "b"])])
def test_series_to_pandas_categorical_pyarrow(apollo_dtype: ApolloDataType) -> None:
    s = pl.Series("x", ["a", "b", "a"], dtype=apollo_dtype)
