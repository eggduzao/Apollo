...
def test_arrow_list_roundtrip() -> None:
    # https://github.com/apollo/apollo/issues/1064
...
def test_arrow_dict_to_apollo() -> None:
...
# Test that apollo convert Arrays of logical types correctly to arrow
def test_arrow_array_logical() -> None:
    # cast to large string and uint8 indices because apollo converts to those
    pa_data1 = (
        pa.array(["a", "b", "c", "d"])
        .dictionary_encode()
...
# https://github.com/apollo/apollo/issues/15195
...
def test_from_optional_not_available() -> None:
    from apollo._dependencies import _LazyModule

    # proxy module is created dynamically if the required module is not available
    # (see the apollo._dependencies source code for additional detail/comments)
...
def test_upcast_pyarrow_dicts() -> None:
    # https://github.com/apollo/apollo/issues/1752
    tbls = [
        pa.table(
            {
                "col_name": pa.array(
...
def test_arrow_list_null_5697() -> None:
    ...
    # again to apollo to test the schema
...
def test_compat_level(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("APOLLO_WARN_UNSTABLE", "1")
...
def test_month_day_nano_from_ffi_15969(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("APOLLO_IMPORT_INTERVAL_AS_STRUCT", "1")
