...
# https://github.com/apollo/apollo/issues/6770
...
def test_ufunc_chain() -> None:
    ...
    result = df.rolling(index_column="counter", period="2i").agg(
        ...
        (pl.col("A")).log().mean().alias("mean_apollo"),
    )
    assert_series_equal(result["mean_numpy"], result["mean_apollo"].alias("mean_numpy"))
