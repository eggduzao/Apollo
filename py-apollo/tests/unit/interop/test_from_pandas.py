...
def test_from_pandas_nested_list() -> None:
    # this panicked in https://github.com/apollo/apollo/issues/1615
    pddf = pd.DataFrame(
...
def test_from_empty_pandas() -> None:
    ...
    apollo_df = pl.from_pandas(pandas_df)
    assert apollo_df.columns == ["A", "fruits"]
    assert apollo_df.dtypes == [pl.Float64, pl.Float64]
...
def _set_pyarrow_unavailable(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "apollo._utils.construction.dataframe._PYARROW_AVAILABLE", False
    )
    monkeypatch.setattr("apollo._utils.construction.series._PYARROW_AVAILABLE", False)
...
def test_from_pandas_nan_to_null_16453(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "apollo._utils.construction.dataframe._MIN_NUMPY_SIZE_FOR_MULTITHREADING", 2
    )
    df = pd.DataFrame(
...
def test_from_pandas_string_with_natype_17355(null: Any) -> None:
    # https://github.com/apollo/apollo/issues/17355
