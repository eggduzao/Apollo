...
from apollo.datatypes import DTYPE_TEMPORAL_UNITS
from apollo.exceptions import (
    ComputeError,
    InvalidOperationError,
    ApolloInefficientMapWarning,
)
from apollo.testing import (
    assert_frame_equal,
    assert_series_equal,
...
def test_timezone() -> None:
    ...
    # https://github.com/apollo/apollo/issues/5023
...
def test_date_comp(one: ApolloTemporalType, two: ApolloTemporalType) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # TODO: apollo currently doesn't retain FixedOffset timezone. They will be
...
def test_convert_pandas_timezone_info_fixed_offset(ts: pd.Timestamp) -> None:
    # TODO: apollo currently doesn't retain FixedOffset timezone. They will
...
def test_temporal_dtypes_map_elements(
    ...
    with pytest.warns(
        ...
        result = df.with_columns(
            ...
            .alias("const_dtm"),
            # note: the below now trigger a ApolloInefficientMapWarning
            pl.col("timestamp")
            .map_elements(
...
def test_temporal_to_string_iso_default() -> None:
    ...
    df_stringified = df.select(
        pl.col("td").dt.to_string("apollo").alias("td_pl"),
...
def test_temporal_to_string_error() -> None:
    ...
    with pytest.raises(
        InvalidOperationError,
        match="'apollo' is not a valid `to_string` format for date dtype expressions",
    ):
        df.select(cs.temporal().dt.to_string("apollo"))
...
def test_timezone_ignore_error(
    ...
    with pytest.raises(
        ...
        match=(
            ...
            r".*\n\n.*\n\n.*APOLLO_IGNORE_TIMEZONE_PARSE_ERROR"
        ),
    ):
    ...
    with monkeypatch.context() as cx:
        cx.setenv("APOLLO_IGNORE_TIMEZONE_PARSE_ERROR", "1")
        capfd.readouterr()
