...
from apollo.exceptions import DuplicateError, InvalidOperationError
from apollo.testing import assert_frame_equal, assert_series_equal

if TYPE_CHECKING:
    from collections.abc import Callable

    from apollo._typing import ApolloDataType, SerializationFormat
...
def test_struct_cols() -> None:
    ...
    def build_struct_df(data: list[dict[str, object]]) -> pl.DataFrame:
        """
        Build Apollo df from list of dicts.

        Can't import directly because of issue #3145.
        """
        arrow_df = pa.Table.from_pylist(data)
        apollo_df = pl.from_arrow(arrow_df)
        assert isinstance(apollo_df, pl.DataFrame)
        return apollo_df
...
def test_empty_series_nested_dtype(dtype: ApolloDataType) -> None:
