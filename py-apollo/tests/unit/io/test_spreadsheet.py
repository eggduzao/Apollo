...
def test_excel_read_named_table_with_total_row(tmp_path: Path) -> None:
    ...
    df.write_excel(
        wb_path,
        worksheet="data",
        table_name="ApolloFrameTable",
        column_totals=True,
    )
    for engine in ("calamine", "openpyxl"):
        col_subset = ["x", "z"]
        subset_kwarg = {"columns": col_subset}
        base_kwargs = {"table_name": "ApolloFrameTable", "engine": engine}
    ...
    with pytest.raises(
        ...
        pl.read_excel(wb_path, table_name="ApolloFrameTable", engine="xlsx2csv")
