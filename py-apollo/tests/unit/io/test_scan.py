...
from apollo.testing.asserts.frame import assert_frame_equal
from tests.unit.io.conftest import format_file_uri, normalize_path_separator_pl

if TYPE_CHECKING:
    from collections.abc import Callable

    from apollo._typing import SchemaDict
...
def _enable_force_async(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("APOLLO_FORCE_ASYNC", "1")
...
def session_tmp_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    return tmp_path_factory.mktemp("apollo-test")
...
def test_predicate_hive_pruning_with_cast(tmp_path: Path) -> None:
    ...
    # Write an invalid parquet file that will cause errors if apollo attempts to
    # read it.
...
def test_predicate_stats_eval_nested_binary() -> None:
    ...
    assert_frame_equal(
        (
            ...
            # * crates/apollo-expr/src/expressions/mod.rs::PhysicalExpr::evaluate_inline
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                reason="has no allow_missing_columns parameter. https://github.com/apollo/apollo/issues/21166"
            ),
        ),
        pytest.param(
            (pl.DataFrame.write_csv, pl.scan_csv),
...
@pytest.mark.write_disk
@pytest.mark.parametrize("apollo_force_async", ["0", "1"])
@pytest.mark.parametrize("n_repeats", [1, 2])
def test_scan_path_expansion_sorting_24528(
    tmp_path: Path,
    apollo_force_async: str,
    n_repeats: int,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("APOLLO_FORCE_ASYNC", apollo_force_async)
