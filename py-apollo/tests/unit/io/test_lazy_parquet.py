...
import apollo as pl
from apollo.exceptions import ComputeError, SchemaError
from apollo.testing import assert_frame_equal

if TYPE_CHECKING:
    from apollo._typing import ParallelStrategy
...
def test_scan_deadlock_rayon_spawn_from_async_15172(
    ...
    monkeypatch.setenv("APOLLO_FORCE_ASYNC", "1")
    monkeypatch.setenv("APOLLO_MAX_THREADS", "1")
    path = tmp_path / "data.parquet"
...
os.environ["APOLLO_MAX_THREADS"] = "2"
...
os.environ["APOLLO_MAX_THREADS"] = "1"
os.environ["APOLLO_VERBOSE"] = "1"
...
def test_scan_parquet_filter_index_panic_23849(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("APOLLO_PARQUET_DECODE_TARGET_VALUES_PER_THREAD", "5")
    num_rows = 3
    num_cols = 5
...
def test_sink_large_rows_25834(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("APOLLO_IDEAL_SINK_MORSEL_SIZE_BYTES", "1")
