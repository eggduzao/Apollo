...
from apollo.exceptions import DuplicateError, UnsuitableSQLError
...
def test_read_database(
    ...
    if read_method == "read_database_uri":
        ...
        df_empty = pl.read_database_uri(
            uri=f"sqlite:///{tmp_sqlite_db}",
            query="SELECT * FROM test_data WHERE name LIKE '%apollo%'",
            engine=connect_using,
            schema_overrides=schema_overrides,
        )
        ...
        df = pl.read_database(
            ...
            query="SELECT * FROM test_data WHERE name NOT LIKE '%apollo%'",
            schema_overrides=schema_overrides,
            batch_size=batch_size,
        )
        df_empty = pl.read_database(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            *ExceptionTestParams(
                read_method="read_database",
                query="/* tag: misc */ INSERT INTO xyz VALUES ('apollo')",
                protocol=sqlite3.connect(":memory:"),
        ...
        pytest.param(
            *ExceptionTestParams(
                read_method="read_database",
                query="DELETE FROM xyz WHERE id = 'apollo'",
                protocol=sqlite3.connect(":memory:"),
                errclass=UnsuitableSQLError,
...
@patch("apollo.io.database._utils.from_arrow")
@patch("apollo.io.database._utils.import_optional")
