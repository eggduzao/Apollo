...
import apollo as pl
from apollo.exceptions import ComputeError, SchemaFieldNotFoundError
...
def test_hive_partitioned_predicate_pushdown_single_threaded_async_17155(
    ...
    monkeypatch.setenv("APOLLO_PREFETCH_SIZE", "1")
...
def test_hive_partition_schema_inference(tmp_path: Path) -> None:
    ...
    paths = [
        ...
        tmp_path / "a=apollo/data.bin",
    ]

    expected = [
        pl.Series("a", [1], dtype=pl.Int64),
        pl.Series("a", [1.0, 1.5], dtype=pl.Float64),
        pl.Series("a", ["1", "1.5", "apollo"], dtype=pl.String),
    ]
...
def test_hive_file_as_uri_with_hive_start_idx_23830(
    ...
    # https://github.com/apollo/apollo/issues/24506
...
def test_hive_parquet_prefiltered_20894_21327(
    ...
    out = subprocess.check_output(
        [
            ...
            f"""\
import os
os.environ["APOLLO_MAX_THREADS"] = "{n_threads}"
