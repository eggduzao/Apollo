...
from apollo.io.iceberg._dataset import IcebergDataset, _NativeIcebergScanData
from apollo.io.iceberg._utils import (
    _convert_predicate,
...
@pytest.mark.write_disk
def test_scan_iceberg_apollo_storage_options_keys(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capfd: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setenv("APOLLO_VERBOSE_SENSITIVE", "1")
    catalog = SqlCatalog(
        "default",
        uri="sqlite:///:memory:",
        warehouse=format_file_uri_iceberg(tmp_path),
...
def test_scan_iceberg_nested_column_cast_deletion_rename(tmp_path: Path) -> None:
    ...
    # The Iceberg table schema has a `Map` type, whereas the apollo DataFrame
...
* Issue on Apollo repo - https://github.com/apollo/apollo/issues/23715
...
def test_scan_iceberg_parquet_prefilter_with_column_mapping(
    ...
    with monkeypatch.context() as cx:
        ...
        cx.setenv("APOLLO_FORCE_EMPTY_READER_CAPABILITIES", "0")
        capfd.readouterr()
        out = q.collect()
        capture = capfd.readouterr().err
...
def test_scan_iceberg_idxsize_limit() -> None:
    ...
    with pytest.raises(
        ...
        match=r"row count \(4294967296\) exceeded maximum supported of 4294967295.*Consider installing 'apollo\[rt64\]'.",
    ):
