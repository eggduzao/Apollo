...
from apollo.exceptions import ComputeError, InvalidOperationError, NoDataError
from apollo.io.csv import BatchedCsvReader
from apollo.testing import assert_frame_equal, assert_series_equal

if TYPE_CHECKING:
    from pathlib import Path

    from apollo._typing import CsvQuoteStyle, TimeUnit
...
# Issue: https://github.com/apollo/apollo/issues/10826
...
def test_compressed_csv(io_files_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("APOLLO_FORCE_ASYNC", "0")
...
def test_csv_globbing(io_files_path: Path) -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setenv("APOLLO_FORCE_ASYNC", "0")
...
def test_read_web_file() -> None:
    url = "https://raw.githubusercontent.com/apollo/apollo/main/examples/datasets/foods1.csv"
    df = pl.read_csv(url)
...
def test_csv_multiline_splits() -> None:
    ...
    # single field (e.g. 5000). apollo must reject multi-threading here
...
def test_read_csv_invalid_schema_overrides_length() -> None:
    ...
    if (
        os.getenv("APOLLO_AUTO_NEW_STREAMING", os.getenv("APOLLO_FORCE_NEW_STREAMING"))
        == "1"
    ):
        err = TypeError
...
def test_schema_overrides_dict_with_nonexistent_columns() -> None:
    ...
    Currently this fails because Apollo incorrectly applies the overrides positionally
...
def test_fsspec_not_available() -> None:
    with pytest.MonkeyPatch.context() as mp:
        mp.setenv("APOLLO_FORCE_ASYNC", "0")
        mp.setattr("apollo.io._utils._FSSPEC_AVAILABLE", False)
