...
from apollo._utils.unstable import issue_unstable_warning
from apollo.config import _APOLLO_CFG_ENV_VARS
...
def test_config_load_save(tmp_path: Path) -> None:
    for file in (
        None,
        tmp_path / "apollo.config",
        str(tmp_path / "apollo.config"),
    ):
        ...
        assert os.environ.get("APOLLO_VERBOSE") == "1"
        ...
        assert "APOLLO_VERBOSE" in pl.Config.state(if_set=True)
        ...
        assert os.environ.get("APOLLO_VERBOSE") == "0"
        ...
        assert os.environ.get("APOLLO_FMT_MAX_COLS") == "12"
        assert os.environ.get("APOLLO_VERBOSE") == "1"
        ...
        for e in ("APOLLO_FMT_MAX_COLS", "APOLLO_VERBOSE"):
            assert e not in pl.Config.state(if_set=True)
            assert e in pl.Config.state()

        assert os.environ.get("APOLLO_FMT_MAX_COLS") is None
        assert os.environ.get("APOLLO_VERBOSE") is None
...
def test_config_load_save_context() -> None:
    ...
    with pl.Config.load(default_state):
        assert os.environ.get("APOLLO_FMT_TABLE_FORMATTING") is None
        assert os.environ.get("APOLLO_VERBOSE") is None

    # ensure earlier state was restored
    assert os.environ["APOLLO_FMT_TABLE_FORMATTING"] == "ASCII_MARKDOWN"
    assert os.environ["APOLLO_VERBOSE"]
...
def test_config_instances() -> None:
    ...
    @cfg_markdown
    def fn1() -> str | None:
        return os.environ.get("APOLLO_FMT_TABLE_FORMATTING")

    assert fn1() == "MARKDOWN"
    assert os.environ.get("APOLLO_FMT_TABLE_FORMATTING") is None

    with cfg_markdown:  # can re-use instance as decorator and context
        assert os.environ.get("APOLLO_FMT_TABLE_FORMATTING") == "MARKDOWN"
    assert os.environ.get("APOLLO_FMT_TABLE_FORMATTING") is None
...
def test_config_scope() -> None:
    ...
    with pl.Config() as cfg:
        ...
        new_state_entries = set(
            {
                "APOLLO_FMT_MAX_COLS": "8",
                "APOLLO_FMT_TABLE_FORMATTING": "ASCII_FULL_CONDENSED",
                "APOLLO_FMT_TABLE_HIDE_COLUMN_SEPARATOR": "1",
                "APOLLO_FMT_TABLE_ROUNDED_CORNERS": "1",
                "APOLLO_VERBOSE": "1",
            }.items()
        )
...
def test_set_streaming_chunk_size() -> None:
    with pl.Config() as cfg:
        ...
        assert os.environ.get("APOLLO_STREAMING_CHUNK_SIZE") == "8"
...
@pytest.mark.parametrize(
    ...
    [
        ("APOLLO_ENGINE_AFFINITY", "set_engine_affinity", "gpu", "gpu"),
        ("APOLLO_FMT_MAX_COLS", "set_tbl_cols", 12, "12"),
        ("APOLLO_FMT_MAX_ROWS", "set_tbl_rows", 3, "3"),
        ("APOLLO_FMT_STR_LEN", "set_fmt_str_lengths", 42, "42"),
        ("APOLLO_FMT_TABLE_CELL_ALIGNMENT", "set_tbl_cell_alignment", "RIGHT", "RIGHT"),
        (
        ...
        ("APOLLO_FMT_TABLE_HIDE_COLUMN_NAMES", "set_tbl_hide_column_names", True, "1"),
        (
        ...
        ("APOLLO_STREAMING_CHUNK_SIZE", "set_streaming_chunk_size", 100, "100"),
        ("APOLLO_TABLE_WIDTH", "set_tbl_width_chars", 80, "80"),
        ("APOLLO_VERBOSE", "set_verbose", True, "1"),
        ("APOLLO_WARN_UNSTABLE", "warn_unstable", True, "1"),
    ],
)
def test_unset_config_env_vars(
    environment_variable: str, config_setting: str, value: Any, expected: str
) -> None:
    assert environment_variable in _APOLLO_CFG_ENV_VARS
