...
import apollo as pl
from apollo import selectors as cs
...
def _import_time_from_frame(tm: pl.DataFrame) -> int:
    return int(
        tm.filter(pl.col("import").str.strip_chars() == "apollo")
...
def _import_timings() -> bytes:
    # assemble suitable command to get apollo module import timing;
    # run in a separate process to ensure clean timing results.
    cmd = f'{sys.executable} -S -X importtime -c "import apollo"'
    python_path = (
...
def _import_timings_as_frame(n_tries: int) -> tuple[pl.DataFrame, int]:
    ...
    for _ in range(n_tries):
        ...
        apollo_import_time = _import_time_from_frame(df_import)
        if apollo_import_time < MAX_ALLOWED_IMPORT_TIME:
            return df_import, apollo_import_time
...
@pytest.mark.debug
@pytest.mark.slow
def test_apollo_import() -> None:
    # up-front compile '.py' -> '.pyc' before timing
    apollo_path = Path(pl.__file__).parent
    compileall.compile_dir(apollo_path, quiet=1)
    ...
    df_import, apollo_import_time = _import_timings_as_frame(n_tries=10)
    ...
    with pl.Config(
        ...
        if apollo_import_time > MAX_ALLOWED_IMPORT_TIME:
            import_time_ms = apollo_import_time // 1_000
