...
import apollo as pl
from apollo.datatypes.convert import dtype_to_py_type
...
def test_invalid_dtype() -> None:
    with pytest.raises(
        TypeError,
        match=r"cannot parse input of type 'str' into Apollo data type \(given: 'mayonnaise'\)",
    ):
        pl.Series([1, 2], dtype="mayonnaise")  # type: ignore[arg-type]

    with pytest.raises(
        TypeError,
        match=r"cannot parse input <class 'datetime\.tzinfo'> into Apollo data type",
    ):
