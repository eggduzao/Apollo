...
if TYPE_CHECKING:
    ...
    from apollo._typing import JoinStrategy, ApolloDataType
...
def test_join_lazy_frame_on_expression() -> None:
    ...
    # https://github.com/apollo/apollo/issues/19822
...
def test_joins_dispatch() -> None:
    ...
    # https://github.com/apollo/apollo/issues/1688
    dfa = pl.DataFrame(
        {
            "a": ["a", "b", "c", "a"],
            "b": [1, 2, 3, 1],
...
def test_join_chunks_alignment_4720() -> None:
    # https://github.com/apollo/apollo/issues/4720
    ...
    long_unique = pl.DataFrame(
        {
            "id": [1, 2, 3, 4, 5],
            "id_str": ["1", "2", "3", "4", "5"],
            "name": ["hello", "world", "rust", "apollo", "meow"],
        }
    )
...
# https://github.com/apollo/apollo/issues/10663
...
def test_cross_join() -> None:
    ...
    # https://github.com/apollo/apollo/blob/5f5acb2a523ce01bc710768b396762b8e69a9e07/apollo/apollo-core/src/frame/cross_join.rs#L34
...
def test_join_non_elementwise_keys_raises(left_on: pl.Expr, right_on: pl.Expr) -> None:
    # https://github.com/apollo/apollo/issues/17184
...
def test_join_on_nested(dtype: ApolloDataType, data: list[Any]) -> None:
    lhs = pl.DataFrame(
        {
            "a": data[:3],
            "b": [1, 2, 3],
...
def test_join_where_literals(dtype: ApolloDataType) -> None:
...
def test_join_filter_pushdown_inner_join() -> None:
    ...
    assert (
        extract[0]
        == 'LEFT PLAN ON: [col("a").cast(Int64), col("_APOLLO_0").cast(Int64)]'
    )
    assert '(col("a")) == (1)' in extract[1]
    assert '(col("b")) >= (1)' in extract[1]
    assert (
        extract[2]
        == 'RIGHT PLAN ON: [col("_APOLLO_1").cast(Int64), col("b").cast(Int64)]'
    )
    ...
    # Duplicate filter removal - https://github.com/apollo/apollo/issues/23243
    q = (
...
def test_join_filter_pushdown_semi_join() -> None:
    ...
    assert extract[2:] == [
        'RIGHT PLAN ON: [col("b"), col("_APOLLO_0").cast(Int64)]',
        'FILTER [(col("b")) == (2)]',
    ]
...
def test_join_filter_pushdown_iejoin() -> None:
    ...
    # Join filter pushdown inside CSE - https://github.com/apollo/apollo/issues/23489
