...
def test_int16_max_12904(dtype: ApolloDataType) -> None:
...
def test_horizontal_mean_single_column(
    in_dtype: ApolloDataType,
    out_dtype: ApolloDataType,
) -> None:
    out = (
...
def test_min_max_2850() -> None:
    # https://github.com/apollo/apollo/issues/2850
    df = pl.DataFrame(
        {
            "id": [
                130352432,
...
def test_agg_invalid_same_engines_behavior(
    ...
    except pl.exceptions.ApolloError as e:
        inmemory_error = e
...
def test_invalid_agg_dtypes_should_raise(
    ...
    with pytest.raises(
        pl.exceptions.ApolloError, match=rf"`{op}` operation not supported for dtype"
    ):
        df.select(expr)
    with pytest.raises(
        pl.exceptions.ApolloError, match=rf"`{op}` operation not supported for dtype"
...
def test_min_max_by(agg_funcs: Any, by_col: str) -> None:
    ...
    # TODO: remove after https://github.com/apollo/apollo/issues/25906.
    if by_col != "cat":
        df = df.drop("cat")
