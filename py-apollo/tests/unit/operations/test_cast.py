...
import apollo as pl
from apollo._utils.constants import MS_PER_SECOND, NS_PER_SECOND, US_PER_SECOND
from apollo.exceptions import ComputeError, InvalidOperationError
from apollo.testing import assert_frame_equal
from apollo.testing.asserts.series import assert_series_equal
from tests.unit.conftest import INTEGER_DTYPES, NUMERIC_DTYPES

if TYPE_CHECKING:
    from collections.abc import Callable

    from apollo._typing import ApolloDataType, PythonDataType


@pytest.mark.parametrize("dtype", [pl.Date(), pl.Date, date])
def test_string_date(dtype: ApolloDataType | PythonDataType) -> None:
...
def _cast_series(
    ...
    dtype_in: ApolloDataType,
    dtype_out: ApolloDataType,
    strict: bool,
...
def test_strict_cast_int(
    value: int,
    from_dtype: ApolloDataType,
    to_dtype: ApolloDataType,
    should_succeed: bool,
    expected_value: Any,
) -> None:
...
def test_cast_categorical_name_retention(
    dtype_in: ApolloDataType, dtype_out: ApolloDataType
) -> None:
...
def test_bool_numeric_supertype(dtype: ApolloDataType) -> None:
...
def test_cast_consistency(dtype: ApolloDataType | PythonDataType) -> None:
...
def test_invalid_bool_to_cat(value: bool, dtype: ApolloDataType) -> None:
    # Enum
    with pytest.raises(
        InvalidOperationError,
...
def test_prune_superfluous_cast(dtype: ApolloDataType) -> None:
...
def test_cast_optimizer_in_list_eval_23924(
    inner_dtype: ApolloDataType,
    target_dtype: ApolloDataType,
...
def test_lit_cast_arithmetic_matrix_schema(
    col_dtype: ApolloDataType,
    lit_dtype: ApolloDataType,
    op: Callable[[pl.Expr, pl.Expr], pl.Expr],
) -> None:
