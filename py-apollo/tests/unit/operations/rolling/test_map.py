...
import apollo as pl
from apollo.testing import assert_series_equal
...
def test_rolling_map_std(dtype: ApolloDataType) -> None:
...
@pytest.mark.parametrize("dtype", [pl.Float32, pl.Float64])
def test_rolling_map_std_weights(dtype: ApolloDataType) -> None:
    s = pl.Series("A", [1.0, 2.0, 9.0, 2.0, 13.0], dtype=dtype)
...
def test_rolling_map_sum_int(dtype: ApolloDataType) -> None:
...
@pytest.mark.parametrize("dtype", INTEGER_DTYPES)
def test_rolling_map_sum_int_cast_to_float(dtype: ApolloDataType) -> None:
...
def test_rolling_map_sum_float(dtype: ApolloDataType) -> None:
    s = pl.Series("A", [1, 2, 9, 2, 13], dtype=dtype)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_rolling_map_sum_float_weights(dtype: ApolloDataType) -> None:
    s = pl.Series("A", [1, 2, 9, 2, 13], dtype=dtype)
