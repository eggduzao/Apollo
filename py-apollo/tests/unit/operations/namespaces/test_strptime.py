...
import apollo as pl
from apollo.exceptions import ChronoFormatWarning, ComputeError, InvalidOperationError
from apollo.testing import assert_frame_equal, assert_series_equal

if TYPE_CHECKING:
    from apollo._typing import ApolloTemporalType, TimeUnit
...
def test_strptime_abbrev_month(
    time_string: str, fmt: str, datatype: ApolloTemporalType, expected: date
) -> None:
...
def test_single_digit_month(
    datatype: ApolloTemporalType, expected: datetime | date
) -> None:
...
def test_apollo_parser_fooled_by_trailing_nonsense_22167() -> None:
    with pytest.raises(InvalidOperationError):
