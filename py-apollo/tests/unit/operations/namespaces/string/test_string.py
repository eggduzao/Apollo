...
def test_str_strip_chars_expr() -> None:
    ...
    expected = pl.DataFrame(
        {
            "strip_chars": ["hello", "world", "hi", "apollo", None],
            "strip_chars_start": ["hello ", "world^^", "hi&&", "apollo  ", None],
            "strip_chars_end": [" hello", "^^world", "&&hi", "  apollo", None],
        }
    )
...
def test_extract_regex() -> None:
    s = pl.Series(
        [
            "http://vote.com/ballon_dor?candidate=messi&ref=apollo",
            "http://vote.com/ballon_dor?candidat=jorginho&ref=apollo",
