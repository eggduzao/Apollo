...
if TYPE_CHECKING:
    ...
    from apollo._typing import MapElementsStrategy, ApolloDataType
...
def test_cum_agg(dtype: ApolloDataType) -> None:
...
def test_from_epoch(input_dtype: ApolloDataType) -> None:
    ldf = pl.LazyFrame(
        [
    ...
    expected = pl.DataFrame(
        [
            ...
            pl.Series("timestamp_s", [exp_dt]),  # s is no Apollo dtype, defaults to us
            pl.Series("timestamp_ms", [exp_dt]).cast(pl.Datetime("ms")),
            pl.Series("timestamp_us", [exp_dt]),  # us is Apollo Datetime default
...
def test_compare_aggregation_between_lazy_and_eager_6904(
    dtype: ApolloDataType, func: pl.Expr
) -> None:
    df = pl.DataFrame(
        {
