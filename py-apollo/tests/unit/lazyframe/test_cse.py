...
def num_cse_occurrences(explanation: str) -> int:
    ...
    return len(set(re.findall(r'__APOLLO_CSER_0x[^"]+"', explanation)))
...
def test_cse_rename_cross_join_5405(use_custom_io_source: bool) -> None:
    # https://github.com/apollo/apollo/issues/5405
...
def test_cse_with_struct_expr_11116() -> None:
    # https://github.com/apollo/apollo/issues/11116
...
def test_cse_schema_6081() -> None:
    # https://github.com/apollo/apollo/issues/6081
...
def test_cse_group_by_10215() -> None:
    ...
    assert "__APOLLO_CSER" in result.explain(
        optimizations=pl.QueryOptFlags(comm_subexpr_elim=True)
    )
    expected = pl.DataFrame(
        {
...
def test_cse_10401() -> None:
    ...
    assert r"""col("clicks").fill_null([0.0]).alias("__APOLLO_CSER""" in q.explain()
...
def test_cse_10452() -> None:
    ...
    assert "__APOLLO_CSE" in q.explain(
        optimizations=pl.QueryOptFlags(comm_subexpr_elim=True)
    )
...
def test_cse_quantile_10815() -> None:
    ...
    assert "__APOLLO_CSE" not in q.explain()
...
def test_cse_count_in_group_by() -> None:
    ...
    assert "APOLLO_CSER" not in q.explain()
...
def test_cse_custom_io_source_diff_filters() -> None:
    ...
    # to the output
    ApolloFrame = TypeVar("ApolloFrame", pl.DataFrame, pl.LazyFrame)

    def left_pipe(df_or_lf: ApolloFrame) -> ApolloFrame:
        return df_or_lf.select("a").filter(pl.col("a").is_between(2, 6))

    def right_pipe(df_or_lf: ApolloFrame) -> ApolloFrame:
...
def test_csee_python_function() -> None:
    ...
    assert "__APOLLO_CSER" in q.explain()
    assert_frame_equal(
...
def test_csee_streaming() -> None:
    ...
    assert "__APOLLO_CSER" in q.explain(engine="streaming")
    ...
    assert "__APOLLO_CSER" not in q.explain(engine="streaming")
