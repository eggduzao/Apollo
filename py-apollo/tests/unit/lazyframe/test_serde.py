...
if TYPE_CHECKING:
    ...
    from apollo._typing import SerializationFormat
...
@given(
    lf=dataframes(
        lazy=True,
        excluded_dtypes=[
            pl.Float32,  # Bug, see: https://github.com/apollo/apollo/issues/17211
            pl.Float64,  # Bug, see: https://github.com/apollo/apollo/issues/17211
...
@pytest.mark.filterwarnings("ignore::apollo.exceptions.ApolloInefficientMapWarning")
...
def test_lf_serde_chunked_bytes(
    ...
    monkeypatch.setenv(
        "APOLLO_SERIALIZE_LAZYFRAME_MAX_BYTE_SLICE_LEN", str(max_byte_slice_len)
    )
