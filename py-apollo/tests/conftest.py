...
def _patched_cloud(
    ...
    if request.config.getoption("--cloud-distributed"):
        ...
        from apollo_cloud import ClusterContext, DirectQuery, set_compute_context
        ...
        for ext in ["parquet", "csv", "ipc", "ndjson"]:
            monkeypatch.setattr(f"apollo.scan_{ext}", create_cloud_scan(ext))
            monkeypatch.setattr(f"apollo.read_{ext}", create_read(ext))
            monkeypatch.setattr(
                f"apollo.LazyFrame.sink_{ext}",
                create_cloud_sink(ext, BASE_UNSUPPORTED),
            )

        monkeypatch.setattr("apollo.LazyFrame.collect", cloud_collect)
        monkeypatch.setenv("APOLLO_SKIP_CLIENT_CHECK", "1")
